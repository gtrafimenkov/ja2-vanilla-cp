...
enum CreatureMobility { CREATURE_MOBILE = 0, CREATURE_CRAWLER, CREATURE_IMMOBILE };
...
void CreatureCall(SOLDIERTYPE *pCaller) {
  ...
  if (pCaller->bHunting)  // which should only be set for females outside of the
                          // hive
  {
  ...
  FOR_EACH_IN_TEAM(pReceiver, pCaller->bTeam) {
    if (pReceiver->bInSector && pReceiver->bLife >= OKLIFE && pReceiver != pCaller &&
      ...
      if (bPriority > pReceiver->bCallPriority) {
        ...
        if ((bPriority > FRENZY_THRESHOLD) &&
            (pReceiver->ubBodyType == ADULTFEMALEMONSTER || pReceiver->ubBodyType == YAF_MONSTER)) {
          // go berzerk!
...
static int8_t CreatureDecideActionGreen(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->bMobility != CREATURE_IMMOBILE) {
    ...
    if (pSoldier->bMobility == CREATURE_MOBILE)
    {
            ...
            // reduce chance for any injury, less likely to wander around when
    hurt iChance -= (pSoldier->bLifeMax - pSoldier->bLife);
...
static int8_t CreatureDecideActionBlack(SOLDIERTYPE *pSoldier) {
  ...
  if (CAN_CALL(pSoldier)) {
    if ((pSoldier->bActionPoints >= AP_RADIO) &&
      ...
      if (pSoldier->bLife < pSoldier->bOldLife) {
        // got injured, maybe call
        /*
        // don't call when crippled and have target... save breath for
        attacking! if ((pSoldier->bOldLife == pSoldier->bLifeMax) &&
        (pSoldier->bOldLife - pSoldier->bLife > 10))
        {
                // hurt for first time!
                pSoldier->usActionData = CALL_CRIPPLED;
                pSoldier->bOldLife = pSoldier->bLife;  // don't want to call
        more than once return(AI_ACTION_CREATURE_CALL);
        }
        else
        */
  ...
  if (bCanAttack) {
        ...
        if (pSoldier->inv[bWeaponIn].ubGunShotsLeft > 0) {
          ...
          if (pSoldier->bActionPoints >= ubMinAPCost) {
            ...
            if (BestShot.ubPossible) {
              ...
              // !serviced) (usually, this means all the guys we see our
              // unconscious, but, on
              //  rare occasions, we may not be able to shoot a healthy guy,
              //  too)
              const SOLDIERTYPE *const opp = BestShot.opponent;
              if (opp->bLife < OKLIFE) {
                // if our attitude is NOT aggressive
                if (pSoldier->bAttitude != AGGRESSIVE) {
                  // get the location of the closest CONSCIOUS reachable
                  // opponent
                  ...
                  if (sClosestDisturbance != NOWHERE) {
                    ...
                    // conscious targets then make decision as if at alert
                    // status RED, but make sure we don't try to SEEK OPPONENT
                    // the unconscious guy!
              ...
              // now we KNOW FOR SURE that we will do something (shoot, at
              // least)
              NPCDoesAct(pSoldier);
            }
          }
    ...
    // if attack is still desirable (meaning it's also preferred to taking
    // cover)
...
void CreatureDecideAlertStatus(SOLDIERTYPE *pSoldier) {
  ...
  // GREEN - No one sensed, no suspicious noise heard, go about doing regular
  // stuff YELLOW - Suspicious noise was heard personally RED - Either saw
  // OPPONENTS in person, or definite contact had been called BLACK - Currently
  // has one or more OPPONENTS in sight
