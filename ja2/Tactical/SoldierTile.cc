...
static void SetFinalTile(SOLDIERTYPE *pSoldier, int16_t sGridNo, BOOLEAN fGivenUp) {
...
static void MarkMovementReserved(SOLDIERTYPE &s, int16_t const sGridNo) {
...
static int8_t TileIsClear(SOLDIERTYPE *pSoldier, int8_t bDirection, int16_t sGridNo,
                          int8_t bLevel) {
                                        ...
                                        // if ( 0 )
      {
        ...
        if (tgt->sFinalDestination == tgt->sGridNo) {
          ...
          if (tgt->fBlockedByAnotherMerc &&
            ...
            if (PlotPath(pSoldier, pSoldier->sFinalDestination, NO_COPYROUTE, NO_PLOT,
              ...
              if (!(pSoldier->uiStatusFlags & SOLDIER_MULTITILE)) {
                // If we are to swap and we're near a door, open door first and
                // then close it...?
                ...
                // With these two guys swapped, they should try and continue on
                // their way.... Start them both again along their way...
                EVENT_GetNewSoldierPath(pSoldier, pSoldier->sFinalDestination,
                                        pSoldier->usUIMovementMode);
...
BOOLEAN HandleNextTile(SOLDIERTYPE *pSoldier, int8_t bDirection, int16_t sGridNo,
  ...
  /// if ( ( gTacticalStatus.uiFlags & REALTIME ) || !( gTacticalStatus.uiFlags
  /// & INCOMBAT ) )
...

  {
    ...
    if (bBlocked != MOVE_TILE_CLEAR) {
      ...
      if (sGridNo == sFinalDestTile && pSoldier->ubWaitActionToDo == 0 &&
          (pSoldier->bTeam == OUR_TEAM || pSoldier->sAbsoluteFinalDestination == NOWHERE)) {
      ...
      if (bOverTerrainType == LOW_WATER || bOverTerrainType == MED_WATER ||
        ...
        // Check if we are of prone or crawl height and change stance
        // accordingly....
...
void HandleNextTileWaiting(SOLDIERTYPE *const pSoldier) {
  ...
  if (pSoldier->fDelayedMovement) {
    if (TIMECOUNTERDONE(pSoldier->NextTileCounter, NEXT_TILE_CHECK_DELAY)) {
      ...
      const uint8_t bCauseDirection =
          GetDirectionToGridNoFromGridNo(pSoldier->sGridNo, pSoldier->sDelayedMovementCauseGridNo);
      ...
      if (bBlocked != MOVE_TILE_TEMP_BLOCKED) {
        ...
        // Now, if we are in the state where we are desparently trying to get
        // out... Use other flag CJC: path-through-people includes ignoring
        // person at dest
        /*
        ...
        */
        ...
        if (pSoldier->sAbsoluteFinalDestination == pSoldier->sFinalDestination) {
          ...
          if (pSoldier->fDelayedMovement >= 150) {
            ...
            // OK, look around dest for the first one!
            sCheckGridNo =
                FindGridNoFromSweetSpotThroughPeople(pSoldier, pSoldier->sFinalDestination, 6);

            if (sCheckGridNo == NOWHERE) {
              // If this is nowhere, try harder!
              sCheckGridNo =
                  FindGridNoFromSweetSpotThroughPeople(pSoldier, pSoldier->sFinalDestination, 16);
            }
          }
        } else {
        ...
        if (pSoldier->fDelayedMovement == 3 &&
          ...
          // if either on a mission from god, or two AI guys not on
          // stationary...
          SOLDIERTYPE *const tgt =
              WhoIsThere2(pSoldier->sDelayedMovementCauseGridNo, pSoldier->bLevel);
          if (tgt != NULL) {
            if (pSoldier->ubQuoteRecord != 0 ||
                (pSoldier->bTeam != OUR_TEAM && pSoldier->bOrders != STATIONARY &&
                 tgt->bTeam != OUR_TEAM && tgt->bOrders != STATIONARY) ||
                (pSoldier->bTeam == OUR_TEAM && gTacticalStatus.fAutoBandageMode &&
                 (tgt->bTeam != CIV_TEAM || tgt->bOrders != STATIONARY))) {
              // Swap now!
              // tgt->fBlockedByAnotherMerc = FALSE;
              ...
              // With these two guys swapped, we should try to continue on our
              // way....
              pSoldier->fDelayedMovement = FALSE;

              // We must calculate the path here so that we can give it the
              // "through people" parameter
              ...
              EVENT_GetNewSoldierPath(pSoldier, pSoldier->sAbsoluteFinalDestination,
                                      pSoldier->usUIMovementMode);
              // EVENT_GetNewSoldierPath(tgt, tgt->sFinalDestination,
              // tgt->usUIMovementMode);
            }
          }
        }
...
void SwapMercPositions(SOLDIERTYPE &s1, SOLDIERTYPE &s2) {
  ...
  if (NewOKDestination(&s1, gridno2, TRUE, 0) &&
      NewOKDestination(&s2, gridno1, TRUE,
                       0)) {  // Call teleport function for each
...
BOOLEAN CanExchangePlaces(SOLDIERTYPE *pSoldier1, SOLDIERTYPE *pSoldier2, BOOLEAN fShow) {
  ...
  if (EnoughPoints(pSoldier1, AP_EXCHANGE_PLACES, 0, fShow)) {
    if (EnoughPoints(pSoldier2, AP_EXCHANGE_PLACES, 0, fShow)) {
      ...
      // must NOT be hostile, must NOT have stationary orders OR militia team,
      // must be >= OKLIFE
      ...
      if (fShow) {
        if (pSoldier2->ubProfile == NO_PROFILE) {
          ...
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, gzLateLocalizedString[STR_LATE_03],
                    pSoldier2->name);
        }
      }
    }
