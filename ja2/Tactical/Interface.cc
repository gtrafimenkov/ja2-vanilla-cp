...
static int32_t giMenuAnchorX;
static int32_t giMenuAnchorY;

static const SGPBox g_progress_bar_box = {5, 2, 630, 12};

static BOOLEAN gfProgBarActive = FALSE;
static uint8_t gubProgNumEnemies = 0;
static uint8_t gubProgCurEnemy = 0;
...
static uint16_t gusUIMessageWidth;
static uint16_t gusUIMessageHeight;
...
static uint16_t gsUpArrowX;
static uint16_t gsUpArrowY;
static uint16_t gsDownArrowX;
static uint16_t gsDownArrowY;
...
void SetCurrentTacticalPanelCurrentMerc(SOLDIERTYPE *s) {
  ...
  if (gsCurInterfacePanel == SM_PANEL) {
    // If we are not of merc bodytype, or am an epc, and going into inv, goto
    // another....
...
static void BtnMovementCallback(GUI_BUTTON *btn, int32_t reason);

static void MakeButtonMove(uint32_t const idx, uint32_t const gfx, int16_t const x, int16_t const y,
                           UI_EVENT *const event, wchar_t const *const help, bool const disabled) {
  GUIButtonRef const btn =
      QuickCreateButton(iIconImages[gfx], x, y, MSYS_PRIORITY_HIGHEST - 1, BtnMovementCallback);
...
static void MovementMenuBackregionCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
void PopupMovementMenu(UI_EVENT *const ev) {
  ...
  MSYS_DefineRegion(&gMenuOverlayRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                    MSYS_PRIORITY_HIGHEST - 1, CURSOR_NORMAL, MSYS_NO_CALLBACK,
                    MovementMenuBackregionCallback);
  ...
  int32_t const x = giMenuAnchorX + 9;
  int32_t const y = giMenuAnchorY + 8;
  ...
  wchar_t const *const help =
      is_vehicle ? TacticalStr[DRIVE_POPUPTEXT] : pTacticalPopupButtonStrings[WALK_ICON];
  MakeButtonMove(WALK_ICON, WALK_IMAGES, x + 40, y, ev, help, is_uncontrolled_robot);

  uint32_t action_image;
  ...
  if (is_vehicle) {
    ...
    disable_action = true;
  } else {  // Create button based on what is in our hands at the moment!
    uint16_t const item = s->inv[HANDPOS].usItem;
    if (item == TOOLKIT) {
  ...
  MakeButtonMove(CANCEL_ICON, CANCEL_IMAGES, x + 20, y + 20, ev,
                 pTacticalPopupButtonStrings[CANCEL_ICON], false);
  MakeButtonMove(SNEAK_ICON, SNEAK_IMAGES, x + 40, y + 20, ev,
                 pTacticalPopupButtonStrings[SNEAK_ICON], !IsValidStance(s, ANIM_CROUCH));
  ...
  MakeButtonMove(HAND_ICON, HAND_IMAGES, x + 20, y + 40, ev, pTacticalPopupButtonStrings[HAND_ICON],
                 is_epc || is_vehicle);
  MakeButtonMove(CRAWL_ICON, CRAWL_IMAGES, x + 40, y + 40, ev,
                 pTacticalPopupButtonStrings[CRAWL_ICON], !IsValidStance(s, ANIM_PRONE));
...
void RenderArrows() {
  ...
  if (guiShowUPDownArrows & ARROWS_SHOW_UP_BESIDE) {
    ...
    BltVideoObject(FRAME_BUFFER, TileElem.hTileSurface, TileElem.usRegionIndex, gsUpArrowX,
                   gsUpArrowY);
  }
  ...
  if (guiShowUPDownArrows & ARROWS_SHOW_DOWN_BESIDE) {
    ...
    BltVideoObject(FRAME_BUFFER, TileElem.hTileSurface, TileElem.usRegionIndex, gsDownArrowX,
                   gsDownArrowY);
  }
...
static void GetArrowsBackground() {
  ...
  giUpArrowRect = RegisterBackgroundRect(BGND_FLAG_PERMANENT, gsUpArrowX, gsUpArrowY, sArrowWidth,
                                         sArrowHeight);
}

void GetSoldierAboveGuyPositions(SOLDIERTYPE const *const s, int16_t *const psX, int16_t *const psY,
                                 BOOLEAN const fRadio) {
  int16_t const sTextBodyTypeYOffset = 62;

  int16_t sStanceOffset;
  ...
  switch (s->ubBodyType) {
    ...
    default: {
      ...
      uint8_t const ubAnimUseHeight =
          ac.uiFlags & ANIM_NOMOVE_MARKER ? ac.ubHeight : ac.ubEndHeight;
  ...
  *psY = sMercScreenY - sTextBodyTypeYOffset + sStanceOffset;
  ...
  if (s->ubProfile != NO_PROFILE && !fRadio &&
      (s->fDisplayDamage || *psY < gsVIEWPORT_WINDOW_START_Y)) {
    *psY = sMercScreenY;
  }
}

static void PrintAboveGuy(int16_t const x, int16_t const y, wchar_t const *const text) {
  int16_t cx;
  int16_t cy;
...
static void DrawBarsInUIBox(const SOLDIERTYPE *pSoldier, int16_t sXPos, int16_t sYPos,
                            int16_t sWidth, int16_t sHeight);
...
void DrawSelectedUIAboveGuy(SOLDIERTYPE &s) {
  ...
  if (s.fFlashLocator) {
    if (s.bVisible == -1) {
      ...
      if (TIMECOUNTERDONE(s.BlinkSelCounter, 80)) {
        ...
        s.fShowLocator = TRUE;
        if (++s.sLocatorFrame == 5) {  // Update time we do this
          ++s.fFlashLocator;
          s.sLocatorFrame = 0;
        }
      }
      ...
      SGPVObject const *const gfx = s.bNeutral || s.bSide == OUR_TEAM ? guiRADIO : guiRADIO2;
  ...
  if (s.ubProfile != NO_PROFILE || s.uiStatusFlags & SOLDIER_VEHICLE) {
    ...
    if (&s == gUIValidCatcher && gfUIMouseOnValidCatcher == 1) {
      ...
      action = buf;
    } else if (s.bTeam == OUR_TEAM && s.bAssignment < ON_DUTY && s.bAssignment != CurrentSquad() &&
    ...
    if (s.ubProfile < FIRST_RPC || RPC_RECRUITED(&s) || AM_AN_EPC(&s) ||
...
static void DrawBarsInUIBox(const SOLDIERTYPE *pSoldier, int16_t sXPos, int16_t sYPos,
  ...
  SetClippingRegionAndImageWidth(l.Pitch(), 0, gsVIEWPORT_WINDOW_START_Y, SCREEN_WIDTH,
  ...
  RectangleDraw(TRUE, sXPos + 1, sYPos + 7, (int32_t)(sXPos + dWidth + 1), sYPos +
  7, usLineColor, pDestBuf);
  */
}
...
static void BtnDoorMenuCallback(GUI_BUTTON *btn, int32_t reason);

static void MakeButtonDoor(uint32_t idx, uint32_t gfx, int16_t x, int16_t y, int16_t ap, int16_t bp,
                           BOOLEAN disable, const wchar_t *help) {
  GUIButtonRef const btn =
      QuickCreateButton(iIconImages[gfx], x, y, MSYS_PRIORITY_HIGHEST - 1, BtnDoorMenuCallback);
  iActionIcons[idx] = btn;
...
static void DoorMenuBackregionCallback(MOUSE_REGION *pRegion, int32_t iReason);

static void PopupDoorOpenMenu(BOOLEAN fClosingDoor) {
  int32_t dx = gOpenDoorMenu.sX;
  int32_t dy = gOpenDoorMenu.sY;
  ...
  MSYS_DefineRegion(&gMenuOverlayRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                    MSYS_PRIORITY_HIGHEST - 1, CURSOR_NORMAL, MSYS_NO_CALLBACK,
                    DoorMenuBackregionCallback);

  const BOOLEAN d0 = fClosingDoor || AM_AN_EPC(gOpenDoorMenu.pSoldier);
  BOOLEAN d;

  d = d0 || !SoldierHasKey(*gOpenDoorMenu.pSoldier, ANYKEY);
  MakeButtonDoor(USE_KEYRING_ICON, USE_KEYRING_IMAGES, dx + 20, dy, AP_UNLOCK_DOOR, BP_UNLOCK_DOOR,
                 d, pTacticalPopupButtonStrings[USE_KEYRING_ICON]);

  d = fClosingDoor || FindUsableObj(gOpenDoorMenu.pSoldier, CROWBAR) == NO_SLOT;
  MakeButtonDoor(USE_CROWBAR_ICON, CROWBAR_DOOR_IMAGES, dx + 40, dy, AP_USE_CROWBAR, BP_USE_CROWBAR,
                 d, pTacticalPopupButtonStrings[USE_CROWBAR_ICON]);

  d = d0 || FindObj(gOpenDoorMenu.pSoldier, LOCKSMITHKIT) == NO_SLOT;
  MakeButtonDoor(LOCKPICK_DOOR_ICON, LOCKPICK_DOOR_IMAGES, dx + 40, dy + 20, AP_PICKLOCK,
                 BP_PICKLOCK, d, pTacticalPopupButtonStrings[LOCKPICK_DOOR_ICON]);

  d = d0 || FindObj(gOpenDoorMenu.pSoldier, SHAPED_CHARGE) == NO_SLOT;
  MakeButtonDoor(EXPLOSIVE_DOOR_ICON, EXPLOSIVE_DOOR_IMAGES, dx + 40, dy + 40, AP_EXPLODE_DOOR,
                 BP_EXPLODE_DOOR, d, pTacticalPopupButtonStrings[EXPLOSIVE_DOOR_ICON]);

  const wchar_t *const help =
      pTacticalPopupButtonStrings[fClosingDoor ? CANCEL_ICON + 1 : OPEN_DOOR_ICON];
  MakeButtonDoor(OPEN_DOOR_ICON, OPEN_DOOR_IMAGES, dx, dy, AP_OPEN_DOOR, BP_OPEN_DOOR, FALSE, help);

  MakeButtonDoor(EXAMINE_DOOR_ICON, EXAMINE_DOOR_IMAGES, dx, dy + 20, AP_EXAMINE_DOOR,
                 BP_EXAMINE_DOOR, d0, pTacticalPopupButtonStrings[EXAMINE_DOOR_ICON]);
  ...
  MakeButtonDoor(UNTRAP_DOOR_ICON, UNTRAP_DOOR_ICON, dx + 20, dy + 40, AP_UNTRAP_DOOR,
                 BP_UNTRAP_DOOR, d0, pTacticalPopupButtonStrings[UNTRAP_DOOR_ICON]);
...
static void DoorAction(int16_t const ap, int16_t const bp, HandleDoor const action) {
...
static uint32_t CalcUIMessageDuration(const wchar_t *wString);
...
void BeginUIMessage(BOOLEAN fUseSkullIcon, const wchar_t *text) {
  ...
  MercPopupBoxFlags const flags =
      fUseSkullIcon ? MERC_POPUP_PREPARE_FLAGS_SKULLICON : MERC_POPUP_PREPARE_FLAGS_STOPICON;

  // Prepare text box
  g_ui_message_box =
      PrepareMercPopupBox(g_ui_message_box, BASIC_MERC_POPUP_BACKGROUND, BASIC_MERC_POPUP_BORDER,
                          text, 200, 10, 0, 0, &gusUIMessageWidth, &gusUIMessageHeight, flags);
  ...
  if (g_ui_message_overlay == NULL) {
    int16_t const x = (SCREEN_WIDTH - gusUIMessageWidth) / 2;
    g_ui_message_overlay =
        RegisterVideoOverlay(RenderUIMessage, x, 150, gusUIMessageWidth, gusUIMessageHeight);
  }
...
void BeginMapUIMessage(int16_t delta_y, const wchar_t *text) {
  ...
  g_ui_message_box = PrepareMercPopupBox(
      g_ui_message_box, BASIC_MERC_POPUP_BACKGROUND, BASIC_MERC_POPUP_BORDER, text, 200, 10, 0, 0,
      &gusUIMessageWidth, &gusUIMessageHeight, MERC_POPUP_PREPARE_FLAGS_TRANS_BACK);

  if (g_ui_message_overlay == NULL) {
    int16_t const x = MAP_VIEW_START_X + (MAP_VIEW_WIDTH - gusUIMessageWidth) / 2 + 20;
    int16_t const y = MAP_VIEW_START_Y + (MAP_VIEW_HEIGHT - gusUIMessageHeight) / 2 + delta_y;
    g_ui_message_overlay =
        RegisterVideoOverlay(RenderUIMessage, x, y, gusUIMessageWidth, gusUIMessageHeight);
  }
}
...
static void CreateTopMessage() {
  ...
  const SGPBox *const bar = &g_progress_bar_box;
  {
    ...
    if (fDoLimitBar) {
      ...
      uint32_t length = (bar->w - 3) * ts->usTactialTurnLimitCounter / ts->usTactialTurnLimitMax;
      while (length-- != 0) {
  ...
  switch (ts->ubTopMessageType) {
    case COMPUTER_TURN_MESSAGE: {
      const uint8_t team = ts->ubCurrentTeam;
...
void HandleTopMessages() {
  ...
  switch (ts->ubTopMessageType) {
    ...
    case PLAYER_TURN_MESSAGE:
    case PLAYER_INTERRUPT_MESSAGE:
      if (gGameOptions.fTurnTimeLimit && !gfUserTurnRegionActive && !AreWeInAUIMenu() &&
          GetJA2Clock() - ts->uiTactialTurnLimitClock >
              PLAYER_TEAM_TIMER_GRACE_PERIOD)  // Check Grace period...
      {
        ...
        if (TIMECOUNTERDONE(giTimerTeamTurnUpdate, PLAYER_TEAM_TIMER_SEC_PER_TICKS)) {
          ...
          if (ts->fTactialTurnLimitStartedBeep) {
            ...
            if (ts->usTactialTurnLimitMax - ts->usTactialTurnLimitCounter <
  ...
  if (gfTopMessageDirty || gTopMessage.sWorldRenderX != gsRenderCenterX ||
...
void UpdateEnemyUIBar() {
  ...
  if (gTacticalStatus.fInTopMessage && gTacticalStatus.ubTopMessageType == COMPUTER_TURN_MESSAGE) {
    // Update message!
    CreateTopMessage();
  }
}
...
const wchar_t *GetSoldierHealthString(const SOLDIERTYPE *const s) {
  int32_t i;
  const int32_t start = (s->bLife == s->bLifeMax ? 4 : 0);
  for (i = start; i < 6; ++i) {
...
void SetupPhysicsTrajectoryUI() {
  if (gfDisplayPhysicsUI && gfUIHandlePhysicsTrajectory) {
    const uint16_t idx = (gfBadPhysicsCTGT ? FIRSTPOINTERS12 : FIRSTPOINTERS8);
    LEVELNODE *n;
...
static uint32_t CalcUIMessageDuration(const wchar_t *wString) {
  // base + X per letter
  return (1000 + 50 * (uint32_t)wcslen(wString));
}

static BOOLEAN gfMultipurposeLocatorOn = FALSE;
static uint32_t guiMultiPurposeLocatorLastUpdate;
static int8_t gbMultiPurposeLocatorFrame;
static int16_t gsMultiPurposeLocatorGridNo;
static int8_t gbMultiPurposeLocatorLevel;
static int8_t gbMultiPurposeLocatorCycles;

void BeginMultiPurposeLocator(const int16_t sGridNo, const int8_t bLevel) {
