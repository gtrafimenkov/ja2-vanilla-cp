...
static uint32_t guiSaveTacticalStatusFlags;  // saves the tactical status flags
...
static uint16_t gusGameLightLevel = 0;
static uint16_t gusSavedLightLevel = 0;
...
static uint8_t gubOldCurScrollSpeedID;  // Saved for returning to previous settings
...
static uint16_t iCurBankMapIndex;
...
static int32_t iPrevDrawMode = DRAW_MODE_NOTHING;
static uint16_t PrevCurrentPaste = FIRSTTEXTURE;
static int32_t gPrevCurrentBackground = FIRSTTEXTURE;
...
static void EditModeInit() {
  ...
  // essentially, we are turning the game off so the game doesn't process in
...
static BOOLEAN EditModeShutdown() {
  ...
  if (gfNeedToInitGame) {
    InitStrategicLayer();
    WarpGameTime(1,
                 WARPTIME_PROCESS_EVENTS_NORMALLY);  // to avoid helicopter setup
...
static BOOLEAN DrawTempMouseCursorObject() {
  ...
  switch (iDrawMode) {
    case DRAW_MODE_ROOM:
      ...
      return FALSE;  // a special case where we just want to get the info and not
                     // display a cursor.
    case DRAW_MODE_NEWROOF:
...
void ShowCurrentDrawingMode() {
  ...
  switch (iShowMode) {
    ...
    case DRAW_MODE_SMART_WALLS: {
      const GridNo pos = GetMouseMapPos();
      if (pos == NOWHERE || !CalcWallInfoUsingSmartMethod(pos, &usObjIndex, &usUseIndex)) {
    ...
    case DRAW_MODE_SMART_DOORS: {
      const GridNo pos = GetMouseMapPos();
      if (pos == NOWHERE || !CalcDoorInfoUsingSmartMethod(pos, &usObjIndex, &usUseIndex)) {
    ...
    case DRAW_MODE_SMART_WINDOWS: {
      const GridNo pos = GetMouseMapPos();
      if (pos == NOWHERE || !CalcWindowInfoUsingSmartMethod(pos, &usObjIndex, &usUseIndex)) {
    ...
    case DRAW_MODE_SMART_BROKEN_WALLS: {
      const GridNo pos = GetMouseMapPos();
      if (pos == NOWHERE || !CalcBrokenWallInfoUsingSmartMethod(pos, &usObjIndex, &usUseIndex)) {
  ...
  if ((usUseIndex != 0xffff) && (usObjIndex != 0xffff)) {
    ...
    int32_t const iStartX = (w - e.usWidth) / 2 - e.sOffsetX;
    int32_t const iStartY = (h - e.usHeight) / 2 - e.sOffsetY;

    ts->CurrentShade(DEFAULT_SHADE_LEVEL);
    BltVideoObject(FRAME_BUFFER, ts, usUseIndex, x + iStartX, y + iStartY);
  }

  // Set the color for the window's border. Blueish color = Normal, Red = Fake
  // lighting is turned on
  uint16_t usFillColor = GetGenericButtonFillColor();

  {
    SGPVSurface::Lock l(FRAME_BUFFER);
    RectangleDraw(FALSE, x, y, x + w - 1, y + h, usFillColor, l.Buffer<uint16_t>());
  }
            ...
            if (gfBasement) {
              ...
              for (i = 0; i < WORLD_MAX; i++) {
                if (gubWorldRoomInfo[i]) {
                  ...
                  uint16_t usTileIndex = GetTileIndexFromTypeSubIndex(usRoofType, usRoofIndex);
  ...
  switch (iCurrentAction) {
    ...
    case ACTION_QUICK_ERASE:
      if (gViewportRegion.uiFlags & MSYS_MOUSE_IN_AREA) {
        const uint32_t pos = GetMouseMapPos();
...
static void CreateNewMap() {
  ...
  if (gfPendingBasement) {
    ...
    for (i = 0; i < WORLD_MAX; i++) {
      ...
      uint16_t usIndex = GetTileIndexFromTypeSubIndex(FIRSTROOF, 9 + Random(3));
    ...
    for (i = 0; i < WORLD_MAX; i++) {
      ...
      uint16_t usIndex = GetTileIndexFromTypeSubIndex(FIRSTWALL, 60 + Random(6));
      AddCave(i, usIndex);
    }
...
BOOLEAN PlaceLight(int16_t const radius, GridNo const pos) try {
  STRING512 filename;
  ...
  if (!l) {
    ...
    uint8_t const intensity = radius / LIGHT_DECAY;
    ...
    if (!l) {
      ...
      DebugMsg(TOPIC_GAME, DBG_LEVEL_1,
               String("PlaceLight: Can't create light sprite of radius %d", radius));
      return FALSE;
    }
  }
...
//	RemoveLight
//
//	Removes (erases) all lights at a given map tile location. Lights that
// are attached to a merc 	are not deleted.
//
...
BOOLEAN RemoveLight(const GridNo pos) {
   ...
   * Ex:  L-RO5.LHT */
  const uint16_t usRadius = pLastLightName[4] - 0x30;
...
void ShowLightPositionHandles() {
  CFOR_EACH_LIGHT_SPRITE(l) {
    ...
    int32_t const iMapIndex = (int32_t)l->iY * WORLD_COLS + (int32_t)l->iX;
...
//	CheckForSlantRoofs
//
//	Verifies whether there are any slant roofs selected in the "multiwall"
// or "room" selection list. 	if so, and no flat roofs were selected, then we
// return TRUE, otherwise we return FALSE.
//
//	This function is used to determine if we should force the area selection
...
//	Runs through all map locations, and if it's outside the visible world,
// then we remove 	EVERYTHING from it since it will never be seen!
//
//	If it can be seen, then we remove all extraneous land tiles. We find the
// tile that has the first 	FULL TILE indicator, and delete anything that may come
// after it (it'll never be seen anyway)
//
...
//	CheckForFences
//
//	Verifies whether the selection list has only fence pieces. If so, turns
// off random selection 	for that selection list.
//
...
static void HandleMouseClicksInGameScreen() {
  ...
  if (_LeftButtonDown) {
    ...
    if (iDrawMode >= DRAW_MODE_ERASE) {
      ...
      for (int16_t y = gSelectRegion.iTop; y <= gSelectRegion.iBottom; ++y) {
        for (int16_t x = gSelectRegion.iLeft; x <= gSelectRegion.iRight; ++x) {
    ...
    switch (iDrawMode) {
      ...
      case DRAW_MODE_WALLS:
        PasteSingleWall(map_idx);
        break;
      case DRAW_MODE_DOORS:
        PasteSingleDoor(map_idx);
        break;
      case DRAW_MODE_WINDOWS:
        PasteSingleWindow(map_idx);
        break;
      case DRAW_MODE_ROOFS:
        PasteSingleRoof(map_idx);
        break;
      case DRAW_MODE_BROKEN_WALLS:
        PasteSingleBrokenWall(map_idx);
        break;
      case DRAW_MODE_DECOR:
        PasteSingleDecoration(map_idx);
        break;
      ...
      case DRAW_MODE_TOILET:
        PasteSingleToilet(map_idx);
        break;
      case DRAW_MODE_SMART_WALLS:
        PasteSmartWall(map_idx);
        break;
      case DRAW_MODE_SMART_DOORS:
        PasteSmartDoor(map_idx);
        break;
      case DRAW_MODE_SMART_WINDOWS:
        PasteSmartWindow(map_idx);
        break;
      case DRAW_MODE_SMART_BROKEN_WALLS:
        PasteSmartBrokenWall(map_idx);
        break;
    ...
    switch (iDrawMode) {
      ...
      // Handle the right clicks in the main window to bring up the appropriate
      // selection window
      case DRAW_MODE_WALLS:
    ...
    switch (iDrawMode) {
      ...
      case DRAW_MODE_COPY_BUILDING:
        PasteBuilding(map_idx);
        break;
      case DRAW_MODE_MOVE_BUILDING:
        MoveBuilding(map_idx);
        break;
    }
  }
}
...
void ShowEntryPoints() {  // Make entry points visible
...
static void HideEntryPoints() {  // Remove entry point indicators
  MAPCREATE_STRUCT const &m = gMapInformation;
  if (m.sNorthGridNo != -1)
    RemoveAllTopmostsOfTypeRange(m.sNorthGridNo, FIRSTPOINTERS, FIRSTPOINTERS);
  if (m.sEastGridNo != -1)
    RemoveAllTopmostsOfTypeRange(m.sEastGridNo, FIRSTPOINTERS, FIRSTPOINTERS);
  if (m.sSouthGridNo != -1)
    RemoveAllTopmostsOfTypeRange(m.sSouthGridNo, FIRSTPOINTERS, FIRSTPOINTERS);
  if (m.sWestGridNo != -1)
    RemoveAllTopmostsOfTypeRange(m.sWestGridNo, FIRSTPOINTERS, FIRSTPOINTERS);
}
...
// For any drawing modes that support large cursors, or even area selection,
...
static void DrawObjectsBasedOnSelectionRegion() {
  ...
  // Certain drawing modes are placed with 100% density.  Those cases are
  // checked here, so the density test can be skipped.
  fSkipTest = FALSE;
  ...
  // to even out the binary tree undo placements.  Otherwise, the placements
  // within the undo binary tree would alway choose the right branch because the
  // imapindex is always greater than the previously positioned one. Process the
  // cursor area
...
ScreenID EditScreenHandle() {
  ...
  if (gfWorldLoaded && gMapInformation.ubMapVersion <= 7 && !gfCorruptMap) {
    ScreenMsg(FONT_MCOLOR_RED, MSG_ERROR,
              L"Map data has just been corrupted.  Don't save, don't quit, get Kris! "
              L" If he's not here, save the map using a temp filename and document "
              L"everything you just did, especially your last action!");
    gfCorruptMap = TRUE;
  }
  if (gfWorldLoaded && gubScheduleID > 40 && !gfCorruptSchedules) {
    ...
    if (gubScheduleID > 32) {
      ScreenMsg(FONT_MCOLOR_RED, MSG_ERROR,
                L"Schedule data has just been corrupted.  Don't save, don't "
                L"quit, get Kris!  If he's not here, save the map using a temp "
                L"filename and document everything you just did, especially "
                L"your last action!");
  ...
  // If we are copying or moving a building, we process, then delete the
  // building layout immediately after releasing the mouse button.  If released
  // in the world, then the building would have been processed in above function,
  // HandleMouseClicksInGameScreen().
...
static void CreateGotoGridNoUI() {
  ...
  guiGotoGridNoUIButtonID = CreateLabel(L"Enter gridno:", FONT10ARIAL, FONT_YELLOW, FONT_BLACK, 290,
                                        155, 60, 50, MSYS_PRIORITY_NORMAL);
  guiGotoGridNoUIButtonID->SpecifyTextOffsets(5, 5, FALSE);
  // Create a blanket region so nobody can use
  MSYS_DefineRegion(&GotoGridNoUIRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                    MSYS_PRIORITY_NORMAL + 1, 0, MSYS_NO_CALLBACK, MSYS_NO_CALLBACK);
