...
static void InternallyClassifyEdgepoints(SOLDIERTYPE *pSoldier, INT16 sGridNo, INT16 **psArray1,
                                         UINT16 *pusMiddleIndex1, UINT16 *pusArraySize1,
                                         INT16 **psArray2, UINT16 *pusMiddleIndex2,
                                         UINT16 *pusArraySize2) {
  INT32 i;
...
static void SaveMapEdgepoint(HWFILE const f, UINT16 const &n, UINT16 const &idx,
                             INT16 const *const array) {
...
bool LoadMapEdgepoints(HWFILE const f) {
  ...
  if (gMapInformation.ubMapVersion < 17) { /* To prevent invalidation of older maps, which only used
                                            * one layer of edgepoints, and a UINT8 for containing
...
void ChooseMapEdgepoints(MAPEDGEPOINTINFO *const pMapEdgepointInfo,
                         const UINT8 ubStrategicInsertionCode, UINT8 ubNumDesiredPoints) {
  AssertMsg(
