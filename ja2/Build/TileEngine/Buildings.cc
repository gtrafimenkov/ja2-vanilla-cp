...
static BUILDING *CreateNewBuilding(uint8_t *pubBuilding) {
...
static BUILDING *GenerateBuilding(int16_t sDesiredSpot) {
  uint32_t uiLoop;
  ...
  while (1) {
    ...
    if (gpWorldLevelData[sTempGridNo].uiFlags & MAPELEMENT_REACHABLE) {
      ...
      // we KNOW that the spot in the original direction is blocked, so only
      // loop 3 times
    ...
    if (!(gpWorldLevelData[sCurrGridNo].ubExtFlags[0] & MAPELEMENT_EXT_ROOFCODE_VISITED)) {
      ...
      switch (bDirection) {
        ...
        case EAST:
          ...
          break;
        case SOUTH:
          sWallGridNo = (int16_t)(sCurrGridNo + DirectionInc(TwoCDirection(bDirection)));
          bDesiredOrientation = OUTSIDE_TOP_RIGHT;
          break;
        case WEST:
          sWallGridNo = (int16_t)(sCurrGridNo + DirectionInc(TwoCDirection(bDirection)));
      ...
      if (fFoundWall) {
        ...
        if ((sPrevGridNo != NOWHERE) && (pBuilding->ubNumClimbSpots > 0)) {
          if (pBuilding->sDownClimbSpots[pBuilding->ubNumClimbSpots - 1] == sCurrGridNo) {
            ...
            pBuilding->ubNumClimbSpots--;
            // overwrote a selected spot so go into automatic selection for
            // later
            uiChanceIn = 1;
#ifdef ROOF_DEBUG
            // reset marker
...
BUILDING *FindBuilding(int16_t sGridNo) {
  ...
  if (ubBuildingID == NO_BUILDING) {
    ...
    // building
    if (GetRoom(sGridNo) != NO_ROOM && FindStructure(sGridNo,
    STRUCTURE_NORMAL_ROOF) != NULL)
    {
...
void GenerateBuildings() {
  ...
  for (uint32_t uiLoop = 0; uiLoop < WORLD_MAX; ++uiLoop) {
