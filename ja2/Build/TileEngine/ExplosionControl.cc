...
extern int8_t gbSAMGraphicList[NUMBER_OF_SAMS];

struct ExplosionInfo {
  const char *blast_anim;
  SoundID sound;
  uint8_t blast_speed;
  uint8_t transparent_key_frame;
  uint8_t damage_key_frame;
};
...
static int16_t gsTempActionGridNo = NOWHERE;
...
// GENERATE EXPLOSION
void InternalIgniteExplosion(SOLDIERTYPE *const owner, const int16_t sX, const int16_t sY,
                             const int16_t sZ, const int16_t sGridNo, const uint16_t usItem,
                             const BOOLEAN fLocate, const int8_t bLevel) {
...
void IgniteExplosion(SOLDIERTYPE *const owner, const int16_t z, const int16_t sGridNo,
                     const uint16_t item, const int8_t level) {
  int16_t x;
  int16_t y;
...
void IgniteExplosionXY(SOLDIERTYPE *const owner, const int16_t sX, const int16_t sY,
                       const int16_t sZ, const int16_t sGridNo, const uint16_t usItem,
                       const int8_t bLevel) {
...
static void GenerateExplosionFromExplosionPointer(EXPLOSIONTYPE *pExplosion) {
  ...
  // Assign param values
  const int16_t sX = pExplosion->sX;
  const int16_t sY = pExplosion->sY;
  int16_t sZ = pExplosion->sZ;
  const int16_t sGridNo = pExplosion->sGridNo;
  const int16_t bLevel = pExplosion->bLevel;
...
void UpdateExplosionFrame(EXPLOSIONTYPE *const e, const int16_t sCurrentFrame) {
...
static void HandleFencePartnerCheck(int16_t sStructGridNo) {
  ...
  if (pFenceStructure) {
    ...
    if (pFenceStructure->pDBStructureRef->pDBStructure->bDestructionPartner < 0) {
      ...
      // Get type from index...
      const uint32_t uiFenceType = GetTileType(pFenceNode->usIndex);

      bFenceDestructionPartner =
          -1 * (pFenceBaseStructure->pDBStructureRef->pDBStructure->bDestructionPartner);

      // Get new index
      uint16_t usTileIndex = GetTileIndexFromTypeSubIndex(uiFenceType, bFenceDestructionPartner);
...
static void ReplaceWall(GridNo const grid_no, uint8_t const orientation, int16_t const sub_idx) {
  ...
  uint16_t const new_idx =
      GetTileIndexFromTypeSubIndex(gTileDatabase[node->usIndex].fType, sub_idx);
  ApplyMapChangesToMapTempFile app;
...
static STRUCTURE *RemoveOnWall(GridNo const grid_no, StructureFlags const flags, STRUCTURE *next) {
  while (STRUCTURE *const attached = FindStructure(grid_no, flags)) {
    ...
    while (next && next->usStructureID == base->usStructureID) {  // The next structure will also be
                                                                  // deleted, so skip past it
      next = next->pNext;
    }
...
static bool ExplosiveDamageStructureAtGridNo(STRUCTURE *const pCurrent,
                                             STRUCTURE **const ppNextCurrent, int16_t const grid_no,
                                             int16_t const wound_amt, uint32_t const uiDist,
                                             BOOLEAN *const pfRecompileMovementCosts,
                                             BOOLEAN const only_walls, SOLDIERTYPE *const owner,
                                             int8_t const level) {
  ...
  if (pCurrent->fFlags & STRUCTURE_CORPSE && gGameSettings.fOptions[TOPTION_BLOOD_N_GORE] &&
    ...
    if (uiDist <= 1) {  // Remove corpse
      VaporizeCorpse(grid_no, pCurrent->usStructureID);
    }
  } else if (!(pCurrent->fFlags & STRUCTURE_PERSON)) {
    // Damage structure!
    int16_t const sX = CenterX(grid_no);
    int16_t const sY = CenterY(grid_no);
    int8_t bDamageReturnVal =
        DamageStructure(pCurrent, wound_amt, STRUCTURE_DAMAGE_EXPLOSION, grid_no, sX, sY, NULL);
    ...
    int8_t const orig_destruction_partner =
        base->pDBStructureRef->pDBStructure->bDestructionPartner;
    ...
    // Remove the beast!
    while (*ppNextCurrent && (*ppNextCurrent)->usStructureID ==
                                 pCurrent->usStructureID) {  // The next structure will also be
                                                             // deleted, so skip past it
    ...
    if (orig_destruction_partner > 0) {
      ...
      uint8_t const n_tiles = base->pDBStructureRef->pDBStructure->ubNumberOfTiles;
      DB_STRUCTURE_TILE *const *const ppTile = base->pDBStructureRef->ppTile;

      destruction_partner = orig_destruction_partner;

      // OK, destrcution index is , as default, the partner, until we go over
      // the first set of explsion debris...
      uint16_t const tile_idx =
          destruction_partner >= 40
      ...
      for (uint8_t ubLoop = BASE_TILE; ubLoop != n_tiles; ++ubLoop) {
    ...
    // OK, Check if this is a wall, then search and change other walls based on
    // this
    if (pCurrent->fFlags & STRUCTURE_WALLSTUFF) {
      /* ATE
      ...
      switch (uint8_t const orientation = pCurrent->ubWallOrientation) {
        case OUTSIDE_TOP_LEFT:
        case INSIDE_TOP_LEFT:
          ReplaceWall(NewGridNo(base_grid_no, DirectionInc(WEST)), orientation,
                      orientation == OUTSIDE_TOP_LEFT ? 48 : 52);
          ReplaceWall(NewGridNo(base_grid_no, DirectionInc(EAST)), orientation,
                      orientation == OUTSIDE_TOP_LEFT ? 49 : 53);
        ...
        case OUTSIDE_TOP_RIGHT:
        case INSIDE_TOP_RIGHT:
          ReplaceWall(NewGridNo(base_grid_no, DirectionInc(NORTH)), orientation,
                      orientation == OUTSIDE_TOP_RIGHT ? 51 : 55);
          ReplaceWall(NewGridNo(base_grid_no, DirectionInc(SOUTH)), orientation,
                      orientation == OUTSIDE_TOP_RIGHT ? 50 : 54);

          // looking for attached structures to remove in base tile
          RemoveOnWall(base_grid_no, STRUCTURE_ON_RIGHT_WALL,
                       0);  // XXX no next_current on base tile?
      ...
      // CJC, Sept 16: if we destroy any wall of the brothel, make Kingpin's men
      // hostile!
      if (gWorldSectorX == 5 && gWorldSectorY == MAP_ROW_C && gbWorldSectorZ == 0) {
        uint8_t room = GetRoom(grid_no);
    ...
    uint32_t const tile_type = GetTileType(node->usIndex);
    // Check if we are a fountain!
    if (strcasecmp(gTilesets[giCurrentTilesetID].TileSurfaceFilenames[tile_type],
                   "fount1.sti") == 0) {  // Remove water
      ApplyMapChangesToMapTempFile app;
      uint16_t sNewIndex;
    ...
    if (fContinue == 2) {  // We have a levelnode, get new index for new graphic
      uint16_t const tile_idx = GetTileIndexFromTypeSubIndex(tile_type, destruction_partner);
      ApplyMapChangesToMapTempFile app;
    ...
    *pfRecompileMovementCosts = TRUE;
    ...
    if (is_explosive) {
      InternalIgniteExplosion(owner, CenterX(base_grid_no), CenterY(base_grid_no), 0, base_grid_no,
                              STRUCTURE_EXPLOSION, FALSE, level);
    }
...
static void ExplosiveDamageGridNo(const int16_t sGridNo, const int16_t sWoundAmt,
                                  const uint32_t uiDist, BOOLEAN *const pfRecompileMovementCosts,
                                  const BOOLEAN fOnlyWalls, int8_t bMultiStructSpecialFlag,
                                  SOLDIERTYPE *const owner, const int8_t bLevel) {
  STRUCTURE *pCurrent, *pNextCurrent, *pStructure;
  STRUCTURE *pBaseStructure;
  int16_t sDesiredLevel;
  uint8_t ubLoop, ubLoop2;
  int16_t sNewGridNo, sNewGridNo2;
  ...
  uint8_t ubNumberOfTiles = 0;        // XXX HACK000E
  ...
  while (pCurrent != NULL) {
    ...
    if (pCurrent->sCubeOffset == sDesiredLevel) {
      fExplodeDamageReturn =
          ExplosiveDamageStructureAtGridNo(pCurrent, &pNextCurrent, sGridNo, sWoundAmt, uiDist,
                                           pfRecompileMovementCosts, fOnlyWalls, owner, bLevel);
              ...
              if (pStructure) {
                ...
                if (bMultiStructSpecialFlag == fMultiStructSpecialFlag) {
                  ...
                  ExplosiveDamageGridNo(sNewGridNo2, sWoundAmt, uiDist, pfRecompileMovementCosts,
                                        fOnlyWalls, bMultiStructSpecialFlag, owner, bLevel);

                  InternalIgniteExplosion(owner, CenterX(sNewGridNo2), CenterY(sNewGridNo2), 0,
...
static BOOLEAN DamageSoldierFromBlast(SOLDIERTYPE *const pSoldier, SOLDIERTYPE *const owner,
                                      const int16_t sBombGridNo, const int16_t sWoundAmt,
                                      const int16_t sBreathAmt, const uint32_t uiDist,
                                      const uint16_t usItem) {
  ...
  EVENT_SoldierGotHit(pSoldier, usItem, sNewWoundAmt, sBreathAmt, ubDirection, uiDist, owner, 0,
                      ANIM_CROUCH, sBombGridNo);
...
BOOLEAN DishOutGasDamage(SOLDIERTYPE *const pSoldier, EXPLOSIVETYPE const *const pExplosive,
                         int16_t const sSubsequent, BOOLEAN const fRecompileMovementCosts,
                         int16_t sWoundAmt, int16_t sBreathAmt, SOLDIERTYPE *const owner) {
          ...
          // ATE/CJC: gas stuff
  {
    ...
    if (bPosOfMask != NO_SLOT) {
      if (pSoldier->inv[bPosOfMask].bStatus[0] < GASMASK_MIN_STATUS) {
        // GAS MASK reduces breath loss by its work% (it leaks if not at least
        // 70%)
...
static void HandleBuldingDestruction(int16_t sGridNo, const SOLDIERTYPE *owner);

// Spreads the effects of explosions...
static BOOLEAN ExpAffect(const int16_t sBombGridNo, const int16_t sGridNo, const uint32_t uiDist,
                         const uint16_t usItem, SOLDIERTYPE *const owner, const int16_t sSubsequent,
                         BOOLEAN *const pfMercHit, const int8_t bLevel,
                         const SMOKEEFFECT *const smoke) {
  ...
  if (fBlastEffect) {
    ...
    // damage structures
    if (uiDist <= std::max((uint32_t)1, (uint32_t)(pExplosive->ubDamage / 30))) {
      ...
      ExplosiveDamageGridNo(sGridNo, sStructDmgAmt, uiDist, &fRecompileMovementCosts, FALSE, -1,
                            owner, bLevel);
    ...
    if (Random(50) < 15 && uiDist == 1) {
      // if (!TypeRangeExistsInObjectLayer(sGridNo, FIRSTEXPLDEBRIS,
      // SECONDEXPLDEBRIS))
      //{
      //	uint16_t usTileIndex =
      // GetTileIndexFromTypeSubIndex(SECONDEXPLDEBRIS, Random(10) + 1);
    ...
    if (fBlastEffect) {
      ...
      if (bLevel == 1) {
        ...
        if (tgt_below != NULL) {
          if ((sWoundAmt / 2) > 20) {
            // debris damage!
            const int16_t breath = sBreathAmt / 2 - 20 > 0 ? Random(sBreathAmt / 2 - 20) : 1;
            DamageSoldierFromBlast(tgt_below, owner, sBombGridNo, Random(sWoundAmt / 2 - 20),
                                   breath, uiDist, usItem);
          }
        }
      }
    } else {
      ...
      fRecompileMovementCosts = DishOutGasDamage(
          pSoldier, pExplosive, sSubsequent, fRecompileMovementCosts, sWoundAmt, sBreathAmt, owner);
      /*
                       if (!pSoldier->bActive || !pSoldier->bInSector ||
         !pSoldier->bLife || AM_A_ROBOT( pSoldier ) )
                       {
                               return( fRecompileMovementCosts );
                       }
         ...
         HIT_BY_CREATUREGAS )
                               {
                                      // already affected by creature gas this
         turn return( fRecompileMovementCosts );
                               }
                       }
                       else // no gas mask help from creature attacks
                              // ATE/CJC: gas stuff
                              {
                               ...
                               if ( pExplosive->ubType == EXPLOSV_TEARGAS )
                               {
                                      // ignore whether subsequent or not if hit
         this turn if ( pSoldier->fHitByGasFlags & HIT_BY_TEARGAS )
                                       {
                                              // already affected by creature
         gas this turn return( fRecompileMovementCosts );
                                       }
                               }
                               else if ( pExplosive->ubType == EXPLOSV_MUSTGAS )
                               {
         ...
         && pSoldier->inv[ HEAD1POS ].bStatus[0] >= USABLE )
                               {
                                              bPosOfMask = HEAD1POS;
                               }
                               else if ( pSoldier->inv[ HEAD2POS ].usItem ==
         GASMASK && pSoldier->inv[ HEAD2POS ].bStatus[0] >= USABLE )
                               {
                                              bPosOfMask = HEAD2POS;
                               }
                               ...
                               if ( bPosOfMask != NO_SLOT  )
                               {
                                       if ( pSoldier->inv[ bPosOfMask
         ].bStatus[0] < GASMASK_MIN_STATUS )
                                       {
                                               // GAS MASK reduces breath loss
         by its work% (it leaks if not at least 70%) sBreathAmt = ( sBreathAmt *
         ( 100 - pSoldier->inv[ bPosOfMask ].bStatus[0] ) ) / 100; if (
         sBreathAmt > 500 )
                                               {
                                                              // if at least 500
         of breath damage got through
                                                              // the soldier
         within the blast radius is gassed for at least one
                                                              // turn, possibly
         more if it's tear gas (which hangs around a while)
         ...
         |= SOLDIER_GASSED;
                                               ...
                                               if ( sWoundAmt > 1 )
                                               {
                                                pSoldier->inv[ bPosOfMask
         ].bStatus[0] -= (int8_t) Random( 4 ); sWoundAmt = ( sWoundAmt * ( 100 -
         pSoldier->inv[ bPosOfMask ].bStatus[0] ) ) / 100;
                                               }
                                               else if ( sWoundAmt == 1 )
                                               {
                                                      pSoldier->inv[ bPosOfMask
         ].bStatus[0] -= (int8_t) Random( 2 );
                                               }
                                       }
                                       else
                                       {
         ...
         |= HIT_BY_CREATUREGAS; break; case EXPLOSV_TEARGAS:
                                                      pSoldier->fHitByGasFlags
         |= HIT_BY_TEARGAS; break; case EXPLOSV_MUSTGAS:
                                                      pSoldier->fHitByGasFlags
         |= HIT_BY_MUSTARDGAS; break; default: break;
                                      }
                                      // a gas effect, take damage directly...
                                      SoldierTakeDamage(pSoldier, sWoundAmt,
         sBreathAmt, TAKE_DAMAGE_GAS, NULL); if ( pSoldier->bLife >=
         CONSCIOUSNESS )
                                      {
                                              DoMercBattleSound(pSoldier,
         BATTLE_SOUND_HIT1);
                                      }
                              }
                              */
...
static void GetRayStopInfo(uint32_t uiNewSpot, uint8_t ubDir, int8_t bLevel, BOOLEAN fSmokeEffect,
  ...
  if (fTravelCostObs) {
    if (fSmokeEffect) {
      ...
      if (fTravelCostObs) {
        // ATE: For windows, check to the west and north for a broken window, as
        // movement costs will override there...
...
void SpreadEffect(const int16_t sGridNo, const uint8_t ubRadius, const uint16_t usItem,
                  SOLDIERTYPE *const owner, const BOOLEAN fSubsequent, const int8_t bLevel,
                  const SMOKEEFFECT *const smoke) {
  int32_t uiNewSpot, uiTempSpot, uiBranchSpot, branchCnt;
  ...
  // Set values for recompile region to optimize area we need to recompile for
  // MPs
  ...
  // multiply range by 2 so we can correctly calculate approximately round
  // explosion regions
  sRange = ubRadius * 2;
  ...
  for (ubDir = NORTH; ubDir <= NORTHWEST; ubDir++) {
    ...
    while (cnt <= uiTempRange)  // end of range loop
    {
      ...
      if (ubKeepGoing) {
        ...
        // DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String("Explosion affects %d",
        // uiNewSpot) );
        // ok, do what we do here...
        if (ExpAffect(sGridNo, uiNewSpot, cnt / 2, usItem, owner, fSubsequent, &fAnyMercHit, bLevel,
                      smoke)) {
          fRecompileMovement = TRUE;
        }
        ...
        if (ubBranchRange) {
          ...
          while (branchCnt <= ubBranchRange)  // end of range loop
          {
            ...
            if (uiNewSpot != uiBranchSpot) {
              ...
              if (ubKeepGoing) {
                // ok, do what we do here
                // DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String("Explosion affects
                // %d", uiNewSpot) );
                if (ExpAffect(sGridNo, uiNewSpot, (int16_t)((cnt + branchCnt) / 2), usItem, owner,
                              fSubsequent, &fAnyMercHit, bLevel, smoke)) {
                  fRecompileMovement = TRUE;
                }
                uiBranchSpot = uiNewSpot;
              }
              // else
              {
                // check if it's ANY door, and if so, affect that spot so it's
                // damaged
  ...
  if (fRecompileMovement) {
    ...
    // if anything has been done to change movement costs and this is a
    // potential POW situation, check paths for POWs
  ...
  if (fSubsequent != BLOOD_SPREAD_EFFECT) {
    MakeNoise(NULL, sGridNo, bLevel, Explosive[Item[usItem].ubClassIndex].ubVolume,
              NOISE_EXPLOSION);
  }
}

void SpreadEffectSmoke(const SMOKEEFFECT *const s, const BOOLEAN subsequent, const int8_t level) {
  SpreadEffect(s->sGridNo, s->ubRadius, s->usItem, s->owner, subsequent, level, s);
}

static void ToggleActionItemsByFrequency(int8_t bFrequency) {
...
static void TogglePressureActionItemsInGridNo(int16_t sGridNo) {
  // Go through all the bombs in the world, and look for remote ones
  CFOR_EACH_WORLD_BOMB(wb) {
...
static BOOLEAN HookerInRoom(uint8_t ubRoom) {
...
static void PerformItemAction(int16_t sGridNo, OBJECTTYPE *pObj) {
  ...
  switch (pObj->bActionValue) {
    ...
    case ACTION_ITEM_ENTER_BROTHEL:
      ...
      if ( ! (gTacticalStatus.uiFlags & INCOMBAT) )
      {
              ...
              if (tgt != NULL && tgt->bTeam == OUR_TEAM)
              {
                      if (tgt->sOldGridNo == sGridNo + DirectionInc(SOUTH))
                      {
                              ...
                              // If we for any reason trigger Madame's record 34
      then we don't bother to do
                              // anything else

                              // Billy always moves back on a timer so that the
      player has a chance to sneak
                              // someone else through

                              // Madame's quote about female mercs should
      therefore not be made on a timer
                              ...
                              if ( gMercProfiles[ MADAME ].bNPCData2 ==
      gMercProfiles[ MADAME ].bNPCData )
                              {
                                      // full # of mercs who paid have entered
      brothel
      ...
      BillyBlocksDoorCallback, FALSE );
                              ...
                              else if ( gMercProfiles[ MADAME ].bNPCData2 >
      gMercProfiles[ MADAME ].bNPCData )
                              {
                                      // more than full # of mercs who paid have
      entered brothel
                                      // have Billy block the way again?
                                      if ( CheckFact(
      FACT_PLAYER_FORCED_WAY_INTO_BROTHEL, 0 ) )
                                      {
      ...
      2000, BillyBlocksDoorCallback, FALSE ); SetFactTrue(
      FACT_PLAYER_FORCED_WAY_INTO_BROTHEL ); TriggerNPCRecord( MADAME, 34 );
                                      }
                              }
    ...
    case ACTION_ITEM_EXIT_BROTHEL:
      ...
      {
              const SOLDIERTYPE* const tgt = WhoIsThere2(sGridNo, 0);
              if (tgt != NULL && tgt->bTeam == OUR_TEAM && tgt->sOldGridNo ==
      sGridNo + DirectionInc(NORTH))
              {
                      gMercProfiles[ MADAME ].bNPCData2--;
                      ...
                      SetCustomizableTimerCallbackAndDelay( 1000,
      DelayedBillyTriggerToBlockOnExit, TRUE );
              }
      }
      */
      break;
...

      {
        FOR_EACH_IN_TEAM(civ, CIV_TEAM) {
          if (civ->bInSector && civ->ubCivilianGroup == KINGPIN_CIV_GROUP) {
            for (uint8_t ubID2 = gTacticalStatus.Team[OUR_TEAM].bFirstID;
                 ubID2 <= gTacticalStatus.Team[OUR_TEAM].bLastID; ++ubID2) {
    ...
    case ACTION_ITEM_SEX:
      ...
      if ( ! (gTacticalStatus.uiFlags & INCOMBAT) )
      {
              ...
              if (tgt != NULL)
                      if (tgt->bTeam == OUR_TEAM)
                      {
                              ...
                              uint8_t const old_room = GetRoom(tgt->sOldGridNo);
                              if (room != NO_ROOM && old_room != NO_ROOM &&
      old_room != room)
                              {
                                      // also require there to be a miniskirt civ
      in the room if (HookerInRoom(room))
                                      {
                                              ...
                                              switch( sGridNo )
                                              {
                                                      case 13414:
                                                              sDoorSpot = 13413;
                                                              sTeleportSpot =
      13413; break; case 11174: sDoorSpot = 11173; sTeleportSpot = 11173; break;
                                                      case 12290:
                                                              sDoorSpot = 12290;
                                                              sTeleportSpot =
      12291; break;
      ...
      NOWHERE;
                                              ...
                                              if ( sDoorSpot != NOWHERE &&
      sTeleportSpot != NOWHERE )
                                              {
                                                      // close the door...
                                                      DoorCloser.bActionValue =
      ACTION_ITEM_CLOSE_DOOR; PerformItemAction( sDoorSpot, &DoorCloser );

                                                      // have sex
                                                      HandleNPCDoAction( 0,
      NPC_ACTION_SEX, 0 );

                                                      // move the merc outside of
      the room again sTeleportSpot = FindGridNoFromSweetSpotWithStructData(tgt,
      STANDING, sTeleportSpot, 2, &ubDirection, FALSE); ChangeSoldierState(tgt,
      STANDING, 0, TRUE); TeleportSoldier(*tgt, sTeleportSpot, false);

                                                      HandleMoraleEvent(tgt,
      MORALE_SEX, gWorldSectorX, gWorldSectorY, gbWorldSectorZ);
                                                      FatigueCharacter(tgt);
                                                      FatigueCharacter(tgt);
                                                      FatigueCharacter(tgt);
                                                      FatigueCharacter(tgt);
                                                      DirtyMercPanelInterface(tgt,
      DIRTYLEVEL1);
                                              }
                                      }
...
static void AddBombToQueue(uint32_t const world_bomb_idx, uint32_t const timestamp) {
...
void HandleExplosionQueue() {
  ...
  for (uint32_t i = 0; i != gubElementsOnExplosionQueue; ++i) {
    ...
    OBJECTTYPE &o = wi.o;
    int16_t const gridno = wi.sGridNo;
    uint8_t const level = wi.ubLevel;
    ...
    if (o.usItem == ACTION_ITEM && o.bActionValue != ACTION_ITEM_BLOW_UP) {
      ...
      uint16_t &flags = gpWorldLevelData[gridno].uiFlags;
  ...
  while (gubElementsOnExplosionQueue > 0 &&
         !gExplosionQueue[gubElementsOnExplosionQueue - 1].fExists) {
    --gubElementsOnExplosionQueue;
  }

  TacticalStatusType &ts = gTacticalStatus;
  if (gubElementsOnExplosionQueue == 0 &&
      (!gPersonToSetOffExplosions || ts.ubAttackBusyCount == 0)) {  // Turn off explosion queue
    ...
    if (gfExplosionQueueMayHaveChangedSight) {
      // Set variable so we may at least have someone to resolve interrupts
      // against
    ...
    if (!(ts.uiFlags & INCOMBAT) ||
        ts.ubCurrentTeam == OUR_TEAM) {  // Don't end UI lock when it's a computer turn
...
void SetOffBombsByFrequency(SOLDIERTYPE *const s, const int8_t bFrequency) {
  uint32_t uiWorldBombIndex;
  uint32_t uiTimeStamp;
...
void SetOffPanicBombs(SOLDIERTYPE *const s, const int8_t bPanicTrigger) {
  ...
  switch (bPanicTrigger) {
    ...
    case 1:
      SetOffBombsByFrequency(s, PANIC_FREQUENCY_2);
      break;
    case 2:
      SetOffBombsByFrequency(s, PANIC_FREQUENCY_3);
      break;
...
BOOLEAN SetOffBombsInGridNo(SOLDIERTYPE *const s, const int16_t sGridNo, const BOOLEAN fAllBombs,
                            const int8_t bLevel) {
  uint32_t uiWorldBombIndex;
  uint32_t uiTimeStamp;
    ...
    if (!(o.fFlags & OBJECT_DISABLED_BOMB)) {
      if (fAllBombs || o.bDetonatorType == BOMB_PRESSURE) {
        if (!fAllBombs && s->bTeam != OUR_TEAM) {
          // ignore this unless it is a mine, etc which would have to have been
          // placed by the player, seeing as how the others are all marked as
          // known to the AI.
        ...
        if (o.ubBombOwner > 1 && (s->bTeam == OUR_TEAM || s->bTeam == MILITIA_TEAM)) {
          continue;
        }
...
void ActivateSwitchInGridNo(SOLDIERTYPE *const s, const int16_t sGridNo) {
  // Go through all the bombs in the world, and look for mines at this location
  CFOR_EACH_WORLD_BOMB(wb) {
    ...
    OBJECTTYPE const &o = wi.o;
    if (o.usItem == SWITCH && !(o.fFlags & OBJECT_DISABLED_BOMB) &&
        o.bDetonatorType == BOMB_SWITCH) {
      // send out a signal to detonate other bombs, rather than this which
      // isn't a bomb but a trigger

      // first set attack busy count to 0 in case of a lingering a.b.c.
      // problem...
...
void SaveExplosionTableToSaveGameFile(HWFILE const hFile) {
  ...
  FileWrite(hFile, &uiExplosionCount, sizeof(uint32_t));
...
bool DoesSAMExistHere(int16_t const x, int16_t const y, int16_t const z, GridNo const gridno) {
  // ATE: If we are below, return right away
  if (z != 0) return false;

  int16_t const sector = SECTOR(x, y);
  for (int32_t i = 0; i != NUMBER_OF_SAMS; ++i) {
...
void UpdateSAMDoneRepair(int16_t const x, int16_t const y, int16_t const z) {
  // ATE: If we are below, return right away
  if (z != 0) return;
  ...
  for (int32_t i = 0; i != NUMBER_OF_SAMS; ++i) {
    ...
    uint16_t const good_graphic = GetTileIndexFromTypeSubIndex(EIGHTISTRUCT, gbSAMGraphicList[i]);
    uint16_t const damaged_graphic = good_graphic - 2;  // Damaged one (current) is 2 less
    GridNo const gridno = pSamGridNoAList[i];
    if (x == gWorldSectorX && y == gWorldSectorY &&
        z == gbWorldSectorZ) {  // Sector loaded, update graphic
      ApplyMapChangesToMapTempFile app;
      RemoveStruct(gridno, damaged_graphic);
      AddStructToHead(gridno, good_graphic);
    } else {  // We add temp changes to map not loaded
...
static void HandleBuldingDestruction(const int16_t sGridNo, const SOLDIERTYPE *const owner) {
  ...
  FOR_EACH_IN_TEAM(pSoldier, CIV_TEAM) {
    if (pSoldier->bInSector && pSoldier->bLife && pSoldier->bNeutral) {
      if (pSoldier->ubProfile != NO_PROFILE) {
        // ignore if the player is fighting the enemy here and this is a good
        // guy
...
static int32_t FindActiveTimedBomb() {
  // Go through all the bombs in the world, and look for timed ones
...
void RemoveAllActiveTimedBombs() {
  for (;;) {
    const int32_t item_idx = FindActiveTimedBomb();
    if (item_idx == -1) break;
