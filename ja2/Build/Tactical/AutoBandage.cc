...
void BeginAutoBandage() {
  ...
  if ((gTacticalStatus.uiFlags & INCOMBAT) || (NumEnemyInSector() != 0)) {
    DoMessageBox(MSG_BOX_BASIC_STYLE, g_langRes->Message[STR_SECTOR_NOT_CLEARED], GAME_SCREEN,
  ...
  CFOR_EACH_IN_TEAM(pSoldier, OUR_TEAM) {
    ...
    if (!pSoldier->bInSector || pSoldier->uiStatusFlags & SOLDIER_VEHICLE ||
  ...
  if (!fFoundAGuy) {
    ...
    DoMessageBox(MSG_BOX_BASIC_STYLE, gzLateLocalizedString[STR_LATE_09], GAME_SCREEN,
                 MSG_BOX_FLAG_OK, NULL, NULL);
  } else {
    if (!CanAutoBandage(FALSE)) {
      ...
      DoMessageBox(MSG_BOX_BASIC_STYLE, TacticalStr[BEGIN_AUTOBANDAGE_PROMPT_STR], GAME_SCREEN,
                   MSG_BOX_FLAG_YESNO, BeginAutoBandageCallBack, NULL);
    }
  }
}
...
void AutoBandage(BOOLEAN fStart) {
  if (fStart) {
    ...
    // build a mask
    MSYS_DefineRegion(&gAutoBandageRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
...
static void DisplayAutoBandageUpdatePanel() {
  ...
  for (iCounterA = 0; iCounterA < iNumberDoctorsHigh; iCounterA++) {
    for (iCounterB = 0; iCounterB < iNumberDoctorsWide; iCounterB++) {
      ...
      if (doctor != NULL) {
        ...
        FindFontCenterCoordinates(sCurrentXPosition, sCurrentYPosition,
                                  TACT_UPDATE_MERC_FACE_X_WIDTH - 25, 0, Name, TINYFONT1, &sX, &sY);
  ...
  FindFontCenterCoordinates(sXPosition, sCurrentYPosition, iTotalPixelsWide, 0, Medics, TINYFONT1,
                            &sX, &sY);
  // print medic
  MPrint(sX, sYPosition - 7, Medics);

  // DisplayWrappedString(sXPosition,  sCurrentYPosition - 40, iTotalPixelsWide,
  // 0, TINYFONT1, FONT_WHITE, pUpdateMercStrings[0], FONT_BLACK, 0);
  ...
  for (iCounterA = 0; iCounterA < iNumberPatientsHigh; iCounterA++) {
    BltVideoObject(
        FRAME_BUFFER, hBackGroundHandle, 3, sXPosition - 4,
        sYPosition + ((iCounterA + iNumberDoctorsHigh) * TACT_UPDATE_MERC_FACE_X_HEIGHT));
    BltVideoObject(
        FRAME_BUFFER, hBackGroundHandle, 5, sXPosition + iTotalPixelsWide,
        sYPosition + ((iCounterA + iNumberDoctorsHigh) * TACT_UPDATE_MERC_FACE_X_HEIGHT));
  }
  ...
  BltVideoObject(FRAME_BUFFER, hBackGroundHandle, 2, sXPosition + iTotalPixelsWide,
                 sYPosition - 13);

  iTotalPixelsHigh += 9;

  // the bottom
  BltVideoObject(FRAME_BUFFER, hBackGroundHandle, 17, sXPosition - 4,
  ...
  FindFontCenterCoordinates(sXPosition, sCurrentYPosition, iTotalPixelsWide, 0, Patients, TINYFONT1,
                            &sX, &sY);
  // print patient
  ...
  for (iCounter = 0; iCounter < MAX_CHARACTER_COUNT; iCounter++) {
    ...
    if (patient != NULL) {
      giAutoBandagesSoldierFaces[iCounter + iNumberOfDoctors] =
          Load65Portrait(GetProfile(patient->ubProfile));
    }
  }
...
static BOOLEAN RenderSoldierSmallFaceForAutoBandagePanel(int32_t iIndex, int16_t sCurrentXPosition,
  ...
  // grab the face
  BltVideoObject(FRAME_BUFFER, giAutoBandagesSoldierFaces[iIndex], 0, sCurrentXPosition + 2,
                 sCurrentYPosition + 2);
  ...
  const SOLDIERTYPE *const pSoldier =
      (iIndexCount > iIndex ? gdoctor_list[iIndex] : gpatient_list[iIndex - iIndexCount]);
