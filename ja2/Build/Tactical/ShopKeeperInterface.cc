...
static SGPVSurface *guiCornerWhereTacticalIsStillSeenImage;  // This image is for where the
                                                             // corner of tactical is still seen
                                                             // through the shop keeper interface
...
static ArmsDealerID gbSelectedArmsDealerID =
    ARMS_DEALER_INVALID;  // Contains the enum value for the currently selected
                          // arms dealer

// the quote that is in progress, in certain circumstances, we don't want
// queuing of related but different quotes
static int32_t giShopKeepDialogueEventinProgress = -1;
...
struct SELECTED_ARMS_DEALERS_STATS {
  ...
  uint8_t ubCurrentPage;
  uint8_t ubNumberOfPages;

  uint8_t ubFirstItemIndexOnPage;
};
...
static uint32_t guiRandomQuoteDelayTime = SKI_DEALERS_RANDOM_QUOTE_DELAY;
...
static uint8_t gubArrayOfEmployedMercs[MAX_CHARACTER_COUNT];
static SGPVObject *guiSmallSoldiersFace[MAX_CHARACTER_COUNT];
static uint8_t gubNumberMercsInArray;

static uint16_t gusPositionOfSubTitlesX = 0;

static BOOLEAN gfExitSKIDueToMessageBox = FALSE;

OBJECTTYPE *pShopKeeperItemDescObject = NULL;

static uint32_t guiNextFreeInvSlot;
...
static uint32_t guiLastTimeDealerSaidNormalEvaluationQuote = 0;
...
static void BtnSKI_InvPageUpButtonCallback(GUI_BUTTON *btn, int32_t reason);
...
static void BtnSKI_InvPageDownButtonCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiSKI_InvPageDownButtonImage;
static GUIButtonRef guiSKI_InvPageDownButton;

// Transaction buttons
static void BtnSKI_TransactionButtonCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiSKI_TransactionButtonImage;
static GUIButtonRef guiSKI_TransactionButton;

// Done buttons
static void BtnSKI_DoneButtonCallback(GUI_BUTTON *btn, int32_t reason);
...
static void EnterShopKeeperInterface();
static void ExitShopKeeperInterface();
static void GetShopKeeperInterfaceUserInput();
static void HandleShopKeeperInterface();
static void RenderShopKeeperInterface();
...
ScreenID ShopKeeperScreenHandle() {
  ...
  if (gfDisplayNoRoomMsg) {
    ...
    // ARM: message is delayed because we need the mouse restriction to be in
    // place BEFORE it comes up so it gets lifted/restored
...
void ShopKeeperScreenShutdown() { ShutDownArmsDealers(); }

static GUIButtonRef MakeButton(BUTTON_PICS *const img, const wchar_t *const text, const int16_t x,
                               const int16_t prio, const GUI_CALLBACK click,
                               const wchar_t *const help) {
  const int16_t text_col = SKI_BUTTON_COLOR;
  const int16_t shadow_col = DEFAULT_SHADOW;
  GUIButtonRef const btn =
      CreateIconAndTextButton(img, text, SKI_BUTTON_FONT, text_col, shadow_col, text_col,
                              shadow_col, x, SKI_BUTTON_Y, prio, click);
  btn->SpecifyDisabledStyle(GUI_BUTTON::DISABLED_STYLE_HATCHED);
  btn->SetFastHelpText(help);
  return btn;
}

static uint8_t CountNumberOfItemsInThePlayersOfferArea();
static void CreateSkiInventorySlotMouseRegions();
static void HandlePossibleRepairDelays();
static void HandleShopKeeperDialog(uint8_t ubInit);
static BOOLEAN InitShopKeepersFace(uint8_t ubMercID);
static void InitializeShopKeeper(BOOLEAN fResetPage);
static BOOLEAN OfferObjectToDealer(OBJECTTYPE *pComplexObject, uint8_t ubOwnerProfileId,
                                   int8_t bOwnerSlotId);
static void ResetAllQuoteSaidFlags();
static void SelectArmsDealersDropItemToGroundMovementRegionCallBack(MOUSE_REGION *pRegion,
                                                                    int32_t iReason);
static void SelectArmsDealersDropItemToGroundRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectArmsDealersFaceRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);

static void EnterShopKeeperInterface() {
  // make sure current merc is close enough and eligible to talk to the
  // shopkeeper.
  AssertMsg(CanMercInteractWithSelectedShopkeeper(GetSelectedMan()),
            "Selected merc can't interact with shopkeeper.  Send save AM-1");

  // Create a video surface to blt corner of the tactical screen that still
  // shines through
  guiCornerWhereTacticalIsStillSeenImage = AddVideoSurface(
      SKI_TACTICAL_BACKGROUND_START_WIDTH, SKI_TACTICAL_BACKGROUND_START_HEIGHT, PIXEL_DEPTH);
  ...
  // Check to make sure the inventory is null ( should always be null if we are
  // just coming in to the SKI )
  ...
  // Create an array of all mercs (anywhere!) currently in the player's employ,
  // and load their small faces
  // This is to support showing of repair item owner's faces even when they're
  // not in the sector, as long as they still work for player
  ...
  guiSKI_InvPageUpButton =
      QuickCreateButton(guiSKI_InvPageUpButtonImage, SKI_PAGE_UP_ARROWS_X, SKI_PAGE_UP_ARROWS_Y,
                        MSYS_PRIORITY_HIGHEST, BtnSKI_InvPageUpButtonCallback);
  guiSKI_InvPageUpButton->SpecifyDisabledStyle(GUI_BUTTON::DISABLED_STYLE_HATCHED);

  // Page down button for the merchant inventory
  guiSKI_InvPageDownButton = QuickCreateButton(
      guiSKI_InvPageDownButtonImage, SKI_PAGE_DOWN_ARROWS_X, SKI_PAGE_DOWN_ARROWS_Y,
      MSYS_PRIORITY_HIGHEST, BtnSKI_InvPageDownButtonCallback);
  ...
  // Transaction button
  // if the dealer repairs, use the repair fast help text for the transaction
  // button
  const wchar_t *help;
  ...
  guiSKI_TransactionButton = MakeButton(guiSKI_TransactionButtonImage,
                                        SKI_Text[SKI_TEXT_TRANSACTION], SKI_TRANSACTION_BUTTON_X,
                                        MSYS_PRIORITY_HIGH, BtnSKI_TransactionButtonCallback, help);

  // Done button
  guiSKI_DoneButton = MakeButton(guiSKI_DoneButtonImage, SKI_Text[SKI_TEXT_DONE], SKI_DONE_BUTTON_X,
                                 MSYS_PRIORITY_HIGH + 10, BtnSKI_DoneButtonCallback,
                                 SkiMessageBoxText[SKI_DONE_BUTTON_HELP_TEXT]);

  // Blanket the entire screen
  MSYS_DefineRegion(&gSKI_EntireScreenMouseRegions, 0, 0, SCREEN_WIDTH, 339, MSYS_PRIORITY_HIGH - 2,
  ...
  MSYS_DefineRegion(&gSkiInventoryMovementAreaMouseRegions, SKI_ITEM_MOVEMENT_AREA_X,
                    ...
                    (uint16_t)(SKI_ITEM_MOVEMENT_AREA_X + SKI_ITEM_MOVEMENT_AREA_WIDTH),
                    (uint16_t)(SKI_ITEM_MOVEMENT_AREA_Y + SKI_ITEM_MOVEMENT_AREA_HEIGHT),
                    MSYS_PRIORITY_HIGH - 1, CURSOR_NORMAL, MSYS_NO_CALLBACK, MSYS_NO_CALLBACK);
  ...
  MSYS_DefineRegion(&gArmsDealersFaceMouseRegions, SKI_FACE_X, SKI_FACE_Y,
                    (uint16_t)(SKI_FACE_X + SKI_FACE_WIDTH),
                    (uint16_t)(SKI_FACE_Y + SKI_FACE_HEIGHT), MSYS_PRIORITY_HIGH - 1, CURSOR_NORMAL,
                    MSYS_NO_CALLBACK, SelectArmsDealersFaceRegionCallBack);
  ...
  if (gItemToAdd.fActive) {
    ...
    int8_t bSlotNum = gItemToAdd.bPreviousInvPos;

    // if this is NOT a repair dealer or he is is but there is enough space in
    // the player's offer area
    // (you can't be out of space if it isn't a repairman, only they can fill it
    // up with repaired items!)
    if ((ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) ||
        (CountNumberOfItemsInThePlayersOfferArea() < SKI_NUM_ARMS_DEALERS_INV_SLOTS)) {
      // if we're supposed to store the original pocket #, but that pocket still
      // holds more of these
      if ((bSlotNum != -1) && (gpSMCurrentMerc->inv[bSlotNum].ubNumberOfObjects > 0)) {
        // then we can't store the pocket #, because our system can't return
        // stacked objects
        bSlotNum = -1;
      }
...
static BOOLEAN InitShopKeepersFace(uint8_t ubMercID) {
  SOLDIERTYPE *const pSoldier =
      FindSoldierByProfileID(ArmsDealerInfo[gbSelectedArmsDealerID].ubShopKeeperID);
...
static void DestroySkiInventorySlotMouseRegions();
static void RemoveShopKeeperSubTitledText();
static void ShutUpShopKeeper();
...
static void DisplayArmsDealerCurrentInventoryPage();
static void DisplayArmsDealerOfferArea();
static void DisplayPlayersOfferArea();
static void DisplayTalkingArmsDealer();
static void DisplayTheSkiDropItemToGroundString();
static void EnableDisableEvaluateAndTransactionButtons();
static void EvaluateItemAddedToPlayersOfferArea(int8_t bSlotID, BOOLEAN fFirstOne);
static void HandleCheckIfEnoughOnTheTable();
...
static void HandleShopKeeperInterface() {
  ...
  if (InItemDescriptionBox() && pShopKeeperItemDescObject != NULL) {
    ...
    // make sure the shop keeper doesn't start talking ( reset the timing
    // variable )
  ...
  if (gfDoEvaluationAfterOpening) {
    ...
    for (iCounter = 0; iCounter < SKI_NUM_TRADING_INV_SLOTS; iCounter++) {
      if (PlayersOfferArea[iCounter].fActive) {
        EvaluateItemAddedToPlayersOfferArea((int8_t)iCounter, fFirstOne);
        fFirstOne = FALSE;
      }
    }
  ...
  // if the Ski dirty flag was changed to a lower value, make sure it is set
  // properly
  ...
  if (gfIsTheShopKeeperTalking &&
    ...
    RenderMercPopUpBox(g_popup_box, gusPositionOfSubTitlesX, SKI_POSITION_SUBTITLES_Y,
                       FRAME_BUFFER);
  }
...
static void CrossOutUnwantedItems();
static void RestoreTacticalBackGround();
...
static void RenderShopKeeperInterface() {
  ...
  BltVideoObject(FRAME_BUFFER, guiMainTradeScreenImage, 0, SKI_MAIN_BACKGROUND_X,
                 SKI_MAIN_BACKGROUND_Y);

  // Display the Title
  DrawTextToScreen(SKI_Text[SKI_TEXT_MERCHADISE_IN_STOCK], SKI_MAIN_TITLE_X, SKI_MAIN_TITLE_Y,
                   SKI_MAIN_TITLE_WIDTH, SKI_TITLE_FONT, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK,
                   CENTER_JUSTIFIED);

  // if the dealer repairs
  if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
    // Display the Repair cost text
    DisplayWrappedString(SKI_TOTAL_COST_X, SKI_TOTAL_COST_Y, SKI_TOTAL_COST_WIDTH, 2,
                         SKI_LABEL_FONT, SKI_TITLE_COLOR, SKI_Text[SKI_TEXT_REPAIR_COST],
                         FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  } else {
    // Display the Total cost text
    DisplayWrappedString(SKI_TOTAL_COST_X, SKI_TOTAL_COST_Y, SKI_TOTAL_COST_WIDTH, 2,
                         SKI_LABEL_FONT, SKI_TITLE_COLOR, SKI_Text[SKI_TEXT_TOTAL_COST],
                         FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  }

  // Display the total value text
  DisplayWrappedString(SKI_TOTAL_VALUE_X, SKI_TOTAL_VALUE_Y, SKI_TOTAL_VALUE_WIDTH, 2,
                       SKI_LABEL_FONT, SKI_TITLE_COLOR, SKI_Text[SKI_TEXT_TOTAL_VALUE],
                       FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

  // Display the players current balance text
  DisplayWrappedString(SKI_PLAYERS_CURRENT_BALANCE_X, SKI_PLAYERS_CURRENT_BALANCE_Y,
                       SKI_PLAYERS_CURRENT_BALANCE_WIDTH, 2, SKI_LABEL_FONT, SKI_TITLE_COLOR,
                       SkiMessageBoxText[SKI_PLAYERS_CURRENT_BALANCE], FONT_MCOLOR_BLACK,
                       CENTER_JUSTIFIED);

  // Display the players current balance value
  SPrintMoney(zMoney, LaptopSaveInfo.iCurrentBalance);
  DrawTextToScreen(zMoney, SKI_PLAYERS_CURRENT_BALANCE_X,
                   SKI_PLAYERS_CURRENT_BALANCE_OFFSET_TO_VALUE, SKI_PLAYERS_CURRENT_BALANCE_WIDTH,
                   FONT10ARIAL, SKI_ITEM_PRICE_COLOR, FONT_MCOLOR_BLACK,
  ...
  if (gfRenderScreenOnNextLoop) {
    //	BlitBufferToBuffer(FRAME_BUFFER, guiCornerWhereTacticalIsStillSeenImage,
...
static void RestoreTacticalBackGround() {
  ...
  // SKI_TACTICAL_BACKGROUND_START_Y, SKI_TACTICAL_BACKGROUND_START_WIDTH,
  // SKI_TACTICAL_BACKGROUND_START_HEIGHT );

  //	BlitBufferToBuffer(guiCornerWhereTacticalIsStillSeenImage, FRAME_BUFFER,
  // SKI_TACTICAL_BACKGROUND_START_X, SKI_TACTICAL_BACKGROUND_START_Y,
  // SKI_TACTICAL_BACKGROUND_START_WIDTH, SKI_TACTICAL_BACKGROUND_START_HEIGHT);

  BltVideoSurface(FRAME_BUFFER, guiCornerWhereTacticalIsStillSeenImage,
                  SKI_TACTICAL_BACKGROUND_START_X, SKI_TACTICAL_BACKGROUND_START_Y, NULL);

  InvalidateScreen();
}

static void ExitSKIRequested();
    ...
    if (!HandleTextInput(&Event) && Event.usEvent == KEY_DOWN) {
      switch (Event.usParam) {
        ...
        case SDLK_SPACE: {
          ...
          // refresh background for player slots (in case item values change due
          // to Flo's discount)
          gubSkiDirtyLevel = SKI_DIRTY_LEVEL2;
        } break;
      }
...
static void EnableDisableDealersInventoryPageButtons();
...
static void PerformTransaction(uint32_t uiMoneyFromPlayersAccount);

static void BtnSKI_TransactionButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    // if not already doing it (multiple clicks can be queued up while
    // shopkeeper is still talking)
...
static void DealerInventoryScrollRegionCallback(MOUSE_REGION *const, int32_t const reason) {
...
static void SelectDealersInventoryMovementRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectDealersInventoryRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectDealersOfferSlotsMovementRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectDealersOfferSlotsRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectPlayersOfferSlotsMovementRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectPlayersOfferSlotsRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);

static void CreateSkiInventorySlotMouseRegions() {
  MSYS_DefineRegion(&g_dealer_inventory_scroll_region, 161, 27, 532, 137, MSYS_PRIORITY_HIGH,
                    CURSOR_NORMAL, NULL, DealerInventoryScrollRegionCallback);

  bool const does_repairs =
  ...
  for (uint32_t i = 0; i != SKI_NUM_ARMS_DEALERS_INV_SLOTS; ++i) {
    uint16_t const x =
        SKI_ARMS_DEALERS_INV_START_X + i % SKI_NUM_ARMS_DEALERS_INV_COLS * SKI_INV_OFFSET_X;
    uint16_t const y =
        SKI_ARMS_DEALERS_INV_START_Y + i / SKI_NUM_ARMS_DEALERS_INV_COLS * SKI_INV_OFFSET_Y;
    {
      MOUSE_REGION *const r = &gDealersInventoryMouseRegions[i];
      MSYS_DefineRegion(r, x, y, x + SKI_INV_SLOT_WIDTH, y + SKI_INV_SLOT_HEIGHT,
                        MSYS_PRIORITY_HIGH, CURSOR_NORMAL,
                        SelectDealersInventoryMovementRegionCallBack,
                        SelectDealersInventoryRegionCallBack);
      MSYS_SetRegionUserData(r, 0, i);
    }
    if (does_repairs) {  // Small Faces
      MOUSE_REGION *const r = &gRepairmanInventorySmallFaceMouseRegions[i];
      MSYS_DefineRegion(
          r, x + SKI_SMALL_FACE_OFFSET_X, y, x + SKI_SMALL_FACE_OFFSET_X + SKI_SMALL_FACE_WIDTH,
          y + SKI_SMALL_FACE_HEIGHT, MSYS_PRIORITY_HIGH + 1, CURSOR_NORMAL, NULL, NULL);
      MSYS_SetRegionUserData(r, 0, i);
    }
  }

  // Create the mouse regions for the shopkeeper's trading slots
  for (uint32_t i = 0; i != SKI_NUM_TRADING_INV_SLOTS; ++i) {
    uint16_t const x =
        SKI_ARMS_DEALERS_TRADING_INV_X + i % SKI_NUM_TRADING_INV_COLS * SKI_INV_OFFSET_X;
    uint16_t const y =
        SKI_ARMS_DEALERS_TRADING_INV_Y + i / SKI_NUM_TRADING_INV_COLS * SKI_INV_OFFSET_Y;
    {
      MOUSE_REGION *const r = &gDealersOfferSlotsMouseRegions[i];
      MSYS_DefineRegion(r, x, y, x + SKI_INV_SLOT_WIDTH, y + SKI_INV_SLOT_HEIGHT,
                        MSYS_PRIORITY_HIGH, CURSOR_NORMAL,
                        SelectDealersOfferSlotsMovementRegionCallBack,
                        SelectDealersOfferSlotsRegionCallBack);
      MSYS_SetRegionUserData(r, 0, i);
    }
    if (does_repairs) {  // Small Faces
      ...
      MSYS_DefineRegion(
          r, x + SKI_SMALL_FACE_OFFSET_X, y, x + SKI_SMALL_FACE_OFFSET_X + SKI_SMALL_FACE_WIDTH,
          y + SKI_SMALL_FACE_HEIGHT, MSYS_PRIORITY_HIGH + 1, CURSOR_NORMAL,
          SelectDealersOfferSlotsMovementRegionCallBack, SelectDealersOfferSlotsRegionCallBack);
      MSYS_SetRegionUserData(r, 0, i);
    }
  }
  ...
  for (uint32_t i = 0; i != SKI_NUM_TRADING_INV_SLOTS; ++i) {
    uint16_t const x = SKI_PLAYERS_TRADING_INV_X + i % SKI_NUM_TRADING_INV_COLS * SKI_INV_OFFSET_X;
    uint16_t const y = SKI_PLAYERS_TRADING_INV_Y + i / SKI_NUM_TRADING_INV_COLS * SKI_INV_OFFSET_Y;
    {  // Trading Slots
      MOUSE_REGION *const r = &gPlayersOfferSlotsMouseRegions[i];
      MSYS_DefineRegion(r, x, y, x + SKI_INV_SLOT_WIDTH, y + SKI_INV_SLOT_HEIGHT,
                        MSYS_PRIORITY_HIGH, CURSOR_NORMAL,
                        SelectPlayersOfferSlotsMovementRegionCallBack,
    ...
    {  // Small Faces
      ...
      MSYS_DefineRegion(
          r, x + SKI_SMALL_FACE_OFFSET_X, y, x + SKI_SMALL_FACE_OFFSET_X + SKI_SMALL_FACE_WIDTH,
          y + SKI_SMALL_FACE_HEIGHT, MSYS_PRIORITY_HIGH + 1, CURSOR_NORMAL,
          SelectPlayersOfferSlotsMovementRegionCallBack, SelectPlayersOfferSlotsRegionCallBack);
      MSYS_SetRegionUserData(r, 0, i);
    }
  }
}
...
static int8_t AddItemToArmsDealerOfferArea(const INVENTORY_IN_SLOT *pInvSlot,
                                           int8_t bSlotIdInOtherLocation);
static void InitShopKeeperItemDescBox(OBJECTTYPE *pObject, uint8_t ubPocket,
                                      uint8_t ubFromLocation);
  ...
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (gpTempDealersInventory[ubSelectedInvSlot].ItemObject.ubNumberOfObjects > 0) {
      ...
      if (!(gpTempDealersInventory[ubSelectedInvSlot].uiFlags & ARMS_INV_ITEM_SELECTED)) {
        // if the dealer repairs
        if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
          // ignore left clicks on items under repair.  Fully repaired items are
          // moved out to player's slots automatically
        } else  // not a repairman
        {
          ubLocation = AddItemToArmsDealerOfferArea(&gpTempDealersInventory[ubSelectedInvSlot],
                                                    ubSelectedInvSlot);
      ...
      } else  // some of this item are already in dealer's offer area
      {
        ...
        ArmsDealerOfferArea[gpTempDealersInventory[ubSelectedInvSlot].bSlotIdInOtherLocation]
            .ItemObject.ubNumberOfObjects += ubNumToMove;
    ...
    if (!InItemDescriptionBox()) {
      InitShopKeeperItemDescBox(&gpTempDealersInventory[ubSelectedInvSlot].ItemObject,
                                ubSelectedInvSlot, ARMS_DEALER_INVENTORY);
    } else {
      DeleteItemDescriptionBox();
    }
    ...
    /*
                    ...
                    if( gpTempDealersInventory[ ubSelectedInvSlot ].uiFlags &
       ARMS_INV_ITEM_SELECTED )
                    {
                            //Check to see it there is more then 1 item in the
       location INVENTORY_IN_SLOT* const a =
       &ArmsDealerOfferArea[gpTempDealersInventory[ubSelectedInvSlot].bSlotIdInOtherLocation];
                            if (a->ItemObject.ubNumberOfObjects > 0)
                            {
                                    //Increase the number in the dealer
       inventory gpTempDealersInventory[ ubSelectedInvSlot
       ].ItemObject.ubNumberOfObjects ++;

                                    //Decrease the number in the dealer offer
       area a->ItemObject.ubNumberOfObjects--;

                                    //if there is nothing left in the arms
       dealer offer area if (a->ItemObject.ubNumberOfObjects == 0)
                                    {
                                            RemoveItemFromArmsDealerOfferArea(
       gpTempDealersInventory[ ubSelectedInvSlot ].bSlotIdInOtherLocation, FALSE
       );
                                    }
...
static void SelectDealersInventoryMovementRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_GAIN_MOUSE) {
    ...
    if (!gpTempDealersInventory[ubSelectedInvSlot].fActive) return;
...
static int8_t AddInventoryToSkiLocation(const INVENTORY_IN_SLOT *pInv, uint8_t ubSpotLocation,
                                        uint8_t ubWhere);
...
static BOOLEAN RemoveItemFromArmsDealerOfferArea(int8_t bSlotId, BOOLEAN fKeepItem);
static BOOLEAN RemoveRepairItemFromDealersOfferArea(int8_t bSlot);
  ...
  } else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP)  // MSYS_CALLBACK_REASON_LBUTTON_UP)
  {
    ...
    */
    ...
    if (a->fActive) {
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        ...
        if (gMoveingItem.sItemIndex == 0) {
          // if the dealer is a repair dealer, allow the player to pick up the
          // item
          if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
          ...
          // if the slot is overloaded (holds more objects than we have room for
          // valid statuses of)
          if (a->ItemObject.ubNumberOfObjects > MAX_OBJECTS_PER_SLOT) {
            // then ignore the click - we can't do the swap, or anything very
            // useful, because we can't allow overloaded items into the player's
            // cursor - if he put them into a merc's inventory, the extra
            // statuses are missing! At best, we could do some sort of message
            // here.
            return;
          }
    ...
    } else  // empty slot
    {
      ...
      if (gMoveingItem.sItemIndex > 0) {
        // we'd better talking to a repairman, cursor is locked out of this area
        // while full for non-repairmen!
...
static BOOLEAN RemoveItemFromPlayersOfferArea(int8_t bSlot);
    ...
    } else  // we have something in the cursor
    {
      ...
      if (o->fActive) {
        ...
        if (o->ItemObject.ubNumberOfObjects > MAX_OBJECTS_PER_SLOT) {
          // then ignore the click - we can't do the swap, or anything very
          // useful, because we can't allow overloaded items into the player's
          // cursor - if he put them into a merc's inventory, the extra statuses
          // are missing! At best, we could do some sort of message here.
          return;
        }
      ...
      } else  // slot is empty
      {
        ...
        if ((gMoveingItem.uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_BEEN_EVALUATED) ||
          ...
          // place the item that is in the player's hand into this player offer
          // area slot
          bAddedToSlotID =
              AddInventoryToSkiLocation(&gMoveingItem, ubSelectedInvSlot, PLAYERS_OFFER_AREA);
          Assert(bAddedToSlotID != -1);
        } else {
          // this splits complex items for repairs.  Also puts things into the
          // first free POA slot
          OfferObjectToDealer(&gMoveingItem.ItemObject, gMoveingItem.ubIdOfMercWhoOwnsTheItem,
                              gMoveingItem.bSlotIdInOtherLocation);
        }
    ...
    if (o->fActive) {
      ...
      item description
                              else
                              {
                                      if ( !InItemDescriptionBox( ) )
                                      {
                                              InitItemDescriptionBox(gpSMCurrentMerc,
      o->bSlotIdInOtherLocation, 214, 1 + INV_INTERFACE_START_Y, 0);
                                      }
                                      else
                                      {
                                              DeleteItemDescriptionBox( );
...
static void DetermineArmsDealersSellingInventory();
...
static void InitializeShopKeeper(BOOLEAN fResetPage) {
  ...
  gSelectArmsDealerInfo.uiNumDistinctInventoryItems =
      CountDistinctItemsInArmsDealersInventory(gbSelectedArmsDealerID);
  ...
  if (gSelectArmsDealerInfo.uiNumDistinctInventoryItems > 0) {
    ...
    /* No longer necessary. ARM
                    //Some of the number might have changed in
       DetermineArmsDealersSellingInventory(), recalc the values
                    gSelectArmsDealerInfo.uiNumDistinctInventoryItems =
       CountDistinctItemsInArmsDealersInventory( gbSelectedArmsDealerID );
    */
  }

  gSelectArmsDealerInfo.ubNumberOfPages =
      (uint8_t)(gSelectArmsDealerInfo.uiNumDistinctInventoryItems / SKI_NUM_ARMS_DEALERS_INV_SLOTS);
...
static void CalculateFirstItemIndexOnPage() {
  gSelectArmsDealerInfo.ubFirstItemIndexOnPage =
      (gSelectArmsDealerInfo.ubCurrentPage - 1) * SKI_NUM_ARMS_DEALERS_INV_SLOTS;
}

static uint32_t DisplayInvSlot(uint8_t slot_num, uint16_t usItemIndex, uint16_t x, uint16_t y,
                               OBJECTTYPE const &pItemObject, bool hatched_out, uint8_t ubItemArea);
static void HatchOutInvSlot(uint16_t usPosX, uint16_t usPosY);
static void SetSkiFaceRegionHelpText(const INVENTORY_IN_SLOT *pInv, MOUSE_REGION *pRegion,
                                     uint8_t ubScreenArea);
static void SetSkiRegionHelpText(const INVENTORY_IN_SLOT *pInv, MOUSE_REGION *pRegion,
                                 uint8_t ubScreenArea);
...
static void DisplayArmsDealerCurrentInventoryPage() {
  ...
  if (gpTempDealersInventory != NULL) {
    if (gubSkiDirtyLevel != SKI_DIRTY_LEVEL0) {
      ...
      RestoreExternBackgroundRect(SKI_ARMS_DEALERS_INV_START_X, SKI_ARMS_DEALERS_INV_START_Y, 370,
                                  107);

      // Display the current inventory page
      DrawTextToScreen(SKI_Text[SKI_TEXT_PAGE], SKI_PAGE_X, SKI_PAGE_Y + 3, SKI_PAGE_WIDTH,
                       SKI_LABEL_FONT, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

      // Display the Current Page number
      uiFontHeight = GetFontHeight(SKI_LABEL_FONT);
      swprintf(zTemp, lengthof(zTemp), L"%d/%d", gSelectArmsDealerInfo.ubCurrentPage,
               gSelectArmsDealerInfo.ubNumberOfPages);
      DrawTextToScreen(zTemp, SKI_PAGE_X, SKI_PAGE_Y + uiFontHeight + 6, SKI_PAGE_WIDTH,
                       SKI_LABEL_FONT, SKI_TITLE_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
    }

    // Display all the items for the current page
    for (usCnt = gSelectArmsDealerInfo.ubFirstItemIndexOnPage;
         ((usCnt < gSelectArmsDealerInfo.uiNumDistinctInventoryItems) &&
          (sItemCount < SKI_NUM_ARMS_DEALERS_INV_SLOTS));
         usCnt++) {
      ...
      if (gpTempDealersInventory[usCnt].sItemIndex != 0) {
        ...
        DisplayInvSlot(usCnt, inv.sItemIndex, usPosX, usPosY, inv.ItemObject, fDisplayHatchOnItem,
                       ARMS_DEALER_INVENTORY);

        if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
          SetSkiRegionHelpText(&gpTempDealersInventory[usCnt],
                               &gDealersInventoryMouseRegions[sItemCount], ARMS_DEALER_INVENTORY);

          // if the dealer repairs
          if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
            SetSkiFaceRegionHelpText(&gpTempDealersInventory[usCnt],
                                     &gRepairmanInventorySmallFaceMouseRegions[sItemCount],
                                     ARMS_DEALER_INVENTORY);
          }
        }
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    // This handles the remaining (empty) slots, resetting Fast Help text, and
    // hatching out disabled ones
    while (sItemCount < SKI_NUM_ARMS_DEALERS_INV_SLOTS) {
      ...
      // if the dealer repairs
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        SetSkiFaceRegionHelpText(NULL, &gRepairmanInventorySmallFaceMouseRegions[sItemCount],
                                 ARMS_DEALER_INVENTORY);
      }
...
static void BuildDoneWhenTimeString(wchar_t sString[], size_t Length, ArmsDealerID,
                                    uint16_t usItemIndex, uint8_t ubElement);
static uint32_t CalcShopKeeperItemPrice(BOOLEAN fDealerSelling, BOOLEAN fUnitPriceOnly,
                                        uint16_t usItemID, float dModifier,
                                        const OBJECTTYPE *pItemObject);
static int8_t GetSlotNumberForMerc(uint8_t ubProfile);
static bool IsGunOrAmmoOfSameTypeSelected(OBJECTTYPE const &);

static uint32_t DisplayInvSlot(uint8_t const slot_num, uint16_t const item_idx, uint16_t const x,
                               uint16_t const y, OBJECTTYPE const &item_o, bool const hatched_out,
                               uint8_t const item_area) {
  wchar_t buf[64];
  ...
  {  // Display the item graphic
    ...
    int16_t cen_x = x + 7 + abs(SKI_INV_WIDTH - 3 - e.usWidth) / 2 - e.sOffsetX;
    int16_t cen_y = y + abs(SKI_INV_HEIGHT - e.usHeight) / 2 - e.sOffsetY;
  ...
  if (item_area == PLAYERS_OFFER_AREA) {
    ...
    if (dealer_info.ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) {
      if (!hatched_out || item_o.ubNumberOfObjects !=
                              0) {  // Show the unit price, not the total value of all if stacked
        item_cost = CalcShopKeeperItemPrice(DEALER_SELLING, TRUE, item_idx,
                                            dealer_info.u.price.sell, &item_o);
      }
    } else {  // Display the length of time needed to repair the item
      INVENTORY_IN_SLOT const &inv = gpTempDealersInventory[slot_num];
      Assert(inv.sSpecialItemElement != -1);
      BuildDoneWhenTimeString(buf, lengthof(buf), gbSelectedArmsDealerID, item_idx,
                              inv.sSpecialItemElement);
      DrawTextToScreen(buf, x + SKI_INV_PRICE_OFFSET_X, y + SKI_INV_PRICE_OFFSET_Y,
                       SKI_INV_SLOT_WIDTH, SKI_ITEM_DESC_FONT, SKI_ITEM_PRICE_COLOR,
                       FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  ...
  {
    ARMS_DEALER_INFO const &dealer_info = ArmsDealerInfo[gbSelectedArmsDealerID];
    if (dealer_info.ubTypeOfArmsDealer ==
        ARMS_DEALER_REPAIRS) {  // The dealer repairs, there is an item here,
                                // therefore display the item's owner's face
      ...
      item_cost = CalcShopKeeperItemPrice(DEALER_SELLING, FALSE, item_idx, dealer_info.u.price.sell,
                                          &item_o);
    }
  }

  if (item_cost != 0) {  // Display the item's price
  ...
  // If the there is more then 1 or if the item is stackable and some of it has
  // been bought and only 1 remains
  if (item_o.ubNumberOfObjects > 1 ||
      (item_o.ubNumberOfObjects == 1 && DealerItemIsSafeToStack(item_idx) &&
    ...
    DrawTextToScreen(buf, x + SKI_ITEM_NUMBER_TEXT_OFFSET_X, y + SKI_ITEM_NUMBER_TEXT_OFFSET_Y,
                     SKI_ITEM_NUMBER_TEXT_WIDTH, SKIT_NUMBER_FONT, SKI_ITEM_PRICE_COLOR,
                     FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
  }

  if (owner != NO_PROFILE) {  // Display the face
    int8_t const face_slot = GetSlotNumberForMerc(owner);
    if (face_slot != -1) {  // Still in player's employ
      BltVideoObject(FRAME_BUFFER, guiSmallSoldiersFace[face_slot], 0, x + SKI_SMALL_FACE_OFFSET_X,
                     y);
    }
  }

  if (ItemHasAttachments(item_o)) {  // Display the '*' in the bottom right corner of the square
    DrawTextToScreen(L"*", x + SKI_ATTACHMENT_SYMBOL_X_OFFSET, y + SKI_ATTACHMENT_SYMBOL_Y_OFFSET,
                     0, TINYFONT1, FONT_GREEN, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  }

  {  // Display 'JAMMED' or 'REPAIRED', if appropriate
    wchar_t const *const overlay_text = item_o.bGunAmmoStatus < 0 ? TacticalStr[JAMMED_ITEM_STR]
                                        : print_repaired          ? SKI_Text[SKI_TEXT_REPAIRED]
                                                                  : 0;
    if (overlay_text) {
      int16_t cen_x;
      int16_t cen_y;
      FindFontCenterCoordinates(x, y, SKI_INV_SLOT_WIDTH, SKI_INV_HEIGHT, overlay_text, TINYFONT1,
                                &cen_x, &cen_y);
      DrawTextToScreen(overlay_text, cen_x, cen_y, SKI_INV_SLOT_WIDTH, TINYFONT1, FONT_RED,
...
static int RepairmanItemQsortCompare(void const *pArg1, void const *pArg2) {
  ...
  uint32_t const repair_time1 =
      dih[inv_slot1.sItemIndex].SpecialItem[inv_slot1.sSpecialItemElement].uiRepairDoneTime;
  uint32_t const repair_time2 =
      dih[inv_slot2.sItemIndex].SpecialItem[inv_slot2.sSpecialItemElement].uiRepairDoneTime;

  // lower reapir time first
  return repair_time1 < repair_time2 ? -1 : repair_time1 > repair_time2 ? 1 : 0;
}

static void AddItemsToTempDealerInventory(uint16_t usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
                                          int16_t sSpecialItemElement, uint8_t ubHowMany,
                                          uint8_t ubOwner);
static BOOLEAN RepairIsDone(uint16_t usItemIndex, uint8_t ubElement);
...
static void DetermineArmsDealersSellingInventory() {
  ...
  gpTempDealersInventory =
      MALLOCNZ(INVENTORY_IN_SLOT, gSelectArmsDealerInfo.uiNumDistinctInventoryItems);
  guiNextFreeInvSlot = 0;

  // loop through the dealer's permanent inventory items, adding them all to the
  // temp inventory list
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex].ubTotalItems > 0) {
      ...
      if (gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex].ubPerfectItems > 0) {
        ...
        AddItemsToTempDealerInventory(
            usItemIndex, &SpclItemInfo, -1,
            gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex].ubPerfectItems, NO_PROFILE);
      }

      // add all active special items
      for (ubElement = 0;
           ubElement < gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex].ubElementsAlloced;
           ubElement++) {
        pSpecialItem =
            &(gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex].SpecialItem[ubElement]);
        ...
        if (pSpecialItem->fActive) {
          ...
          if (fAddSpecialItem) {
            ...
            AddItemsToTempDealerInventory(usItemIndex, &(pSpecialItem->Info), ubElement, 1,
                                          ubOwner);
          }
        }
      }
  ...
  if (guiNextFreeInvSlot > 1) {
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
      // sort this list by object category, and by ascending price within each
      // category
      qsort(gpTempDealersInventory, guiNextFreeInvSlot, sizeof(INVENTORY_IN_SLOT),
            RepairmanItemQsortCompare);
    } else {
      // sort this list by object category, and by ascending price within each
      // category
      qsort(gpTempDealersInventory, guiNextFreeInvSlot, sizeof(INVENTORY_IN_SLOT),
            ArmsDealerItemQsortCompare);
    }
  }
}

static void StoreObjectsInNextFreeDealerInvSlot(uint16_t usItemIndex,
...
static void AddItemsToTempDealerInventory(uint16_t usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
  ...
  if (ubHowMany == 1) {
    ...
    StoreObjectsInNextFreeDealerInvSlot(usItemIndex, pSpclItemInfo, sSpecialItemElement, 1,
                                        ubOwner);
  } else  // more than one
  {
    // if the items can be stacked
    // NOTE: This test must match the one inside
    // CountDistinctItemsInArmsDealersInventory() exactly!
    if (DealerItemIsSafeToStack(usItemIndex)) {
      // then we can store them all together in the same box safely, even if
      // there's more than MAX_OBJECTS_PER_SLOT
      StoreObjectsInNextFreeDealerInvSlot(usItemIndex, pSpclItemInfo, sSpecialItemElement,
                                          ubHowMany, ubOwner);
    } else {
      // non-stacking items must be stored in one / box , because each may have
      // unique fields besides bStatus[] Example: guns all have ammo, ammo type,
      // etc.  We need these uniquely represented for pricing & manipulation
...
static void StoreObjectsInNextFreeDealerInvSlot(uint16_t usItemIndex,
  ...
  MakeObjectOutOfDealerItems(usItemIndex, pSpclItemInfo, &(pDealerInvSlot->ItemObject),
                             (uint8_t)std::min(ubHowMany, (uint8_t)MAX_OBJECTS_PER_SLOT));

  if (ubHowMany > MAX_OBJECTS_PER_SLOT) {
    // HACK:  Override ItemObject->ubNumberOfObjects (1-8) with the REAL # of
    // items in this box. Note that this makes it an illegal OBJECTTYPE, since
    // there ubHowMany can easily be more than MAX_OBJECTS_PER_SLOT, so there's
    // no room to store the status of all of them one.  But we only so this for
    // perfect items, so we don't care, it works & saves us a lot hassles here.
    // Just be careful using the damn things!!!  You can't just pass them off
    // the most functions in Items.C(), use ShopkeeperAutoPlaceObject() and
    // ShopkeeperAddItemToPool() instead.
    pDealerInvSlot->ItemObject.ubNumberOfObjects = ubHowMany;
  }
}

static int8_t AddItemToPlayersOfferArea(uint8_t ubProfileID, const INVENTORY_IN_SLOT *pInvSlot,
                                        int8_t bSlotIdInOtherLocation);
...
static BOOLEAN RepairIsDone(uint16_t usItemIndex, uint8_t ubElement) {
  ...
  RepairItem.ubIdOfMercWhoOwnsTheItem = gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex]
  ...
  MakeObjectOutOfDealerItems(
      usItemIndex,
      &(gArmsDealersInventory[gbSelectedArmsDealerID][usItemIndex].SpecialItem[ubElement].Info),
  ...
  for (ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++) {
    if (RepairItem.ItemObject.usAttachItem[ubCnt] != NONE) {
      /* ARM: Can now repair with removeable attachments still attached...
                              ...
                              if ( Item[ RepairItem.ItemObject.usAttachItem[
         ubCnt ] ].fFlags & ITEM_INSEPARABLE )
      */
  ...
  // if the item is imprinted (by anyone, even player's mercs), and it's Fredo
  // repairing it
  if (/*( gArmsDealersInventory[ gbSelectedArmsDealerID ][ usItemIndex
         ].SpecialItem[ ubElement ].Info.ubImprintID == (NO_PROFILE + 1) ) && */
...
void DrawHatchOnInventory(SGPVSurface *const uiSurface, const uint16_t usPosX,
                          const uint16_t usPosY, const uint16_t usWidth, const uint16_t usHeight) {
  SGPRect ClipRect;
...
static float ItemConditionModifier(uint16_t usItemIndex, int8_t bStatus);
...
static uint32_t CalcShopKeeperItemPrice(BOOLEAN fDealerSelling, BOOLEAN fUnitPriceOnly,
  ...
  uint8_t ubItemsToCount = 0;
  uint8_t ubItemsNotCounted = 0;
  ...
  switch (Item[usItemID].usItemClass) {
    ...
    default:
      ...
      if (pItemObject->ubNumberOfObjects <= MAX_OBJECTS_PER_SLOT) {
        ...
        ubItemsNotCounted = 0;
        // in this situation, uiUnitPrice will actually be the sum of the values
        // of ALL the multiple objects
      } else {
        // overloaded amount, count just the first, the others must all be
        // identical
        ubItemsToCount = 1;
        ubItemsNotCounted = pItemObject->ubNumberOfObjects - 1;
      }

      // add the value of each magazine (multiple mags may have vastly different
      // #bullets left)
      for (ubCnt = 0; ubCnt < ubItemsToCount; ubCnt++) {
        // for bullets, ItemConditionModifier will convert the #ShotsLeft into a
        // percentage
        uiUnitPrice +=
            (uint32_t)(CalcValueOfItemToDealer(gbSelectedArmsDealerID, usItemID, fDealerSelling) *
        ...
        if (fUnitPriceOnly) {
          // want price for only one of them.  All statuses must be the same in
  ...
  if (gpSMCurrentMerc->ubProfile == FLO) {
    // if it's a GUN or AMMO (but not Launchers, and all attachments and payload
    // is included)
    switch (Item[usItemID].usItemClass) {
      case IC_GUN:
      case IC_AMMO:
  ...
  // if he is buying this from a player, then we allow a value of 0, since that
  // has a special "worthless" quote #18
...
static float ItemConditionModifier(uint16_t usItemIndex, int8_t bStatus) {
  ...
  // if the item is ammo, the condition modifier is based on how many shots are
  // left
...
static void DisplayArmsDealerOfferArea() {
  ...
  if (gubSkiDirtyLevel != SKI_DIRTY_LEVEL0) {
    ...
    RestoreExternBackgroundRect(usPosX, usPosY, SKI_ARMS_DEALERS_TRADING_INV_WIDTH,
                                SKI_ARMS_DEALERS_TRADING_INV_HEIGHT);
  }
  ...
  for (sCnt = 0; sCnt < SKI_NUM_TRADING_INV_SLOTS; sCnt++) {
    ...
    if (a->fActive) {
      ...
      // Display the inventory slot
      uiTotalCost += DisplayInvSlot(sCnt, a->sItemIndex, usPosX, usPosY, a->ItemObject,
                                    fDisplayHatchOnItem, ARMS_DEALER_OFFER_AREA);
    } else  // empty
    {
      // hatch it out if region is disabled
      if (!(gDealersOfferSlotsMouseRegions[sCnt].uiFlags & MSYS_REGION_ENABLED)) {
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    ...
    RestoreExternBackgroundRect(SKI_ARMS_DEALER_TOTAL_COST_X, SKI_ARMS_DEALER_TOTAL_COST_Y,
                                SKI_ARMS_DEALER_TOTAL_COST_WIDTH,
                                SKI_ARMS_DEALER_TOTAL_COST_HEIGHT);

    // Display the total cost text
    SPrintMoney(zTemp, uiTotalCost);
    DrawTextToScreen(zTemp, SKI_ARMS_DEALER_TOTAL_COST_X, SKI_ARMS_DEALER_TOTAL_COST_Y + 5,
                     SKI_INV_SLOT_WIDTH, SKI_LABEL_FONT, SKI_ITEM_PRICE_COLOR, FONT_MCOLOR_BLACK,
                     CENTER_JUSTIFIED);
  }
}
  ...
  for (bCnt = 0; bCnt < SKI_NUM_TRADING_INV_SLOTS; bCnt++) {
    ...
    if (!a->fActive) {
      ...
      *a = *pInvSlot;
      ...
      SetSkiFaceRegionHelpText(a, &gDealersOfferSlotsSmallFaceMouseRegions[bCnt],
...
static void ClearArmsDealerOfferSlot(int32_t ubSlotToClear);
...
static BOOLEAN RemoveItemFromArmsDealerOfferArea(int8_t bSlotId, BOOLEAN fKeepItem) {
  ...
  //	for( usCnt=0; usCnt<SKI_NUM_TRADING_INV_SLOTS; usCnt++)
  {
    ...
    if (a->fActive)  // bSlotIdInOtherLocation == bSlotIdInOtherLocation )
    {
      ...
      //			if( ArmsDealerOfferArea[ usCnt
      //].ItemObject.ubNumberOfObjects > 1 )
      if (fKeepItem) {
        gpTempDealersInventory[a->bSlotIdInOtherLocation].ItemObject.ubNumberOfObjects +=
            a->ItemObject.ubNumberOfObjects;
      }
...
static void BuildItemHelpTextString(wchar_t sString[], size_t Length, const INVENTORY_IN_SLOT *pInv,
                                    uint8_t ubScreenArea);
...
static void SetSkiFaceRegionHelpText(const INVENTORY_IN_SLOT *pInv, MOUSE_REGION *pRegion,
  ...
  if ((pInv != NULL) && (pInv->ubIdOfMercWhoOwnsTheItem != NO_PROFILE)) {
    ...
    // add who owns it
    swprintf(zHelpText, lengthof(zHelpText), L"%ls%ls %ls",
...
static INVENTORY_IN_SLOT *GetPtrToOfferSlotWhereThisItemIs(uint8_t ubProfileID, int8_t bInvPocket);
...
static int8_t AddItemToPlayersOfferArea(uint8_t ubProfileID, const INVENTORY_IN_SLOT *pInvSlot,
  ...
  if (bSlotIdInOtherLocation != -1) {
    ...
    if (GetPtrToOfferSlotWhereThisItemIs(ubProfileID, bSlotIdInOtherLocation) != NULL) return (-1);
  }
  ...
  for (bCnt = 0; bCnt < SKI_NUM_TRADING_INV_SLOTS; bCnt++) {
    ...
    if (!o->fActive) {
      *o = *pInvSlot;
      ...
      SetSkiFaceRegionHelpText(o, &gPlayersOfferSlotsSmallFaceMouseRegions[bCnt],
                               PLAYERS_OFFER_AREA);
...
static void ClearPlayersOfferSlot(int32_t ubSlotToClear);
...
static BOOLEAN RemoveItemFromPlayersOfferArea(int8_t bSlot) {
  ...
  // if the item doesn't have a duplicate copy in its owner merc's inventory
  // slot
  if (o->bSlotIdInOtherLocation == -1 && !SKITryToReturnInvToOwnerOrCurrentMerc(o)) {
    // failed to add item, inventory probably filled up or item is unowned and
    // current merc ineligible
    return (FALSE);
  }
...
static uint32_t EvaluateInvSlot(INVENTORY_IN_SLOT *pInvSlot);
static BOOLEAN WillShopKeeperRejectObjectsFromPlayer(int8_t bDealerId, int8_t bSlotId);
...
static void DisplayPlayersOfferArea() {
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    // Restore the area before blitting the new data on
    RestoreExternBackgroundRect(SKI_PLAYERS_TRADING_INV_X, SKI_PLAYERS_TRADING_INV_Y,
                                SKI_PLAYERS_TRADING_INV_WIDTH, SKI_PLAYERS_TRADING_INV_HEIGHT);
  }
    ...
    if (o->fActive) {
      ...
      if (Item[o->sItemIndex].usItemClass == IC_MONEY) {
        ...
        if (o->bSlotIdInOtherLocation != -1 && o->ubIdOfMercWhoOwnsTheItem != NO_PROFILE) {
          const SOLDIERTYPE *const s =
              FindSoldierByProfileIDOnPlayerTeam(o->ubIdOfMercWhoOwnsTheItem);
          Assert(s != NULL);
        ...
        if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) {
          ...
          if (!WillShopKeeperRejectObjectsFromPlayer(gbSelectedArmsDealerID, sCnt)) {
            ...
            if (!(o->uiFlags & ARMS_INV_JUST_PURCHASED)) {
              // re-evaluate the value of the item (needed for Flo's discount
              // handling)
              EvaluateInvSlot(o);
            }
          }
      ...
      //			fDisplayHatchOnItem = (o->uiFlags &
      //(ARMS_INV_PLAYERS_ITEM_HAS_BEEN_EVALUATED | ARMS_INV_JUST_PURCHASED)) ==
      // 0;

      // Display the inventory slot
      DisplayInvSlot(sCnt, o->sItemIndex, usPosX, usPosY, o->ItemObject, fDisplayHatchOnItem,
                     PLAYERS_OFFER_AREA);
  ...
  if (gubSkiDirtyLevel == SKI_DIRTY_LEVEL2) {
    ...
    // Restore the previous area
    RestoreExternBackgroundRect(SKI_PLAYERS_TOTAL_VALUE_X, SKI_PLAYERS_TOTAL_VALUE_Y,
                                SKI_PLAYERS_TOTAL_VALUE_WIDTH, SKI_PLAYERS_TOTAL_VALUE_HEIGHT);

    // Display the total cost text
    SPrintMoney(zTemp, uiTotalCost);
    DrawTextToScreen(zTemp, SKI_PLAYERS_TOTAL_VALUE_X, SKI_PLAYERS_TOTAL_VALUE_Y + 5,
                     SKI_INV_SLOT_WIDTH, SKI_LABEL_FONT, SKI_ITEM_PRICE_COLOR, FONT_MCOLOR_BLACK,
                     CENTER_JUSTIFIED);
  }
...
static INVENTORY_IN_SLOT *GetPtrToOfferSlotWhereThisItemIs(uint8_t ubProfileID, int8_t bInvPocket) {
  ...
  for (ubCnt = 0; ubCnt < SKI_NUM_TRADING_INV_SLOTS; ubCnt++) {
    ...
    if (o->bSlotIdInOtherLocation == bInvPocket && o->ubIdOfMercWhoOwnsTheItem == ubProfileID &&
        o->ItemObject.ubNumberOfObjects != 0) {
      return o;
    }

    INVENTORY_IN_SLOT *const a = &ArmsDealerOfferArea[ubCnt];
    if (a->bSlotIdInOtherLocation == bInvPocket && a->ubIdOfMercWhoOwnsTheItem == ubProfileID &&
...
BOOLEAN ShouldSoldierDisplayHatchOnItem(uint8_t ubProfileID, int16_t sSlotNum) {
  ...
  pInvSlot = GetPtrToOfferSlotWhereThisItemIs(ubProfileID, (int8_t)sSlotNum);
...
static uint32_t CalculateTotalArmsDealerCost() {
  uint32_t uiCnt;
  uint32_t uiTotal = 0;
  ...
  for (uiCnt = 0; uiCnt < SKI_NUM_TRADING_INV_SLOTS; uiCnt++) {
    ...
    if (a->fActive) {
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
        ...
        uiTotal += CalcShopKeeperItemPrice(DEALER_SELLING, FALSE, a->sItemIndex,
                                           ArmsDealerInfo[gbSelectedArmsDealerID].u.price.sell,
                                           &a->ItemObject);
      }
    }
  }
...
static uint32_t CalculateTotalPlayersValue() {
  uint8_t ubCnt;
  uint32_t uiTotal = 0;
...
class DialogueEventShopkeeperMoney : public DialogueEvent {
 public:
  DialogueEventShopkeeperMoney(wchar_t const *const message, uint32_t const money_amount,
                               MessageBoxFlags const flags, MSGBOX_CALLBACK callback)
      : message_(message), money_amount_(money_amount), flags_(flags), callback_(callback) {}
 ...
 private:
  ...
  uint32_t const money_amount_;
  MessageBoxFlags const flags_;
  MSGBOX_CALLBACK const callback_;
};

static uint32_t CalculateHowMuchMoneyIsInPlayersOfferArea();
static uint8_t CountNumberOfItemsInTheArmsDealersOfferArea();
static uint8_t CountNumberOfValuelessItemsInThePlayersOfferArea();
static void DealerGetsBribed(uint8_t ubProfileId, uint32_t uiMoneyAmount);
static void GivePlayerSomeChange(uint32_t uiAmount);
static BOOLEAN IsMoneyTheOnlyItemInThePlayersOfferArea();
static void MoveAllArmsDealersItemsInOfferAreaToPlayersOfferArea();
static void MovePlayerOfferedItemsOfValueToArmsDealersInventory();
static void MovePlayersItemsToBeRepairedToArmsDealersInventory();
static BOOLEAN StartShopKeeperTalking(uint16_t usQuoteNum);
...
static void PerformTransaction(uint32_t uiMoneyFromPlayersAccount) {
  ...
  // handle bribing... if the player is giving the dealer money, without buying
  // anything
  if (IsMoneyTheOnlyItemInThePlayersOfferArea() &&
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].uiFlags & ARMS_DEALER_ACCEPTS_GIFTS) {
      // Move all the players evaluated items to the dealer (also adds it to
      // dealer's cash)
      MovePlayerOfferedItemsOfValueToArmsDealersInventory();

      DealerGetsBribed(ArmsDealerInfo[gbSelectedArmsDealerID].ubShopKeeperID,
                       uiMoneyInPlayersOfferArea);
    }
  } else  // not a bribe
  {
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_REPAIRS) {
      uiAvailablePlayerOfferSlots =
          (SKI_NUM_TRADING_INV_SLOTS - CountNumberOfValuelessItemsInThePlayersOfferArea());

      // always reserve an empty slot for change.  We can't tell for sure
      // whether player is getting change because that could depend on how many
      // items he's trying to buy won't fit - he could only be getting change
      // because of that!
    ...
    if (uiArmsDealersItemsCost > uiPlayersTotalMoneyValue) {
      ...
      if (uiArmsDealersItemsCost > uiPlayersTotalMoneyValue + LaptopSaveInfo.iCurrentBalance) {
        ...
        DialogueEvent::Add(new DialogueEventShopkeeperMoney(
            SkiMessageBoxText[SKI_SHORT_FUNDS_TEXT], amount, MSG_BOX_FLAG_OK,
            ConfirmDontHaveEnoughForTheDealerMessageBoxCallBack));
      } else {
        // player doesn't have enough on the table, but can pay for it from his
        // balance
        /// ask player if wants to subtract the shortfall directly from his
        /// balance
        DialogueEvent::Add(new DialogueEventSkipAFrame());
        DialogueEvent::Add(new DialogueEventShopkeeperLockTransactionButton(true));

        wchar_t const *const message =
            uiPlayersTotalMoneyValue != 0
                ? SkiMessageBoxText
        ...
        uint32_t const amount = uiArmsDealersItemsCost - uiPlayersTotalMoneyValue;
        DialogueEvent::Add(new DialogueEventShopkeeperMoney(
            message, amount, MSG_BOX_FLAG_YESNO,
      ...
      // disable further calls to PerformTransaction() until callback calls us
      // again to free this flag
    ...
    // if the dealer repairs
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
      // Make sure there is enough money in the Player's offer area to cover the
      // repair charge
      if ((uiMoneyFromPlayersAccount + uiMoneyInPlayersOfferArea) >= uiArmsDealersItemsCost) {
        // Move all the players evaluated items to the dealer ( For a repairman,
        // this means just: get rid of the money )
        MovePlayerOfferedItemsOfValueToArmsDealersInventory();
        ...
        if (ArmsDealerInfo[gbSelectedArmsDealerID].uiFlags & ARMS_DEALER_GIVES_CHANGE) {
          ...
          iChangeToGiveToPlayer =
              (uiMoneyFromPlayersAccount + uiMoneyInPlayersOfferArea) - uiArmsDealersItemsCost;
    ...
    {
      // DON'T include uiMoneyFromPlayersAccount in change given to player.  The
      // only way this can happen is if he agrees to subtract from his balance,
      // but there isn't enough room.  In that case, the cost of items not
      // transfered is used to reduce his balance deduction instead (not here,
      ...
      if ((ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_BUYS_ONLY) ||
        ...
        if (iChangeToGiveToPlayer >
          ...
          // HACK HACK HACK: We forgot to write/record quote 27 for Jake, so he
          // ALWAYS must have enough money!
      ...
      if (ArmsDealerInfo[gbSelectedArmsDealerID].uiFlags & ARMS_DEALER_GIVES_CHANGE) {
        if (iChangeToGiveToPlayer > 0) {
          ...
          /*
                                                  //Remove the change the arms
             dealer is returning to the player if( ( gArmsDealerStatus[
             gbSelectedArmsDealerID ].uiArmsDealersCash - uiChangeToGiveToPlayer
             ) >= 0 ) gArmsDealerStatus[ gbSelectedArmsDealerID
             ].uiArmsDealersCash -= uiChangeToGiveToPlayer; else
                                                          gArmsDealerStatus[
             gbSelectedArmsDealerID ].uiArmsDealersCash = 0;
          */
      ...
      if (gbSelectedArmsDealerID == ARMS_DEALER_HOWARD) SetFactTrue(FACT_222);
    }

    // add cash transfered from player's account directly to dealer's cash
    // balance
...
static void RemoveItemFromDealersInventory(const INVENTORY_IN_SLOT *pInvSlot, uint8_t ubSlot);
    ...
    if (a->fActive) {
      ...
      if (bSlotID != -1) {
        ...
        if (gbSelectedArmsDealerID == ARMS_DEALER_FRANZ && a->sItemIndex == VIDEO_CAMERA) {
          // set a special flag
          gArmsDealerStatus[gbSelectedArmsDealerID].ubSpecificDealerFlags |=
      ...
      if (!RemoveItemFromArmsDealerOfferArea((uint8_t)uiCnt, FALSE))  // a->bSlotIdInOtherLocation
        Assert(0);
...
static void RemoveItemFromDealersInventory(const INVENTORY_IN_SLOT *pInvSlot, uint8_t ubSlot) {
  ...
  sInvSlot = ubSlot;
  //	sInvSlot = ( gSelectArmsDealerInfo.ubCurrentPage - 1 ) *
  // SKI_NUM_ARMS_DEALERS_INV_SLOTS + ubSlot;
  ...
  RemoveItemFromArmsDealerInventory(gbSelectedArmsDealerID, sItemID, &SpclItemInfo,
                                    pInvSlot->ItemObject.ubNumberOfObjects);
      ...
      if (o->uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE) {
        ...
        if (Item[o->sItemIndex].usItemClass == IC_MONEY) {
          ...
          gArmsDealerStatus[gbSelectedArmsDealerID].uiArmsDealersCash +=
              o->ItemObject.uiMoneyAmount;
        } else {
          // if the dealer doesn't strictly buy items from the player, give the
          // item to the dealer
          if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer != ARMS_DEALER_BUYS_ONLY) {
...
#else
  if (owner_pid == (uint8_t)-1) return gpSMCurrentMerc;
#endif
  SOLDIERTYPE *const owner = FindSoldierByProfileIDOnPlayerTeam(owner_pid);
  return owner ? owner : gpSMCurrentMerc;
}

static void DisableAllDealersInventorySlots();
static void DisableAllDealersOfferSlots();
  ...
  */
  ...
  switch (ubSource) {
    ...
    case ARMS_DEALER_OFFER_AREA: {
      ...
      // Restrict the cursor to the arms dealer offer area, players offer area and
    ...
    case PLAYERS_OFFER_AREA: {
      ...
      // if the slot is overloaded (holds more objects than we have room for valid
      // statuses of)
      if (o->ItemObject.ubNumberOfObjects > MAX_OBJECTS_PER_SLOT) {
        // allow only MAX_OBJECTS_PER_SLOT of those objects to be picked up at a
        // time (sure it kind of sucks, but it's a lot easier than handling
        // overloaded cursor objects in Interface Items!
        gMoveingItem.ItemObject.ubNumberOfObjects = MAX_OBJECTS_PER_SLOT;

        // decrease the number objects left in the slot by that much instead of
        // deleting it
    ...
    case PLAYERS_INVENTORY:
      ...
      // First try to evaluate the item immediately like any other left-click on a
      // merc's inventory slot would be.  Only if that doesn't work (because there
      // isn't enough room in the player's offer area), the item is picked up into
      // the cursor, and may then get placed into the player's offer area
      // directly, but it will NOT get evaluated that way, and so has no
      // possibility of entering the dealer's inventory (where complex items
      // aren't permitted).
      if (fOfferToDealerFirst &&
          OfferObjectToDealer(gpItemPointer, gpSMCurrentMerc->ubProfile, bSlotNum)) {
        // Reset the cursor
        SetSkiCursor(CURSOR_NORMAL);
      } else  // not supposed to offer it, or dealer has no more room
      {
        // store the current contents of the cursor in a temporary object
        // structure. We have to do this before memsetting gMoveingItem, 'cause
        // during swaps, gpItemPointer == &gMoveingItem.ItemObject!
        TempObject = *gpItemPointer;

        // ARM: The memset was previously commented out, in order to preserve the
        // owning merc's inv slot # during a swap of
        // items in an inventory slot.  However, that leads to other bugs: if you
        // picked the thing you're swapping in from a restricted inv slot
        // (headgear, vest, etc.), the item swapped out will end up belonging to
        // an illegal slot, and return there with a right click on it in the
        // player's offer area.  So now ALL items picked up here are unowned.
        memset(&gMoveingItem, 0, sizeof(INVENTORY_IN_SLOT));
        ...
        // By necessity, these items don't belong to a slot (so you can't return
        // them via a right click), because it would be too much work to handle
        // attachments, members of a stack, or even items swapped out of slots.
...
static void EnableAllDealersInventorySlots();
static void EnableAllDealersOfferSlots();
...
void SetSkiCursor(uint16_t usCursor) {
  ...
  if (usCursor == EXTERN_CURSOR) {
    ...
    if (gInvDesc.uiFlags & MSYS_REGION_EXISTS) gInvDesc.ChangeCursor(usCursor);
  ...
  // make sure disabled slot hatching gets updated when items picked up /
  // dropped
  gubSkiDirtyLevel = SKI_DIRTY_LEVEL2;
}
    ...
    case ARMS_DEALER_OFFER_AREA: {
      ...
      if (!a->fActive) {
        *a = *pInv;
        ...
        SetSkiRegionHelpText(a, &gDealersOfferSlotsMouseRegions[ubSpotLocation],
                             ARMS_DEALER_OFFER_AREA);
        SetSkiFaceRegionHelpText(a, &gDealersOfferSlotsSmallFaceMouseRegions[ubSpotLocation],
                                 ARMS_DEALER_OFFER_AREA);
    ...
    case PLAYERS_OFFER_AREA: {
      ...
      if (!o->fActive) {
        ...
        *o = *pInv;
        ...
        SetSkiRegionHelpText(o, &gPlayersOfferSlotsMouseRegions[ubSpotLocation],
                             PLAYERS_OFFER_AREA);
        SetSkiFaceRegionHelpText(o, &gPlayersOfferSlotsSmallFaceMouseRegions[ubSpotLocation],
                                 PLAYERS_OFFER_AREA);
      ...
      //			else if( )
      //			{
      // check to see the type we are adding is the same as the type that is
      // already there
      //			}
      else  // that slot is full
      {
        // add it elsewhere
        bSlotAddedTo = AddItemToPlayersOfferArea(pInv->ubIdOfMercWhoOwnsTheItem, pInv,
                                                 pInv->bSlotIdInOtherLocation);
      }
      break;
    }
  }
...
static void DealWithItemsStillOnTheTable();

static void HandleShopKeeperDialog(uint8_t ubInit) {
  uint32_t uiCurTime = GetJA2Clock();

  static uint32_t uiLastTime = 0;
  static int8_t bSpeech = -1;
  ...
  if (ubInit == 1) {
    // special: if it's Arnie, and we have stuff in for repairs, but it's not
    // fixed yet, use a different opening quote!
    if ((gbSelectedArmsDealerID == ARMS_DEALER_ARNIE) &&
        RepairmanIsFixingItemsButNoneAreDoneYet(ARNIE)) {
    ...
    if (gfStartWithRepairsDelayedQuote) {
      ...
      // treat this as having done business - the player WAS here for a good
      // reason, and it's the dealers fault...
      gfDoneBusinessThisSession = TRUE;
    } else {
  ...
  // if the player has requested to leave
    ...
    if ((uiCurTime - uiLastTime) > (guiRandomQuoteDelayTime + Random(guiRandomQuoteDelayTime))) {
      ...
      if (Chance(35)) {
        int16_t sRandomQuoteToUse = -1;

        // first check if one of the situation warrants one of the more precise
        // quotes

        // if the dealer repairs
        if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
          // if there are items in the arms dealer's offer area (must be
          // awaiting repairs)
        ...
        {
          // if there is any items in the player's area with value to this
          // dealer other than just money
        ...
        // if neither of the more precise quotes fit, or 33 percent of the time
        // anyways
        ...
        if (!gfCommonQuoteUsedThisSession[sRandomQuoteToUse]) {
          StartShopKeeperTalking((uint16_t)sRandomQuoteToUse);
...
static BOOLEAN StartShopKeeperTalking(uint16_t usQuoteNum) {
  ...
  class DialogueEventShopkeeperSetQuote : public DialogueEvent {
   public:
    DialogueEventShopkeeperSetQuote(int32_t const quote) : quote_(quote) {}
  ...
  // post quote dialogue
  CharacterDialogue(ArmsDealerInfo[gbSelectedArmsDealerID].ubShopKeeperID, usQuoteNum,
                    giShopKeeperFaceIndex, DIALOGUE_SHOPKEEPER_UI, FALSE);
...
static void ShopKeeperSubTitleRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);

void InitShopKeeperSubTitledText(const wchar_t *pString) {
  // Now setup the popup box
  if (gGameSettings.fOptions[TOPTION_SUBTITLES]) {
    uint16_t usActualWidth = 0;
    uint16_t usActualHeight = 0;
    ...
    g_popup_box =
        PrepareMercPopupBox(g_popup_box, BASIC_MERC_POPUP_BACKGROUND, BASIC_MERC_POPUP_BORDER,
                            ShopKeeperTalkingText, 300, 0, 0, 0, &usActualWidth, &usActualHeight);
    ...
    if (!(gShopKeeperSubTitleMouseRegion.uiFlags & MSYS_REGION_EXISTS)) {
      MSYS_DefineRegion(&gShopKeeperSubTitleMouseRegion, gusPositionOfSubTitlesX,
                        ...
                        (int16_t)(gusPositionOfSubTitlesX + usActualWidth),
                        (int16_t)(SKI_POSITION_SUBTITLES_Y + usActualHeight), MSYS_PRIORITY_HIGH,
...
static void RemoveShopKeeperSubTitledText() {
  ...
  if (gGameSettings.fOptions[TOPTION_SUBTITLES]) MSYS_RemoveRegion(&gShopKeeperSubTitleMouseRegion);
}
...
static int8_t GetSlotNumberForMerc(uint8_t ubProfile) {
  ...
  for (bCnt = 0; bCnt < gubNumberMercsInArray; bCnt++) {
    if (gubArrayOfEmployedMercs[bCnt] == ubProfile) return (bCnt);
  }
...
static void EnableDisableDealersInventoryPageButtons() {
  ...
  EnableButton(guiSKI_InvPageDownButton,
               gSelectArmsDealerInfo.ubCurrentPage != gSelectArmsDealerInfo.ubNumberOfPages);
}
...
static void EnableDisableEvaluateAndTransactionButtons() {
  ...
  // loop through all the items in the players offer area and determine if they
  // can be sold here.
  for (ubCnt = 0; ubCnt < SKI_NUM_TRADING_INV_SLOTS; ubCnt++) {
    INVENTORY_IN_SLOT *const o = &PlayersOfferArea[ubCnt];
    // if there is an item here
    if (o->fActive) {
      ...
      if (o->uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE) {
        ...
        if (Item[o->sItemIndex].usItemClass != IC_MONEY) {
          fItemEvaluated = TRUE;
        } else if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer !=
                       ARMS_DEALER_REPAIRS &&
                   Item[o->sItemIndex].usItemClass == IC_MONEY) {
  ...
  if (fItemEvaluated) {
    //		//and if the players offer area value exceeds the shopkeeper's
    // area 		if( uiPlayersOfferAreaTotalCost >= uiArmsDealerTotalCost )
    EnableButton(guiSKI_TransactionButton);
    //		else
    //			DisableButton( guiSKI_TransactionButton );

    /*
                    //if the only item in the players offer area is money, and
       there is nothing in the dealers offer area if(
       IsMoneyTheOnlyItemInThePlayersOfferArea( ) && uiArmsDealerTotalCost == 0
       ) DisableButton( guiSKI_TransactionButton );
    */
  ...
  /* Allow transaction attempt when dealer can't afford to buy that much - he'll
     reject it with a special quote! else if( ( gArmsDealerStatus[
     gbSelectedArmsDealerID ].uiArmsDealersCash + uiArmsDealerTotalCost ) < (
     uiPlayersOfferAreaTotalCost - uiPlayersOfferAreaTotalMoney ) )
          {
                  DisableButton( guiSKI_TransactionButton );
          }
  */
  else if ((uiPlayersOfferAreaTotalCost == uiPlayersOfferAreaTotalMoney) &&
           (uiPlayersOfferAreaTotalMoney > 0) && (uiArmsDealerTotalCost == 0)) {
    DisableButton(guiSKI_TransactionButton);
  }
  ...
  // ARM: Always permit trying bribes, even if they don't work on a given
  // dealer!
  // if the arms dealer is the kind of person who accepts gifts, and there is
  // stuff to take
  //	if( ArmsDealerInfo[ gbSelectedArmsDealerID ].uiFlags &
  ...
  if (gfUserHasRequestedToLeave) DisableButton(guiSKI_TransactionButton);
...
static void MovePlayersItemsToBeRepairedToArmsDealersInventory() {
  ...
  for (uiCnt = 0; uiCnt < SKI_NUM_TRADING_INV_SLOTS; uiCnt++) {
    ...
    if (a->fActive) {
      // NOTE:  Any items that make it into a repairman's dealer offer area are
      // guaranteed to be:
      //				a) Repairable
      //				b) Actually damaged
      //				c) Already stripped of all attachments
      //				d) If a gun, stripped of any
      // non-ammo-class GunAmmoItems, and bullets

      // add it to the arms dealer's inventory
      GiveObjectToArmsDealerForRepair(gbSelectedArmsDealerID, &a->ItemObject,
                                      a->ubIdOfMercWhoOwnsTheItem);
...
static int8_t GetInvSlotOfUnfullMoneyInMercInventory(SOLDIERTYPE *pSoldier) {
  uint8_t ubCnt;

  // loop through the soldier's inventory
  for (ubCnt = 0; ubCnt < NUM_INV_SLOTS; ubCnt++) {
    // Look for MONEY only, not Gold or Silver!!!  And look for a slot not
    // already full
    if ((pSoldier->inv[ubCnt].usItem == MONEY) &&
        (pSoldier->inv[ubCnt].uiMoneyAmount < MoneySlotLimit(ubCnt))) {
      return (ubCnt);
    }
  }
  return (-1);
...
static void CheckAndHandleClearingOfPlayerOfferArea();
...
static BOOLEAN CanShopkeeperOverrideDialogue();

static void EvaluateItemAddedToPlayersOfferArea(int8_t bSlotID, BOOLEAN fFirstOne) {
  uint32_t uiEvalResult = EVAL_RESULT_NORMAL;
  BOOLEAN fRocketRifleWasEvaluated = FALSE;
  uint8_t ubNumberOfItemsAddedToRepairDuringThisEvaluation = 0;
  ...
  if (!gfDealerHasSaidTheEvaluateQuoteOnceThisSession &&
  ...
  if (!WillShopKeeperRejectObjectsFromPlayer(gbSelectedArmsDealerID, bSlotID)) {
    ...
    if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
      ...
      // if there is already enough items in for repairs, complain about it and
      // DON'T accept the item for repairs
      if ((uiNumberOfItemsAlreadyEvaluated + ubNumberOfItemsAddedToRepairDuringThisEvaluation +
           uiNumberOfItemsInForRepairs) > SKI_MAX_AMOUNT_OF_ITEMS_DEALER_CAN_REPAIR_AT_A_TIME) {
      ...
      // if the item is damaged, or is a rocket rifle (which always "need
      // repairing" even at 100%, to reset imprinting)
      if (o->ItemObject.bStatus[0] < 100 || fRocketRifleWasEvaluated) {
        ...
        if (bSlotAddedTo != -1) {
          ...
          /*	ARM: Leave it there, until transaction occurs it should be
             recallable
                                                  //Remove the item from the
             owner merc's inventory IfMercOwnedRemoveItemFromMercInv(a);
          */
          ...
          // check if it's the first time a rocket rifle is being submitted to
          // Fredo
          if (fRocketRifleWasEvaluated && (gbSelectedArmsDealerID == ARMS_DEALER_FREDO)) {
            // if he hasn't yet said his quote
            if (!(gArmsDealerStatus[gbSelectedArmsDealerID].ubSpecificDealerFlags &
                  ARMS_DEALER_FLAG__FREDO_HAS_SAID_ROCKET_RIFLE_QUOTE)) {
  ...
  // when evaluating complex items that get split into multiple subobjects,
  // dealer will talk only about the first one! don't bother with any of this if
  // shopkeeper can't talk right now
  if (fFirstOne && CanShopkeeperOverrideDialogue()) {
    int16_t sQuoteNum = -1;
    uint8_t ubChanceToSayNormalQuote;

    switch (uiEvalResult) {
      case EVAL_RESULT_DONT_HANDLE:
        if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_SELLS_ONLY) {
          // then he doesn't have quotes 17, 19, or 20, always use 4.  Devin
          // doesn't have 18 either, while the text of 18 seems wrong for Sam &
          // Howard if offered something they should consider valuable.
      ...
      case EVAL_RESULT_NON_REPAIRABLE:
        // use quote 4 for this situation...  It's a bit weird but better than
        // "that looks fine / ain't broke"
        sQuoteNum = SK_QUOTES_NOT_INTERESTED_IN_THIS_ITEM;
        break;
    ...
    if (sQuoteNum != -1) {
      // if the appropriate quote hasn't already been used during this
      // evaluation cycle
      if (!gfEvalResultQuoteSaid[uiEvalResult]) {
        // try to say it
        if (StartShopKeeperTalking((uint16_t)sQuoteNum)) {
          ...
          if (sQuoteNum == FREDO_QUOTE_CAN_RESET_IMPRINTING) {
            // Set the fact that we have said this (to prevent that quote from
            // coming up again)
            gArmsDealerStatus[gbSelectedArmsDealerID].ubSpecificDealerFlags |=
                ARMS_DEALER_FLAG__FREDO_HAS_SAID_ROCKET_RIFLE_QUOTE;
          }
...
void DoSkiMessageBox(wchar_t const *const zString, ScreenID const uiExitScreen,
                     MessageBoxFlags const ubFlags, MSGBOX_CALLBACK const ReturnCallback) {
  // reset exit mode
  gfExitSKIDueToMessageBox = TRUE;

  // do message box and return
  SGPBox const centering_rect = {0, 0, SCREEN_WIDTH, 339};
  DoMessageBox(MSG_BOX_BASIC_STYLE, zString, uiExitScreen, ubFlags, ReturnCallback,
               &centering_rect);
}
...
void ConfirmToDeductMoneyFromPlayersAccountMessageBoxCallBack(
...
// run through what the player has on the table and see if the shop keep will
// accept it or not
static BOOLEAN WillShopKeeperRejectObjectsFromPlayer(int8_t bDealerId, int8_t bSlotId) {
...
static void CrossOutUnwantedItems() {
  int8_t bSlotId = 0;
  int16_t sBoxStartX = 0, sBoxStartY = 0;
  int16_t sBoxWidth = 0, sBoxHeight = 0;
  ...
  for (bSlotId = 0; bSlotId < SKI_NUM_TRADING_INV_SLOTS; bSlotId++) {
    ...
    if (o->fActive) {
      // skip purchased items!
      if (!(o->uiFlags & ARMS_INV_JUST_PURCHASED)) {
        // If item can't be sold here, or it's completely worthless (very cheap
        // / very broken)
        if (WillShopKeeperRejectObjectsFromPlayer(gbSelectedArmsDealerID, bSlotId) ||
            !(o->uiFlags & ARMS_INV_PLAYERS_ITEM_HAS_VALUE)) {
          // get x and y positions
          sBoxStartX =
              SKI_PLAYERS_TRADING_INV_X + (bSlotId % SKI_NUM_TRADING_INV_COLS) * (SKI_INV_OFFSET_X);
          sBoxStartY =
...
static void InitShopKeeperItemDescBox(OBJECTTYPE *pObject, uint8_t ubPocket,
  ...
  switch (ubFromLocation) {
    case ARMS_DEALER_INVENTORY: {
      uint8_t ubSelectedInvSlot = ubPocket - gSelectArmsDealerInfo.ubFirstItemIndexOnPage;

      sPosX = SKI_ARMS_DEALERS_INV_START_X +
              (SKI_INV_OFFSET_X * (ubSelectedInvSlot % SKI_NUM_ARMS_DEALERS_INV_COLS) - (358 / 2)) +
              SKI_INV_SLOT_WIDTH / 2;

      sPosY = SKI_ARMS_DEALERS_INV_START_Y +
              ((SKI_INV_OFFSET_Y * ubSelectedInvSlot / SKI_NUM_ARMS_DEALERS_INV_COLS) + 1) -
              (128 / 2) + SKI_INV_SLOT_HEIGHT / 2;

      // if the start position + the height of the box is off the screen,
      // reposition
      if (sPosY < 0) sPosY = 0;

      // if the start position + the width of the box is off the screen,
      // reposition
      if (sPosX + 358 > SCREEN_WIDTH) sPosX = SCREEN_WIDTH - 358 - 5;

      // if it is starting to far to the left
      else if (sPosX < 0)
        sPosX = 0;

      // if the box will appear over the mercs face, move the box over so it
      // doesn't obstruct the face
      if (sPosY < SKI_FACE_Y + SKI_FACE_HEIGHT + 20)
        if (sPosX < 160) sPosX = 160;

    } break;

    case ARMS_DEALER_OFFER_AREA: {
      sPosX = SKI_ARMS_DEALERS_TRADING_INV_X +
              (SKI_INV_OFFSET_X * (ubPocket % (SKI_NUM_TRADING_INV_SLOTS / 2)) - (358 / 2)) +
              SKI_INV_SLOT_WIDTH / 2;

      sPosY = SKI_ARMS_DEALERS_TRADING_INV_Y +
              ((SKI_INV_OFFSET_Y * ubPocket / (SKI_NUM_TRADING_INV_SLOTS / 2)) + 1) - (128 / 2) +
              SKI_INV_SLOT_HEIGHT / 2;
...
void StartSKIDescriptionBox() {
  ...
  // if the current merc is too far away, dont shade the SM panel because it is
  // already shaded
  const uint16_t h = (gfSMDisableForItems ? INV_INTERFACE_START_Y : SCREEN_HEIGHT);
...
void DeleteShopKeeperItemDescBox() {
  ...
  CheckForDisabledForGiveItem();
  DisableInvRegions(gfSMDisableForItems);  // actually does an enable if current
                                           // merc is in range
...
static BOOLEAN AddObjectForEvaluation(OBJECTTYPE *pObject, uint8_t ubOwnerProfileId,
                                      int8_t bOwnerSlotId, BOOLEAN fFirstOne);
static void CountSubObjectsInObject(OBJECTTYPE *pComplexObject, uint8_t *pubTotalSubObjects,
                                    uint8_t *pubRepairableSubObjects,
                                    uint8_t *pubNonRepairableSubObjects);
static void SplitComplexObjectIntoSubObjects(OBJECTTYPE *pComplexObject);

static BOOLEAN OfferObjectToDealer(OBJECTTYPE *pComplexObject, uint8_t ubOwnerProfileId,
                                   int8_t bOwnerSlotId) {
  uint8_t ubTotalSubObjects;
  uint8_t ubRepairableSubObjects;
  uint8_t ubNonRepairableSubObjects;
  uint8_t ubDealerOfferAreaSlotsNeeded;
  uint8_t ubPlayerOfferAreaSlotsNeeded;
  uint8_t ubDiff;
  uint8_t ubHowManyMoreItemsCanDealerTake;
  uint8_t ubSubObject;
  BOOLEAN fFirstOne = TRUE;
  ...
  } else  // dealing with a repairman
  {
    ...
    // determine how many subobjects of each category this complex object will
    // have to be broken up into
    CountSubObjectsInObject(pComplexObject, &ubTotalSubObjects, &ubRepairableSubObjects,
                            &ubNonRepairableSubObjects);

    // in the simplest situation, the # subobjects of each type gives us how
    // many slots of each type are needed
    ubDealerOfferAreaSlotsNeeded = ubRepairableSubObjects;
    ubPlayerOfferAreaSlotsNeeded = ubNonRepairableSubObjects;

    // consider that if dealer is at or will reach his max # of items repaired
    // limit, not everything repairable will move
    ubHowManyMoreItemsCanDealerTake =
        SKI_MAX_AMOUNT_OF_ITEMS_DEALER_CAN_REPAIR_AT_A_TIME -
        CountTotalItemsRepairDealerHasInForRepairs(gbSelectedArmsDealerID) -
        CountNumberOfItemsInTheArmsDealersOfferArea();

    // if he can't repair everything repairable that we're about to submit, the
    // space we'll need changes
    ...
    if (ubDealerOfferAreaSlotsNeeded > 0) {
      // we need at least one EXTRA empty slot in player's area to pass
      // repairable objects through for their evaluation
      ubPlayerOfferAreaSlotsNeeded++;
    }

    Assert(SKI_MAX_AMOUNT_OF_ITEMS_DEALER_CAN_REPAIR_AT_A_TIME < SKI_NUM_TRADING_INV_SLOTS);
    /*
                    This code is commented out because a repair dealer will
       never be allowed to repair over more then
                    SKI_MAX_AMOUNT_OF_ITEMS_DEALER_CAN_REPAIR_AT_A_TIME ( 4 )
       items at a time, therefore, cant fill up the dealer offer area

                    // check if we have room for all of each type
                    if ( ( ubDealerOfferAreaSlotsNeeded +
       CountNumberOfItemsInTheArmsDealersOfferArea() ) >
       SKI_NUM_TRADING_INV_SLOTS )
                    {
                            // tell player there's not enough room in the
       dealer's offer area DoSkiMessageBox(L"There is not enough room in the
       dealers repair area.", SHOPKEEPER_SCREEN, MSG_BOX_FLAG_OK, NULL);
                            return(FALSE);	// no room
                    }
    */

    if ((ubPlayerOfferAreaSlotsNeeded + CountNumberOfItemsInThePlayersOfferArea()) >
        SKI_NUM_TRADING_INV_SLOTS) {
      gfDisplayNoRoomMsg = TRUE;
      return (FALSE);  // no room
    }
    ...
    for (ubSubObject = 0; ubSubObject < MAX_SUBOBJECTS_PER_OBJECT; ubSubObject++) {
      ...
      if (gSubObject[ubSubObject].usItem != NONE) {
        ...
        if ((ubSubObject == 0) && (ubTotalSubObjects == 1)) {
          ...
          fSuccess = AddObjectForEvaluation(&gSubObject[ubSubObject], ubOwnerProfileId,
                                            bOwnerSlotId, fFirstOne);
        } else  // attachments, bullets/payload
        {
          ...
          // support & interface
          fSuccess =
              AddObjectForEvaluation(&gSubObject[ubSubObject], ubOwnerProfileId, -1, fFirstOne);
        }

        // it has to succeed, or we have a bug in our earlier check for
        // sufficient room
        Assert(fSuccess);
  ...
  // ARM: This comment isn't true unless parent/child support is added.  Right
  // now repairmen don't do this!
  // NOTE that, either way, if owned, the item remains in the merc's inventory
  // (hatched out) until the transaction is completed
  ...
  if (pComplexObject->ubNumberOfObjects == 1) {
    ...
    *pNextObj = *pComplexObject;
    ...
    if (Item[pComplexObject->usItem].usItemClass == IC_GUN) {
      // Exception: don't do this with rocket launchers, their "shots left" are
      // fake and this screws 'em up!
      if (pComplexObject->usItem != ROCKET_LAUNCHER) {
        pNextObj->ubGunShotsLeft = 0;
        pNextObj->usGunAmmoItem = NONE;
      }

      /* gunAmmoStatus is currently not being used that way, it's strictly used
         as a jammed/unjammed, and so should never be 0
    ...
    /* ARM: Can now repair with removeable attachments still attached...
                    ...
                    for( ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++ )
                    {
                            if ( pComplexObject->usAttachItem[ ubCnt ] != NONE )
                            {
                                    ...
                                    if (! (Item[ pComplexObject->usAttachItem[
       ubCnt ] ].fFlags & ITEM_INSEPARABLE ) )
                                    {
                                            pNextObj->usAttachItem[ ubCnt ] =
       NONE; pNextObj->bAttachStatus[ ubCnt ] = 0;
                                    }
                            }
                    }
    */
    ...
    /* ARM: Can now repair with removeable attachments still attached...
       ...
       ubCnt ] ].fFlags & ITEM_INSEPARABLE ) )
                                    {
                                            CreateItem(
       pComplexObject->usAttachItem[ ubCnt ], pComplexObject->bAttachStatus[
       ubCnt ], pNextObj );
  ...
  *pubNonRepairableSubObjects = 0;
  ...
  for (ubSubObject = 0; ubSubObject < MAX_SUBOBJECTS_PER_OBJECT; ubSubObject++) {
    ...
    if (gSubObject[ubSubObject].usItem != NONE) {
      ...
      // A jammed gun with a 100% status is NOT repairable - shouldn't ever
      // happen
...
static BOOLEAN AddObjectForEvaluation(OBJECTTYPE *pObject, uint8_t ubOwnerProfileId,
  ...
  if (bAddedToSlotID != -1) {
    // This will move any repairable (sub)objects into the dealer's offer area
    // immediately
...
// The Shopkeeper interface *MUST* use this intermediary function instead of
// calling AutoPlaceObject() directly! This is because the OBJECTTYPEs used
// within Shopkeeper may contain an illegal ubNumberOfObjects
static BOOLEAN ShopkeeperAutoPlaceObject(SOLDIERTYPE *pSoldier, OBJECTTYPE *pObject,
                                         BOOLEAN fNewItem) {
  OBJECTTYPE CopyOfObject;
  uint8_t ubObjectsLeftToPlace;

  // the entire pObj will get memset to 0 by RemoveObjs() if all the items are
  // successfully placed, so we have to keep a copy to retrieve with every
  ...
  while (ubObjectsLeftToPlace > 0) {
    ...
    if (!AutoPlaceObject(pSoldier, pObject, fNewItem)) {
      // no more room, didn't all fit - add back in any that we didn't even get
      // to yet
...
// The Shopkeeper interface *MUST* use this intermediary function instead of
// calling AddItemToPool() directly! This is because the OBJECTTYPEs used within
// Shopkeeper may contain an illegal ubNumberOfObjects
static void ShopkeeperAddItemToPool(int16_t const sGridNo, OBJECTTYPE *const pObject,
...
static void IfMercOwnedCopyItemToMercInv(const INVENTORY_IN_SLOT *pInv) {
  // if the item picked up was in a previous location, and that location is on a
  // merc's inventory
...
static void IfMercOwnedRemoveItemFromMercInv2(uint8_t ubOwnerProfileId, int8_t bOwnerSlotId);
...
static void IfMercOwnedRemoveItemFromMercInv2(uint8_t ubOwnerProfileId, int8_t bOwnerSlotId) {
  ...
  // if this item was in a previous location, and that location is on a merc's
  // inventory
...
static BOOLEAN SKITryToReturnInvToOwnerOrCurrentMerc(INVENTORY_IN_SLOT *pInv) {
  // don't use this if the item has a copy in merc's inventory!!  It would
  // create a duplicate!!
  ...
  if (pInv->ubIdOfMercWhoOwnsTheItem != NO_PROFILE) {
    ...
    // For owners of repaired items, this checks that owner is still hired, in
    // sector, on current squad, close enough to the shopkeeper, etc.
    if (!CanMercInteractWithSelectedShopkeeper(s)) return FALSE;

    // Try to find a place to put in its owner's inventory (regardless of which
    // merc is currently displayed!)
    if (SKITryToAddInvToMercsInventory(pInv, s)) return TRUE;

    // owner's inventory is full, so we'll try to give it to the current merc
    // instead
  }
  ...
  // failed to add item, either inventory was filled up, or nobody was even
...
static BOOLEAN SKITryToAddInvToMercsInventory(INVENTORY_IN_SLOT *pInv, SOLDIERTYPE *pSoldier) {
  ...
  if (Item[pInv->sItemIndex].usItemClass == IC_MONEY) {
    // search through the merc's inventory for a pocket of money which isn't
    // full already
    bMoneyInvPos = GetInvSlotOfUnfullMoneyInMercInventory(pSoldier);

    // if he has a money pocket going
    if (bMoneyInvPos != -1) {
      // try to add to it.  If successful, it will delete the object...  It
      // returns TRUE even if not all would fit!
...
BOOLEAN CanMercInteractWithSelectedShopkeeper(const SOLDIERTYPE *pSoldier) {
  ...
  const SOLDIERTYPE *const pShopkeeper =
      FindSoldierByProfileID(ArmsDealerInfo[gbSelectedArmsDealerID].ubShopKeeperID);
  ...
  if (pSoldier->bActive && pSoldier->bInSector && IsMercOnCurrentSquad(pSoldier) &&
      (pSoldier->bLife >= OKLIFE) && !IsMechanical(*pSoldier)) {
    ...
    sDistVisible = DistanceVisible(pSoldier, DIRECTION_IRRELEVANT, DIRECTION_IRRELEVANT,
                                   sDestGridNo, bDestLevel);

    // If he has LOS...
    if (SoldierTo3DLocationLineOfSightTest(pSoldier, sDestGridNo, bDestLevel, 3,
                                           (uint8_t)sDistVisible, TRUE)) {
      // Get range to shopkeeper
      uiRange = GetRangeFromGridNoDiff(pSoldier->sGridNo, sDestGridNo);

      // and is close enough to talk to the shopkeeper (use this define INSTEAD
      // of PASSING_ITEM_DISTANCE_OKLIFE!)
  ...
  if (!gfRemindedPlayerToPickUpHisStuff) {
    ...
    if (fPlayerOwnedStuffOnTable) {
      ...
      if (gfUserHasRequestedToLeave) {
        // has already asked to leave, shut the dealer up and allow the player
        // to leave
        ShutUpShopKeeper();
        return;
      }
...
static void DealWithItemsStillOnTheTable() {
  ...
  // in case we have have to drop stuff off at someone's feet, figure out where
  // it's all gonna go

  // use the current merc, unless he's ineligible, then use the selected merc
  // instead.
  if (!gfSMDisableForItems) {
  ...
  // loop through the players offer area and add return any items there to the
  // player
  for (ubCnt = 0; ubCnt < SKI_NUM_TRADING_INV_SLOTS; ubCnt++) {
    INVENTORY_IN_SLOT *const o = &PlayersOfferArea[ubCnt];
  ...
  // if the dealer repairs
  if (ArmsDealerInfo[gbSelectedArmsDealerID].ubTypeOfArmsDealer == ARMS_DEALER_REPAIRS) {
    // loop through the arms dealers' offer area and return any items there to
    // the player
    for (ubCnt = 0; ubCnt < SKI_NUM_TRADING_INV_SLOTS; ubCnt++) {
      INVENTORY_IN_SLOT *const a = &ArmsDealerOfferArea[ubCnt];
      // if there is an item here, give it back somehow
...
static void ReturnItemToPlayerSomehow(INVENTORY_IN_SLOT *pInvSlot, SOLDIERTYPE *pDropSoldier) {
  ...
  if (pInvSlot->bSlotIdInOtherLocation == -1) {
    // first try to give it to its owner, or if he's unavailable, the current
    // merc
    if (!SKITryToReturnInvToOwnerOrCurrentMerc(pInvSlot)) {
      // failed to add item, inventory probably filled up or item is unowned and
      // current merc ineligible. drop it at the specified guy's feet instead
...
static void GivePlayerSomeChange(uint32_t uiAmount) {
  ...
  if ((gbSelectedArmsDealerID == ARMS_DEALER_JAKE) &&
    ...
    // HACK HACK HACK: We forgot to write/record quote 27 for Jake, so he ALWAYS
    // must have enough money!
    gArmsDealerStatus[gbSelectedArmsDealerID].uiArmsDealersCash +=
        (uiAmount - gArmsDealerStatus[gbSelectedArmsDealerID].uiArmsDealersCash);
  }
...
static void DealerGetsBribed(uint8_t ubProfileId, uint32_t uiMoneyAmount) {
  ...
  switch (ubProfileId) {
    case FRANK:
      fBribable = TRUE;
      uiMinBribe = 10;  // This should match the handling of a bribe submitted
                        // outside the SKI, using the GIVE system
...
static void DelayRepairsInProgressBy(uint32_t uiMinutesDelayed);
static BOOLEAN RepairmanFixingAnyItemsThatShouldBeDoneNow(
    uint32_t *puiHoursSinceOldestItemRepaired);
...
static void HandlePossibleRepairDelays() {
  ...
  // because the quotes are so specific, we'll only use them once per game per
  // repairman
  if (status.fRepairDelayBeenUsed) return;

  // and it's been a while since the player last dealt with this repairman
  // (within SKI that is) this serves 2 purposes: a) reduces delays being much
  // more likely if player checks time remaining very frequently, AND b) gives
  // time for the events described in the text of the dealers' excuses to happen
  // (e.g. scouting trip)
  if (GetWorldTotalMin() - status.uiTimePlayerLastInSKI < 3 * 60) return;

  // if he should have been finished, but it's only been a few hours since then
  // (not days!)
  ...
  // then there's a fair chance he'll be delayed.  Use pre-chance to hopefully
...
static BOOLEAN RepairmanFixingAnyItemsThatShouldBeDoneNow(
  ...
  *puiHoursSinceOldestItemRepaired = 0;
  ...
  // loop through the dealers inventory and check if there are only unrepaired
  // items
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
        ...
        if (pSpecialItem->fActive) {
          ...
          if (pSpecialItem->Info.bItemCondition < 0) {
            ...
            if (pSpecialItem->uiRepairDoneTime <= GetWorldTotalMin()) {
              ...
              // calculate how many WORKING hours since this item's been
              // repaired
              uiWorkingHoursSinceThisItemRepaired =
                  (uiMinutesSinceItWasDone - uiMinutesShopClosedSinceItWasDone) / 60;

              // we need to determine how long it's been since the item that's
              // been repaired for the longest time was done
  ...
  // if FALSE returned here, he's either not repairing anything, or none of it's
  // done yet
  return (fFoundOne);
}
        ...
        if (pSpecialItem->fActive) {
          ...
          if (pSpecialItem->Info.bItemCondition < 0) {
            uiMinutesShopClosedBeforeItsDone = CalculateOvernightRepairDelay(
                gbSelectedArmsDealerID, pSpecialItem->uiRepairDoneTime, uiMinutesDelayed);
...
// Mouse Call back for the Arms delaers face
static void SelectArmsDealersDropItemToGroundRegionCallBack(MOUSE_REGION *pRegion,
                                                            int32_t iReason) {
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    SOLDIERTYPE *pDropSoldier;
    ...
    if (gMoveingItem.sItemIndex != 0) {
      ...
      ShopkeeperAddItemToPool(pDropSoldier->sGridNo, &gMoveingItem.ItemObject,
                              pDropSoldier->bLevel);
...
static BOOLEAN CanTheDropItemToGroundStringBeDisplayed();
...
static void DisplayTheSkiDropItemToGroundString() {
  ...
  usHeight =
      DisplayWrappedString(SKI_DROP_ITEM_TO_GROUND_START_X, SKI_DROP_ITEM_TO_GROUND_TEXT_START_Y,
                           SCREEN_WIDTH - SKI_DROP_ITEM_TO_GROUND_START_X, 2, SKI_LABEL_FONT,
                           SKI_TITLE_COLOR, SKI_Text[SKI_TEXT_DROP_ITEM_TO_GROUND],
                           FONT_MCOLOR_BLACK, CENTER_JUSTIFIED | DONT_DISPLAY_TEXT);

  // display the 'drop item to ground' text
  DisplayWrappedString(SKI_DROP_ITEM_TO_GROUND_START_X,
                       SKI_DROP_ITEM_TO_GROUND_TEXT_START_Y - usHeight,
                       SCREEN_WIDTH - SKI_DROP_ITEM_TO_GROUND_START_X, 2, SKI_LABEL_FONT,
                       SKI_TITLE_COLOR, SKI_Text[SKI_TEXT_DROP_ITEM_TO_GROUND], FONT_MCOLOR_BLACK,
...
static uint32_t EvaluateInvSlot(INVENTORY_IN_SLOT *pInvSlot) {
  ...
  if (gbSelectedArmsDealerID == ARMS_DEALER_MICKY) {
    const SOLDIERTYPE *const s =
        FindSoldierByProfileIDOnPlayerTeam(ArmsDealerInfo[gbSelectedArmsDealerID].ubShopKeeperID);
  ...
  uiBuyingPrice = CalcShopKeeperItemPrice(DEALER_BUYING, FALSE, pInvSlot->sItemIndex,
                                          dPriceModifier, &(pInvSlot->ItemObject));
  ...
  if (uiBuyingPrice > 0) {
    ...
    // he normally buys this, but it's in such bad shape that he won't, it's
    // worthless (different quote)
...
static void BuildRepairTimeString(wchar_t sString[], size_t Length,
                                  uint32_t uiTimeInMinutesToFixItem) {
  uint16_t usNumberOfHoursToFixItem = 0;
  ...
  if ((uiTimeInMinutesToFixItem % REPAIR_MINUTES_INTERVAL) > 0) {
    uiTimeInMinutesToFixItem +=
        REPAIR_MINUTES_INTERVAL - (uiTimeInMinutesToFixItem % REPAIR_MINUTES_INTERVAL);
  }
  ...
  if (uiTimeInMinutesToFixItem <= 90) {
    ...
    usNumberOfHoursToFixItem = (uint16_t)((uiTimeInMinutesToFixItem + 45) / 60);
...
static void BuildDoneWhenTimeString(wchar_t sString[], size_t const Length,
                                    ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex,
                                    uint8_t const ubElement) {
  uint32_t uiDoneTime;
  ...
  Assert(
      gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].Info.bItemCondition <
      0);
  ...
  uiDoneTime =
      gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].uiRepairDoneTime;
...
static void HatchOutInvSlot(uint16_t usPosX, uint16_t usPosY) {
  ...
  const uint16_t usSlotWidth = SKI_INV_SLOT_WIDTH;
  const uint16_t usSlotHeight = SKI_INV_SLOT_HEIGHT;
