...
SOLDIERTYPE *TacticalCreateSoldier(SOLDIERCREATE_STRUCT const &c) try {
  ...
  if (guiCurrentScreen == AUTORESOLVE_SCREEN) {
    ...
    id = team.bFirstID;
    // ATE: If we are a vehicle, and a player, start at a different slot (2 -
    // max)
    if (team_id == OUR_TEAM) {
    ...
    uint8_t const last_id = team.bLastID;
    for (;;) {
      s = &GetMan(id);
      if (!s->bActive) break;
  ...
  // Some values initialized here but could be changed before going to the
  // common one
  InitSoldierStruct(*s);
  s->ubID = id;
  ...
  if (team_id == CIV_TEAM) {
    s->bNeutral = profile == WARDEN ? FALSE
  ...
  if (profile == NO_PROFILE && team_id == ENEMY_TEAM && s->ubBodyType == REGFEMALE &&
  ...
  if (team_id != OUR_TEAM) {
    ...
    for (int32_t i = BIGPOCK1POS; i <= BIGPOCK4POS; ++i) {
      OBJECTTYPE &o = s->inv[i];
      if (!(Item[o.usItem].usItemClass & IC_FACE)) continue;

      if (!second_face_item) { /* Don't check for compatibility, automatically
                                * assume there are no head positions filled. */
     ...
     * camouflaged */
    if (guiCurrentScreen != AUTORESOLVE_SCREEN && team_id == ENEMY_TEAM &&
        FindObj(s, CAMOUFLAGEKIT) != NO_SLOT &&
        Random(5) < SoldierDifficultyLevel(s)) {  // Start camouflaged
      s->bCamo = 100;
    }
  }
  ...
  switch (s->ubBodyType) {
    ...
    case TANK_NE: {
      ...
      s->bVehicleID = c.fUseGivenVehicle ? c.bUseGivenVehicleID
                                         : (int8_t)AddVehicleToList(s->sSectorX, s->sSectorY,
                                                                    s->bSectorZ, ubVehicleID);
  ...
  if (guiCurrentScreen == AUTORESOLVE_SCREEN) {
    uint8_t const sector_id = GetAutoResolveSectorID();
...
SOLDIERTYPE *TacticalCreateSoldierFromExisting(const SOLDIERTYPE *const existing) {
  ...
  if (s.ubProfile != NO_PROFILE && s.bTeam == OUR_TEAM && !(s.uiStatusFlags & SOLDIER_VEHICLE)) {
    InitSoldierFace(s);
  }
...
static void TacticalCopySoldierFromProfile(SOLDIERTYPE &s, SOLDIERCREATE_STRUCT const &c) {
  ...
  if (HAS_SKILL_TRAIT(&s,
                      CAMOUFLAGED)) {  // Set camouflaged to 100 automatically
    s.bCamo = 100;
  }
}
...
static int32_t ChooseHairColor(SOLDIERTYPE *pSoldier, int32_t skin) {
  int32_t iRandom;
  int32_t hair = 0;
  iRandom = Random(100);
  switch (skin) {
...
static void GeneratePaletteForSoldier(SOLDIERTYPE *pSoldier, uint8_t ubSoldierClass) {
  int32_t skin, hair;
  ...
  // merc clothing scheme is much larger and general and is an exclusive
  // superset of the civilian clothing scheme which means the civilians will
  // choose the merc clothing scheme often ( actually 60% of the time ).
  if (!pSoldier->PantsPal[0] || !pSoldier->VestPal[0]) {
    ...
    {
      if (Random(100) < 30) {  // 30% chance that the civilian will choose a
                               // gaudy yellow shirt with pants.
...
static void TacticalCopySoldierFromCreateStruct(SOLDIERTYPE &s, SOLDIERCREATE_STRUCT const &c) {
  ...
  switch (s.ubSoldierClass) {
    int32_t chance;
    uint8_t expert_progess;
    int32_t expert_chance;
    ...
    case SOLDIER_CLASS_REG_MILITIA:
      ...
      expert_chance = 20;
    maybe_nightops:
      uint8_t const progress = HighestPlayerProgressPercentage();
  ...
  switch (c.bTeam) {
    case ENEMY_TEAM:
      name = TacticalStr[ENEMY_TEAM_MERC_NAME];
      break;
    case MILITIA_TEAM:
      name = TacticalStr[MILITIA_TEAM_MERC_NAME];
      break;

    case CIV_TEAM:
      name = s.ubSoldierClass == SOLDIER_CLASS_MINER ? TacticalStr[CIV_TEAM_MINER_NAME]
             : s.ubBodyType == CROW                  ? TacticalStr[CROW_HIT_LOCATION_STR]
                                                     : TacticalStr[CIV_TEAM_MERC_NAME];
      break;

    case CREATURE_TEAM:
      name = s.ubBodyType == BLOODCAT ? gzLateLocalizedString[STR_LATE_36]
                                      : TacticalStr[CREATURE_TEAM_MERC_NAME];
      break;

    default:
      goto no_name;  // XXX fishy
  }
...
void InternalTacticalRemoveSoldier(SOLDIERTYPE &s, BOOLEAN const fRemoveVehicle) {
  ...
  if (guiCurrentScreen != AUTORESOLVE_SCREEN) {
    ...
    }  // people specified off-map have already been removed from their team
       // count
...
void TacticalRemoveSoldier(SOLDIERTYPE &s) { InternalTacticalRemoveSoldier(s, TRUE); }

static uint8_t GetLocationModifier(uint8_t ubSoldierClass);

// returns a soldier difficulty modifier from 0 to 100 based on player's
// progress, distance from the Palace, mining income, and playing difficulty
// level.  Used for generating soldier stats, equipment, and AI skill level.
int8_t CalcDifficultyModifier(uint8_t ubSoldierClass) {
  ...
  // bump it a bit once we've accomplished something (killed some enemies or
  // taken an important sector)
  if (ubProgress > 0) {
    // +5
  ...
  // should be no way to go over 100, although it's possible to go below 0 when
  ...
  // CreateDetailedPlacementGivenBasicPlacementInfo(), and
  // SoldierDifficultyLevel().

  return (bDiffModifier);
}

static void ReduceHighExpLevels(int8_t *pbExpLevel);
...
void CreateDetailedPlacementGivenBasicPlacementInfo(SOLDIERCREATE_STRUCT *pp,
  ...
  // experience level is modified by game difficulty, player's progress, &
  // proximity to Queen's Palace, etc. This formula gives the following results:
  //	DIFFICULTY FACTOR			EXP. LEVEL  MODIFIER		LEVEL OF AVG
  // REGULAR TROOP 			   0 to 19
  //-2
  // 2
  //			  20 to 39 -1
  // 3
  ...
  // if in the upper half of this difficulty rating (10-19, 30-39, 50-59, 70-79,
  // and 90-99)
  ...
  // Adjust level and statistics for Linda's prespecified relative attribute
  // level
  switch (bp->bRelativeAttributeLevel) {
    // NOTE: bStatsModifier already includes the bExpLevelModifier since it's
    // based on the level itself!
    case 0:
      bExpLevelModifier += -1;
      bStatsModifier += -1;
      break;
  ...
  // Set the experience level based on the soldier class and exp level modifier
  // or relative attribute level
  switch (ubSoldierClass) {
    case SOLDIER_CLASS_ADMINISTRATOR:
  ...
  // Stat range is currently 49-99, bell-curved around a range of 16 values
  // dependent on the stats level
  ...
  // If it is a detailed placement, don't do this yet, as detailed placements
...
void CreateStaticDetailedPlacementGivenBasicPlacementInfo(SOLDIERCREATE_STRUCT *spp,
  ...
  // The static placement needs to be later *regenerated* in order to create a
  // valid soldier.
  spp->bExpLevel = -1;
  spp->bLifeMax = -1;
  spp->bLife = -1;
...
// slot, this function will generate the proper detailed placement slot given the
// static detailed placement and it's accompanying basic placement. For the
...
void CreateDetailedPlacementGivenStaticDetailedPlacementAndBasicPlacementInfo(
  ...
  // This isn't perfect, however, it blindly brings over the items from the
  // static detailed placement.  Due to the order of things, other items would
  for (i = 0; i < NUM_INV_SLOTS; i++) {
    // copy over static items and empty slots that are droppable (signifies a
    // forced empty slot)
...
static void UpdateStaticDetailedPlacementWithProfileInformation(SOLDIERCREATE_STRUCT *spp,
                                                                uint8_t ubProfile);
...
void UpdateSoldierWithStaticDetailedInformation(SOLDIERTYPE *s, SOLDIERCREATE_STRUCT *spp) {
  // First, check to see if the soldier has a profile.  If so, then it'll
  // extract the information and update the soldier with the profile information
  // instead.  This has complete override authority.
  ...
  if (spp->bExpLevel != -1) {  // We have a static experience level, so generate
                               // all of the soldier's attributes.
    int8_t bBaseAttribute;
  ...
  // Replace any soldier attributes with any static values in the detailed
  // placement.
...
// In the case of setting a profile ID in order to extract a soldier from the
// profile array, we also want to copy that information to the static detailed
// placement, for editor viewing purposes.
static void UpdateStaticDetailedPlacementWithProfileInformation(SOLDIERCREATE_STRUCT *spp,
                                                                uint8_t ubProfile) {
  uint32_t cnt;
...
// When the editor modifies the soldier's relative attribute level,
// this function is called to update that information.
void ModifySoldierAttributesWithNewRelativeLevel(SOLDIERTYPE *s, int8_t bRelativeAttributeLevel) {
  int8_t bBaseAttribute;
  // Set the experience level based on the relative attribute level
  // NOTE OF WARNING: THIS CURRENTLY IGNORES THE ENEMY CLASS (ADMIN/REG/ELITE)
  // FOR CALCULATING LEVEL & ATTRIBUTES
...
static SOLDIERTYPE *ReserveTacticalSoldierForAutoresolve(uint8_t ubSoldierClass) try {
  // This code looks for a soldier of specified type that currently exists in
  // tactical and returns the pointer to that soldier.  This is used when copying
  // the exact status of all remaining enemy troops (or creatures) to finish the
  // battle in autoresolve.  To signify that the troop has already been reserved,
  // we simply set their gridno to NOWHERE.
  const int8_t team = (ubSoldierClass == SOLDIER_CLASS_CREATURE ? CREATURE_TEAM : ENEMY_TEAM);
...
SOLDIERTYPE *TacticalCreateEnemySoldier(SoldierClass const sc) {
  ...
  if (sc == SOLDIER_CLASS_ELITE) { /* SPECIAL! Certain events in the game can
                                    * cause profiled NPCs to become enemies. The
                                    * two cases are adding Mike and Iggy. We
                                    * will only add one NPC in any given combat
                                    * and the conditions for setting the
                                    * associated facts are done elsewhere. There
                                    * is also another place where NPCs can get
                                    * added, which is in AddPlacementToWorld()
                                    * used for inserting defensive enemies.
                                    * NOTE: We don't want to add Mike or Iggy if
                                    * this is being called from autoresolve! */
    OkayToUpgradeEliteToSpecialProfiledEnemy(&pp);
  }

  SOLDIERTYPE *const s = TacticalCreateSoldier(pp);
  if (s) {  // Send soldier to centre of map, roughly
    s->sNoiseGridno = CENTRAL_GRIDNO + (Random(CENTRAL_RADIUS * 2 + 1) - CENTRAL_RADIUS) +
...
void RandomizeRelativeLevel(int8_t *pbRelLevel, uint8_t ubSoldierClass) {
  ...
  // We now adjust the relative level by location on the map, so enemies in NE
  // corner will be generally very crappy (lots of bad and poor, with avg about
  // best), while enemies in the SW will have lots of great and good, with avg
  // about as lousy as it gets.  Militia are generally unmodified by distance,
  // and never get bad or great at all.
...
static void CopyProfileItems(SOLDIERTYPE &s, SOLDIERCREATE_STRUCT const &c) {
  ...
  if (s.bTeam != OUR_TEAM) {
    for (uint32_t i = 0; i != NUM_INV_SLOTS; ++i) {
      uint16_t const item = p.inv[i];
      OBJECTTYPE *const slot = &s.inv[i];
      if (item != NOTHING) {
        uint8_t const count = p.bInvNumber[i];
        if (Item[item].usItemClass == IC_KEY) {
          ...
          if (KEY_1 <= item && item <= KEY_32) {
            switch (c.ubProfile) {
              case BREWSTER:
                CreateKeyObject(slot, count, 19);
                break;
              case SKIPPER:
                CreateKeyObject(slot, count, 11);
                break;
              case DOREEN:
                CreateKeyObject(slot, count, 32);
                break;
              default:
    ...
    for (uint32_t money_left = p.uiMoney; money_left > 0;) {
      int8_t const slot_id = FindEmptySlotWithin(&s, BIGPOCK1POS, SMALLPOCK8POS);
      if (slot_id == NO_SLOT) break;
      OBJECTTYPE *const slot = &s.inv[slot_id];

      uint32_t const slot_limit = MoneySlotLimit(slot_id);
      uint32_t const slot_amount = std::min(money_left, slot_limit);
...
void TrashAllSoldiers() { FOR_EACH_SOLDIER(i) TacticalRemoveSoldier(*i); }
...
static uint8_t GetLocationModifier(uint8_t ubSoldierClass) {
  ...
  // ignore sSectorZ - treat any underground enemies as if they were on the
  // surface!
  switch (GetTownIdForSector(SECTOR(sSectorX, sSectorY))) {
    case ORTA:
    case TIXA:
      // enemy troops in these special places are stronger than geography would
      // indicate
      ubPalaceDistance = 4;
      break;
...
static void ReduceHighExpLevels(int8_t *const exp_level) {
  int8_t max_level;
  switch (gGameOptions.ubDifficultyLevel) {
    case DIF_LEVEL_EASY:
      max_level = 6;
  ...
  uint32_t const chance_max_lvl = (*exp_level - max_level + 1) * 25;
