...
//		The second is the amount of the items the dealer will try to keep
// in his inventory
...
static DEALER_POSSIBLE_INV const gKeithInventory[] = {
    ...
    // WARNING: Keith must not carry any guns, it would conflict with his
    // story/quest
...
static DEALER_POSSIBLE_INV const gJakeInventory[] = {
    ...
    // stuff a real pawn shop wouldn't have, but it does make him a bit more
    // useful
    {COMPOUND18, 1},
    {CERAMIC_PLATES, 1},
...
DEALER_POSSIBLE_INV const *GetPointerToDealersPossibleInventory(ArmsDealerID const ubArmsDealerID) {
  switch (ubArmsDealerID) {
    ...
    default:
      throw std::logic_error("Invalid arms dealer ID");
  }
}
...
static uint8_t GetCurrentSuitabilityForItem(ArmsDealerID const bArmsDealer,
  ...
  // become more and more infrequent later in the game, although they never
  // quite vanish.

  // items illegal in this game are unsuitable [this checks guns vs. current
  // GunSet!]
  ...
  if (ubItemCoolness == 0) {
    // items without a coolness rating can't be sold to the player by
    // shopkeepers
    return (ITEM_SUITABILITY_NONE);
  }

  // the following staple items are always deemed highly suitable regardless of
  // player's progress:
  switch (usItemIndex) {
    case CLIP38_6:
    case CLIP9_15:
  ...
  // figure out the appropriate range of coolness based on player's maximum
  // progress so far
  ...
  if (bArmsDealer == ARMS_DEALER_TONY) {
    ...
    // almost everything Devin sells is pretty cool (4+), so gotta apply a
    // minimum or he'd have nothing early on
    if (ubMinCoolness < 3) {
      ubMinCoolness = 3;
      ubMaxCoolness = 4;
    }
  ...
  // item is way uncool for player's current progress, but it's still possible
...
uint8_t ChanceOfItemTransaction(ArmsDealerID const bArmsDealer, uint16_t const usItemIndex,
  ...
  switch (GetCurrentSuitabilityForItem(bArmsDealer, usItemIndex)) {
    case ITEM_SUITABILITY_NONE:
      if (fDealerIsSelling) {
        // dealer always gets rid of stuff that is too advanced or inappropriate
        // ASAP
        ubChance = 100;
...
uint8_t HowManyItemsToReorder(uint8_t ubWanted, uint8_t ubStillHave) {
  ...
  // randomize the amount. 33% of the time we add to it, 33% we subtract from
  // it, rest leave it alone
  switch (Random(3)) {
    case 0:
