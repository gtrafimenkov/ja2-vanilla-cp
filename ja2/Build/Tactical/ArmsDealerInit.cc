...
// To reduce memory fragmentation from frequent MemRealloc(), we allocate memory
// for more than one special slot each time we run out of space.  Odds are that
// if we need one, we'll need another soon.
#define SPECIAL_ITEMS_ALLOCED_AT_ONCE 3
// Once allocated, the special item slots remain allocated for the duration of
// the game, or until the dealer dies. This is a little bit wasteful, but saves
// an awful lot of hassles, and avoid unnecessary memory fragmentation
...
uint8_t gubLastSpecialItemAddedAtElement = 255;
...
const ARMS_DEALER_INFO ArmsDealerInfo[NUM_ARMS_DEALERS] = {
    ...
    {{{0.8f, 1.1f}},
     ...
     ARMS_DEALER_ONLY_USED_ITEMS | ARMS_DEALER_GIVES_CHANGE},
    /* Gabby Mulnick   */
    {{{1.0f, 1.0f}}, GABBY, ARMS_DEALER_BUYS_SELLS, 3000, ARMS_DEALER_GIVES_CHANGE},

    /* Devin Connell   */
    {{{0.75f, 1.25f}}, DEVIN, ARMS_DEALER_SELLS_ONLY, 5000, ARMS_DEALER_GIVES_CHANGE},
    /* Howard Filmore  */
    {{{1.0f, 1.0f}}, HOWARD, ARMS_DEALER_SELLS_ONLY, 3000, ARMS_DEALER_GIVES_CHANGE},
    /* Sam Rozen       */
    {{{1.0f, 1.0f}}, SAM, ARMS_DEALER_SELLS_ONLY, 3000, ARMS_DEALER_GIVES_CHANGE},
    /* Frank           */
    {{{1.0f, 1.0f}}, FRANK, ARMS_DEALER_SELLS_ONLY, 500, ARMS_DEALER_ACCEPTS_GIFTS},

    /* Bar Bro 1       */
    {{{1.0f, 1.0f}}, HERVE, ARMS_DEALER_SELLS_ONLY, 250, ARMS_DEALER_ACCEPTS_GIFTS},
    /* Bar Bro 2       */
    {{{1.0f, 1.0f}}, PETER, ARMS_DEALER_SELLS_ONLY, 250, ARMS_DEALER_ACCEPTS_GIFTS},
    /* Bar Bro 3       */
    {{{1.0f, 1.0f}}, ALBERTO, ARMS_DEALER_SELLS_ONLY, 250, ARMS_DEALER_ACCEPTS_GIFTS},
    /* Bar Bro 4       */
    {{{1.0f, 1.0f}}, CARLO, ARMS_DEALER_SELLS_ONLY, 250, ARMS_DEALER_ACCEPTS_GIFTS},
    ...
    /* Elgin           */
    {{{1.0f, 1.0f}}, DRUGGIST, ARMS_DEALER_SELLS_ONLY, 500, ARMS_DEALER_ACCEPTS_GIFTS},
    /* Manny           */
    {{{1.0f, 1.0f}}, MANNY, ARMS_DEALER_SELLS_ONLY, 500, ARMS_DEALER_ACCEPTS_GIFTS}};
...
void InitAllArmsDealers() {
  ...
  for (ArmsDealerID ubArmsDealer = ARMS_DEALER_FIRST; ubArmsDealer < NUM_ARMS_DEALERS;
       ++ubArmsDealer) {
...
static void ArmsDealerGetsFreshStock(ArmsDealerID, uint16_t usItemIndex, uint8_t ubNumItems);
...
static void InitializeOneArmsDealer(ArmsDealerID const ubArmsDealer) {
  ...
  // if the arms dealer isn't supposed to have any items (includes all
  // repairmen)
  ...
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (CanDealerTransactItem(ubArmsDealer, usItemIndex, FALSE)) {
      // Setup an initial amount for the items (treat items as new, how many are
      // used isn't known yet)
      ubNumItems = DetermineInitialInvItems(
          ubArmsDealer, usItemIndex, GetDealersMaxItemAmount(ubArmsDealer, usItemIndex), FALSE);
...
void SaveArmsDealerInventoryToSaveGameFile(HWFILE const f) {
  ...
  for (DEALER_ITEM_HEADER const(*dealer)[MAXITEMS] = gArmsDealersInventory;
       dealer != endof(gArmsDealersInventory); ++dealer) {
  ...
  for (ArmsDealerID dealer = ARMS_DEALER_FIRST; dealer != NUM_ARMS_DEALERS; ++dealer) {
    for (uint16_t item_idx = 1; item_idx != MAXITEMS; ++item_idx) {
...
static void AllocMemsetSpecialItemArray(DEALER_ITEM_HEADER *, uint8_t ubElementsNeeded);

void LoadArmsDealerInventoryFromSavedGameFile(HWFILE const f, uint32_t const savegame_version) {
  // Free all the dealers special inventory arrays
  ShutDownArmsDealers();

  // Elgin was added to the dealers list in Game Version #54, enlarging these 2
  // tables Manny was added to the dealers list in Game Version #55, enlarging
  // these 2 tables
  uint32_t const n_dealers_saved = savegame_version < 54 ? NUM_ARMS_DEALERS - 2
                                                         :  // without Elgin and Manny
                                       savegame_version < 55 ? NUM_ARMS_DEALERS - 1
                                                             :  // without Manny
                                       NUM_ARMS_DEALERS;

  FileRead(f, gArmsDealerStatus, n_dealers_saved * sizeof(*gArmsDealerStatus));

  for (DEALER_ITEM_HEADER(*dealer)[MAXITEMS] = gArmsDealersInventory;
       dealer != gArmsDealersInventory + n_dealers_saved; ++dealer) {
...
static void SimulateArmsDealerCustomer() {
  ...
  for (ArmsDealerID ubArmsDealer = ARMS_DEALER_FIRST; ubArmsDealer < NUM_ARMS_DEALERS;
    ...
    // if the arms dealer isn't supposed to have any items (includes all
    // repairmen)
    if (ArmsDealerInfo[ubArmsDealer].uiFlags & ARMS_DEALER_HAS_NO_INVENTORY) continue;
    ...
    for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
      ...
      if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems > 0) {
        ...
        if (usItemIndex == JAR_ELIXIR) {
          // only allow selling of standard # of items so those converted from
          ...
          ubItemsSold = HowManyItemsAreSold(
              ubArmsDealer, usItemIndex,
              gArmsDealersInventory[ubArmsDealer][usItemIndex].ubPerfectItems, FALSE);
        }
        if (ubItemsSold > 0) {
        ...
        // next, try to sell all the used ones, gotta do these one at a time so
        // we can remove them by element
        for (ubElement = 0;
             ubElement < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced;
             ubElement++) {
          // don't worry about negative condition, repairmen can't come this
          // far, they don't sell!
          if (gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].fActive) {
            ...
            if (HowManyItemsAreSold(ubArmsDealer, usItemIndex, 1, TRUE) > 0) {
              ...
              RemoveSpecialItemFromArmsDealerInventoryAtElement(ubArmsDealer, usItemIndex,
                                                                ubElement);
            }
          }
        }
      ...
      if (CanDealerTransactItem(ubArmsDealer, usItemIndex, FALSE)) {
        ...
        if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubQtyOnOrder == 0) {
          ...
          if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems <=
            ...
            // determine if the item can be restocked (assume new, use items
            // aren't checked for until the stuff arrives)
            if (ItemTransactionOccurs(ubArmsDealer, usItemIndex, DEALER_BUYING, FALSE)) {
              // figure out how many items to reorder (items are reordered an
              // entire batch at a time)
              ubNumItems = HowManyItemsToReorder(
                  ubMaxSupply, gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems);

              // if this is the first day the player is eligible to have access
              // to this thing
              if (!fPrevElig) {
                ...
                if ((ubArmsDealer == ARMS_DEALER_TONY) || (ubArmsDealer == ARMS_DEALER_DEVIN)) {
                  // the stuff Tony and Devin sell is imported, so it takes
                  // longer to arrive (for game balance)
                  ubReorderDays = (uint8_t)(2 + Random(2));  // 2-3 days
                } else {
                  ubReorderDays = (uint8_t)(1 + Random(2));  // 1-2 days
                }
...
static void ConvertCreatureBloodToElixir() {
  uint8_t ubBloodAvailable;
  uint8_t ubAmountToConvert;
  SPECIAL_ITEM_INFO SpclItemInfo;
  ...
  if (ubBloodAvailable) {
    ...
    // ubAmountToConvert = (uint8_t) std::min( 5 + Random( 3 ), ubBloodAvailable );
    ...
    // Now remove that many NEW ones (condition 100) of that item
    RemoveItemFromArmsDealerInventory(ARMS_DEALER_GABBY, JAR_CREATURE_BLOOD, &SpclItemInfo,
                                      ubAmountToConvert);
...
static void AddItemToArmsDealerInventory(ArmsDealerID, uint16_t usItemIndex,
                                         SPECIAL_ITEM_INFO *pSpclItemInfo, uint8_t ubHowMany);
static void GuaranteeAtLeastOneItemOfType(ArmsDealerID, uint32_t uiDealerItemType);
static void GuaranteeMinimumAlcohol(ArmsDealerID);
static void LimitArmsDealersInventory(ArmsDealerID, uint32_t uiDealerItemType,
                                      uint8_t ubMaxNumberOfItemType);
...
static void AdjustCertainDealersInventory() {
  ...
  // If the player hasn't bought a video camera from Franz yet, make sure Franz
  // has one to sell
  if (!(gArmsDealerStatus[ARMS_DEALER_FRANZ].ubSpecificDealerFlags &
        ARMS_DEALER_FLAG__FRANZ_HAS_SOLD_VIDEO_CAMERA_TO_PLAYER)) {
    GuaranteeAtLeastXItemsOfIndex(ARMS_DEALER_FRANZ, VIDEO_CAMERA, 1);
  }
}

static uint32_t GetArmsDealerItemTypeFromItemNumber(uint16_t usItem);
static void RemoveRandomItemFromArmsDealerInventory(ArmsDealerID, uint16_t usItemIndex,
                                                    uint8_t ubHowMany);

static void LimitArmsDealersInventory(ArmsDealerID const ubArmsDealer, uint32_t uiDealerItemType,
                                      uint8_t ubMaxNumberOfItemType) {
  uint16_t usItemIndex = 0;
  uint32_t uiItemsToRemove = 0;
  SPECIAL_ITEM_INFO SpclItemInfo;

  uint16_t usAvailableItem[MAXITEMS] = {NOTHING};
  uint8_t ubNumberOfAvailableItem[MAXITEMS] = {0};
  uint32_t uiTotalNumberOfItems = 0, uiRandomChoice;
  uint32_t uiNumAvailableItems = 0, uiIndex;

  // not permitted for repair dealers - would take extra code to avoid counting
  // items under repair!
    ...
    do {
      ...
      for (uiIndex = 0; uiIndex < uiNumAvailableItems; uiIndex++) {
        if (uiRandomChoice <= ubNumberOfAvailableItem[uiIndex]) {
          ...
          if (uiDealerItemType == ARMS_DEALER_AMMO) {
            // remove all of them, since each ammo item counts as only one
            // "item" here create item info describing a perfect item
            SetSpecialItemInfoToDefaults(&SpclItemInfo);
            // ammo will always be only condition 100, there's never any in
            // special slots
            RemoveItemFromArmsDealerInventory(
                ubArmsDealer, usItemIndex, &SpclItemInfo,
                gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems);
          } else {
            // pick 1 random one, don't care about its condition
            RemoveRandomItemFromArmsDealerInventory(ubArmsDealer, usItemIndex, 1);
          }
          // now remove entry from the array by replacing it with the last and
      ...
      for( usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++ )
      {
              ...
              if( gArmsDealersInventory[ ubArmsDealer ][ usItemIndex
      ].ubTotalItems )
              {
                      //if the item is of the same dealer item type
                      if( uiDealerItemType &
      GetArmsDealerItemTypeFromItemNumber( usItemIndex ) )
                      {
                              ...
                              if( Random( 100 ) < 30 )
                              {
                                      ...
                                      if( uiDealerItemType == ARMS_DEALER_AMMO )
                                      {
                                              // remove all of them, since each
      ammo item counts as only one "item" here

                                              // create item info describing a
      perfect item SetSpecialItemInfoToDefaults( &SpclItemInfo );
                                              // ammo will always be only
      condition 100, there's never any in special slots
                                              RemoveItemFromArmsDealerInventory(
      ubArmsDealer, usItemIndex, &SpclItemInfo, gArmsDealersInventory[
      ubArmsDealer ][ usItemIndex ].ubTotalItems );
                                      }
                                      else
                                      {
                                              // pick 1 random one, don't care
      about its condition RemoveRandomItemFromArmsDealerInventory( ubArmsDealer,
      usItemIndex, 1 );
                                      }
...
static void GuaranteeAtLeastOneItemOfType(ArmsDealerID const ubArmsDealer,
                                          uint32_t uiDealerItemType) {
  uint16_t usItemIndex;
  uint8_t ubChance;
  uint16_t usAvailableItem[MAXITEMS] = {NOTHING};
  uint8_t ubChanceForAvailableItem[MAXITEMS] = {0};
  ...
  // if there aren't any such items, the following loop would never finish, so
  // quit before trying it!
  ...
  // randomize number within uiTotalChances and then loop forwards till we find
  // that item
...
void GuaranteeAtLeastXItemsOfIndex(ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex,
  ...
  // if he can stock it (it appears in his inventory list)
  // RESTRICTION REMOVED: Jake must be able to guarantee GAS even though it's
  // not in his list, it's presence is conditional
  //	if( GetDealersMaxItemAmount( ubArmsDealer, usItemIndex ) > 0)
  {
    // add the item
    ArmsDealerGetsFreshStock(
        ubArmsDealer, usItemIndex,
        (uint8_t)(ubHowMany - gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems));
  }
}
  ...
  switch (Item[usItem].usItemClass) {
    ...
    case IC_GUN:
      switch (Weapon[Item[usItem].ubClassIndex].ubWeaponClass) {
        ...
        case RIFLECLASS:
          return ItemIsARocketRifle(usItem) ? ARMS_DEALER_ROCKET_RIFLE : ARMS_DEALER_RIFLECLASS;
    ...
    default:
      AssertMsg(FALSE, String("GetArmsDealerItemTypeFromItemNumber(), invalid "
                              "class %d for item %d.  DF 0.",
                              Item[usItem].usItemClass, usItem));
      break;
  }
  return (0);
}
...
BOOLEAN IsMercADealer(uint8_t ubMercID) {
  ...
  // Manny is not actually a valid dealer unless a particular event sets that
  // fact
...
ArmsDealerID GetArmsDealerIDFromMercID(uint8_t const ubMercID) {
  // loop through the list of arms dealers
...
BOOLEAN RepairmanIsFixingItemsButNoneAreDoneYet(uint8_t ubProfileID) {
  ...
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (gArmsDealersInventory[bArmsDealer][usItemIndex].ubTotalItems) {
      ...
      for (ubElement = 0;
           ubElement < gArmsDealersInventory[bArmsDealer][usItemIndex].ubElementsAlloced;
           ubElement++) {
...
static BOOLEAN DoesItemAppearInDealerInventoryList(ArmsDealerID, uint16_t usItemIndex,
                                                   BOOLEAN fPurchaseFromPlayer);

BOOLEAN CanDealerTransactItem(ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex,
                              BOOLEAN const fPurchaseFromPlayer) {
  switch (ArmsDealerInfo[ubArmsDealer].ubTypeOfArmsDealer) {
    case ARMS_DEALER_SELLS_ONLY:
      if (fPurchaseFromPlayer) {
        // this dealer only sells stuff to player, so he can't buy anything from
        // him
        return (FALSE);
      }
      break;
    ...
    case ARMS_DEALER_BUYS_ONLY:
      if (!fPurchaseFromPlayer) {
        // this dealer only buys stuff from player, so he can't sell anything to
        // him
        return (FALSE);
      }
      break;
    ...
    case ARMS_DEALER_BUYS_SELLS:
      switch (ubArmsDealer) {
        ...
        case ARMS_DEALER_FRANZ:
          if (fPurchaseFromPlayer) {
            // these guys will buy nearly anything from the player, regardless of
            // what they carry for sale!
    ...
    case ARMS_DEALER_REPAIRS:
      // repairmen don't have a complete list of what they'll repair in their
      // inventory, so we must check the item's properties instead.
      return (CanDealerRepairItem(ubArmsDealer, usItemIndex));

    default:
      AssertMsg(FALSE, String("CanDealerTransactItem(), type of dealer %d.  AM 0.",
...
BOOLEAN CanDealerRepairItem(ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex) {
  uint32_t uiFlags;
  ...
  switch (ubArmsDealer) {
    ...
    default:
      AssertMsg(FALSE, String("CanDealerRepairItem(), Arms Dealer %d is not a "
                              "recognized repairman!.  AM 1.",
                              ubArmsDealer));
  }
...
static void AllocMemsetSpecialItemArray(DEALER_ITEM_HEADER *const pDealerItem,
                                        uint8_t const ubElementsNeeded) {
  Assert(pDealerItem);
...
static void ResizeSpecialItemArray(DEALER_ITEM_HEADER *const pDealerItem,
                                   uint8_t const ubElementsNeeded) {
  Assert(pDealerItem);
  ...
  pDealerItem->SpecialItem =
      REALLOC(pDealerItem->SpecialItem, DEALER_SPECIAL_ITEM, ubElementsNeeded);

  // if adding more elements
  if (ubElementsNeeded > pDealerItem->ubElementsAlloced) {
    // zero them out (they're inactive until an item is actually added)
    memset(&(pDealerItem->SpecialItem[pDealerItem->ubElementsAlloced]), 0,
           sizeof(DEALER_SPECIAL_ITEM) * (ubElementsNeeded - pDealerItem->ubElementsAlloced));
  }
...
static uint8_t DetermineDealerItemCondition(ArmsDealerID, uint16_t usItemIndex);

static void ArmsDealerGetsFreshStock(ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex,
                                     uint8_t const ubNumItems) {
  uint8_t ubCnt;
  uint8_t ubItemCondition;
  uint8_t ubPerfectOnes = 0;
  SPECIAL_ITEM_INFO SpclItemInfo;

  // create item info describing a perfect item
  SetSpecialItemInfoToDefaults(&SpclItemInfo);

  // determine the condition of each one, counting up new ones, but adding
  // damaged ones right away
...
static BOOLEAN ItemContainsLiquid(uint16_t usItemIndex);

static uint8_t DetermineDealerItemCondition(ArmsDealerID const ubArmsDealer,
                                            uint16_t const usItemIndex) {
  uint8_t ubCondition = 100;
  ...
  if ((Item[usItemIndex].fFlags & ITEM_DAMAGEABLE) && !ItemContainsLiquid(usItemIndex)) {
    // if he ONLY has used items, or 50% of the time if he carries both used &
    // new items
    if ((ArmsDealerInfo[ubArmsDealer].uiFlags & ARMS_DEALER_ONLY_USED_ITEMS) ||
      ...
      ubCondition = (uint8_t)(20 + Random(60));
    }
  }
...
static uint8_t CountActiveSpecialItemsInArmsDealersInventory(ArmsDealerID, uint16_t usItemIndex);

uint32_t CountDistinctItemsInArmsDealersInventory(ArmsDealerID const ubArmsDealer) {
  ...
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems > 0) {
      ...
      if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubPerfectItems > 0) {
        ...
        // AddItemsToTempDealerInventory() exactly!
        if (DealerItemIsSafeToStack(usItemIndex)) {
          // regardless of how many there are, they count as 1 *distinct* item!
          // They will all be together in one box...
          uiNumOfItems++;
        } else {
          // non-stacking items must be stored in one / box , because each may
          // have unique fields besides bStatus[] Example: guns all have ammo,
          // ammo type, etc.  We need these uniquely represented for pricing &
          // manipulation
          uiNumOfItems += gArmsDealersInventory[ubArmsDealer][usItemIndex].ubPerfectItems;
        }
      }

      // each *active* special item counts as one additional distinct item (each
      // one occupied a separate shopkeeper box!) NOTE: This is including items
      // being repaired!!!
...
static uint8_t CountActiveSpecialItemsInArmsDealersInventory(ArmsDealerID const ubArmsDealer,
                                                             uint16_t const usItemIndex) {
  uint8_t ubActiveSpecialItems = 0;
  uint8_t ubElement;

  // next, try to sell all the used ones, gotta do these one at a time so we can
  // remove them by element
  for (ubElement = 0;
       ubElement < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced;
       ubElement++) {
    // don't worry about negative condition, repairmen can't come this far, they
    // don't sell!
    if (gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].fActive) {
...
static uint8_t CountSpecificItemsRepairDealerHasInForRepairs(ArmsDealerID, uint16_t usItemIndex);

uint16_t CountTotalItemsRepairDealerHasInForRepairs(ArmsDealerID const ubArmsDealer) {
  uint16_t usItemIndex;
  uint16_t usHowManyInForRepairs = 0;

  // if the dealer is not a repair dealer, no need to count, return 0
  if (!DoesDealerDoRepairs(ubArmsDealer)) return (0);

  // loop through the dealers inventory and count the number of items in for
  // repairs
  for (usItemIndex = 0; usItemIndex < MAXITEMS; usItemIndex++) {
    usHowManyInForRepairs +=
        CountSpecificItemsRepairDealerHasInForRepairs(ubArmsDealer, usItemIndex);
  }

  return (usHowManyInForRepairs);
}

static uint8_t CountSpecificItemsRepairDealerHasInForRepairs(ArmsDealerID const ubArmsDealer,
                                                             uint16_t const usItemIndex) {
  uint8_t ubElement;
  uint8_t ubHowManyInForRepairs = 0;
...
static void AddAmmoToArmsDealerInventory(ArmsDealerID, uint16_t usItemIndex, uint8_t ubShotsLeft);
...
void AddObjectToArmsDealerInventory(ArmsDealerID const ubArmsDealer, OBJECTTYPE *const pObject) {
  ...
  // split up all the components of an objecttype and add them as seperate items
  // into the dealer's inventory
  switch (Item[pObject->usItem].usItemClass) {
    case IC_GUN:
      // add the gun (keeps the object's status and imprintID)
      // if the gun was jammed, this will forget about the jam (i.e. dealer
      // immediately unjams anything he buys)
      ...
      if (pObject->usGunAmmoItem != NONE) {
        ...
        if (Item[pObject->usGunAmmoItem].usItemClass == IC_AMMO) {
          ...
          if (pObject->ubGunShotsLeft > 0) {
            ...
            AddAmmoToArmsDealerInventory(ubArmsDealer, pObject->usGunAmmoItem,
    ...
    case IC_AMMO:
      // add the contents of each magazine (multiple mags may have vastly
      // different #bullets left)
    ...
    default:
      // add each object seperately (multiple objects may have vastly different
      // statuses, keep any imprintID)
      for (ubCnt = 0; ubCnt < pObject->ubNumberOfObjects; ubCnt++) {
  ...
  for (ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++) {
    if (pObject->usAttachItem[ubCnt] != NONE) {
      // ARM: Note: this is only used for selling, not repairs, so attachmentes
      // are seperated when sold to a dealer If the attachment is detachable
      if (!(Item[pObject->usAttachItem[ubCnt]].fFlags & ITEM_INSEPARABLE)) {
        // add this particular attachment (they can't be imprinted, or
        // themselves have attachments!)
        SetSpecialItemInfoToDefaults(&SpclItemInfo);
...
static void AddAmmoToArmsDealerInventory(ArmsDealerID const ubArmsDealer,
                                         uint16_t const usItemIndex, uint8_t ubShotsLeft) {
  uint8_t ubMagCapacity;
  uint8_t *pubStrayAmmo;
  SPECIAL_ITEM_INFO SpclItemInfo;
  ...
  if (ubShotsLeft >= ubMagCapacity) {
    ...
    SetSpecialItemInfoToDefaults(&SpclItemInfo);
    AddItemToArmsDealerInventory(ubArmsDealer, usItemIndex, &SpclItemInfo,
                                 (uint8_t)(ubShotsLeft / ubMagCapacity));
    ubShotsLeft %= ubMagCapacity;
  }

  // any shots left now are "strays" - not enough to completely fill a magazine
  // of this type
  if (ubShotsLeft > 0) {
    ...
    *pubStrayAmmo += ubShotsLeft;
    ...
    // if dealer has accumulated enough stray ammo to make another full
    // magazine, convert it!
    if (*pubStrayAmmo >= ubMagCapacity) {
      SetSpecialItemInfoToDefaults(&SpclItemInfo);
      AddItemToArmsDealerInventory(ubArmsDealer, usItemIndex, &SpclItemInfo,
                                   (uint8_t)(*pubStrayAmmo / ubMagCapacity));
      *pubStrayAmmo = *pubStrayAmmo % ubMagCapacity;
    }
    // I know, I know, this is getting pretty anal...  But what the hell, it was
    // easy enough to do.  ARM.
  }
}

static void AddSpecialItemToArmsDealerInventoryAtElement(ArmsDealerID ubArmsDealer,
...
// Use AddObjectToArmsDealerInventory() instead of this when converting a
// complex item in OBJECTTYPE format.
static void AddItemToArmsDealerInventory(ArmsDealerID const ubArmsDealer,
                                         uint16_t const usItemIndex,
                                         SPECIAL_ITEM_INFO *const pSpclItemInfo,
                                         uint8_t ubHowMany) {
  uint8_t ubRoomLeft;
  uint8_t ubElement;
  uint8_t ubElementsToAdd;
  BOOLEAN fFoundOne;
  ...
  if (IsItemInfoSpecial(pSpclItemInfo)) {
    // Anything that's used/damaged or imprinted is store as a special item in
    // the SpecialItem array, exactly one item per element.  We (re)allocate
    // memory dynamically as necessary to hold the additional items.

    do {
      // search for an already allocated, empty element in the special item
      // array
      fFoundOne = FALSE;
      for (ubElement = 0;
           ubElement < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced;
        ...
        if (!(gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].fActive)) {
          ...
          AddSpecialItemToArmsDealerInventoryAtElement(ubArmsDealer, usItemIndex, ubElement,
                                                       pSpclItemInfo);
          fFoundOne = TRUE;
          break;
        }
      ...
      if (!fFoundOne) {
        ...
        if (gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced == 0) {
          ...
          AllocMemsetSpecialItemArray(&gArmsDealersInventory[ubArmsDealer][usItemIndex],
                                      ubElementsToAdd);
        } else {
          // we have some allocated, but they're all full and we need more.
          // MemRealloc existing amount + # addition elements
          ResizeSpecialItemArray(
              &gArmsDealersInventory[ubArmsDealer][usItemIndex],
              (uint8_t)(gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced +
                        ubElementsToAdd));
        }

        // now add the special item at the first of the newly added elements
        // (still stored in ubElement!)
        AddSpecialItemToArmsDealerInventoryAtElement(ubArmsDealer, usItemIndex, ubElement,
                                                     pSpclItemInfo);
      }

      // store the # of the element it was placed in globally so anyone who
      // needs that can grab it there
...
static void AddSpecialItemToArmsDealerInventoryAtElement(ArmsDealerID const ubArmsDealer,
                                                         uint16_t const usItemIndex,
...
// removes ubHowMany items of usItemIndex with the matching Info from dealer
// ubArmsDealer
void RemoveItemFromArmsDealerInventory(ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex,
                                       SPECIAL_ITEM_INFO *const pSpclItemInfo, uint8_t ubHowMany) {
  ...
  // decide whether this item is "special" or not
  if (IsItemInfoSpecial(pSpclItemInfo)) {
    // look through the elements, trying to find special items matching the
    // specifications
    for (ubElement = 0;
         ubElement < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubElementsAlloced;
         ubElement++) {
      ...
      if (pSpecialItem->fActive) {
        ...
        if (memcmp(&(gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].Info),
                   pSpclItemInfo, sizeof(SPECIAL_ITEM_INFO)) == 0) {
          // Got one!  Remove it
    ...
    // when we've searched all the special item elements, we'd better not have
    // any more items to remove!
    Assert(ubHowMany == 0);
...
static void RemoveRandomItemFromArmsDealerInventory(ArmsDealerID const ubArmsDealer,
                                                    uint16_t const usItemIndex, uint8_t ubHowMany) {
  uint8_t ubWhichOne;
  uint8_t ubSkippedAlready;
  BOOLEAN fFoundIt;
  uint8_t ubElement;
  SPECIAL_ITEM_INFO SpclItemInfo;

  // not permitted for repair dealers - would take extra code to subtract items
  // under repair from ubTotalItems!!!
  Assert(!DoesDealerDoRepairs(ubArmsDealer));
  ...
  while (ubHowMany > 0) {
    ...
    ubWhichOne = (uint8_t)Random(gArmsDealersInventory[ubArmsDealer][usItemIndex].ubTotalItems);
    ...
    if (ubWhichOne < gArmsDealersInventory[ubArmsDealer][usItemIndex].ubPerfectItems) {
      ...
      // Yikes!  Gotta look through the special items.  We already know it's not
      // any of the perfect ones, subtract those
      ...
      for (ubElement = 0;
        ...
        if (gArmsDealersInventory[ubArmsDealer][usItemIndex]
                ...
                .fActive)  // &&
                           ...
                           // ubArmsDealer
        //][ usItemIndex ].SpecialItem[ ubElement ].Info.bItemCondition > 0 ) )
        {
...
void RemoveSpecialItemFromArmsDealerInventoryAtElement(ArmsDealerID const ubArmsDealer,
                                                       uint16_t const usItemIndex,
                                                       uint8_t const ubElement) {
  ...
  memset(&(gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement]), 0,
...
BOOLEAN AddDeadArmsDealerItemsToWorld(SOLDIERTYPE const *const pSoldier) {
  uint16_t usItemIndex;
  uint8_t ubElement;
  uint8_t ubHowManyMaxAtATime;
  uint8_t ubLeftToDrop;
  uint8_t ubNowDropping;
  ...
  // loop through all the items in the dealer's inventory, and drop them all
  // where the dealer was set up.
  ...
  for (usItemIndex = 1; usItemIndex < MAXITEMS; usItemIndex++) {
    ...
    if (gArmsDealersInventory[bArmsDealer][usItemIndex].ubTotalItems > 0) {
      ...
      if (gArmsDealersInventory[bArmsDealer][usItemIndex].ubPerfectItems > 0) {
        ...
        // drop stackable items like ammo in stacks of whatever will fit into a
        // large pocket instead of one at a time
        ...
        // ATE: While it IS leagal here to use pSoldier->sInitialGridNo, cause
        ...
        RemoveItemFromArmsDealerInventory(
            bArmsDealer, usItemIndex, &SpclItemInfo,
...
void MakeObjectOutOfDealerItems(uint16_t usItemIndex, SPECIAL_ITEM_INFO *pSpclItemInfo,
  ...
  // if the item condition is below 0, the item is in for repairs, so flip the
  // sign
  if (bItemCondition < 0) {
    bItemCondition *= -1;
  }
  ...
  if (Item[pObject->usItem].usItemClass == IC_GUN) {
    // Empty out the bullets put in by CreateItem().  We now sell all guns empty
    // of bullets.  This is so that we don't have to keep track of #bullets in a
    // gun throughout dealer inventory.  Without this, players could "reload"
    // guns they don't have ammo for by selling them to Tony & buying them right
    // back fully loaded!  One could repeat this ad nauseum (empty the gun
    // between visits) as a (really expensive) way to get unlimited special ammo
    // like rockets.
    pObject->ubGunShotsLeft = 0;
  }
}

static void GiveItemToArmsDealerforRepair(ArmsDealerID, uint16_t usItemIndex,
                                          SPECIAL_ITEM_INFO *pSpclItemInfo,
                                          uint8_t ubOwnerProfileId);

void GiveObjectToArmsDealerForRepair(ArmsDealerID const ubArmsDealer,
                                     OBJECTTYPE const *const pObject,
                                     uint8_t const ubOwnerProfileId) {
  //	uint8_t ubCnt;
  SPECIAL_ITEM_INFO SpclItemInfo;
  ...
  /* ARM: Can now repair with removeable attachments still attached...
          ...
          for( ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++ )
          {
                  if ( pObject->usAttachItem[ ubCnt ] != NONE )
                  {
                          ...
                          if (! (Item[ pObject->usAttachItem[ubCnt] ].fFlags &
     ITEM_INSEPARABLE ) )
                          {
                                  Assert( 0 );
                          }
  ...
  */
  ...
  //		e) If a gun, stripped of any non-ammo-class GunAmmoItems, and
  // bullets
  if (Item[pObject->usItem].usItemClass == IC_GUN) {
    // if any GunAmmoItem is specified
    if (pObject->usGunAmmoItem != NONE) {
  ...
  // if the gun was jammed, this will forget about the jam (i.e. dealer
  // immediately unjams anything he will be repairing)
  GiveItemToArmsDealerforRepair(ubArmsDealer, pObject->usItem, &SpclItemInfo, ubOwnerProfileId);
}

static uint32_t CalculateSpecialItemRepairTime(ArmsDealerID, uint16_t usItemIndex,
                                               SPECIAL_ITEM_INFO *pSpclItemInfo);
static uint32_t WhenWillRepairmanBeAllDoneRepairing(ArmsDealerID);

// PLEASE: Use GiveObjectToArmsDealerForRepair() instead of this when repairing
// a item in OBJECTTYPE format.
static void GiveItemToArmsDealerforRepair(ArmsDealerID const ubArmsDealer,
                                          uint16_t const usItemIndex,
                                          SPECIAL_ITEM_INFO *const pSpclItemInfo,
                                          uint8_t const ubOwnerProfileId) {
  ...
  // figure out the earliest the repairman will be free to start repairing this
  // item
  ...
  gArmsDealersInventory[ubArmsDealer][usItemIndex]
      .SpecialItem[gubLastSpecialItemAddedAtElement]
...
static uint32_t WhenWillRepairmanBeAllDoneRepairing(ArmsDealerID const ubArmsDealer) {
  uint32_t uiWhenFree;
  uint16_t usItemIndex;
  uint8_t ubElement;
      ...
      for (ubElement = 0;
        ...
        if (gArmsDealersInventory[ubArmsDealer][usItemIndex].SpecialItem[ubElement].fActive) {
          ...
          if (gArmsDealersInventory[ubArmsDealer][usItemIndex]
                  .SpecialItem[ubElement]
                  .Info.bItemCondition < 0) {
            // if this item will be done later than the latest we've found so
            // far
            if (gArmsDealersInventory[ubArmsDealer][usItemIndex]
                    .SpecialItem[ubElement]
                    .uiRepairDoneTime > uiWhenFree) {
              // then we're busy til then!
              uiWhenFree = gArmsDealersInventory[ubArmsDealer][usItemIndex]
                               .SpecialItem[ubElement]
                               .uiRepairDoneTime;
            }
          }
...
static uint32_t CalculateSimpleItemRepairTime(ArmsDealerID, uint16_t usItemIndex,
                                              int8_t bItemCondition);

static uint32_t CalculateSpecialItemRepairTime(ArmsDealerID const ubArmsDealer,
                                               uint16_t const usItemIndex,
                                               SPECIAL_ITEM_INFO *const pSpclItemInfo) {
  ...
  uiRepairTime =
      CalculateSimpleItemRepairTime(ubArmsDealer, usItemIndex, pSpclItemInfo->bItemCondition);

  // add time to repair any attachments on it
  for (ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++) {
    if (pSpclItemInfo->usAttachment[ubCnt] != NONE) {
      // if damaged and repairable
      if ((pSpclItemInfo->bAttachmentStatus[ubCnt] < 100) &&
          CanDealerRepairItem(ubArmsDealer, pSpclItemInfo->usAttachment[ubCnt])) {
        uiRepairTime +=
            CalculateSimpleItemRepairTime(ubArmsDealer, pSpclItemInfo->usAttachment[ubCnt],
...
uint32_t CalculateObjectItemRepairTime(ArmsDealerID const ubArmsDealer,
                                       OBJECTTYPE const *const pItemObject) {
  uint32_t uiRepairTime;
  uint8_t ubCnt;

  uiRepairTime =
      CalculateSimpleItemRepairTime(ubArmsDealer, pItemObject->usItem, pItemObject->bStatus[0]);

  // add time to repair any attachments on it
  for (ubCnt = 0; ubCnt < MAX_ATTACHMENTS; ubCnt++) {
    if (pItemObject->usAttachItem[ubCnt] != NONE) {
      // if damaged and repairable
      if ((pItemObject->bAttachStatus[ubCnt] < 100) &&
          CanDealerRepairItem(ubArmsDealer, pItemObject->usAttachItem[ubCnt])) {
        uiRepairTime += CalculateSimpleItemRepairTime(
            ubArmsDealer, pItemObject->usAttachItem[ubCnt], pItemObject->bAttachStatus[ubCnt]);
      }
    }
  }
...
static uint32_t CalculateSimpleItemRepairCost(ArmsDealerID, uint16_t usItemIndex,
                                              int8_t bItemCondition);

static uint32_t CalculateSimpleItemRepairTime(ArmsDealerID const ubArmsDealer,
                                              uint16_t const usItemIndex,
                                              int8_t const bItemCondition) {
  uint32_t uiTimeToRepair = 0;
  uint32_t uiRepairCost = 0;

  Assert(DoesDealerDoRepairs(ubArmsDealer));

  // first calc what he'll charge - that takes care of item condition, repair
  // ease, and his repair cost "markup"
  ...
  // For a repairman, his BUY modifier controls his REPAIR SPEED (1.0 means
  // minutes to repair = price in $) with a REPAIR SPEED of 1.0, typical gun
  // price of $2000, and a REPAIR COST of 0.5 this works out to 16.6 hrs
  //		 for a full 100% status repair...  Not bad.
  uiTimeToRepair = (uint32_t)(uiRepairCost * ArmsDealerInfo[ubArmsDealer].u.repair.speed);

  // repairs on electronic items take twice as long if the guy doesn't have the
  // skill for dealers, this means anyone but Fredo the Electronics guy takes
  // twice as long (but doesn't charge double) (Mind you, current he's the ONLY
  // one who CAN repair Electronics at all!  Oh well.)
...
uint32_t CalculateObjectItemRepairCost(ArmsDealerID const ubArmsDealer,
                                       OBJECTTYPE const *const pItemObject) {
  uint32_t uiRepairCost;
  uint8_t ubCnt;

  uiRepairCost =
      CalculateSimpleItemRepairCost(ubArmsDealer, pItemObject->usItem, pItemObject->bStatus[0]);
...
static uint32_t CalculateSimpleItemRepairCost(ArmsDealerID const ubArmsDealer,
                                              uint16_t const usItemIndex,
                                              int8_t const bItemCondition) {
  ...
  // figure out the full value of the item, modified by this dealer's personal
  // Sell (i.e. repair cost) modifier don't use CalcShopKeeperItemPrice - we
  // want FULL value!!!
  uiItemCost = (uint32_t)(Item[usItemIndex].usPrice * ArmsDealerInfo[ubArmsDealer].u.repair.cost);
  ...
  // calculate repair cost, the more broken it is the more it costs, and the
  // difficulty of repair it is also a factor
  uiRepairCost =
  ...
  */
  ...
  if (ItemIsARocketRifle(usItemIndex)) {
    // resetting imprinting for a rocket rifle costs something extra even if
    // rifle is at 100%
    uiRepairCost += 100;
  }
...
static BOOLEAN DoesItemAppearInDealerInventoryList(ArmsDealerID const ubArmsDealer,
                                                   uint16_t const usItemIndex,
                                                   BOOLEAN const fPurchaseFromPlayer) {
  uint16_t usCnt;

  // the others will buy only things that appear in their own "for sale"
  // inventory lists
  DEALER_POSSIBLE_INV const *const pDealerInv = GetPointerToDealersPossibleInventory(ubArmsDealer);

  // loop through the dealers' possible inventory and see if the item exists
  // there
  usCnt = 0;
  while (pDealerInv[usCnt].sItemIndex != LAST_DEALER_ITEM) {
    // if the initial dealer inv contains the required item, the dealer can sell
    // the item
    if (pDealerInv[usCnt].sItemIndex == usItemIndex) {
      // if optimal quantity listed is 0, it means dealer won't sell it himself,
      // but will buy it from the player!
...
uint16_t CalcValueOfItemToDealer(ArmsDealerID const ubArmsDealer, uint16_t const usItemIndex,
                                 BOOLEAN const fDealerSelling) {
  uint16_t usBasePrice;
  uint8_t ubItemPriceClass;
  uint8_t ubDealerPriceClass;
  uint16_t usValueToThisDealer;
  ...
  // the rest of this function applies only to the "general" dealers ( Jake,
  // Keith, and Franz )

  // Micky & Gabby specialize in creature parts & such, the others don't buy
  // these at all (exception: jars)
  if ((usItemIndex != JAR) &&
  ...
  if (!fDealerSelling) {
    // junk dealer won't buy expensive stuff at all, expensive dealer won't buy
    // junk at all
  ...
  // Tony specializes in guns, weapons, and ammo, so make others pay much less
...
BOOLEAN DealerItemIsSafeToStack(uint16_t usItemIndex) {
  // basically any item type with nothing unique about it besides its status can
  // be stacked in dealer's inventory boxes... NOTE: This test is only applied
  // to items already KNOWN to be perfect - special items are obviously
  // not-stackable
...
static void GuaranteeMinimumAlcohol(ArmsDealerID const ubArmsDealer) {
  GuaranteeAtLeastXItemsOfIndex(ubArmsDealer, BEER,
                                (uint8_t)(GetDealersMaxItemAmount(ubArmsDealer, BEER) / 3));
  GuaranteeAtLeastXItemsOfIndex(ubArmsDealer, WINE,
                                (uint8_t)(GetDealersMaxItemAmount(ubArmsDealer, WINE) / 3));
  GuaranteeAtLeastXItemsOfIndex(ubArmsDealer, ALCOHOL,
...
static BOOLEAN GetArmsDealerShopHours(ArmsDealerID const ubArmsDealer,
                                      uint32_t *const puiOpeningTime,
                                      uint32_t *const puiClosingTime) {
...
uint32_t CalculateOvernightRepairDelay(ArmsDealerID const ubArmsDealer,
                                       uint32_t uiTimeWhenFreeToStartIt, uint32_t uiMinutesToFix) {
  ...
  // convert world time into 24hr military time for the day he's gonna start on
  // it
...
uint32_t CalculateMinutesClosedBetween(ArmsDealerID const ubArmsDealer, uint32_t uiStartTime,
                                       uint32_t uiEndTime) {
  uint32_t uiOpeningTime, uiClosingTime;
  uint32_t uiMinutesClosedOvernight;
  ...
  // NOTE: this assumes stored are only closed overnight, so all we have to do
  // is compare the day portion
  ...
  // treat end time of midnight as 24:00 hours to prevent indefinite recursion
  // and make formulas work
  if (uiEndTime == 0) {
  ...
  if (uiStartTime < uiEndTime) {
    ...
    // recursive calls!  Add two separate times: before midnight, and after
    // midnight
