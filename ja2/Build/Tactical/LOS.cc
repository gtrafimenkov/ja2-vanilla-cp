...
static const double ddShotgunSpread[3][BUCKSHOT_SHOTS][2] = {
    {// spread of about 2 degrees in all directions
     ...
     {+0.008, +0.008}},
    {
        // duckbill flattens the spread and makes it wider horizontally (5
        // degrees) Horiz,	 Vert
        {0.0, 0.0},
        {-0.008, 0.0},
        {+0.008, 0.0},
...
static const uint8_t gubTreeSightReduction[ANIM_STAND + 1] = {
    0,
    8,  // prone
    0,
    7,  // crouched
...
static const uint32_t
    guiStructureHitChance[MAX_DIST_FOR_LESS_THAN_MAX_CHANCE_TO_HIT_STRUCTURE + 1] = {
        0,  // 0 tiles
        0,  0,  2,  4,
        7,  // 5 tiles
        10, 14, 18, 23,
...
static uint32_t guiLocalStructureCTH[MAX_LOCAL_STRUCTURES];
static uint8_t gubLocalStructureNumTimesHit[MAX_LOCAL_STRUCTURES];
...
static float FixedToFloat(FIXEDPT qN) { return (((float)qN) / FIXEDPT_FRACTIONAL_RESOLUTION); }
...
enum LocationCode { LOC_OTHER, LOC_0_4, LOC_3_4, LOC_4_0, LOC_4_3, LOC_4_4 };

static BOOLEAN ResolveHitOnWall(STRUCTURE *pStructure, int32_t iGridNo, int8_t bLOSIndexX,
  ...
  if (fEastWest) {
    ...
    if (bLocation == LOC_4_4) {
      if (pStructure->ubWallOrientation == NO_ORIENTATION) {
        // very top north corner of building, and going (screenwise) west or
        // east
        return (FALSE);
      }
    } else if (bLocation == LOC_4_0) {
  ...
  */
  ...
  /*
          ...
          if ( bLOSIndexX == 0)
          {
                  ...
                  {
                          // maybe looking E-W at (screenwise) north corner of
     building, or through open door if ( ( ddHorizAngle > (0) && ddHorizAngle <
     (PI * 1 / 2) ) || ( ddHorizAngle < (-PI * 1 / 2) && ddHorizAngle > ( -PI )
     ) )
                          {
                                  // if door is normal and OPEN and inside type
     then we let E-W pass if ( (pStructure->fFlags & STRUCTURE_DOOR) &&
     (pStructure->fFlags & STRUCTURE_OPEN) )
                                  {
                                          if ( pStructure->ubWallOrientation ==
     INSIDE_TOP_LEFT || pStructure->ubWallOrientation == INSIDE_TOP_RIGHT )
                                          {
                                                  fResolveHit = FALSE;
                                          }
                                  }

                                  // if wall orientation is top-left, then check
     W of this location
                                  // if no wall of same orientation there, let
     bullet through if ( pStructure->ubWallOrientation == INSIDE_TOP_LEFT ||
     pStructure->ubWallOrientation == OUTSIDE_TOP_LEFT )
                                  {
                                          if
     (!WallOrClosedDoorExistsOfTopLeftOrientation( (int16_t) (iGridNo +
     DirectionInc( WEST )) ) && !WallOrClosedDoorExistsOfTopRightOrientation(
     (int16_t) (iGridNo + DirectionInc( WEST )) ) )
                                          {
                                                  fResolveHit = FALSE;
                                          }
                          ...
                          else if ( ( ddHorizAngle < (0) && ddHorizAngle > ( -PI
     * 1 / 2) ) || ( ddHorizAngle > ( PI * 1 / 2 ) && ddHorizAngle < (PI) ) )
                          {
                                  // maybe looking N-S at (screenwise) west
     corner of building
     ...
     (int16_t) (iGridNo + DirectionInc( WEST )) ) )
          ...
          else if (bLOSIndexX == (PROFILE_X_SIZE - 1))
          {
                  ...
                  {
                          // maybe looking E-W at (screenwise) north corner of
     building if ( ( ddHorizAngle > (0) && ddHorizAngle < (PI * 1 / 2) ) || (
     ddHorizAngle < (-PI * 1 / 2) && ddHorizAngle > ( -PI ) ) )
                          {
                                  // if wall orientation is top-right, then
     check N of this location
                                  // if no wall of same orientation there, let
     bullet through if ( pStructure->ubWallOrientation == INSIDE_TOP_RIGHT ||
     pStructure->ubWallOrientation == OUTSIDE_TOP_RIGHT )
                                  {
                                          if
     (!WallOrClosedDoorExistsOfTopRightOrientation( (int16_t) (iGridNo +
     ...
     (int16_t) (iGridNo + DirectionInc( NORTH )) ) )
                          ...
                          {
                                  // if door is normal and OPEN and outside type
     then we let N-S pass if ( (pStructure->fFlags & STRUCTURE_DOOR) &&
     (pStructure->fFlags & STRUCTURE_OPEN) )
                                  {
                                          if ( pStructure->ubWallOrientation ==
     OUTSIDE_TOP_LEFT || pStructure->ubWallOrientation == OUTSIDE_TOP_RIGHT )
                                          {
                                                  fResolveHit = FALSE;
                                          }
                                  }
                  ...
                  else if ( bLOSIndexY == (PROFILE_Y_SIZE - 1) || bLOSIndexY ==
     (PROFILE_Y_SIZE - 2) )
                  {
                          // maybe (SCREENWISE) west or east corner of building
     and looking N if ( ( ddHorizAngle < (0) && ddHorizAngle > ( -PI * 1 / 2) )
     || ( ddHorizAngle > ( PI * 1 / 2 ) && ddHorizAngle < (PI) ) )
                          {
                                  // if wall orientation is top-right, then
     check S of this location
                                  // if wall orientation is top-left, then check
     E of this location
                                  // if no wall of same orientation there, let
     bullet through if ( pStructure->ubWallOrientation == INSIDE_TOP_LEFT ||
     pStructure->ubWallOrientation == OUTSIDE_TOP_LEFT )
                                  {
                                          if (
     !WallOrClosedDoorExistsOfTopLeftOrientation( (int16_t) (iGridNo +
     DirectionInc( EAST )) ) && !WallOrClosedDoorExistsOfTopRightOrientation(
     (int16_t) (iGridNo) ) && !OpenLeftOrientedDoorWithDoorOnLeftOfEdgeExists(
     (int16_t) (iGridNo + DirectionInc( EAST )) ) )
                                          {
                                                  fResolveHit = FALSE;
                                          }
                                  }
                                  else if ( pStructure->ubWallOrientation ==
     INSIDE_TOP_RIGHT || pStructure->ubWallOrientation == OUTSIDE_TOP_RIGHT )
                                  {
                                          if
     (!WallOrClosedDoorExistsOfTopRightOrientation( (int16_t) (iGridNo +
     DirectionInc( SOUTH )) ) && !WallOrClosedDoorExistsOfTopLeftOrientation(
     (int16_t) (iGridNo) ) && !OpenRightOrientedDoorWithDoorOnRightOfEdgeExists(
     (int16_t) (iGridNo + DirectionInc( SOUTH )) ) )
                                          {
                                                  fResolveHit = FALSE;
                                          }
     ...
     NO_ORIENTATION)
                                  {
                                          // very top north corner of building,
     and going (screenwise) west or east fResolveHit = FALSE;
                                  }
                          }
                  }
          }
...
/*
 * The line of sight code is now used to simulate smelling through the air (for
 * monsters); It obeys the following rules:
 * - ignores trees and vegetation
 * - ignores people
 * - should always start off with head height for both source and target, so
 * that lying down makes no difference
 ...
 */
static int32_t LineOfSightTest(GridNo start_pos, float dStartZ, GridNo end_pos, float dEndZ,
                               uint8_t ubTileSightLimit, uint8_t ubTreeSightReduction,
  ...
  // TreeSightReduction is the reduction in 10ths of tiles in max visibility for
  // each LOS cube (5th of a tile) of vegetation hit Aware is whether the looker
  // is aware of the target Smell is whether this is a sight or a smell test

  // Now returns not a boolean but the adjusted (by cover) distance to the
  // target, or 0 for unseen
  ...
  int16_t start_cell_x;
  int16_t start_cell_y;
  ...
  // plus a fractional part equal to half of the difference between the delta
  // and the increment times the distance
  ...
  do {
    ...
    if (fCheckForRoof) {
      ...
      if (pRoofStructure) {
        ...
        // if just on going to next tile we cross boundary, then roof stops
        // sight here!
...
// ground is in the way!
      ...
      if (pStructure == NULL) {  // no structures in this tile, AND THAT INCLUDES ROOFS! :-)
        ...
        /*
                                        ...
                                        {
                                                qDistToTravelX =
           INT32_TO_FIXEDPT( CELL_X_SIZE ) - (qCurrX % INT32_TO_FIXEDPT(
           CELL_X_SIZE )); iStepsToTravelX = qDistToTravelX / qIncrX;
                                        }
                                        else if (qIncrX < 0)
                                        {
                                                qDistToTravelX = qCurrX %
           INT32_TO_FIXEDPT( CELL_X_SIZE ); iStepsToTravelX = qDistToTravelX /
           (-qIncrX);
                                        }
                                        else
                                        {
                                                // make sure we don't consider X
           a limit :-) iStepsToTravelX = 1000000;
                                        }

                                        if (qIncrY > 0)
                                        {
                                                qDistToTravelY =
           INT32_TO_FIXEDPT( CELL_Y_SIZE ) - (qCurrY % INT32_TO_FIXEDPT(
           CELL_Y_SIZE )); iStepsToTravelY = qDistToTravelY / qIncrY;
                                        }
                                        else if (qIncrY < 0)
                                        {
                                                qDistToTravelY = qCurrY %
           INT32_TO_FIXEDPT( CELL_Y_SIZE ); iStepsToTravelY = qDistToTravelY /
           (-qIncrY);
                                        }
                                        else
                                        {
                                                // make sure we don't consider Y
           a limit :-) iStepsToTravelY = 1000000;
                                        }
...
// ground is in the way!
        ...
        if (iCurrCubesAboveLevelZ < STRUCTURE_ON_ROOF_MAX) {
          ...
          while (pStructure != NULL) {
            ...
            // normal roof structures should be skipped here because their only
            // bits are roof lips and those should act as transparent
            ...
            if (pStructure->fFlags & STRUCTURE_ROOF) {
              ...
              if ((iLoop < 2 * CELL_X_SIZE || (iDistance - iLoop) < 2 * CELL_X_SIZE)) {
                if (iLoop <= CELL_X_SIZE + 1 || (iDistance - iLoop) <= CELL_X_SIZE + 1) {
                  // right near edge, allow sighting at 3 tiles from roof edge
                  // if prone less if standing, and we can tell that with
                  // iStartCubesZ and iEndCubesZ
                  if (iStartCubesZ < iEndCubesZ) {
                    // looking up, so reduce for the target stance-height
                    // according to iEndCubesZ
                  ...
                  if (fSmell) {
                    if (pStructure->fFlags & STRUCTURE_TREE) {
                      ...
                      if (pStructure->fFlags & STRUCTURE_WALLSTUFF) {
                        // possibly at corner in which case we should let it
                        // pass
...
// out of visual range
                        ...
                        #ifdef LOS_DEBUG
                                gLOSTestResults.iStoppedX = FIXEDPT_TO_INT32(
                        qCurrX ); gLOSTestResults.iStoppedY = FIXEDPT_TO_INT32(
                        qCurrY ); gLOSTestResults.iStoppedZ = FIXEDPT_TO_INT32(
                        qCurrZ ); gLOSTestResults.iCurrCubesZ =
                        iCurrCubesAboveLevelZ; #endif return( 0 );
                        */
                      }
                    }
                  } else {
...
// out of visual range
                      ...
                      // do nothing; windows are transparent (except ones marked
                      // as special)
                      if (psWindowGridNo != NULL) {
                        // we're supposed to note the location of this window!
                        // but if a location has already been set then there are
                        // two windows, in which case we abort
                      ...
                      if (pStructure->fFlags & STRUCTURE_WALLSTUFF) {
                        // possibly shooting at corner in which case we should
                        // let it pass
                        fResolveHit = ResolveHitOnWall(pStructure, iGridNo, bLOSIndexX, bLOSIndexY,
                                                       ddHorizAngle);
                      } else {
                        if (iCurrCubesAboveLevelZ < (STANDING_CUBES - 1)) {
                          if ((iLoop <= CLOSE_TO_FIRER) &&
                            ...
                            // and this is the height of the structure, then
                            // allow sight to go through NB cubes are 0 based,
                            // heights 1 based
                            ...
                            // if we are in the same vertical cube as our
                            // destination, and this is the height of the
                            // structure, and we are aware then allow sight to
                            // go through NB cubes are 0 based, heights 1 based
                            iStructureHeight = StructureHeight(pStructure);
                            fResolveHit = (iCurrCubesAboveLevelZ != (iStructureHeight - 1));
                          } else {
                            fResolveHit = TRUE;
...
// hit the obstacle!
        ...
        do {
          ...
          // that will be caught below and this loop shouldn't go for more than
    ...
    if (pMapElement->ubExtFlags[0] &
      ...
      if ((pMapElement->ubExtFlags[0] & MAPELEMENT_EXT_SMOKE) && !fSmell) {
        ...
        {
                // losing 1/3rd results in chances to hit which are WAY too low
        when firing from out of
...
// but we shouldn't(?) need to check it because the target is there!
  ...
  // it adds the difference between the original and adjusted sight limit, = the
  // amount of cover it then scales the value based on the difference between
  // the original sight limit and the
...
int32_t SoldierToSoldierLineOfSightTest(const SOLDIERTYPE *const pStartSoldier,
                                        const SOLDIERTYPE *const pEndSoldier,
                                        uint8_t ubTileSightLimit, const int8_t bAware) {
  ...
  return LineOfSightTest(pStartSoldier->sGridNo, dStartZPos, pEndSoldier->sGridNo, dEndZPos,
...
int16_t SoldierToLocationWindowTest(const SOLDIERTYPE *pStartSoldier, int16_t sEndGridNo) {
  ...
  // We don't want to consider distance limits here so pass in tile sight limit
  // of 255 and consider trees as little as possible
  int16_t sWindowGridNo = NOWHERE;
  LineOfSightTest(pStartSoldier->sGridNo, dStartZPos, sEndGridNo, dEndZPos, 255, 0, TRUE, 0, FALSE,
                  &sWindowGridNo);
...
int32_t SoldierTo3DLocationLineOfSightTest(const SOLDIERTYPE *pStartSoldier, int16_t sGridNo,
  ...
  return LineOfSightTest(pStartSoldier->sGridNo, dStartZPos, sGridNo, dEndZPos, ubTileSightLimit,
...
int32_t SoldierToBodyPartLineOfSightTest(SOLDIERTYPE *pStartSoldier, int16_t sGridNo, int8_t bLevel,
  ...
  // CJC August 13, 2002: for this routine to work there MUST be a target at the
  // location specified
...
int32_t LocationToLocationLineOfSightTest(int16_t sStartGridNo, int8_t bStartLevel,
  ...
  if (start_soldier != NULL) {
    return SoldierTo3DLocationLineOfSightTest(start_soldier, sEndGridNo, bEndLevel, 0,
  ...
  return LineOfSightTest(sStartGridNo, dStartZPos, sEndGridNo, dEndZPos, ubTileSightLimit,
                         gubTreeSightReduction[ANIM_STAND], bAware, 0, FALSE, NULL);
}

/*
int32_t BulletImpactReducedByRange( int32_t iImpact, int32_t iDistanceTravelled, int32_t
iRange )
{
...
static BOOLEAN BulletHitMerc(BULLET *pBullet, STRUCTURE *pStructure, BOOLEAN fIntended) {
  ...
  if (pBullet->usFlags & BULLET_FLAG_KNIFE) {
    ...
    if (bSlot == NO_SLOT) {
      ...
      CreateItem(BLOODY_THROWING_KNIFE, (int8_t)pBullet->ubItemStatus, &tgt.inv[bSlot]);
    }
  ...
  // at least partly compensate for "near miss" increases for this guy, after
  // all, the bullet actually hit him! take this out for now at least... no
  // longer certain that he was awarded a suppression pt when the bullet got
  // near him
  //--tgt.ubSuppressionPoints;

  if (tgt.uiStatusFlags & SOLDIER_VEHICLE ||
      (tgt.ubBodyType == COW || tgt.ubBodyType == CROW || tgt.ubBodyType == BLOODCAT)) {
    ...
    if (CREATURE_OR_BLOODCAT(&tgt)) {
      ...
      if (ADULTFEMALEMONSTER <= tgt.ubBodyType && tgt.ubBodyType <= YAM_MONSTER) {
        ubAttackDirection =
            (uint8_t)GetDirectionToGridNoFromGridNo(pBullet->pFirer->sGridNo, tgt.sGridNo);
    ...
    if (ubAmmoType == AMMO_MONSTER && (ubHitLocation == AIM_SHOT_HEAD) &&
        !(tgt.uiStatusFlags & SOLDIER_MONSTER)) {
      ...
      } else if ((tgt.inv[HEAD1POS].usItem == NIGHTGOGGLES ||
                  tgt.inv[HEAD1POS].usItem == SUNGOGGLES || tgt.inv[HEAD1POS].usItem == GASMASK) &&
                 PreRandom(100) < tgt.inv[HEAD1POS].bStatus[0]) {
        // lucky bastard was wearing protective stuff
        bHeadSlot = HEAD1POS;
      } else if ((tgt.inv[HEAD2POS].usItem == NIGHTGOGGLES ||
                  tgt.inv[HEAD2POS].usItem == SUNGOGGLES || tgt.inv[HEAD2POS].usItem == GASMASK) &&
  ...
  int16_t const sRange = GetRangeInCellCoordsFromGridNoDiff(pFirer->sGridNo, tgt.sGridNo);
  if (gTacticalStatus.uiFlags & GODMODE && !(pFirer->uiStatusFlags & SOLDIER_PC)) {
    ...
    // reduce due to range
    iImpact = pBullet->iImpact;  // BulletImpactReducedByRange( pBullet->iImpact,
    ...
    // iImpact = BulletImpactReducedByRange( pBullet->iImpact, pBullet->iLoop,
    // pBullet->iRange );
    iImpact -= pBullet->iImpactReduction;
    if (iImpact < 0) {
      // shouldn't happen but
  ...
  if (ubAmmoType == AMMO_MONSTER) {
    if (bHeadSlot != NO_SLOT) {
      tgt.inv[bHeadSlot].bStatus[0] -= (int8_t)(iImpact / 2 + Random(iImpact / 2));
    ...
    if (tgt.inv[bHeadSlot].usItem != NOTHING) {
      tgt.inv[bHeadSlot].bStatus[0] -= (int8_t)(Random(iImpact / 2));
      if (tgt.inv[bHeadSlot].bStatus[0] < 0) {
        // just break it...
  ...
  // check to see if the guy is a friendly?..if so, up the number of times
  // wounded
  if (tgt.bTeam == OUR_TEAM) {
    ++gMercProfiles[tgt.ubProfile].usTimesWounded;
  }

  // check to see if someone was accidentally hit when no target was specified
  // by the player
  ...
  if ((pFirer->bDoBurst) && (ubSpecial == FIRE_WEAPON_NO_SPECIAL)) {
    // the animation required by the bullet hit (head explosion etc) overrides
    // the hit-by-a-burst animation
    ubSpecial = FIRE_WEAPON_BURST_SPECIAL;
  }

  // breath loss is based on original impact of bullet
  int16_t const breath_loss =
      (iImpact * BP_GET_WOUNDED * (tgt.bBreathMax * 100 - tgt.sBreathRed)) / 10000;
  uint16_t const hit_direction = GetDirectionFromGridNo(pFirer->sGridNo, &tgt);
  ...
  if (!(tgt.uiStatusFlags & SOLDIER_VEHICLE) &&
    ...
    uiChanceThrough = (uint8_t)std::max(0, (iImpact - 20));
    if (PreRandom(100) < uiChanceThrough) {
  ...
  if (fStopped) {
    ...
    // get a new gridno based on direction it was moving.  Check to see if we're
    // not going through walls, etc by testing for a path, unless on the roof,
    // in which case it would always be legal, but the bLevel May change...
    sNewGridNo = NewGridNo(pBullet->sGridNo, DirectionInc(OppositeDirection(hit_direction)));

    bSpewBloodLevel = tgt.bLevel;
    fCanSpewBlood = TRUE;

    // If on anything other than bLevel of 0, we can pretty much freely spew
    // blood
    if (bSpewBloodLevel == 0) {
      if (gubWorldMovementCosts[sNewGridNo][OppositeDirection(hit_direction)][0] >=
          TRAVELCOST_BLOCKED) {
        fCanSpewBlood = FALSE;
      }
    } else {
  ...
  // handle hit!
  WeaponHit(&tgt, pFirer->usAttackingWeapon, iDamage, breath_loss, hit_direction, tgt.dXPos,
            tgt.dYPos, 20, sRange, pFirer, ubSpecial, ubHitLocation);
  return (fStopped);
}

static void BulletHitStructure(BULLET *pBullet, uint16_t usStructureID, int32_t iImpact,
                               BOOLEAN fStopped) {
  const FIXEDPT qCurrX = pBullet->qCurrX;
  const FIXEDPT qCurrY = pBullet->qCurrY;
  const FIXEDPT qCurrZ = pBullet->qCurrZ;
  int16_t sXPos = FIXEDPT_TO_INT32(qCurrX + FloatToFixed(0.5f));  // + 0.5);
  int16_t sYPos = FIXEDPT_TO_INT32(qCurrY + FloatToFixed(0.5f));  // (dCurrY + 0.5);
  int16_t sZPos = CONVERT_HEIGHTUNITS_TO_PIXELS(
...
static void BulletHitWindow(BULLET *pBullet, int16_t sGridNo, uint16_t usStructureID,
                            BOOLEAN fBlowWindowSouth) {
  WindowHit(sGridNo, usStructureID, fBlowWindowSouth, FALSE);
}

static uint32_t ChanceOfBulletHittingStructure(int32_t iDistance, int32_t iDistanceToTarget,
                                               int16_t sHitBy) {
...
static int32_t StructureResistanceIncreasedByRange(int32_t iImpactReduction, int32_t iGunRange,
                                                   int32_t iDistance) {
  ...
  {
          return( iImpactReduction * ( 100 + PERCENT_BULLET_SLOWED_BY_RANGE *
  (iDistance - iGunRange) / iGunRange ) / 100 );
  }
  else
  {
          return( iImpactReduction );
...
static int32_t HandleBulletStructureInteraction(BULLET *pBullet, STRUCTURE *pStructure,
                                                BOOLEAN *pfHit) {
  DOOR *pDoor;
  int16_t sLockDamage;
  ...
  *pfHit = FALSE;
    ...
    if (pDoor && LockTable[pDoor->ubLockID].ubPickDifficulty < 50 &&
      ...
      if (PreRandom(2) == 0) {
        ...
        if (pDoor->bLockDamage > LockTable[pDoor->ubLockID].ubSmashDifficulty) {
          ...
          // succeeded! door can never be locked again, so remove from door
          // list...
  ...
  if (PreRandom(100) < pStructure->pDBStructureRef->pDBStructure->ubDensity) {
    ...
    // iCurrImpact = BulletImpactReducedByRange( pBullet->iImpact,
    // pBullet->iLoop, pBullet->iRange );
    ...
    // really weak stuff like grass should never *stop* a bullet, maybe slow it
    // though
...
static int32_t CTGTHandleBulletStructureInteraction(BULLET *pBullet, STRUCTURE *pStructure) {
  ...
  //	 scratch at each collision with an obstacle
  //   reduction due to range is 25% per "max range"
  // iCurrImpact = BulletImpactReducedByRange( pBullet->iImpact, pBullet->iLoop,
  // pBullet->iRange );
  iCurrImpact = pBullet->iImpact;
...
static uint8_t CalcChanceToGetThrough(BULLET *pBullet) {
  FIXEDPT qLandHeight;
  int32_t iCurrAboveLevelZ;
  int32_t iCurrCubesAboveLevelZ;
  int16_t sDesiredLevel;
  ...
  do {
    ...
    while (pStructure) {
      if (pStructure->fFlags & ALWAYS_CONSIDER_HIT) {
        ...
        // only consider roofs if the flag is set; don't add them to the array
        ...
        if (&person != pBullet->pFirer && &person != pBullet->target) {
          // ignore intended target since we will get closure upon reaching the
          // center of the destination tile

          // ignore intervening target if not visible; PCs are always visible so
          // AI will never skip them on that basis
          if (!fIntended && person.bVisible == TRUE) {
            // in actually moving the bullet, we consider only count friends as
            // targets if the bullet is unaimed (buckshot), if they are the
            // intended target, or beyond the range of automatic friendly fire
            // hits OR a 1 in 30 chance occurs
            if (gAnimControl[person.usAnimState].ubEndHeight == ANIM_STAND &&
              ...
              gpLocalStructure[iNumLocalStructures] = pStructure;
              // CJC commented this out because of tank trying to shoot through
              // another tank
              // guiLocalStructureCTH[iNumLocalStructures] = uiChanceOfHit;
              guiLocalStructureCTH[iNumLocalStructures] = 100;
              gubLocalStructureNumTimesHit[iNumLocalStructures] = 0;
    ...
    do {
      ...
      // check for the existence of structures
      pStructure = pMapElement->pStructureHead;
      if (pStructure == NULL) {  // no structures in this tile, and THAT INCLUDES
                                 // ROOFS AND PEOPLE! :-)
        // new system; figure out how many steps until we cross the next edge
        // and then fast forward that many steps.
            ...
            if (pStructure && pStructure->sCubeOffset == sDesiredLevel) {
              if (((*(pStructure->pShape))[pBullet->bLOSIndexX][pBullet->bLOSIndexY] &
                ...
                if (pStructure->fFlags & STRUCTURE_PERSON) {
                  ...
                  if (fResolveHit) {
                    // the bullet would keep on going!  unless we're considering
                    // a knife...
                    if (pBullet->usFlags & BULLET_FLAG_KNIFE) {
                      gubLocalStructureNumTimesHit[iStructureLoop]++;
                    }
                  ...
                  if (pStructure->fFlags & STRUCTURE_WALLSTUFF) {
                    // possibly shooting at corner in which case we should let
                    // it pass
                    fResolveHit = ResolveHitOnWall(pStructure, iGridNo, pBullet->bLOSIndexX,
                                                   pBullet->bLOSIndexY, pBullet->ddHorizAngle);
                  } else {
        ...
        do {
          ...
          if (pRoofStructure) {
            ...
            if ((qLastZ > qWallHeight && pBullet->qCurrZ < qWallHeight) ||
              ...
              // CTGTHandleBulletStructureInteraction( pBullet, pRoofStructure );
        ...
        DebugLOS(
            String("  CTGT at %ld %ld %ld after moving in nonempty tile "
                   "from %ld %ld %ld",
                   pBullet->bLOSIndexX, pBullet->bLOSIndexY, pBullet->iCurrCubesZ, bOldLOSIndexX,
                   bOldLOSIndexY, iOldCubesZ));
    ...
    if (pBullet->iLoop > pBullet->iRange * 2) {
      ...
      // decrement is scaled down depending on how fast the bullet is (effective
      // range)
    ...
    if (iNumLocalStructures > 0) {
      for (iStructureLoop = 0; iStructureLoop < iNumLocalStructures; iStructureLoop++) {
        // Calculate the total impact based on the number of points in the
        // structure that were hit
...
static int8_t ChanceToGetThrough(SOLDIERTYPE *pFirer, GridNo end_pos, float dEndZ);

static uint8_t SoldierToSoldierChanceToGetThrough(SOLDIERTYPE *const pStartSoldier,
                                                  const SOLDIERTYPE *const pEndSoldier) {
  float dEndZPos;
  BOOLEAN fOk;
...
uint8_t SoldierToSoldierBodyPartChanceToGetThrough(SOLDIERTYPE *const pStartSoldier,
                                                   const SOLDIERTYPE *const pEndSoldier,
                                                   const uint8_t ubAimLocation) {
...
uint8_t SoldierToLocationChanceToGetThrough(SOLDIERTYPE *const pStartSoldier, const int16_t sGridNo,
                                            const int8_t bLevel, const int8_t bCubeLevel,
                                            const SOLDIERTYPE *const target) {
  float dEndZPos;
  ...
  if (pEndSoldier != NULL) {
    ...
    if (bCubeLevel) {
      ...
      int8_t const bStructHeight = GetStructureTargetHeight(sGridNo, bLevel == 1);
      if (bStructHeight > 0) {
...
uint8_t AISoldierToSoldierChanceToGetThrough(SOLDIERTYPE *const pStartSoldier,
                                             const SOLDIERTYPE *const pEndSoldier) {
  // Like a standard CTGT algorithm BUT fakes the start soldier at standing
  // height
  float dEndZPos;
  BOOLEAN fOk;
  uint8_t ubChance;
...
static void CalculateFiringIncrements(double ddHorizAngle, double ddVerticAngle,
                                      double dd2DDistance, BULLET *pBullet,
                                      double *pddNewHorizAngle, double *pddNewVerticAngle) {
  ...
  if (iMissedBy > 0) {
    ...
    // chance of shooting over target is 60 for horizontal shots, up to 80% for
    // shots at 22.5 degrees, and then down again to 50% for shots at 45+%.
    ...
    // Want to make sure that not too many misses actually hit the target after
    // all to miss a target at 1 tile is about 30 degrees off, at 5 tiles, 6
    // degrees off at 15 tiles, 2 degrees off.  Thus 30 degrees divided by the #
    // of tiles distance.
  ...
  *pddNewVerticAngle = ddVerticAngle;
  ...
  // this is the same as multiplying the X and Y increments by the projection of
  // the line in 3-space onto the horizontal plane, without reducing the X/Y
  // increments and thus slowing the LOS code
  pBullet->qIncrZ =
      FloatToFixed((float)(sin(ddVerticAngle) / sin((PI / 2) - ddVerticAngle) * 2.56));
}

static int8_t FireBullet(BULLET *pBullet, BOOLEAN fFake) {
  pBullet->iCurrTileX = FIXEDPT_TO_INT32(pBullet->qCurrX) / CELL_X_SIZE;
  pBullet->iCurrTileY = FIXEDPT_TO_INT32(pBullet->qCurrY) / CELL_Y_SIZE;
  pBullet->bLOSIndexX =
      CONVERT_WITHINTILE_TO_INDEX(FIXEDPT_TO_INT32(pBullet->qCurrX) % CELL_X_SIZE);
...
int8_t FireBulletGivenTarget(SOLDIERTYPE *const pFirer, const float dEndX, const float dEndY,
                             const float dEndZ, const uint16_t usHandItem, int16_t sHitBy,
                             const BOOLEAN fBuckshot, const BOOLEAN fFake) {
  // fFake indicates that we should set things up for a call to
  // ChanceToGetThrough
  float dStartZ;
  ...
  for (ubLoop = 0; ubLoop < ubShots; ubLoop++) {
    ...
    // NB we can only apply correction for leftovers if the bullet is going to
    // hit because otherwise the increments are not right for the calculations!
    if (pBullet->sHitBy >= 0) {
    ...
    // this distance limit only applies in a "hard" sense to fake bullets for
    // chance-to-get-through, but is used for determining structure hits by the
    // regular code
...
static int8_t ChanceToGetThrough(SOLDIERTYPE *const pFirer, const GridNo end_pos,
                                 const float dEndZ) {
  uint16_t weapon = pFirer->usAttackingWeapon;
  BOOLEAN buck_shot;
  if (Item[weapon].usItemClass == IC_GUN || Item[weapon].usItemClass == IC_THROWING_KNIFE) {
...
void MoveBullet(BULLET *const pBullet) {
  ...
  do {
    ...
    if (!GridNoOnVisibleWorldTile((int16_t)iGridNo) ||
      ...
      // NB remove bullet only flags a bullet for deletion; we still have access
      // to the information in the structure
      RemoveBullet(pBullet);
      ShotMiss(pBullet);
      return;
    }
    ...
    // calculate which level bullet is on for suppression and close call
    // purposes figure out the LOS cube level of the current point
    ...
    while (pStructure) {
      if (pStructure->fFlags & ALWAYS_CONSIDER_HIT) {
        ...
        // fence is special
        if (pStructure->fFlags & STRUCTURE_ANYFENCE) {
          // If the density of the fence is less than 100%, or this is the top
          // of the fence, then roll the dice NB cubes are 0 based, heights 1
          // based
          if (pStructure->pDBStructureRef->pDBStructure->ubDensity < 100) {
            // requires roll
            if (PreRandom(100) < uiChanceOfHit) {
              ...
              iNumLocalStructures++;
                     ...
                     (pBullet->bEndCubesAboveLevelZ >= iCurrCubesAboveLevelZ) &&
                     iCurrCubesAboveLevelZ == (StructureHeight(pStructure) - 1)) {
            // near firer and at top of structure and at same level as bullet's
            // start requires roll
            if (PreRandom(100) < uiChanceOfHit) {
              gpLocalStructure[iNumLocalStructures] = pStructure;
              gubLocalStructureNumTimesHit[iNumLocalStructures] = 0;
              iNumLocalStructures++;
                     ...
                     (iCurrCubesAboveLevelZ <= pBullet->bEndCubesAboveLevelZ) &&
                     iCurrCubesAboveLevelZ == (StructureHeight(pStructure) - 1)) {
            // near target and at top of structure and at same level as bullet's
            // end requires roll
            if (PreRandom(100) < uiChanceOfHit) {
              gpLocalStructure[iNumLocalStructures] = pStructure;
              gubLocalStructureNumTimesHit[iNumLocalStructures] = 0;
        ...
        if (&tgt != pBullet->pFirer) {
          ...
          // ignore *intervening* target if not visible; PCs are always visible
          // so AI will never skip them on that basis
          if (fIntended) {
            // could hit this person!
            gpLocalStructure[iNumLocalStructures] = pStructure;
            iNumLocalStructures++;
          } else if (pBullet->pFirer->uiStatusFlags & SOLDIER_MONSTER) {
            // monsters firing will always accidentally hit people but never
          ...
          if (tgt.bTeam == OUR_TEAM && pBullet->pFirer->bTeam != OUR_TEAM &&
              sDesiredLevel == tgt.bLevel) {
            tgt.fCloseCall = TRUE;
          }
          ...
          if (IS_MERC_BODY_TYPE(&tgt)) {
            ...
            if (tgt.bSide != pBullet->pFirer->bSide || pBullet->iLoop > MIN_DIST_FOR_HIT_FRIENDS) {
              // buckshot has only a 1 in 2 chance of applying a suppression
              // point
    ...
    if (pMapElement->ubAdjacentSoldierCnt > 0) {
      ...
      // figure out the LOS cube level of the current point
      // iCurrCubesAboveLevelZ = CONVERT_HEIGHTUNITS_TO_INDEX( FIXEDPT_TO_INT32(
          ...
          if (gubWorldMovementCosts[iAdjGridNo][sDesiredLevel][bDir] < TRAVELCOST_BLOCKED) {
            ...
            if (pTarget != NULL) {
              if (IS_MERC_BODY_TYPE(pTarget) && pBullet->pFirer->bSide != pTarget->bSide) {
                ...
                /*
                                                                                // this could be a
                   close call if ( pTarget->bTeam == OUR_TEAM && pBullet->pFirer->bTeam != OUR_TEAM
                   )
                                                                                {
                                                                                        pTarget->fCloseCall
                   = TRUE;
    ...
    do {
      ...
      // check for the existence of structures
      if (iNumLocalStructures == 0 && !pRoofStructure) {  // no structures in this tile, AND THAT
                                                          // INCLUDES ROOFS! :-)
        // new system; figure out how many steps until we cross the next edge
        // and then fast forward that many steps.
            ...
            if (pStructure && pStructure->sCubeOffset == sDesiredLevel) {
              if (((*(pStructure->pShape))[pBullet->bLOSIndexX][pBullet->bLOSIndexY] &
                ...
                if (pStructure->fFlags & STRUCTURE_PERSON) {
                  ...
                  if (fStopped) {
                    // remove bullet function now called from within
                    // BulletHitMerc, so just quit
                    return;
                  } else {
                    // set pointer to null so that we don't consider hitting
                    // this person again
                  ...
                  if (fResolveHit) {
                    if (pBullet->usFlags & BULLET_FLAG_KNIFE) {
                      ...
                      if (iRemainingImpact <= 0) {
                        // check angle of knife and place on ground
                        // appropriately
                        OBJECTTYPE Object;
                        ...
                        if (pStructure->ubWallOrientation == INSIDE_TOP_RIGHT ||
                          ...
                          if (pBullet->qIncrY > 0) {
                            // heading south so place wall to north, in same
                            // tile of window
                          } else {
                      ...
                      BulletHitWindow(pBullet,
                                      pBullet->iCurrTileX + pBullet->iCurrTileY * WORLD_COLS,
                                      pStructure->usStructureID, blow_south);
                      LocateBullet(pBullet);
                      // have to remove this window from future hit
                      // considerations so the deleted structure data can't be
                      // referenced!
                      gpLocalStructure[iStructureLoop] = NULL;
        ...
        do {
          ...
          if (pRoofStructure) {
            ...
            if ((qLastZ > qWallHeight && pBullet->qCurrZ <= qWallHeight) ||
              ...
              {
                      // ATE: Found this: Should we be calling this because if
              we do, it will
...
int32_t CheckForCollision(float dX, float dY, float dZ, float dDeltaX, float dDeltaY, float dDeltaZ,
  ...
  if (pStructure == NULL) {  // no structures in this tile
    ...
    //	if ( (dLastZ > WALL_HEIGHT && pBullet->dCurrZ < WALL_HEIGHT) || (dLastZ
    //< WALL_HEIGHT && pBullet->dCurrZ > WALL_HEIGHT))
    //	{
    //		// generate roof-hitting event
