...
static UICursorID HandleNonActivatedTargetCursor(SOLDIERTYPE *, GridNo map_pos, BOOLEAN show_APs,
                                                 BOOLEAN fRecalc, MouseMoveState);
static UICursorID HandleKnifeCursor(SOLDIERTYPE *, GridNo map_pos, BOOLEAN activated,
                                    MouseMoveState);
static UICursorID HandlePunchCursor(SOLDIERTYPE *, GridNo map_pos, BOOLEAN activated,
                                    MouseMoveState);
...
static UICursorID HandleNonActivatedTossCursor(SOLDIERTYPE *, GridNo map_pos, BOOLEAN recalc,
                                               MouseMoveState, ItemCursor);
...
static UICursorID HandleBombCursor(SOLDIERTYPE *, GridNo map_pos, BOOLEAN activated,
                                   MouseMoveState);
...
BOOLEAN GetMouseRecalcAndShowAPFlags(MouseMoveState *const puiCursorFlags,
                                     BOOLEAN *const pfShowAPs) {
  ...
  bool show_APs = false;
  if (cursor_flags != MOUSE_STATIONARY) { /* If cursor was previously stationary, make the
                                           * additional check of grid pos change */
...
UICursorID GetProperItemCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                               BOOLEAN const activated) {
  ...
  switch (item_cursor) {
    case TARGETCURS:
      cursor = activated
                   ? HandleActivatedTargetCursor(s, tgt_grid_no, recalc)
                   : HandleNonActivatedTargetCursor(s, tgt_grid_no, show_APs, recalc, cursor_flags);

      if (gCurrentUIMode == ACTION_MODE && gTacticalStatus.uiFlags & INCOMBAT && recalc && tgt &&
          IsValidTargetMerc(tgt) && EnoughAmmo(s, FALSE, HANDPOS) &&  // ATE: Check for ammo
          guiUIFullTargetFlags & ENEMY_MERC &&  // IF it's an ememy, goto confirm action mode
          guiUIFullTargetFlags & VISIBLE_MERC && !(guiUIFullTargetFlags & DEAD_MERC) &&
          !gfCannotGetThrough) {
    ...
    case TRAJECTORYCURS:
      cursor =
          activated && gfUIHandlePhysicsTrajectory
              ? HandleActivatedTossCursor()
              : HandleNonActivatedTossCursor(s, tgt_grid_no, recalc, cursor_flags, item_cursor);
      break;

    case PUNCHCURS:
      cursor = HandlePunchCursor(s, tgt_grid_no, activated, cursor_flags);
      break;
    case KNIFECURS:
      cursor = HandleKnifeCursor(s, tgt_grid_no, activated, cursor_flags);
      break;
    case AIDCURS:
      cursor = HandleAidCursor(s, map_pos, activated, cursor_flags);
      break;
    case BOMBCURS:
      cursor = HandleBombCursor(s, tgt_grid_no, activated, cursor_flags);
      break;
    case REMOTECURS:
      cursor = HandleRemoteCursor(s, activated, cursor_flags);
      break;
    case WIRECUTCURS:
      cursor = HandleWirecutterCursor(s, tgt_grid_no, cursor_flags);
      break;
    case REPAIRCURS:
      cursor = HandleRepairCursor(s, tgt_grid_no, cursor_flags);
      break;
    case JARCURS:
      cursor = HandleJarCursor(s, tgt_grid_no, cursor_flags);
      break;
    case TINCANCURS:
      cursor = HandleTinCanCursor(s, tgt_grid_no, cursor_flags);
      break;
    case REFUELCURS:
      cursor = HandleRefuelCursor(s, tgt_grid_no, cursor_flags);
      break;
    case INVALIDCURS:
      cursor = INVALID_ACTION_UICURSOR;
      break;
    default:
      break;
  }
...
static UICursorID HandleActivatedTargetCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                              BOOLEAN const recalc) {
  ...
  if (is_throwing_knife) {
    ...
    if (!(gTacticalStatus.uiFlags & INCOMBAT) &&
        gAnimControl[s->usAnimState].uiFlags & ANIM_STATIONARY && gUITargetShotWaiting) {
  ...
  if (max_point_limit_hit) {
    ...
    cursor = s->fDoSpread ? ACTION_TARGETREDBURST_UICURSOR : ACTION_TARGETCONFIRMBURST_UICURSOR;
  } else {
    switch (s->bShownAimTime) {
      ...
      case REFINE_AIM_MID1:
        cursor = ACTION_TARGETAIM2_UICURSOR;
        break;
      case REFINE_AIM_MID2:
        cursor = ACTION_TARGETAIM4_UICURSOR;
        break;
      case REFINE_AIM_MID3:
        cursor = ACTION_TARGETAIM6_UICURSOR;
        break;
      case REFINE_AIM_MID4:
        cursor = ACTION_TARGETAIM8_UICURSOR;
        break;
    }
  }
  ...
  if (!max_point_limit_hit) {
    ...
    } else if (!InRange(s, map_pos)) {  // OK, make buddy flash!
...
static UICursorID HandleNonActivatedTargetCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                                 BOOLEAN const show_APs, BOOLEAN const fRecalc,
                                                 MouseMoveState const uiCursorFlags) {
  bool const is_throwing_knife = Item[s->inv[HANDPOS].usItem].usItemClass == IC_THROWING_KNIFE;
  if (!is_throwing_knife) {
    if (gTacticalStatus.uiFlags & REALTIME || !(gTacticalStatus.uiFlags & INCOMBAT)) {
  ...
  } else if (!InRange(s, map_pos)) {  // Flash cursor!
...
static void DetermineCursorBodyLocation(SOLDIERTYPE *const s, BOOLEAN const display,
                                        BOOLEAN const recalc) {
  if (gTacticalStatus.ubAttackBusyCount > 0) { /* ATE: Return if attacker busy count > 0, this helps
                                                * in RT with re-setting the flag to random */
    return;
  }
  ...
  if (recalc) {
    ...
    if (!tgt) {  // Check if we can find a soldier here
  ...
  if (tgt->ubBodyType == CROW) {
    ...
    switch (s->bAimShotLocation) {
      ...
      case AIM_SHOT_TORSO:
        hit_location = TacticalStr[TORSO_HIT_LOCATION_STR];
        break;
      case AIM_SHOT_LEGS:
        hit_location = TacticalStr[LEGS_HIT_LOCATION_STR];
        break;
...
static UICursorID HandleKnifeCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                    BOOLEAN const activated, MouseMoveState const uiCursorFlags) {
...
static UICursorID HandlePunchCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                    BOOLEAN const activated, MouseMoveState const uiCursorFlags) {
  HandleUIMovementCursor(s, uiCursorFlags, map_pos, MOVEUI_TARGET_MERCS);
...
static UICursorID HandleAidCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                  BOOLEAN const activated, MouseMoveState const uiCursorFlags) {
...
static UICursorID HandleActivatedTossCursor() { return ACTION_TOSS_UICURSOR; }

static UICursorID HandleNonActivatedTossCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                               BOOLEAN const recalc,
                                               MouseMoveState const uiCursorFlags,
  ...
  // Add APs
  if (gTacticalStatus.uiFlags & TURNBASED && gTacticalStatus.uiFlags & INCOMBAT) {
    gsCurrentActionPoints = ubItemCursor == TRAJECTORYCURS
                                ? CalcTotalAPsToAttack(s, map_pos, TRUE, s->bShownAimTime / 2)
...
static UICursorID HandleWirecutterCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                         MouseMoveState const uiCursorFlags) {
...
static UICursorID HandleRepairCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                     MouseMoveState const uiCursorFlags) {
...
static UICursorID HandleRefuelCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                     MouseMoveState const uiCursorFlags) {
...
static UICursorID HandleJarCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                  MouseMoveState const uiCursorFlags) {
  HandleUIMovementCursor(s, uiCursorFlags, map_pos, MOVEUI_TARGET_JAR);
  return IsCorpseAtGridNo(map_pos, s->bLevel) ? GOOD_JAR_UICURSOR : BAD_JAR_UICURSOR;
}

static UICursorID HandleTinCanCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                     MouseMoveState const uiCursorFlags) {
...
static UICursorID HandleRemoteCursor(SOLDIERTYPE *const s, BOOLEAN const activated,
                                     MouseMoveState const uiCursorFlags) {
  if (gTacticalStatus.uiFlags & TURNBASED && gTacticalStatus.uiFlags & INCOMBAT) {
...
static UICursorID HandleBombCursor(SOLDIERTYPE *const s, GridNo const map_pos,
                                   BOOLEAN const activated, MouseMoveState const uiCursorFlags) {
