...
Version 0 -- Kris -- obsolete November 14, 1997
        The newly created soldier information had a couple bugs regarding
initialization Bug 1)	Soldier placements without detailed slots weren't
initialized. Bug 2)  The attitude variable was accidentally being generated like
attributes which usually put it completely out of range. Version 1 -- Kris --
obsolete January 7, 1998 Bug 3)  New changes to the wall decals, made the
currently existing ones in the world unpredictable, and are automatically
removed, so new ones can be used. Version 2 -- Kris -- obsolete February 3, 1998
...
Version 6 -- Kris -- obsolete March 9, 1998
  Bug 8)  Change droppable status of merc items so that they are all
undroppable. Version 7 -- Kris -- obsolete April 14, 1998 Bug 9)  Priority
placements have been dropped in favor of splitting it into two categories. The
first is Detailed placements, and the second is priority existance.  So, all
                                        current detailed placements will also
have priority existance. Version 8 -- Kris -- obsolete April 16, 1998 MAJOR
...
// This will automatically update obsolete map versions to the new ones.  This
// will even work in the game itself, but would require conversion to happen
...
static void UpdateOldVersionMap() {
  ...
  if (gMapInformation.ubMapVersion < 19) {
    // Do nothing, this is used to force regenerate the map edgepoints in map
    // edgepoints.c
  ...
  if (gMapInformation.ubMapVersion < 21) {
    ...
    // Laymen terms:  If any items slots are locked to be empty, make them empty
    // but available for random item generation.
  ...
  if (gMapInformation.ubMapVersion < 22) {  // Allow map edgepoints to be regenerated as new system
                                            // has been reenabled.
    gMapInformation.ubMapVersion = 22;
  }
  if (gMapInformation.ubMapVersion < 23) {  // Allow map edgepoints to be regenerated as new system
                                            // has been reenabled.
...
static void AutoCalculateItemNoOverwriteStatus() {
  ...
  // different cases: 1)  If detailed placement has item, the item "no overwrite"
  // flag is set 2)  If detailed placement doesn't have item, but item is set to
  // drop (forced empty slot), the "no overwrite" flag is set.
  CFOR_EACH_SOLDIERINITNODE(curr) {
    if (curr->pDetailedPlacement) {
      for (i = 0; i < NUM_INV_SLOTS; i++) {
        pItem = &curr->pDetailedPlacement->Inv[i];
        ...
        } else if (!(pItem->fFlags & OBJECT_UNDROPPABLE)) {  // case 2 (see
                                                             // above)
          pItem->fFlags |= OBJECT_NO_OVERWRITE;
        }
      }
...
void ValidateAndUpdateMapVersionIfNecessary() {
  // Older versions of mercs may require updating due to past bug fixes, new
  // changes, etc.
...
// This function is used to avoid conflicts between minor version updates and
// sector summary info. By updating the summary info in conjunction with minor
// version updates, we can avoid these conflicts and really prevent major map
// updates.
