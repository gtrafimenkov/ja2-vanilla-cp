...
bool IsThisVehicleAccessibleToSoldier(SOLDIERTYPE const &s, VEHICLETYPE const &v) {
  return !s.fBetweenSectors && !v.fBetweenSectors && s.sSectorX == v.sSectorX &&
         s.sSectorY == v.sSectorY && s.bSectorZ == v.sSectorZ &&
...
static bool AddSoldierToVehicle(SOLDIERTYPE &s, VEHICLETYPE &v) {
  ...
  if (vs) {
    // can't call SelectSoldier in mapscreen, that will initialize interface
    // panels!!!
    if (guiCurrentScreen == GAME_SCREEN) {
      SelectSoldier(vs, SELSOLDIER_FORCE_RESELECT);
    }

    PlayLocationJA2Sample(vs->sGridNo, g_vehicle_type_info[v.ubVehicleType].enter_sound, HIGHVOLUME,
                          1);
  }
  ...
  for (INT32 i = 0; i < seats; ++i) {
    ...
    // Are we the first?
    s.uiStatusFlags |= GetNumberInVehicle(v) == 1 ? SOLDIER_DRIVER : SOLDIER_PASSENGER;
    ...
    if (vs) {
      ...
      // can't call SetCurrentSquad OR SelectSoldier in mapscreen, that will
      // initialize interface panels!!!
      if (guiCurrentScreen == GAME_SCREEN) {
...
void SetSoldierExitHelicopterInsertionData(SOLDIERTYPE *const s) {
  ...
  if (s->sSectorX == BOBBYR_SHIPPING_DEST_SECTOR_X &&
      ...
      s->bSectorZ == BOBBYR_SHIPPING_DEST_SECTOR_Z) {  // This is Drassen, make insertion
                                                       // gridno specific
    s->ubStrategicInsertionCode = INSERTION_CODE_GRIDNO;
    s->usStrategicInsertionData = 10125;
  } else {  // Not anything different here - just use center gridno
...
static bool RemoveSoldierFromVehicle(SOLDIERTYPE &s) {
  ...
  if (IsHelicopter(v)) {     /* The vehicle the helicopter? It can continue moving
                              * when no soldiers aboard (Skyrider remains) */
    if (s.bLife >= OKLIFE) { /* Mark the sector as visited (flying around in the chopper
                              * doesn't, so this does it as soon as we get off it) */
    ...
    if (v.fBetweenSectors) { /* The vehicle was abandoned between sectors.
                              * Teleport it to the closer of its current and
                              * next sectors (it beats having it arrive empty
                              * later) */
...
BOOLEAN TakeSoldierOutOfVehicle(SOLDIERTYPE *const s) {
  ...
  if (s->sSectorX == gWorldSectorX && s->sSectorY == gWorldSectorY && s->bSectorZ == 0 &&
      s->bInSector && !InHelicopter(*s))  // helicopter isn't a soldiertype instance
  {
...
bool PutSoldierInVehicle(SOLDIERTYPE &s, VEHICLETYPE &v) {
  ...
  if (s.sSectorX == gWorldSectorX && s.sSectorY == gWorldSectorY && s.bSectorZ == 0 &&
      !IsHelicopter(v) && guiCurrentScreen == GAME_SCREEN) {
...
BOOLEAN ExitVehicle(SOLDIERTYPE *const s) {
  ...
  PlayLocationJA2Sample(vs.sGridNo,
                        g_vehicle_type_info[pVehicleList[vs.bVehicleID].ubVehicleType].enter_sound,
                        HIGHVOLUME, 1);
  return TRUE;
}
...
INT8 RepairVehicle(VEHICLETYPE const &v, INT8 const bRepairPtsLeft,
  ...
  // ARM: personally, I'd love to know where in Arulco the mechanic gets the
  // PARTS to do this stuff, but hey, it's a game!
...
void UpdateAllVehiclePassengersGridNo(SOLDIERTYPE *const vs) {
  ...
  CFOR_EACH_PASSENGER(v, i) { EVENT_SetSoldierPositionXY(*i, vs->dXPos, vs->dYPos, SSP_NONE); }
}
...
BOOLEAN OKUseVehicle(UINT8 ubProfile) {
  if (ubProfile == PROF_HUMMER) {
    ...
    // don't allow mercs to get inside vehicle if it's grounded (enemy
    // controlled, Skyrider owed money, etc.)
...
static void TeleportVehicleToItsClosestSector(const UINT8 ubGroupID) {
  ...
  // change where it is and where it's going, then make it arrive there.  Don't
  // check for battle
...
static bool CanSoldierDriveVehicle(SOLDIERTYPE const &s, INT32 const vehicle_id,
  ...
  return s.bAssignment == VEHICLE &&            // In a vehicle?
         ...
         !AM_AN_EPC(&s) && s.bLife >= OKLIFE &&      // Too wounded to drive?
...
void HandleVehicleMovementSound(const SOLDIERTYPE *const s, const BOOLEAN fOn) {
  ...
  if (fOn) {
    if (v->iMovementSoundID == NO_SAMPLE) {
      v->iMovementSoundID = PlayLocationJA2Sample(
          s->sGridNo, g_vehicle_type_info[v->ubVehicleType].move_sound, HIGHVOLUME, 1);
    }
  } else {
    if (v->iMovementSoundID != NO_SAMPLE) {
