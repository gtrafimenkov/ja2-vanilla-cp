                    ...
                    if (sel->bDoBurst) {
                      ...
                      *puiNewEvent = A_CHANGE_TO_CONFIM_ACTION;
                      ...
                      if (UIMouseOnValidAttackLocation(sel)) {
                        // OK< going into confirm will call a function that will
                        // automatically move us to shoot in most vases ( grenades
                        // need a confirm mode regardless )
                        *puiNewEvent = A_CHANGE_TO_CONFIM_ACTION;
              ...
              case MOVE_MODE:
                ...
                if (!HandleCheckForExitArrowsInput(FALSE) && gpItemPointer == NULL) {
                  if (gUIFullTarget != NULL && guiUIFullTargetFlags & OWNED_MERC) {
                    // Reset , if this guy is selected merc, reset any multi
                    // selections...
        ...
        if (g_ui_message_overlay == NULL && !gfRTHaveClickedRightWhileLeftDown) {
          ...
          switch (gCurrentUIMode) {
            ...
            case CONFIRM_MOVE_MODE: {
              // First check if we clicked on a guy, if so, make selected if it's
              // ours
                      ...
                      if (gfUIWaitingForUserSpeechAdvance && fCanCheckForSpeechAdvance) {
                        ...
                        switch (gCurrentUIMode) {
                          case MENU_MODE:
                            ...
                            // If we get a hit here and we're in menu mode, quit
                            // the menu mode
                            ...
                            if (!HandleCheckForExitArrowsInput(FALSE) && gpItemPointer == NULL) {
                              ...
                              if (gUIFullTarget != NULL && guiUIFullTargetFlags & OWNED_MERC) {
                                  ...
                                  if (pSoldier != NULL && gpItemPointer == NULL) {
                                      ...
                                      if (!_KeyDown(ALT)) {
                                        ...
                                        *puiNewEvent = I_SELECT_MERC;
                                        ...
                                        if (pSoldier->uiStatusFlags & SOLDIER_MULTI_SELECTED) {
                                          ...
                                          // OK, if we have a selected guy.. make
                                          // him part too....
                                          if (sel != NULL) {
                                    ...
                                    if (!_KeyDown(ALT)) {
                                      ...
                                      *puiNewEvent = I_SELECT_MERC;
                                      ...
                                      // OK, if we have a selected guy.. make him
                                      // part too....
                                      if (sel != NULL) {
                                        sel->uiStatusFlags |= SOLDIER_MULTI_SELECTED;
                                      }
                                ...
                                if (bReturnCode == -1) {
                                  ...
                                  // else
                                  {
                                    if (sel != NULL) {
                                      BeginDisplayTimedCursor(
                                          GetInteractiveTileCursor(guiCurrentUICursor, TRUE), 300);
                                  ...
                                  if (sel != NULL) {
                                    // First check if we clicked on a guy, if so,
                                    // make selected if it's ours
                                    const SOLDIERTYPE *const s = FindSoldierFromMouse();
                                    if (s && IsOwnedMerc(*s)) {
                                      // Select guy
                                      *puiNewEvent = I_SELECT_MERC;
                                      ...
                                      if (gsCurrentActionPoints == 0 && !gfUIAllMoveOn &&
                                        ...
                                        if (fResult == 1) {
                                          if (gfUIAllMoveOn) {
                                            // ATE: Select everybody in squad and
                                            // make move! Make move!
                                            *puiNewEvent = C_MOVE_MERC;
                                            fValidDoubleClickPossible = TRUE;
                                          } else {
                                            // We're on terrain in which we can
                                            // walk, walk If we're on terrain,
                                            if (gGameSettings.fOptions[TOPTION_RTCONFIRM]) {
                                              *puiNewEvent = C_WAIT_FOR_CONFIRM;
                                              gfPlotNewMovement = TRUE;
                                            } else {
                          ...
                          case LOOKCURSOR_MODE:
                            // If we cannot actually do anything, return to
                            // movement mode
                            *puiNewEvent = LC_LOOK;
                            break;
                ...
                if (!gfUIAllMoveOn) {
                  ...
                  if (gfUICanBeginAllMoveCycle) {
                    ...
                    if (!UIOKMoveDestination(sel, usMapPos)) {
                      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK,
                                TacticalStr[CANT_MOVE_THERE_STR]);
                      gfRTClickLeftHoldIntercepted = TRUE;
                    }
                    // else if ( gsCurrentActionPoints == 0 )
                    //{
                    //	ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK,
                    // TacticalStr[ NO_PATH ] ); 	gfRTClickLeftHoldIntercepted =
                    // TRUE;
                    //}
                    else {
            ...
            switch (gCurrentUIMode) {
              ...
              case MOVE_MODE: {
                ...
                if (tgt != NULL && !(tgt->uiStatusFlags & SOLDIER_VEHICLE) &&
                    guiUIFullTargetFlags & OWNED_MERC && guiUIFullTargetFlags & VISIBLE_MERC &&
