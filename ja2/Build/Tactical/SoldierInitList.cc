...
SOLDIERINITNODE *AddBasicPlacementToSoldierInitList(BASIC_SOLDIERCREATE_STRUCT const &bp) {
  ...
  *si->pBasicPlacement = bp;
  ...
  } else {  // TEMP: no sorting, just enemies
...
BOOLEAN SaveSoldiersToMap(HWFILE fp) {
  ...
  if (gMapInformation.ubNumIndividuals > MAX_NUM_SOLDIERS) return FALSE;

  // If we are perhaps in the alternate version of the editor, we don't want bad
  // things to happen.  This is probably the only place I know where the user
  // gets punished now.  If the person was in the alternate editor mode, then
  // decided to save the game, the current mercs may not be there.  This would be
  // bad.  What we do is override any merc editing done while in this mode, and
  // kill them all, while replacing them with the proper ones.  Not only that,
  // the alternate editing mode is turned off, and if intentions are to play the
  // game, the user will be facing many enemies!
...
void LoadSoldiersFromMap(HWFILE const f, bool stracLinuxFormat) {
  ...
  if (n_individuals > MAX_NUM_SOLDIERS) {
    throw std::runtime_error(
        "Corrupt map check failed. ubNumIndividuals is "
        "greater than MAX_NUM_SOLDIERS.");
  }
  ...
  for (uint32_t i = 0; i != n_individuals; ++i) {
    ...
    if (bp.fDetailedPlacement) { /* Add the static detailed placement
                                  * information in the same newly created node
                                  * as the basic placement. */
...
// Because soldiers, creatures, etc., maybe added to the game at anytime
// theoretically, the list will need to be sorted to reflect this.  It is quite
// likely that this won't be needed, but the flexibility is there just incase.
// Now the list is sorted in the following manner: -1st priority:  Any nodes
// containing valid pointers to soldiers are moved to the end of the list.
// We don't ever want to use two identical placements. -2nd priority:  Any nodes that have priority
// existance and detailed placement information are
// put first in the list. -3rd priority:	Any nodes that have priority existance and no
// detailed placement information are used next. -4th priority:	Any nodes that have detailed
// placement and no priority existance information are used next. -5th priority:  The rest of the
// nodes are basic
// placements and are placed in
// the center of the list.  Of 								these, they
...
static void SortSoldierInitList() {
  ...
  while (curr) {
    if (curr->pSoldier && curr != gSoldierInitTail) {  // This node has an existing soldier, so
                                                       // move to end of list.
      // copy node
      temp = curr;
      if (temp == gSoldierInitHead) {  // If we dealing with the head, we need to
                                       // move it now.
        gSoldierInitHead = gSoldierInitHead->next;
        if (gfOriginalList)
          gOriginalSoldierInitListHead = gSoldierInitHead;
  ...
  while (curr) {
    if (!curr->pSoldier && !curr->pBasicPlacement->fPriorityExistance && curr->pDetailedPlacement &&
        curr != gSoldierInitHead) {  // Priority existance nodes without detailed
                                     // placement info are moved to beginning of list
      // copy node
      temp = curr;
      if (temp == gSoldierInitTail) {  // If we dealing with the tail, we need to
                                       // move it now.
        gSoldierInitTail = gSoldierInitTail->prev;
        gSoldierInitTail->next = NULL;
        temp->prev = NULL;
  ...
  while (curr) {
    if (!curr->pSoldier && curr->pBasicPlacement->fPriorityExistance && curr->pDetailedPlacement &&
        curr != gSoldierInitHead) {  // Priority existance nodes are moved to
                                     // beginning of list
      // copy node
      temp = curr;
      if (temp == gSoldierInitTail) {  // If we dealing with the tail, we need to
  ...
  //			 priority placements are at the beginning of the list, then
  // the 			 basic placements are in the middle.
}
...
bool AddPlacementToWorld(SOLDIERINITNODE *const init) {
  ...
  // First check if this guy has a profile and if so check his location such
  // that it matches
  if (SOLDIERCREATE_STRUCT *const init_dp = init->pDetailedPlacement) {
    if (!gfEditMode) {
      ...
      if (pid != NO_PROFILE) {
        ...
        if (p.ubMiscFlags & (PROFILE_MISC_FLAG_RECRUITED | PROFILE_MISC_FLAG_EPCACTIVE))
          return false;
      ...
      // CJC, August 18, 1999: don't do this code unless the ice cream truck is
      // on our team
      if (init_dp->bBodyType == ICECREAMTRUCK &&
          FindSoldierByProfileIDOnPlayerTeam(
              ICECREAMTRUCK)) {  // Check to see if Hamous is here and not
                                 // recruited. If so, add truck
    ...
    CreateDetailedPlacementGivenStaticDetailedPlacementAndBasicPlacementInfo(&dp, init_dp,
                                                                             init->pBasicPlacement);
  } else {
    ...
    if (dp.bTeam == CIV_TEAM) {
      ...
      if (x == 5 && y == MAP_ROW_C) {
        ...
        if (dp.ubCivilianGroup == KINGPIN_CIV_GROUP &&
          ...
          if (dp.ubProfile == NO_PROFILE) {  // These guys should be guarding Tony
            dp.sInsertionGridNo = 13531 + PreRandom(8) * (PreRandom(1) ? -1 : 1) +
            ...
            switch (PreRandom(3)) {
              case 0:
                ...
                break;
              case 1:
                dp.bOrders = CLOSEPATROL;
                break;
              case 2:
                dp.bOrders = ONCALL;
                break;
            }
          } else if (dp.ubProfile == BILLY) {  // Billy should now be able to roam around
            dp.sInsertionGridNo = 13531 + PreRandom(30) * (PreRandom(1) ? -1 : 1) +
                                  PreRandom(30) * (PreRandom(1) ? -1 : 1) * WORLD_ROWS;
            dp.bOrders = SEEKENEMY;
          } else if (dp.ubProfile == MADAME) {  // She shouldn't be here
            return true;
      ...
      } else if (x == 3 && y == MAP_ROW_P && z == 0 &&
                 !gfInMeanwhile) {  // Special civilian setup for queen's palace
        if (gubFact[FACT_QUEEN_DEAD]) {
          ...
          if (dp.ubCivilianGroup != QUEENS_CIV_GROUP) {  // The free civilians aren't added if queen
                                                         // is alive
            return true;
          }
        }
      } else if (x == TIXA_SECTOR_X && y == TIXA_SECTOR_Y &&
                 z == 0) { /* Tixa prison, once liberated, should not have any
                            * civs without profiles unless they are kids */
        if (!StrategicMap[TIXA_SECTOR_X + TIXA_SECTOR_Y * MAP_WORLD_X].fEnemyControlled &&
            dp.ubProfile == NO_PROFILE && dp.bBodyType != HATKIDCIV &&
            dp.bBodyType != KIDCIV) {  // not there
          return true;
        }
      } else if (x == 13 && y == MAP_ROW_C && z == 0) {
        if (CheckFact(FACT_KIDS_ARE_FREE, 0) &&
            (dp.bBodyType == HATKIDCIV ||
             dp.bBodyType == KIDCIV)) {  // Not there any more, kids have been freed.
          return true;
        }
      }
    } else if (dp.bTeam == ENEMY_TEAM &&
               dp.ubSoldierClass == SOLDIER_CLASS_ELITE) { /* Special! Certain events in the game
                                                            * can cause profiled NPCs to become
                                                            * enemies. The two cases are adding
                                                            * Mike and Iggy. We will only add one
                                                            * NPC in any given combat and the
                                                            * conditions for setting the
                                                            * associated facts are done
                                                            * elsewhere. There is also another
                                                            * place where NPCs can get added,
                                                            * which is in TacticalCreateElite()
                                                            * used for inserting offensive
                                                            * enemies.  */
  ...
  if (SOLDIERTYPE *const s = TacticalCreateSoldier(dp)) {
    ...
    LiveMessage(
        "Failed to create soldier using TacticalCreateSoldier within "
        "AddPlacementToWorld");
    return false;
  }
}
...
void AddSoldierInitListTeamToWorld(int8_t const team) {
  ...
  for (SOLDIERINITNODE *i = gSoldierInitHead; i && !i->pSoldier; i = i->next) {
     ...
     * AddPlacementToWorld() returns false for people (civilians) who have a
     * profile but are not currently in the sector of the loaded map. It doesn't
     * mean that there isn't any remaining slot for them in this case.
     */
    if (!AddPlacementToWorld(i) &&
        i->pBasicPlacement->bTeam !=
            CIV_TEAM) { /* If it fails to create the soldier, it is likely that
                         * it is because the slots in the tactical engine are
                         * already full. Besides, the strategic AI shouldn't be
                         * trying to fill a map with more than the maximum
                         * allowable soldiers of team. All teams can have a max
                         * of 32 individuals, except for the player which is 20.
                         * Players aren't processed in this list anyway. */
      break;
    }
  }
}
...
void AddSoldierInitListEnemyDefenceSoldiers(uint8_t ubTotalAdmin, uint8_t ubTotalTroops,
  ...
  // Specs call for only one profiled enemy can be in a sector at a time due to
  // flavor reasons.
  gfProfiledEnemyAdded = FALSE;

  // Because the enemy defence forces work differently than the regular map
  // placements, the numbers of each type of enemy may not be the same.  Elites
  // will choose the best placements, then army, then administrators.
  ...
  // Now count the number of nodes that are basic placements of desired team AND
  // CLASS This information will be used to randomly determine which of these
  // placements will be added based on the number of slots we can still add.
  ...
  // FIRST we now have the numbers of available slots for each soldier class, so
  // loop through three times and randomly choose some (or all) of the matching
  // slots to fill.  This is done randomly.
  for (ubCurrClass = SOLDIER_CLASS_ADMINISTRATOR; ubCurrClass <= SOLDIER_CLASS_ARMY;
    ...
    // Now, loop through the priority existance and detailed placement section
    // of the list.
    FOR_EACH_SOLDIERINITNODE(curr) {
      if (ubMaxNum == 0 || *pCurrTotal == 0 || *pCurrSlots == 0 ||
          curr->pDetailedPlacement == NULL || !curr->pBasicPlacement->fPriorityExistance) {
        break;
      }
      if (!curr->pSoldier && curr->pBasicPlacement->bTeam == ENEMY_TEAM) {
        if (curr->pBasicPlacement->ubSoldierClass == ubCurrClass) {
          ...
          (*pCurrSlots)--;
          // With the decrementing of the slot vars in this manner, the chances
          // increase so that all slots will be full by the time the end of the
          // list comes up.
        }
      }
      curr = curr->next;
  ...
  // ADD PLACEMENTS WITH PRIORITY EXISTANCE AND NO DETAILED PLACEMENT
  // INFORMATION SECOND we now have the numbers of available slots for each
  // soldier class, so loop through three times and randomly choose some (or all)
  // of the matching slots to fill.  This is done randomly.
  for (ubCurrClass = SOLDIER_CLASS_ADMINISTRATOR; ubCurrClass <= SOLDIER_CLASS_ARMY;
       ubCurrClass++) {
    // First, prepare the counters.
    switch (ubCurrClass) {
    ...
    // Now, loop through the priority existance and non detailed placement
    // section of the list.
    curr = mark;
  ...
  // ADD PLACEMENTS WITH NO DETAILED PLACEMENT AND PRIORITY EXISTANCE
  // INFORMATION SECOND we now have the numbers of available slots for each
  // soldier class, so loop through three times and randomly choose some (or all)
  // of the matching slots to fill.  This is done randomly.
  for (ubCurrClass = SOLDIER_CLASS_ADMINISTRATOR; ubCurrClass <= SOLDIER_CLASS_ARMY;
  ...
  // This must be done to ensure all priority existant slots are filled before
  // ANY other slots are filled, even if that means changing the class of the
  // slot.  Also, assume that there are no matching fits left for priority
  // existance slots.  All of the matches have been already assigned in the above
  // passes. We'll have to convert the soldier type of the slot to match.
  curr = gSoldierInitHead;
  ...
  // we now have the numbers of available slots for each soldier class, so loop
  // through three times and randomly choose some (or all) of the matching slots
  // to fill.  This is done randomly.
  for (ubCurrClass = SOLDIER_CLASS_ADMINISTRATOR; ubCurrClass <= SOLDIER_CLASS_ARMY;
       ubCurrClass++) {
    // First, prepare the counters.
    switch (ubCurrClass) {
  ...
  // If we are at this point, that means that there are some compatibility
  // issues.  This is fine.  An example would be a map containing 1 elite
  // placement, and 31 troop placements.  If we had 3 elites move into this
  // sector, we would not have placements for two of them.  What we have to do
  // is override the class information contained in the list by choosing unused
  // placements, and assign them to the elites.  This time, we will use all free
  // slots including priority placement slots (ignoring the priority placement
  // information).
  ...
  // Now, loop through the entire list again, but for the last time.  All
  // enemies will be inserted now ignoring detailed placements and classes.
  FOR_EACH_SOLDIERINITNODE(curr) {
    if (ubFreeSlots == 0 || ubMaxNum == 0) break;
    if (!curr->pSoldier && curr->pBasicPlacement->bTeam == ENEMY_TEAM) {
      // Randomly determine if we will use this slot; the more available slots
      // in proportion to the number of enemies, the lower the chance of
      // accepting the slot.
      ...
      ubFreeSlots--;
      // With the decrementing of the slot vars in this manner, the chances
      // increase so that all slots will be full by the time the end of the list
      // comes up.
    }
  }
}
...
// First of all, they exist exclusively to the enemy troops, so if the militia
...
void AddSoldierInitListMilitia(uint8_t ubNumGreen, uint8_t ubNumRegs, uint8_t ubNumElites) {
  ...
  // First fill up only the priority existance slots (as long as the
  // availability and class are okay)
...
void AddSoldierInitListCreatures(BOOLEAN fQueen, uint8_t ubNumLarvae, uint8_t ubNumInfants,
  ...
  // First fill up only the priority existance slots (as long as the
  // availability and bodytypes match)
  FOR_EACH_SOLDIERINITNODE(curr) {
  ...
  // Now, if we still have creatures to place, do so completely randomly,
  // overriding priority placements, etc.
  FOR_EACH_SOLDIERINITNODE(curr) {
...
void SaveSoldierInitListLinks(HWFILE const hfile) {
  ...
  // Now, go through each node, and save just the ubSoldierID, if that soldier
  // is alive.
  FOR_EACH_SOLDIERINITNODE(curr) {
    ...
    FOR_EACH_SOLDIERINITNODE(curr) {
      ...
      if ((team[ENEMY_TEAM].bFirstID <= soldier_id && soldier_id <= team[CREATURE_TEAM].bLastID) ||
          (team[CIV_TEAM].bFirstID <= soldier_id &&
           soldier_id <= team[CIV_TEAM].bLastID)) {  // only enemies, creatures
                                                     // and civilians
...
void AddSoldierInitListBloodcats() {
  ...
  if (pSector->bBloodCats > 0) {  // Add them to the world now...
    ...
    // This information will be used to randomly determine which of these
    // placements will be added based on the number of slots we can still add.
    mark = curr;
    ubSlotsAvailable = 0;
    ...
    // we now have the number, so compared it to the num we can add, and
    // determine how we will randomly determine which nodes to add.
    if (!ubSlotsAvailable) {  // There aren't any basic placements of desired
                              // team, so exit.
      return;
    }
    curr = mark;
    // while we have a list, with no active soldiers, the num added is less than
    // the max num requested, and we have slots available, process the list to
    // add new soldiers.
      ...
      if (curr->pBasicPlacement->bBodyType == BLOODCAT) {
        if (ubSlotsAvailable <= ubSlotsToFill || Random(ubSlotsAvailable) < ubSlotsToFill) {
          ...
          if (AddPlacementToWorld(curr)) {
            ...
            // if it fails to create the soldier, it is likely that it is
            // because the slots in the tactical engine are already full.
            // Besides, the strategic AI shouldn't be trying to fill a map with
            // more than the maximum allowable soldiers of team.  All teams can
            // have a max of 32 individuals, except for the player which is 20.
            // Players aren't processed in this list anyway.
            return;
          }
          ubSlotsToFill--;
...
void AddProfilesUsingProfileInsertionData() {
  for (int32_t i = FIRST_RPC; i != PROF_HUMMER; ++i) {
    ...
    if (!ps) {  // Create a new soldier, as this one doesn't exist
    ...
    if (p.ubQuoteRecord != 0) {
      ...
      if (s.ubQuoteActionID == QUOTE_ACTION_ID_CHECKFORDEST) {  // Gridno will have been changed to
                                                                // destination, so we're there
...
void NewWayOfLoadingEnemySoldierInitListLinks(HWFILE const f) {
  ...
  for (uint8_t n = slots; n != 0; --n) {
    ...
    FOR_EACH_SOLDIERINITNODE(curr) {
      ...
      curr->ubSoldierID = soldier_id;
      TacticalTeamType const *const team = gTacticalStatus.Team;
      if (soldier_id < team[ENEMY_TEAM].bFirstID || team[CREATURE_TEAM].bLastID < soldier_id)
        continue;
...
void StripEnemyDetailedPlacementsIfSectorWasPlayerLiberated() {
  if (!gfWorldLoaded || gbWorldSectorZ != 0) { /* No world loaded or underground.  Underground
                                                * sectors don't matter seeing enemies (not
                                                * creatures) never rejuvenate underground. */
    return;
  }

  SECTORINFO const &sector = SectorInfo[SECTOR(gWorldSectorX, gWorldSectorY)];
  if (sector.uiTimeLastPlayerLiberated == 0) {  // The player has never owned the sector.
    return;
  }
