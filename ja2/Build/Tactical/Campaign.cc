...
// Convert hired mercs' stats subpoint changes into actual point changes where
// warranted
static void ProcessUpdateStats(MERCPROFILESTRUCT &, SOLDIERTYPE *);

static void ProcessStatChange(MERCPROFILESTRUCT &, StatKind, uint16_t usNumChances,
                              StatChangeCause);

/* Give soldier n_chances to improve stat. If it's from training, it doesn't
 * count towards experience level gain */
void StatChange(SOLDIERTYPE &s, StatKind const stat, uint16_t const n_chances,
                StatChangeCause const reason) {
  Assert(s.bActive);
  ...
  if (s.bAssignment == ASSIGNMENT_POW) {
    ScreenMsg(FONT_ORANGE, MSG_BETAVERSION,
              L"ERROR: StatChange: %ls improving stats while POW! stat %d", s.name, stat);
    return;
  }
...
// this is the equivalent of StatChange(), but for use with mercs not currently
// on player's team give pProfile usNumChances to improve ubStat.  If it's from
// training, it doesn't count towards experience level gain
static void ProfileStatChange(MERCPROFILESTRUCT &p, StatKind const ubStat,
                              uint16_t const usNumChances, StatChangeCause const ubReason) {
  // dead guys don't do nuthin' !
  if (p.bMercStatus == MERC_IS_DEAD) return;

  if (p.bLife < OKLIFE) return;
...
static uint16_t SubpointsPerPoint(StatKind, int8_t bExpLevel);

static void ProcessStatChange(MERCPROFILESTRUCT &p, StatKind const ubStat,
                              uint16_t const usNumChances, StatChangeCause const ubReason) {
  ...
  if (p.bEvolution == NO_EVOLUTION) return;  // No change possible, quit right away
  ...
  switch (ubStat) {
    case HEALTHAMT:
      ...
      // NB physical stat checks not affected by wisdom, unless training is going
      // on
    ...
    default:
      // BETA message
      ScreenMsg(FONT_ORANGE, MSG_BETAVERSION, L"ERROR: ProcessStatChange: Rcvd unknown ubStat %d",
                ubStat);
      return;
  }
  ...
  for (uiCnt = 0; uiCnt < usNumChances; uiCnt++) {
    ...
    {
      // if this is improving from a failure, and a successful roll would give
      // us enough to go up a point
      ...
      if (ubStat != EXPERAMT) {
        // NON-experience level changes, actual usChance depends on
        // bCurrentRating Base usChance is '100 - bCurrentRating'
        usChance = 100 - (bCurrentRating + (*psStatGainPtr / usSubpointsPerPoint));

        // prevent training beyond the training cap
        if ((ubReason == FROM_TRAINING) &&
            (bCurrentRating + (*psStatGainPtr / usSubpointsPerPoint) >= TRAINING_RATING_CAP)) {
          usChance = 0;
        }
      } else {
      ...
      if (usChance > 0 && fAffectedByWisdom) {
        usChance +=
            (usChance *
             (p.bWisdom + (p.sWisdomGain / SubpointsPerPoint(WISDOMAMT, p.bExpLevel)) - 50)) /
            100;
      }
      ...
      */
      ...
      if (PreRandom(100) < usChance) {
        ...
        // as long as we're not dealing with exp_level changes (already added
        // above!) and it's not from training, and the exp level isn't max'ed
        // out already
        if ((ubStat != EXPERAMT) && (ubReason != FROM_TRAINING)) {
          ...
          if (uiEffLevel < MAXEXPLEVEL) {
            // if this is NOT improving from a failure, OR it would NOT give us
            // enough to go up a level
            if (ubReason != FROM_FAILURE || p.sExpLevelGain + 1 < usSubpointsPerLevel) {
              // all other stat changes count towards experience level changes
              // (1 for 1 basis)
              p.sExpLevelGain++;
            }
          }
    ...
    {
      // regression can happen from both failures and successes (but not
      // training, checked above)
      ...
      if (ubStat != EXPERAMT) {
        ...
        switch (ubStat) {
          ...
          case WISDOMAMT:
          case STRAMT:
            // Base usChance is 'bCurrentRating - 1', since these must remain at
            // 1-100
      ...
      if (PreRandom(100) < usChance) {
        ...
        if ((ubStat != EXPERAMT) && (ubReason != FROM_TRAINING)) {
          ...
          if (uiEffLevel > 1) {
            // all other stat changes count towards experience level changes (1
            // for 1 basis)
            p.sExpLevelGain--;
          }
        }
  ...
  if (sSubPointChange != 0) {
    // debug message
    ScreenMsg(MSG_FONT_RED, MSG_DEBUG, L"%ls's %ls changed by %d", p.zNickname,
              wDebugStatStrings[ubStat], sSubPointChange);
  }
#endif
...
static void ProfileUpdateStats(MERCPROFILESTRUCT &p) { ProcessUpdateStats(p, NULL); }

static uint32_t CalcNewSalary(uint32_t uiOldSalary, BOOLEAN fIncrease, uint32_t uiMaxLimit);

static void ChangeStat(MERCPROFILESTRUCT &p, SOLDIERTYPE *const pSoldier, StatKind const ubStat,
                       int16_t const sPtsChanged) {
  ...
  if (sPtsChanged != 0) {
    ...
    *pbStatPtr += sPtsChanged;
    ...
    // if this merc is currently on the player's team (DON'T count increases
    // earned outside the player's employ)
    if (pSoldier != NULL) {
      // also note the delta (how much this stat has changed since start of
      // game)
    ...
    *psStatGainPtr = (*psStatGainPtr) % usSubpointsPerPoint;
    ...
    if (pSoldier != NULL) {
      ...
      *pbSoldierStatPtr = *pbStatPtr;
      ...
      // if ( (ubStat != EXPERAMT) && (ubStat != HEALTHAMT) && ( Random( 100 ) <
      // 25 ) )
      {
        // Pipe up with "I'm getting better at this!"
        class CharacterDialogueEventDisplayStatChange : public CharacterDialogueEvent {
         public:
          CharacterDialogueEventDisplayStatChange(SOLDIERTYPE &soldier,
                                                  BOOLEAN const change_type_increase,
                                                  uint16_t const pts_changed, StatKind const stat)
          ...
          bool Execute() {
            ...
            wchar_t buf[128];
            BuildStatChangeString(buf, lengthof(buf), soldier_.name, change_type_increase_,
                                  pts_changed_, stat_);
         ...
         private:
          ...
          uint16_t const pts_changed_;
          StatKind const stat_;
        };

        DialogueEvent::Add(new CharacterDialogueEventDisplayStatChange(
            *pSoldier, fChangeTypeIncrease, sPtsChanged, ubStat));
        TacticalCharacterDialogue(pSoldier, QUOTE_EXPERIENCE_GAIN);
      } else {
        wchar_t wTempString[128];

        // tell player about it
        BuildStatChangeString(wTempString, lengthof(wTempString), pSoldier->name,
      ...
      // remember what time it changed at, it's displayed in a different color
    ...
    // merc salaries increase if level goes up (but they don't go down if level
    // drops!)
    if ((ubStat == EXPERAMT) && fChangeTypeIncrease) {
      ...
      if (pSoldier != NULL) {
        switch (pSoldier->ubWhatKindOfMercAmI) {
          ...
          case MERC_TYPE__MERC:
            ...
            // offset for the 2 profiles of Larry (we only have one email for
            // Larry..but 2 profile entries
            ...
            //	DEF: 03/06/99 Now sets an event that will be processed later in
            // the day 						ubEmailOffset = MERC_UP_LEVEL_BIFF +
            // MERC_UP_LEVEL_LENGTH_BIFF * ( ubMercMercIdValue );
            // AddEmail( ubEmailOffset, MERC_UP_LEVEL_LENGTH_BIFF, SPECK_FROM_MERC,
            // GetWorldTotalMin() );
      ...
      {
        // only AIM and M.E.R.C.s update stats when not on player's team, and
        // both of them DO change salary
        fChangeSalary = TRUE;
      }

      if (fChangeSalary) {
        // increase all salaries and medical deposits, once for each level
        // gained
        for (uiLevelCnt = 0; uiLevelCnt < (uint32_t)sPtsChanged; uiLevelCnt++) {
          p.sSalary = (int16_t)CalcNewSalary(p.sSalary, fChangeTypeIncrease, MAX_DAILY_SALARY);
          p.uiWeeklySalary = CalcNewSalary(p.uiWeeklySalary, fChangeTypeIncrease, MAX_LARGE_SALARY);
          p.uiBiWeeklySalary =
              CalcNewSalary(p.uiBiWeeklySalary, fChangeTypeIncrease, MAX_LARGE_SALARY);
          p.sMedicalDepositAmount = (int16_t)CalcNewSalary(p.sMedicalDepositAmount,
                                                           fChangeTypeIncrease, MAX_DAILY_SALARY);

          // if (pSoldier != NULL)
          // DON'T increase the *effective* medical deposit, it's already been
          // paid out pSoldier->usMedicalDeposit = p.sMedicalDepositAmount;
        }
      }
    }
  }
...
static void ProcessUpdateStats(MERCPROFILESTRUCT &p, SOLDIERTYPE *const pSoldier) {
  // this function will run through the soldier's profile and update their stats
  // based on any accumulated gain pts.
  int16_t *psStatGainPtr = NULL;
  int8_t *pbStatPtr = NULL;
  ...
  if (pSoldier != NULL) {
    // ATE: if in the midst of an attack, if in the field, delay all stat
    // changes until the check made after the 'attack'...
  ...
  for (StatKind ubStat = FIRST_CHANGEABLE_STAT; ubStat <= LAST_CHANGEABLE_STAT; ++ubStat) {
    ...
    // this stat (positive OR negative!) NOTE: for simplicity, this hopes nobody
    // will go up more than one level at once, which would change the
    // subpoints/pt
...
static uint32_t RoundOffSalary(uint32_t uiSalary);
...
static uint32_t CalcNewSalary(uint32_t uiOldSalary, BOOLEAN fIncrease, uint32_t uiMaxLimit) {
  ...
  if (uiNewSalary > uiMaxLimit) uiNewSalary = uiMaxLimit;

  if (uiNewSalary < 5) uiNewSalary = 5;
...
static uint32_t RoundOffSalary(uint32_t uiSalary) {
  ...
  if (uiSalary % uiMultiple) {
    // then we have to make it so, as Picard would say <- We have to wonder how
    // much Alex gets out and while we're at it, we round up to next higher
    // multiple if halfway
...
static uint16_t SubpointsPerPoint(StatKind const ubStat, int8_t const bExpLevel) {
  uint16_t usSubpointsPerPoint;
  ...
  switch (ubStat) {
    ...
    default:
      // BETA message
      ScreenMsg(FONT_ORANGE, MSG_BETAVERSION, L"SubpointsPerPoint: ERROR - Unknown ubStat %d",
                ubStat);
      return (100);
  }
...
void HandleUnhiredMercImprovement(MERCPROFILESTRUCT &p) {
  ...
  if (p.bMercStatus == MERC_WORKING_ELSEWHERE) {
    ...
    // 80 wisdom gives 8 rolls per stat per day, 10 stats, avg success rate 40%
    // = 32pts per day, so about 10 working days to hit lvl 2.  This seems high,
    // but mercs don't actually "work" that often, and it's twice as long to hit
    // level 3.  If we go lower, attribs & skills will barely move.
    usNumChances = p.bWisdom / 10;
    for (StatKind ubStat = FIRST_CHANGEABLE_STAT; ubStat <= LAST_CHANGEABLE_STAT; ++ubStat) {
      ProfileStatChange(p, ubStat, usNumChances, FROM_SUCCESS);
    }
  } else {
    // if the merc just takes it easy (high level or stupid mercs are more
    // likely to)
    if ((int8_t)Random(10) < p.bExpLevel || (int8_t)Random(100) > p.bWisdom) {
      // no chance to change today
      return;
    }
    ...
    StatKind ubStat;
    do {
      // pick ONE stat at random to focus on (it may be beyond training cap, but
      // so what, too hard to weed those out)
    ...
    ProfileStatChange(p, ubStat, (uint16_t)(p.bWisdom / 2), FROM_TRAINING);
  }
...
void HandleUnhiredMercDeaths(int32_t iProfileID) {
  ...
  if (!(p.ubMiscFlags3 & PROFILE_MISC_FLAG3_PLAYER_HAD_CHANCE_TO_HIRE)) {
    // then we're not allowed to kill him (to avoid really pissing off player by
    // killing his very favorite merc)
    return;
  }

  // how many in total can be killed like this depends on player's difficulty
  // setting
...
static uint8_t CalcImportantSectorControl();
...
uint8_t CurrentPlayerProgressPercentage() {
  ...
  if (uiPossibleIncome > 0) {
    ubCurrentProgress = (uint8_t)((uiCurrentIncome * PROGRESS_PORTION_INCOME) / uiPossibleIncome);
  } else {
    ubCurrentProgress = 0;
  }

  // kills per point depends on difficulty, and should match the ratios of
  // starting enemy populations (730/1050/1500)
  switch (gGameOptions.ubDifficultyLevel) {
    case DIF_LEVEL_EASY:
      ubKillsPerPoint = 7;
      break;
  ...
  // 19 sectors in mining towns + 3 wilderness SAMs each count double.  Balime &
  // Meduna are extra and not required
...
void HourlyProgressUpdate() {
  ...
  if (ubCurrentProgress > gStrategicStatus.ubHighestProgress) {
    ...
    // debug message
    ScreenMsg(MSG_FONT_RED, MSG_DEBUG, L"New player progress record: %d%%",
...
void AwardExperienceBonusToActiveSquad(uint8_t ubExpBonusType) {
  ...
  // to do: find guys in sector on the currently active squad, those that are
  // conscious get this amount in XPs
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s->bInSector && IsMercOnCurrentSquad(s) && s->bLife >= CONSCIOUSNESS && !IsMechanical(*s)) {
      StatChange(*s, EXPERAMT, usXPs, FROM_SUCCESS);
    }
  }
}

void BuildStatChangeString(wchar_t *const wString, size_t const Length, wchar_t const *const wName,
                           BOOLEAN const fIncrease, int16_t const sPtsChanged,
                           StatKind const ubStat) {
  ...
  swprintf(wString, Length, L"%ls %ls %d %ls %ls", wName, sPreStatBuildString[fIncrease ? 1 : 0],
           abs(sPtsChanged), sPreStatBuildString[ubStringIndex],
           sStatGainStrings[ubStat - FIRST_CHANGEABLE_STAT]);
}

static uint8_t CalcImportantSectorControl() {
  uint8_t ubMapX, ubMapY;
  uint8_t ubSectorControlPts = 0;
  ...
  for (ubMapX = 1; ubMapX < MAP_WORLD_X - 1; ubMapX++) {
    for (ubMapY = 1; ubMapY < MAP_WORLD_Y - 1; ubMapY++) {
      ...
      if (!StrategicMap[CALCULATE_STRATEGIC_INDEX(ubMapX, ubMapY)].fEnemyControlled) {
        // towns where militia can be trained and SAM sites are important
        // sectors
