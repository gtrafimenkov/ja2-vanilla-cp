...
// DESCRIPTION
//
//	The UI system here decouples event determination from event execution.
// IN other words, 	first any user input is gathered and analysed for an event to
// happen. Once the event is determined, 	it is then executed. For example, if the
// left mouse button is used to select a guy, it does not
//  execute the code to selected the guy, rather it sets a flag to a particular
//  event, in this case
//	the I_SELECT_MERC event is set. The code then executes this event after
// all input is analysed. In
//  this way, more than one input method from the user will cause the came event
//  to occur and hence no duplication of code. Also, events have cetain
//  charactoristics. The select merc event is executed just once and then
//  returns to the previous event. Most events are set to run continuously until
//  new input changes to another event. Other events have a 'SNAP-BACK' feature
//  which snap the mouse back to it's position before the event was executed.
//  Another issue is UI modes. In order to filter out input depending on other
//  flags, for example we do not want to cancel the confirm when a user moves to
//  another
//	tile unless we are in the 'confirm' mode.  This could be done by flags (
// and in effect it is ) where
//  if staements are used, but here at input collection time, we can switch on
//  our current mode to handle input differently based on the mode. Doing it
//  this way also allows us to group certain commands togther like menu commands
//  which are initialized and deleted in the same manner.
//
//	UI_EVENTS
/////////////
//
//	UI_EVENTS have flags to itendtify themselves with special
// charactoristics, a UI_MODE catagory which
//  signifies the UI mode this event will cause the system to move to. Also, a
//  pointer to a handle function is used to actually handle the particular
//  event. UI_EVENTS also have a couple of param variables and a number of
//  boolean flags used during run-time to determine states of events.
//
////////////////////////////////////////////////////////////////////////////////////////////////
...
static int16_t gsRequesterGridNo;
static int16_t gsOverItemsGridNo = NOWHERE;
static int16_t gsOverItemsLevel = 0;
static BOOLEAN gfUIInterfaceSetBusy = FALSE;
static uint32_t guiUIInterfaceBusyTime = 0;

static uint32_t gfTacticalForceNoCursor = FALSE;
...
static uint16_t gsOutOfRangeGridNo = NOWHERE;
static uint8_t gubOutOfRangeMerc = NOBODY;
static BOOLEAN gfOKForExchangeCursor = FALSE;
static uint32_t guiUIInterfaceSwapCursorsTime = 0;
int16_t gsJumpOverGridNo = 0;

#define M(flags, to, handler)                                      \
  {                                                                \
    flags, to, handler, FALSE, FALSE, DONT_CHANGEMODE, { 0, 0, 0 } \
  }
...
static uint16_t gusSavedMouseX;
static uint16_t gusSavedMouseY;
...
static uint32_t guiTimerLastUpdate = 0;
static uint32_t guiTimerCursorDelay = 0;
...
static int8_t gbAdjustStanceDiff = 0;
static int8_t gbClimbID = 0;
...
ScreenID HandleTacticalUI() {
  ...
  // Check if current event has changed and clear event if so, to prepare it for
  // execution Clearing it does things like set first time flag, param
  // variavles, etc
  ...
  // This is needed to hook into the interface stuff which sets the fDoneMenu
  // flag
  ...
  if (gCurrentUIMode == MOVE_MODE) {
    ...
    if (gfUIShowExitSouth) {
      ...
      if (gusMouseYPos < SCREEN_HEIGHT - 2) {
        ...
        // Define region for viewport
        MSYS_RemoveRegion(&gViewportRegion);
        MSYS_DefineRegion(&gViewportRegion, 0, 0, gsVIEWPORT_END_X, SCREEN_HEIGHT,
                          MSYS_PRIORITY_NORMAL, VIDEO_NO_CURSOR, MSYS_NO_CALLBACK,
                          MSYS_NO_CALLBACK);
...
static ScreenID UIHandleNewBadMerc(UI_EVENT *) {
  ...
  if (gbWorldSectorZ == 0) {
    ...
    if (UNDERGROUND_SECTORINFO *const sector =
...
static ScreenID UIHandleEnterEditMode(UI_EVENT *pUIEvent) { return (EDIT_SCREEN); }
...
static ScreenID UIHandleTestHit(UI_EVENT *pUIEvent) {
  ...
  if (gUIFullTarget != NULL) {
    ...
    EVENT_SoldierGotHit(tgt, 1, bDamage, 10, tgt->bDirection, 320, NULL, FIRE_WEAPON_NO_SPECIAL,
                        tgt->bAimShotLocation, NOWHERE);
  }
  return (GAME_SCREEN);
}
...
static bool UIHandleInteractiveTilesAndItemsOnTerrain(SOLDIERTYPE *pSoldier, int16_t usMapPos,
...
static ScreenID UIHandleMOnTerrain(UI_EVENT *pUIEvent) {
  ...
  if (!UIHandleOnMerc(TRUE)) {
    // Are we over items...
    const ITEM_POOL *pItemPool = GetItemPool(usMapPos, (uint8_t)gsInterfaceLevel);
      ...
      if (pUIEvent->uiParams[2] == MOVEMENT_MENU_LOOK) {
        ...
        switch (pUIEvent->uiParams[0]) {
          ...
          case MOVEMENT_MENU_SWAT:
            UIHandleSoldierStanceChange(sel, ANIM_CROUCH);
            break;
          case MOVEMENT_MENU_PRONE:
            UIHandleSoldierStanceChange(sel, ANIM_PRONE);
            break;
        }
...
static ScreenID UIHandlePositionMenu(UI_EVENT *pUIEvent) { return (GAME_SCREEN); }
...
static ScreenID UIHandleAOnTerrain(UI_EVENT *pUIEvent) {
  ...
  // ATE: Add stuff here to display a system message if we are targeting
  // smeothing and
  //  are out of range.
  // Are we using a gun?
  ...
  if (gTacticalStatus.uiFlags & REALTIME || !(gTacticalStatus.uiFlags & INCOMBAT)) {
    if (gAnimControl[sel->usAnimState].uiFlags & ANIM_STATIONARY &&
...
static ScreenID UIHandleCWait(UI_EVENT *) {
  ...
  if (int_tile && gpInvTileThatCausedMoveConfirm != int_tile) {  // Get out og this mode
    guiPendingOverrideEvent = A_CHANGE_TO_MOVE;
    return GAME_SCREEN;
  }
...
static BOOLEAN HandleMultiSelectionMove(int16_t sDestGridNo);

// NOTE, ONCE AT THIS FUNCTION, WE HAVE ASSUMED TO HAVE CHECKED FOR ENOUGH APS
// THROUGH SelectedMercCanAffordMove
    ...
    if (fAllMove) {
      ...
      FOR_EACH_IN_TEAM(pSoldier, OUR_TEAM) {
        if (OkControllableMerc(pSoldier) && pSoldier->bAssignment == CurrentSquad() &&
          ...
          // if ( !( gTacticalStatus.uiFlags & INCOMBAT ) && ( gAnimControl[
          // pSoldier->usAnimState ].uiFlags & ANIM_MOVING ) )
          //{
      ...
      if (gTacticalStatus.uiFlags & REALTIME || !(gTacticalStatus.uiFlags & INCOMBAT)) {
        sel->usUIMovementMode =
            GetMoveStateBasedOnStance(sel, gAnimControl[sel->usAnimState].ubEndHeight);
      }
      ...
      if (gTacticalStatus.fAtLeastOneGuyOnMultiSelect && pIntTile == NULL) {
        ...
        EVENT_InternalGetNewSoldierPath(sel, sDestGridNo, sel->usUIMovementMode, TRUE,
...
static ScreenID UIHandleCOnTerrain(UI_EVENT *pUIEvent) { return (GAME_SCREEN); }
...
static void UIHandleMercAttack(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pTargetSoldier,
                               uint16_t usMapPos) {
  ...
  // this is to stop the shooting trigger/happy duded from contiously pressing
  // fire...
...
static ScreenID UIHandleCAMercShoot(UI_EVENT *pUIEvent) {
  ...
  if (tgt != NULL) {
    ...
    if ((tgt->bTeam == OUR_TEAM || tgt->bTeam == MILITIA_TEAM) &&
        ...
        gTacticalStatus.ubLastRequesterTargetID != tgt->ubProfile && tgt != sel) {
      gpRequesterMerc = sel;
      ...
      DoMessageBox(MSG_BOX_BASIC_STYLE, zStr, GAME_SCREEN, MSG_BOX_FLAG_YESNO,
...
static ScreenID UIHandleCAEndConfirmAction(UI_EVENT *pUIEvent) { return GAME_SCREEN; }
...
static ScreenID UIHandleIChangeToIdle(UI_EVENT *pUIEvent) { return (GAME_SCREEN); }

static uint8_t GetAdjustedAnimHeight(uint8_t ubAnimHeight, int8_t bChange);
...
static ScreenID UIHandlePADJAdjustStance(UI_EVENT *pUIEvent) {
  ...
  if (gbClimbID == 1) {
    ...
    const uint8_t ubNewStance =
        GetAdjustedAnimHeight(gAnimControl[sel->usAnimState].ubEndHeight, gbAdjustStanceDiff);
    // Set state to result
...
BOOLEAN SelectedMercCanAffordAttack() {
  ...
  switch (ubItemCursor) {
    case INVALIDCURS:
      return FALSE;
    case BOMBCURS:
      return EnoughPoints(sel, GetTotalAPsToDropBomb(sel, usMapPos), 0, TRUE);
    case REMOTECURS:
      return EnoughPoints(sel, GetAPsToUseRemote(sel), 0, TRUE);

    default: {
      // Look for a soldier at this position
      const SOLDIERTYPE *const tgt = gUIFullTarget;
      const int16_t sTargetGridNo = (tgt != NULL ? tgt->sGridNo : usMapPos);
      const int16_t sAPCost =
          CalcTotalAPsToAttack(sel, sTargetGridNo, TRUE, sel->bShownAimTime / 2);
...
BOOLEAN SelectedMercCanAffordMove() {
  ...
  uint16_t sAPCost = PtsToMoveDirection(sel, guiPathingData[0]);
...
void ToggleLookCursorMode() {
  guiPendingOverrideEvent =
      gCurrentUIMode == LOOKCURSOR_MODE ? A_CHANGE_TO_MOVE : LC_CHANGE_TO_LOOK;
  HandleTacticalUI();
}
...
static void SetUIbasedOnStance(SOLDIERTYPE *pSoldier, int8_t bNewStance);
static BOOLEAN SoldierCanAffordNewStance(SOLDIERTYPE *pSoldier, uint8_t ubDesiredStance);
...
void UIHandleSoldierStanceChange(SOLDIERTYPE *s, int8_t bNewStance) {
  ...
  if ((gTacticalStatus.uiFlags & REALTIME) || !(gTacticalStatus.uiFlags & INCOMBAT)) {
    ...
    if (gAnimControl[s->usAnimState].uiFlags & ANIM_STATIONARY) {
      ...
      if (s->usUIMovementMode == CRAWLING &&
          gAnimControl[s->usAnimState].ubEndHeight != ANIM_PRONE) {
...
static ScreenID UIHandleIETEndTurn(UI_EVENT *pUIEvent) { return (GAME_SCREEN); }
...
MouseMoveState GetCursorMovementFlags() {
  ...
  } else if (fStationary && usOldMapPos == usMapPos)  // If cursor was previously stationary, make
                                                      // the additional check of grid pos change
  {
...
static int8_t DrawUIMovementPath(SOLDIERTYPE *pSoldier, uint16_t usMapPos, MoveUITarget);

BOOLEAN HandleUIMovementCursor(SOLDIERTYPE *const pSoldier, MouseMoveState const uiCursorFlags,
                               uint16_t const usMapPos, MoveUITarget const uiFlags) {
  ...
  if ((gTacticalStatus.uiFlags & REALTIME) || !(gTacticalStatus.uiFlags & INCOMBAT) ||
      ...
      pSoldier->fNoAPToFinishMove) {
    // If we are targeting a merc for some reason, don't go thorugh normal
    // channels if we are on someone now
...
static int8_t DrawUIMovementPath(SOLDIERTYPE *const pSoldier, uint16_t usMapPos,
                                 MoveUITarget const uiFlags) {
  int16_t sAPCost;
  int16_t sActionGridNo;
  STRUCTURE *pStructure;
  ...
  if (uiFlags == MOVEUI_TARGET_INTTILES) {
    ...
    if (pIntTile != NULL) {
      ...
      sAPCost += UIPlotPath(pSoldier, sActionGridNo, NO_COPYROUTE, fPlot,
    ...
    sAPCost += UIPlotPath(pSoldier, sActionGridNo, NO_COPYROUTE, fPlot, pSoldier->usUIMovementMode,
    ...
    if (IsRepairableStructAtGridNo(usMapPos, &tgt) == 2) {
      const int16_t sNewGridNo = FindGridNoFromSweetSpotWithStructDataFromSoldier(
          pSoldier, pSoldier->usUIMovementMode, 5, 0, tgt);
    ...
    if (tgt != NULL) {
      ...
      if (sGotLocation == NOWHERE) {
        sActionGridNo =
            FindAdjacentGridEx(pSoldier, tgt->sGridNo, NULL, &sAdjustedGridNo, TRUE, FALSE);
    ...
    if (sGotLocation != NOWHERE) {
      ...
      sAPCost += UIPlotPath(pSoldier, sGotLocation, NO_COPYROUTE, fPlot, pSoldier->usUIMovementMode,
                            pSoldier->bActionPoints);
    ...
    sAPCost += UIPlotPath(pSoldier, usMapPos, NO_COPYROUTE, fPlot, pSoldier->usUIMovementMode,
                          pSoldier->bActionPoints);
...
bool UIMouseOnValidAttackLocation(SOLDIERTYPE *const s) {
  ...
  if (HasObjectImprint(o) && s->ubProfile != o.ubImprintID) {  // Access denied
    PlayJA2Sample(RG_ID_INVALID, HIGHVOLUME, 1, MIDDLE);
    ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, L"\"%ls\"",
  ...
  if (item.usItemClass == IC_MEDKIT) {  // If a guy's here, check if he needs medical help!
    if (!tgt) return false;
    ...
    if (tgt->bBleeding == 0 && tgt->bLife != tgt->bLifeMax) {
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, gzLateLocalizedString[STR_LATE_19],
                tgt->name);
      return false;
    }
...
static ScreenID UIHandleLCOnTerrain(UI_EVENT *pUIEvent) {
  ...
  const int16_t sFacingDir = GetDirectionFromGridNo(pos, sel);
...
static BOOLEAN MakeSoldierTurn(SOLDIERTYPE *const pSoldier, const GridNo pos) {
  // Get direction from mouse pos
  const int16_t sFacingDir = GetDirectionFromGridNo(pos, pSoldier);

  if (sFacingDir != pSoldier->bDirection) {
    const int16_t sAPCost = GetAPsToLook(pSoldier);
...
static ScreenID UIHandleTOnTerrain(UI_EVENT *pUIEvent) {
  ...
  const uint32_t uiRange = GetRangeFromGridNoDiff(sel->sGridNo, sTargetGridNo);

  // ATE: Check if we have good LOS
  // is he close enough to see that gridno if he turns his head?
  const int16_t sDistVisible =
      DistanceVisible(sel, DIRECTION_IRRELEVANT, DIRECTION_IRRELEVANT, sTargetGridNo, sel->bLevel);
  ...
  if (tgt != NULL) {
    if (!SoldierTo3DLocationLineOfSightTest(sel, sTargetGridNo, sel->bLevel, 3, sDistVisible,
                                            TRUE)) {
...
static void GetGridNoScreenXY(int16_t sGridNo, int16_t *pScreenX, int16_t *pScreenY) {
...
static BOOLEAN HandleMultiSelectionMove(int16_t sDestGridNo) {
  ...
  CFOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s->bInSector && s->uiStatusFlags & SOLDIER_MULTI_SELECTED && s == sel) {
...
static ScreenID UIHandleJumpOver(UI_EVENT *pUIEvent) {
  ...
  const int8_t bDirection = GetDirectionFromGridNo(usMapPos, sel);
...
SOLDIERTYPE *GetValidTalkableNPCFromMouse(BOOLEAN fGive, BOOLEAN fAllowMercs,
  ...
  if (!IsValidTalkableNPC(tgt, fGive, fAllowMercs, fCheckCollapsed)) return NULL;
  return tgt;
}

BOOLEAN IsValidTalkableNPC(const SOLDIERTYPE *pSoldier, BOOLEAN fGive, BOOLEAN fAllowMercs,
                           BOOLEAN fCheckCollapsed) {
  ...
  if (pTSoldier != NULL) {
    ...
    if (IsValidTalkableNPC(pTSoldier, FALSE, TRUE, FALSE)) {
      if (pTSoldier != sel) {
        // ATE: Check if we have good LOS
        // is he close enough to see that gridno if he turns his head?
        const int16_t sDistVisible = DistanceVisible(
            sel, DIRECTION_IRRELEVANT, DIRECTION_IRRELEVANT, pTSoldier->sGridNo, pTSoldier->bLevel);

        // Check LOS!
        if (!SoldierTo3DLocationLineOfSightTest(sel, pTSoldier->sGridNo, pTSoldier->bLevel, 3,
                                                sDistVisible, TRUE)) {
          if (pTSoldier->ubProfile != NO_PROFILE) {
            ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, TacticalStr[NO_LOS_TO_TALK_TARGET],
                      sel->name, pTSoldier->name);
          } else {
            ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, gzLateLocalizedString[STR_LATE_45],
                      sel->name);
          }
          return (FALSE);
        }
      ...
      if (pTSoldier->bCollapsed) {
        ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, gzLateLocalizedString[STR_LATE_21],
                  pTSoldier->name);
        return (FALSE);
      }
      ...
      if (pTSoldier->bTeam == OUR_TEAM && !AM_AN_EPC(pTSoldier)) {
        if (pTSoldier->ubProfile == DIMITRI) {
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, gzLateLocalizedString[STR_LATE_32],
                    pTSoldier->name);
          return (FALSE);
        }
      ...
      // Check distance
      const uint32_t uiRange = GetRangeFromGridNoDiff(sel->sGridNo, usMapPos);
      ...
      if (uiRange > NPC_TALK_RADIUS) {
        ...
        const int16_t sActionGridNo =
            FindAdjacentGridEx(sel, pTSoldier->sGridNo, NULL, NULL, FALSE, TRUE);
        ...
        if (UIPlotPath(sel, sActionGridNo, NO_COPYROUTE, FALSE, sel->usUIMovementMode,
                       sel->bActionPoints) == 0) {
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, TacticalStr[NO_PATH]);
          return (FALSE);
        }
        ...
        const int16_t sGoodGridNo =
            FindGridNoFromSweetSpotWithStructData(sel, sel->usUIMovementMode, pTSoldier->sGridNo,
                                                  NPC_TALK_RADIUS - 1, &ubNewDirection, TRUE);
        ...
        EVENT_InternalGetNewSoldierPath(sel, sGoodGridNo, sel->usUIMovementMode, TRUE,
                                        sel->fNoAPToFinishMove);
...
void BeginDisplayTimedCursor(UICursorID const uiCursorID, uint32_t const uiDelay) {
...
static bool UIHandleInteractiveTilesAndItemsOnTerrain(SOLDIERTYPE *const pSoldier,
                                                      int16_t const usMapPos,
                                                      BOOLEAN const fUseOKCursor,
                                                      BOOLEAN const fItemsOnlyIfOnIntTiles) {
  ...
  if (tgt) {
    ...
    if (!fItemsOnlyIfOnIntTiles && guiUIFullTargetFlags & ENEMY_MERC &&
      ...
      // Set UI CURSOR
      guiNewUICursor = fUseOKCursor || (gTacticalStatus.uiFlags & INCOMBAT &&
  ...
  if (fItemsOnlyIfOnIntTiles && !pIntTile) {
    // If we want only on int tiles, and we have no int tiles, then ignore
    // items!
  } else if (fItemsOnlyIfOnIntTiles && pIntTile && pStructure->fFlags & STRUCTURE_HASITEMONTOP) {
    // if in this mode, we don't want to automatically show hand cursor over
    // items on strucutres
  } else if (pIntTile && pStructure->fFlags & STRUCTURE_SWITCH) {
    // We don't want switches messing around with items ever!
  } else if (pIntTile && pStructure->fFlags & STRUCTURE_ANYDOOR &&
             (sActionGridNo != usMapPos || fItemsOnlyIfOnIntTiles)) {
    // Next we look for if we are over a door and if the mouse position is !=
    // base door position, ignore items!
  } else {
...
BOOLEAN SelectedGuyInBusyAnimation() {
  ...
  return sel->usAnimState == LOB_ITEM || sel->usAnimState == THROW_ITEM ||
         sel->usAnimState == PICKUP_ITEM || sel->usAnimState == DROP_ITEM ||
         sel->usAnimState == OPEN_DOOR || sel->usAnimState == OPEN_STRUCT ||
         sel->usAnimState == OPEN_STRUCT || sel->usAnimState == END_OPEN_DOOR ||
         sel->usAnimState == END_OPEN_LOCKED_DOOR || sel->usAnimState == ADJACENT_GET_ITEM ||
         sel->usAnimState == DROP_ADJACENT_OBJECT || sel->usAnimState == OPEN_DOOR_CROUCHED ||
         sel->usAnimState == BEGIN_OPENSTRUCT_CROUCHED || sel->usAnimState == CLOSE_DOOR_CROUCHED ||
         sel->usAnimState == OPEN_DOOR_CROUCHED || sel->usAnimState == OPEN_STRUCT_CROUCHED ||
         ...
         sel->usAnimState == END_OPENSTRUCT_LOCKED_CROUCHED || sel->usAnimState == BEGIN_OPENSTRUCT;
}

void GotoHigherStance(SOLDIERTYPE *const s) {
  switch (gAnimControl[s->usAnimState].ubEndHeight) {
    case ANIM_STAND:
      if (FindHigherLevel(s)) BeginSoldierClimbUpRoof(s);
      break;
    case ANIM_CROUCH:
...
void GotoLowerStance(SOLDIERTYPE *const s) {
  switch (gAnimControl[s->usAnimState].ubEndHeight) {
    ...
    case ANIM_CROUCH:
      ...
      break;
    case ANIM_PRONE:
      if (FindLowerLevel(s)) BeginSoldierClimbDownRoof(s);
      break;
  }
}
...
BOOLEAN ValidQuickExchangePosition() {
  ...
  if (pOverSoldier != NULL) {
    // KM: Replaced this older if statement for the new one which allows
    // exchanging with militia if ( ( pOverSoldier->bSide != OUR_TEAM ) &&
    // pOverSoldier->bNeutral  )
    if ((pOverSoldier->bTeam != OUR_TEAM && pOverSoldier->bNeutral) ||
      ...
      if (!(pOverSoldier->uiStatusFlags & (SOLDIER_ANIMAL))) {
        ...
        if (sel != NULL && PythSpacesAway(sel->sGridNo, pOverSoldier->sGridNo) == 1) {
          ...
          const int16_t sDistVisible =
              DistanceVisible(sel, DIRECTION_IRRELEVANT, DIRECTION_IRRELEVANT,
                              pOverSoldier->sGridNo, pOverSoldier->bLevel);
          if (SoldierTo3DLocationLineOfSightTest(sel, pOverSoldier->sGridNo, pOverSoldier->bLevel,
                                                 3, sDistVisible, TRUE)) {
            // ATE:
            // Check that the path is good!
            if (FindBestPath(sel, pOverSoldier->sGridNo, sel->bLevel, sel->usUIMovementMode,
                             NO_COPYROUTE, PATH_IGNORE_PERSON_AT_DEST) == 1) {
              fOnValidGuy = TRUE;
            }
          }
        }
...
BOOLEAN IsValidJumpLocation(const SOLDIERTYPE *pSoldier, int16_t sGridNo, BOOLEAN fCheckForPath) {
  ...
  for (cnt = 0; cnt < 4; cnt++) {
    ...
    if (IS_TRAVELCOST_DOOR(ubMovementCost)) {
      ubMovementCost =
          DoorTravelCost(pSoldier, sIntSpot, ubMovementCost, pSoldier->bTeam == OUR_TEAM, NULL);
    }
    ...
    if (WhoIsThere2(sSpot, pSoldier->bLevel) == pSoldier) {
      ...
      if (NewOKDestination(pSoldier, sGridNo, TRUE, (int8_t)gsInterfaceLevel)) {
        ...
        if (!pSoldier->fTurningUntilDone) {
          ...
          if (guy_there != NULL && guy_there != pSoldier &&
