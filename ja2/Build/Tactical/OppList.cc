...
// this variable is a flag used in HandleSight to determine whether (while in
// non-combat RT) someone has just been seen, EITHER THE MOVER OR SOMEONE THE
// MOVER SEES
...
static const INT8 gbLookDistance[8][8] = {
    ...
    /* NORTHEAST */
    {ANGLE, STRAIGHT, ANGLE, SIDE, SBEHIND, BEHIND, SBEHIND, SIDE},
    /* EAST      */
    {SIDE, ANGLE, STRAIGHT, ANGLE, SIDE, SBEHIND, BEHIND, SBEHIND},
    /* SOUTHEAST */
    {SBEHIND, SIDE, ANGLE, STRAIGHT, ANGLE, SIDE, SBEHIND, BEHIND},
    /* SOUTH     */
    {BEHIND, SBEHIND, SIDE, ANGLE, STRAIGHT, ANGLE, SIDE, SBEHIND},
    /* SOUTHWEST */
    {SBEHIND, BEHIND, SBEHIND, SIDE, ANGLE, STRAIGHT, ANGLE, SIDE},
    /* WEST      */
    {SIDE, SBEHIND, BEHIND, SBEHIND, SIDE, ANGLE, STRAIGHT, ANGLE},
    /* NORTHWEST */
...
static void ReevaluateBestSightingPosition(SOLDIERTYPE *pSoldier, INT8 bInterruptDuelPts) {
  ...
  if (fPointsGotLower) {
    // loop to end of array less 1 entry since we can't swap the last entry out
    // of the array
    ...
    // this guy has fewer interrupt pts vs another enemy!  reduce position
    // unless in last place
    if (fFound) {
      // set new points
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("RBSP: reducing points for %d to %d", pSoldier->ubID, bInterruptDuelPts));
      ...
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("RBSP: reduced points for last individual %d to %d", pSoldier->ubID,
                      bInterruptDuelPts));
      pSoldier->bInterruptDuelPts = bInterruptDuelPts;
    }
  } else {
    ...
    if (!fFound) {
      for (ubLoop = 0; ubLoop < gubBestToMakeSightingSize; ubLoop++) {
        if (gBestToMakeSighting[ubLoop] == NULL ||
            bInterruptDuelPts > gBestToMakeSighting[ubLoop]->bInterruptDuelPts) {
          if (gBestToMakeSighting[gubBestToMakeSightingSize - 1] != NULL) {
            ...
            DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                     String("RBSP: resetting points for %d to zilch", pSoldier->ubID));
          }

          // set new points
          DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                   String("RBSP: setting points for %d to %d", pSoldier->ubID, bInterruptDuelPts));
          pSoldier->bInterruptDuelPts = bInterruptDuelPts;
    ...
    // else points didn't get lower, so do nothing (because we want to leave
    // each merc with as low int points as possible)
  }

  for (ubLoop = 0; ubLoop < BEST_SIGHTING_ARRAY_SIZE; ubLoop++) {
    if (gBestToMakeSighting[ubLoop] != NULL) {
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("RBSP entry %d: %d (%d pts)", ubLoop, gBestToMakeSighting[ubLoop]->ubID,
  ...
  if (gBestToMakeSighting[0] != NULL) {
    ...
    // if (gfHumanSawSomeoneInRealtime)
    {
      if (gBestToMakeSighting[1] == NULL) {
        ...
        // if 1st and 2nd on same team, or 1st and 3rd on same team, or there IS
        // no 3rd, award turn to 1st
...
void HandleSight(SOLDIERTYPE &s, SightFlags const sight_flags) {
  ...
  if (gubBestToMakeSightingSize != BEST_SIGHTING_ARRAY_SIZE_ALL_TEAMS_LOOK_FOR_ALL) {
     ...
     * array size */
    if (gTacticalStatus.uiFlags & INCOMBAT) {  // NB the incombat size is 0
  ...
  if (sight_flags & SIGHT_LOOK) {
    ...
    if (s.bLife >= OKLIFE) {  // He looks for all other soldiers not on his own team
  ...
  if (sight_flags & SIGHT_RADIO) {
    ...
    } else if (gGameOptions.ubDifficultyLevel >= DIF_LEVEL_MEDIUM) {  // Unless in easy mode allow
                                                                      // alerted enemies to radio
      // Don't allow admins to radio
      if (s.bTeam == ENEMY_TEAM && gTacticalStatus.Team[ENEMY_TEAM].bAwareOfOpposition &&
    ...
    FOR_EACH_MERC(i) {
      ...
      if (them.uiStatusFlags & SOLDIER_PC) {
        ...
        RadioSightings(&them, &s, them.bTeam);
                                                                        ...
                                                                        // alerted enemies to radio
        // Don't allow admins to radio
        if (them.bTeam == ENEMY_TEAM && gTacticalStatus.Team[ENEMY_TEAM].bAwareOfOpposition &&
...
static void OurTeamRadiosRandomlyAbout(SOLDIERTYPE *const about) {
  ...
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
     ...
     * our list */
    if (s->bInSector && s->bLife >= OKLIFE) radio_men[radio_cnt++] = s;
  }
...
static INT16 TeamNoLongerSeesMan(const UINT8 ubTeam, SOLDIERTYPE *const pOpponent,
  ...
  CFOR_EACH_IN_TEAM(pMate, ubTeam) {
    ...
    if (pMate == exclude) continue;  // skip to next teammate, I KNOW I don't see him...
    ...
    if (!pMate->bInSector || pMate->bLife < OKLIFE) continue;  // next merc
...
static INT16 DistanceSmellable(const SOLDIERTYPE *const pSubject) {
  ...
  if (pSubject) {
    if (pSubject->uiStatusFlags & SOLDIER_MONSTER) {
      ...
      // smelling a human or animal; if they are coated with monster smell,
      // distance shrinks
...
INT16 DistanceVisible(const SOLDIERTYPE *pSoldier, INT8 bFacingDir, INT8 bSubjectDir,
  ...
  if (!TANK(pSoldier) &&
    ...
    if (pSoldier->sGridNo == sSubjectGridNo) {
      // looking up or down or two people accidentally in same tile... don't
      // want it to be 0!
      sDistVisible = MaxDistanceVisible();
    } else {
    ...
    if (IsWearingHeadGear(*pSoldier, NIGHTGOGGLES) || IsWearingHeadGear(*pSoldier, UVGOGGLES) ||
      ...
      if (IsWearingHeadGear(*pSoldier, NIGHTGOGGLES) || AM_A_ROBOT(pSoldier)) {
        if (bLightLevel > NORMAL_LIGHTLEVEL_NIGHT) {
          // when it gets really dark, light-intensification goggles become less
          // effective
    ...
    if (pOtherSoldier->bOppList[pSoldier->ubID] == SEEN_CURRENTLY) {
      if (pOtherSoldier->sGridNo != NOWHERE) {
        if (PythSpacesAway(pOtherSoldier->sGridNo, pSoldier->sGridNo) >
          ...
          HandleManNoLongerSeen(pOtherSoldier, pSoldier, &(pOtherSoldier->bOppList[pSoldier->ubID]),
                                &(gbPublicOpplist[pOtherSoldier->bTeam][pSoldier->ubID]));
        }
        // else this person is still seen, if the looker is on our side or the
        // militia the person should stay visible
#ifdef WE_SEE_WHAT_MILITIA_SEES_AND_VICE_VERSA
...
static void ManLooksForOtherTeams(SOLDIERTYPE *pSoldier) {
  ...
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
           String("MANLOOKSFOROTHERTEAMS ID %d(%ls) team %d side %d", pSoldier->ubID,
  ...
  FOR_EACH_MERC(i) {
    ...
    if (pOpponent->bLife) {
      ...
      if (pSoldier->bTeam != pOpponent->bTeam) {
        ...
        // OK, We now want to , if in non-combat, set visiblity to 0 if not
        // visible still.... This allows us to walk away from buddy and have
...
static void HandleManNoLongerSeen(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pOpponent, INT8 *pPersOL,
  ...
  /*
#ifdef RECORDOPPLIST
fprintf(OpplistFile,"ManLooksForMan: changing personalOpplist to %d for guynum
%d, opp %d\n",SEEN_THIS_TURN,ptr->guynum,oppPtr->guynum); #endif
  ...
  *pPersOL = SEEN_THIS_TURN;
  ...
  if (*pbPublOL == SEEN_CURRENTLY) {
...
#ifdef TESTOPPLIST
      DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
               String("TeamNoLongerSeesMan: ID %d(%ls) to ID %d", pSoldier->ubID, pSoldier->name,
                      pOpponent->ubID));
#endif
...
#ifdef WE_SEE_WHAT_MILITIA_SEES_AND_VICE_VERSA
      if ((pSoldier->bTeam == OUR_TEAM || pSoldier->bTeam == MILITIA_TEAM) &&
          !(pOpponent->bTeam == OUR_TEAM || pOpponent->bTeam == MILITIA_TEAM))
#else
    ...
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("ManLooksForMan: ID %d(%ls) to ID %d Personally seen, public %d",
 ...
 */
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("ERROR: ManLooksForMan - WE are inactive/dead etc ID "
                    "%d(%ls)to ID %d",
  ...
  // if we're somehow looking for a guy who is inactive, at base, or already
  // dead
  if (!pOpponent->bActive || !pOpponent->bInSector || pOpponent->bLife <= 0 ||
      pOpponent->sGridNo == NOWHERE) {
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("ERROR: ManLooksForMan - TARGET is inactive etc ID %d(%ls)to ID %d",
                    pSoldier->ubID, pSoldier->name, pOpponent->ubID));
#endif
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("ERROR: ManLooksForMan - SAME TEAM ID %d(%ls)to ID %d", pSoldier->ubID,
 ...
 */
...
#ifdef TESTOPPLIST
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
           String("MANLOOKSFORMAN: ID %d(%ls) to ID %d: sDistAway %d sDistVisible %d",
  ...
  if (sDistAway <= sDistVisible) {
    ...
    if (SoldierToSoldierLineOfSightTest(pSoldier, pOpponent, (UINT8)sDistVisible, bAware)) {
      ...
      DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
               String("FAILED LINEOFSIGHT: ID %d (%ls)to ID %d Personally %d, "
                      "public %d",
  ...
  /*
  #ifdef RECORDOPPLIST
   fprintf(OpplistFile,"MLFM: %s by %2d(g%4d,x%3d,y%3d,%s) at
  %2d(g%4d,x%3d,y%3d,%s), aware %d, dA=%d,dV=%d, desDir=%d, %s\n", (success) ?
  "SCS" : "FLR",
  ...
  */
  ...
  if (!bSuccess && (*pPersOL == SEEN_CURRENTLY)) {
    ...
    if (!bSuccess) {
#ifdef TESTOPPLIST
      DebugMsg(
          TOPIC_JA2OPPLIST, DBG_LEVEL_3,
          String("NO LONGER VISIBLE ID %d (%ls)to ID %d Personally %d, "
                 "public %d success: %d",
      ...
      DebugMsg(
          TOPIC_JA2OPPLIST, DBG_LEVEL_3,
          String("COOL. STILL VISIBLE ID %d (%ls)to ID %d Personally %d, "
                 "public %d success: %d",
                 pSoldier->ubID, pSoldier->name, pOpponent->ubID, *pPersOL, *pbPublOL, bSuccess));
#endif
  }
...
static void ManSeesMan(SOLDIERTYPE &s, SOLDIERTYPE &opponent, UINT8 const caller2) {
  ...
  if (s.bOppList[opponent.ubID] != SEEN_CURRENTLY) {
    if (opponent.bTeam == OUR_TEAM) {
      if (s.ubProfile != NO_PROFILE) {
        if (s.bTeam == CIV_TEAM) {
          ...
          if (s.bNeutral) {
            if (s.ubCivilianGroup != NON_CIV_GROUP &&
                gTacticalStatus.fCivGroupHostile[s.ubCivilianGroup] >=
                    CIV_GROUP_WILL_BECOME_HOSTILE) {
              AddToShouldBecomeHostileOrSayQuoteList(&s);
              not_added_to_list = false;
            }
          } else if (NPCHasUnusedRecordWithGivenApproach(s.ubProfile,
            ...
            switch (s.ubProfile) {
              ...
              case TYRONE:
                ...
                if (s.bOrders != SEEKENEMY) {
                  s.bOrders = SEEKENEMY;
                  if (s.bOppCnt == 0) {  // Didn't see anyone before!
                    CancelAIAction(&s);
                    SetNewSituation(&s);
                  }
                }
              ...
              case ANGEL:
                if (opponent.ubProfile == MARIA) {
                  if (CheckFact(FACT_MARIA_ESCORTED_AT_LEATHER_SHOP, MARIA) ==
                      TRUE) {  // She was rescued! yay!
        ...
        if (s.ubCivilianGroup != NON_CIV_GROUP &&
            ...
            s.bNeutral) {
          AddToShouldBecomeHostileOrSayQuoteList(&s);
        } else if (s.ubCivilianGroup == KINGPIN_CIV_GROUP) {  // Generic kingpin goon

          // Check to see if we are looking at Maria or unauthorized personnel
          // in the brothel
          if (opponent.ubProfile == MARIA) {
            MakeCivHostile(&s, 2);
            if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
    ...
    if (!CONSIDERED_NEUTRAL(&opponent, &s) && !CONSIDERED_NEUTRAL(&s, &opponent) &&
        s.bSide != opponent.bSide) {
      AddOneOpponent(&s);

#ifdef TESTOPPLIST
      DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
               String("ManSeesMan: ID %d(%ls) to ID %d NEW TO ME", s.ubID, s.name, opponent.ubID));
#endif
      ...
      if (s.bOppList[opponent.ubID] != SEEN_THIS_TURN) {
        ...
        if (s.bTeam == OUR_TEAM && opponent.bTeam == ENEMY_TEAM &&
    ...
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("ManSeesMan: ID %d(%ls) to ID %d ALREADYSEENCURRENTLY", s.ubID, s.name,
                    opponent.ubID));
  }
#endif
  ...
  if (caller2 == MANLOOKSFOROTHERTEAMS || caller2 == OTHERTEAMSLOOKFORMAN ||
      caller2 == CALLER_UNKNOWN)  // unknown->hearing
  {
    if (gubBestToMakeSightingSize != BEST_SIGHTING_ARRAY_SIZE_INCOMBAT &&
        gTacticalStatus.bBoxingState == NOT_BOXING && new_opponent) {
      if (gTacticalStatus.uiFlags & INCOMBAT) { /* Presumably a door opening, we do require standard
        ...
        if (StandardInterruptConditionsMet(&s, &opponent, old_opp_list)) {
          ReevaluateBestSightingPosition(&s, CalcInterruptDuelPts(&s, &opponent, TRUE));
                 ...
                 opponent.bLife >= OKLIFE) { /* Require the enemy not to be dying if we are
                                              * the sighter; in other words, always add for AI
                                              * guys, and always add for people with life >=
                                              * OKLIFE */
        ReevaluateBestSightingPosition(&s, CalcInterruptDuelPts(&s, &opponent, TRUE));
      }
    }
  ...
  if (seen == FALSE) {  // Remember that he is just seeing him now for the first
                        // time (-1)
    seen = -1;
  } else {  // Man is seeing an opponent AGAIN whom he has seen at least once
            // before
    seen = TRUE;
  }
...
#endif
  {
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("!!! ID %d (%ls) MAKING %d VISIBLE", s.ubID, s.name, opponent.ubID));
#endif
    ...
    if (do_locate) {
      ...
      if (gTacticalStatus.uiFlags & TURNBASED &&
          ...
          !opponent.bNeutral && s.bSide != opponent.bSide) {
...
#else
  if ((pOpponent->bTeam != OUR_TEAM) && (pOpponent->bVisible >= 0 && pOpponent->bVisible < 2) &&
      pOpponent->bLife)
#endif
  {
...
#ifdef TESTOPPLIST
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
           String("OTHERTEAMSLOOKFORMAN ID %d(%ls) team %d side %d", pOpponent->ubID,
  ...
  FOR_EACH_MERC(i) {
    ...
    if (pSoldier->bLife >= OKLIFE && (pSoldier->ubBodyType != LARVAE_MONSTER)) {
      ...
      if (ManLooksForMan(pSoldier, pOpponent, OTHERTEAMSLOOKFORMAN)) {
        ...
        // & INCOMBAT) && pSoldier->bNewOppCnt)
        // Calc interrupt points in non-combat because we might get an interrupt
        // or be interrupted on our first turn
        ...
        if (gubBestToMakeSightingSize == BEST_SIGHTING_ARRAY_SIZE_INCOMBAT) {
          if ((gTacticalStatus.uiFlags & TURNBASED) && (gTacticalStatus.uiFlags & INCOMBAT) &&
            ...
            if (gubSightFlags & SIGHT_INTERRUPT &&
                StandardInterruptConditionsMet(pSoldier, pOpponent, bOldOppList)) {
              // calculate the interrupt duel points
              pSoldier->bInterruptDuelPts = CalcInterruptDuelPts(pSoldier, pOpponent, TRUE);
              DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                       String("Calculating int duel pts in "
                              "OtherTeamsLookForMan, %d has %d points",
...
static void AddOneOpponent(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->bTeam == OUR_TEAM) {
    // adding an opponent for player; reset # of turns that we haven't seen an
    // enemy
...
static void RemoveOneOpponent(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->bOppCnt < 0) {
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("Oppcnt for %d (%ls) tried to go below 0", pSoldier->ubID, pSoldier->name));
    pSoldier->bOppCnt = 0;
  }
...
static void SaySeenQuote(SOLDIERTYPE *pSoldier, BOOLEAN fSeenCreature, BOOLEAN fVirginSector) {
  ...
  if (fSeenCreature == 1) {
    ...
    if (gMercProfiles[pSoldier->ubProfile].ubMiscFlags & PROFILE_MISC_FLAG_HAVESEENCREATURE) {
      // Are there multiplaes and we have not said this quote during this
      // battle?
      if (!(pSoldier->usQuoteSaidFlags & SOLDIER_QUOTE_SAID_MULTIPLE_CREATURES)) {
        ...
        FOR_EACH_MERC(i) {
          const SOLDIERTYPE *const t = *i;
          if (OK_ENEMY_MERC(t) && t->uiStatusFlags & SOLDIER_MONSTER &&
...
void RadioSightings(SOLDIERTYPE *const pSoldier, SOLDIERTYPE *const about, UINT8 ubTeamToRadioTo) {
...
#ifdef TESTOPPLIST
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
           String("RADIO SIGHTINGS: for %d about %d", pSoldier->ubID, SOLDIER2ID(about)));
#endif
  ...
  for (iLoop = start; iLoop < end; iLoop++, pOpponent++, pPersOL++, pbPublOL++) {
    ...
    if ((!gubKnowledgeValue[*pbPublOL - OLDEST_HEARD_VALUE][*pPersOL - OLDEST_HEARD_VALUE]) &&
        (*pbPublOL != *pPersOL)) {
#ifdef RECORDOPPLIST
      // fprintf(OpplistFile,"no new knowledge (per %d, pub
      // %d)\n",*pPersOL,*pbPublOL);
#endif
    ...
    if (IsOnOurTeam(*pSoldier) && *pPersOL == SEEN_CURRENTLY &&
        pOpponent->bSide != pSoldier->bSide && !pOpponent->bNeutral) {
      // don't care whether and how many new enemies are seen if everyone
      // visible and he's healthy enough to be a threat (so is worth talking
      // about)

      // do the following if we're radioing to our own team; if radioing to
      // militia then alert them instead
          ...
          if (fContactSeen) {
            if (pSoldier->bTeam == OUR_TEAM) {
              if (gTacticalStatus.ubCurrentTeam != OUR_TEAM) {
                // Save some stuff!
                if (gTacticalStatus.fEnemySightingOnTheirTurn) {
                  // this has already come up so turn OFF the pause-all-anims
                  // flag for the previous person and set it for this next
                  // person
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3,
             String("...............UPDATE PUBLIC: soldier %d SEEING soldier %d", pSoldier->ubID,
                    pOpponent->ubID));
#endif
  ...
  if (IsOnOurTeam(*pSoldier)) {
    ...
    // if (!(gTacticalStatus.uiFlags & SHOW_ALL_MERCS))
    {
      ...
      if (revealedEnemies || unknownEnemies) {
        // First check for a virgin map and set to false if we see our first
        // guy.... Only if this guy is an ememy!
...
void DebugSoldierPage2() {
  ...
  if (s != NULL) {
    ...
    gprintf(150, y, L"%d %d %d %d %d %d %d %d", opp_list[0], opp_list[1], opp_list[2], opp_list[3],
    ...
    gprintf(150, y, L"%d %d %d", s->usPathingData[0], s->usPathingData[1], s->usPathingData[2]);

    GHeader(y += h, L"Next 3 Steps:");
    gprintf(150, y, L"%d %d %d", s->usPathingData[s->usPathIndex],
            s->usPathingData[s->usPathIndex + 1], s->usPathingData[s->usPathIndex + 2]);
...
void DebugSoldierPage3() {
  ...
  if (s != NULL) {
    ...
    if (gubWatchedLocPoints[s->ubID][0] > 0) {
      gprintf(350, y, L"Watch %d/%d for %d pts", gsWatchedLoc[s->ubID][0],
              gbWatchedLocLevel[s->ubID][0], gubWatchedLocPoints[s->ubID][0]);
    }
    ...
    if (gubWatchedLocPoints[s->ubID][1] > 0) {
      gprintf(350, y, L"Watch %d/%d for %d pts", gsWatchedLoc[s->ubID][1],
              gbWatchedLocLevel[s->ubID][1], gubWatchedLocPoints[s->ubID][1]);
    }

    GPrintStat(150, y += h, L"Last Action:", gzActionStr[s->bLastAction]);

    if (gubWatchedLocPoints[s->ubID][2] > 0) {
      gprintf(350, y, L"Watch %d/%d for %d pts", gsWatchedLoc[s->ubID][2],
              gbWatchedLocLevel[s->ubID][2], gubWatchedLocPoints[s->ubID][2]);
    }
    ...
    if (sel != NULL && sel->ubProfile < FIRST_NPC && s->ubProfile != NO_PROFILE) {
      GPrintStat(150, y += h, L"NPC Opinion:",
                 GetProfile(s->ubProfile).bMercOpinion[sel->ubProfile]);
    }
  } else {
    const GridNo map_pos = GetMouseMapPos();
    if (map_pos == NOWHERE) return;
    ...
    if (door == NULL) {
      ...
      wchar_t const *const door_state = door->ubFlags & DOOR_OPEN ? L"OPEN" : L"CLOSED";
      MPrintStat(200, y += h, L"Actual Status:", door_state);

      wchar_t const *const perceived_state = door->ubFlags & DOOR_PERCEIVED_NOTSET ? L"NOT SET"
...
static void WriteQuantityAndAttachments(OBJECTTYPE const *o, INT32 const y) {
  ...
  if (Item[o->usItem].usItemClass == IC_AMMO) {
    ...
    if (o->usAttachItem[0] || o->usAttachItem[1] || o->usAttachItem[2] || o->usAttachItem[3]) {
    ...
    if (s->bTeam != OUR_TEAM) {
      ...
      switch (s->bOrders) {
        ...
        case SEEKENEMY:
          ...
          break;
        case CLOSEPATROL:
          orders = L"CLOSE PATROL";
          break;
        case FARPATROL:
          orders = L"FAR PATROL";
          break;
        case POINTPATROL:
          orders = L"POINT PATROL";
          break;
        case RNDPTPATROL:
          orders = L"RND PT PATROL";
          break;
        default:
          orders = L"UNKNOWN";
          break;
      ...
      switch (s->bAttitude) {
        case DEFENSIVE:
          attitude = L"DEFENSIVE";
          break;
        case BRAVESOLO:
          attitude = L"BRAVE SOLO";
          break;
        case BRAVEAID:
          attitude = L"BRAVE AID";
          break;
        case AGGRESSIVE:
          attitude = L"AGGRESSIVE";
          break;
        case CUNNINGSOLO:
          attitude = L"CUNNING SOLO";
          break;
        case CUNNINGAID:
          attitude = L"CUNNING AID";
          break;
        default:
          attitude = L"UNKNOWN";
          break;
      ...
      y += h;
      if (node) {
        gprintf(0, y, L"%ls, %ls, REL EQUIP: %d, REL ATTR: %d", orders, attitude,
...
UINT8 MovementNoise(SOLDIERTYPE *pSoldier)  // XXX TODO000B
{
  ...
  /*
          ...
          if ((pSoldier->bTrait == OVER_ENTHUS) || (pSoldier->bAttitude ==
     AGGRESSIVE))
          {
...
void OurNoise(SOLDIERTYPE *const noise_maker, INT16 const sGridNo, INT8 const bLevel,
  ...
  if (noise_maker != NULL && gTacticalStatus.uiFlags & TURNBASED &&
      gTacticalStatus.uiFlags & INCOMBAT && !gfDelayResolvingBestSightingDueToDoor) {
...
static void ProcessNoise(SOLDIERTYPE *const noise_maker, INT16 const sGridNo, INT8 const bLevel,
  ...
  fprintf(OpplistFile, "PN: nType=%s, nMaker=%d, g=%d, bVol=%d\n", NoiseTypeStr[noiseType],
          SOLDIER2ID(noise_maker), sGridNo, baseVolume);
#endif
  ...
  if (noise_maker != NULL && ubNoiseType != NOISE_EXPLOSION) {
    ...
    if (!noise_maker->bActive || !noise_maker->bInSector ||
        noise_maker->uiStatusFlags & SOLDIER_DEAD) {
#ifdef BETAVERSION
      NumMessage(
          "ProcessNoise: ERROR - Noisemaker is inactive/not in "
          "sector/dead, Guy #",
          noise_maker->ubID);
#endif
      return;
    ...
    if (noise_maker != NULL) {
      switch (gbPublicOpplist[bTeam][noise_maker->ubID]) {
        ...
        default:
          ...
          {
                  ScreenMsg(MSG_FONT_YELLOW, MSG_TESTVERSION, L"Handling noise
          from person not currently seen in player's public opplist");
          }
          */
          break;
      }
    ...
    FOR_EACH_IN_TEAM(pSoldier, bTeam) {
      ...
      if (noise_maker != NULL) {
        ...
        if (pSoldier->bOppList[noise_maker->ubID] == SEEN_CURRENTLY) {
          // civilians care about gunshots even if they come from someone they
          // can see
        ...
        switch (noise_maker->bTeam) {
          case OUR_TEAM:
            // if the listener is militia and still on our side, ignore noise from
            // us
          ...
          case ENEMY_TEAM:
            ...
            break;
          case MILITIA_TEAM:
            // if the noisemaker is militia and still on our side, ignore noise if
            // we're listening
    ...
    if (bHeard) {
      ...
      if (bTeam == OUR_TEAM) {
        ...
        if (bTellPlayer && heard_loudest_by != NULL) {
          ...
          TellPlayerAboutNoise(heard_loudest_by, noise_maker, sGridNo, bLevel, ubLoudestEffVolume,
                               ubNoiseType, ubLoudestNoiseDir);
...
static UINT8 CalcEffVolume(SOLDIERTYPE const *const pSoldier, INT16 const sGridNo,
  ...
  AdjPixelsAway(pSoldier->x,pSoldier->y,CenterX(sGridNo),CenterY(sGridNo));
         ...
         distance /= 15;      // divide by 15 to convert from adj. pixels to
  tiles
         */
  ...
  {
          if (pSoldier->ubCivilianGroup == 0 && pSoldier->ubProfile ==
  NO_PROFILE)
          {
                  // nameless civs reduce effective volume by 2 for gunshots etc
                  // (double the reduction due to distance)
                  // so that they don't cower from attacks that are really far
  away switch (ubNoiseType)
                  {
                          case NOISE_GUNFIRE:
                          case NOISE_BULLET_IMPACT:
                          case NOISE_GRENADE_IMPACT:
          ...
          {
                  // NPCs and people in groups ignore attack noises unless they
  are no longer neutral switch (ubNoiseType)
                  {
                          case NOISE_GUNFIRE:
                          case NOISE_BULLET_IMPACT:
                          case NOISE_GRENADE_IMPACT:
...
static void HearNoise(SOLDIERTYPE *const pSoldier, SOLDIERTYPE *const noise_maker,
  ...
  // volume %d", pSoldier->ubID, SOLDIER2ID(noise_maker), sGridNo, bLevel,
  // ubVolume));
  ...
  if (ubNoiseType == NOISE_GUNFIRE && noise_maker != NULL && noise_maker->fMuzzleFlash) {
    ...
    if (pSoldier->bDirection != bDirection && pSoldier->bDirection != OneCDirection(bDirection) &&
        pSoldier->bDirection != OneCCDirection(bDirection)) {
      // temporarily turn off muzzle flash so DistanceVisible can be calculated
      // without it
  ...
  if (PythSpacesAway(pSoldier->sGridNo, sGridNo) <= sDistVisible) {
    ...
    fprintf(OpplistFile, "HN: %s by %2d(g%4d,x%3d,y%3d) at %2d(g%4d,x%3d,y%3d), hTT=%d\n",
            ...
            pSoldier->sY, SOLDIER2ID(noise_maker), sGridNo, sNoiseX, sNoiseY, bHadToTurn);
#endif
  }
  ...
  if (noise_maker != NULL) {
    ...
    } else  // noise maker still can't be seen
    {
      ...
      // CJC: set the noise gridno for the soldier, if appropriate - this is
      // what is looked at by the AI!
    ...
    if (!CONSIDERED_NEUTRAL(noise_maker, pSoldier) && !CONSIDERED_NEUTRAL(pSoldier, noise_maker) &&
      ...
      // regardless of whether the noisemaker (who's not NOBODY) was seen or
      // not, as long as listener meets minimum interrupt conditions
      if (gfDelayResolvingBestSightingDueToDoor) {
        if (bSourceSeen &&
  ...
  {
    // if noise type was unmistakably that of an explosion (seen or not) or
    // alarm
    if (!(pSoldier->uiStatusFlags & SOLDIER_PC)) {
    ...
    if (gubBestToMakeSightingSize == BEST_SIGHTING_ARRAY_SIZE_INCOMBAT) {
      ...
      if ((gTacticalStatus.uiFlags & TURNBASED) && (gTacticalStatus.uiFlags & INCOMBAT) &&
        ...
        // reasonable to assume the guy throwing wants to wait for their
        // reaction!
...
static void TellPlayerAboutNoise(SOLDIERTYPE *const s, SOLDIERTYPE const *const noise_maker,
  ...
  wchar_t const *const direction =
      level == s->bLevel || noise_type == NOISE_EXPLOSION || noise_type == NOISE_SCREAM ||
              noise_type == NOISE_ROCK_IMPACT || noise_type == NOISE_GRENADE_IMPACT
          ? pDirectionStr[noise_dir]
      : level > s->bLevel ? gzLateLocalizedString[STR_LATE_06]
                          :                    // From above
          gzLateLocalizedString[STR_LATE_07];  // From below
  ScreenMsg(MSG_FONT_YELLOW, MSG_INTERFACE, pNewNoiseStr[noise_type], s->name,
            pNoiseVolStr[volume_idx], direction);

  // If the sound was faint, say something
  if (volume_idx == 0 && !AreInMeanwhile() && !(gTacticalStatus.uiFlags & ENGAGED_IN_CONV) &&
...
#ifdef BETAVERSION
    sprintf(tempstr,
            "VerifyAndDecayOpplist: WARNING - %d(%ls) still has %d NEW "
            "OPPONENTS - lastCaller %ls/%ls",
  ...
  if (pSoldier->bLife < OKLIFE) {
    ...
    FOR_EACH_SOLDIER(tgt) {
      if (pSoldier->bOppList[tgt->ubID] == SEEN_CURRENTLY) {
        HandleManNoLongerSeen(pSoldier, tgt, &pSoldier->bOppList[tgt->ubID],
                              &gbPublicOpplist[pSoldier->bTeam][tgt->ubID]);
      }
    }
...
void VerifyPublicOpplistDueToDeath(SOLDIERTYPE *pSoldier) {
  ...
  FOR_EACH_MERC(i) {
    ...
    if (pOpponent->bLife) {
      ...
      // untimely demise)
      if (*pPersOL == SEEN_CURRENTLY) {
        // then we need to know if any teammates ALSO see this opponent, so loop
        // through trying to find ONE witness to the death...
        FOR_EACH_MERC(j) {
...
void RecalculateOppCntsDueToNoLongerNeutral(SOLDIERTYPE *pSoldier) {
  ...
  if (!pSoldier->bNeutral) {
    FOR_EACH_MERC(i) {
      SOLDIERTYPE *const pOpponent = *i;
      // for every living soldier on ANOTHER team
      if (pOpponent->bLife && !pOpponent->bNeutral && pOpponent->bTeam != pSoldier->bTeam &&
        ...
        if (pOpponent->bOppList[pSoldier->ubID] == SEEN_CURRENTLY) {
          // have to add to opponent's oppcount as well since we just became
          // non-neutral
...
void RecalculateOppCntsDueToBecomingNeutral(SOLDIERTYPE *pSoldier) {
  if (pSoldier->bNeutral) {
    ...
    FOR_EACH_MERC(i) {
      ...
      // for every living soldier on ANOTHER team
      if (pOpponent->bLife && !pOpponent->bNeutral && pOpponent->bTeam != pSoldier->bTeam &&
          !CONSIDERED_NEUTRAL(pSoldier, pOpponent) && pSoldier->bSide != pOpponent->bSide) {
        if (pOpponent->bOppList[pSoldier->ubID] == SEEN_CURRENTLY) {
          // have to rem from opponent's oppcount as well since we just became
          // neutral
          RemoveOneOpponent(pOpponent);
        }
      }
...
void NoticeUnseenAttacker(SOLDIERTYPE *pAttacker, SOLDIERTYPE *pDefender, INT8 bReason) {
  ...
  if (PythSpacesAway(pAttacker->sGridNo, pDefender->sGridNo) <= MaxDistanceVisible()) {
    ...
    if (pAttacker->fMuzzleFlash) {
      ...
      if (pDefender->bDirection != bDirection &&
        ...
        // temporarily turn off muzzle flash so DistanceVisible can be
  ...
  } else  // victim NOTICED the attack, but CAN'T SEE the actual attacker
  {
    ...
    if (pDefender->uiStatusFlags & SOLDIER_PC) {
      ...
      UpdatePublic(pDefender->bTeam, pAttacker, HEARD_THIS_TURN, pAttacker->sGridNo,
                   pAttacker->bLevel);
    }
  }
  ...
  if (StandardInterruptConditionsMet(pDefender, pAttacker, bOldOppList)) {
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("INTERRUPT: NoticeUnseenAttacker, standard conditions are "
                    "met; defender %d, attacker %d",
  ...
  if (pDefender->bInterruptDuelPts != NO_INTERRUPT) {
    ...
    if (InterruptDuel(pDefender, pAttacker)) {
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("INTERRUPT: NoticeUnseenAttacker, defender pts %d, "
                      "attacker pts %d, defender gets interrupt",
    ...
    for (cnt2 = 0; cnt2 < NUM_WATCHED_LOCS; cnt2++) {
      if (gsWatchedLoc[cnt][cnt2] != NOWHERE &&
        ...
        if (gfWatchedLocReset[cnt][cnt2]) {
          ...
          // flag was false so set to true (will be reset if new people seen
          // there next turn)
