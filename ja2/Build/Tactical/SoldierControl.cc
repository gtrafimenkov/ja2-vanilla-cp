...
static uint8_t Dir2ExtDir(const uint8_t dir) { return dir * 4; }

static uint8_t ExtOneCDirection(const uint8_t exdir) {
...
static const BATTLESNDS_STRUCT gBattleSndsData[] = {
    {"ok1", 2, 1, 1, 2},   {"ok2", 0, 1, 1, 2},   {"cool", 0, 0, 1, 0},   {"curse", 0, 1, 1, 0},
    {"hit1", 2, 1, 1, 1},  {"hit2", 0, 1, 1, 1},  {"laugh", 0, 1, 1, 0},  {"attn", 0, 0, 1, 0},
    {"die", 0, 1, 1, 1},   {"humm", 0, 0, 1, 1},  {"noth", 0, 0, 1, 1},   {"gotit", 0, 0, 1, 1},
    {"lmok1", 2, 0, 1, 2}, {"lmok2", 0, 0, 1, 2}, {"lmattn", 0, 0, 1, 0}, {"locked", 0, 0, 1, 0},
    {"enem", 0, 1, 1, 0}};

uint8_t bHealthStrRanges[] = {15, 30, 45, 60, 75, 90, 101};

static const int16_t gsTerrainTypeSpeedModifiers[] = {
    5,   // Flat ground
    5,   // Floor
    5,   // Paved road
    5,   // Dirt road
...
// Palette ranges
static uint32_t guiNumPaletteSubRanges;
static PaletteSubRangeType *gpPaletteSubRanges;
// Palette replacements
static uint32_t guiNumReplacements;
...
static bool IsCrowWithShadow(SOLDIERTYPE const &s) {
  return s.ubBodyType == CROW && s.usAnimState == CROW_FLY && s.pAniTile;
}
...
static const uint8_t gubMaxActionPoints[] = {
    AP_MAXIMUM,          // REGMALE
    AP_MAXIMUM,          // BIGMALE
...
int8_t CalcActionPoints(const SOLDIERTYPE *const pSold) {
  ...
  if (pSold->ubBodyType == BLOODCAT) {
    ...
    // if frenzied, female monsters get more APs! (for young females,
    // cumulative!)
    if (pSold->bFrenzied) {
...
void DoNinjaAttack(SOLDIERTYPE *pSoldier) {
  ...
  if (pTSoldier != NULL) {
    ...
    if ((pTSoldier->bLife <= 30 || pTSoldier->bBreath <= 30) && ubTargetStance != ANIM_PRONE) {
      ...
      if (ubTargetStance != ANIM_PRONE) {
        const uint16_t state = (Random(2) == 0 ? NINJA_LOWKICK : NINJA_PUNCH);
  ...
  if (pSoldier->ubProfile == DR_Q) {
    ...
    uint32_t volume = CalculateSpeechVolume(HIGHVOLUME);
    ...
    const uint32_t pan = SoundDir(pSoldier->sGridNo);
    ...
    const uint32_t uiSoundID = SoundPlay(filename, volume, pan, 1, NULL, NULL);
...
void CreateSoldierCommon(SOLDIERTYPE &s) try {
  // if we are loading a saved game, we DO NOT want to reset the opplist, look
  // for enemies, or say a dying commnet
  ...
  uint16_t ani_state = s.usAnimState;
  if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
...
void DeleteSoldier(SOLDIERTYPE &s) {
  if (s.sGridNo != NOWHERE) {
    ...
    for (int8_t bDir = 0; bDir < NUM_WORLD_DIRECTIONS; ++bDir) {
      int32_t const iGridNo = s.sGridNo + DirIncrementer[bDir];
  ...
  FOR_EACH(uint16_t *, i, s.pGlowShades) {
    if (*i == NULL) continue;
    MemFree(*i);
    *i = NULL;
  }
...
BOOLEAN ReevaluateEnemyStance(SOLDIERTYPE *pSoldier, uint16_t usAnimState) {
  ...
  if (OK_ENEMY_MERC(pSoldier) && pSoldier != gTacticalStatus.the_chosen_one &&
...
static void CheckForFreeupFromHit(SOLDIERTYPE *pSoldier, uint32_t uiOldAnimFlags,
                                  uint32_t uiNewAnimFlags, uint16_t usOldAniState,
                                  uint16_t usNewState) {
  // THIS COULD POTENTIALLY CALL EVENT_INITNEWAnim() if the GUY was SUPPRESSED
  // CHECK IF THE OLD ANIMATION WAS A HIT START THAT WAS NOT FOLLOWED BY A HIT
  // FINISH IF SO, RELEASE ATTACKER FROM ATTACKING
  ...
  if (usOldAniState != usNewState && (uiOldAnimFlags & ANIM_HITSTART) &&
    ...
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             String("@@@@@@@ Releasesoldierattacker, normal hit animation "
                    "ended NEW: %hs ( %d ) OLD: %hs ( %d )",
  ...
  if ((uiOldAnimFlags & ANIM_HITWHENDOWN) &&
    ...
    // Release attacker
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             "@@@@@@@ Releasesoldierattacker, animation of kill on the ground "
             "ended");
    ReleaseSoldiersAttacker(pSoldier);
...
static bool IsRifle(uint16_t const item_id) {
  return item_id != NOTHING && item_id != ROCKET_LAUNCHER && Item[item_id].usItemClass == IC_GUN &&
         Item[item_id].fFlags & ITEM_TWO_HANDED;
}

static void HandleAnimationProfile(SOLDIERTYPE &, uint16_t usAnimState, BOOLEAN fRemove);
static void SetSoldierLocatorOffsets(SOLDIERTYPE *pSoldier);

void EVENT_InitNewSoldierAnim(SOLDIERTYPE *const pSoldier, uint16_t usNewState,
                              uint16_t const usStartingAniCode, BOOLEAN const fForce) {
  int16_t sAPCost = 0;
  int16_t sBPCost = 0;
  ...
  if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
    // CHECK IF WE ARE TRYING TO INTURRUPT A SCRIPT WHICH WE DO NOT WANT
    // INTERRUPTED!
    ...
    // Check state, if we are not at the same height, set this ani as the
    // pending one and change stance accordingly ATE: ONLY IF WE ARE STARTING AT
    // START OF ANIMATION!
    if (usStartingAniCode == 0) {
      if (gAnimControl[usNewState].ubHeight != gAnimControl[pSoldier->usAnimState].ubEndHeight &&
        ...
        // Check if we are going from crouched height to prone height, and
        // adjust fast turning accordingly Make guy turn while crouched THEN go
        // into prone
        ...
        // Check if we are in realtime and we are going from stand to crouch
          ...
          // Continue with this course of action IE: Do animation and skip from
          // stand to crouch
        }
    ...
    // if ( pSoldier->bLife > INJURED_CHANGE_THREASHOLD )
    {
      ...
      // ATE: LOWER AIMATION IS GOOD, RAISE ONE HOWEVER MAY CAUSE PROBLEMS FOR
      // AI....
    ...
    // Check if we are an enemy, and we are in an animation what should be
    // overriden by if he sees us or not.
    ...
    if ((gAnimControl[usNewState].uiFlags & ANIM_MOVING &&
      ...
      // ATE: Additional check here if we have just been told to update
      // animation ONLY, not goto gridno stuff...
      if (!pSoldier->usDontUpdateNewGridNoOnMoveAnimChange) {
        if (usNewState != SWATTING) {
          DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                   String("Handling New gridNo for %d: Old %hs, New %hs", pSoldier->ubID,
  ...
  // CHECKING IF WE HAVE A HIT FINISH BUT NO DEATH IS DONE WITH A SPECIAL ANI
  // CODE IN THE HIT FINSIH ANI SCRIPTS

  // CHECKING IF WE HAVE FINISHED A DEATH ANIMATION IS DONE WITH A SPECIAL ANI
  // CODE IN THE DEATH SCRIPTS

  // CHECK IF THIS NEW STATE IS NON-INTERRUPTABLE
  // IF SO - SET NON-int32_t FLAG
  ...
  switch (usNewState) {
    ...
    case PRONE_DOWN:
      ...
      // sAPCost = GetAPsToChangeStance( pSoldier, gAnimControl[ usNewState
      // ].ubEndHeight ); DeductPoints( pSoldier, sAPCost, 0 ); break;
    ...
    case BODYEXPLODING:
      ...
      pSoldier->uiPendingActionData1 =
          PlaySoldierJA2Sample(pSoldier, FIRE_ON_MERC, HIGHVOLUME, 5, TRUE);
      break;
  }
  ...
  if (!(pSoldier->uiStatusFlags & SOLDIER_LOCKPENDINGACTIONCOUNTER)) {
    ...
    if (pSoldier->ubPendingActionAnimCount > 0 &&
      ...
      // That's because this represents ANY animation other than the one we
      // began when the pending action was started ATE: Added to ignore this
      // count if we are waiting for someone to move out of our way...
...
static void InternalRemoveSoldierFromGridNo(SOLDIERTYPE &s, BOOLEAN const force) {
  ...
  for (int8_t dir = 0; dir < NUM_WORLD_DIRECTIONS; ++dir) {
    int32_t const grid_no = s.sGridNo + DirIncrementer[dir];
...
void RemoveSoldierFromGridNo(SOLDIERTYPE &s) { InternalRemoveSoldierFromGridNo(s, FALSE); }

static void SetSoldierGridNo(SOLDIERTYPE &, int16_t sNewGridNo, BOOLEAN fForceRemove);

static void EVENT_InternalSetSoldierPosition(SOLDIERTYPE *pSoldier, GridNo pos, float dNewXPos,
                                             float dNewYPos, SetSoldierPosFlags flags) {
  // Not if we're dead!
...
void EVENT_SetSoldierPosition(SOLDIERTYPE *const s, const GridNo pos,
                              const SetSoldierPosFlags flags) {
  int16_t x;
  int16_t y;
...
void EVENT_SetSoldierPositionNoCenter(SOLDIERTYPE *const s, const GridNo pos,
                                      const SetSoldierPosFlags flags) {
  int16_t x;
  int16_t y;
...
void EVENT_SetSoldierPositionXY(SOLDIERTYPE *const s, const float x, const float y,
                                const SetSoldierPosFlags flags) {
...
void SetSoldierHeight(SOLDIERTYPE *const s, float const new_height) {
  ...
  s->sHeightAdjustment = (int16_t)new_height;

  if (s->sHeightAdjustment > 0) {
    ApplyTranslucencyToWalls((int16_t)(s->dXPos / CELL_X_SIZE), (int16_t)(s->dYPos / CELL_Y_SIZE));
...
static void SetSoldierGridNo(SOLDIERTYPE &s, GridNo new_grid_no, BOOLEAN const fForceRemove) {
  ...
  if (gAnimControl[s.usAnimState].uiFlags & (ANIM_MOVING | ANIM_SPECIALMOVE)) {
    if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
      if (new_grid_no != s.sDestination) {  // This must be our new one, make it so
  ...
  if (GetSelectedMan() == &s && guiCurrentEvent == C_WAIT_FOR_CONFIRM) {  // Update path!
  ...
  if (!in_vehicle) {
    ...
    // JA2Gold: If the player wants the merc to cast the fake light AND it is
    // night
  ...
  int8_t const old_over_terrain_type = s.bOverTerrainType;
  ...
  if (!in_vehicle) {
    ...
    if (in_water != s.fPrevInWater) {
      ...
      } else { /* ATE: Check if we are going from water to land - if so, resume
  ...
  // Merc got to a new tile by "sneaking". Did we theoretically sneak past an
  // enemy?
  if (s.bTeam == OUR_TEAM && s.bStealthMode && s.bOppCnt > 0)  // opponents in sight
  {
    ...
    CFOR_EACH_SOLDIER(enemy) {
      ...
      if (PythSpacesAway(new_grid_no, enemy->sGridNo) >=
          DistanceVisible(enemy, DIRECTION_IRRELEVANT, DIRECTION_IRRELEVANT, new_grid_no, s.bLevel))
        continue;
...
static uint16_t SelectFireAnimation(SOLDIERTYPE *pSoldier, uint8_t ubHeight);
...
void EVENT_FireSoldierWeapon(SOLDIERTYPE *pSoldier, int16_t sTargetGridNo) {
  ...
  // IF WE ARE AN NPC, SLIDE VIEW TO SHOW WHO IS SHOOTING
  {
    ...
    // EVENT_InitNewSoldierAnim( pSoldier, SelectFireAnimation( pSoldier,
    // gAnimControl[ pSoldier->usAnimState ].ubEndHeight ), 0, FALSE );
    ...
    // else
    {
      if (pSoldier->uiStatusFlags & SOLDIER_MONSTER) {
        ...
        if (fDoFireRightAway) {
          ...
          // Set flag indicating we are about to shoot once destination
          // direction is hit
...
//					ChangeSoldierState( pSoldier, SHOOT_RIFLE_STAND,
// 0 , FALSE );
  ...
  switch (ubHeight) {
    case ANIM_STAND:
      ...
      if (IsValidSecondHandShot(pSoldier)) {
        // Increment the number of people busy doing stuff because of an attack
        // gTacticalStatus.ubAttackBusyCount++;
        // DebugMsg( TOPIC_JA2, DBG_LEVEL_3, String("!!!!!!! Starting attack with
        // 2 guns, attack count now %d", gTacticalStatus.ubAttackBusyCount) );

        return (SHOOT_DUAL_STAND);
      } else {
        // OK, while standing check distance away from target, and shoot low if we
        // should!
        ...
        // ATE: Made distence away long for psitols such that they never use
        // this....
...
uint16_t GetMoveStateBasedOnStance(const SOLDIERTYPE *const s, const uint8_t ubStanceHeight) {
  switch (ubStanceHeight) {
    case ANIM_STAND:
      return s->fUIMovementFast && !(s->uiStatusFlags & SOLDIER_VEHICLE) ? RUNNING : WALKING;
...
BOOLEAN SoldierReadyWeapon(SOLDIERTYPE *const pSoldier, const GridNo tgt_pos,
                           const BOOLEAN fEndReady) {
  const int16_t sFacingDir = GetDirectionFromGridNo(tgt_pos, pSoldier);
  return InternalSoldierReadyWeapon(pSoldier, sFacingDir, fEndReady);
}

static void EVENT_InternalSetSoldierDesiredDirection(SOLDIERTYPE *pSoldier, uint16_t usNewDirection,
...
static uint8_t CalcScreamVolume(SOLDIERTYPE *pSoldier, uint8_t ubCombinedLoss);
static uint32_t SleepDartSuccumbChance(const SOLDIERTYPE *pSoldier);
static void SoldierGotHitBlade(SOLDIERTYPE *pSoldier);
static void SoldierGotHitExplosion(SOLDIERTYPE *pSoldier, uint16_t usWeaponIndex,
                                   uint16_t bDirection, uint16_t sRange);
static void SoldierGotHitGunFire(SOLDIERTYPE *pSoldier, uint16_t bDirection, SOLDIERTYPE *att,
                                 uint8_t ubSpecial);
static void SoldierGotHitPunch(SOLDIERTYPE *pSoldier);

// ATE: THIS FUNCTION IS USED FOR ALL SOLDIER TAKE DAMAGE FUNCTIONS!
void EVENT_SoldierGotHit(SOLDIERTYPE *pSoldier, const uint16_t usWeaponIndex, int16_t sDamage,
                         int16_t sBreathLoss, const uint16_t bDirection, const uint16_t sRange,
                         SOLDIERTYPE *const att, const uint8_t ubSpecial,
                         const uint8_t ubHitLocation, const int16_t sLocationGrid) {
  uint8_t ubCombinedLoss, ubVolume, ubReason;
  SOLDIERTYPE *pNewSoldier;

  ubReason = 0;

  // ATE: If we have gotten hit, but are still in our attack animation, reduce
  // count!
  ...
  if (sDamage > 25) {
    if (att != NULL) {
      if (att->bTeam == OUR_TEAM) {
        HandleMoraleEvent(att, MORALE_DID_LOTS_OF_DAMAGE, att->sSectorX, att->sSectorY,
                          att->bSectorZ);
      }
    }
    if (pSoldier->bTeam == OUR_TEAM) {
  ...
  if (ubReason != TAKE_DAMAGE_TENTACLES && ubReason != TAKE_DAMAGE_OBJECT) {
    // OK, OK: THis is hairy, however, it's ness. because the normal freeup call
    // uses the attckers intended target, and here we want to use thier actual
    // target....
  ...
  if (Item[usWeaponIndex].usItemClass & IC_BLADE) {
    ...
    PlayLocationJA2Sample(pSoldier->sGridNo, SoundRange<BULLET_IMPACT_1, BULLET_IMPACT_3>(),
                          MIDVOLUME, 1);
  }
  ...
  if (pSoldier->bLife >= CONSCIOUSNESS) {
    if (pSoldier->ubBodyType == CROW) {
      ...
      // ATE: This is to disallow large amounts of smaples being played which is
      // load!
  ...
  if ((gAnimControl[pSoldier->usAnimState].uiFlags & ANIM_HITSTOP)) {
    ...
    switch (pSoldier->usAnimState) {
      case FLYBACKHIT_STOP:
        state = FALLBACK_DEATHTWICH;
        break;
      case STAND_FALLFORWARD_STOP:
        state = GENERIC_HIT_DEATHTWITCHNB;
        break;
      case JFK_HITDEATH_STOP:
        state = JFK_HITDEATH_TWITCHB;
        break;
      case FALLBACKHIT_STOP:
        state = FALLBACK_HIT_DEATHTWITCHNB;
        break;
      case PRONE_LAYFROMHIT_STOP:
        state = PRONE_HIT_DEATHTWITCHNB;
        break;
      case PRONE_HITDEATH_STOP:
        state = PRONE_HIT_DEATHTWITCHB;
        break;
      case FALLFORWARD_HITDEATH_STOP:
        state = GENERIC_HIT_DEATHTWITCHB;
        break;
      case FALLBACK_HITDEATH_STOP:
        state = FALLBACK_HIT_DEATHTWITCHB;
        break;
      case FALLOFF_DEATH_STOP:
        state = FALLOFF_TWITCHB;
        break;
      case FALLOFF_STOP:
        state = FALLOFF_TWITCHNB;
        break;
      case FALLOFF_FORWARD_DEATH_STOP:
        state = FALLOFF_FORWARD_TWITCHB;
        break;
      case FALLOFF_FORWARD_STOP:
        state = FALLOFF_FORWARD_TWITCHNB;
        break;
  ...
  switch (pSoldier->ubBodyType) {
    ...
    {
      // OK, do some code here to allow the fact that poor buddy can be thrown
      // back if it's a big enough hit...
...
static uint8_t CalcScreamVolume(SOLDIERTYPE *pSoldier, uint8_t ubCombinedLoss) {
  ...
  /*
                  // the "Speck factor"...  He's a whiner, and extra-sensitive
     to pain! if (ptr->trait == NERVOUS) ubVolume += 2;
  */
...
static void DoGenericHit(SOLDIERTYPE *pSoldier, uint8_t ubSpecial, int16_t bDirection) {
...
static void ChangeToFlybackAnimation(SOLDIERTYPE *pSoldier, int8_t bDirection);

static void SoldierGotHitGunFire(SOLDIERTYPE *const pSoldier, const uint16_t bDirection,
                                 SOLDIERTYPE *const att, const uint8_t ubSpecial) {
  uint16_t usNewGridNo;
  ...
  // MAYBE CHANGE TO SPECIAL ANIMATION BASED ON VALUE SET BY DAMAGE CALCULATION
  // CODE ALL THESE ONLY WORK ON STANDING PEOPLE
  if (!(pSoldier->uiStatusFlags & SOLDIER_MONSTER) &&
      gAnimControl[pSoldier->usAnimState].ubEndHeight == ANIM_STAND) {
      ...
      if (ubSpecial == FIRE_WEAPON_HEAD_EXPLODE_SPECIAL) {
        ...
        if (gGameSettings.fOptions[TOPTION_BLOOD_N_GORE]) {
          if (SpacesAway(pSoldier->sGridNo, att->sGridNo) <= MAX_DISTANCE_FOR_MESSY_DEATH) {
            ...
            if (pSoldier->bDirection == bDirection) {
              const uint8_t opp_dir = OppositeDirection(bDirection);
              ...
              if (OKFallDirection(pSoldier, usNewGridNo, pSoldier->bLevel, opp_dir, FLYBACK_HIT)) {
                ...
                if (OKFallDirection(pSoldier, usNewGridNo, pSoldier->bLevel, opp_dir,
                                    pSoldier->usAnimState)) {
                  fBlownAway = TRUE;
                }
              }
        ...
        if (IsValidStance(pSoldier, ANIM_PRONE)) {
          ...
          if (gAnimControl[pSoldier->usAnimState].ubEndHeight == ANIM_STAND &&
              !MercInWater(pSoldier)) {
            fFallenOver = TRUE;
            ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, gzLateLocalizedString[STR_LATE_20],
                      pSoldier->name);
          }
        }
      }
  ...
  if (fHeadHit) {
    ...
    if (pSoldier->ubBodyType <= REGFEMALE || pSoldier->ubBodyType == HATKIDCIV ||
...
static void SoldierGotHitExplosion(SOLDIERTYPE *const pSoldier, const uint16_t usWeaponIndex,
                                   const uint16_t bDirection, const uint16_t sRange) {
  int16_t sNewGridNo;
  ...
  switch (gAnimControl[pSoldier->usAnimState].ubEndHeight) {
    ...
    case ANIM_CROUCH:
      ...
      if (OKFallDirection(pSoldier, sNewGridNo, pSoldier->bLevel, OppositeDirection(bDirection),
                          FLYBACK_HIT)) {
...
BOOLEAN EVENT_InternalGetNewSoldierPath(SOLDIERTYPE *pSoldier, uint16_t sDestGridNo,
  ...
  if ((!(gTacticalStatus.uiFlags & INCOMBAT) &&
    ...
    if (uiDist > 0) {
      ...
      if (fAdvancePath) {
        ...
        ubPathingMaxDirection = (uint8_t)usPathingData[MAX_PATH_LIST_SIZE - 1];
        ...
        if (pSoldier->usPathDataSize == MAX_PATH_LIST_SIZE) {
          ...
          // NewGridNo(pSoldier->sFinalDestination,
          // DirectionInc(OppositeDirection(ubPathingMaxDirection)));
        } else {
          pSoldier->usPathDataSize++;
        }
      }
      ...
      // Change animation only.... set value to NOT call any goto new gridno
      // stuff.....
  ...
  // we can use the soldier's level here because we don't have pathing across
  // levels right now...
  ...
  if (fContinue) {
    ...
    // at leasty change new dest. This will be redundent if the ANI is a totaly
    // new one
...
void SoldierGotoStationaryStance(SOLDIERTYPE *pSoldier) {
  // ATE: This is to turn off fast movement, that us used to change movement
  // mode for ui display on stance changes....
  if (pSoldier->bTeam == OUR_TEAM) {
  ...
  } else if (pSoldier->service_partner != NULL && pSoldier->bLife >= OKLIFE &&
             pSoldier->bBreath > 0) {
...
static uint16_t GetNewSoldierStateFromNewStance(SOLDIERTYPE *pSoldier, uint8_t ubDesiredStance);
...
static int8_t MultiTiledTurnDirection(SOLDIERTYPE *pSoldier, int8_t bStartDirection,
  ...
  const STRUCTURE_FILE_REF *const pStructureFileRef =
      GetAnimationStructureRef(pSoldier, usAnimSurface, pSoldier->usUIMovementMode);
  ...
  while (bLoop < 2) {
    while (bCurrentDirection != bDesiredDirection) {
      ...
      fOk = OkayToAddStructureToWorld(
          ...
          &pStructureFileRef->pDBStructureRef[OneCDirection(bCurrentDirection)], usStructureID);
      if (!fOk) {
        break;
      }
    }
...
static void EVENT_InternalSetSoldierDesiredDirection(SOLDIERTYPE *const pSoldier,
                                                     uint16_t usNewDirection,
                                                     const BOOLEAN fInitalMove,
  ...
  if (pSoldier->fNoAPToFinishMove && (gAnimControl[usAnimState].uiFlags & ANIM_MOVING)) {
    // ATE; Commented this out: NEVER, EVER, start a new anim from this
    // function, as an eternal loop will result....
    // SoldierGotoStationaryStance( pSoldier );
    // Reset flag!
    AdjustNoAPToFinishMove(pSoldier, FALSE);
  }
  ...
  if (pSoldier->bDesiredDirection != pSoldier->bDirection) {
    ...
    if (gAnimControl[usAnimState].uiFlags & ANIM_STATIONARY || pSoldier->fNoAPToFinishMove ||
        fInitalMove) {
      if (gAnimControl[usAnimState].ubHeight == ANIM_PRONE) {
        // Set this beasty of a flag to allow us to go back down to prone if we
        // choose! ATE: Alrighty, set flag to go back down only if we are not
        // moving anywhere
  ...
  if (pSoldier->uiStatusFlags & SOLDIER_VEHICLE) {
    const uint8_t hires_desired_dir = Dir2ExtDir(pSoldier->bDesiredDirection);
    pSoldier->bTurningIncrement =
        ExtQuickestDirection(pSoldier->ubHiResDirection, hires_desired_dir);
  } else {
...
void EVENT_SetSoldierDesiredDirectionForward(SOLDIERTYPE *const s, const uint16_t new_direction) {
...
static int32_t CheckBleeding(SOLDIERTYPE *pSoldier);
...
void EVENT_BeginMercTurn(SOLDIERTYPE &s) {
  ...
  if (!gTacticalStatus.fAutoBandageMode) {
    ...
    if (!(s.uiStatusFlags & (SOLDIER_VEHICLE | SOLDIER_ROBOT))) {
      ...
      {
        int32_t const blood = CheckBleeding(&s);  // check if he might lose another life point
  ...
  if (s.sNoiseGridno != NOWHERE &&
      s.ubNoiseVolume != 0) {      // The volume of the noise "decays" by 1 point
    if (--s.ubNoiseVolume == 0) {  // The volume has reached zero, forget about the noise
      s.sNoiseGridno = NOWHERE;
    }
  }

  if (s.uiStatusFlags & SOLDIER_GASSED) {  // Must get a gas mask or leave the
                                           // gassed area to get over it
    if (IsWearingHeadGear(s, GASMASK) ||
        GetSmokeEffectOnTile(s.sGridNo, s.bLevel) == NO_SMOKE_EFFECT) {  // Turn off gassed flag
      s.uiStatusFlags &= ~SOLDIER_GASSED;
    }
  }
  ...
  if (s.bInSector) {
    ...
    // CJC Nov 30: handle RT opplist decaying in another function which operates
    // less often
    if (gTacticalStatus.uiFlags & INCOMBAT) {
      ...
      switch (GetProfile(s.ubProfile).bPersonalityTrait) {
        case FEAR_OF_INSECTS:
          if (MercSeesCreature(s)) {
            HandleMoraleEvent(&s, MORALE_INSECT_PHOBIC_SEES_CREATURE, s.sSectorX, s.sSectorY,
                              s.bSectorZ);
        ...
        case NERVOUS:
          if (DistanceToClosestFriend(&s) > NERVOUS_RADIUS) {
            ...
            if (s.bMorale > 45) {  // Turn flag off, so that we say it every two turns
...
BOOLEAN ConvertAniCodeToAniFrame(SOLDIERTYPE *const s, uint16_t ani_frame) {
  static uint8_t const gDirectionFrom8to2[] = {0, 0, 1, 1, 0, 1, 1, 0};
  ...
  uint8_t temp_dir = OneCDirection(s->bDirection);
  ...
  switch (as.uiNumDirections) {
    ...
    case 4:
      ...
      break;
    case 32:
      temp_dir = ExtOneCDirection(s->ubHiResDirection);
      break;
  ...
  if (!as.hVideoObject) {
    ...
    if (ani_frame >= as.hVideoObject->SubregionCount()) {
      // Debug msg here....
      // ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION, L"Soldier Animation:
...
void TurnSoldier(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->fTurningToShoot) {
    if (pSoldier->bDirection == pSoldier->bDesiredDirection) {
      ...
      // Else check if we are trying to shoot and once was prone, but am now
      // crouched because we needed to turn...
  ...
  if (pSoldier->bDirection == pSoldier->bDesiredDirection) {
    ...
    if (pSoldier->fTurningFromPronePosition == TURNING_FROM_PRONE_ON) {
      // ATE: Don't do this if we have something in our hands we are going to
      // throw!
    ...
    if (pSoldier->uiStatusFlags & SOLDIER_TURNINGFROMHIT) {
      if (pSoldier->fGettingHit == 1) {
        ...
        // Release attacker
        DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                 "@@@@@@@ Releasesoldierattacker, turning from hit animation ended");
        ReleaseSoldiersAttacker(pSoldier);
  ...
  if (pSoldier->uiStatusFlags & SOLDIER_VEHICLE) {
    ...
    if (pSoldier->ubBodyType == TANK_NW || pSoldier->ubBodyType == TANK_NE) {
      if (pSoldier->iTuringSoundID == NO_SAMPLE) {
        pSoldier->iTuringSoundID =
            PlaySoldierJA2Sample(pSoldier, TURRET_MOVE, HIGHVOLUME, 100, TRUE);
      }
    }
    ...
    // Get new direction
    // sDirection = pSoldier->bDirection + QuickestDirection(
...
static const uint8_t gRedGlowR[] = {
    0,  // Normal shades
...
#if 0
static const uint8_t gOrangeGlowR[]=
{
	0,			// Normal shades
	20,
	40,
...
#if 0
static const uint8_t gOrangeGlowG[]=
{
	0,			// Normal shades
	5,
	10,
...
void CreateSoldierPalettes(SOLDIERTYPE &s) {
  ...
  if (!substitution) {
    ...
    uint16_t const palette_anim_surface = LoadSoldierAnimationSurface(s, STANDING);
    if (palette_anim_surface != INVALID_ANIMATION_SURFACE) {
      // Use palette from HVOBJECT, then use substitution for pants, etc
      memcpy(tmp_pal, gAnimSurfaceDatabase[palette_anim_surface].hVideoObject->Palette(),
             sizeof(*tmp_pal) * 256);
  ...
  for (int32_t i = 0; i < NUM_SOLDIER_SHADES; ++i) {
    if (s.pShades[i]) {
      MemFree(s.pShades[i]);
      s.pShades[i] = 0;
    }
  ...
  // ATE: OK, piggyback on the shades we are not using for 2 colored
  // lighting.... ORANGE, VISIBLE GUY
  s.pShades[20] = Create16BPPPaletteShaded(pal, 255, 255, 255, FALSE);
  for (int32_t i = 21; i < 30; ++i) {
...
static void CalculateSoldierAniSpeed(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pStatsSoldier) {
  ...
  if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
    ...
    // pSoldier->sAniDelay = pSoldier->sAniDelay * ( 1 *
    // gTacticalStatus.bRealtimeSpeed / 2 );
  }
}
...
void LoadPaletteData() {
  ...
  gubpNumReplacementsPerRange = MALLOCN(uint8_t, guiNumPaletteSubRanges);

  // Read # of types for each!
  for (cnt = 0; cnt < guiNumPaletteSubRanges; cnt++) {
    FileRead(hFile, &gubpNumReplacementsPerRange[cnt], sizeof(uint8_t));
  }

  // Loop for each one, read in data
  for (cnt = 0; cnt < guiNumPaletteSubRanges; cnt++) {
    FileRead(hFile, &gpPaletteSubRanges[cnt].ubStart, sizeof(uint8_t));
    FileRead(hFile, &gpPaletteSubRanges[cnt].ubEnd, sizeof(uint8_t));
  }
...
void SetPaletteReplacement(SGPPaletteEntry *const p8BPPPalette, PaletteRepID aPalRep) {
  ...
  const uint8_t ubPalIndex = GetPaletteRepIndexFromID(aPalRep);
...
uint8_t GetPaletteRepIndexFromID(const PaletteRepID pal_rep) {
  ...
  for (uint32_t i = 0; i < guiNumReplacements; ++i) {
...
static uint32_t SleepDartSuccumbChance(const SOLDIERTYPE *pSoldier) {
  ...
  // add in a bonus based on how long it's been since shot... highest chance at
  // the beginning
...
static void HandleTakeDamageDeath(SOLDIERTYPE *pSoldier, uint8_t bOldLife, uint8_t ubReason) {
  switch (ubReason) {
...
uint8_t SoldierTakeDamage(SOLDIERTYPE *const pSoldier, int16_t sLifeDeduct, int16_t sBreathLoss,
                          const uint8_t ubReason, SOLDIERTYPE *const attacker) {
  int8_t bOldLife;
  ...
  // CJC Jan 21 99: add check to see if we are hurting an enemy in an
  // enemy-controlled sector; if so, this is a sign of player activity
    ...
    if (Random(100) < (uint16_t)sChanceToDrop) {
      ...
      if (pSoldier->inv[HANDPOS].usItem != NOTHING) {
        if (!(pSoldier->inv[HANDPOS].fFlags & OBJECT_UNDROPPABLE)) {
          ...
          Visibility const bVisible = pSoldier->bTeam == OUR_TEAM ? VISIBLE : INVISIBLE;
...
BOOLEAN InternalDoMercBattleSound(SOLDIERTYPE *s, BattleSound battle_snd_id,
                                  int8_t const bSpecialCode) {
  ...
  switch (battle_snd_id) {
    ...
    case BATTLE_SOUND_HIT2:
      switch (s->ubBodyType) {
        ...
        case ADULTFEMALEMONSTER:
        case AM_MONSTER:
          sub_snd = Random(2) == 0 ? ACR_DIE_PART1 : ACR_LUNGE;
          break;
        case INFANT_MONSTER:
          sub_snd = BCR_SHRIEK;
          break;
        ...
        case LARVAE_MONSTER:
          ...
          break;
        case BLOODCAT:
          sub_snd = BLOODCAT_HIT_1;
          break;
        case ROBOTNOWEAPON:
          sub_snd = SoundRange<S_METAL_IMPACT1, S_METAL_IMPACT2>();
          break;
    ...
    default:
      ...
      switch (s->ubBodyType) {
        case ROBOTNOWEAPON:
          sub_snd = battle_snd_id == BATTLE_SOUND_ATTN1 ? ROBOT_GREETING : ROBOT_BEEP;
          break;
...
no_sub:
  ...
  if (s->bOldBattleSnd == battle_snd_id && battle_snd->fDontAllowTwoInRow &&
  ...
  if (s->bMorale < LOW_MORALE_BATTLE_SND_THREASHOLD) {
    switch (battle_snd_id) {
      case BATTLE_SOUND_OK1:
        battle_snd_id = BATTLE_SOUND_LOWMARALE_OK1;
        break;
      case BATTLE_SOUND_ATTN1:
        battle_snd_id = BATTLE_SOUND_LOWMARALE_ATTN1;
        break;
      default:
        break;
    }
  ...
  // if the sound to be played is a confirmation, check to see if we are to play
  // it
  if (battle_snd_id == BATTLE_SOUND_OK1 && gGameSettings.fOptions[TOPTION_MUTE_CONFIRMATIONS]) {
    return TRUE;
  }
...
file_exists:;
  ...
  uint32_t const base_volume = bSpecialCode == BATTLE_SND_LOWER_VOLUME ? MIDVOLUME : HIGHVOLUME;
  uint32_t volume = CalculateSpeechVolume(base_volume);
  ...
  uint32_t const uiSoundID = SoundPlay(filename, volume, pan, 1, NULL, NULL);
...
void MakeCharacterDialogueEventDoBattleSound(SOLDIERTYPE &s, BattleSound const sound,
                                             uint32_t const delay) {
  class CharacterDialogueEventDoBattleSound : public CharacterDialogueEvent {
   public:
    CharacterDialogueEventDoBattleSound(SOLDIERTYPE &s, BattleSound const sound,
                                        uint32_t const delay)
        : CharacterDialogueEvent(s), sound_(sound), time_stamp_(GetJA2Clock()), delay_(delay) {}
   ...
   private:
    ...
    uint32_t const time_stamp_;
    uint32_t const delay_;
  };
...
BOOLEAN DoMercBattleSound(SOLDIERTYPE *const s, BattleSound const battle_snd_id) {
  ...
  if (gBattleSndsData[battle_snd_id].fStopDialogue == 1 || s->ubProfile == NO_PROFILE ||
      InOverheadMap() || !IsMercSayingDialogue(s->ubProfile)) {
...
BOOLEAN CheckSoldierHitRoof(SOLDIERTYPE *pSoldier) {
  ...
  if (FindLowerLevel(pSoldier, &bNewDirection)) {
    ...
    if (gAnimControl[pSoldier->usAnimState].ubHeight == ANIM_STAND) {
      ...
      // Alrighty, let's not blindly change here, look at whether the dest
      // gridno is good!
...
int16_t GetDirectionFromGridNo(const int16_t sGridNo, const SOLDIERTYPE *const s) {
...
static void AdjustForFastTurnAnimation(SOLDIERTYPE *pSoldier) {
  ...
  if (gAnimControl[pSoldier->usAnimState].uiFlags & ANIM_FASTTURN && pSoldier->bTeam == OUR_TEAM &&
      !(pSoldier->uiStatusFlags & SOLDIER_TURNINGFROMHIT)) {
    if (pSoldier->bDirection != pSoldier->bDesiredDirection) {
...
void SendGetNewSoldierPathEvent(SOLDIERTYPE *const pSoldier, uint16_t const sDestGridNo) {
...
// This function just encapolates the check for turnbased and having an attacker
// in the first place
...
static void HandleAnimationProfile(SOLDIERTYPE &s, uint16_t const usAnimState,
                                   BOOLEAN const fRemove) {
  uint16_t const anim_surface = DetermineSoldierAnimationSurface(&s, usAnimState);
  CHECKV(anim_surface != INVALID_ANIMATION_SURFACE);

  int8_t const profile_id = gAnimSurfaceDatabase[anim_surface].bProfile;
  ...
  for (uint32_t tile_count = 0; tile_count != profile_dir.ubNumTiles; ++tile_count) {
    ...
    if (fRemove) {  // Remove from world
      RemoveMerc(grid_no, s, true);
    } else {  // Place into world
...
void EVENT_SoldierBeginBladeAttack(SOLDIERTYPE *pSoldier, int16_t sGridNo, uint8_t ubDirection) {
  ...
  if (pSoldier->uiStatusFlags & SOLDIER_MONSTER) {
    ...
    if (tgt != NULL &&
        ((tgt->bLife < OKLIFE && tgt->bLife > 0) || (tgt->bBreath < OKBREATH && tgt->bCollapsed))) {
    ...
    if (pTSoldier != NULL) {
      ...
      if (!NewOKDestination(pSoldier, sGridNo, FALSE, pSoldier->bLevel)) {
        ...
        const uint16_t state =
            (c != NULL && IsValidDecapitationCorpse(c) ? DECAPITATE : CROUCH_STAB);
      ...
      if (!fRefused) {
        if (CREATURE_OR_BLOODCAT(pTSoldier)) {
          ...
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK,
                    g_langRes->Message[STR_REFUSE_FIRSTAID_FOR_CREATURE]);
...
uint32_t SoldierDressWound(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pVictim, int16_t sKitPts,
  ...
  // in case he has multiple kits in hand, limit influence of kit status to
  // 100%!
  if (sStatus >= 100) {
    sStatus = 100;
  }
...
void HaultSoldierFromSighting(SOLDIERTYPE *pSoldier, BOOLEAN fFromSightingEnemy) {
  ...
  if (pSoldier->pTempObject != NULL && fFromSightingEnemy) {
    ...
    DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
             "@@@@@@@ Reducing attacker busy count..., ending throw because "
             "saw something");
    ReduceAttackBusyCount(pSoldier, FALSE);
  ...
  if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
    ...
    if (pSoldier->fTurningToShoot && fFromSightingEnemy) {
      ...
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               "@@@@@@@ Reducing attacker busy count..., ending fire because "
               "saw something");
      ReduceAttackBusyCount(pSoldier, FALSE);
    }
...
void EVENT_StopMerc(SOLDIERTYPE *const s) { EVENT_StopMerc(s, s->sGridNo, s->bDirection); }

// Halt event is used to stop a merc - networking should check / adjust to
// gridno?
void EVENT_StopMerc(SOLDIERTYPE *const s, GridNo const grid_no, int8_t const direction) {
  if (!s->fDelayedMovement) {  // Cancel pending events
  ...
  s->sDestXPos = (int16_t)s->dXPos;
  s->sDestYPos = (int16_t)s->dYPos;
...
void ReLoadSoldierAnimationDueToHandItemChange(SOLDIERTYPE *const s, uint16_t const usOldItem,
                                               uint16_t const usNewItem) {
...
static uint16_t *CreateEnemyGreyGlow16BPPPalette(const SGPPaletteEntry *pPalette, uint32_t rscale,
  ...
  for (uint32_t cnt = 0; cnt < 256; cnt++) {
    uint32_t lumin = (pPalette[cnt].r * 299 + pPalette[cnt].g * 587 + pPalette[cnt].b * 114) / 1000;
    uint32_t rmod = 100 * lumin / 256;
    uint32_t gmod = 100 * lumin / 256;
    uint32_t bmod = 100 * lumin / 256;
...
bool CheckForBreathCollapse(SOLDIERTYPE &s) {
  ...
  if (s.bBreathMax > 70 && s.bBreath < 20 &&
      !(s.usQuoteSaidFlags & SOLDIER_QUOTE_SAID_LOW_BREATH) &&
      gAnimControl[s.usAnimState].ubEndHeight == ANIM_STAND) {  // Warn
...
BOOLEAN InternalIsValidStance(const SOLDIERTYPE *pSoldier, int8_t bDirection, int8_t bNewStance) {
  ...
  if (pStructureFileRef != NULL) {
    ...
    if (!OkayToAddStructureToWorld(pSoldier->sGridNo, pSoldier->bLevel,
                                   &pStructureFileRef->pDBStructureRef[OneCDirection(bDirection)],
                                   usOKToAddStructID)) {
      return (FALSE);
    }
  }
          ...
          } else  // soldier is either not bandaged at all or is dying
          {
            if (pSoldier->bLife < OKLIFE)  // if he's dying
            {
              ...
              if (pSoldier->bLifeMax >= OKLIFE) {
                // bleeding while "dying" costs a PERMANENT point of life each
                // time!
                pSoldier->bLifeMax--;
                pSoldier->bBleeding--;
              }
...
static float CalcSoldierNextBleed(SOLDIERTYPE *pSoldier) {
  ...
  // pSoldier->nextbleed = 2 + (pSoldier->life / (10 + pSoldier->tilesMoved));
  // // min = 2
...
void HandlePlacingRoofMarker(SOLDIERTYPE &s, bool const set, bool const force) {
  ...
  } else if (!IndexExistsInRoofLayer(gridno, FIRSTPOINTERS11)) {  // It does not exist already
...
void EVENT_SoldierBeginRepair(SOLDIERTYPE &s, GridNo const gridno, uint8_t const direction) {
  SOLDIERTYPE *tgt;
  int8_t const repair_item = IsRepairableStructAtGridNo(gridno, &tgt);
  ...
  {
    case 2: /* Vehicle */
      SetSoldierAssignmentRepair(s, FALSE, FALSE, tgt->ubID);
      break;
    case 3: /* SAM     */
      SetSoldierAssignmentRepair(s, TRUE, FALSE, -1);
      break;
  }
}
...
void ChangeToFallbackAnimation(SOLDIERTYPE *pSoldier, int8_t bDirection) {
  ...
  // Get dest gridno, convert to center coords
  usNewGridNo = NewGridNo(pSoldier->sGridNo, DirectionInc(OppositeDirection(bDirection)));
  // usNewGridNo = NewGridNo( (uint16_t)usNewGridNo, (uint16_t)(-1 * DirectionInc(
  // bDirection ) ) );
...
BOOLEAN PlayerSoldierStartTalking(SOLDIERTYPE *pSoldier, uint8_t ubTargetID, BOOLEAN fValidate) {
  ...
  // Get APs...
  const int16_t sAPCost = AP_TALK;

  // Deduct points from our guy....
  DeductPoints(pSoldier, sAPCost, 0);

  int16_t const sFacingDir = GetDirectionFromGridNo(tgt.sGridNo, pSoldier);
...
bool IsValidSecondHandShot(SOLDIERTYPE const *const s) {
  ...
  return Item[o.usItem].usItemClass == IC_GUN && !(Item[o.usItem].fFlags & ITEM_TWO_HANDED) &&
         !s->bDoBurst && Item[s->inv[HANDPOS].usItem].usItemClass == IC_GUN &&
         o.bGunStatus >= USABLE && o.ubGunShotsLeft > 0;
}
...
bool IsValidSecondHandShotForReloadingPurposes(SOLDIERTYPE const *const s) {
   ...
   * out! */
  OBJECTTYPE const &o = s->inv[SECONDHANDPOS];
  return Item[o.usItem].usItemClass == IC_GUN && !s->bDoBurst &&
         Item[s->inv[HANDPOS].usItem].usItemClass == IC_GUN && o.bGunStatus >= USABLE;
}
...
BOOLEAN ControllingRobot(const SOLDIERTYPE *s) {
  ...
  // allow control from within vehicles - allows strategic travel in a vehicle
  // with robot!
  ...
  if (pRobot->sSectorX == s->sSectorX && pRobot->sSectorY == s->sSectorY &&
...
SOLDIERTYPE *GetRobotController(SOLDIERTYPE *pSoldier) { return pSoldier->robot_remote_holder; }
    ...
    if (pSoldier->bNewSituation == IS_NEW_SITUATION) {
      ...
      if (pSoldier->uiStatusFlags & SOLDIER_UNDERAICONTROL) {
        if (pSoldier->fTurningToShoot) {
          ...
          // Release attacker
          // OK - this is hightly annoying , but due to the huge combinations of
          // things that can happen - 1 of them is that sLastTarget will get
          // unset after turn is done - so set flag here to tell it not to...
          pSoldier->fDontUnsetLastTargetFromTurn = TRUE;
          DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                   "@@@@@@@ Reducing attacker busy count..., ending fire "
                   "because saw something: DONE IN SYSTEM NEW SITUATION");
          ReduceAttackBusyCount(pSoldier, FALSE);
        }
        ...
        if (pSoldier->pTempObject != NULL) {
          ...
          // Decrement attack counter...
          DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                   "@@@@@@@ Reducing attacker busy count..., ending throw "
                   "because saw something: DONE IN SYSTEM NEW SITUATION");
          ReduceAttackBusyCount(pSoldier, FALSE);
        }
      }
...
static void InternalPlaySoldierFootstepSound(SOLDIERTYPE *const s) {
  ...
  uint8_t random_max = 4;
  if (s->usAnimState == CRAWLING) {
    sound_base = CRAWL_1;
  } else
    switch (s->bOverTerrainType)  // Pick base based on terrain over
    {
      case FLAT_FLOOR:
        sound_base = WALK_LEFT_IN;
        break;
      case DIRT_ROAD:
      case PAVED_ROAD:
        sound_base = WALK_LEFT_ROAD;
        break;
      case LOW_WATER:
      case MED_WATER:
        sound_base = WATER_WALK1_IN;
        random_max = 2;
        break;
      case DEEP_WATER:
        sound_base = SWIM_1;
...
void CrowsFlyAway(const uint8_t ubTeam) {
  FOR_EACH_IN_TEAM(s, ubTeam) {
    if (s->bInSector && s->ubBodyType == CROW && s->usAnimState != CROW_FLY) {
...
void SetSoldierAsUnderAiControl(SOLDIERTYPE *pSoldierToSet) {
  ...
  FOR_EACH_SOLDIER(s) { s->uiStatusFlags &= ~SOLDIER_UNDERAICONTROL; }
...
static void EnableDisableSoldierLightEffects(BOOLEAN const enable_lights) {
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    ...
    if (enable_lights) {  // Add the light around the merc
      PositionSoldierLight(s);
    } else {  // Delete the fake light the merc casts
      DeleteSoldierLight(s);
