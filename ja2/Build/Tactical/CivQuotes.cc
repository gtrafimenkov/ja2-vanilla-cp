...
static BOOLEAN GetCivQuoteText(UINT8 ubCivQuoteID, UINT8 ubEntryID, wchar_t *zQuote) try {
  ...
  LoadEncryptedDataFromFile(zFileName, zQuote, CIV_QUOTE_TEXT_SIZE * ubEntryID,
                            CIV_QUOTE_TEXT_SIZE);
  return zQuote[0] != L'\0';
} catch (...) {
  return FALSE;
...
static void ShutDownQuoteBox(BOOLEAN fForce) {
  ...
  if ((GetJA2Clock() - gCivQuoteData.uiTimeOfCreation) > 300 || fForce) {
    ...
    if (gCivQuoteData.pCiv && gCivQuoteData.pCiv->bAction == AI_ACTION_OFFER_SURRENDER) {
      DoMessageBox(MSG_BOX_BASIC_STYLE, g_langRes->Message[STR_SURRENDER], GAME_SCREEN,
                   MSG_BOX_FLAG_YESNO, SurrenderMessageBoxCallBack, NULL);
    }
  }
}
...
void BeginCivQuote(SOLDIERTYPE *pCiv, UINT8 ubCivQuoteID, UINT8 ubEntryID, INT16 sX, INT16 sY) {
  ...
  // Prepare text box
  gCivQuoteData.dialogue_box = PrepareMercPopupBox(
      0, BASIC_MERC_POPUP_BACKGROUND, BASIC_MERC_POPUP_BORDER, gzCivQuote, DIALOGUE_DEFAULT_WIDTH,
      0, 0, 0, &gusCivQuoteBoxWidth, &gusCivQuoteBoxHeight);
  ...
  // Define main region
  MSYS_DefineRegion(&gCivQuoteData.MouseRegion, sX, sY, sX + w, sY + h, MSYS_PRIORITY_HIGHEST,
...
static UINT8 DetermineCivQuoteEntry(SOLDIERTYPE *pCiv, UINT8 *pubCivHintToUse,
  ...
  if (pCiv->ubCivilianGroup == HICKS_CIV_GROUP) {
    // Are they friendly?
    // if ( gTacticalStatus.fCivGroupHostile[ HICKS_CIV_GROUP ] <
  ...
  if (pCiv->ubCivilianGroup == KINGPIN_CIV_GROUP) {
    // Are they friendly?
    // if ( gTacticalStatus.fCivGroupHostile[ KINGPIN_CIV_GROUP ] <
    // CIV_GROUP_WILL_BECOME_HOSTILE )
    if (pCiv->bNeutral) {
