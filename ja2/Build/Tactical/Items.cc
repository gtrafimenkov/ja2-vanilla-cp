...
const INVTYPE Item[] = {
    ...
    {IC_GUN, 2, TARGETCURS, 0, 2, 6, 1, 480, 2, /* Glock 18        */ +1, +1,
     IF_STANDARD_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 3, TARGETCURS, 0, 3, 11, 1, 450, 2, /* Beretta 92F     */ -1, -1,
     IF_STANDARD_GUN | ITEM_BIGGUNLIST},
    ...
    {IC_GUN, 6, TARGETCURS, 0, 6, 10, 1, 300, 1, /* .357 Barracuda  */ +3, +3,
     IF_STANDARD_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 7, TARGETCURS, 0, 7, 17, 1, 300, 1, /* .357 DesertEagle*/ -1, -1, IF_STANDARD_GUN},
    {IC_GUN, 8, TARGETCURS, 0, 8, 11, 1, 400, 2, /* .45 M1911 */ 0, 0,
     IF_STANDARD_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 9, TARGETCURS, 0, 9, 21, 0, 980, 3, /* H&K MP5K      	 */ -1, 0, IF_STANDARD_GUN},
    {IC_GUN, 10, TARGETCURS, 0, 10, 28, 0, 1170, 4,
     /* .45 MAC-10	     */ -2, -1, IF_STANDARD_GUN},

    {IC_GUN, 11, TARGETCURS, 0, 11, 48, 0, 700, 3, /* Thompson M1A1   */ +3, -3,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 12, TARGETCURS, 0, 12, 26, 0, 1330, 5, /* Colt Commando   */ 0, -1, IF_TWOHANDED_GUN},
    {IC_GUN, 13, TARGETCURS, 0, 13, 31, 0, 770, 3,
     /* H&K MP53		 		 */ -1, -1, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 14, TARGETCURS, 0, 14, 39, 0, 1180, 4, /* AKSU-74         */ -2, -1,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 15, TARGETCURS, 0, 15, 28, 0, 2750, 9, /* 5.7mm FN P90    */ -2, -4,
     IF_STANDARD_GUN | ITEM_NOT_BUYABLE},
    {IC_GUN, 16, TARGETCURS, 0, 16, 19, 0, 620, 3, /* Type-85         */ -4, +2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 17, TARGETCURS, 0, 17, 39, 0, 1350, 5, /* SKS             */ -4, -2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 18, TARGETCURS, 0, 18, 43, 0, 1930, 6, /* Dragunov        */ +2, +2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 19, TARGETCURS, 0, 19, 66, 0, 1950, 6, /* M24             */ +4, +4, IF_TWOHANDED_GUN},
    {IC_GUN, 20, TARGETCURS, 0, 20, 36, 0, 2380, 8, /* Steyr AUG       */ +1, -2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    ...
    {IC_GUN, 24, TARGETCURS, 0, 24, 36, 0, 1970, 7, /* FA-MAS          */ -2, -2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 25, TARGETCURS, 0, 25, 36, 0, 1830, 6, /* AK-74           */ -1, -2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 26, TARGETCURS, 0, 26, 43, 0, 1450, 5, /* AKM             */ +2, +2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    ...
    {IC_GUN, 29, TARGETCURS, 0, 29, 44, 0, 1570, 5, /* H&K G3A3        */ +1, -1,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 30, TARGETCURS, 0, 30, 38, 0, 2530, 8, /* H&K G11         */ +3, -4,
     IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE},

    {IC_GUN, 31, TARGETCURS, 0, 31, 36, 0, 670, 3, /* Remington M870  */ +3, +3, IF_TWOHANDED_GUN},
    {IC_GUN, 32, TARGETCURS, 0, 32, 38, 0, 980, 4, /* SPAS-15         */ -2, -2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 33, TARGETCURS, 0, 33, 41, 0, 2900, 9, /* CAWS            */ -3, -3,
     IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE},
    {IC_GUN, 34, TARGETCURS, 0, 34, 68, 0, 3100, 10, /* FN Minimi       */ -1, -2,
     IF_TWOHANDED_GUN},
    {IC_GUN, 35, TARGETCURS, 0, 35, 48, 0, 3180, 10, /* RPK-74          */ -1, -2,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_GUN, 36, TARGETCURS, 0, 36, 93, 0, 3420, 10, /* H&K 21E         */ +2, +1,
     IF_TWOHANDED_GUN | ITEM_BIGGUNLIST},
    {IC_BLADE, 37, KNIFECURS, 1, 79, 9, 2, 100, 2, /* combat knife    */ +2, +2, IF_STANDARD_BLADE},
    {IC_THROWING_KNIFE, 38, TARGETCURS, 1, 53, 1, 4, 50, 3,
     /* throwing knife  */ -1, -1, IF_STANDARD_BLADE},
    {IC_THROWN, 39, TOSSCURS, 1, 57, 5, 2, 0, 0, /* rock            */ 0, 0, ITEM_NOT_BUYABLE},
    {IC_LAUNCHER, 40, TRAJECTORYCURS, 0, 37, 26, 0, 900, 7,
     /* grenade launcher*/ 0, -1, IF_TWOHANDED_GUN},
    ...
    {IC_BLADE, 43, KNIFECURS, 0, 0, 0, 1, 0, 0, /* yng male claws */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_BLADE, 44, KNIFECURS, 0, 0, 0, 1, 0, 0, /* yng fem claws  */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_BLADE, 45, KNIFECURS, 0, 0, 0, 1, 0, 0, /* old male claws */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_BLADE, 46, KNIFECURS, 0, 0, 0, 1, 0, 0, /* old fem claws  */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_TENTACLES, 47, KNIFECURS, 0, 0, 0, 1, 0, 0, /* queen tentacles*/ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_GUN, 48, TARGETCURS, 0, 0, 0, 1, 0, 0, /* queen spit */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_PUNCH, 49, PUNCHCURS, 1, 102, 1, 4, 20, 2, /* brass knuckles */ 0, 0, IF_STANDARD_BLADE},
    {IC_LAUNCHER, 50, INVALIDCURS, 0, 39, 13, 0, 500, 8, /* underslung g.l.*/ 0, 0,
     IF_STANDARD_GUN},

    {IC_GUN, 51, TARGETCURS, 0, 38, 21, 0, 500, 9, /* rocket Launcher*/ 0, -3,
     IF_TWOHANDED_GUN},  // now repairable
    {IC_BLADE, 52, KNIFECURS, 0, 0, 0, 1, 0, 0, /* bloodcat claws*/ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_BLADE, 53, KNIFECURS, 0, 0, 0, 1, 0, 0, /* bloodcat bite */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_BLADE, 54, KNIFECURS, 0, 41, 13, 0, 200, 3, /* machete */ 0, +3, IF_STANDARD_BLADE},
    {IC_GUN, 55, TARGETCURS, 0, 45, 40, 0, 5000, 0, /* rocket rifle */ 0, -5,
     IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE | ITEM_ELECTRONIC},
    {IC_GUN, 56, TARGETCURS, 0, 40, 12, 0, 1000, 0, /* Automag III */ 0, -2,
     IF_STANDARD_GUN | ITEM_NOT_BUYABLE},
    {IC_GUN, 57, TARGETCURS, 0, 0, 0, 0, 0, 0, /* infant spit */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_GUN, 58, TARGETCURS, 0, 0, 0, 0, 0, 0, /* yng male spit */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_GUN, 59, TARGETCURS, 0, 0, 0, 0, 0, 0, /* old male spit */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    ...
    {IC_THROWING_KNIFE, 62, TARGETCURS, 1, 95, 1, 4, 50, 0,
     /*bloody throw.knife*/ 0, +4, IF_STANDARD_BLADE | ITEM_NOT_BUYABLE},
    {IC_GUN, 63, TARGETCURS, 0, 48, 18, 0, 0, 0, /* flamethrower */ 0, 0,
     IF_STANDARD_GUN | ITEM_NOT_BUYABLE},
    {IC_PUNCH, 64, PUNCHCURS, 1, 85, 30, 0, 40, 1, /* Crowbar       */ 0, -4,
     ITEM_METAL | ITEM_DAMAGEABLE},
    {IC_GUN, 65, TARGETCURS, 0, 45, 40, 0, 10000, 0, /* rocket rifle */ 0, -5,
     IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE | ITEM_ELECTRONIC},
    ...
    {IC_AMMO, 21, INVALIDCURS, 1, 9, 6, 4, 150, 5, /* CLIP545_30_AP */ 0, 0,
     IF_STANDARD_CLIP | ITEM_BIGGUNLIST},
    {IC_AMMO, 22, INVALIDCURS, 1, 10, 6, 4, 100, 4, /* CLIP545_30_HP */ 0, 0,
     IF_STANDARD_CLIP | ITEM_BIGGUNLIST},
    ...
    {IC_AMMO, 25, INVALIDCURS, 1, 22, 3, 6, 60, 6, /* CLIP762W_10_AP */ 0, 0,
     IF_STANDARD_CLIP | ITEM_BIGGUNLIST},
    {IC_AMMO, 26, INVALIDCURS, 1, 29, 8, 4, 180, 4, /* CLIP762W_30_AP */ 0, 0,
     IF_STANDARD_CLIP | ITEM_BIGGUNLIST},
    {IC_AMMO, 27, INVALIDCURS, 1, 23, 3, 6, 40, 5, /* CLIP762W_10_HP */ 0, 0,
     IF_STANDARD_CLIP | ITEM_BIGGUNLIST},
    {IC_AMMO, 28, INVALIDCURS, 1, 30, 8, 4, 120, 3, /* CLIP762W_30_HP */ 0, 0,
     IF_STANDARD_CLIP | ITEM_BIGGUNLIST},
    ...
    {IC_AMMO, 44, INVALIDCURS, 0, 49, 8, 4, 0, 0, /* flameThrwr clip */ 0, 0,
    ...
    {IC_GRENADE, 0, TOSSCURS, 1, 38, 6, 4, 100, 6,
     /* stun grenade				*/ 0, -2,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 1, TOSSCURS, 1, 48, 6, 4, 120, 5, /* tear gas grenade   */ 0, -2,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 2, TOSSCURS, 1, 41, 6, 4, 500, 8, /* mustard gas grenade*/ 0, -3,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 3, TOSSCURS, 1, 50, 3, 6, 150, 6, /* mini hand grenade  */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 4, TOSSCURS, 1, 49, 6, 4, 200, 7, /* reg hand grenade   */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_BOMB, 5, INVALIDCURS, 2, 3, 11, 2, 400, 7, /* RDX                */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 6, INVALIDCURS, 2, 0, 11, 1, 500, 6, /* TNT (="explosives")*/ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 7, INVALIDCURS, 2, 23, 11, 1, 1000, 8, /* HMX (=RDX+TNT)     */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 8, INVALIDCURS, 1, 45, 11, 1, 750, 7, /* C1  (=RDX+min oil) */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 9, INVALIDCURS, 1, 40, 41, 2, 400, 9, /* mortar shell       */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},

    {IC_BOMB, 10, BOMBCURS, 1, 46, 8, 1, 300, 5, /* mine               */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_BOMB, 11, INVALIDCURS, 1, 44, 11, 1, 1500, 9,
     /* C4  ("plastique")  */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 12, BOMBCURS, 1, 42, 4, 2, 0, 0,
     /* trip flare				  */ 0, -2,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 13, BOMBCURS, 1, 43, 4, 2, 0, 0, /* trip klaxon        */ 0, -2,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE | ITEM_REPAIRABLE},
    {IC_BOMB, 14, INVALIDCURS, 1, 107, 2, 4, 250, 6,
     /* shaped charge			*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 15, TOSSCURS, 2, 24, 1, 6, 50, 3, /* break light (flare)*/ 0, 0,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_GRENADE, 16, INVALIDCURS, 1, 97, 10, 4, 400, 8,
     /* 40mm HE grenade		*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 17, INVALIDCURS, 1, 111, 10, 4, 250, 6,
     /* 40mm tear gas grnd */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 18, INVALIDCURS, 1, 113, 10, 4, 200, 5,
     /* 40mm stun grenade  */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_GRENADE, 19, INVALIDCURS, 1, 112, 10, 4, 100, 7,
     /* 40mm smoke grenade */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},

    {IC_GRENADE, 20, TOSSCURS, 1, 98, 6, 4, 50, 4, /* smoke hand grenade */ 0, -2,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE},
    {IC_BOMB, 21, INVALIDCURS, 1, 40, 41, 8, 450, 0, /* tank shell       */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR},
    {IC_BOMB, 22, INVALIDCURS, 1, 40, 41, 2, 450, 0, /* fake struct ignite*/ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE},
    {IC_GRENADE, 23, TOSSCURS, 2, 37, 6, 4, 50, 0, /* creature cocktail*/ 0, 0,
     ITEM_DAMAGEABLE | ITEM_METAL},
    {IC_BOMB, 24, INVALIDCURS, 1, 40, 41, 2, 450, 0, /* fake struct xplod*/ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE},
    {IC_BOMB, 25, INVALIDCURS, 1, 40, 41, 2, 450, 0, /* fake vehicle xplod*/ 0, -4,
     ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE},
    {IC_GRENADE, 26, TOSSCURS, 1, 48, 6, 4, 0, 0, /* BIG tear gas grenade*/ 0, -2,
    ...
    //---ARMOUR---
    {IC_ARMOUR, 0, INVALIDCURS, 1, 66, 20, 0, 300, 2, /* Flak jacket     */ 0, +2,
     IF_STANDARD_ARMOUR},
    {IC_ARMOUR, 1, INVALIDCURS, 2, 18, 22, 0, 350, 0, /* Flak jacket w X */ 0, +1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 2, INVALIDCURS, 2, 11, 18, 0, 400, 0, /* Flak jacket w Y */ 0, +3,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 3, INVALIDCURS, 1, 64, 32, 0, 500, 4, /* Kevlar jacket   */ 0, 0,
     IF_STANDARD_ARMOUR},
    {IC_ARMOUR, 4, INVALIDCURS, 2, 16, 35, 0, 600, 0, /* Kevlar jack w X */ 0, -1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 5, INVALIDCURS, 2, 9, 29, 0, 700, 0, /* Kevlar jack w Y */ 0, +1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 6, INVALIDCURS, 1, 65, 32, 0, 1000, 8, /* Spectra jacket  */ 0, -2,
     IF_STANDARD_ARMOUR},
    {IC_ARMOUR, 7, INVALIDCURS, 2, 17, 35, 0, 1100, 0, /* Spectra jack w X*/ 0, -3,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 8, INVALIDCURS, 2, 10, 29, 0, 1200, 0, /* Spectra jack w Y*/ 0, -1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 9, INVALIDCURS, 1, 67, 39, 0, 650, 5, /* Kevlar leggings */ 0, 0,
     IF_STANDARD_ARMOUR},

    {IC_ARMOUR, 10, INVALIDCURS, 2, 19, 43, 0, 800, 0, /* Kevlar legs w X */ 0, -1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 11, INVALIDCURS, 2, 12, 35, 0, 950, 0, /* Kevlar legs w Y */ 0, +1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 12, INVALIDCURS, 1, 68, 39, 0, 900, 8, /* Spectra leggings*/ 0, -2,
     IF_STANDARD_ARMOUR},
    {IC_ARMOUR, 13, INVALIDCURS, 2, 20, 43, 0, 1100, 0, /* Spectra legs w X*/ 0, -3,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 14, INVALIDCURS, 2, 13, 35, 0, 1300, 0, /* Spectra legs w Y*/ 0, -1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 15, INVALIDCURS, 1, 61, 14, 0, 50, 2, /* Steel helmet    */ 0, +2,
     IF_STANDARD_ARMOUR | ITEM_METAL},
    {IC_ARMOUR, 16, INVALIDCURS, 1, 63, 14, 0, 200, 4, /* Kevlar helmet   */ 0, 0,
     IF_STANDARD_ARMOUR},
    {IC_ARMOUR, 17, INVALIDCURS, 2, 15, 15, 0, 250, 0, /* Kevlar helm w X */ 0, -1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 18, INVALIDCURS, 2, 8, 13, 0, 300, 0, /* Kevlar helm w Y */ 0, +1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 19, INVALIDCURS, 1, 62, 14, 0, 450, 7, /* Spectra helmet  */ 0, -2,
     IF_STANDARD_ARMOUR},

    {IC_ARMOUR, 20, INVALIDCURS, 2, 14, 15, 0, 550, 0, /* Spectra helm w X*/ 0, -3,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 21, INVALIDCURS, 2, 7, 13, 0, 650, 0, /* Spectra helm w Y*/ 0, -1,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 22, INVALIDCURS, 1, 81, 12, 2, 250, 5, /* Ceramic plates  */ 0, -4,
     (IF_STANDARD_ARMOUR | ITEM_ATTACHMENT) & (~ITEM_REPAIRABLE)},
    {IC_ARMOUR, 23, INVALIDCURS, 1, 0, 0, 0, 0, 0, /* Infant crt hide */ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_ARMOUR, 24, INVALIDCURS, 1, 0, 0, 0, 0, 0, /* Yng male hide */ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_ARMOUR, 25, INVALIDCURS, 1, 0, 0, 0, 0, 0, /* Old male hide */ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_ARMOUR, 26, INVALIDCURS, 1, 0, 0, 0, 0, 0, /* Queen cret hide */ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_ARMOUR, 27, INVALIDCURS, 1, 96, 20, 0, 200, 2, /* Leather jacket  */ 0, +4,
     IF_STANDARD_ARMOUR},
    // NOTE: THE FOLLOWING ITEM'S PRICE VALUE IS IN DIALOGUE AND SHOULD NOT BE
    // CHANGED
    {IC_ARMOUR, 28, INVALIDCURS, 1, 116, 20, 0, 950, 0, /* L jacket w kev  */ 0, +2,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 29, INVALIDCURS, 1, 117, 20, 0, 1200, 0,
     /* L jacket w kev 18*/ 0, +1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},

    {IC_ARMOUR, 30, INVALIDCURS, 1, 118, 20, 0, 1500, 0,
     /* L jacket w kev c*/ 0, +3, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 31, INVALIDCURS, 1, 0, 0, 0, 0, 0, /* yng fem hide */ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_ARMOUR, 32, INVALIDCURS, 1, 0, 0, 0, 0, 0, /* old fem hide */ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE},
    {IC_ARMOUR, 33, INVALIDCURS, 2, 25, 3, 1, 10, 1, /* t-shirt */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_SHOW_STATUS | ITEM_UNAERODYNAMIC},
    {IC_ARMOUR, 33, INVALIDCURS, 2, 34, 3, 1, 10, 1, /* t-shirt D. rules*/ 0, 0,
     ITEM_DAMAGEABLE | ITEM_SHOW_STATUS | ITEM_UNAERODYNAMIC},
    {IC_ARMOUR, 34, INVALIDCURS, 1, 137, 32, 0, 700, 6, /* Kevlar2 jacket  */ 0, -1,
     IF_STANDARD_ARMOUR},
    {IC_ARMOUR, 35, INVALIDCURS, 2, 40, 35, 0, 800, 0, /* Kevlar2 jack w X*/ 0, -2,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_ARMOUR, 36, INVALIDCURS, 2, 41, 29, 0, 900, 0, /* Kevlar2 jack w Y*/ 0, 0,
     IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE},
    {IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0},
    {IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0},
    ...
    {IC_MEDKIT, 0, AIDCURS, 1, 86, 18, 0, 300, 1, /* Medical Kit   */ 0, 0,
     IF_STANDARD_KIT | ITEM_METAL},
    {IC_KIT, 0, REPAIRCURS, 2, 21, 50, 0, 250, 1, /* Tool Kit	     */ 0, 0,
     IF_STANDARD_KIT | ITEM_METAL},
    {IC_KIT, 0, INVALIDCURS, 1, 78, 3, 1, 250, 3, /* Locksmith kit */ 0, -2,
    ...
    {IC_MISC, 0, INVALIDCURS, 1, 52, 5, 4, 300, 5, /* Silencer      */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT},
    {IC_MISC, 0, INVALIDCURS, 1, 76, 9, 4, 500, 6, /* Sniper scope  */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT},
    {IC_MISC, 0, INVALIDCURS, 1, 69, 5, 2, 50, 4, /* Bipod         */ 0, +5,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT},
    {IC_FACE, 0, INVALIDCURS, 1, 77, 9, 1, 400, 7, /* Extended ear	 */ 0, -3,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},

    {IC_FACE, 0, INVALIDCURS, 1, 74, 9, 1, 800, 7, /* Night goggles */ 0, -1,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_FACE, 0, INVALIDCURS, 1, 55, 2, 4, 150, 3, /* Sun goggles	 */ 0, +3,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_FACE, 0, INVALIDCURS, 1, 75, 9, 1, 100, 4, /* Gas mask   	 */ 0, +1,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE},
    {IC_KIT, 0, INVALIDCURS, 2, 5, 10, 4, 10, 1, /* Canteen       */ 0, 0, ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 80, 10, 1, 200, 4, /* Metal detector*/ 0, -2,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MISC, 0, INVALIDCURS, 2, 6, 1, 4, 900, 7, /* Compound 18	 */ 0, 0, ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 2, 42, 1, 4, 2500, 0, /* Jar/QueenBlood*/ 0, 0,
     ITEM_DAMAGEABLE | ITEM_NOT_BUYABLE},
    {IC_MISC, 0, INVALIDCURS, 2, 2, 1, 4, 500, 1, /* Jar/Elixir    */ 0, 0, ITEM_DAMAGEABLE},
    {IC_MONEY, 0, INVALIDCURS, 2, 1, 1, 1, 0, 0, /* Money         */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, JARCURS, 1, 71, 1, 2, 10, 1, /* Glass jar		 */ 0, 0, ITEM_DAMAGEABLE},

    {IC_MISC, 0, INVALIDCURS, 1, 72, 5, 2, 50, 1, /* Jar/CreatureBlood*/ 0, 0,
     ITEM_DAMAGEABLE | ITEM_NOT_BUYABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 70, 1, 8, 150, 4, /* Adren Booster */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, INVALIDCURS, 1, 47, 1, 4, 100, 3, /* Detonator     */ 0, +1,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ATTACHMENT | ITEM_METAL},
    {IC_MISC, 0, INVALIDCURS, 1, 47, 1, 4, 200, 6, /* Rem Detonator */ 0, -1,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ATTACHMENT | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MISC, 0, INVALIDCURS, 1, 93, 1, 8, 0, 0, /* Videotape     */ 0, 0, ITEM_NOT_BUYABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 94, 1, 8, 0, 0, /* Deed          */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, INVALIDCURS, 1, 56, 1, 1, 0, 0,
     /* Letter				 */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    ...
    {IC_MISC, 0, INVALIDCURS, 1, 120, 1, 4, 50, 0, /* Bloodcat claws*/ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},

    {IC_MISC, 0, INVALIDCURS, 1, 121, 1, 4, 100, 0, /* Bloodcat teeth*/ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 3, 9, 60, 0, 400, 0, /* Bloodcat pelt */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 54, 0, 99, 0, 0, /* Switch        */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_MISC, 0, INVALIDCURS, 1, 47, 0, 99, 0, 0, /* Action item   */ 0, 0, ITEM_NOT_BUYABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 70, 1, 6, 300, 6, /* Regen Booster */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, INVALIDCURS, 1, 70, 0, 99, 0, 0, /* syringe 3     */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, INVALIDCURS, 1, 70, 0, 99, 0, 0, /* syringe 4     */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, INVALIDCURS, 1, 70, 0, 99, 0, 0, /* syringe 5     */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    ...
    {IC_MISC, 0, INVALIDCURS, 1, 51, 4, 4, 750, 8, /* Laser scope   */ 0, -1,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT | ITEM_ELECTRONIC},
    {IC_MISC, 0, REMOTECURS, 1, 54, 9, 4, 400, 6, /* Remote bomb trig*/ 0, -2,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MISC, 0, WIRECUTCURS, 1, 88, 4, 2, 20, 2, /* Wirecutters   */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL},
    {IC_MISC, 0, INVALIDCURS, 1, 90, 9, 4, 30, 2, /* Duckbill      */ 0, +5,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT},
    {IC_MISC, 0, INVALIDCURS, 1, 106, 20, 1, 30, 1, /* Alcohol  */ 0, 0, ITEM_DAMAGEABLE},
    {IC_FACE, 0, INVALIDCURS, 1, 74, 11, 1, 1500, 10, /* UV goggles */ 0, -1,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MISC, 0, INVALIDCURS, 0, 44, 21, 0, 30, 0, /* discarded LAW*/ 0, 0,
     IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE},
    ...
    {IC_MISC, 0, INVALIDCURS, 0, 43, 20, 0, 900, 6, /* video camera */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_FACE, 0, INVALIDCURS, 0, 42, 5, 1, 2500, 0, /* robot remote */ 0, -5,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MISC, 0, INVALIDCURS, 1, 103, 20, 0, 500, 0, /* creature claws */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},

    {IC_MISC, 0, INVALIDCURS, 3, 7, 40, 0, 250, 0, /* creature flesh */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 104, 10, 0, 1000, 0, /* creature organ */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
    {IC_MISC, 0, REMOTECURS, 1, 54, 9, 4, 400, 6, /* Remote trigger*/ 0, -2,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL},
    {IC_MISC, 0, INVALIDCURS, 0, 47, 2, 8, 500, 2, /* gold watch */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL},
    {IC_MISC, 0, INVALIDCURS, 1, 136, 100, 0, 200, 2, /* golf clubs */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL},
    {IC_FACE, 0, INVALIDCURS, 3, 11, 5, 1, 100, 1, /* walkman */ 0, -4,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MISC, 0, INVALIDCURS, 3, 8, 50, 0, 300, 2, /* portable tv */ 0, -3,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC},
    {IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0},
    {IC_MISC, 0, INVALIDCURS, 3, 10, 10, 1, 30, 1, /* cigars */ 0, 0, ITEM_DAMAGEABLE},
    {IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0},

    {IC_KEY, 0, INVALIDCURS, 1, 82, 1, 8, 0, 0, /* dull gold key */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_KEY, 1, INVALIDCURS, 1, 83, 1, 8, 0, 0, /* silver key */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_KEY, 2, INVALIDCURS, 1, 84, 1, 8, 0, 0, /* diamond-shpd key */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_KEY, 3, INVALIDCURS, 1, 87, 1, 8, 0, 0, /* bright gold key */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_KEY, 4, INVALIDCURS, 1, 91, 1, 8, 0, 0, /* gold key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_KEY, 5, INVALIDCURS, 1, 92, 1, 8, 0, 0, /* small gold key */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    {IC_KEY, 6, INVALIDCURS, 1, 108, 1, 8, 0, 0, /* electronic */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_KEY, 7, INVALIDCURS, 1, 109, 1, 8, 0, 0, /* passcard       */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_METAL},
    ...
    {IC_MISC, 0, INVALIDCURS, 2, 31, 4, 0, 150, 2, /* silver platter */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_METAL},
    {IC_MISC, 0, INVALIDCURS, 2, 30, 1, 6, 5, 1, /* duct tape */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 2, 28, 3, 1, 20, 0, /* aluminum rod */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 2, 32, 1, 8, 0, 0, /* spring */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_UNAERODYNAMIC | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 2, 33, 4, 1, 25, 0, /* a. rod & spring */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_REPAIRABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_INSEPARABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 133, 4, 1, 20, 0, /* steel rod */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 1, 132, 2, 6, 5, 3, /* quick glue */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 1, 134, 6, 1, 150, 0, /* gun barrel xtndr */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_REPAIRABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_INSEPARABLE},

    {IC_MISC, 0, INVALIDCURS, 2, 35, 1, 8, 0, 0, /* string */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 2, 27, 1, 1, 0, 0, /* tin can */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, TINCANCURS, 2, 36, 2, 4, 0, 0, /* string & tin can */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, INVALIDCURS, 1, 122, 3, 6, 5, 0, /* marbles */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 127, 6, 1, 200, 6, /* lame boy */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 1, 128, 1, 8, 5, 1, /* copper wire */ 0, 0,
     ITEM_METAL | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 1, 126, 7, 1, 50, 0, /* display unit */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 1, 130, 1, 1, 100, 5, /* fumble pak */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC},
    {IC_MISC, 0, INVALIDCURS, 1, 131, 1, 2, 10, 5, /* xray bulb */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 2, 29, 1, 8, 1, 1, /* chewing gum */ 0, 0,
     ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON},

    {IC_MISC, 0, INVALIDCURS, 1, 129, 3, 1, 100, 0, /* flash device */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC |
         ITEM_HIDDEN_ADDON},
    {IC_MISC, 0, INVALIDCURS, 2, 26, 1, 6, 5, 1, /* batteries */ 0, 0, ITEM_DAMAGEABLE},
    {IC_MISC, 0, INVALIDCURS, 1, 123, 1, 8, 0, 0, /* elastic */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC},
    {IC_MISC, 0, REMOTECURS, 1, 125, 10, 1, 2500, 0, /* xray device */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC},
    {IC_MONEY, 0, INVALIDCURS, 2, 38, 2, 1, 100, 0, /* silver   */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
    {IC_MONEY, 0, INVALIDCURS, 1, 135, 2, 1, 300, 0, /* gold */ 0, 0,
     ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE},
...
// it is a slot used for noting the skill check required for a merge or
// multi-item attachment
...
bool ItemHasAttachments(OBJECTTYPE const &o) {
  return o.usAttachItem[0] != NOTHING || o.usAttachItem[1] != NOTHING ||
         o.usAttachItem[2] != NOTHING || o.usAttachItem[3] != NOTHING;
}
...
BOOLEAN ValidItemAttachment(const OBJECTTYPE *const pObj, const uint16_t usAttachment,
  ...
  if (!ValidAttachment(usAttachment, pObj->usItem)) {
    ...
    if ((FindAttachment(pObj, UNDER_GLAUNCHER) != ITEM_NOT_FOUND) &&
      ...
      {
              // if there is no other grenade attached already, then we can
      attach it if (FindAttachmentByClass( pObj, IC_GRENADE) != ITEM_NOT_FOUND)
              {
                      return( FALSE );
              }
      ...
      */
      ...
      if (fAttemptingAttachment && ValidAttachmentClass(usAttachment, pObj->usItem)) {
        ...
        ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, g_langRes->Message[STR_CANT_ATTACH],
                  ItemNames[usAttachment], ItemNames[pObj->usItem]);
      }
  ...
  if (fAttemptingAttachment) {
    if (fSameItem) {
      ...
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, g_langRes->Message[STR_CANT_USE_TWO_ITEMS],
                ItemNames[usSimilarItem], ItemNames[usAttachment]);
      return (FALSE);
    }
  }
...
uint8_t CalculateObjectWeight(OBJECTTYPE const *const o) {
  ...
  if (item.ubPerPocket <= 1) {
    ...
    if (Item[o->usItem].usItemClass == IC_GUN && o->ubGunShotsLeft > 0) {  // Add in weight of ammo
...
void CleanUpStack(OBJECTTYPE *const o, OBJECTTYPE *const cursor_o) {
  INVTYPE const &item = Item[o->usItem];
  if (!(item.usItemClass & IC_AMMO) && !(item.usItemClass & IC_KIT) &&
...
BOOLEAN ReloadGun(SOLDIERTYPE *pSoldier, OBJECTTYPE *pGun, OBJECTTYPE *pAmmo) {
  ...
  if (Item[pGun->usItem].usItemClass == IC_LAUNCHER || pGun->usItem == TANK_CANNON) {
    ...
    fSameMagazineSize =
        (Magazine[Item[pAmmo->usItem].ubClassIndex].ubMagSize == Weapon[pGun->usItem].ubMagSize);
    ...
    switch (bReloadType) {
      ...
      case RELOAD_SWAP:
        ...
        if (fReloadingWithStack) {
          ...
          // Copying the old ammo to the cursor in turnbased could screw up for
  ...
  if (pSoldier->bTeam == OUR_TEAM) {
    ...
    ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, g_langRes->Message[STR_PLAYER_RELOADS],
              pSoldier->name);
  }
...
BOOLEAN EmptyWeaponMagazine(OBJECTTYPE *pWeapon, OBJECTTYPE *pAmmo) {
  ...
  if (pWeapon->ubGunShotsLeft > 0) {
    ...
    // pWeapon->usGunAmmoItem		= 0; // leaving the ammo item the same for
    // auto-reloading purposes
...
int8_t FindAmmoToReload(SOLDIERTYPE *pSoldier, int8_t bWeaponIn, int8_t bExcludeSlot) {
  ...
  if (Item[pObj->usItem].usItemClass == IC_GUN && pObj->usItem != TANK_CANNON) {
    ...
    // look for any ammo that matches which is of the same calibre and magazine
    // size
    bSlot = FindAmmo(pSoldier, Weapon[pObj->usItem].ubCalibre, Weapon[pObj->usItem].ubMagSize,
                     bExcludeSlot);
    if (bSlot != NO_SLOT) {
      return (bSlot);
    } else {
      // look for any ammo that matches which is of the same calibre (different
      // size okay)
...
BOOLEAN AutoReload(SOLDIERTYPE *pSoldier) {
  ...
  if (Item[pObj->usItem].usItemClass == IC_GUN || Item[pObj->usItem].usItemClass == IC_LAUNCHER) {
    ...
    if (bSlot != NO_SLOT) {
      ...
      // if we are valid for two-pistol shooting (reloading) and we have enough
      // APs still then do a reload of both guns!
        ...
        if (bSlot != NO_SLOT) {
          ...
          if (EnoughPoints(pSoldier, (int16_t)bAPCost, 0, FALSE)) {
            ...
            ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE,
                      g_langRes->Message[STR_RELOAD_ONLY_ONE_GUN], pSoldier->name);
          }
        }
      }
...
bool AttachObject(SOLDIERTYPE *const s, OBJECTTYPE *const pTargetObj,
                  OBJECTTYPE *const pAttachment) {
  ...
  if (valid_launchable || ValidItemAttachment(&target, attachment.usItem, TRUE)) {
    ...
    if (valid_launchable ||
      ...
      if (attach_pos != NO_SLOT &&
          attachment.ubNumberOfObjects > 1) {  // we can only do a swap if there is
                                               // only 1 grenade being attached
        return false;
      }
    } else {
      // try replacing if possible
    ...
    if (s) {
      ...
      if (ValidItemAttachment(&target, attachment.usItem,
                              TRUE))  // not launchable
      {                               // Attachment sounds
        INVTYPE const &tgt_item = Item[target.usItem];
        SoundID const sound = tgt_item.usItemClass & IC_WEAPON   ? ATTACH_TO_GUN
    ...
    if (temp_item != NOTHING) {  // overwrite/swap!
  ...
  switch (merge_kind) {
    case COMBINE_POINTS: {
      ...
      for (int8_t bLoop = attachment.ubNumberOfObjects - 1; bLoop >= 0; --bLoop) {
        ...
        } else {                                 // Add part of this one and then we're done
...
BOOLEAN PlaceObject(SOLDIERTYPE *pSoldier, int8_t bPos, OBJECTTYPE *pObj) {
  ...
  // If the position is either head slot, then the item must be IC_FACE (checked
  // in CanItemFitInPosition).
  if (bPos == HEAD1POS) {
      ...
      switch (Item[pInSlot->usItem].usItemClass) {
        case IC_GUN:
          if (Item[pObj->usItem].usItemClass == IC_AMMO) {
            if (Weapon[pInSlot->usItem].ubCalibre ==
  ...
  // ATE: Put this in to see if we should update the robot, if we were given a
  // controller...
...
static BOOLEAN InternalAutoPlaceObject(SOLDIERTYPE *pSoldier, OBJECTTYPE *pObj, BOOLEAN fNewItem,
  ...
  // Overrides to the standard system: put guns in hand, armour on body (if slot
  // empty)
  switch (pItem->usItemClass) {
    ...
    case IC_FACE:
      if ((pSoldier->inv[HEAD1POS].usItem == NOTHING) &&
          CompatibleFaceItem(pObj->usItem, pSoldier->inv[HEAD2POS].usItem)) {
        ...
        if (pObj->ubNumberOfObjects == 0) {
          return (TRUE);
                 ...
                 CompatibleFaceItem(pObj->usItem, pSoldier->inv[HEAD1POS].usItem)) {
  ...
  if (ubPerSlot == 0) {
    ...
    // would require a bunch of functions for finding the same object by two
    // values...
    if (ubPerSlot > 1 || Item[pObj->usItem].usItemClass == IC_KEY ||
      ...
      while (1) {
        ...
        if (bSlot != bExcludeSlot) {
          if (((Item[pObj->usItem].usItemClass == IC_MONEY) &&
              ...
              (Item[pObj->usItem].usItemClass != IC_MONEY &&
               pSoldier->inv[bSlot].ubNumberOfObjects < ItemSlotLimit(pObj->usItem, bSlot))) {
            // NEW: If in SKI, don't auto-place anything into a stackable slot
            // that's currently hatched out!  Such slots will disappear in their
            // entirety if sold/moved, causing anything added through here to
            // vanish also!
...
static uint16_t FindReplacementMagazine(AmmoKind const calibre, uint8_t const mag_size,
  ...
  for (uint8_t i = 0;; i++) {
    ...
    if (default_mag == NOTHING) {  // Store this one to use if all else fails
...
void CreateItems(uint16_t const usItem, int8_t const bStatus, uint8_t ubNumber,
  ...
  // ARM: to avoid whacking memory once Assertions are removed...  Items will be
...
BOOLEAN ArmBomb(OBJECTTYPE *pObj, int8_t bSetting) {
  ...
  if (fRemote) {
    ...
    // In realtime the player could choose to put down a bomb right before a
    // turn expires, SO add 1 to the setting in RT
  ...
  if ((Item[pObject->usItem].fFlags & ITEM_DAMAGEABLE ||
    ...
    for (bLoop = 0; bLoop < pObject->ubNumberOfObjects; bLoop++) {
      ...
      if (pObject->usItem != NOTHING && pObject->bStatus[bLoop] > 0) {
        ...
        // remove item from index AFTER checking explosions because need item
        // data for explosion!
        if (pObject->bStatus[bLoop] == 1) {
          if (pObject->ubNumberOfObjects > 1) {
            RemoveObjFrom(pObject, bLoop);
            // since an item was just removed, the items above the current were
            // all shifted down one; to process them properly, we have to back
            // up 1 in the counter
            bLoop = bLoop - 1;
          }
        }
  ...
  FOR_EACH_SOLDIER_INV_SLOT(i, *pSoldier) {
    ...
    if (fBlowsUp) {
      ...
      SOLDIERTYPE *const owner =
          (gTacticalStatus.ubAttackBusyCount ? pSoldier->attacker : pSoldier);
...
BOOLEAN IsMedicalKitItem(const OBJECTTYPE *pObject) { return pObject->usItem == MEDICKIT; }
...
void WaterDamage(SOLDIERTYPE &s) {
  ...
  if (s.bCamo > 0 && !HAS_SKILL_TRAIT(&s, CAMOUFLAGED)) {
    ...
    if (s.bCamo == 0) {
      ...
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, g_langRes->Message[STR_CAMO_WASHED_OFF],
                s.name);
    }
  }
...
void ActivateXRayDevice(SOLDIERTYPE *pSoldier) {
  ...
  FOR_EACH_MERC(i) {
    ...
    if (tgt->ubMiscSoldierFlags & SOLDIER_MISC_XRAYED && tgt->xrayed_by == pSoldier) {
...
bool HasObjectImprint(OBJECTTYPE const &o) {
  return (o.usItem == ROCKET_RIFLE || o.usItem == AUTO_ROCKET_RIFLE) && o.ubImprintID != NO_PROFILE;
}
...
void DumpItemsList() {
  ...
  for (usItem = 0; usItem < MAXITEMS; usItem++) {
    ...
    if (pItem->ubCoolness > 0) {
      fprintf(FDump, "%28ls     %2d     $%4d\n", ItemNames[usItem], pItem->ubCoolness,
              pItem->usPrice);
    }
  }
