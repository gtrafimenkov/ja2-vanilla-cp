...
//
// This file contains code devoted to the player AI-controlled medical system.
// Maybe it can be used or adapted for the enemies too...
//

#define NOT_GOING_TO_DIE -1
#define NOT_GOING_TO_COLLAPSE -1

static BOOLEAN FindAutobandageClimbPoint(int16_t sDesiredGridNo, BOOLEAN fClimbUp) {
  // checks for existance of location to climb up to building, not occupied by a
  // medic
  BUILDING *pBuilding;
...
static BOOLEAN FullPatientCheck(const SOLDIERTYPE *const pPatient) {
  ...
  if (pPatient->bLevel != 0) {  // look for a clear spot for jumping up
    ...
    // special "closest" search that ignores climb spots IF they are occupied by
    // non-medics
    ...
    if (CanCharacterBeAutoBandagedByTeammate(pSoldier)) {
      ...
      if (fDoFullCheck) {
        if (soldier_for_full_check == NULL) {
          ...
          // set soldier for full check to NULL; will be set to someone later in
          // loop, or to the first guy on the next pass
...
BOOLEAN CanCharacterAutoBandageTeammate(const SOLDIERTYPE *const pSoldier) {
  ...
  // they must have oklife or more, not be collapsed, have some level of medical
  // competence, and have a med kit of some sort
...
BOOLEAN
CanCharacterBeAutoBandagedByTeammate(const SOLDIERTYPE *const pSoldier) {
...
static int8_t FindBestPatient(SOLDIERTYPE *pSoldier, BOOLEAN *pfDoClimb) {
  int16_t bBestPriority = 0;
  int16_t sPatientGridNo;
    ...
    if (pPatient->bLife > 0 && pPatient->bBleeding && pPatient->ubServiceCount == 0) {
      ...
      if (bPatientPriority >= bBestPriority) {
        if (!ClimbingNecessary(pSoldier, pPatient->sGridNo, pPatient->bLevel)) {
          ...
          int16_t sAdjacentGridNo =
              FindAdjacentGridEx(pSoldier, sPatientGridNo, NULL, NULL, FALSE, FALSE);
          if (sAdjacentGridNo == -1 &&
              gAnimControl[pPatient->usAnimState].ubEndHeight == ANIM_PRONE) {
            // prone; could be the base tile is inaccessible but the rest
            // isn't...
            for (uint8_t cnt2 = 0; cnt2 < NUM_WORLD_DIRECTIONS; ++cnt2) {
          ...
          if (sAdjacentGridNo != -1) {
            ...
            if (sPathCost != 0) {
              ...
              if (pOtherMedic != NULL && pOtherMedic != pSoldier) {
                ...
                // the other medic's
                const int16_t sOtherAdjacentGridNo =
                    FindAdjacentGridEx(pOtherMedic, sPatientGridNo, NULL, NULL, FALSE, FALSE);
                if (sOtherAdjacentGridNo != -1) {
                  if (sOtherAdjacentGridNo == pOtherMedic->sGridNo) {
                    sOtherMedicPathCost = 1;
                  } else {
                    sOtherMedicPathCost =
                        PlotPath(pOtherMedic, sOtherAdjacentGridNo, FALSE, FALSE, RUNNING, 0);
                  }

                  if (sPathCost >= sOtherMedicPathCost) {
                    // this patient is best served by the merc moving to them
                    // now
                    continue;
                  }
                }
