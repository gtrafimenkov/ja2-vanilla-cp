...
void LoadWeaponIfNeeded(SOLDIERTYPE *pSoldier) {
...
#ifdef BETAVERSION
      NumMessage(
          "LoadWeaponIfNeeded: ERROR - no mortar shells found to load "
          "MORTAR!  Guynum",
          pSoldier->ubID);
#endif
...
#ifdef BETAVERSION
      NumMessage(
          "LoadWeaponIfNeeded: ERROR - no grenades found to load "
          "GLAUNCHER!  Guynum",
          pSoldier->ubID);
#endif
  ...
  // remove payload from its pocket, and add it as the hand weapon's first
  // attachment
  ...
  if ((Item[pSoldier->inv[bPayloadPocket].usItem].ubPerPocket == 1) ||
...
static int32_t EstimateShotDamage(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pOpponent,
...
void CalcBestShot(SOLDIERTYPE *pSoldier, ATTACKTYPE *pBestShot) {
  ...
  uint8_t const ubBurstAPs = CalcAPsToBurst(CalcActionPoints(pSoldier), pSoldier->inv[HANDPOS]);
  ...
  FOR_EACH_MERC(i) {
    ...
    //   ChanceToGetThrough(pSoldier,pOpponent->sGridNo,NOTFAKE,ACTUAL,TESTWALLS,9999,M9PISTOL,NOT_FOR_LOS);
    ...
    if ((pSoldier->uiStatusFlags & SOLDIER_MONSTER) && (pSoldier->ubBodyType != QUEENMONSTER)) {
      ...
      if (pStructureFileRef) {
        ...
        // must make sure that structure data can be added in the direction of
        // the target
        bDir = (int8_t)GetDirectionToGridNoFromGridNo(pSoldier->sGridNo, pOpponent->sGridNo);
        ...
        if (!OkayToAddStructureToWorld(pSoldier->sGridNo, pSoldier->bLevel,
                                       &pStructureFileRef->pDBStructureRef[OneCDirection(bDir)],
                                       usStructureID)) {
    ...
    for (ubAimTime = AP_MIN_AIM_ATTACK; ubAimTime <= ubMaxPossibleAimTime; ubAimTime++) {
      ...
      // sprintf(tempstr,"Vs. %s, at AimTime %d, ubChanceToHit =
      // %d",ExtMen[pOpponent->ubID].name,ubAimTime,ubChanceToHit);
      // PopMessage(tempstr);
    ...
    if (Item[pSoldier->usAttackingWeapon].usItemClass == IC_THROWING_KNIFE &&
        (ubChanceToReallyHit < 30 || (PythSpacesAway(pSoldier->sGridNo, pOpponent->sGridNo) >
                                      CalcMaxTossRange(pSoldier, THROWING_KNIFE, FALSE) / 2)))
...
// JA2Gold: added
static BOOLEAN CloseEnoughForGrenadeToss(int16_t sGridNo, int16_t sGridNo2) {
  int16_t sTempGridNo;
  uint8_t ubMovementCost;

  if (sGridNo == sGridNo2) {
    // checking the same space; if there is a closed door next to location in
    // ANY direction then forget it (could be the player closed a door on us)
    for (int8_t bDirection = 0; bDirection < NUM_WORLD_DIRECTIONS; ++bDirection) {
    ...
    const int8_t bDirection = GetDirectionToGridNoFromGridNo(sGridNo, sGridNo2);

    // For each step of the loop, we are checking for door or obstacle movement
    // costs.  If we find we're blocked, then this is no good for grenade
    // tossing!
    do {
...
static int32_t EstimateThrowDamage(SOLDIERTYPE *pSoldier, uint8_t ubItemPos, SOLDIERTYPE *pOpponent,
                                   int16_t sGridno);
...
static void CalcBestThrow(SOLDIERTYPE *pSoldier, ATTACKTYPE *pBestThrow) {
  ...
  int32_t iEstDamage;
  uint8_t ubFriendCnt = 0;
  uint8_t ubOpponentCnt = 0;
  ...
  FOR_EACH_MERC(i) {
    ...
    */
    ...
    if ((usInHand == MORTAR) || (usInHand == GLAUNCHER)) {
      ...
      /*
      if (bPersOL != SEEN_CURRENTLY && bPersOL != SEEN_LAST_TURN)     // if not
      in sight right now
      {
              continue;          // next soldier
      }
      */
      if (bPersOL == SEEN_CURRENTLY) {
        // active KNOWN opponent, remember where he is so that we DO blow him
        // up!
        sOpponentTile[ubOpponentCnt] = pOpponent->sGridNo;
        bOpponentLevel[ubOpponentCnt] = pOpponent->bLevel;
      } else if (bPersOL == SEEN_LAST_TURN) {
        // cheat; only allow throw if person is REALLY within 2 tiles of where
        // last seen

        // JA2Gold: UB checks were screwed up
        /*
        if ( pOpponent->sGridNo == gsLastKnownOppLoc[ pSoldier->ubID ][
        pOpponent->ubID ] )
        {
                continue;
        }
        else */
        if (!CloseEnoughForGrenadeToss(pOpponent->sGridNo,
        ...
        /*
        if ( SpacesAway( pOpponent->sGridNo, gsLastKnownOppLoc[ pSoldier->ubID
        ][ pOpponent->ubID ] ) > 2 )
        {
                continue;
        }
        sOpponentTile[ubOpponentCnt] = gsLastKnownOppLoc[ pSoldier->ubID ][
        pOpponent->ubID ]; bOpponentLevel[ubOpponentCnt] = gbLastKnownOppLevel[
        ...
        */
        ...
        // Weird detail: if the opponent is in the same location then they may
        // have closed a door on us. In which case, don't throw!
    ...
    iOppThreatValue[ubOpponentCnt] =
        CalcManThreatValue(pOpponent, pSoldier->sGridNo, FALSE, pSoldier);
  ...
  // this is try to minimize enemies wasting their (limited) toss attacks, with
  // the exception of break lights
  if (usGrenade != BREAK_LIGHT) {
    switch (ubDiff) {
      case 0:
      case 1:
        // they won't use them until they have 2+ opponents as long as half life
        // left
        ...
        break;
      case 2:
        // they won't use them until they have 2+ opponents as long as 3/4 life
        // left
  ...
  for (uint8_t ubLoop = 0; ubLoop < ubOpponentCnt; ++ubLoop) {
    ...
    for (bYOffset = -bMaxUp; bYOffset <= bMaxDown; bYOffset++) {
      for (bXOffset = -bMaxLeft; bXOffset <= bMaxRight; bXOffset++) {
        ...
        // if considering a gas/smoke grenade, check to see if there is such
        // stuff already there!
        if (usGrenade) {
          switch (usGrenade) {
            case SMOKE_GRENADE:
        ...
        if (gGameOptions.ubDifficultyLevel == DIF_LEVEL_EASY) {
          ...
          // Only use it if we are in a surface sector ( basement will be hard
          // enough, plus more chances of mercs being clumped together )
        ...
        // ubChanceToGetThrough =
        // ChanceToGetThrough(pSoldier,sGridNo,NOTFAKE,ACTUAL,TESTWALLS,9999,M9PISTOL,NOT_FOR_LOS);

        if (EXPLOSIVE_GUN(usInHand)) {
          ubChanceToGetThrough =
              AISoldierToLocationChanceToGetThrough(pSoldier, sGridNo, bOpponentLevel[ubLoop], 0);
          if (ubChanceToGetThrough == 0) {
            continue;  // next gridno
          }
        } else {
          ubChanceToGetThrough =
              100 * CalculateLaunchItemChanceToGetThrough(
                        pSoldier, &(pSoldier->inv[HANDPOS]), sGridNo, bOpponentLevel[ubLoop], 0,
                        &sEndGridNo, TRUE, &bEndLevel, FALSE);  // NumMessage("Chance to get through
                                                                // = ",ubChanceToGetThrough);
          // if we can't possibly get through all the cover
          if (ubChanceToGetThrough == 0) {
            if (bEndLevel == bOpponentLevel[ubLoop] && ubSafetyMargin > 1) {
              // rate "chance of hitting" according to how far away this is from
              // the target but keeping in mind that we don't want to hit far,
              // subtract 1 from the radius here to penalize being far from the
              // target
        ...
        // this is try to minimize enemies wasting their (few) mortar shells or
        // LAWs they won't use them on less than 2 targets as long as half life
        // left
        ...
        if (iAttackValue > pBestThrow->iAttackValue) {
          ...
          // sprintf(tempstr,"new best THROW AttackValue = %d at grid
          // #%d",iAttackValue/100000,gridno); PopMessage(tempstr);
        }
      }
    }
  }
...
static int32_t EstimateStabDamage(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pOpponent,
...
void CalcBestStab(SOLDIERTYPE *pSoldier, ATTACKTYPE *pBestStab, BOOLEAN fBladeAttack) {
  ...
  uint8_t ubRawAPCost, ubMinAPCost, ubMaxPossibleAimTime, ubAimTime;
  uint8_t ubChanceToHit, ubChanceToReallyHit, ubBestChanceToHit = 0;
  uint16_t usTrueMovementMode;

  uint8_t ubBestAimTime = (uint8_t)-1;  // XXX HACK000E
  ...
  FOR_EACH_MERC(i) {
    ...
    // Human: if I don't have enough APs left to get there & stab at this guy,
    // skip 'im. Monster:  I'll do an extra check later on to see if I can reach
    // the guy this turn.
    ...
    if (ubMinAPCost > pSoldier->bActionPoints || ubMinAPCost == 0) {
      ...
      /*
       ...
       {
               // hardcode ubMinAPCost so that aiming time is 0 and can start
       move to stab
               // at any time
...
void CalcTentacleAttack(SOLDIERTYPE *pSoldier, ATTACKTYPE *pBestStab) {
  ...
  FOR_EACH_MERC(i) {
    ...
    if (GetRangeInCellCoordsFromGridNoDiff(pSoldier->sGridNo, pOpponent->sGridNo) >
    ...
    // determine if this is a surprise stab (for tentacles, enemy must not see
    // us, no dist limit)
    fSurpriseStab = FALSE;  // assume it is not a surprise stab
...
static uint8_t NumMercsCloseTo(int16_t sGridNo, uint8_t ubMaxDist) {
  int8_t bNumber = 0;
  FOR_EACH_MERC(i) {
    const SOLDIERTYPE *const s = *i;
    if (s->bTeam == OUR_TEAM && s->bLife >= OKLIFE &&
...
static int32_t EstimateShotDamage(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pOpponent,
  ...
  iDamage = (Weapon[pSoldier->inv[HANDPOS].usItem].ubImpact * (100 - iPowerLost + ubBonus)) / 100;
...
static int32_t EstimateThrowDamage(SOLDIERTYPE *pSoldier, uint8_t ubItemPos, SOLDIERTYPE *pOpponent,
  ...
  switch (pSoldier->inv[ubItemPos].usItem) {
    case GL_SMOKE_GRENADE:
    case SMOKE_GRENADE:
      // Don't want to value throwing smoke very much.  This value is based
      // relative to the value for knocking somebody down, which was giving values
      // that were too high
      return (5);
...
static int32_t EstimateStabDamage(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pOpponent,
  ...
  if (!fBladeAttack) {
    ...
    iImpact = iImpact *
              ...
              100;
    iImpact = iImpact *
              (100 + gbSkillTraitBonus[HANDTOHAND] * NUM_SKILL_TRAITS(pSoldier, HANDTOHAND)) / 100;
...
static int8_t TryToReload(SOLDIERTYPE *const s) {
  OBJECTTYPE &hand = s->inv[HANDPOS];
  WEAPONTYPE const &weapon = Weapon[hand.usItem];
  int8_t const slot = FindAmmo(s, weapon.ubCalibre, weapon.ubMagSize, NO_SLOT);
  return slot != NO_SLOT && ReloadGun(s, &hand, &s->inv[slot]) ? TRUE : NOSHOOT_NOAMMO;
}
...
int8_t CanNPCAttack(SOLDIERTYPE *pSoldier) {
  ...
  if (bCanAttack == NOSHOOT_NOAMMO)  // || NOLOAD
  {
    ...
    if (bWeaponIn != NO_SLOT) {
      ...
      if ((Item[pSoldier->inv[HANDPOS].usItem].usItemClass == IC_GUN) &&
        ...
        if (bWeaponIn != NO_SLOT && (Item[pSoldier->inv[bWeaponIn].usItem].usItemClass == IC_GUN) &&
  ...
  {
    sprintf(tempstr, "%s can't attack! (not OKToAttack, Reason code = %d)", pSoldier->name,
            bCanAttack);
  ...
  if (TANK(pSoldier)) {
    ...
    if (pBestThrow->bWeaponIn == NO_SLOT) {
      ...
      if (pBestThrow->bWeaponIn == NO_SLOT) {
        ...
        // Have rocket launcher... maybe have grenades as well.  which one to
        // use?
...
static int8_t CountAdjacentSpreadTargets(SOLDIERTYPE *pSoldier, int16_t sFirstTarget,
                                         int8_t bTargetLevel) {
  ...
  bDir = (int8_t)GetDirectionToGridNoFromGridNo(pSoldier->sGridNo, sFirstTarget);
  ...
  for (bDirLoop = 0; bDirLoop < 8; bDirLoop++) {
    ...
    if (bDirLoop == 6 && bTargets > 1) {
      // we're done!  otherwise we continue and try to find people in
      // front/behind
      break;
