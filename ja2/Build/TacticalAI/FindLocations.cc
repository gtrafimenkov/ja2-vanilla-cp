...
static INT8 CalcWorstCTGTForPosition(SOLDIERTYPE *const pSoldier, const SOLDIERTYPE *const opponent,
                                     const INT16 sOppGridNo, const INT8 bLevel,
                                     const INT32 iMyAPsLeft) {
...
static INT8 CalcAverageCTGTForPosition(SOLDIERTYPE *const pSoldier,
                                       const SOLDIERTYPE *const opponent, const INT16 sOppGridNo,
                                       const INT8 bLevel, const INT32 iMyAPsLeft) {
  // When considering a gridno for cover, we want to take into account cover if
  // we lie down, so we return the LOWEST chance to get through for that
  // location.
  INT8 bCubeLevel;
...
static INT8 CalcBestCTGT(SOLDIERTYPE *const pSoldier, const SOLDIERTYPE *const opponent,
                         const INT16 sOppGridNo, const INT8 bLevel, const INT32 iMyAPsLeft) {
...
static INT32 CalcCoverValue(SOLDIERTYPE *pMe, INT16 sMyGridNo, INT32 iMyThreat, INT32 iMyAPsLeft,
