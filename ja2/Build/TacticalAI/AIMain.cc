...
void HandleSoldierAI(SOLDIERTYPE *pSoldier) {
  // ATE
  // Bail if we are engaged in a NPC conversation/ and/or sequence ... or we
  // have a pause because we just saw someone... or if there are bombs on the
  // bomb queue
  ...
  if (pSoldier->uiStatusFlags & SOLDIER_PC) {
    // if we're in autobandage, or the AI control flag is set and the player has
    // a quote record to perform, or is a boxer, let AI process this merc;
    // otherwise abort
  ...
  // If TURN BASED and NOT NPC's turn, or realtime and not our chance to think,
  // bail...
  if (gfTurnBasedAI) {
    if ((pSoldier->bTeam != OUR_TEAM) && gTacticalStatus.ubCurrentTeam == OUR_TEAM) {
      return;
    }
    // why do we let the quote record thing be in here?  we're in turnbased the
    // quote record doesn't matter, we can't act out of turn!
...
// bail out!
  ...
  // in the unlikely situation (Sgt Krott et al) that we have a quote trigger
  // going on during turnbased, don't do any AI
  ...
  if (pSoldier->bNewSituation == IS_NEW_SITUATION) {
    ...
    if (gTacticalStatus.ubAttackBusyCount > 0) {
      ...
      if (pSoldier->bAction == AI_ACTION_FIRE_GUN) {
        if (guiNumBullets == 0) {
          ...
          // lobotomized due to new situation for %d", pSoldier->ubID ) );
        ...
        if (guiNumObjectSlots == 0) {
          ...
          DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
                   String(">>>>>> Attack busy count lobotomized due to new "
                          "situation for %d",
                          pSoldier->ubID));
          gTacticalStatus.ubAttackBusyCount = 0;
          fProcessNewSituation = TRUE;
    ...
    if (fProcessNewSituation) {
      if ((pSoldier->uiStatusFlags & SOLDIER_UNDERAICONTROL) &&
        ...
        // don't force, don't want escorted mercs reacting to new opponents,
        // etc. now we don't have AI controlled escorted mercs though - CJC
        CancelAIAction(pSoldier);
      ...
      if (pSoldier->usPathIndex == pSoldier->usPathDataSize) {
        if (pSoldier->sAbsoluteFinalDestination != NOWHERE) {
          if (!ACTING_ON_SCHEDULE(pSoldier) &&
            ...
            // This is close enough... reached final destination for NPC system
            // move
            ...
            // change action data so that we consider this our final destination
            // below
...
// reached destination
        ...
        if (pSoldier->sGridNo == pSoldier->sFinalDestination) {
          if (pSoldier->bAction == AI_ACTION_MOVE_TO_CLIMB) {
            ...
            if (IsActionAffordable(pSoldier)) {
              // set action to none and next action to climb roof so we do that
              // next
      ...
      // ATE: Let's also test if we are in any stationary animation...
        ...
        // If we are trying to open door, or jump fence  don't continue until
        // done...
...
void EndAIGuysTurn(SOLDIERTYPE &s) {
  ...
  if (SOLDIERTYPE *const s = RemoveFirstAIListEntry()) {
    StartNPCAI(*s);
  } else {  // We are at the end, return control to next team
...
void FreeUpNPCFromAttacking(SOLDIERTYPE *const pSoldier) {
  ...
  /*
          ...
          {
  #ifdef TESTAI
                  DebugMsg( TOPIC_JA2AI, DBG_LEVEL_0, String(
  "FreeUpNPCFromAttacking for %d", pSoldier->ubID ) ); #endif if
  (pSoldier->bAction == AI_ACTION_FIRE_GUN)
                  {
                          if (pSoldier->bDoBurst)
                          {
                                  ...
                                  {
                                          // now find the target and have them
  say "close call" quote if
                                          // applicable
                                          pTarget =
  WhoIsThere2(pSoldier->sTargetGridNo, pSoldier->bTargetLevel); if (pTarget &&
  pTarget->bTeam == OUR_TEAM && pTarget->fCloseCall && pTarget->bShock == 0)
                                          {
                                                  // say close call quote!
                                                  TacticalCharacterDialogue(
  pTarget, QUOTE_CLOSE_CALL ); pTarget->fCloseCall = FALSE;
                                          }
                                          ActionDone(pSoldier);
                          ...
                          {
                                  pTarget = WhoIsThere2(pSoldier->sTargetGridNo,
  pSoldier->bTargetLevel); if (pTarget && pTarget->bTeam == OUR_TEAM &&
  pTarget->fCloseCall && pTarget->bShock == 0)
                                  {
                                          // say close call quote!
  ...
  (pSoldier->bAction == AI_ACTION_KNIFE_STAB))
          ...
          // This is here to speed up resolution of interrupts that have already
  been
          // delayed while AttackingPerson was still set (causing ChangeControl
  to
          // bail).  Without it, an interrupt would have to wait until next ani
  frame!
          //if (SwitchTo > -1)
          //  ChangeControl();
          */
...
void ActionDone(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->bAction != AI_ACTION_NONE) {
    ...
    //{
    //	gTacticalStatus.ubAttackBusyCount = 0;
    //	DebugMsg(TOPIC_JA2, DBG_LEVEL_3, "Setting attack busy count to 0 due to
    // Action Done"); 	pSoldier->bBulletsLeft = 0;
    //}
    ...
    /*
                    if ( pSoldier->bLastAction == AI_ACTION_CHANGE_STANCE ||
       pSoldier->bLastAction == AI_ACTION_COWER || pSoldier->bLastAction ==
...
void NPCDoesAct(SOLDIERTYPE *pSoldier) {
  // if the action is visible and we're in a hidden turnbased mode, go to
  // turnbased
  if (gTacticalStatus.uiFlags & TURNBASED && !(gTacticalStatus.uiFlags & INCOMBAT) &&
  ...
  // *** IAN deleted lots of interrupt related code here to simplify JA2
  // development

  // CJC Feb 18 99: make sure that soldier is not in the middle of a turn due to
  // visual crap to make enemies face and point their guns at us
...
void CancelAIAction(SOLDIERTYPE *const pSoldier) {
  ...
  if (SkipCoverCheck) DebugAI("CancelAIAction: SkipCoverCheck turned OFF\n");
#endif
...
static void TurnBasedHandleNPCAI(SOLDIERTYPE *pSoldier) {
  /*
   ...
   if (!pSoldier->bActive || !pSoldier->bInSector || (pSoldier->bLife < OKLIFE))
    {
  #ifdef DEBUGDECISIONS
     AINumMessage("HandleManAI - Unavailable man, skipping
  guy#",pSoldier->ubID); #endif
    ...
    {
  #ifdef DEBUGDECISIONS
     AINumMessage("HandleManAI - Civilian is being serviced, skipping
  guy#",pSoldier->ubID); #endif
  ...
  if (anim < BREATHING)
   {
    ...
    if ((pSoldier->bBreath < OKBREATH) || (pSoldier->bActionPoints < (AP_GET_UP
 + AP_ROLL_OVER))
        || pSoldier->service)
     {
 #ifdef DEBUGDECISIONS
 ...
 #ifdef DEBUGBUSY
      AINumMessage("HandleManAI - About to get up, skipping
 guy#",pSoldier->ubID); #endif
  ...
  if (!AnimControl[anim].interruptable)
   {
 #ifdef DEBUGBUSY
    AINumMessage("HandleManAI - uninterruptable animation, skipping guy
 #",pSoldier->ubID); #endif
 ...
 */
  ...
  /*
          ...
          if (pSoldier->bNewSituation)
                  // don't force, don't want escorted mercs reacting to new
     opponents, etc. CancelAIAction(pSoldier);
  ...
  */
  ...
  if ((pSoldier->bAction != AI_ACTION_NONE) && pSoldier->bActionInProgress) {
    /*
       ...
       pSoldier->sGridNo )
                                    //(IC?) if (pSoldier->bAction ==
       AI_ACTION_RANDOM_PATROL && ( pSoldier->usPathIndex ==
       pSoldier->usPathDataSize ) )
                                    //(old?) if (pSoldier->bAction ==
       AI_ACTION_RANDOM_PATROL && ( pSoldier->usActionData == pSoldier->sGridNo
       ) )
                                    {
            #ifdef TESTAI
                                            DebugMsg( TOPIC_JA2AI, DBG_LEVEL_0,
       String("OPPONENT %d REACHES DEST - ACTION DONE",pSoldier->ubID ) );
                                    ...
                                    // *** TRICK- TAKE INTO ACCOUNT PAUSED FOR
       NO TIME ( FOR NOW ) if (pSoldier->fNoAPToFinishMove)
                                    //if (pSoldier->bAction ==
       AI_ACTION_RANDOM_PATROL && pSoldier->fNoAPToFinishMove)
                                    {
                                            // OK, we have a move to finish...

            #ifdef TESTAI
                                            DebugMsg( TOPIC_JA2AI, DBG_LEVEL_0,
       String("GONNA TRY TO CONTINUE PATH FOR %d", pSoldier->ubID ) ); #endif

                                            SoldierTriesToContinueAlongPath(pSoldier);

                                            // since we just gave up on our
       action due to running out of points, better end our turn
                                            //EndAIGuysTurn(*pSoldier);
                                    }
                            }
    */
...
INT8 ExecuteAction(SOLDIERTYPE *pSoldier) {
  ...
  if (gfTurnBasedAI || gTacticalStatus.fAutoBandageMode) {
    DebugAI(String("%d does %hs (a.d. %d) in %d with %d APs left", pSoldier->ubID,
                   gzActionStr[pSoldier->bAction], pSoldier->usActionData, pSoldier->sGridNo,
                   pSoldier->bActionPoints));
  }
#endif

  DebugAI(String("%d does %hs (a.d. %d) at time %ld", pSoldier->ubID,
  ...
  switch (pSoldier->bAction) {
    ...
    case AI_ACTION_WAIT:  // hold AI_ACTION_NONE for a specified time
      if (gfTurnBasedAI) {
        ...
        if (pSoldier->ubProfile != NO_PROFILE) {
          // DebugMsg( TOPIC_JA2, DBG_LEVEL_0, String( "%s waiting %d from %d",
    ...
    case AI_ACTION_CHANGE_FACING:  // turn this way & that to look
      ...
      /*
                              if
      (!StartTurn(pSoldier,pSoldier->usActionData,FASTTURN))
                              {
      #ifdef BETAVERSION
                                      sprintf(tempstr,"ERROR: %s tried TURN to
      direction %d, StartTurn failed, action %d CANCELED",
                                                      pSoldier->name,pSoldier->usActionData,pSoldier->bAction);
                                      PopMessage(tempstr);
      #endif

                                      // ZAP NPC's remaining action points so this
      isn't likely to repeat pSoldier->bActionPoints = 0;

                                      CancelAIAction(pSoldier);
                                      return(FALSE);         // nothing is in
      progress
                              }
                              */
      break;
    ...
    case AI_ACTION_RUN:
      ...
      if (pSoldier->ubBodyType == BLOODCAT) {
        if ((gTacticalStatus.uiFlags & INCOMBAT)) {
          if (Random(2) == 0) {
            PlaySoldierJA2Sample(pSoldier, SoundRange<BLOODCAT_GROWL_1, BLOODCAT_GROWL_4>(),
                                 HIGHVOLUME, 1, TRUE);
          }
        }
      }
      ...
      if (!pSoldier->bPathStored) {
        ...
        if (!pSoldier->bPathStored) {
          ...
          if (pSoldier->sAbsoluteFinalDestination != NOWHERE &&
              !(gTacticalStatus.uiFlags & INCOMBAT)) {
            // ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_ERROR, L"AI %ls failed to get
            // path for dialogue-related move!", pSoldier->name);
      ...
      NewDest(pSoldier,
              pSoldier->usActionData);  // set new ->sDestination to actionData

      // make sure it worked (check that pSoldier->sDestination ==
      ...
      if (pSoldier->sFinalDestination != pSoldier->usActionData) {
        ...
        sprintf(tempstr,
                "ExecuteAction: ERROR - %s tried MOVE to gridno %d, NewDest "
                "failed, action %d CANCELED",
                pSoldier->name, pSoldier->usActionData, pSoldier->bAction);
...
#endif
      ItemHandleResult const iRetCode =
          HandleItem(pSoldier, pSoldier->usActionData, pSoldier->bTargetLevel,
                     pSoldier->inv[HANDPOS].usItem, FALSE);
      if (iRetCode != ITEM_HANDLE_OK) {
        if (iRetCode != ITEM_HANDLE_BROKEN)  // if the item broke, this is 'legal'
                                             // and doesn't need reporting
        {
          DebugAI(
              String("AI %d got error code %d from HandleItem, doing action "
                     "%d, has %d APs... aborting deadlock!",
                     pSoldier->ubID, iRetCode, pSoldier->bAction, pSoldier->bActionPoints));
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION,
                    L"AI %d got error code %d from HandleItem, doing action "
                    L"%d... aborting deadlock!",
    ...
    case AI_ACTION_PULL_TRIGGER:  // activate an adjacent panic trigger
      ...
      // gTacticalStatus.fPanicFlags					= 0; // turn all flags
      // off
    ...
    case AI_ACTION_RED_ALERT:  // tell friends opponent(s) seen
      ...
      // ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION, L"Debug: AI radios your
      // position!" );
      // DROP THROUGH HERE!
    case AI_ACTION_YELLOW_ALERT:  // tell friends opponent(s) heard
      // ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION, L"Debug: AI radios
      // about a noise!" );
