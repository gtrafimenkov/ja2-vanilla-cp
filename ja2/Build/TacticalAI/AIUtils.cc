...
int8_t OKToAttack(SOLDIERTYPE *pSoldier, int target) {
...
uint8_t ShootingStanceChange(SOLDIERTYPE *pSoldier, ATTACKTYPE *pAttack, int8_t bDesiredDirection) {
  ...
  switch (gAnimControl[usRealAnimState].ubEndHeight) {
    // set a stance number comparable with our loop variable so we can easily
    // compute stance differences and thus AP cost
    case ANIM_STAND:
      bStanceNum = 0;
      break;
    case ANIM_CROUCH:
  ...
  for (bLoop = 0; bLoop < 3; bLoop++) {
    ...
    uiChanceOfDamage =
        SoldierToLocationChanceToGetThrough(pSoldier, pAttack->sTarget, pSoldier->bTargetLevel,
                                            pSoldier->bTargetCubeLevel, pAttack->opponent) *
        CalcChanceToHitGun(pSoldier, pAttack->sTarget, pAttack->ubAimTime, AIM_SHOT_TORSO) / 100;
    if (uiChanceOfDamage > 0) {
      uiStanceBonus = 0;
      // artificially augment "chance of damage" to reflect penalty to be shot
      // at various stances
      ...
      // reduce stance bonus according to how much we have to change stance to
      // get there
...
void NewDest(SOLDIERTYPE *pSoldier, uint16_t usGridNo) {
  ...
  if (IS_MERC_BODY_TYPE(pSoldier) && pSoldier->bAction == AI_ACTION_TAKE_COVER &&
    ...
    // getting real movement anim for someone who is going to take cover, not
    // just considering
    usMovementMode =
        MovementMode[AI_ACTION_TAKE_COVER][Urgency[pSoldier->bAlertStatus][pSoldier->bAIMorale]];
    if (usMovementMode != SWATTING) {
      // really want to look at path, see how far we could get on path while
      // swatting
    ...
    if (pSoldier->bTeam == ENEMY_TEAM && pSoldier->bAlertStatus == STATUS_RED) {
      // switch( pSoldier->bAction )
      {
        ...
        case AI_ACTION_RUN_AWAY:
                pSoldier->usUIMovementMode = DetermineMovementMode( pSoldier,
        pSoldier->bAction ); fSet = TRUE; break;*/
        // default:
  ...
  // EVENT_GetNewSoldierPath( pSoldier, pSoldier->sDestination,
  ...
  switch (pSoldier->bAction) {
    ...
    case AI_ACTION_THROW_KNIFE:
      ...
      if (ptsNeeded > pSoldier->bActionPoints) {
        /*
                sprintf(tempstr,"AI ERROR: %s has insufficient points for attack
           action %d at grid %d",
    ...
    default:
#ifdef BETAVERSION
      // NumMessage("AffordableAction - Illegal action type =
      // ",pSoldier->bAction);
#endif
      break;
  }
...
int16_t RandomFriendWithin(SOLDIERTYPE *const s) {
  // obtain maximum roaming distance from soldier's origin
  int16_t usOrigin;
  const uint16_t usMaxDist = RoamingRange(s, &usOrigin);
  ...
  while (ubFriendCount != 0) {
    ...
    const uint32_t chosen_idx = PreRandom(ubFriendCount);
    ...
    if (SpacesAway(usOrigin, chosen->sGridNo) - 1 <= usMaxDist) {
      // should be close enough, try to find a legal ->sDestination within 1
      // tile

      BOOLEAN fDirChecked[8];
      // clear dirChecked flag for all 8 directions
      for (uint16_t usDirection = 0; usDirection < 8; ++usDirection) {
      ...
      for (uint8_t ubDirsLeft = 8; ubDirsLeft--;) {
        ...
        // direction
        const uint16_t usDest =
            NewGridNo(chosen->sGridNo, DirectionInc((int16_t)(usDirection + 1)));
...
int16_t RandDestWithinRange(SOLDIERTYPE *pSoldier) {
  ...
  int16_t usOrigin;
  uint16_t usMaxDist = RoamingRange(pSoldier, &usOrigin);

  uint8_t room = NO_ROOM;
  ...
  if (pSoldier->ubBodyType == LARVAE_MONSTER) {
    ...
    // keep rolling random ->sDestinations until one's satisfactory or retries
    // used
    while ((ubTriesLeft--) && !fFound) {
      if (fLimited) {
      ...
      if (room != NO_ROOM) {
        uint8_t const temp_room = GetRoom(sRandDest);
...
int16_t ClosestReachableDisturbance(SOLDIERTYPE *pSoldier, uint8_t ubUnconsciousOK,
  ...
  *pfChangeLevel = FALSE;
  ...
  int8_t bClosestLevel = 0;  // XXX HACK000E
  FOR_EACH_MERC(i) {
...
int16_t ClosestKnownOpponent(SOLDIERTYPE *pSoldier, int16_t *psGridNo, int8_t *pbLevel) {
  ...
  FOR_EACH_MERC(i) {
    ...
    // this function is used only for turning towards closest opponent or
    // changing stance as such, if they AI is in a building, we should ignore
    // people who are on the roof of the same building as the AI
    if ((bLevel != pSoldier->bLevel) && SameBuilding(pSoldier->sGridNo, sGridNo)) {
      continue;
    }

    // I hope this will be good enough; otherwise we need a
    // fractional/world-units-based 2D distance function
...
int16_t ClosestPC(const SOLDIERTYPE *pSoldier, int16_t *psDistance) {
  ...
  CFOR_EACH_IN_TEAM(pTargetSoldier, OUR_TEAM) {
    ...
    // if this PC is not visible to the soldier, then add a penalty to the
    // distance so that we weight in favour of visible mercs
    if (pTargetSoldier->bTeam != pSoldier->bTeam &&
        pSoldier->bOppList[pTargetSoldier->ubID] != SEEN_CURRENTLY) {
      sDist += 10;
    }
...
static int16_t FindClosestClimbPointAvailableToAI(SOLDIERTYPE *pSoldier, int16_t sStartGridNo,
                                                  int16_t sDesiredGridNo, BOOLEAN fClimbUp) {
  int16_t sGridNo;
  int16_t sRoamingOrigin;
  ...
  // since climbing necessary involves going an extra tile, we compare against 1
  // less than the roam range... or add 1 to the distance to the climb point
  ...
  if (pSoldier->bLevel == bDestLevel) {
    if ((pSoldier->bLevel == 0) ||
      ...
      *pfClimbingNecessary = FALSE;
      ...
      // different buildings!
      // yes, pass in same gridno twice... want closest climb-down spot for
      // building we are on!
      ...
      if (sClimbGridNo == NOWHERE) {
        ...
        if (sPathCost != 0) {
          ...
          if (fAddCostAfterClimbingUp) {
            ...
            // add in an estimate of getting there after climbing down, *but not
            // on top of roof*
            sPathCost +=
    ...
    if (sClimbGridNo == NOWHERE) {
      ...
      if (sPathCost != 0) {
        ...
        if (pSoldier->bLevel == 0) {
          ...
          if (fAddCostAfterClimbingUp) {
            // add to path a rough estimate of how far to go from the climb
            // gridno to the friend estimate walk cost
            sPathCost += (AP_MOVEMENT_FLAT + WALKCOST) * PythSpacesAway(sClimbGridNo, sDestGridNo);
          }
        } else {
          // must climb down
          sPathCost += AP_CLIMBOFFROOF;
          // add to path a rough estimate of how far to go from the climb gridno
          // to the friend estimate walk cost
          sPathCost += (AP_MOVEMENT_FLAT + WALKCOST) * PythSpacesAway(sClimbGridNo, sDestGridNo);
        }
        *pfClimbingNecessary = TRUE;
        *psClimbGridNo = sClimbGridNo;
...
int16_t ClosestReachableFriendInTrouble(SOLDIERTYPE *pSoldier, BOOLEAN *pfClimbingNecessary) {
  ...
  FOR_EACH_MERC(i) {
    ...
    // CJC: restrict "last one to radio" to only if that guy saw us this turn or
    // last turn

    // if this friend is not under fire, and isn't the last one to radio
    if (!pFriend->bUnderFire &&
        (pFriend != gTacticalStatus.Team[pFriend->bTeam].last_merc_to_radio ||
    ...
    // if we can get there
    if (sPathCost != 0) {
      // sprintf(tempstr,"Path cost to friend %s's location is
      // %d",pFriend->name,pathCost); PopMessage(tempstr);
...
bool InGas(SOLDIERTYPE const *const s, GridNo const grid_no) {
  return gpWorldLevelData[grid_no].ubExtFlags[s->bLevel] &
...
bool WearGasMaskIfAvailable(SOLDIERTYPE *const s) {
  int8_t const slot = FindObj(s, GASMASK);
  if (slot == NO_SLOT || slot == HEAD1POS || slot == HEAD2POS) return false;

  int8_t const new_slot = s->inv[HEAD1POS].usItem == NOTHING ? HEAD1POS
                          : s->inv[HEAD2POS].usItem == NOTHING
                              ? HEAD2POS
...
int8_t CalcMorale(SOLDIERTYPE *pSoldier) {
  ...
  const int8_t *pSeenOpp = &gbSeenOpponents[pSoldier->ubID][0];
  ...
  FOR_EACH_MERC(i) {
    ...
    // sprintf(tempstr,"Known opponent %s, opplist status %d, percent %d, threat
    // = %d",
    ...
    FOR_EACH_MERC(j) {
      ...
      // subtract HEARD_2_TURNS_AGO (which is negative) to make values start at
      // 0 and be positive otherwise
      iPercent = ThreatPercent[pFriend->bOppList[pOpponent->ubID] - OLDEST_HEARD_VALUE];

      // reduce the percentage value based on how far away they are from the
      // enemy, if they only hear him
      ...
      // sprintf(tempstr,"Known by friend %s, opplist status %d, percent %d,
      // threat = %d",
...
int32_t CalcManThreatValue(SOLDIERTYPE *pEnemy, int16_t sMyGrid, uint8_t ubReduceForCover,
                           SOLDIERTYPE *pMe) {
  ...
  if (pEnemy->bLife >= OKLIFE) {
    ...
    if (ubReduceForCover && (sMyGrid != NOWHERE)) {
      ...
      // iThreatValue = (iThreatValue * ChanceToGetThrough( pEnemy, myGrid,
      // FAKE, ACTUAL, TESTWALLS, 9999, M9PISTOL, NOT_FOR_LOS)) / 100;
      // iThreatValue = (iThreatValue * SoldierTo3DLocationChanceToGetThrough(
      // pEnemy, myGrid, FAKE, ACTUAL, TESTWALLS, 9999, M9PISTOL, NOT_FOR_LOS)) /
      // 100;
      iThreatValue = iThreatValue *
                     SoldierToLocationChanceToGetThrough(pEnemy, sMyGrid, pMe->bLevel, 0, pMe) /
                     100;
    }
  } else {
...
int16_t RoamingRange(SOLDIERTYPE *pSoldier, int16_t *pusFromGridNo) {
  ...
  switch (pSoldier->bOrders) {
    // JA2 GOLD: give non-NPCs a 5 tile roam range for cover in combat when being
    // shot at
    case STATIONARY:
