...
static INT8 DecideActionSchedule(SOLDIERTYPE *pSoldier) {
  ...
  switch (ubScheduleAction) {
    case SCHEDULE_ACTION_LOCKDOOR:
      // Uses first gridno for locking door, then second to move to after door is
      // locked. It is possible that the second gridno will border the edge of the
      // map, meaning that the individual will walk off of the map. If this is a
      // "merchant", make sure that nobody occupies the building/room.
            ...
            if (pStructure->fFlags & STRUCTURE_OPEN) {
              ...
              if (pDoor) {
                ...
                ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_BETAVERSION,
                          L"Schedule involved locked door at %d but there's no "
                          L"lock there!",
                          usGridNo1);
                fDoUseDoor = FALSE;
              }
    ...
    case SCHEDULE_ACTION_CLOSEDOOR:
      // Uses first gridno for opening/closing/unlocking door, then second to move
      // to after door is opened. It is possible that the second gridno will border
      // the edge of the map, meaning that the individual will walk off of the map.
      switch (pSoldier->bAIScheduleProgress) {
        case 0:  // move to gridno specified
          if (pSoldier->sGridNo == usGridNo1) {
            pSoldier->bAIScheduleProgress++;
  ...
  if (room == BOXING_RING) {
    ...
    if (sDesiredMercLoc != NOWHERE) {
      ...
      if (pSoldier->bDirection != ubDesiredMercDir &&
        ...
        sprintf(tempstr, "%s - TURNS TOWARDS CLOSEST PC to face direction %d", pSoldier->name,
...
static INT8 DecideActionGreen(SOLDIERTYPE *pSoldier) {
  ...
  if (gTacticalStatus.bBoxingState != NOT_BOXING) {
    ...
    // else if ( (gTacticalStatus.bBoxingState == PRE_BOXING ||
    // gTacticalStatus.bBoxingState == BOXING) && ( PythSpacesAway(
  ...
  if (fCivilian) {
    ...
    if (!gfTurnBasedAI) {
      ...
      if (pSoldier->ubProfile != NO_PROFILE) {
        ...
        // can we act again? not for a minute since we were last spoken
        // to/triggered a record
  ...
  if (!gubNPCPathCount)  // try to limit pathing in Green AI
  {
    ...
    switch (pSoldier->bOrders) {
      ...
      ( pSoldier->bInitialActionPoints < pSoldier->bActionPoints ) // could be less because of
      carried-over points
                                                                                             {
  ...
  if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier->bActionPoints) {
    ...
    if (pSoldier->bLastAction != AI_ACTION_CHANGE_FACING) {
      ...
      if ((INT16)PreRandom(100) < iChance) {
        ...
        // current
        do {
          // if man has a LEGAL dominant facing, and isn't facing it, he will
          // turn back towards that facing 50% of the time here (normally just
          // enemies)
...
static INT8 DecideActionYellow(SOLDIERTYPE *pSoldier) {
  ...
  if (!fCivilian && (pSoldier->bActionPoints >= AP_RADIO) &&
    ...
    iChance = 5 * WhatIKnowThatPublicDont(pSoldier,
                                          FALSE);  // use 5 * for YELLOW alert

    // if I actually know something they don't and I ain't swimming (deep water)
    if (iChance && !DeepWater(pSoldier->sGridNo)) {
      // CJC: this addition allows for varying difficulty levels for soldier
      // types
  ...
  if (!(pSoldier->bTeam == CIV_TEAM && pSoldier->ubProfile != NO_PROFILE &&
    ...
    // IF WE ARE MILITIA/CIV IN REALTIME, CLOSE TO NOISE, AND CAN SEE THE SPOT
    // WHERE THE NOISE CAME FROM, FORGET IT
    ...
    if (fReachable) {
      // remember that noise value is negative, and closer to 0 => more
      // important!
      ...
      // reduce chance if breath is down, less likely to wander around when
      // tired
      iChance -= (100 - pSoldier->bBreath);
...
INT8 DecideActionRed(SOLDIERTYPE *pSoldier, UINT8 ubUnconsciousOK) {
  ...
  if ((!IsOnCivTeam(pSoldier) || pSoldier->ubProfile == WARDEN) &&
      (gTacticalStatus.Team[pSoldier->bTeam].bAwareOfOpposition ||
       pSoldier == gTacticalStatus.the_chosen_one || pSoldier->ubProfile == WARDEN) &&
  ...
  // can't do this in realtime, because the player could be shooting a gun or
  // whatever at the same time!
  if (gfTurnBasedAI && !fCivilian && !bInWater && !bInGas &&
      !(pSoldier->uiStatusFlags & SOLDIER_BOXER) && (CanNPCAttack(pSoldier) == TRUE)) {
    ...
    if (BestThrow.ubPossible) {
      ...
      if (pSoldier->inv[BestThrow.bWeaponIn].usItem == MORTAR) {
        ...
        if (!OKFallDirection(pSoldier, sCheckGridNo, pSoldier->bLevel, ubOpponentDir,
          ...
          sCheckGridNo =
              NewGridNo(pSoldier->sGridNo, DirectionInc(OppositeDirection(ubOpponentDir)));
          if (OKFallDirection(pSoldier, sCheckGridNo, pSoldier->bLevel,
                              OppositeDirection(ubOpponentDir), pSoldier->usAnimState)) {
  ...
  if ((pSoldier->bBreath < 25) && !bInWater && !pSoldier->bUnderFire) {
    ...
    if (!fCivilian && !PTR_CROUCHED && IsValidStance(pSoldier, ANIM_CROUCH)) {
#ifdef DEBUGDECISIONS
      sprintf(tempstr, "%s CROUCHES, NEEDING REST (STATUS RED), breath = %d", pSoldier->name,
              pSoldier->bBreath);
      AIPopMessage(tempstr);
#endif
  ...
  if (!fCivilian && (pSoldier->bActionPoints >= AP_RADIO) &&
    ...
    if (!(gTacticalStatus.Team[pSoldier->bTeam].bAwareOfOpposition) ||
      ...
      // base chance depends on how much new info we have to radio to the others
      iChance = 10 * WhatIKnowThatPublicDont(pSoldier,
                                             FALSE);  // use 10 * for RED alert
      ...
      while ((bSeekPts > -90) || (bHelpPts > -90) || (bHidePts > -90)) {
        ...
        if ((bSeekPts > -90) && (bSeekPts >= bHelpPts) && (bSeekPts >= bHidePts) &&
          ...
          if (sClosestDisturbance != NOWHERE) {
            //////////////////////////////////////////////////////////////////////
            // SEEK CLOSEST DISTURBANCE: GO DIRECTLY TOWARDS CLOSEST KNOWN
            // OPPONENT
            //////////////////////////////////////////////////////////////////////
            ...
            if (pSoldier->usActionData != NOWHERE) {
              ...
              // let's be a bit cautious about going right up to a location
              // without enough APs to shoot
              switch (pSoldier->bAttitude) {
                case DEFENSIVE:
                case CUNNINGSOLO:
                case CUNNINGAID:
        ...
        if ((bWatchPts > -90) && (bWatchPts >= bSeekPts) && (bWatchPts >= bHelpPts) &&
            (bWatchPts >= bHidePts)) {
          // take a look at our highest watch point... if it's still visible,
          // turn to face it and then wait
          bHighestWatchLoc = GetHighestVisibleWatchedLoc(pSoldier);
          // sDistVisible =  DistanceVisible( pSoldier, DIRECTION_IRRELEVANT,
          // DIRECTION_IRRELEVANT, gsWatchedLoc[ pSoldier->ubID ][
          // bHighestWatchLoc ], gbWatchedLocLevel[ pSoldier->ubID ][
          // bHighestWatchLoc ] );
          ...
          if (!SkipCoverCheck && sClosestOpponent != NOWHERE) {
            ...
            if (pSoldier->usActionData != NOWHERE) {
              ...
              if (sClosestDisturbance != NOWHERE &&
                ...
                // ensure will we have enough APs for a possible crouch plus a
                // shot
  ...
  if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier->bActionPoints) {
    ...
    if (sClosestOpponent != NOWHERE) {
      ...
      // if soldier is not already facing in that direction,
      // and the opponent is close enough that he could possibly be seen
      // note, have to change this to use the level returned from
...
#ifdef DEBUGDECISIONS
          sprintf(tempstr, "%s - TURNS TOWARDS CLOSEST ENEMY to face direction %d", pSoldier->name,
                  pSoldier->usActionData);
          AIPopMessage(tempstr);
#endif
    ...
    if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier->bActionPoints) {
      ...
      if ((pSoldier->bDirection != ubOpponentDir)) {
        if ((pSoldier->bActionPoints == pSoldier->bInitialActionPoints ||
          ...
          // limit turning a bit... if the last thing we did was also a turn,
          // add a 60% chance of this being our last turn
  ...
  if (ubCanMove && !pSoldier->bNeutral && (gfTurnBasedAI || pSoldier->bTeam == ENEMY_TEAM)) {
    pSoldier->bAction =
        SearchForItems(*pSoldier, SEARCH_GENERAL_ITEMS, pSoldier->inv[HANDPOS].usItem);
  ...
  if ((pSoldier->bAlertStatus != STATUS_BLACK) && !pSoldier->bUnderFire && ubCanMove &&
    ...
    if (pSoldier->bBleeding > MIN_BLEEDING_THRESHOLD) {
      // reduce bleeding by 1 point per AP (in RT, APs will get recalculated so
      // it's okay)
      pSoldier->bBleeding = __max(0, pSoldier->bBleeding - pSoldier->bActionPoints);
      return (AI_ACTION_NONE);  // will end-turn/wait depending on whether we're
                                // in TB or realtime
    }
#ifdef DEBUGDECISIONS
    ...
    // Skip RED until new situation/next turn, 30% extra chance to do GREEN
    // actions
  ...
  // if not in water and not already crouched, try to crouch down first
  if (!fCivilian && !bInWater && (gAnimControl[pSoldier->usAnimState].ubHeight == ANIM_STAND) &&
      IsValidStance(pSoldier, ANIM_CROUCH)) {
    sClosestOpponent = ClosestKnownOpponent(pSoldier, NULL, NULL);
...
static INT8 DecideActionBlack(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->ubProfile != NO_PROFILE) {
    // if they see enemies, the Queen will keep going to the staircase, but Joe
    // will fight
  ...
  if (pSoldier->uiStatusFlags & SOLDIER_BOXER) {
    ...
    // determine if we happen to be in water (in which case we're in BIG
    // trouble!)
  ...
  if ((bInDeepWater || bInGas) && ubCanMove) {
    ...
    if (pSoldier->usActionData != NOWHERE) {
#ifdef DEBUGDECISIONS
      sprintf(tempstr, "%s - NO LAND NEAR, RUNNING AWAY to grid %d", pSoldier->name,
              pSoldier->usActionData);
      AIPopMessage(tempstr);
#endif
  ...
  if (pSoldier->bTeam == ENEMY_TEAM && pSoldier->bVisible == TRUE &&
    ...
    if (!IsTeamActive(MILITIA_TEAM) && NumPCsInSector() < 4 &&
  ...
  if (pSoldier->bActionPoints < 2 || bInDeepWater || bInGas) {
    ...
    do {
      ...
      if (bCanAttack != TRUE) {
        if (fCivilian) {
          if ((bCanAttack == NOSHOOT_NOWEAPON) && !(pSoldier->uiStatusFlags & SOLDIER_BOXER) &&
  ...
  if (bCanAttack) {
    ...
    if (bWeaponIn != NO_SLOT) {
      ...
      // now it better be a gun, or the guy can't shoot (but has other
      // attack(s))
      if (Item[pSoldier->inv[HANDPOS].usItem].usItemClass == IC_GUN &&
        ...
        if (pSoldier->bActionPoints >= ubMinAPCost) {
          ...
          if (BestShot.ubPossible) {
            ...
            // !serviced) (usually, this means all the guys we see are
            // unconscious, but, on
            ...
            if (opp->bLife < OKLIFE) {
              ...
              if (pSoldier->bAttitude != AGGRESSIVE || BestShot.ubChanceToReallyHit < 60) {
                ...
                // if we found one
                if (sClosestDisturbance != NOWHERE) {
                  // don't bother checking GRENADES/KNIVES, he can't have
                  // conscious targets then make decision as if at alert status
                  // RED, but make sure we don't try to SEEK OPPONENT the
                  // unconscious guy!
                  return (DecideActionRed(pSoldier, FALSE));
                }
                // else kill the guy, he could be the last opponent alive in
                // this sector
              }
    ...
    // this looks for throwables, and sets BestThrow.ubPossible if it can be
    // done
    ...
    if (bWeaponIn != NO_SLOT) {
      ...
      // if we can afford the minimum AP cost to stab with/throw this knife
      // weapon
      if (pSoldier->bActionPoints >= ubMinAPCost) {
        ...
        if (Item[pSoldier->inv[HANDPOS].usItem].usItemClass & IC_THROWING_KNIFE) {
          ...
          // %d",pSoldier->name,ubMinAPCost); PopMessage(tempstr);
  ...
  // NB a desire of 4 or more is only achievable by brave/aggressive guys with
  // high morale
  if (pSoldier->bActionPoints == pSoldier->bInitialActionPoints &&
    ...
    if (ubCanMove && pSoldier->bOrders != STATIONARY && !gfHiddenInterrupt &&
      ...
      if (((pSoldier->bTeam == MILITIA_TEAM) && (PreRandom(20) > BestAttack.ubChanceToReallyHit)) ||
        ...
        // ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_TESTVERSION, L"AI %d allowing
        // cover check, chance to hit is only %d, at range %d",
        // BestAttack.ubChanceToReallyHit, PythSpacesAway( pSoldier->sGridNo,
        // BestAttack.sTarget ) );
...
#ifdef DEBUGDECISIONS
    DebugAI(
        String("%s - CHOICE: iOffense = %d, iDefense = %d\n", pSoldier->name, iOffense, iDefense));
#endif
  ...
  if (ubBestAttackAction != AI_ACTION_NONE) {
    ...
    if (ubBestAttackAction == AI_ACTION_FIRE_GUN) {
      ...
      if (!TANK(pSoldier) &&
        ...
        // since the AI considers shooting chance from standing primarily, if we
        // are not standing we should always consider a stance change
        if (gAnimControl[pSoldier->usAnimState].ubEndHeight != ANIM_STAND) {
          iChance = 100;
        } else {
          iChance = 50;
        ...
        if ((INT32)PreRandom(100) < iChance ||
          ...
          if (ubBestStance != 0) {
            ...
            if (pSoldier->bDirection != bDirection &&
                InternalIsValidStance(pSoldier, bDirection,
                                      gAnimControl[pSoldier->usAnimState].ubEndHeight)) {
              // we're not facing towards him, so turn first!
              pSoldier->usActionData = bDirection;

#ifdef DEBUGDECISIONS
              sprintf(tempstr, "%s - TURNS to face CLOSEST OPPONENT in direction %d",
        ...
        if (pSoldier->bActionPoints - (BestAttack.ubAPCost - BestAttack.ubAimTime) >= ubBurstAPs) {
          ...
          if (TANK(pSoldier)) {
            ...
            if (PythSpacesAway(pSoldier->sGridNo, BestAttack.sTarget) < 10) {
              iChance += (10 - PythSpacesAway(pSoldier->sGridNo, BestAttack.sTarget)) *
          ...
          if ((INT32)PreRandom(100) < iChance) {
            ...
            BestAttack.ubAPCost =
                ...
                CalcAPsToBurst(CalcActionPoints(pSoldier), pSoldier->inv[HANDPOS]);
    ...
    if (fChangeStanceFirst) {  // currently only for guns...
      ...
      DebugAI(
          String("%d(%s) %s %d(%s) at gridno %d (%d APs aim)\n", pSoldier->ubID, pSoldier->name,
                 ...
                 BestAttack.opponent->ubID, BestAttack.opponent->name, BestAttack.target,
                 BestAttack.aimTime));
#endif
  ...
  if (pSoldier->uiStatusFlags & SOLDIER_BOXER) {
    if (ubCanMove) {
      ...
      if (sClosestOpponent != NOWHERE) {
        ...
        // STATIONARY so he has a good roaming range
...
#ifdef DEBUGDECISIONS
    DebugAI(String("%s - TAKING COVER at gridno %d (%d%% better)\n", pSoldier->name, sBestCover,
                   iCoverPercentBetter));
#endif
    // ScreenMsg( FONT_MCOLOR_LTYELLOW, MSG_TESTVERSION, L"AI %d taking cover,
    // morale %d, from %d to %d", pSoldier->ubID, pSoldier->bAIMorale,
  ...
  // if soldier has enough APs left to move at least 1 square's worth
  if (ubCanMove &&
      (pSoldier->bTeam != OUR_TEAM || pSoldier->fAIFlags & AI_RTP_OPTION_CAN_RETREAT)) {
  ...
  // if a militia has absofreaking nothing else to do, maybe they should radio
  // in a report!
...
void DecideAlertStatus(SOLDIERTYPE *pSoldier) {
  ...
  if (gTacticalStatus.bBoxingState == NOT_BOXING) {
    ...
    if (pSoldier->bAlertStatus != bOldStatus) {
      // HERE ARE TRYING TO AVOID NPCs SHUFFLING BACK & FORTH BETWEEN RED &
      // BLACK if either status is < RED (ie. anything but RED->BLACK &&
      // BLACK->RED)
