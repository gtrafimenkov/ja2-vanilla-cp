...
//
// LegalNPCDestination - mode hardcoded to walking; C.O. tear gas related stuff
// commented out TryToResumeMovement - C.O. EscortedMoveCanceled call
...
int LegalNPCDestination(SOLDIERTYPE *pSoldier, INT16 sGridno, UINT8 ubPathMode, UINT8 ubWaterOK,
  ...
  // skip mercs if turnbased and adjacent AND not doing an IGNORE_PATH check
  // (which is used almost exclusively by GoAsFarAsPossibleTowards)
  fSkipTilesWithMercs =
  ...
  || IsWearingHeadGear(*pSoldier, GASMASK)) && ( sGridno != pSoldier->sGridNo )
  && ( sGridno != pSoldier->sBlackList ) )*/
  /*
                                 ...
                                 */
  {
    ...
    switch (ubPathMode) {
      ...
      case ENSURE_PATH_COST:
        return PlotPath(pSoldier, sGridno, FALSE, FALSE, WALKING, 0);
...
int TryToResumeMovement(SOLDIERTYPE *pSoldier, INT16 sGridno) {
  ...
  if (LegalNPCDestination(pSoldier, sGridno, ENSURE_PATH, WATEROK, 0)) {
    ...
    // make sure that it worked (check that pSoldier->sDestination ==
    // pSoldier->sGridNo)
...
#ifdef BETAVERSION
      NumMessage(
          "PROBLEM WITH SCENARIO: All other patrol points are invalid "
          "for guynum ",
          pSoldier->ubID);
#endif
...
INT8 RandomPointPatrolAI(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->sGridNo == sPatrolPoint || pSoldier->bNextPatrolPnt == 0) {
    // find next valid patrol point
    // we keep a count of the # of times we are in here to make sure we don't
    // get into an endless
    // loop
    bCnt = 0;
    do {
    ...
    if (bCnt == pSoldier->bPatrolCnt) {
      // ok, we tried doing this randomly, didn't work well, so now do a linear
      // search
...
INT16 InternalGoAsFarAsPossibleTowards(SOLDIERTYPE *pSoldier, INT16 sDesGrid, INT8 bReserveAPs,
  ...
  fPathFlags = 0;
  if (CREATURE_OR_BLOODCAT(pSoldier)) { /*
                                        if ( PythSpacesAway( pSoldier->sGridNo, sDesGrid ) <=
                                        PATH_CLOSE_RADIUS )
                                        {
                                                // then do a limited range path search and see if
                                        we can get there gubNPCDistLimit = 10; if (
                                        !LegalNPCDestination( pSoldier, sDesGrid, ENSURE_PATH,
                                        NOWATER, fPathFlags) )
                                                {
                                                        gubNPCDistLimit = 0;
                                                        return( NOWHERE );
                                                ...
                                                {
                                                        // allow attempt to path without 'good
                                        enough' flag on gubNPCDistLimit = 0;
                                                }
                                        }
                                        else
                                        {
  ...
  for (sLoop = 0; sLoop < (pSoldier->usPathDataSize - pSoldier->usPathIndex); sLoop++) {
...
#ifdef BETAVERSION
      sprintf(tempstr,
              "GoAsFarAsPossibleTowards: ERROR - gridno along valid route is "
              "invalid!  guynum %d, sTempDest = %d",
    ...
    if (gfTurnBasedAI) {
      ...
      if (sLoop == 0) {
        /*
         // first, add any additional costs - such as intermediate animations,
         etc. switch(pSoldier->anitype[pSoldier->anim])
                {
                 // in theory, no NPC should ever be in one of these animations
         as
                 // things stand (they don't medic anyone), but leave it for
         robustness case START_AID   : case GIVING_AID  : sAnimCost =
         AP_STOP_FIRST_AID; break;
...
void SoldierTriesToContinueAlongPath(SOLDIERTYPE *pSoldier) {
  ...
  // efects... AdjustNoAPToFinishMove( pSoldier, FALSE );
