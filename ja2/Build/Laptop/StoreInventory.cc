...
UINT8 StoreInventory[MAXITEMS][BOBBY_RAY_LISTS] = {
    ...
    // BOBBY_RAY_NEW,
    // The second column is for Bobby Rays used inventory,
    // BOBBY_RAY_USED,
    //
    {0, 0}, /* nothing */
    ...
    {0, 0},
    /* Leather jacket w kevlar 18 */  // 190
...
// SetupStoreInventory sets up the initial quantity on hand for all of Bobby
// Ray's inventory items
void SetupStoreInventory(STORE_INVENTORY *pInventoryArray, BOOLEAN fUsed) {
  ...
  for (i = 0; i < LaptopSaveInfo.usInventoryListLength[fUsed]; i++) {
    ...
    ubNumBought = DetermineInitialInvItems(ARMS_DEALER_BOBBYR, usItemIndex,
                                           StoreInventory[usItemIndex][fUsed], fUsed);
    if (ubNumBought > 0) {
      // If doing used items
      if (fUsed) {
