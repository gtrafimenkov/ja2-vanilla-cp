...
void EnterMercsAccount() {
  ...
  guiMercAuthorizeBoxButton =
      MakeButton(MercAccountText[MERC_ACCOUNT_AUTHORIZE], MERC_AC_AUTHORIZE_BUTTON_X,
                 BtnMercAuthorizeButtonCallback);
  guiMercAuthorizeBoxButton->SpecifyDisabledStyle(GUI_BUTTON::DISABLED_STYLE_SHADED);
  guiMercBackBoxButton = MakeButton(MercAccountText[MERC_ACCOUNT_HOME], MERC_AC_CANCEL_BUTTON_X,
                                    BtnMercBackButtonCallback);

  //	RenderMercsAccount();

  // if true, will display a msgbox telling user that they dont have enough
  // funds
...
void RenderMercsAccount() {
  ...
  BltVideoObject(FRAME_BUFFER, guiAccountNumberGrid, 0, MERC_AC_ACCOUNT_NUMBER_X,
                 MERC_AC_ACCOUNT_NUMBER_Y);

  // Display Players account number
  swprintf(sText, lengthof(sText), L"%ls %05d", MercAccountText[MERC_ACCOUNT_ACCOUNT],
  ...
  DrawTextToScreen(sText, MERC_AC_ACCOUNT_NUMBER_TEXT_X, MERC_AC_ACCOUNT_NUMBER_TEXT_Y, 0,
                   MERC_ACCOUNT_TEXT_FONT, MERC_ACCOUNT_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);
  ...
  DrawTextToScreen(MercAccountText[MERC_ACCOUNT_DAYS], MERC_AC_SECOND_COLUMN_X,
                   ...
                   MERC_ACCOUNT_TEXT_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  ...
  // giMercTotalContractCharge  gets set with the price in DisplayHiredMercs(),
  // so if there is currently no charge, disable the button
...
static void BtnMercAuthorizeButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    swprintf(wzAuthorizeString, lengthof(wzAuthorizeString),
...
static void DisplayHiredMercs() {
  ...
  for (i = 0; i <= 10; i++) {
    ...
    if (IsMercOnTeam(pid) || p.iMercMercContractLength != 0) {
      ...
      // Display Mercs Name
      DrawTextToScreen(p.zName, MERC_AC_FIRST_COLUMN_X + 5, usPosY, MERC_AC_FIRST_COLUMN_WIDTH,
                       MERC_ACCOUNT_DYNAMIC_TEXT_FONT, ubFontColor, FONT_MCOLOR_BLACK,
                       LEFT_JUSTIFIED);
...
static void SettleMercAccounts() {
  ...
  if (iPartialPayment != giMercTotalContractCharge)
    ...
    // Since the player has paid, make sure speck wont complain about the lack
    // of payment
  ...
  /*
          ...
          //if the player doesnt have enough money to fully pay for the all the
  ...
  FindSoldierByProfileIDOnPlayerTeam(ubMercID);
                                  ...
                                  //Calc the contract charge
                                  iContractCharge = gMercProfiles[ ubMercID
  ].sSalary * pSoldier->iTotalContractLength;
  ...
  iContractCharge )
                                  {
                                          pSoldier =
  FindSoldierByProfileIDOnPlayerTeam(ubMercID); pSoldier->iTotalContractLength =
  0;
                  ...
                  {
                          // add the transaction to the finance page
                          AddTransactionToPlayersBook( PAY_SPECK_FOR_MERC,
  GetMercIDFromMERCArray( gubCurMercIndex ), GetWorldTotalMin(),
  -iPartialPayment ); AddHistoryToPlayersLog( HISTORY_SETTLED_ACCOUNTS_AT_MERC,
  GetMercIDFromMERCArray( gubCurMercIndex ), GetWorldTotalMin(), -1, -1 );
                  }


  //		DoLapTopMessageBox( MSG_BOX_BLUE_ON_GREY,
  MercAccountText[MERC_ACCOUNT_NOT_ENOUGH_MONEY], LAPTOP_SCREEN,
  ...
  SPECK_QUOTE_PLAYER_MAKES_PARTIAL_PAYMENT;
          ...
          AddTransactionToPlayersBook( PAY_SPECK_FOR_MERC,
  GetMercIDFromMERCArray( gubCurMercIndex ), GetWorldTotalMin(),
  -giMercTotalContractCharge); AddHistoryToPlayersLog(
  HISTORY_SETTLED_ACCOUNTS_AT_MERC, GetMercIDFromMERCArray( gubCurMercIndex ),
  GetWorldTotalMin(), -1, -1 );
