...
// These enums represent which paragraph they are located in the AIMHISTORYFILE
// file
  ...
  switch (gubCurPageNum) {
    ...
    case 4:
      ...
      DisplayWrappedString(AIM_HISTORY_PARAGRAPH_X, 210 + LAPTOP_SCREEN_WEB_DELTA_Y,
                           ...
                           AIM_HISTORY_TEXT_COLOR, sText, FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
      break;
    ...
    case 5:
      ...
      DisplayWrappedString(AIM_HISTORY_PARAGRAPH_X, 290 + LAPTOP_SCREEN_WEB_DELTA_Y,
                           AIM_HISTORY_PARAGRAPH_WIDTH, 2, AIM_HISTORY_TEXT_FONT,
                           AIM_HISTORY_TEXT_COLOR, sText, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
      break;
  }
...
static void InitAimHistoryMenuBar() {
  ...
  for (i = 0; i < AIM_HISTORY_MENU_BUTTON_AMOUNT; i++) {
    guiHistoryMenuButton[i] = CreateIconAndTextButton(
        ...
        AIM_BUTTON_ON_COLOR, DEFAULT_SHADOW, AIM_BUTTON_OFF_COLOR, DEFAULT_SHADOW, usPosX,
        AIM_HISTORY_MENU_Y, MSYS_PRIORITY_HIGH, BtnHistoryMenuButtonCallback);
...
static void InitTocMenu() {
  ...
  for (i = 0; i < NUM_AIM_HISTORY_PAGES; i++) {
    ...
    DrawTextToScreen(sText, AIM_HISTORY_TOC_X, usPosY + AIM_HISTORY_TOC_Y, AIM_CONTENTBUTTON_WIDTH,
...
static void ExitTocMenu() {
  if (!gfInToc) return;
  gfInToc = FALSE;
  FOR_EACH(MOUSE_REGION, i, gSelectedHistoryTocMenuRegion)
  MSYS_RemoveRegion(&*i);
}
