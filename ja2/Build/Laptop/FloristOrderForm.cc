...
static uint32_t guiFlowerPrice;
...
static uint8_t gubFlowerDestDropDownMode;
static uint8_t gubCurrentlySelectedFlowerLocation;
...
static uint8_t const gubFlowerOrder_AdditioanalServicePrices[] = {10, 20, 10, 10};

static void BtnFlowerOrderBackButtonCallback(GUI_BUTTON *btn, int32_t reason);
static GUIButtonRef guiFlowerOrderBackButton;

static void BtnFlowerOrderSendButtonCallback(GUI_BUTTON *btn, int32_t reason);
static GUIButtonRef guiFlowerOrderSendButton;

static void BtnFlowerOrderClearButtonCallback(GUI_BUTTON *btn, int32_t reason);
static GUIButtonRef guiFlowerOrderClearButton;

static void BtnFlowerOrderGalleryButtonCallback(GUI_BUTTON *btn, int32_t reason);
...
static GUIButtonRef MakeButton(const wchar_t *text, int16_t x, int16_t y, GUI_CALLBACK click) {
  const int16_t shadow_col = FLORIST_BUTTON_TEXT_SHADOW_COLOR;
  GUIButtonRef const btn = CreateIconAndTextButton(
      guiFlowerOrderButtonImage, text, FLORIST_BUTTON_TEXT_FONT, FLORIST_BUTTON_TEXT_UP_COLOR,
      shadow_col, FLORIST_BUTTON_TEXT_DOWN_COLOR, shadow_col, x, y, MSYS_PRIORITY_HIGH, click);
  btn->SetCursor(CURSOR_WWW);
  return btn;
}

static void InitFlowerOrderTextInputBoxes();
static void SelectFloristCardGalleryLinkRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectFloristDisableDropDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectFloristDropDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectFloristGalleryLinkRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectFlorsitCheckBoxRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
...
void EnterFloristOrderForm() {
  ...
  guiFlowerOrderBackButton =
      MakeButton(sOrderFormText[FLORIST_ORDER_BACK], FLOWER_ORDER_BACK_BUTTON_X,
                 FLOWER_ORDER_BACK_BUTTON_Y, BtnFlowerOrderBackButtonCallback);
  guiFlowerOrderSendButton =
      MakeButton(sOrderFormText[FLORIST_ORDER_SEND], FLOWER_ORDER_SEND_BUTTON_X,
                 FLOWER_ORDER_SEND_BUTTON_Y, BtnFlowerOrderSendButtonCallback);
  guiFlowerOrderClearButton =
      MakeButton(sOrderFormText[FLORIST_ORDER_CLEAR], FLOWER_ORDER_CLEAR_BUTTON_X,
                 FLOWER_ORDER_CLEAR_BUTTON_Y, BtnFlowerOrderClearButtonCallback);
  guiFlowerOrderGalleryButton =
      MakeButton(sOrderFormText[FLORIST_ORDER_GALLERY], FLOWER_ORDER_GALLERY_BUTTON_X,
                 FLOWER_ORDER_GALLERY_BUTTON_Y, BtnFlowerOrderGalleryButtonCallback);

  // The check box mouse regions
  for (uint32_t i = 0; i < lengthof(g_order_check_box_pos); ++i) {
    MOUSE_REGION *const r = &gSelectedFloristCheckBoxRegion[i];
    SGPPoint const *const xy = &g_order_check_box_pos[i];
    MSYS_DefineRegion(r, xy->iX, xy->iY, xy->iX + FLOWER_ORDER_CHECK_WIDTH,
                      xy->iY + FLOWER_ORDER_CHECK_HEIGHT, MSYS_PRIORITY_HIGH, CURSOR_WWW,
                      MSYS_NO_CALLBACK, SelectFlorsitCheckBoxRegionCallBack);
  ...
  uint16_t const h = GetFontHeight(FLOWER_ORDEER_SMALL_FONT);
  MSYS_DefineRegion(&gSelectedFloristCardGalleryLinkRegion, x, FLOWER_ORDER_LINK_TO_CARD_GALLERY_Y,
                    x + w, FLOWER_ORDER_LINK_TO_CARD_GALLERY_Y + h, MSYS_PRIORITY_HIGH, CURSOR_WWW,
                    MSYS_NO_CALLBACK, SelectFloristCardGalleryLinkRegionCallBack);

  // flower link
  MSYS_DefineRegion(&gSelectedFloristGalleryLinkRegion, FLOWER_ORDER_FLOWER_BOX_X,
                    FLOWER_ORDER_FLOWER_BOX_Y,
                    FLOWER_ORDER_FLOWER_BOX_X + FLOWER_ORDER_FLOWER_BOX_WIDTH,
                    FLOWER_ORDER_FLOWER_BOX_Y + FLOWER_ORDER_FLOWER_BOX_HEIGHT, MSYS_PRIORITY_HIGH,
                    CURSOR_WWW, MSYS_NO_CALLBACK, SelectFloristGalleryLinkRegionCallBack);

  // drop down city location
  MSYS_DefineRegion(&gSelectedFloristDropDownRegion, FLOWER_ORDER_DELIVERY_LOCATION_X,
                    FLOWER_ORDER_DELIVERY_LOCATION_Y,
                    FLOWER_ORDER_DELIVERY_LOCATION_X + FLOWER_ORDER_DELIVERY_LOCATION_WIDTH,
                    FLOWER_ORDER_DELIVERY_LOCATION_Y + FLOWER_ORDER_DELIVERY_LOCATION_HEIGHT,
...
static BOOLEAN CreateDestroyFlowerOrderDestDropDown(uint8_t ubDropDownMode);
...
void RenderFloristOrderForm() {
  ...
  BltVideoObject(FRAME_BUFFER, guiDeliveryLocation, 0, FLOWER_ORDER_DELIVERY_LOCATION_X,
                 FLOWER_ORDER_DELIVERY_LOCATION_Y);
  BltVideoObject(FRAME_BUFFER, guiFlowerFrame, 0, FLOWER_ORDER_FLOWER_BOX_X,
                 FLOWER_ORDER_FLOWER_BOX_Y);
  BltVideoObject(FRAME_BUFFER, guiCurrentlySelectedFlowerImage, 0, FLOWER_ORDER_FLOWER_BOX_X + 5,
                 FLOWER_ORDER_FLOWER_BOX_Y + 5);
  BltVideoObject(FRAME_BUFFER, guiNameBox, 0, FLOWER_ORDER_NAME_BOX_X, FLOWER_ORDER_NAME_BOX_Y);
  BltVideoObject(FRAME_BUFFER, guiPersonalSentiments, 0, FLOWER_ORDER_SENTIMENT_BOX_X,
  ...
  DrawTextToScreen(sOrderFormText[FLORIST_ORDER_NAME_BOUQUET], FLOWER_ORDER_FLOWER_NAME_X,
                   ...
                   FLOWER_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  DrawTextToScreen(sTemp, usPosX, FLOWER_ORDER_FLOWER_NAME_Y, 0, FLOWER_ORDEER_SMALL_FONT,
                   FLOWER_ORDEER_SMALL_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  DrawTextToScreen(sOrderFormText[FLORIST_ORDER_ADDITIONAL_CHARGES],
                   ...
                   FLOWER_ORDEER_BIG_FONT, FLOWER_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);
  ...
  DrawTextToScreen(sOrderFormText[FLORIST_ORDER_NAME], FLOWER_ORDER_NAME_TEXT_X,
                   ...
                   FLOWER_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
  ...
  DrawTextToScreen(sOrderFormText[FLORIST_ORDER_STANDARDIZED_CARDS], usPosX,
                   ...
                   FLOWER_ORDEER_LINK_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
...
static void BtnFlowerOrderSendButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    AddTransactionToPlayersBook(PURCHASED_FLOWERS, 0, GetWorldTotalMin(), -(int32_t)guiFlowerPrice);
...
static void DisplayFloristCheckBox() {
  ...
  BltVideoObject(FRAME_BUFFER, guiFlowerOrderCheckBoxButtonImage, gfFLoristCheckBox2Down ? 1 : 0,
                 FLOWER_ORDER_CHECK_BOX_2_X, FLOWER_ORDER_CHECK_BOX_2_Y);
  BltVideoObject(FRAME_BUFFER, guiFlowerOrderCheckBoxButtonImage, gfFLoristCheckBox3Down ? 1 : 0,
                 FLOWER_ORDER_CHECK_BOX_3_X, FLOWER_ORDER_CHECK_BOX_3_Y);
  BltVideoObject(FRAME_BUFFER, guiFlowerOrderCheckBoxButtonImage, gfFLoristCheckBox4Down ? 1 : 0,
                 FLOWER_ORDER_CHECK_BOX_4_X, FLOWER_ORDER_CHECK_BOX_4_Y);
  BltVideoObject(FRAME_BUFFER, guiFlowerOrderCheckBoxButtonImage, gfFLoristCheckBox5Down ? 1 : 0,
...
static void DisplayFlowerDynamicItems() {
  ...
  swprintf(sTemp, lengthof(sTemp), L"$%d.00 %ls", guiFlowerPrice,
...
static void FlowerOrderDrawSelectedCity(uint8_t ubNumber);

static void SelectFlowerDropDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason) {
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
...
static void SelectFlowerDropDownMovementCallBack(MOUSE_REGION *pRegion, int32_t reason) {
...
static void GetInputText() {
  ...
  uint8_t const text_field_id = GetActiveFieldID();
  if (text_field_id == 1) {  // The personel sentiment field
    wcsncpy(gsSentimentTextField, GetStringFromField(text_field_id),
            lengthof(gsSentimentTextField));
  } else if (text_field_id == 2) {  // The name field
  ...
  switch (ubDropDownMode) {
    ...
    case FLOWER_ORDER_DROP_DOWN_CREATE: {
      ...
      for (i = 0; i < FLOWER_ORDER_NUMBER_OF_DROP_DOWN_LOCATIONS; i++) {
        MSYS_DefineRegion(&gSelectedFlowerDropDownRegion[i], usPosX, usPosY + 4,
                          usPosX + FLOWER_ORDER_DROP_DOWN_LOCATION_WIDTH, usPosY + usFontHeight,
                          MSYS_PRIORITY_HIGH + 3, CURSOR_WWW, SelectFlowerDropDownMovementCallBack,
                          SelectFlowerDropDownRegionCallBack);
    ...
    case FLOWER_ORDER_DROP_DOWN_DESTROY: {
      ...
      DrawTextToScreen((pDeliveryLocationStrings[gubCurrentlySelectedFlowerLocation]),
                       ...
                       FLOWER_ORDEER_DROP_DOWN_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
...
static void FlowerOrderDrawSelectedCity(uint8_t ubNumber) {
  ...
  DrawTextToScreen((pDeliveryLocationStrings[ubNumber]), FLOWER_ORDER_DROP_DOWN_CITY_START_X + 6,
                   usPosY + 3, 0, FLOWER_ORDEER_DROP_DOWN_FONT, 2, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);
...
static void InitFlowerOrderTextInputBoxes() {
  ...
  if (gbCurrentlySelectedCard != -1) {
    ...
    const uint32_t uiStartLoc = FLOR_CARD_TEXT_TITLE_SIZE * gbCurrentlySelectedCard;
  ...
  AddTextInputField(FLOWER_ORDER_PERSONAL_SENT_BOX_X, FLOWER_ORDER_PERSONAL_SENT_BOX_Y,
                    ...
                    FLOWER_ORDER_PERSONEL_SENTIMENT_NUM_CHARS, INPUTTYPE_FULL_TEXT);
