...
static LaptopMode const gubBobbyRPages[] = {LAPTOP_MODE_BOBBY_R_USED, LAPTOP_MODE_BOBBY_R_MISC,
...
static void InitBobbiesMouseRegion(uint8_t ubNumerRegions, uint16_t *usMouseRegionPosArray,
                                   MOUSE_REGION *MouseRegion);

void EnterBobbyR() {
  uint8_t i;

  // an array of mouse regions for the bobbies signs.  Top Left corner, bottom
  // right corner
...
void ExitBobbyR() {
  ...
  FOR_EACH(MOUSE_REGION, i, gSelectedBobbiesSignMenuRegion)
  MSYS_RemoveRegion(&*i);
...
void RenderBobbyR() {
  ...
  if (LaptopSaveInfo.fBobbyRSiteCanBeAccessed) {
    ...
    //	ShadowText( FRAME_BUFFER, BobbyRaysFrontText[BOBBYR_ADVERTISMENT_1],
    // BOBBIES_SENTENCE_FONT, BOBBIES_FIRST_SENTENCE_X, BOBBIES_FIRST_SENTENCE_Y
    //);
    DrawTextToScreen(BobbyRaysFrontText[BOBBYR_ADVERTISMENT_1], BOBBIES_FIRST_SENTENCE_X,
                     ...
                     BOBBIES_SENTENCE_COLOR, BOBBIES_SIGN_BACKCOLOR,
  ...
  DisplayWrappedString(BOBBIES_USED_SIGN_X, BOBBIES_USED_SIGN_TEXT_OFFSET,
                       ...
                       BobbyRaysFrontText[BOBBYR_USED], BOBBIES_SIGN_BACKCOLOR, CENTER_JUSTIFIED);
  ...
  DisplayWrappedString(BOBBIES_MISC_SIGN_X, BOBBIES_MISC_SIGN_TEXT_OFFSET, BOBBIES_MISC_SIGN_WIDTH,
                       ...
                       BOBBIES_SIGN_BACKCOLOR, CENTER_JUSTIFIED);
  ...
  DisplayWrappedString(BOBBIES_ARMOUR_SIGN_X, BOBBIES_ARMOUR_SIGN_TEXT_OFFSET,
                       ...
                       BobbyRaysFrontText[BOBBYR_ARMOR], BOBBIES_SIGN_BACKCOLOR, CENTER_JUSTIFIED);
  ...
  if (!LaptopSaveInfo.fBobbyRSiteCanBeAccessed) {
    FRAME_BUFFER->ShadowRect(LAPTOP_SCREEN_UL_X, LAPTOP_SCREEN_WEB_UL_Y, LAPTOP_SCREEN_LR_X,
...
void DeleteBobbyRWoodBackground() { DeleteVideoObject(guiWoodBackground); }
...
static void SelectBobbiesSignMenuRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
...
static void HandleBobbyRUnderConstructionAni(BOOLEAN fReset) {
  ...
  if (((uiCurTime - uiLastTime) > BOBBYR_UNDERCONSTRUCTION_ANI_DELAY) || (fReDrawScreenFlag)) {
    ...
    BltVideoObject(FRAME_BUFFER, guiUnderConstructionImage, usCount, BOBBYR_UNDERCONSTRUCTION_X,
    ...
    DrawTextToScreen(BobbyRaysFrontText[BOBBYR_UNDER_CONSTRUCTION],
                     ...
                     BOBBIES_SIGN_BACKCOLOR, CENTER_JUSTIFIED | INVALIDATE_TEXT);
...
static void InitBobbyRayNewInventory() {
  ...
  // add all the NEW items he can ever sell into his possible inventory list,
  // for now in order by item #
  for (i = 0; i < MAXITEMS; i++) {
    // if Bobby Ray sells this, it can be sold, and it's allowed into this game
    // (some depend on e.g. gun-nut option)
...
static void InitBobbyRayUsedInventory() {
  ...
  for (i = 0; i < MAXITEMS; i++) {
    ...
    if ((StoreInventory[i][BOBBY_RAY_USED] != 0) && !(Item[i].fFlags & ITEM_NOT_BUYABLE) &&
      ...
      if ((StoreInventory[i][BOBBY_RAY_USED] != 0) && !(Item[i].fFlags & ITEM_NOT_BUYABLE) &&
          ItemIsLegal(i))
        // in case his store inventory list is wrong, make sure this category of
        // item can be sold used
...
static uint8_t HowManyBRItemsToOrder(uint16_t usItemIndex, uint8_t ubCurrentlyOnHand,
                                     uint8_t ubBobbyRayNewUsed);
static void OrderBobbyRItem(uint16_t usItemIndex);
...
void DailyUpdateOfBobbyRaysNewInventory() {
  ...
  for (i = 0; i < LaptopSaveInfo.usInventoryListLength[BOBBY_RAY_NEW]; i++) {
    ...
    // make sure this item is still sellable in the latest version of the store
    // inventory
    ...
    if (LaptopSaveInfo.BobbyRayInventory[i].ubQtyOnOrder == 0) {
      ...
      if (LaptopSaveInfo.BobbyRayInventory[i].ubQtyOnHand <=
        ...
        if (LaptopSaveInfo.BobbyRayInventory[i].ubQtyOnOrder > 0) {
          // if this is the first day the player is eligible to have access to
          // this thing
          if (!fPrevElig) {
            ...
            if (usItemIndex == ROCKET_LAUNCHER)
              MapScreenMessage(0, MSG_DEBUG, L"%ls: BR Ordered %d, Has %d", WORLDTIMESTR,
  ...
  for (i = 0; i < LaptopSaveInfo.usInventoryListLength[BOBBY_RAY_USED]; i++) {
    ...
    if (LaptopSaveInfo.BobbyRayUsedInventory[i].ubQtyOnOrder == 0) {
      ...
      if (LaptopSaveInfo.BobbyRayUsedInventory[i].ubQtyOnHand == 0) {
        ...
        // make sure this item is still sellable in the latest version of the
        // store inventory
...
static uint8_t HowManyBRItemsToOrder(uint16_t usItemIndex, uint8_t ubCurrentlyOnHand,
  ...
  Assert(usItemIndex < MAXITEMS);
  // formulas below will fail if there are more items already in stock than
  // optimal
  Assert(ubCurrentlyOnHand <= StoreInventory[usItemIndex][ubBobbyRayNewUsed]);
  Assert(ubBobbyRayNewUsed < BOBBY_RAY_LISTS);

  // decide if he can get stock for this item (items are reordered an entire
  // batch at a time)
...
static void OrderBobbyRItem(uint16_t usItemIndex) {
  ...
  // add the new item to the queue.  The new item will arrive in 'uiArrivalTime'
  // minutes.
...
void AddFreshBobbyRayInventory(uint16_t usItemIndex) {
  ...
  if (usItemIndex == ROCKET_LAUNCHER && !fUsed)
    MapScreenMessage(0, MSG_DEBUG, L"%ls: BR Bought %d, Has %d", WORLDTIMESTR,
...
static void SimulateBobbyRayCustomer(STORE_INVENTORY *pInventoryArray, BOOLEAN fUsed) {
  ...
  for (i = 0; i < LaptopSaveInfo.usInventoryListLength[fUsed]; i++) {
    ...
    if (pInventoryArray[i].ubQtyOnHand > 0) {
      ubItemsSold = HowManyItemsAreSold(ARMS_DEALER_BOBBYR, pInventoryArray[i].usItemIndex,
      ...
      if (ubItemsSold > 0) {
        if (i == ROCKET_LAUNCHER && !fUsed)
          MapScreenMessage(0, MSG_DEBUG, L"%ls: BR Sold %d, Has %d", WORLDTIMESTR, ubItemsSold,
