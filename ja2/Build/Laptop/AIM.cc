...
void RenderAIM() {
  ...
  DrawTextToScreen(AimBottomMenuText[AIM_MEMBERS], MEMBERCARD_X, MEMBERS_TEXT_Y, LINK_SIZE_X,
                   FONT12ARIAL, AIM_FONT_MCOLOR_WHITE, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  // Policies
...
void DisplayAimSlogan() {
  ...
  DisplayWrappedString(AIM_LOGO_TEXT_X, AIM_LOGO_TEXT_Y, AIM_LOGO_TEXT_WIDTH, 2, AIM_LOGO_FONT,
                       AIM_FONT_MCOLOR_WHITE, sSlogan, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
}
...
void DisplayAimCopyright() {
  ...
  DrawTextToScreen(sSlogan, AIM_COPYRIGHT_X, AIM_COPYRIGHT_Y, AIM_COPYRIGHT_WIDTH,
                   AIM_COPYRIGHT_FONT, FONT_MCOLOR_DKWHITE, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
...
void InitAimMenuBar() {
  ...
  FOR_EACHX(GUIButtonRef, i, guiBottomButtons, x += BOTTOM_BUTTON_START_WIDTH) {
    GUIButtonRef const b = CreateIconAndTextButton(
        ...
        DEFAULT_SHADOW, x, y, MSYS_PRIORITY_HIGH, BtnAimBottomButtonsCallback);
  ...
  if (fInit)
    ...
    if (ubPreviousAdvertisment == AIM_AD_DONE) {
      ...
      /*
                              ...
                              {
                                      //if the player has NOT ever been to
         drassen if( !LaptopSaveInfo.fBobbyRSiteCanBeAccessed )
                                      {
                              ...
                              if( !fSkip )
                              {
                                      //if the current ad is not supposed to be
         available, loop back to the first ad switch( gubCurrentAdvertisment )
                                      {
                                              case AIM_AD_FUNERAL_ADS:
                                                      if( uiDay <
         AIM_AD_DAY_FUNERAL_AD_STARTS ) gubCurrentAdvertisment =
         AIM_AD_WARNING_BOX; break;

                                              case AIM_AD_FLOWER_SHOP:
                                                      if( uiDay <
         AIM_AD_DAY_FLOWER_AD_STARTS ) gubCurrentAdvertisment =
         AIM_AD_WARNING_BOX; break;

                                              case AIM_AD_INSURANCE_AD:
                                                      if( uiDay <
         AIM_AD_DAY_INSURANCE_AD_STARTS ) gubCurrentAdvertisment =
         AIM_AD_WARNING_BOX; break;
                                      }
                                      fInit = TRUE;
                              }
    ...
    if (ubSubImage == AIM_FLOWER_NUM_SUBIMAGES) {
      if (ubCount == 0 || fRedraw) {
        ...
        DisplayWrappedString(AIM_WARNING_TEXT_X, AIM_FLOWER_LINK_TEXT_Y, AIM_WARNING_TEXT_WIDTH, 2,
                             FONT12ARIAL, 2, AimScreenText[AIM_INFO_7], FONT_MCOLOR_BLACK,
                             CENTER_JUSTIFIED);
...
static BOOLEAN DrawWarningBox(BOOLEAN fInit, BOOLEAN fRedraw) {
  ...
  if (fInit || fRedraw) {
    ...
    DisplayWrappedString(AIM_WARNING_TEXT_X, AIM_WARNING_TEXT_Y, AIM_WARNING_TEXT_WIDTH, 2,
                         AIM_WARNING_FONT, FONT_RED, sText, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
    ...
    case AIM_AD_BOBBY_RAY_AD:
      ...
      if (ubCurSubImage <= AIM_AD_BOBBYR_AD_NUM_DUCK_SUBIMAGES) {
        ...
        DisplayWrappedString(AIM_BOBBYR1_LINK_TEXT_X, AIM_BOBBYR1_LINK_TEXT_Y,
                             ...
                             FONT_MCOLOR_BLACK, LEFT_JUSTIFIED | INVALIDATE_TEXT);
        SetFontShadow(DEFAULT_SHADOW);
      }
      ...
      AIM_BOBBYR3_LINK_TEXT_Y, AIM_WARNING_TEXT_WIDTH, 2,
      BOBBYR_UNDER_CONSTRUCTION_AD_FONT, FONT_MCOLOR_WHITE,
      AimScreenText[AIM_BOBBYR_ADD3], FONT_MCOLOR_BLACK, LEFT_JUSTIFIED |
