...
// this is the percentage of daily salary used as a base to calculate daily
// insurance premiums
...
static void BtnInsContractPrevButtonCallback(GUI_BUTTON *btn, int32_t reason);
static GUIButtonRef guiInsContractPrevBackButton;

static BUTTON_PICS *guiInsContractNextButtonImage;
static void BtnInsContractNextButtonCallBack(GUI_BUTTON *btn, int32_t reason);
...
static void SelectInsuranceContractRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
...
void EnterInsuranceContract() {
  ...
  guiInsContractPrevBackButton =
      MakeButtonBig(guiInsContractPrevButtonImage, InsContractText[INS_CONTRACT_PREVIOUS],
                    INS_INFO_LEFT_ARROW_BUTTON_X, BtnInsContractPrevButtonCallback, 17);

  // Right arrow
  guiInsContractNextButtonImage = LoadButtonImage(LAPTOPDIR "/insrightbutton.sti", 2, 0, -1, 1, -1);
  guiInsContractNextBackButton =
      MakeButtonBig(guiInsContractNextButtonImage, InsContractText[INS_CONTRACT_NEXT],
                    INS_INFO_RIGHT_ARROW_BUTTON_X, BtnInsContractNextButtonCallBack, 18);
...
static BOOLEAN DisplayOrderGrid(uint8_t ubGridNumber, SOLDIERTYPE *);
...
void RenderInsuranceContract() {
  ...
  // disable the next or previous button depending on how many more mercs we
  // have to display
  ...
  DisplayWrappedString(usPosX, INS_CTRCT_BOTTON_LINK_Y + 18, INS_CTRCT_BOTTOM_LINK_RED_WIDTH, 2,
                       INS_FONT_MED, INS_FONT_COLOR, pMessageStrings[MSG_HOMEPAGE],
  ...
  BltVideoObject(FRAME_BUFFER, guiInsOrderBulletImage, 0, INS_CTRCT_FIRST_BULLET_TEXT_X,
  ...
  DisplayWrappedString(INS_CTRCT_FIRST_BULLET_TEXT_X + INSURANCE_BULLET_TEXT_OFFSET_X,
                       ...
                       INS_FONT_MED, INS_FONT_COLOR, sText, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  const uint32_t count_insurance_grids = gubNumberofDisplayedInsuranceGrids;
  for (uint32_t i = 0; i < count_insurance_grids; ++i) {
...
static int32_t CalculateSoldiersInsuranceContractLength(const SOLDIERTYPE *pSoldier);
static BOOLEAN CanSoldierExtendInsuranceContract(const SOLDIERTYPE *pSoldier);
static uint32_t GetTimeRemainingOnSoldiersContract(const SOLDIERTYPE *pSoldier);
static uint32_t GetTimeRemainingOnSoldiersInsuranceContract(const SOLDIERTYPE *pSoldier);

static BOOLEAN DisplayOrderGrid(const uint8_t ubGridNumber, SOLDIERTYPE *const pSoldier) try {
  ...
  const int32_t dx = INS_CTRCT_ORDER_GRID_X + INS_CTRCT_ORDER_GRID_OFFSET_X * ubGridNumber;
  const int32_t dy = INS_CTRCT_ORDER_GRID_Y;
  ...
  bool const is_dead = IsMercDead(p);
  {
    ...
    if (is_dead) {
      ...
      uiInsMercFaceImage->pShades[0] =
          Create16BPPPaletteShaded(uiInsMercFaceImage->Palette(), DEAD_MERC_COLOR_RED,
    ...
    BltVideoObject(FRAME_BUFFER, uiInsMercFaceImage, 0, dx + INS_CTRCT_OG_FACE_OFFSET_X,
                   dy + INS_CTRCT_OG_FACE_OFFSET_Y);
  }

  // display the mercs nickname
  DrawTextToScreen(p.zNickname, dx + INS_CTRCT_OG_NICK_NAME_OFFSET_X,
                   dy + INS_CTRCT_OG_NICK_NAME_OFFSET_Y, 0, INS_FONT_MED, INS_FONT_COLOR,
  ...
  if (is_dead) {
    ...
    DisplayWrappedString(dx + INS_CTRCT_OG_HAS_CONTRACT_OFFSET_X,
                         dy + INS_CTRCT_OG_HAS_CONTRACT_OFFSET_Y,
                         INS_CTRCT_CONTRACT_STATUS_TEXT_WIDTH, 2, INS_FONT_SMALL,
                         INS_FONT_COLOR_RED, sText, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  } else {
    // if the merc has a contract
    if (pSoldier->usLifeInsurance) {
    ...
    const uint8_t colour =
        (fDisplayMercContractStateTextColorInRed ? INS_FONT_COLOR_RED : INS_FONT_COLOR);
    DisplayWrappedString(dx + INS_CTRCT_OG_HAS_CONTRACT_OFFSET_X,
                         dy + INS_CTRCT_OG_HAS_CONTRACT_OFFSET_Y,
                         INS_CTRCT_CONTRACT_STATUS_TEXT_WIDTH, 2, INS_FONT_SMALL, colour, sText,
                         FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  }

  // Display the Emplyment contract text
  GetInsuranceText(INS_SNGL_EMPLOYMENT_CONTRACT, sText);
  DrawTextToScreen(sText, dx + INS_CTRCT_EMPLYMNT_CNTRCT_TEXT_OFFSET_X,
                   dy + INS_CTRCT_EMPLYMNT_CNTRCT_TEXT_OFFSET_Y, INS_CTRCT_ORDER_GRID_WIDTH,
                   INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

  // Display the merc contract Length text
  GetInsuranceText(INS_SNGL_LENGTH, sText);
  DrawTextToScreen(sText, dx + INS_CTRCT_LENGTH_OFFSET_X, dy + INS_CTRCT_LENGTH_OFFSET_Y, 0,
                   INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);

  // Display the mercs contract length
  swprintf(sText, lengthof(sText), L"%d", pSoldier->iTotalContractLength);
  DrawTextToScreen(sText, dx + INS_CTRCT_OG_BOX_OFFSET_X, dy + INS_CTRCT_LENGTH_OFFSET_Y,
  ...
  DrawTextToScreen(sText, dx + INS_CTRCT_LENGTH_OFFSET_X, dy + INS_CTRCT_DAYS_REMAINING_OFFSET_Y, 0,
                   INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  DrawTextToScreen(sText, dx + INS_CTRCT_OG_BOX_OFFSET_X, dy + INS_CTRCT_DAYS_REMAINING_OFFSET_Y,
                   INS_CTRCT_OG_BOX_WIDTH, INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  ...
  DrawTextToScreen(sText, dx + INS_CTRCT_EMPLYMNT_CNTRCT_TEXT_OFFSET_X,
                   dy + INS_CTRCT_INSURANCE_CNTRCT_OFFSET_Y, INS_CTRCT_ORDER_GRID_WIDTH,
                   INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

  GetInsuranceText(INS_SNGL_LENGTH, sText);
  DrawTextToScreen(sText, dx + INS_CTRCT_LENGTH_OFFSET_X, dy + INS_CTRCT_LENGTH_OFFSET_Y + 54, 0,
                   INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);

  // Display the insurance days remaining text
  GetInsuranceText(INS_SNGL_DAYS_REMAINING, sText);
  DrawTextToScreen(sText, dx + INS_CTRCT_LENGTH_OFFSET_X,
                   dy + INS_CTRCT_DAYS_REMAINING_OFFSET_Y + 54, 0, INS_FONT_MED, INS_FONT_COLOR,
                   FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  DrawTextToScreen(sText, dx + INS_CTRCT_OG_BOX_OFFSET_X,
                   dy + INS_CTRCT_DAYS_REMAINING_OFFSET_Y + 54, INS_CTRCT_OG_BOX_WIDTH,
                   INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
  ...
  if (iCostOfContract < 0) {
    ...
    DrawTextToScreen(sText, dx + INS_CTRCT_EMPLYMNT_CNTRCT_TEXT_OFFSET_X,
                     dy + INS_CTRCT_PREMIUM_OWING_OFFSET_Y, INS_CTRCT_ORDER_GRID_WIDTH,
                     INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  ...
  DrawTextToScreen(sText, dx + 32, dy + 179, 72, INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  ...
  DrawTextToScreen(sText, dx + INS_CTRCT_OG_BOX_OFFSET_X, dy + INS_CTRCT_LENGTH_OFFSET_Y + 52 + 2,
                   INS_CTRCT_OG_BOX_WIDTH, INS_FONT_MED, INS_FONT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
...
static void BtnInsuranceAcceptClearFormButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    uint32_t const idx = btn->GetUserData();
...
static void DisableInsuranceContractNextPreviousbuttons() {
  ...
  EnableButton(
      guiInsContractNextBackButton,
      gsCurrentInsuranceMercIndex + gubNumberofDisplayedInsuranceGrids < g_n_insurable_mercs);
  // if we are currently displaying the first set of mercs, disable the previous
  // button
...
static void CreateDestroyInsuranceContractFormButtons(BOOLEAN fCreate) {
  ...
  if (fCreate && !fButtonsCreated) {
    ...
    const int16_t text_col = INS_FONT_BTN_COLOR;
    const int16_t shadow_col = INS_FONT_BTN_SHADOW_COLOR;
    const int16_t y = INS_CTRCT_ORDER_GRID_Y + INS_CTRCT_ACCEPT_BTN_Y;
    for (uint32_t i = 0; i != gubNumberofDisplayedInsuranceGrids; ++i) {
      const int16_t x =
          INS_CTRCT_ORDER_GRID_X + INS_CTRCT_ORDER_GRID_OFFSET_X * i + INS_CTRCT_ACCEPT_BTN_X;
      GUIButtonRef const btn =
          CreateIconAndTextButton(guiInsuranceAcceptClearFormButtonImage, text, INS_FONT_MED,
                                  text_col, shadow_col, text_col, shadow_col, x, y,
                                  MSYS_PRIORITY_HIGH, BtnInsuranceAcceptClearFormButtonCallback);
...
void DailyUpdateOfInsuredMercs() {
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    ...
    if (s->usLifeInsurance) {
      ...
      if ((int16_t)GetWorldDay() != s->iStartOfInsuranceContract) {
...
static float DiffFromNormRatio(int16_t sThisValue, int16_t sNormalValue);
...
int32_t CalculateInsuranceContractCost(int32_t iLength, uint8_t ubMercID) {
  ...
  // only mercs with at least 2 days to go on their employment contract are
  // insurable def: 2/5/99.  However if they already have insurance is SHOULD be
  // ok
  ...
  // calculate the overall insurability risk factor for this merc by combining
  // all the subfactors
  ...
  // premium depend on merc's salary, the base insurance rate, and the
  // individual's risk factor at this time
  uiDailyInsurancePremium =
      (uint32_t)(p.sSalary * INSURANCE_PREMIUM_RATE * flRiskFactor / 100 + 0.5);
...
// values passed in must be such that exceeding the normal value REDUCES
// insurance premiums
static float DiffFromNormRatio(int16_t sThisValue, int16_t sNormalValue) {
  float flRatio;
...
void AddLifeInsurancePayout(SOLDIERTYPE *const pSoldier) {
  ...
  if (LaptopSaveInfo.ubNumberLifeInsurancePayouts <=
    ...
    LaptopSaveInfo.pLifeInsurancePayouts =
        REALLOC(LaptopSaveInfo.pLifeInsurancePayouts, LIFE_INSURANCE_PAYOUT,
                LaptopSaveInfo.ubNumberLifeInsurancePayouts);
  ...
  // This uses the merc's latest salaries, ignoring that they may be higher than
  // the salaries paid under the current contract if the guy has recently gained
  // a level.  We could store his daily salary when he was last contracted, and
  // use that, but it still doesn't easily account for the fact that renewing a
  // leveled merc early means that the first part of his contract is under his
  // old salary and the second part is under his new one.  Therefore, I chose to
  // ignore this wrinkle, and let the player awlays get paid out using the
  // higher amount.  ARM

  // figure out which of the 3 salary rates the merc has is the cheapest, and
  // use it to calculate the paid amount, to avoid getting back more than the
  // merc cost if he was on a 2-week contract!
  ...
  // if the death was suspicious, or he's already been investigated twice or
  // more
...
void StartInsuranceInvestigation(uint8_t ubPayoutID) {
  ...
  if (gStrategicStatus.ubInsuranceInvestigationsCnt == 0) {
    ...
    AddEmailWithSpecialData(INSUR_SUSPIC, INSUR_SUSPIC_LENGTH, INSURANCE_COMPANY,
                            GetWorldTotalMin(), lip->iPayOutPrice, lip->ubMercID);
  } else {
    // subsequent offense
  ...
  // post an event to end the investigation that many days in the future (at
  // 4pm)
...
void InsuranceContractPayLifeInsuranceForDeadMerc(uint8_t ubPayoutID) {
  ...
  if (lip->ubSoldierID == s.ubID) {
    // and if the soldier is still active ( player hasn't removed carcass yet ),
    // reset insurance flag
    if (s.bActive) s.usLifeInsurance = 0;
  }

  // add transaction to players account
  AddTransactionToPlayersBook(INSURANCE_PAYOUT, lip->ubMercID, GetWorldTotalMin(),
                              lip->iPayOutPrice);
...
static BOOLEAN MercIsInsurable(const SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->ubWhatKindOfMercAmI == MERC_TYPE__AIM_MERC) {
    // with more than one day left on their employment contract are eligible for
    // insurance the second part is because the insurance doesn't pay for any
    // working day already started at time of death
    ...
    if (CanSoldierExtendInsuranceContract(pSoldier) || pSoldier->usLifeInsurance) {
      ...
      // POWs are also uninsurable - if already insured, that insurance IS valid
      // but no new contracts or extension allowed
      if (pSoldier->bAssignment != ASSIGNMENT_POW) {
        return (TRUE);
      }
    }
...
static uint32_t GetTimeRemainingOnSoldiersContract(const SOLDIERTYPE *pSoldier) {
  ...
  // Since the merc is leaving in the afternoon, we must adjust since the time
  // left would be different if we did the calc at 11:59 or 12:01 ( noon )
  ...
  //	return( ( pSoldier->iEndofContractTime - (int32_t)GetWorldTotalMin( ) ) /
  // 1440 );
}

static int32_t CalcStartDayOfInsurance(SOLDIERTYPE *pSoldier);
