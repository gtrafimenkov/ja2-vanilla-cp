...
static SGPVObject *guiTotalSaveArea;  // used as a savebuffer for the subtotal,
                                      // s&h, and grand total values
...
static int32_t giGrandTotal;
static uint32_t guiShippingCost;
static uint32_t guiSubTotal;

static uint8_t gubSelectedLight;
...
static uint8_t gubDropDownAction;
static int8_t gbSelectedCity = -1;  // keeps track of the currently selected city
static uint8_t gubCityAtTopOfList;
...
// Clear Order Button
static void BtnBobbyRClearOrderCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiBobbyRClearOrderImage;
static GUIButtonRef guiBobbyRClearOrder;

// Accept Order Button
static void BtnBobbyRAcceptOrderCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiBobbyRAcceptOrderImage;
static GUIButtonRef guiBobbyRAcceptOrder;

// Back Button
static void BtnBobbyRBackCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiBobbyRBackImage;
static GUIButtonRef guiBobbyRBack;

// Home Button
static void BtnBobbyRHomeCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiBobbyRHomeImage;
static GUIButtonRef guiBobbyRHome;

// Goto Shipment Page Button
static void BtnBobbyRGotoShipmentPageCallback(GUI_BUTTON *btn, int32_t reason);
...
static GUIButtonRef MakeButton(BUTTON_PICS *const img, const wchar_t *const text, const int16_t x,
  ...
  GUIButtonRef const btn = CreateIconAndTextButton(
      img, text, BOBBYR_ORDER_TITLE_TEXT_FONT, BOBBYR_GUNS_TEXT_COLOR_ON, shadow_col,
      BOBBYR_GUNS_TEXT_COLOR_OFF, shadow_col, x, y, MSYS_PRIORITY_HIGH, click);
  btn->SetCursor(CURSOR_LAPTOP_SCREEN);
  return btn;
}

static void SelectActivateCityDroDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectCloseDroDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectConfirmOrderRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectShippingSpeedRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
...
void EnterBobbyRMailOrder() {
  ...
  guiBobbyRClearOrder =
      MakeButton(guiBobbyRClearOrderImage, BobbyROrderFormText[BOBBYR_CLEAR_ORDER],
                 BOBBYR_CLEAR_ORDER_X, BOBBYR_CLEAR_ORDER_Y + 4, BtnBobbyRClearOrderCallback);
  ...
  guiBobbyRAcceptOrder =
      MakeButton(guiBobbyRAcceptOrderImage, BobbyROrderFormText[BOBBYR_ACCEPT_ORDER],
                 BOBBYR_ACCEPT_ORDER_X, BOBBYR_ACCEPT_ORDER_Y + 4, BtnBobbyRAcceptOrderCallback);
  ...
  guiBobbyRBack = MakeButton(guiBobbyRBackImage, BobbyROrderFormText[BOBBYR_BACK],
                             BOBBYR_BACK_BUTTON_X, BOBBYR_BACK_BUTTON_Y, BtnBobbyRBackCallback);

  guiBobbyRHomeImage = UseLoadedButtonImage(guiBobbyRBackImage, 0, 1);
  guiBobbyRHome = MakeButton(guiBobbyRHomeImage, BobbyROrderFormText[BOBBYR_HOME],
                             BOBBYR_HOME_BUTTON_X, BOBBYR_HOME_BUTTON_Y, BtnBobbyRHomeCallback);

  giBobbyRGotoShipmentPageImage = UseLoadedButtonImage(guiBobbyRBackImage, 0, 1);
  guiBobbyRGotoShipmentPage = MakeButton(
      giBobbyRGotoShipmentPageImage, BobbyROrderFormText[BOBBYR_GOTOSHIPMENT_PAGE],
      BOBBYR_SHIPMENT_BUTTON_X, BOBBYR_SHIPMENT_BUTTON_Y, BtnBobbyRGotoShipmentPageCallback);
  ...
  // confirmorder mouse region, occupies the entrie screen and is present only
  // when the confirm order graphic s on screen.  When user clicks anywhere the
  // graphic disappears
  ...
  // click anywhere on the screen to close the window( only when the drop down
  // window is active)
...
static void CreateDestroyBobbyRDropDown(uint8_t ubDropDownAction);
...
void HandleBobbyRMailOrder() {
  ...
  if (gfDrawConfirmOrderGrpahic) {
    // Bobbyray title
    BltVideoObjectOutlineShadow(FRAME_BUFFER, guiConfirmGraphic, 0, BOBBYR_CONFIRM_ORDER_X + 3,
                                BOBBYR_CONFIRM_ORDER_Y + 3);
    BltVideoObject(FRAME_BUFFER, guiConfirmGraphic, 0, BOBBYR_CONFIRM_ORDER_X,
                   BOBBYR_CONFIRM_ORDER_Y);
...
static void DrawShippingSpeedLights(uint8_t ubSelected);
...
void RenderBobbyRMailOrder() {
  ...
  BltVideoObject(FRAME_BUFFER, guiBobbyRLocationGraphic, 0, BOBBYR_LOCATION_BOX_X,
                 BOBBYR_LOCATION_BOX_Y);
  BltVideoObject(FRAME_BUFFER, guiDeliverySpeedGraphic, 0, BOBBYR_DELIVERYSPEED_X,
                 BOBBYR_DELIVERYSPEED_Y);
  BltVideoObject(FRAME_BUFFER, guiPackageWeightImage, 0, BOBBYR_PACKAXGE_WEIGHT_X,
                 BOBBYR_PACKAXGE_WEIGHT_Y);
  ...
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_SHIPPING_LOCATION], BOBBYR_SHIPPING_LOCATION_TEXT_X,
                   ...
                   BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  // Create a string for the weight ( %s ) ( where %s is the weight string,
  // either kg or lbs )
  ...
  swprintf(sTemp, lengthof(sTemp), L"%ls %2.1f %ls.", BobbyROrderFormText[BOBBYR_MINIMUM_WEIGHT],
           GetWeightBasedOnMetricOption(MIN_SHIPPING_WEIGHT) / 10.0, GetWeightUnitString());
  DrawTextToScreen(sTemp, BOBBYR_USED_WARNING_X, BOBBYR_USED_WARNING_Y + usHeight + 1, 0,
                   BOBBYR_DISCLAIMER_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED | TEXT_SHADOWED);
    ...
    if (guiSubTotal && gfCanAcceptOrder) {
      ...
      if (LaptopSaveInfo.iCurrentBalance < giGrandTotal) {
        ...
        if (gbSelectedCity == BR_DRASSEN &&
          ...
          swprintf(zTemp, lengthof(zTemp), BobbyROrderFormText[BOBBYR_CONFIRM_DEST],
        ...
        /*				//if the shipment is going to Drassen, add the
        inventory if( gbSelectedCity == BR_DRASSEN )
                                        {
        //					BobbyRayOrderStruct
        *pBobbyRayPurchase;
        //					uint32_t	uiResetTimeSec;
                                                ...
                                                //if we need to add more array
        elements for the Order Array if(
        ...
        = REALLOC(LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray,
        BobbyRayOrderStruct, LaptopSaveInfo.usNumberOfBobbyRayOrderItems);
                                                        memset(
        &LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[
        LaptopSaveInfo.usNumberOfBobbyRayOrderItems - 1 ], 0, sizeof(
        BobbyRayOrderStruct ) );
                                                }
        ...
        in the array if( !LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[ cnt
        ].fActive ) break;
                                                }

                                                //gets reset when the confirm
        order graphic disappears gfCanAcceptOrder = FALSE;

        //					pBobbyRayPurchase =
        MALLOCZ(BobbyRayOrderStruct);


                                                ubCount = 0;
                                                for(i=0; i<MAX_PURCHASE_AMOUNT;
        i++)
                                                {
                                                        //if the item was
        purchased if( BobbyRayPurchases[ i ].ubNumberPurchased )
                                                        {
                                                                //copy the
        purchases into the struct that will be added to the queue
                                                                LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[cnt].BobbyRayPurchase[ubCount]
        = BobbyRayPurchases[i]; ubCount ++;
                                                        }
                                                }

                                                LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[
        cnt ].ubNumberPurchases = ubCount;
                                                LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[
        cnt ].fActive = TRUE; LaptopSaveInfo.usNumberOfBobbyRayOrderUsed++;

                                                //get the length of time to
        receive the shipment if( gubSelectedLight == 0 )
                                                {
                                                        bDaysAhead =
        OVERNIGHT_EXPRESS;
                                                        //uiResetTimeSec =
        ...
        GetMidnightOfFutureDayInMinutes( STANDARD_SERVICE );
                                                ...
                                                {
                                                        // Sal is dead, so Pablo
        is dead, so the airport is badly run bDaysAhead += (uint8_t) Random( 5 ) +
        1;
                                                }

                                                //add a random amount between so
        it arrives between 8:00 am and noon
                                                //uiResetTimeSec += (8 +
        Random(4) ) * 60;

                                                //AddStrategicEvent(
        EVENT_BOBBYRAY_PURCHASE, uiResetTimeSec, cnt);
                                                AddFutureDayStrategicEvent(
        EVENT_BOBBYRAY_PURCHASE, (8 + Random(4) ) * 60, cnt, bDaysAhead );

                                        }

                                        //Add the transaction to the finance
        page AddTransactionToPlayersBook(BOBBYR_PURCHASE, 0, GetWorldTotalMin(),
        -giGrandTotal);

                                        //display the confirm order graphic
                                        gfDrawConfirmOrderGrpahic = TRUE;

                                        //Get rid of the city drop dowm, if it
        is being displayed gubDropDownAction = BR_DROP_DOWN_DESTROY;
...
static void DisplayShippingCosts(BOOLEAN fCalledFromOrderPage, int32_t iSubTotal, uint16_t usGridX,
...
void DisplayPurchasedItems(BOOLEAN fCalledFromOrderPage, uint16_t usGridX, uint16_t usGridY,
  ...
  // Output the qty
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_QTY], usGridX + BOBBYR_GRID_FIRST_COLUMN_X,
                   usGridY + BOBBYR_GRID_FIRST_COLUMN_Y - BOBBYR_GRID_TITLE_OFFSET,
                   BOBBYR_GRID_FIRST_COLUMN_WIDTH, BOBBYR_ORDER_STATIC_TEXT_FONT,
                   BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
  ...
  // Output the Weight
  DisplayWrappedString(usGridX + BOBBYR_GRID_SECOND_COLUMN_X,
                       usGridY + BOBBYR_GRID_SECOND_COLUMN_Y - 30, BOBBYR_GRID_SECOND_COLUMN_WIDTH,
                       2, BOBBYR_ORDER_STATIC_TEXT_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR, sTemp,
                       FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

  // Output the name
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_NAME], usGridX + BOBBYR_GRID_THIRD_COLUMN_X,
                   usGridY + BOBBYR_GRID_THIRD_COLUMN_Y - BOBBYR_GRID_TITLE_OFFSET,
                   BOBBYR_GRID_THIRD_COLUMN_WIDTH, BOBBYR_ORDER_STATIC_TEXT_FONT,
                   BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

  // Output the unit price
  DisplayWrappedString(usGridX + BOBBYR_GRID_FOURTH_COLUMN_X,
                       usGridY + BOBBYR_GRID_FOURTH_COLUMN_Y - 30, BOBBYR_GRID_FOURTH_COLUMN_WIDTH,
                       2, BOBBYR_ORDER_STATIC_TEXT_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR,
                       BobbyROrderFormText[BOBBYR_UNIT_PRICE], FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

  // Output the total
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_TOTAL], usGridX + BOBBYR_GRID_FIFTH_COLUMN_X,
                   usGridY + BOBBYR_GRID_FIFTH_COLUMN_Y - BOBBYR_GRID_TITLE_OFFSET,
  ...
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_SUB_TOTAL], usGridX + BOBBYR_SUBTOTAL_X,
                   usGridY + BOBBYR_SUBTOTAL_Y, BOBBYR_SUBTOTAL_WIDTH,
                   BOBBYR_ORDER_STATIC_TEXT_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_S_H], usGridX + BOBBYR_SUBTOTAL_X,
                   usGridY + BOBBYR_SHIPPING_N_HANDLE_Y, BOBBYR_SUBTOTAL_WIDTH,
                   BOBBYR_ORDER_STATIC_TEXT_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  DrawTextToScreen(BobbyROrderFormText[BOBBYR_GRAND_TOTAL], usGridX + BOBBYR_SUBTOTAL_X,
                   usGridY + BOBBYR_GRAND_TOTAL_Y, BOBBYR_SUBTOTAL_WIDTH,
                   BOBBYR_ORDER_STATIC_TEXT_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  ...
  int32_t iSubTotal = 0;  // XXX HACK000E
  ...
  // loop through the array of purchases to display only the items that are
  // purchased
  ...
  for (i = 0; i < MAX_PURCHASE_AMOUNT; i++) {
    ...
    if (pBobbyRayPurchase[i].ubNumberPurchased) {
      ...
      DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_FIRST_COLUMN_X - 2, usPosY,
                       BOBBYR_GRID_FIRST_COLUMN_WIDTH, BOBBYR_ORDER_DYNAMIC_TEXT_FONT,
                       BOBBYR_ORDER_DYNAMIC_TEXT_COLOR, FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);

      // weight
      swprintf(sTemp, lengthof(sTemp), L"%3.1f",
      ...
      DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_SECOND_COLUMN_X - 2, usPosY,
      ...
      ReduceStringLength(sText, lengthof(sText), BOBBYR_GRID_THIRD_COLUMN_WIDTH - 4,
                         BOBBYR_ORDER_DYNAMIC_TEXT_FONT);

      DrawTextToScreen(sText, usGridX + BOBBYR_GRID_THIRD_COLUMN_X + 2, usPosY,
                       BOBBYR_GRID_THIRD_COLUMN_WIDTH, BOBBYR_ORDER_DYNAMIC_TEXT_FONT,
                       BOBBYR_ORDER_DYNAMIC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);

      // unit price
      const BobbyRayPurchaseStruct *Purchase = &pBobbyRayPurchase[i];
      SPrintMoney(sTemp, CalcBobbyRayCost(Purchase->usItemIndex, Purchase->usBobbyItemIndex,
                                          Purchase->fUsed));
      DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_FOURTH_COLUMN_X - 2, usPosY,
      ...
      DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_FIFTH_COLUMN_X - 2, usPosY,
...
static uint32_t CalcCostFromWeightOfPackage(uint8_t ubTypeOfService);
...
static void DisplayShippingCosts(BOOLEAN fCalledFromOrderPage, int32_t iSubTotal, uint16_t usGridX,
  ...
  if (fCalledFromOrderPage) {
    BltVideoObject(FRAME_BUFFER, guiTotalSaveArea, 0, BOBBYR_TOTAL_SAVED_AREA_X,
  ...
  if (iSubTotal) {
    ...
    DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_FIFTH_COLUMN_X - 2, usGridY + BOBBYR_SUBTOTAL_Y,
                     BOBBYR_GRID_FIFTH_COLUMN_WIDTH, BOBBYR_ORDER_DYNAMIC_TEXT_FONT,
                     BOBBYR_ORDER_DYNAMIC_TEXT_COLOR, FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);

    // Display the shipping and handling charge
    SPrintMoney(sTemp, iShippingCost);
    DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_FIFTH_COLUMN_X - 2,
                     usGridY + BOBBYR_SHIPPING_N_HANDLE_Y, BOBBYR_GRID_FIFTH_COLUMN_WIDTH,
    ...
    DrawTextToScreen(sTemp, usGridX + BOBBYR_GRID_FIFTH_COLUMN_X - 2,
                     usGridY + BOBBYR_GRAND_TOTAL_Y, BOBBYR_GRID_FIFTH_COLUMN_WIDTH,
                     BOBBYR_ORDER_DYNAMIC_TEXT_FONT, BOBBYR_ORDER_DYNAMIC_TEXT_COLOR,
                     FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
  }
...
static void DrawGoldRectangle(int8_t bCityNum);
static void DrawSelectedCity(uint8_t ubCityNumber);
static void SelectDropDownMovementCallBack(MOUSE_REGION *pRegion, int32_t reason);
static void SelectDropDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectScrollAreaDropDownMovementCallBack(MOUSE_REGION *pRegion, int32_t reason);
static void SelectScrollAreaDropDownRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectUpDownArrowOnScrollAreaRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
  ...
  switch (ubDropDownAction) {
    ...
    case BR_DROP_DOWN_CREATE: {
      ...
      for (i = 0; i < BOBBYR_NUM_DISPLAYED_CITIES; i++) {
        MSYS_DefineRegion(&gSelectedDropDownRegion[i], usPosX, usPosY + 4,
                          usPosX + BOBBYR_DROP_DOWN_WIDTH - 6, usPosY + usFontHeight + 7,
                          MSYS_PRIORITY_HIGH, CURSOR_WWW, SelectDropDownMovementCallBack,
      ...
      for (i = 0; i < 2; i++) {
        MSYS_DefineRegion(&gSelectedUpDownArrowOnScrollAreaRegion[i], usPosX, usPosY,
                          usPosX + BOBBYR_SCROLL_ARROW_WIDTH, usPosY + BOBBYR_SCROLL_ARROW_HEIGHT,
                          MSYS_PRIORITY_HIGH, CURSOR_WWW, MSYS_NO_CALLBACK,
      ...
      // disable the clear order and accept order buttons, (their rendering
      // interferes with the drop down graphics)
    ...
    case BR_DROP_DOWN_DESTROY: {
      ...
      if (gbSelectedCity == -1)
        DrawTextToScreen(BobbyROrderFormText[BOBBYR_SELECT_DEST],
                         ...
                         BOBBYR_ORDER_DROP_DOWN_SELEC_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
      else
      ...
      // enable the clear order and accept order buttons, (because their rendering
      // interferes with the drop down graphics)
    ...
    case BR_DROP_DOWN_DISPLAY: {
      ...
      // top arrow
      BltVideoObject(FRAME_BUFFER, guiGoldArrowImages, 1, BOBBYR_SCROLL_UP_ARROW_X,
                     BOBBYR_SCROLL_UP_ARROW_Y);

      // top arrow
      BltVideoObject(FRAME_BUFFER, guiGoldArrowImages, 0, BOBBYR_SCROLL_DOWN_ARROW_X,
...
static void DrawSelectedCity(uint8_t ubCityNumber) {
  ...
  for (i = gubCityAtTopOfList; i < gubCityAtTopOfList + BOBBYR_NUM_DISPLAYED_CITIES; i++) {
    DrawTextToScreen((pDeliveryLocationStrings[i]),
                     ...
                     BOBBYR_DROPDOWN_FONT, BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                     LEFT_JUSTIFIED);
    usPosY += usFontHeight + 2;
  }
  ...
  DrawTextToScreen(city, BOBBYR_CITY_START_LOCATION_X + BOBBYR_CITY_NAME_OFFSET, usPosY + 5, 0,
                   BOBBYR_DROPDOWN_FONT, BOBBYR_FONT_BLACK, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
...
static void DisplayShippingLocationCity() {
  ...
  const wchar_t *dest = (gbSelectedCity == -1 ? BobbyROrderFormText[BOBBYR_SELECT_DEST]
                                              : (pDeliveryLocationStrings[gbSelectedCity]));
  DrawTextToScreen(dest, BOBBYR_CITY_START_LOCATION_X + BOBBYR_CITY_NAME_OFFSET,
                   BOBBYR_SHIPPING_LOC_AREA_T_Y + 3, 0, BOBBYR_DROPDOWN_FONT,
                   BOBBYR_ORDER_DROP_DOWN_SELEC_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  if (gbSelectedCity != -1) {
    SPrintMoney(sTemp, BobbyROrderLocations[gbSelectedCity].usOverNightExpress /
  ...
  if (gbSelectedCity != -1) {
    SPrintMoney(sTemp, BobbyROrderLocations[gbSelectedCity].us2DaysService /
                           GetWeightBasedOnMetricOption(1));
  }
  ...
  if (gbSelectedCity != -1) {
    SPrintMoney(sTemp, BobbyROrderLocations[gbSelectedCity].usStandardService /
                           GetWeightBasedOnMetricOption(1));
  }
...
static void RemovePurchasedItemsFromBobbyRayInventory() {
  FOR_EACH(BobbyRayPurchaseStruct const, i, BobbyRayPurchases) {
    ...
    // Is the item used?
    STORE_INVENTORY *inv =
        p.fUsed ? LaptopSaveInfo.BobbyRayUsedInventory : LaptopSaveInfo.BobbyRayInventory;
    uint8_t &qty = inv[p.usBobbyItemIndex].ubQtyOnHand;
...
static void SelectTitleLinkRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason) {
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    guiCurrentLaptopMode = LAPTOP_MODE_BOBBY_R;
  }
}
...
static uint32_t CalcPackageTotalWeight();
  ...
  /*	for(i=0; i<MAX_PURCHASE_AMOUNT; i++)
          {
                  ...
                  if( BobbyRayPurchases[ i ].ubNumberPurchased )
                  {
                          ...
                          uiTotalWeight += Item[ BobbyRayPurchases[ i
     ].usItemIndex ].ubWeight * BobbyRayPurchases[ i ].ubNumberPurchased;
                  }
          }
  */
...
static int8_t CalculateOrderDelay(uint8_t ubSelectedService) {
  int8_t bDaysAhead;
  ...
  if (gMercProfiles[SAL].bLife == 0) {
    ...
    // CJC comment: this seems really extreme!! maybe delay by 1 day randomly
    // but that's it!
...
static void AddNewBobbyRShipment(BobbyRayPurchaseStruct *pPurchaseStruct, uint8_t ubDeliveryLoc,
  ...
  if (gbSelectedCity == BR_DRASSEN || gbSelectedCity == BR_MEDUNA) {
    ...
    */
    ...
    /*
                    ...
                    for(i=0; i<MAX_PURCHASE_AMOUNT; i++)
                    {
                            ...
                            if( BobbyRayPurchases[ i ].ubNumberPurchased )
                            {
                                    //copy the purchases into the struct that
       ...
       ].ubNumberPurchases = ubCount;
                    LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[ cnt ].fActive
    ...
    */
    ...
    /*
       ...
       uiResetTimeSec, cnt); AddFutureDayStrategicEvent(
       EVENT_BOBBYRAY_PURCHASE, (8 + Random(4) ) * 60, cnt, bDaysAhead );
    */
  }
     ...
     LaptopSaveInfo.usNumberOfBobbyRayOrderUsed )
          {
                  LaptopSaveInfo.usNumberOfBobbyRayOrderItems++;
                  LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray =
     REALLOC(LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray, BobbyRayOrderStruct,
     LaptopSaveInfo.usNumberOfBobbyRayOrderItems); memset(
     &LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[
     LaptopSaveInfo.usNumberOfBobbyRayOrderItems - 1 ], 0, sizeof(
     BobbyRayOrderStruct ) );
          }
          ...
          {
                  //get an empty element in the array
                  if( !LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[ cnt
     ].fActive ) break;
          }
  */
  ...
  //	LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[cnt].BobbyRayPurchase[0] =
  // Temp;
  ...
  /*
          LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[cnt].BobbyRayPurchase[1]
     = Temp;


          LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[ cnt ].ubNumberPurchases
     = 2; LaptopSaveInfo.BobbyRayOrdersOnDeliveryArray[ cnt ].fActive = TRUE;
          LaptopSaveInfo.usNumberOfBobbyRayOrderUsed++;
  */
  bDaysAhead = CalculateOrderDelay(2) + 2;

  // add a random amount between so it arrives between 8:00 am and noon
  //	AddFutureDayStrategicEvent( EVENT_BOBBYRAY_PURCHASE, (8 + Random(4) ) *
  // 60, cnt, bDaysAhead );
...
static void DisplayPackageWeight() {
  ...
  // Display the weight
  //	swprintf(zTemp, L"%3.1f %ls", fWeight,
  // pMessageStrings[MSG_KILOGRAM_ABBREVIATION]);
  swprintf(zTemp, lengthof(zTemp), L"%3.1f %ls",
  ...
  DrawTextToScreen(zTemp, BOBBYR_PACKAXGE_WEIGHT_X + 3, BOBBYR_PACKAXGE_WEIGHT_Y + 4,
                   ...
                   BOBBYR_ORDER_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
}
...
void DrawBobbyROrderTitle() {
  BltVideoObject(FRAME_BUFFER, guiBobbyRayTitle, 0, BOBBYR_BOBBY_RAY_TITLE_X,
                 BOBBYR_BOBBY_RAY_TITLE_Y);
}
...
static void AddNewBobbyRShipment(BobbyRayPurchaseStruct *pPurchaseStruct, uint8_t ubDeliveryLoc,
  ...
  if (iFoundSpot == -1) {
    ...
    gpNewBobbyrShipments =
        REALLOC(gpNewBobbyrShipments, NewBobbyRayOrderStruct, giNumberOfNewBobbyRShipment);
  ...
  // get the apckage weight, if the weight is "below" the minimum, use the
  // minimum
...
void NewWayOfSavingBobbyRMailOrdersToSaveGameFile(HWFILE const hFile) {
  ...
  FileWrite(hFile, &giNumberOfNewBobbyRShipment, sizeof(int32_t));
...
void NewWayOfLoadingBobbyRMailOrdersToSaveGameFile(HWFILE const hFile) {
  ...
  FileRead(hFile, &giNumberOfNewBobbyRShipment, sizeof(int32_t));
...
TEST(BobbyRMailOrder, asserts) { EXPECT_EQ(sizeof(NewBobbyRayOrderStruct), 104); }
