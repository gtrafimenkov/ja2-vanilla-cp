...
void GameInitBobbyRGuns() { memset(&BobbyRayPurchases, 0, MAX_PURCHASE_AMOUNT); }
...
void DisplayBobbyRBrTitle() {
  ...
  FRAME_BUFFER->ShadowRect(BOBBYR_TO_ORDER_TEXT_X - 2, BOBBYR_TO_ORDER_TEXT_Y - 2,
...
void InitBobbyMenuBar() {
  ...
  guiBobbyRPreviousPage =
      MakeButton(guiBobbyRPreviousPageImage, BobbyRText[BOBBYR_GUNS_PREVIOUS_ITEMS],
                 BOBBYR_PREVIOUS_BUTTON_X, BOBBYR_PREVIOUS_BUTTON_Y, BtnBobbyRPreviousPageCallback);
  ...
  guiBobbyRNextPage =
      MakeButton(guiBobbyRNextPageImage, BobbyRText[BOBBYR_GUNS_MORE_ITEMS], BOBBYR_NEXT_BUTTON_X,
                 BOBBYR_NEXT_BUTTON_Y, BtnBobbyRNextPageCallback);
  ...
  FOR_EACHX(GUIButtonRef, i, guiBobbyRPageMenu,
            x += BOBBYR_CATALOGUE_BUTTON_GAP) {  // Catalogue buttons
  ...
  guiBobbyROrderForm =
      MakeButton(guiBobbyROrderFormImage, BobbyRText[BOBBYR_GUNS_ORDER_FORM], BOBBYR_ORDER_FORM_X,
                 BOBBYR_ORDER_FORM_Y, BtnBobbyROrderFormCallback);

  // Home button
  guiBobbyRHomeImage = LoadButtonImage(LAPTOPDIR "/cataloguebutton.sti", 0, 1);
  guiBobbyRHome = MakeButton(guiBobbyRHomeImage, BobbyRText[BOBBYR_GUNS_HOME], BOBBYR_HOME_BUTTON_X,
                             BOBBYR_HOME_BUTTON_Y, BtnBobbyRHomeButtonCallback);
}
...
void DisplayItemInfo(UINT32 uiItemClass) {
  ...
  DrawTextToScreen(sTemp, BOBBYR_ORDER_SUBTOTAL_X, BOBBYR_ORDER_SUBTOTAL_Y, 0,
                   BOBBYR_ORDER_TITLE_FONT, BOBBYR_ORDER_TEXT_COLOR, FONT_MCOLOR_BLACK,
  ...
  if (gfOnUsedPage) {
    DrawTextToScreen(BobbyRText[BOBBYR_GUNS_PERCENT_FUNCTIONAL], BOBBYR_PERCENT_FUNTCIONAL_X,
                     ...
                     BOBBYR_ORDER_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED | TEXT_SHADOWED);
  }
}
...
static UINT16 DisplayCostAndQty(UINT16 usPosY, UINT16 usIndex, UINT16 usFontHeight,
  ...
  // Display the cost
  DrawTextToScreen(BobbyRText[BOBBYR_GUNS_COST], BOBBYR_ITEM_COST_TEXT_X, usPosY,
                   BOBBYR_ITEM_COST_TEXT_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT,
                   BOBBYR_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  usPosY += usFontHeight + 2;

  SPrintMoney(sTemp, CalcBobbyRayCost(usIndex, usBobbyIndex, fUsed));
  DrawTextToScreen(sTemp, BOBBYR_ITEM_COST_NUM_X, usPosY, BOBBYR_ITEM_COST_TEXT_WIDTH,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;

  // Display Weight Number
  DrawTextToScreen(BobbyRText[BOBBYR_GUNS_WGHT], BOBBYR_ITEM_STOCK_TEXT_X, usPosY,
                   BOBBYR_ITEM_COST_TEXT_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT,
                   BOBBYR_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  usPosY += usFontHeight + 2;

  swprintf(sTemp, lengthof(sTemp), L"%3.2f %ls",
           GetWeightBasedOnMetricOption(Item[usIndex].ubWeight) / 10.0f, GetWeightUnitString());
  DrawTextToScreen(sTemp, BOBBYR_ITEM_STOCK_TEXT_X, usPosY, BOBBYR_ITEM_COST_TEXT_WIDTH,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;

  // Display the # In Stock
  DrawTextToScreen(BobbyRText[BOBBYR_GUNS_IN_STOCK], BOBBYR_ITEM_STOCK_TEXT_X, usPosY,
                   BOBBYR_ITEM_COST_TEXT_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT,
                   BOBBYR_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  usPosY += usFontHeight + 2;

  if (fUsed)
    swprintf(sTemp, lengthof(sTemp), L"% 4d",
...
static UINT16 DisplayRof(UINT16 usPosY, UINT16 usIndex, UINT16 usFontHeight) {
  ...
  DrawTextToScreen(BobbyRText[BOBBYR_GUNS_ROF], BOBBYR_ITEM_WEIGHT_TEXT_X, usPosY, 0,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);

  if (WeaponROF[usIndex] == -1)
    swprintf(sTemp, lengthof(sTemp), L"? %ls", pMessageStrings[MSG_RPM]);
  else
    swprintf(sTemp, lengthof(sTemp), L"%3d/%ls", WeaponROF[usIndex],
             pMessageStrings[MSG_MINUTE_ABBREVIATION]);

  DrawTextToScreen(sTemp, BOBBYR_ITEM_WEIGHT_NUM_X, usPosY, BOBBYR_ITEM_WEIGHT_NUM_WIDTH,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;
  return (usPosY);
...
static UINT16 DisplayRange(UINT16 usPosY, UINT16 usIndex, UINT16 usFontHeight) {
  ...
  DrawTextToScreen(BobbyRText[BOBBYR_GUNS_RANGE], BOBBYR_ITEM_WEIGHT_TEXT_X, usPosY, 0,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);
  swprintf(sTemp, lengthof(sTemp), L"%3d %ls", Weapon[usIndex].usRange,
...
static UINT16 DisplayMagazine(UINT16 usPosY, UINT16 usIndex, UINT16 usFontHeight) {
  ...
  DrawTextToScreen(BobbyRText[BOBBYR_GUNS_MAGAZINE], BOBBYR_ITEM_WEIGHT_TEXT_X, usPosY, 0,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_STATIC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);
  swprintf(sTemp, lengthof(sTemp), L"%3d %ls", Weapon[usIndex].ubMagSize,
...
static UINT16 DisplayCaliber(UINT16 usPosY, UINT16 usIndex, UINT16 usFontHeight) {
  ...
  // ammo or gun?
  AmmoKind const calibre = item->usItemClass == IC_AMMO ? Magazine[item->ubClassIndex].ubCalibre
                                                        : Weapon[item->ubClassIndex].ubCalibre;
  ...
  DrawTextToScreen(zTemp, BOBBYR_ITEM_WEIGHT_NUM_X, usPosY, BOBBYR_ITEM_WEIGHT_NUM_WIDTH,
                   BOBBYR_ITEM_DESC_TEXT_FONT, BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK,
                   RIGHT_JUSTIFIED);
...
static void DisplayItemNameAndInfo(UINT16 usPosY, UINT16 usIndex, UINT16 usBobbyIndex,
...

  {
    ...
    ReduceStringLength(sText, lengthof(sText), BOBBYR_GRID_PIC_WIDTH - 6,
                       BOBBYR_ITEM_NAME_TEXT_FONT);
    DrawTextToScreen(sText, BOBBYR_ITEM_NAME_X, usPosY + BOBBYR_ITEM_NAME_Y_OFFSET, 0,
  ...
  if (ubPurchaseNumber != BOBBY_RAY_NOT_PURCHASED) {
    DrawTextToScreen(BobbyRText[BOBBYR_GUNS_QTY_ON_ORDER], BOBBYR_ITEM_QTY_TEXT_X, usPosY,
    ...
    if (ubPurchaseNumber != BOBBY_RAY_NOT_PURCHASED) {
      ...
      DrawTextToScreen(sTemp, BOBBYR_ITEMS_BOUGHT_X, usPosY, 0, FONT14ARIAL,
                       BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
    }
  }

  // if it's a used item, display how damaged the item is
  if (fUsed) {
    swprintf(sTemp, lengthof(sTemp), L"*%3d%%",
             LaptopSaveInfo.BobbyRayUsedInventory[usBobbyIndex].ubItemQuality);
    DrawTextToScreen(sTemp, BOBBYR_ITEM_NAME_X - 2, usPosY - BOBBYR_ORDER_NUM_Y_OFFSET,
                     BOBBYR_ORDER_NUM_WIDTH, BOBBYR_ITEM_NAME_TEXT_FONT,
                     BOBBYR_ITEM_NAME_TEXT_COLOR, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  }
    ...
    if (ubPurchaseNumber == BOBBY_RAY_NOT_PURCHASED ||
      ...
      if (ubPurchaseNumber == BOBBY_RAY_NOT_PURCHASED) {
        ...
        if (ubPurchaseNumber != BOBBY_RAY_NOT_PURCHASED) {
          ...
          // display error popup because the player is trying to purchase more
          // thenn 10 items
          DoLapTopMessageBox(MSG_BOX_LAPTOP_DEFAULT, BobbyRText[BOBBYR_MORE_THEN_10_PURCHASES],
                             LAPTOP_SCREEN, MSG_BOX_FLAG_OK, NULL);
        }
      }
      // Else If the item is already purchased increment purchase amount.  Only
      // if ordering less then the max amount!
      else {
