...
static uint16_t gusSpeckDialogueX;
static uint16_t gusSpeckDialogueActualWidth;
...
static uint8_t gubCurrentMercVideoMode;
...
uint8_t gubArrivedFromMercSubSite = MERC_CAME_FROM_OTHER_PAGE;  // the merc is arriving from one of
                                                                // the merc sub pages
...
static void BtnAccountBoxButtonCallback(GUI_BUTTON *btn, int32_t reason);
static BUTTON_PICS *guiAccountBoxButtonImage;
static GUIButtonRef guiAccountBoxButton;

// File Box
static void BtnFileBoxButtonCallback(GUI_BUTTON *btn, int32_t reason);
static GUIButtonRef guiFileBoxButton;

// The 'X' to close the video conf window button
static void BtnXToCloseMercVideoButtonCallback(GUI_BUTTON *btn, int32_t reason);
...
void GameInitMercs() {
  ...
  gubMercArray[7] = LARRY_NORMAL;  // if changing this values, change in
                                   // GetMercIDFromMERCArray()
  gubMercArray[8] = LARRY_DRUNK;   // if changing this values, change in
                                   // GetMercIDFromMERCArray()
  gubMercArray[9] = NUMB;
...
static BOOLEAN ShouldSpeckSayAQuote();
...
void EnterMercs() {
  ...
  guiAccountBoxButton =
      QuickCreateButton(guiAccountBoxButtonImage, MERC_ACCOUNT_BUTTON_X, MERC_ACCOUNT_BUTTON_Y,
                        MSYS_PRIORITY_HIGH, BtnAccountBoxButtonCallback);
  guiAccountBoxButton->SetCursor(CURSOR_LAPTOP_SCREEN);

  guiFileBoxButton =
      QuickCreateButton(guiAccountBoxButtonImage, MERC_FILE_BUTTON_X, MERC_FILE_BUTTON_Y,
                        MSYS_PRIORITY_HIGH, BtnFileBoxButtonCallback);
  ...
  guiMercVideoFaceBackground =
      AddVideoSurface(MERC_VIDEO_FACE_WIDTH, MERC_VIDEO_FACE_HEIGHT, PIXEL_DEPTH);
...
static void RemoveSpeckPopupTextBox();
static void StopSpeckFromTalking();
...
void ExitMercs() {
  ...
  /*
     ...
     MERC_SITE_FIRST_VISIT ) LaptopSaveInfo.ubPlayerBeenToMercSiteStatus =
     MERC_SITE_SECOND_VISIT; else LaptopSaveInfo.ubPlayerBeenToMercSiteStatus =
     MERC_SITE_THIRD_OR_MORE_VISITS;
  */
...
static void DrawMercVideoBackGround();
static void HandleTalkingSpeck();
static void InitMercVideoFace();
...
void HandleMercs() {
  ...
  // if the page is redrawn, and we are in video conferencing, redraw the VC
  // backgrund graphic
...
void RenderMercs() {
  ...
  if (LaptopSaveInfo.gubPlayersMercAccountStatus == MERC_NO_ACCOUNT)
    DisplayWrappedString(MERC_ACCOUNT_BOX_TEXT_X, MERC_ACCOUNT_BOX_TEXT_Y,
                         ...
                         MercHomePageText[MERC_OPEN_ACCOUNT], FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
  else
    DisplayWrappedString(MERC_ACCOUNT_BOX_TEXT_X, MERC_ACCOUNT_BOX_TEXT_Y,
                         MERC_ACCOUNT_BOX_TEXT_WIDTH, 2, MERC_TEXT_FONT, MERC_TEXT_COLOR,
                         MercHomePageText[MERC_VIEW_ACCOUNT], FONT_MCOLOR_BLACK, RIGHT_JUSTIFIED);
...
void RemoveMercBackGround() { DeleteVideoObject(guiMercBackGround); }
...
static void ShouldAnyNewMercMercBecomeAvailable();
static BOOLEAN ShouldTheMercSiteServerGoDown();
...
void DailyUpdateOfMercSite(uint16_t usDate) {
  ...
  // If the merc site has never gone down, the number of MERC payment days is
  // above 'X', and the players account status is ok ( cant have the merc site
  // going down when the player owes him money, player may lose account that way
  // )
  if (ShouldTheMercSiteServerGoDown()) {
    ...
    // No lnger removing the bookmark, leave it up, and the user will go to the
    // broken link page Remove the book mark
...
static BOOLEAN HasLarryRelapsed();
...
static BOOLEAN StartSpeckTalking(uint16_t usQuoteNum) {
...
static BOOLEAN HandleSpeckTalking(BOOLEAN fReset) {
  ...
  SGPBox const DestRect = {MERC_VIDEO_FACE_X, MERC_VIDEO_FACE_Y, MERC_VIDEO_FACE_WIDTH,
                           MERC_VIDEO_FACE_HEIGHT};
...
static BOOLEAN DistortVideoMercImage(uint16_t usPosX, uint16_t usPosY, uint16_t usWidth,
                                     uint16_t usHeight);
static BOOLEAN PixelateVideoMercImage(BOOLEAN fUp, uint16_t usPosX, uint16_t usPosY,
                                      uint16_t usWidth, uint16_t usHeight);

static void HandleCurrentMercDistortion() {
...
static BOOLEAN PixelateVideoMercImage(BOOLEAN fUp, uint16_t usPosX, uint16_t usPosY,
  ...
  uint32_t const uiPitch = l.Pitch() / sizeof(uint16_t);
...
static void InitDestroyXToCloseVideoWindow(BOOLEAN fCreate) {
  ...
  if (fCreate && !fButtonCreated) {
    guiXToCloseMercVideoButton = QuickCreateButtonImg(
        LAPTOPDIR "/closebutton.sti", 0, 1, MERC_X_TO_CLOSE_VIDEO_X, MERC_X_TO_CLOSE_VIDEO_Y,
        MSYS_PRIORITY_HIGH, BtnXToCloseMercVideoButtonCallback);
...
static BOOLEAN DisplayMercVideoIntro(uint16_t usTimeTillFinish) {
  uint32_t uiCurTime = GetJA2Clock();
...
static BOOLEAN ShouldSpeckStartTalkingDueToActionOnSubPage();
  ...
  switch (gubCurrentMercVideoMode) {
    ...
    case MERC_VIDEO_VIDEO_MODE:
      ...
      if (gfDisplaySpeckTextBox && gGameSettings.fOptions[TOPTION_SUBTITLES]) {
        ...
        if (gsSpeckDialogueTextPopUp[0] != L'\0') {
          //					guiAccountBoxButton->Draw();
          //					guiAccountBoxButton->uiFlags |=
          // BUTTON_FORCE_UNDIRTY;

          if (g_merc_popup_box) {
            guiAccountBoxButton->Draw();
            guiAccountBoxButton->uiFlags |= BUTTON_FORCE_UNDIRTY;
            RenderMercPopUpBox(g_merc_popup_box, gusSpeckDialogueX, MERC_TEXT_BOX_POS_Y,
                               FRAME_BUFFER);
          }
        }
      }
    ...
    case MERC_VIDEO_EXIT_VIDEO_MODE:
      ...
      if (DisplayMercVideoIntro(MERC_EXIT_TIME)) {
        ...
        // else we are done the exit animation.  The area is not being invalidated
        // anymore
...
static void MercSiteSubTitleRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
...
void DisplayTextForSpeckVideoPopUp(const wchar_t *const pString) {
  ...
  g_merc_popup_box = PrepareMercPopupBox(g_merc_popup_box, BASIC_MERC_POPUP_BACKGROUND,
                                         BASIC_MERC_POPUP_BORDER, gsSpeckDialogueTextPopUp, 300, 0,
                                         0, 0, &gusSpeckDialogueActualWidth, &usActualHeight);
...
static BOOLEAN AreAnyOfTheNewMercsAvailable();
static uint32_t CalcMercDaysServed();
static BOOLEAN CanMercBeAvailableYet(uint8_t ubMercToCheck);
static uint8_t CountNumberOfMercMercsHired();
static uint8_t CountNumberOfMercMercsWhoAreDead();
static BOOLEAN IsAnyMercMercsDead();
static BOOLEAN IsAnyMercMercsHired();
static void MakeBiffAwayForCoupleOfDays();
...
static BOOLEAN GetSpeckConditionalOpening(BOOLEAN fJustEnteredScreen) {
  ...
  // if we are done the intro speech, or arrived from a sub page, get out of the
  // function
  ...
  // We have been here at least 2 times before, Check which quote we should use
    ...
    */
    // else if lots of MERC mercs are DEAD, and speck can say the quote ( dont
    // want him to continously say it )
    ...
    // else if the player owes speck a large sum of money, have speck say so
      ...
      // or if still trying to recruit ( the last recruit hasnt arrived and the
      // player has paid for some of his mercs )
      else if (ubRandom < 80 &&
  ...
  // If Speck has sent an email to the player, and the player still hasnt paid,
  // has speck complain about it.
  ...
  if (IsAnyMercMercsDead()) {
    ...
    // loop through all the mercs and see if any are dead and the quote is not
    // said
...
static BOOLEAN IsMercMercAvailable(uint8_t ubMercID);

static void HandlePlayerHiringMerc(uint8_t ubHiredMercID) {
  gusMercVideoSpeckSpeech = MERC_VIDEO_SPECK_SPEECH_NOT_TALKING;
...
static BOOLEAN IsMercMercAvailable(uint8_t ubMercID) {
  ...
  for (cnt = 0; cnt < LaptopSaveInfo.gubLastMercIndex; cnt++) {
    ...
    if (GetMercIDFromMERCArray(cnt) == ubMercID) {
      ...
      if (IsMercHireable(GetProfile(ubMercID))) return (TRUE);
    }
  }
...
static BOOLEAN ShouldSpeckSayAQuote() {
  // if we are entering from anywhere except a sub page, and we should say the
  // opening quote
  if (gfJustEnteredMercSite && gubArrivedFromMercSubSite == MERC_CAME_FROM_OTHER_PAGE) {
  ...
  if (gfJustHiredAMercMerc) {
    ...
    /*
                    //if the merc has something to say
                    if( gusMercVideoSpeckSpeech !=
       MERC_VIDEO_SPECK_SPEECH_NOT_TALKING ) return( TRUE ); else
                    {
...
static int16_t GetRandomQuoteThatHasBeenSaidTheLeast();
static void IncreaseMercRandomQuoteValue(uint8_t ubQuoteID, uint8_t ubValue);
...
static BOOLEAN CanMercQuoteBeSaid(uint32_t uiQuoteID);
...
static BOOLEAN HasLarryRelapsed() { return (CheckFact(FACT_LARRY_CHANGED, 0)); }
...
static BOOLEAN ShouldTheMercSiteServerGoDown() {
  ...
  // If the merc site has never gone down, the first new merc has shown ( which
  // shows the player is using the site ), and the players account status is ok
  // ( cant have the merc site going down when the player owes him money, player
  // may lose account that way )
...
void GetMercSiteBackOnline() {
  ...
  // Set a flag indicating that the server just went up ( so speck can make a
  // comment when the player next visits the site )
  LaptopSaveInfo.fFirstVisitSinceServerWentDown = TRUE;
}

static void DrawMercVideoBackGround() {
  BltVideoObject(FRAME_BUFFER, guiMercVideoPopupBackground, 0, MERC_VIDEO_BACKGROUND_X,
                 MERC_VIDEO_BACKGROUND_Y);

  // put the title on the window
  DrawTextToScreen(MercHomePageText[MERC_SPECK_COM], MERC_X_VIDEO_TITLE_X, MERC_X_VIDEO_TITLE_Y, 0,
                   MERC_VIDEO_TITLE_FONT, MERC_VIDEO_TITLE_COLOR, FONT_MCOLOR_BLACK,
                   LEFT_JUSTIFIED);
  InvalidateRegion(MERC_VIDEO_BACKGROUND_X, MERC_VIDEO_BACKGROUND_Y,
                   (MERC_VIDEO_BACKGROUND_X + MERC_VIDEO_BACKGROUND_WIDTH),
                   (MERC_VIDEO_BACKGROUND_Y + MERC_VIDEO_BACKGROUND_HEIGHT));
...
static BOOLEAN CanMercQuoteBeSaid(uint32_t uiQuoteID) {
  ...
  // switch onb the quote being said, if hes plugging a merc that has already
  // been hired, dont say it
  switch (uiQuoteID) {
...
static void ShouldAnyNewMercMercBecomeAvailable() {
  ...
  // for Larry
  //	if( GetMercIDFromMERCArray( LaptopSaveInfo.gubLastMercIndex ) ==
  // LARRY_NORMAL || 		GetMercIDFromMERCArray( LaptopSaveInfo.gubLastMercIndex ) ==
  // LARRY_DRUNK )
  {
...
static BOOLEAN CanMercBeAvailableYet(uint8_t ubMercToCheck) {
  ...
  if (c.ubMercArrayID <= LaptopSaveInfo.gubLastMercIndex) return (FALSE);

  // if the merc is already hired
  if (!IsMercHireable(GetProfile(GetMercIDFromMERCArray(c.ubMercArrayID)))) return (FALSE);

  // if player has paid enough money for the merc to be available, and the it is
  // after the current day
  if (c.usMoneyPaid <= LaptopSaveInfo.uiTotalMoneyPaidToSpeck && c.usDay <= GetWorldDay()) {
    return (TRUE);
  }
...
void CalcAproximateAmountPaidToSpeck() {
  ...
  // loop through all the mercs and tally up the amount speck should have been
  // paid
  for (i = 0; i < NUMBER_OF_MERCS; i++) {
    // get the id
    ubMercID = GetMercIDFromMERCArray(i);
