...
DataWriter::DataWriter(void *buf) : m_buf(buf), m_original(buf) {}
...
void DataWriter::move(int numBytes) { m_buf = ((char *)m_buf) + numBytes; }
...
/** Constructor.
 * @param buf Pointer to the buffer for writing data. */
DataReader::DataReader(const void *buf) : m_buf(buf), m_original(buf) {}
 ...
 * @param numChars Number of characters to read.
 * @param fixer Optional encoding corrector.  It is used for fixing incorrectly
 * encoded text. */
...
/** Move pointer to \a numBytes bytes forward. */
void DataReader::move(int numBytes) { m_buf = ((char *)m_buf) + numBytes; }
