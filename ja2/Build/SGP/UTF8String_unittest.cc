...
#if defined(_MSC_VER)
#pragma warning(disable : 4566) /* example: character represented by           \
                                   universal-character-name '\u03BF' cannot be \
...
static std::vector<uint16_t> u16v_testEnglish(u16a_testEnglish,
                                              u16a_testEnglish + ARR_SIZE(u16a_testEnglish));
static std::vector<uint32_t> u32v_testEnglish(u32a_testEnglish,
                                              u32a_testEnglish + ARR_SIZE(u32a_testEnglish));
...
static std::vector<uint16_t> u16v_testRussian(u16a_testRussian,
                                              u16a_testRussian + ARR_SIZE(u16a_testRussian));
static std::vector<uint32_t> u32v_testRussian(u32a_testRussian,
                                              u32a_testRussian + ARR_SIZE(u32a_testRussian));
...
static std::vector<uint16_t> u16v_testChinese(u16a_testChinese,
                                              u16a_testChinese + ARR_SIZE(u16a_testChinese));
static std::vector<uint32_t> u32v_testChinese(u32a_testChinese,
                                              u32a_testChinese + ARR_SIZE(u32a_testChinese));

// Greek word "test" (δοκιμή)
static uint8_t u8a_testGreek[] = {0xce, 0xb4, 0xce, 0xbf, 0xce, 0xba, 0xce,
                                  0xb9, 0xce, 0xbc, 0xce, 0xae, 0};
static uint16_t u16a_testGreek[] = {0x03b4, 0x03bf, 0x03ba, 0x03b9, 0x03bc, 0x03ae, 0};
static uint32_t u32a_testGreek[] = {0x000003b4, 0x000003bf, 0x000003ba, 0x000003b9,
                                    0x000003bc, 0x000003ae, 0};
static std::vector<uint16_t> u16v_testGreek(u16a_testGreek,
                                            u16a_testGreek + ARR_SIZE(u16a_testGreek));
static std::vector<uint32_t> u32v_testGreek(u32a_testGreek,
                                            u32a_testGreek + ARR_SIZE(u32a_testGreek));
...
TEST(UTF8StringTest, GarbageInExceptionOut) {
  const uint8_t garbage8[] = {0x3e, 0x3e, 0x59, 0xfb, 0x2d, 0x68, 0xf7, 0xfb,
                              0x77, 0x15, 0x1c, 0x22, 0xc9, 0xc6, 0x77, 0xb8};
