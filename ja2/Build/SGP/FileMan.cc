...
void InitializeFileManager(const char *exeFolder) {
  ...
  // Create another directory and set is as the current directory for the
  // process Temporary files will be created in this directory.
  // ----------------------------------------------------------------------------
...
void FileDelete(char const *const path) {
  ...
  switch (errno) {
    ...
    case EACCES:
      if ((chmod(path, S_IREAD | S_IWRITE) == 0 && unlink(path) == 0) || errno == ENOENT) {
        return;
      }
      break;
#endif
...
const std::string &FileMan::getExeFolderPath() { return s_exeFolderPath; }
...
BOOLEAN GetFileManFileTime(const HWFILE f, SGP_FILETIME *const pCreationTime,
                           SGP_FILETIME *const pLastAccessedTime,
                           SGP_FILETIME *const pLastWriteTime) {
#if 1  // XXX TODO
  UNIMPLEMENTED;
  return FALSE;
#else
 ...
 * case-sensitive file-systems. * * @return List of paths (dir + filename). */
...
std::vector<std::string> FindAllFilesInDir(const std::string &dirPath, bool sortResults) {
  std::vector<std::string> paths;
  for (const auto &entry : std::filesystem::directory_iterator(dirPath)) {
    if (std::filesystem::is_regular_file(entry)) {
