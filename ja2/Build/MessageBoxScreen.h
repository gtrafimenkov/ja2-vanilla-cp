...
enum MessageBoxFlags {
  ...
  MSG_BOX_FLAG_GENERICCONTRACT,        // displays contract buttoin + 2 user-defined
                                       // text buttons
 ...
 * ReturnCallback Callback for return. Can be NULL. Returns any above return
 * value pCenteringRect Rect to center in. Can be NULL */
void DoMessageBox(MessageBoxStyleID, wchar_t const *zString, ScreenID uiExitScreen, MessageBoxFlags,
                  MSGBOX_CALLBACK ReturnCallback, SGPBox const *centering_rect);
void DoScreenIndependantMessageBox(const wchar_t *zString, MessageBoxFlags,
                                   MSGBOX_CALLBACK ReturnCallback);
void DoLowerScreenIndependantMessageBox(const wchar_t *zString, MessageBoxFlags,
                                        MSGBOX_CALLBACK ReturnCallback);

// wrappers for other screens
void DoMapMessageBoxWithRect(MessageBoxStyleID, wchar_t const *zString, ScreenID uiExitScreen,
                             MessageBoxFlags, MSGBOX_CALLBACK ReturnCallback,
                             SGPBox const *centering_rect);
void DoOptionsMessageBoxWithRect(wchar_t const *zString, ScreenID uiExitScreen, MessageBoxFlags,
                                 MSGBOX_CALLBACK ReturnCallback, SGPBox const *centering_rect);
