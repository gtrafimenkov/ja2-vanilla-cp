...
- Always be aware that German strings should be of equal or shorter length than
the English equivalent. I know that this is difficult to do on many occasions
due to the nature of the German language when compared to English.  By doing so,
...
look for Topware problems and questions.  This is a more efficient method than
...
************
        ...
        As described above, the "!!!" notation should be used by Topware to ask
...
static StrPointer s_rus_AmmoCaliber[AmmoCaliber_SIZE] = {
    ...
    L"12 кал",    L"CAWS",    L"5,45мм", L"5,56мм",  L"7,62мм НАТО",
...
static StrPointer s_rus_BobbyRayAmmoCaliber[BobbyRayAmmoCaliber_SIZE] = {
    L"0",      L",38 кал", L"9мм",     L",45 кал", L",357 кал", L"12 кал", L"CAWS",   L"5,45мм",
    L"5,56мм", L"7,62 НА", L"7,62 ВД", L"4,7мм",   L"5.7мм",    L"Монстр", L"Ракета",
    L"",  // дротик
};

static StrPointer s_rus_WeaponType[WeaponType_SIZE] = {
    L"Другое",          L"Пистолет", L"Автом. пистолет", L"Пистолет-пулемет", L"Винтовка",
    L"Снайп. винтовка", L"Автомат",  L"Легкий пулемет",  L"Дробовик"};

static StrPointer s_rus_TeamTurnString[TeamTurnString_SIZE] = {
    L"Ход игрока",  // player's turn
    L"Ход противника", L"Ход существ", L"Ход ополчения", L"Ход жителей"
    // planning turn
};
...
static StrPointer s_rus_Message[Message_SIZE] = {
    ...
    L"Этот предмет можно присоединить к другим предметам, поместив его в одну "
    L"из ячеек приспособлений.",
    L"Этот предмет можно присоединить к другим предметам, поместив его в одну "
    L"из ячеек приспособлений. (Однако данные предметы несовместимы.)",
    L"В секторе еще остались враги!",
    L"%ls должен получить еще %ls",  /// TRNSL_ASSUME all doctors who can get
                                     /// money are male
    ...
    L"Чтобы воспользоваться вертолетом Всадника, нужно дать бойцам ЗАДАНИЕ "
    L"сесть в ТРАНСПОРТ.",
...
static const wchar_t *s_rus_g_towns_locative[g_towns_locative_SIZE] = {
    L"",      L"Омерте", L"Драссене",
    L"Альма",  // TODO fill in correct forms
    L"Грам",  L"Тикса",  L"Камбрия",  L"Сан Мона", L"Эстони",
    L"Орта",  L"Балайм", L"Медуна",   L"Читзена",
};
...
static const wchar_t *s_rus_pLongAssignmentStrings[pLongAssignmentStrings_SIZE] = {
    L"Отряд 1",   L"Отряд 2",      L"Отряд 3",   L"Отряд 4",   L"Отряд 5",  L"Отряд 6",
    L"Отряд 7",   L"Отряд 8",      L"Отряд 9",   L"Отряд 10",  L"Отряд 11", L"Отряд 12",
    L"Отряд 13",  L"Отряд 14",     L"Отряд 15",  L"Отряд 16",  L"Отряд 17", L"Отряд 18",
    L"Отряд 19",  L"Отряд 20",     L"На службе", L"Доктор",    L"Пациент",  L"Транспорт",
    L"В пути",    L"Ремонт",       L"Практика",  L"Ополчение", L"Тренер",   L"Ученик",
    L"Труп",      L"Без сознания", L"В плену",
    L"Госпиталь",  // patient in a hospital
    L"Пуст",       // Vehicle is empty
};
...
static const wchar_t *s_rus_pAttributeMenuStrings[pAttributeMenuStrings_SIZE] = {
    L"Сила",     L"Ловкость", L"Проворность", L"Здоровье", L"Меткость",
...
static const wchar_t *s_rus_pPersonnelScreenStrings[pPersonnelScreenStrings_SIZE] = {
    L"Медицинский депозит:",  // amount of medical deposit put down on the
                              // merc
...
static const wchar_t *s_rus_gzMercSkillText[gzMercSkillText_SIZE] = {
    L"Нет навыков",  L"Взлом",     L"Рукопашная",         L"Электроника", L"Ночной бой",
    L"Броски",       L"Учитель",   L"Тяжелое вооружение", L"Автоматы",    L"Скрытность",
    L"Амбидекстрия", L"Вор",       L"Боевые искусства",   L"Бой с ножом", L"Стрельба с крыш",
    L"Маскировка",   L"(Эксперт)",
};
...
static const wchar_t *s_rus_pDoorTrapStrings[pDoorTrapStrings_SIZE] = {
    L"нет ловушки", L"мина", L"электроловушка", L"сирена", L"сигнализация"};
...
static const wchar_t *s_rus_pMapScreenMouseRegionHelpText[pMapScreenMouseRegionHelpText_SIZE] = {
    L"Выбрать наемника",           L"Дать задание", L"Проложить маршрут",
    L"Управление контрактом (|C)", L"Убрать бойца", L"Спать",
};

// volumes of noises

static const wchar_t *s_rus_pNoiseVolStr[pNoiseVolStr_SIZE] = {L"ТИХИЙ", L"ЧЕТКИЙ", L"ГРОМКИЙ",
                                                               L"ОГЛУШИТЕЛЬНЫЙ"};
...
static const wchar_t *s_rus_pDirectionStr[pDirectionStr_SIZE] = {
    L"с СЕВ-ВОСТОКА", L"с ВОСТОКА", L"с ЮГО-ВОСТОКА", L"с ЮГА",
    L"с ЮГО-ЗАПАДА",  L"с ЗАПАДА",  L"с СЕВ-ЗАПАДА",  L"с СЕВЕРА"};
...
static const wchar_t *s_rus_pLandTypeStrings[pLandTypeStrings_SIZE] = {
    ...
    L"Роща, дорога", L"Берег", L"Горы, дорога", L"Побережье, дорога", L"Пустыня, дорога",
    L"Болото, дорога", L"Леса, ПВО", L"Пустыня, ПВО", L"Тропики, ПВО", L"Медуна, ПВО",

    // These are descriptions for special sectors
    L"Госпиталь Камбрии", L"Аэропорт Драссена", L"Аэропорт Медуны", L"База ПВО",
...
static const wchar_t *s_rus_gpStrategicString[gpStrategicString_SIZE] = {
    L"%ls обнаружены в секторе %c%d и вот-вот прибудет еще "
    L"один отряд.",  // STR_DETECTED_SINGULAR
    L"%ls обнаружены в секторе %c%d и вот-вот прибудут еще "
    L"отряды.",                                        // STR_DETECTED_PLURAL
    ...
    L"Отступить",  // The retreat button
                   // //STR_AR_RETREAT_BUTTON
    L"Готово",     // The done button
                   // //STR_AR_DONE_BUTTON

    // The headers are for the autoresolve type (MUST BE
    // UPPERCASE)

    L"ОБОРОНА",       // STR_AR_DEFEND_HEADER
    L"АТАКА",         // STR_AR_ATTACK_HEADER
    L"СТОЛКНОВЕНИЕ",  // STR_AR_ENCOUNTER_HEADER
    L"Сектор",        // The Sector A9 part of the header
                      // //STR_AR_SECTOR_HEADER

    // The battle ending conditions

    L"ПОБЕДА!",       // STR_AR_OVER_VICTORY
    L"ПОРАЖЕНИЕ!",    // STR_AR_OVER_DEFEAT  ///TRNSL_CHECK_INGAME
                      // LENGTH
    L"СДАЧА!",        // STR_AR_OVER_SURRENDERED
    L"В ПЛЕНУ!",      // STR_AR_OVER_CAPTURED
    L"ОТСТУПЛЕНИЕ!",  // STR_AR_OVER_RETREATED

    // These are the labels for the different types of enemies
    // we fight in autoresolve.
    ...
    // Goes on the three buttons in the prebattle interface. The
    // Auto resolve button represents a system that automatically
    // resolves the combat for the player without having to do
    // anything. These strings must be short (two lines -- 6-8
    // chars per line)
    ...
    // The different headers(titles) for the prebattle
    // interface.
    ...
    // Various single words for direct translation.  The
    // Civilians represent the civilian militia occupying the
    // sector being attacked.  Limited to 9-10 chars
    ...
    L"Кошки",
    L"Сектор",
    L"Никого",  // If there are no uninvolved mercs in this
                // fight.
    ...
    // Strings used for various popup message boxes.  Can be as
    // long as desired.

    L"Не очень-то хорошее место. Туда не пройти. Выберите "
    L"другую позицию.",
    L"Разместите своих наемников в выделенной области.",

    // These entries are for button popup help text for the
    // prebattle interface.  All popup help text supports the use
    // of \n to denote new line.  Do not use spaces before or
    // after the \n.
    ...
    L"Отступить всем отрядам на исходные позиции (|R)",  // multiple
                                                         // groups
                                                         // with
                                                         // same
                                                         // previous
    ...
    // 1st %d refers to the number of civilians eaten by
    // monsters,  %c%d is the sector -- ex:  A9 Note:  the
    // minimum number of civilians eaten will be two.
    L"Существа напали на гражданских, убито %d в секторе %ls.",
    //%ls is the sector location -- ex:  A9: Omerta
    L"Враги атакуют ваш отряд секторе %ls. Никто из наемников "
    L"не может сражаться!",
    //%ls is the sector location -- ex:  A9: Omerta
    L"Существа атакуют ваш отряд в секторе %ls. Никто из "
    L"наемников не может сражаться!",
...
static const wchar_t *s_rus_gMoneyStatsDesc[gMoneyStatsDesc_SIZE] = {
    ...
    L"Отделить",
    L"сумму:",  // the amount he wants to separate from the overall balance to
                // get two piles of money
...
static const wchar_t *s_rus_gzMoneyAmounts[gzMoneyAmounts_SIZE] = {L"1000$", L"100$",     L"10$",
                                                                   L"OK",    L"Отделить", L"Взять"};
...
static StrPointer s_rus_zTalkMenuStrings[zTalkMenuStrings_SIZE] = {
    ...
    L"Угрожать",  // approach threateningly - talk now, or I'll blow your face
                  // off
    L"Дать", L"Нанять"};
...
static const wchar_t *s_rus_pVehicleStrings[pVehicleStrings_SIZE] = {
    ...
    L"Минивэн",   L"Джип", L"Танк", L"Вертолет",
};
...
static StrPointer s_rus_TacticalStr[TacticalStr_SIZE] = {
    L"Воздушный налет", L"Оказать первую помощь?",
    ...
    L"К замку присоединена %ls.",  /// TRNSL_ASSUME all traps are female gender
                                   /// words
    L"Тут нет замка.", L"Замок без ловушки.",
    // The %ls is a merc name
    L"%ls не имеет нужного ключа.", L"Замок без ловушки.", L"Заперто.", L"ДВЕРЬ", L"С ЛОВУШКОЙ",
    L"ЗАПЕРТАЯ", L"НЕЗАПЕРТАЯ", L"СЛОМАНАЯ", L"Тут есть выключатель. Нажать?",
    L"Обезвредить ловушку?", L"Еще предметы...",

    // In the next 2 strings, %ls is an item name

    L"%ls теперь на земле.", L"Предмет \'%ls\' получает %ls.",

    // In the next 2 strings, %ls is a name

    L"%ls получил(а) всю сумму.", L"%ls ожидает от вас уплаты еще %d.",
    L"Выберите частоту детонатора:",     // in this case, frequency refers to a
                                         // radio signal
    L"Количество ходов перед взрывом:",  // how much time, in turns, until the
                                         // bomb blows
    L"Выберите частоту подрыва:",        // in this case, frequency refers to a radio
                                         // signal
    L"Разрядить ловушку?", L"Убрать флажок?", L"Установить флажок?", L"Завершение хода",

    // In the next string, %ls is a name. Stance refers to way they are
    // standing.

    L"%ls на вашей стороне! Атаковать?", L"Ох, транспорт не может менять положения.",
    L"Робот не может менять положения.",

    // In the next 3 strings, %ls is a name

    L"%ls не может здесь поменять положение.", L"%ls не может быть перевязан.",
    L"%ls не нуждается в перевязке.", L"Туда идти нельзя.",
    L"Команда набрана. Нет места для новобранца.",  // there's no room for a
    ...
    L"Бросить туда? Не получится.",  // Merc can't throw to the destination he
                                     // selected

    // These are different buttons that the player can turn on and off.

    L"Режим скрытности (|Z)", L"Окно карты (|M)", L"Завершить ход (|D)", L"Говорить",
    L"Запретить отзывы", L"Подняться (|P|g|U|p)", L"Уровень курсора (|T|a|b)", L"Залезть/Слезть",
    L"Опуститься (|P|g|D|n)", L"Осмотреть (|C|t|r|l)", L"Предыдущий боец",
    L"Следующий боец (|S|p|a|c|e)", L"Настройки (|O)", L"Стрелять очередью (|B)",
    L"Смотреть/Повернуться (|L)", L"Здоровье: %d/%d\nЭнергия.: %d/%d\nНастрой: %ls",
    L"Чего?",     // this means "what?"
    L"Продолж.",  // an abbrieviation for "Continued"
    L"%ls будет говорить.", L"%ls будет молчать.", L"Состояние: %d/%d\nТопливо: %d/%d",
    L"Выйти из машины", L"Поменять отряд (|S|h|i|f|t |S|p|a|c|e)", L"Вести машину",
    L"Н/П",  // this is an acronym for "Not Applicable."
    L"Применить (Рукопашная)", L"Применить (Оружие)", L"Применить (Нож)", L"Применить (Взрывчатка)",
    L"Применить (Аптечка)", L"(Ловит)",
    L"(Перезарядка)",  /// TRNSL_CHECK_INGAME LENGTH
    L"(Дать)",
    L"Сработала %ls.",  // The %ls here is a string from pDoorTrapStrings  ASSUME
                        // all traps are female gender
    L"%ls прибыл(a).", L"%ls: нет очков действия.", L"%ls: наемник недоступен.",
    L"%ls: успешная перевязка.", L"%ls: нет бинтов.", L"Враг в секторе!",
    L"Нет врагов в поле зрения.", L"Не хватает очков действия.",
    L"Никто не использует дистанционное управление.", L"Обойма опустела!", L"СОЛДАТ", L"РЕПТИОН",
    L"ОПОЛЧЕНЕЦ", L"ЖИТЕЛЬ", L"Выход из сектора", L"OK", L"ОТМЕНА", L"Выбранный боец",
    L"Весь отряд", L"Идти в сектор", L"Идти на карту", L"Этот сектор нельзя покинуть здесь.",
    L"%ls слишком далеко.", L"Вершины деревьев скрыты", L"Вершины деревьев отображены",
    L"ВОРОНА",  // Crow, as in the large black bird
    L"ШЕЯ", L"ГОЛОВА", L"ГРУДЬ", L"НОГИ", L"Сказать королеве то, что она хочет знать?",
    L"Отпечатки пальцев получены", L"Отпечатки неверны. Оружие заблокировано", L"Цель захвачена",
    L"Путь блокирован",
    L"Положить/Взять деньги",  // Help text over the $ button on the Single Merc
                               // Panel
    L"Некого лечить.",
    ...
    // money (e.g. $200)
    L"Заплатить %ls?", L"Согласиться на бесплатное лечение?", L"Согласны женить Дэррела?", L"Ключи",
    L"С эскортируемыми этого сделать нельзя.", L"Пощадить Кротта?",
    L"Цель вне зоны эффективного огня.", L"Шахтер",
    L"Транспорт передвигается только между секторами.", L"Автоперевязку сделать сейчас нельзя",
    L"%ls не может пройти, путь блокирован.",
    L"Здесь томятся ваши бойцы, захваченные армией Дейдраны!", L"Попадание в замок",
    L"Замок разрушен", L"Кто-то еще пытается воспользоваться этой дверью.",
    L"Состояние: %d/%d\nТопливо: %d/%d",
...
static const wchar_t *s_rus_pExitingSectorHelpText[pExitingSectorHelpText_SIZE] = {
    ...
    L"противником прежде чем идти в другие сектора.",
    ...
    L"%ls не может покинуть сектор один - он сопровождает %ls.",      // male
                                                                      // singular
    L"%ls не может покинуть сектор одна - она сопровождает %ls.",     // female
                                                                      // singular
    L"%ls не может покинуть сектор один - он сопровождает группу.",   // male
                                                                      // plural
    L"%ls не может покинуть сектор одна - она сопровождает группу.",  // female
                                                                      // plural
    ...
    L"%ls не может покинуть этот сектор без сопровождения. Для этого ваш "
    L"отряд должен быть рядом.",
};

static const wchar_t *s_rus_pRepairStrings[pRepairStrings_SIZE] = {
    L"Вещи",    // tell merc to repair items in inventory
    L"ПВО",     // tell merc to repair SAM site - SAM is an acronym for Surface to
                // Air Missile
    L"Отмена",  // cancel this menu
...
static const wchar_t *s_rus_sStatGainStrings[sStatGainStrings_SIZE] = {
    L"здоровья.",         L"подвижности.",     L"проворности.", L"мудрости.", L"медицины.",
    L"умения взрывника.", L"умения механика.", L"меткости.",    L"опыта.",    L"силы.",
    L"лидерства.",
};

static const wchar_t *s_rus_pHelicopterEtaStrings[pHelicopterEtaStrings_SIZE] = {
    L"Общее расстояние:  ",  // total distance for helicopter to
                             // travel
    ...
    L"У вертолета мало топлива, придется сесть на территории "
    L"врага!",  // warning that the sector the helicopter is going to
                // use for refueling is under enemy control ->
    L"Пассажиры: ",
...
static const wchar_t s_rus_sMapLevelString[] = L"Подуровень ";  // what level below the ground is
                                                                // the player viewing in mapscreen

static const wchar_t s_rus_gsLoyalString[] =
    L"Отношение %d%%";  // the loyalty rating of a town ie : Loyal 53%

// error message for when player is trying to give a merc a travel order while
// he's underground.
static const wchar_t s_rus_gsUndergroundString[] =
...
static const wchar_t *s_rus_sFacilitiesStrings[sFacilitiesStrings_SIZE] = {
    L"Нет",        L"Госпиталь", L"Заводы", L"Тюрьма", L"База армии", L"Аэропорт",
...
static const wchar_t *s_rus_pwMiscSectorStrings[pwMiscSectorStrings_SIZE] = {
    L"Силы врага", L"Сектор", L"Количество предметов", L"?", L"Сектор захвачен", L"Да", L"Нет",
};
...
static const wchar_t *s_rus_pUpdateMercStrings[pUpdateMercStrings_SIZE] = {
    ...
    L"Контракты закончились:",     // this pop up came up due to a merc contract
                                   // ending
    L"Бойцы выполнили задание:",   // this pop up....due to more than one merc
                                   // finishing assignments
    L"Бойцы снова в строю:",       // this pop up ....due to more than one merc waking
                                   // up and returing to work
    L"Бойцы идут спать:",          // this pop up ....due to more than one merc being
                                   // tired and going to sleep
    L"Контракты скоро кончатся:",  // this pop up came up due to a merc contract
                                   // ending
};
...
static const wchar_t *s_rus_pMapScreenBorderButtonHelpText[pMapScreenBorderButtonHelpText_SIZE] = {
    ...
    L"Показать отряды и врагов (|T)", L"Показать воздушное пространство (|A)",
    L"Показать предметы (|I)",        L"Показать ополчение и врагов (|Z)",
};
...
static const wchar_t *s_rus_pMapScreenBottomFastHelp[pMapScreenBottomFastHelp_SIZE] = {
    ...
    L"(|P|g|U|p)",  // previous message in scrollable list
    L"Следующее сообщение (|D|o|w|n)\nСледующая страница "
    L"(|P|g|D|n)",                        // next message in the scrollable list
    L"Пауза/Снять с паузы (|S|p|a|c|e)",  // start/stop time
                                          // compression
};

static const wchar_t s_rus_pMapScreenBottomText[] =
...
static const wchar_t *s_rus_pSenderNameList[pSenderNameList_SIZE] =  // GOLD and BUKA use slightly
                                                                     // different names
    {
        L"Энрико",
        L"Псих Про Инк.",
...
static const wchar_t *
    s_rusGold_pSenderNameList[pSenderNameList_SIZE] =  // GOLD and BUKA use slightly different names
    {
        L"Энрико",
        L"Псих Про Инк.",
        L"Помощь",
...
static const wchar_t s_rus_pFinanceTitle[] = L"Бухгалтер Плюс";  // the name we made up for the
...
static const wchar_t *s_rus_pTransactionText[pTransactionText_SIZE] = {
    ...
    L"Профилирование в IMP",       // IMP is the acronym for International Mercenary
                                   // Profiling
    L"Застрахован(а) %ls",
    ...
    L"%ls: оплата покупок.",        // is used for the Shop keeper interface.  The
                                    // dealers name will be appended to the end of the
                                    // string.
...
static const wchar_t *s_rus_pSkyriderText[pSkyriderText_SIZE] = {
    ...
    L"У Всадника нет пассажиров. Если вы собирались перевозить бойцов - "
    L"назначьте им ЗАДАНИЕ ТРАНСПОРТ."};

// strings for different levels of merc morale

static const wchar_t *s_rus_pMoralStrings[pMoralStrings_SIZE] = {
    L"Боевой", L"Бодрый", L"Норма", L"Уныние", L"Паника", L"Разгром",
};
...
static const wchar_t *s_rus_pMapScreenStatusStrings[pMapScreenStatusStrings_SIZE] = {
    L"Здоровье",  L"Энергия", L"Настрой",
    L"Состояние",  // the condition of the current vehicle (its "health")
    L"Бензин",     // the fuel level of the current vehicle (its "energy")
};

static const wchar_t
    *s_rus_pMapScreenPrevNextCharButtonHelpText[pMapScreenPrevNextCharButtonHelpText_SIZE] = {
        L"Предыдущий наемник (|L|e|f|t)",   // previous merc in the list
        L"Следующий наемник (|R|i|g|h|t)",  // next merc in the list
};

static const wchar_t s_rus_pEtaString[] =
    L"ОВП:";  // eta is an acronym for Estimated Time of Arrival

static const wchar_t *s_rus_pTrashItemText[pTrashItemText_SIZE] = {
    L"Вы потеряете этот предмет навсегда. Выбросить?",  // do you want to
                                                        // continue and lose the
                                                        // item forever
    L"Похоже, это действительно ВАЖНАЯ вещь. Вы ТОЧНО уверены, что хотите ее "
    L"выбросить?",  // does the user REALLY want to trash this item
};
...
static const wchar_t *s_rus_pMapErrorString[pMapErrorString_SIZE] = {
    ...
    L"У вас в команде пока никого нет",                        // you have no members, can't do
                                                               // anything
    L"Боец не может выполнить приказ.",                        // merc can't comply with your order
                                                               // 6-10
    L"%ls нуждается в сопровождении. Назначьте его в отряд.",  // merc can't move
                                                               // unescorted ..
                                                               // for a male
    ...
    L"Ополчение, которое находится вне города, нельзя переместить в другой "
    L"сектор.",
    // 31-35
    ...
    // 46-50
    L"Один или более ДРУГИХ бойцов сейчас не могут перемещаться.",  /// TRNSL_CHECK
                                                                    /// MEANING
...
static const wchar_t *s_rus_pMapPlotStrings[pMapPlotStrings_SIZE] = {
    L"Кликните на секторе еще раз чтобы направить туда отряд, или щелкните по "
    L"другому сектору для прокладки маршрута.",
...
static const wchar_t
    *s_rus_pMiscMapScreenMouseRegionHelpText[pMiscMapScreenMouseRegionHelpText_SIZE] = {
        L"Просмотр инвентаря (|E|n|t|e|r)",
        L"Выкинуть предмет",
        L"Выйти из инвентаря (|E|n|t|e|r)",
};

static const wchar_t s_rus_str_he_leaves_where_drop_equipment[] =
    L"%ls должен оставить снаряжение в (%ls) или потом, в %ls (%ls), перед "
    L"вылетом из Арулько?";
static const wchar_t s_rus_str_she_leaves_where_drop_equipment[] =
    L"%ls должна оставить свое снаряжение в (%ls) или потом, в %ls (%ls), "
    L"перед вылетом из Арулько?";
static const wchar_t s_rus_str_he_leaves_drops_equipment[] =
    L"%ls собрался уйти, оставив свое снаряжение в %ls.";
static const wchar_t s_rus_str_she_leaves_drops_equipment[] =
...
static const wchar_t *s_rus_pImpPopUpStrings[pImpPopUpStrings_SIZE] = {
    ...
    L"Изучение вашего счета показало, что у вас нет средств для оплаты "
    L"профилирования.",
    L"Сейчас вы не можете это выбрать.",
    L"Чтобы закончить анализ, нужно иметь место еще хотя бы для одного члена "
    L"команды.",
...
static const wchar_t *s_rus_pImpButtonText[pImpButtonText_SIZE] = {
    ...
    L"НЕТ, ХОЧУ НАЧАТЬ СНАЧАЛА.",  // no, I want to start over the profiling
                                   // process
    L"ДА.",
    L"НЕТ",
...
static const wchar_t *s_rus_pFilesSenderList[pFilesSenderList_SIZE] = {
    L"Отчет разведки",  // the recon report sent to the player. Recon is an
                        // abbreviation for reconissance
    L"Перехват #1",     // first intercept file .. Intercept is the title of the
...
static const wchar_t *s_rus_pHistoryStrings[pHistoryStrings_SIZE] = {
    ...
    // 16-20
    L"завершен.",
    L"%ls: состоялся разговор с начальником шахты.",  // talked to head miner of
                                                      // town
    L"%ls освобожден(а).",
    L"Читы включены.",
...
static const wchar_t *s_rus_pLaptopIcons[pLaptopIcons_SIZE] = {
    L"Почта",       L"Сеть", L"Финансы", L"Команда", L"Журнал", L"Файлы", L"Выключить",
...
static const wchar_t *s_rus_pBookMarkStrings[pBookMarkStrings_SIZE] = {
    L"А.I.M.", L"Бобби Рэй", L"I.M.P.", L"М.Е.R.С.", L"Похороны", L"Цветы", L"Страховка", L"Отмена",
};
...
static const wchar_t s_rus_pWebTitle[] = L"сир-ФЕР 4.0";  // our name for the version of the
...
static const wchar_t *s_rus_pLaptopTitles[pLaptopTitles_SIZE] = {
    L"Почтовый ящик", L"Просмотр файлов", L"Команда", L"Бухгалтер Плюс", L"Журнал",
};
...
static const wchar_t *s_rus_pPersonnelDepartedStateStrings[pPersonnelDepartedStateStrings_SIZE] = {
    ...
    L"Убит в бою",
    L"Уволен",
    L"Замужем",  /// TRNSL ASSUME only female merc can depart from team
...
static const wchar_t *s_rus_pPersonelTeamStrings[pPersonelTeamStrings_SIZE] = {
    L"В команде",     L"Убывшие",        L"Гонорар в день:", L"Макс. гонорар:",
    L"Мин. гонорар:", L"Погибли в бою:", L"Уволены:",        L"Другое:",
};

static const wchar_t
    *s_rus_pPersonnelCurrentTeamStatsStrings[pPersonnelCurrentTeamStatsStrings_SIZE] = {
        L"Минимум",
        L"Среднее",
        L"Максимум",
};
...
static const wchar_t *s_rus_pPersonnelTeamStatsStrings[pPersonnelTeamStatsStrings_SIZE] = {
    L"ЗДОР", L"ПРВ", L"ЛОВ", L"СИЛ", L"ЛДР", L"МДР", L"ОПЫТ", L"МТК", L"МЕХ", L"ВЗРВ", L"МЕД",
};
...
static const wchar_t *s_rus_InsInfoText[InsInfoText_SIZE] = {L"Назад", L"Далее"};
...
static const wchar_t *s_rus_MercAccountText[MercAccountText_SIZE] = {
    ...
    L"Плата",
    L"Всего:",
    L"Подтверждаете выплату %ls?",  // the %ls is a string that contains the
...
static const wchar_t *s_rus_MercHomePageText[MercHomePageText_SIZE] = {
    ...
    L"Спек Т. Клайн, основатель", L"Открытие счета", L"Просмотр счета", L"Личные дела",
...
static const wchar_t *s_rus_sFuneralString[sFuneralString_SIZE] = {
    L"Похоронное агенство Макгилликати: скорбим вместе с семьями усопших с "
    L"1983 года.",
    L"Директор по похоронам и бывший наемник А.I.М Мюррэй \"Папаша\" "
    L"Макгилликати - специалист по части похорон.",
    L"Всю жизнь Папашу сопровождали смерть и утраты, поэтому он, как никто, "
    L"познал их тяжесть.",
    L"Агенство Макгилликати предлагает широкий спектр похоронных услуг - от "
    L"жилетки, в которую можно поплакать, до восстановления сильно "
    L"поврежденных останков.",
    L"Доверьтесь похоронному агенству Макгилликати, и ваши родственники почиют "
    L"в мире.",

    // Text for the various links available at the bottom of the page
    L"ДОСТАВКА ЦВЕТОВ", L"КОЛЛЕКЦИЯ УРН И ГРОБОВ", L"УСЛУГИ ПО КРЕМАЦИИ",
    L"ПОМОЩЬ В ПРОВЕДЕНИИ ПОХОРОН", L"ПОХОРОННЫЕ РИТУАЛЫ",

    // The text that comes up when you click on any of the links ( except for
    // send flowers ).
    L"К сожалению, наш сайт не закончен, в связи с утратой в семье. Мы "
    L"постараемся продолжить работу после прочтения завещания и выплат долгов "
    L"умершего.",
...
static const wchar_t *s_rus_sFloristText[sFloristText_SIZE] = {
    ...
    L"\"Авиадоставка по всему миру\"", L"1-555-SCENT-ME", L"333 Др. Носич, Семябург, КА США 90210",
    L"http://www.scent-me.com",

    // detail of the florist page

    L"Работаем быстро и эффективно!",
    L"Гарантированная доставка в течение одного дня в любую точку земного "
    L"шара. Есть ограничения.",
    L"Самые низкие в мире цены!",
    L"Покажите нам рекламу подобных услуг, которые стоят дешевле и получите "
    L"букет роз бесплатно.",
    L"Летающая Флора, Фауна & Цветы с 1981.",
    L"Наши летчики, в прошлом военные бомбардировщики, сбросят ваш букет в "
    L"радиусе 10 км от цели. В любое время! Всегда!",
    L"Позвольте нам воплотить ваши цветочные фантазии в жизнь.",
    L"Брюс, наш всемирно известный флорист, собственноручно соберет вам букет "
    L"свежайших цветов из наших оранжерей.",
...
static const wchar_t *s_rus_sFloristGalleryText[sFloristGalleryText_SIZE] = {
    ...
    L"Примечание: можно заказать увядший или сломанный букет за дополнительные "
    L"$10.",
...
static const wchar_t *s_rus_sFloristCards[sFloristCards_SIZE] = {L"Кликните для выбора", L"Назад"};
...
static const wchar_t *s_rus_BobbyROrderFormText[BobbyROrderFormText_SIZE] = {
    ...
    L"Цена с доставкой",     // The grand total of all item totals + the shipping
                             // and handling
    L"Место доставки",
    ...
    L"У вас нет на это средств.",            // 20	// A popup message that to warn of not
                                             // enough money
    L"<ПУСТО>",                              // Gets displayed when there is no valid city selected
    L"Место доставки заказа %ls. Уверены?",  // A popup that asks if the city
                                             // selected is the correct one
    L"Вес товаров**",                        // Displays the weight of the package
    L"** минимальный вес заказа",            // Disclaimer states that there is a minimum
                                             // weight for the package
    L"Заказы",
};
...
static const wchar_t *s_rus_BobbyRText[BobbyRText_SIZE] = {
    ...
    L"Кликните на предмет. Хотите больше - кликайте еще. Убрать предмет из "
    L"заказа - правый клик. Закончив выбор покупок, заполните бланк заказа.",
    ...
    L"* процент исправности",  // if the item is damaged, displays the percent
                               // function of the item

    // Popup that tells the player that they can only order 10 items at a time

    L"Дорогие клиенты! Онлайновая форма позволяет заказать не более 10 позиций "
    L"за раз. Если вы хотите заказать больше, (а мы надемся, что так и есть), "
    L"заполните еще один бланк и примите наши извинения.",

    // A popup that tells the user that they are trying to order more items then
    // the store has in stock

    L"Извините. Этот товар закончился на складе. Попробуйте заказать его "
    L"позже.",
 ...
 * ammo. */
static const wchar_t s_rus_str_bobbyr_guns_num_guns_that_use_ammo[] =
    L"Количество единиц оружия у вашей команды, использующих этот боеприпас: "
    L"%d";
...
static const wchar_t *s_rus_AimSortText[AimSortText_SIZE] = {
    ...
    L"Просмотреть фото наемников", L"Просмотреть характеристики наемников",
...
static const wchar_t *s_rus_AimPolicyText[AimPolicyText_SIZE] = {
    ...
    L"Назад", L"Главная", L"Правила", L"Дальше", L"Не согласен", L"Согласен"};
...
static const wchar_t *s_rus_CharacterInfo[CharacterInfo_SIZE] = {
    ...
    L"Дополнительная информация",    // Title for the additional info for the
                                     // merc's bio
    L"Действующие наемники A.I.M.",  // Title of the page
    L"Цена снаряжения:",             // Displays the optional gear cost
    L"медицинский депозит",          // If the merc required a medical deposit, this is
                                     // displayed
};
...
static const wchar_t *s_rus_VideoConfercingText[VideoConfercingText_SIZE] = {
    ...
    L"Без снаряжения", L"Со снаряжением",
    ...
    L"Видеоконференция, абонент", L"Соединение. . .",

    L"с депозитом"  // Displays if you are hiring the merc with the medical
                    // deposit
};
...
static const wchar_t *s_rus_AimHistoryText[AimHistoryText_SIZE] = {
    ...
    L"Назад", L"Главная", L"Галерея", L"Дальше"};
...
static const wchar_t *s_rus_AimScreenText[AimScreenText_SIZE] = {
    ...
    L"A.I.M. и логотип A.I.M. - зарегистрированные во многих странах торговые "
    L"марки.",
...
static const wchar_t *s_rus_AimBottomMenuText[AimBottomMenuText_SIZE] = {
    ...
    L"Главная", L"Наемники",
    L"Галерея",  //$$
    L"Правила", L"История",  L"Ссылки"};
...
static const wchar_t *s_rus_SKI_Text[SKI_SIZE] = {
    ...
    L"ОБЩАЯ ЦЕНА",        // The total cost of the the items in the Dealer inventory
                          // area
    L"ОБЩАЯ ЦЕНА",        // The total value of items player wishes to sell
    L"ОЦЕНКА",            // Button text for dealer to evaluate items the player wants to
                          // sell
    ...
    L"ЦЕНА РЕМОНТА",      // The amount the dealer will charge to repair the merc's
                          // goods
    L"1 ЧАС",             // SINGULAR! The text underneath the inventory slot when an item
                          // is given to the dealer to be repaired
    L"%d ЧАСОВ",          // PLURAL!   The text underneath the inventory slot when an
                          // item is given to the dealer to be repaired
    L"ОТРЕМОНТИРОВАНО",   // Text appearing over an item that has just been
                          // repaired by a NPC repairman dealer
    L"Вам уже некуда класть вещи.",  // Message box that tells the user there is
                                     // no more room to put there stuff
    L"%d МИНУТ",                     // The text underneath the inventory slot when an item is given
                                     // to the dealer to be repaired
    L"Бросить на землю",             // hint about dropping item on the ground from
                                     // ShopKeeper screen
};
...
static const wchar_t *s_rus_zSaveLoadText[zSaveLoadText_SIZE] = {
    ...
    L"Версия сохраненной игры отличается от текущей. Скорее всего, игра "
    L"загрузится нормально. Загрузить?",
    L"Файлы сохраненной игры возможно испорчены. Стереть все?",

    L"Попытка загрузки старой версии сохранения. Обновить автоматически и "
    L"загрузить?",
    L"Попытка загрузки старой версии сохранения. Обновить автоматически и "
    L"загрузить?",
    ...
    L"У вас кончается дисковое пространство. Осталось всего %d Mбайт. Нужно "
    L"как минимум %d свободных Mбайт.",

    L"Сохранение...",  // When saving a game, a message box with this string
...
static const wchar_t *s_rus_zMarksMapScreenText[zMarksMapScreenText_SIZE] = {
    ...
    // stuff for contracts
    L"Вы можете платить по контракту, но у вас нет денег на оплату страхования "
    L"жизни.",
    L"%ls застрахован(а) на случай гибели. Для продления страховки надо "
    L"доплатить %ls за %d дней. Согласны?",
...
static const wchar_t *s_rus_pMilitiaConfirmStrings[pMilitiaConfirmStrings_SIZE] = {
    L"Тренировка отряда ополчения будет стоить $",  // telling player how
                                                    // much it will cost
    L"Оплатить расходы?",                           // asking player if they wish to pay the amount
                                                    // requested
    L"У вас нет такой суммы.",                      // telling the player they can't afford to
                                                    // train this town
    L"Продолжить тренировку ополчения в %ls (%ls %d)?",  // continue training
                                                         // this town?
    L"Стоимость $",                                      // the cost in dollars to train militia
    L"(Д/Н)",                                            // abbreviated yes/no
    L"Тренировка ополчения в %d секторах будет стоить $ %d. %ls",  // cost to
                                                                   // train
                                                                   // sveral
                                                                   // sectors
                                                                   // at once
    L"У вас нет $%d на тренировку здесь ополчения.",
    L"%ls: нужно %d%% отношения к вам, чтобы продолжить тренировку "
    L"ополчения.",
...
static const wchar_t s_rus_gzCopyrightText[] =
    ...
    L"Распространение на территории стран СНГ компания БУКА.";
...
static const wchar_t *s_rus_zOptionsScreenHelpText[zOptionsToggleText_SIZE] = {
    ...
    L"Включите эту опцию, чтобы субтитры переходили к следующей фразе по "
    L"нажатию клавиши.",
    ...
    // Never move my mouse
    L"Отключите эту опцию, чтобы курсор автоматически перемещался к "
    L"всплывающим окнам.",

    // Old selection method
    L"Включает выбор персонажей, как в Jagged Alliance 1 (иначе выбор "
    L"происходит наоборот).",

    // Show movement path
    L"Включает отображение пути бойца в реальном времени (или отключите эту "
    L"опцию и используйте SHIFT).",

    // show misses
    L"Включите эту опцию, чтобы видеть, куда попадают пули при \"промахе\".",

    // Sleep/Wake notification
    L"При включенной опции для движения в реальном времени необходим "
    L"дополнительный клик мышью.",

    // Display the enemy indicator
    L"Включает отображение оповещений о том, что ваш боец засыпает или "
    L"просыпается.",

    // Use the metric system
    L"Когда опция включена, игра использует метрическую систему мер, иначе - "
    L"британскую.",

    // Merc Lighted movement
    L"Когда опция включена, боец \"освещает\" землю при движении. Отключите "
    L"для улучшения производительности.",

    // Smart cursor
    L"Когда опция включена, бойцы выбираются, если курсор рядом с ними.",

    // snap cursor to the door
    L"Когда опция включена, курсор перескакивает на дверь, если он рядом с "
    L"ней.",
...
static const wchar_t *s_rus_gzGIOScreenText[gzGIOScreenText_SIZE] = {L"ИГРОВЫЕ НАСТРОЙКИ",
                                                                     L"Стиль игры",
                                                                     L"Реалистичный",
                                                                     L"Фантастичный",
                                                                     L"Количество оружия",
...
static const wchar_t *s_rus_pDeliveryLocationStrings[pDeliveryLocationStrings_SIZE] = {
    ...
    L"Драссен",       // The main place in JA2 that you can receive items.  The
    ...
    L"Медуна",        // Meduna -- the other airport in JA2 that you can receive
                      // items.
    L"Метавира",      // The island of Metavira was the fictional location used
                      // by JA1
    ...
    L"Токио",         // Tokyo, Japan
    L"Ванкувер",      // Vancouver, British Columbia, Canada (west coast near US
                      // border)
};
...
static const wchar_t *s_rus_pMessageStrings[pMessageStrings_SIZE] = {
    ...
    L"в/м",               // Abbreviation for Rounds per minute -- the potential # of bullets
                          // fired in a minute.
    L"мин",               // Abbreviation for minute.
    L"м",                 // One character abbreviation for meter (metric distance measurement
                          // unit).
    ...
    L"Главная страница",  // Home as in homepage on the internet. Used on M.I.S.
                          // insurance web page
    L"USD",               // Abbreviation to US dollars
    L"н/п",               // Lowercase acronym for not applicable.
    L"Тем временем",      // Meanwhile
    L"%ls теперь в секторе %ls%ls",  // Name/Squad has arrived in sector A9.
                                     // Order must not change without notifying
                                     // SirTech
    L"Версия",
    ...
    L"У вас кончается дисковое пространство. У вас осталось %lsМБ свободных, а "
    L"для игры требуется %lsMБ.",
    L"%ls ловит %ls.",                    //'Merc name' has caught 'item' -- let SirTech know if
                                          // name comes after item.
    ...
    // When firing heavier weapons in close quarters, you may not have enough
    // room to do so.
    ...
    // Simple text indications that appear in the game, when the merc can do one
    // of these things.
    L"Уронить",
    L"Бросить",
    L"Передать",

    L"%ls передан %ls.",          //"Item" passed to "merc".  Please try to keep the item
                                  //%ls before the merc %ls, otherwise, must notify
                                  // SirTech.
    L"Нельзя передать %ls %ls.",  // pass "item" to "merc".  Same instructions as
                                  // above.

    // A list of attachments appear after the items.  Ex:  Kevlar vest ( Ceramic
    // Plate 'Attached )'
    L" присоединен)",
    ...
    // Wireframes are shown through buildings to reveal doors and windows that
    // can't otherwise be seen in an isometric engine.  You can toggle this mode
    // freely in the game.
    L"Отображение каркасов зданий включено",
    L"Отображение каркасов зданий выключено",

    // These are used in the cheat modes for changing levels in the game.  Going
    // from a basement level to an upper level, etc.
    L"Нельзя подняться с этого уровня...",
    L"Ниже уровней нет...",
    L"Входим в подвальный уровень %d...",
    L"Уходим из подвала...",

    L".",  // used in the shop keeper inteface to mark the ownership of the item
           // eg Red's gun
    ...
    L"Курсор обычный.",
    L"Отряд %d действует.",
    L"У вас нет денег, чтобы ежедневно выплачивать %ls %ls",  // first %ls is the
                                                              // mercs name, the
                                                              // seconds is a
                                                              // string containing
                                                              // the salary
    ...
    L"Игра была сохранена под именем SaveGame99.sav. Для загрузки переименуйте "
    L"ее в SaveGame01-SaveGame10.",
    L"%ls пъет %ls",
    L"Груз снаряжения прибыл в Драссен.",
    L"%ls прибудет в указанное место высадки (сектор %ls) в день %d, примерно "
    L"в %ls.",  // first %ls is mercs name, next is the sector location and name
                // where they will be arriving in, lastely is the day an the time
                // of arrival
    L"Журнал событий обновлен.",
};

static const wchar_t *s_rus_ItemPickupHelpPopup[ItemPickupHelpPopup_SIZE] = {
    L"Взять выделенные", L"Листать вверх", L"Выделить все", L"Листать вниз", L"Отмена"};
...
static const wchar_t *s_rus_pMilitiaButtonsHelpText[pMilitiaButtonsHelpText_SIZE] = {
    L"Взять новобранцев (Правый клик)/Распределить (Левый "
    L"клик)",  // button help text informing player they can pick
               // up or drop militia with this button
    L"Взять солдат (Правый клик)/Распределить (Левый клик)",
    L"Взять ветеранов (Правый клик)/Распределить (Левый клик)",
    L"Поровну распределить доступное ополчение по всем "
    L"секторам",
};

// to inform the player to hire some mercs to get things going
static const wchar_t s_rus_pMapScreenJustStartedHelpText[] =
    L"Сходите на сайт АIM и наймите наемников (*Подсказка* через ноутбук)";

static const wchar_t s_rus_pAntiHackerString[] =
...
static const wchar_t *s_rus_pNewNoiseStr[pNewNoiseStr_SIZE] = {
    L"%ls слышит %ls звук, идущий %ls.",  L"%ls слышит %ls звук ДВИЖЕНИЯ, идущий %ls.",
    L"%ls слышит %ls СКРИП, идущий %ls.", L"%ls слышит %ls ПЛЕСК, идущий %ls.",
    L"%ls слышит %ls УДАР, идущий %ls.",  //$$
    L"%ls слышит %ls ВЗРЫВ %ls.",         L"%ls слышит %ls КРИК %ls.",
    L"%ls слышит %ls УДАР %ls.",          L"%ls слышит %ls УДАР %ls.",
    L"%ls слышит %ls ЗВОН, идущий %ls.",  L"%ls слышит %ls ГРОХОТ, идущий %ls.",
};
...
static const wchar_t *s_rus_zGioDifConfirmText[zGioDifConfirmText_SIZE] = {
    L"Вы выбрали сложность НОВОБРАНЕЦ. Это подходит для новичков в Jagged "
    L"Alliance, для новичков в жанре стратегий вообще, или для тех, кто желает "
    L"сократить битвы в игре. Ваш выбор скажется на игре в целом, так что "
    L"выбирайте с умом. Уверены, что хотите играть новобранцем?",
    L"Вы выбрали сложность СОЛДАТ. Это подходит для игроков, уже знакомых с "
    L"играми Jagged Alliance или с подобными им. Ваш выбор скажется на игре в "
    L"целом, так что выбирайте с умом. Уверены, что хотите играть солдатом?",
    L"Вы выбрали сложность ВЕТЕРАН. Мы Вас предупредили. Нечего на нас пенять, "
    L"если вас доставят назад в цинковом гробу. Ваш выбор скажется на игре в "
    L"целом, так что выбирайте с умом. Уверены, что хотите играть ветераном?",
};
...
static const wchar_t *s_rus_gzLateLocalizedString[gzLateLocalizedString_SIZE] = {
    ...
    // This message comes up if you have pending bombs waiting to
    // explode in tactical.
    ...
    L"У %lsа кончилось топливо и он остается в %c%d.",  /// TRNSL
                                                        /// Assumes
                                                        /// that all
                                                        /// vehicle
                                                        /// names are
    ...
    // the following two strings are combined with the pNewNoise[]
    // strings above to report noises heard above or below the merc
    L"сверху", L"снизу",

    // The following strings are used in autoresolve for
    // autobandaging related feedback.
    ...
    L"Ваши бойцы не нуждаются в первой помощи.", L"Перевязать наемников автоматически",
    ...
    // In autoresolve if there were 5 mercs fighting 8 enemies the
    // text would be "5 vs. 8" "vs." is the abbreviation of versus.
    L"%d против %d",

    L"В %lsе нет места!",  //(ex "The ice cream truck is
                           // full")///TRNSL_ASSUME vehicle gender is
                           // male

    L"%ls нуждается не в перевязке и первой помощи, а в "
    L"основательном медицинском обследовании и/или отдыхе.",
    ...
    // 26
    // Name has gone psycho -- when the game forces the player into
    // burstmode (certain unstable characters)
    L"%ls впадает в безумие!",

    // 27-28
    // Messages why a player can't time compress.
    L"Нельзя ускорять время, поскольку у вас есть бойцы в секторе "
    L"%ls.",  //
    L"Нельзя ускорять время, когда бойцы находятся в шахтах с "
    L"монстрами.",  //

    // 29-31 singular versions @@@2 elite to veteran
    L"Один ополченец-новобранец стал ветераном.", L"Один ополченец-новобранец стал солдатом.",
    L"Один ополченец-солдат стал ветераном.",

    // 32-34
    L"%ls ничего не говорит.", L"Выбраться на поверхность?", L"(Отряд %d)",

    // 35
    // Ex: "Red has repaired Scope's MP5K".  Careful to maintain the
    // proper order (Red before Scope, Scope before MP5K)
    ...
    L"%ls спотыкается и падает", L"Эту вещь нельзя взять отсюда.",

    // 39
    L"Ваши бойцы не в состоянии сражаться. Ополчение ведет бой с "
    L"существами само.",

    // 40-43
    //%ls is the name of merc.
    L"%ls истратил(а) все медикаменты!", L"%ls не обладает навыками, чтобы лечить кого-либо!",
    L"%ls без инструментов!", L"%ls не обладает навыками, чтобы ремонтировать что-либо!",

    // 44-45
    L"Время ремонта", L"%ls не видит этого человека.",

    // 46-48
    L"%ls ломает удлинитель ствола!", L"Не разрешается больше %d тренеров ополчения на сектор.",
    L"Вы уверены?",

    // 49-50
    L"Ускорение времени", L"Бак теперь заправлен.",

    // 51-52 Fast help text in mapscreen.
    L"Продолжить ускорение времени (|S|p|a|c|e)", L"Прекратить ускорение времени (|E|s|c)",

    // 53-54 "Magic has unjammed the Glock 18" or "Magic has
    // unjammed Raven's H&K G11"
    L"%ls расклинил %ls", L"%ls расклинил для %ls %ls",

    // 55
    L"Невозможно сжимать время при просмотре предметов сектора.",

    // 56
    // Displayed with the version information when cheats are
    // enabled.
...
static const wchar_t s_rus_str_arrival_rerouted[] =
    L"Место высадки новоприбывших наемников перенесено в сектор %ls; ранее "
    L"заданный сектор %ls захвачен врагом.";
...
static const wchar_t *s_rus_str_stat_list[str_stat_list_SIZE] = {
    s_rus_str_stat_health,    s_rus_str_stat_agility,      s_rus_str_stat_dexterity,
    s_rus_str_stat_strength,  s_rus_str_stat_leadership,   s_rus_str_stat_wisdom,
    s_rus_str_stat_exp_level, s_rus_str_stat_marksmanship, s_rus_str_stat_mechanical,
    s_rus_str_stat_explosive, s_rus_str_stat_medical};

static const wchar_t *s_rus_str_aim_sort_list[str_aim_sort_list_SIZE] = {
    s_rus_str_aim_sort_price,     s_rus_str_aim_sort_experience, s_rus_str_aim_sort_marksmanship,
    s_rus_str_aim_sort_medical,   s_rus_str_aim_sort_explosives, s_rus_str_aim_sort_mechanical,
    s_rus_str_aim_sort_ascending, s_rus_str_aim_sort_descending,
};
...
static const wchar_t *s_rus_zNewTacticalMessages[] = {
    ...
    L"Зона обзора",
    L"Это игра для человека со СТАЛЬНОЙ ВОЛЕЙ. Нельзя сохраняться, когда рядом "
    L"враги!",
    L"(Нельзя сохранять игру в бою)",
};

static const wchar_t s_rus_str_iron_man_mode_warning[] =
    L"Вы выбрали режим СТАЛЬНОЙ ВОЛИ. Эта настройка сильно усложняет игру, так "
    L"как вы не сможете сохраняться, когда в секторе есть враги. ЖЕЛЕЗНАЯ ВОЛЯ "
    L"действует в течение всей игры. Уверены, что хотите включить этот режим?";
