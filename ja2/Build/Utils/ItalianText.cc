...
really long string...", must fit on a single line no matter how long the string
...
static StrPointer s_it_AmmoCaliber[AmmoCaliber_SIZE] = {
...
// WP is Warsaw Pact
// cal is an abbreviation for calibre
static StrPointer s_it_BobbyRayAmmoCaliber[BobbyRayAmmoCaliber_SIZE] = {
    L"0",      L"cal .38", L"9 mm",    L"cal .45",    L"cal .357",   L"cal fisso 12",
...
static StrPointer s_it_Message[Message_SIZE] = {
    ...
    L"L'oggetto puntato dal vostro cursore può essere combinato ad alcuni "
    L"oggetti ponendolo in uno dei quattro slot predisposti.",
    L"L'oggetto puntato dal vostro cursore può essere combinato ad alcuni "
    L"oggetti ponendolo in uno dei quattro slot predisposti. (Comunque, in "
    L"questo caso, l'oggetto non è compatibile.)",
    ...
    L"Per viaggiare sull'elicottero di Skyrider, dovrete innanzitutto "
    L"ASSEGNARE mercenari al VEICOLO/ELICOTTERO.",
...
static const wchar_t *s_it_gpStrategicString[gpStrategicString_SIZE] = {
    L"%ls sono stati individuati nel settore %c%d e un'altra squadra sta per "
    L"arrivare.",  // STR_DETECTED_SINGULAR
    L"%ls sono stati individuati nel settore %c%d e un'altra squadra sta per "
    L"arrivare.",                                 // STR_DETECTED_PLURAL
    ...
    L"Ritirarsi",  // The retreat button
    ...
    L"settore",     // The Sector A9 part of the header
    ...
    // These are the labels for the different types of enemies we fight in
    // autoresolve.
    ...
    // Goes on the three buttons in the prebattle interface.  The Auto resolve
    // button represents a system that automatically resolves the combat for the
    // player without having to do anything. These strings must be short (two
    // lines -- 6-8 chars per line)
    ...
    // Various single words for direct translation.  The Civilians represent the
    // civilian militia occupying the sector being attacked.  Limited to 9-10
    // chars
    ...
    // The help text for the four buttons.  Use \n to denote new line (just like
    // enter).

    L"|Mostra chiaramente tutte le postazioni dei mercenari,\ne vi permette di "
    L"rimetterli in gioco manualmente.",
    ...
    L"Cliccate su questo pulsante quando avrete\nscelto le postazioni dei "
    L"vostri mercenari. (|I|n|v|i|o)",
    ...
    L"Non sembra così bello qui. È inacessibile. Provate con una diversa "
    L"postazione.",
    L"Posizionate i vostri mercenari nella sezione illuminata della mappa.",

    // These entries are for button popup help text for the prebattle interface.
    // All popup help text supports the use of \n to denote new line.  Do not use
    // spaces before or after the \n.
    L"|Automaticamente svolge i combattimenti al vostro posto\nsenza caricare "
    L"la mappa.",
    L"Non è possibile utilizzare l'opzione di risoluzione automatica "
    ...
    L"|Rimandate tutti i gruppi ai loro settori precedenti.",  // multiple groups
                                                               // with same
                                                               // previous sector
    ...
    // 1st %d refers to the number of civilians eaten by monsters,  %c%d is the
    // sector -- ex:  A9 Note:  the minimum number of civilians eaten will be
    // two.
    L"Le creature attaccano e uccidono %d civili nel settore %ls.",
    //%ls is the sector location -- ex:  A9: Omerta
    L"I nemici attaccano i vostri mercenari nel settore %ls. Nessuno dei "
    L"vostri mercenari è in grado di combattere!",
    //%ls is the sector location -- ex:  A9: Omerta
    L"I nemici attaccano i vostri mercenari nel settore %ls. Nessuno dei "
    L"vostri mercenari è in grado di combattere!",
...
static const wchar_t *s_it_gMoneyStatsDesc[gMoneyStatsDesc_SIZE] = {
    ...
    L"Ammontare",
    L"Da separare:",  // the amount he wants to separate from the overall balance
...
static StrPointer s_it_zTalkMenuStrings[zTalkMenuStrings_SIZE] = {
    ...
    L"Minaccioso",      // approach threateningly - talk now, or I'll blow your face
                        // off
    L"Dai",
    L"Recluta",
};
...
static StrPointer s_it_TacticalStr[TacticalStr_SIZE] = {
    ...
    L"La serratura ha %ls", L"Non ci sono serrature", L"La serratura non presenta trappole",
    // The %ls is a merc name
    L"%ls non ha la chiave giusta", L"La serratura non presenta trappole", L"Serrato", L"",
    L"TRAPPOLE", L"SERRATO", L"APERTO", L"FRACASSATO",
    L"C'è un interruttore qui. Lo volete attivare?", L"Disattivate le trappole?", L"Più...",

    // In the next 2 strings, %ls is an item name

    L"Il %ls è stato posizionato sul terreno.", L"Il %ls è stato dato a %ls.",

    // In the next 2 strings, %ls is a name

    L"%ls è stato pagato completamente.", L"Bisogna ancora dare %d a %ls.",
    L"Scegliete la frequenza di detonazione:",         // in this case, frequency refers
                                                       // to a radio signal
    L"Quante volte finché la bomba non esploderà:",    // how much time, in turns,
                                                       // until the bomb blows
    L"Stabilite la frequenza remota di detonazione:",  // in this case, frequency
    ...
    L"Siete sicuri di volere attaccare %ls ?", L"Ah, i veicoli non possono cambiare posizione.",
    L"Il robot non può cambiare posizione.",

    // In the next 3 strings, %ls is a name

    L"%ls non può cambiare posizione.", L"%ls non sono ricorsi al pronto soccorso qui.",
    L"%ls non ha bisogno del pronto soccorso.", L"Non può muoversi là.",
    L"La vostra squadra è al completo. Non c'è spazio per una recluta.",  // there's
                                                                          // no
                                                                          // room
                                                                          // for a
                                                                          // recruit
    ...
    // In the next string, the first %ls is a name and the second %ls is an
    // amount of money (including $ sign)

    L"Il salario di %ls ammonta a %ls per giorno?",

    // This line is used repeatedly to ask player if they wish to participate in
    // a boxing match.
    ...
    L"Audio on per %ls.", L"Audio off per %ls.", L"Salute: %d/%d\nCarburante: %d/%d",
    ...
    L"Usa (Kit medico)", L"Afferra", L"Ricarica", L"Dai", L"%ls è partito.", L"%ls è arrivato.",
    L"%ls ha esaurito i Punti Azione.", L"%ls non è disponibile.", L"%ls è tutto bendato.",
    L"%ls non è provvisto di bende.", L"Nemico nel settore!", L"Nessun nemico in vista.",
    ...
    L"%ls è troppo lontano.", L"Rimuovi le fronde degli alberi", L"Mostra le fronde degli alberi",
    ...
    L"Deposita/Preleva soldi",  // Help text over the $ button on the Single Merc
                                // Panel
    ...
    // In the following message, '%ls' would be replaced with a quantity of
    // money (e.g. $200)
    L"Sei d'accordo a pagare %ls?", L"Accetti il trattamento medico gratuito?",
    ...
    //	L"I tuoi mercenari, che erano stati catturati dall'esercito di
    // Deidranna, sono stati imprigionati qui!",
    L"I mercenari catturati dall'esercito di Deidranna, sono stati "
    L"imprigionati qui!",
...
static const wchar_t *s_it_pExitingSectorHelpText[pExitingSectorHelpText_SIZE] = {
    ...
    L"Se selezionato, sarete automaticamente posti nella schermata della "
    L"mappa\nvisto che i vostri mercenari avranno bisogno di tempo per "
    L"viaggiare.",

    // If you attempt to leave a sector when you have multiple squads in a
    // hostile sector.
    L"Questo settore è occupato da nemicie non potete lasciare mercenari "
    L"qui.\nDovete risolvere questa situazione prima di caricare qualsiasi "
    L"altro settore.",
    ...
    L"adiacente verrà immediatamente caricato.",
    L"Rimuovendo i vostri mercenari da questo settore,\nverrete "
    L"automaticamente postinella schermata della mappa\nvisto che i vostri "
    ...
    L"%ls ha bisogno di essere scortato dai vostri mercenari e non può "
    L"lasciare questo settore da solo.",
    ...
    L"%ls non può lasciare questo settore da solo, perché sta scortando "
    L"%ls.",  // male singular
    L"%ls non può lasciare questo settore da solo, perché sta scortando "
    L"%ls.",  // female singular
    L"%ls non può lasciare questo settore da solo, perché sta scortando "
    L"altre persone.",  // male plural
    L"%ls non può lasciare questo settore da solo, perché sta scortando "
    L"altre persone.",  // female plural
    ...
    L"permettere alla squadra di attraversare.",
    ...
    L"riassegnato a un'unica squadra.",

    // Standard helptext for all movement.  Explains what will happen
    // (moving the squad)
    L"Se selezionato, la vostra\nsquadra attualmente selezionata viaggerà, "
    L"lasciando questo settore.",
    ...
    L"%ls è scortato dai vostri mercenari e non può lasciare questo "
    L"settore da solo. Gli altri vostri mercenari devono trovarsi nelle "
    L"vicinanze prima che possiate andarvene.",
};

static const wchar_t *s_it_pRepairStrings[pRepairStrings_SIZE] = {
    L"Oggetti",   // tell merc to repair items in inventory
    L"Sito SAM",  // tell merc to repair SAM site - SAM is an acronym for Surface
                  // to Air Missile
    L"Annulla",   // cancel this menu
...
static const wchar_t *s_it_pHelicopterEtaStrings[pHelicopterEtaStrings_SIZE] = {
    ...
    L"L'elicottero ha poco carburante e deve atterrare in territorio "
    L"nemico!",  // warning that the sector the helicopter is going to use
...
static const wchar_t s_it_sMapLevelString[] = L"Sottolivello:";  // what level below the ground is
                                                                 // the player viewing in mapscreen

static const wchar_t s_it_gsLoyalString[] =
    L"%d%% Leale";  // the loyalty rating of a town ie : Loyal 53%
...
static const wchar_t *s_it_sFacilitiesStrings[sFacilitiesStrings_SIZE] = {
    ...
    L"Militare",
    L"Aeroporto",
    L"Frequenza di fuoco",  // a field for soldiers to practise their shooting
                            // skills
};
...
static const wchar_t *s_it_pMapInventoryErrorString[pMapInventoryErrorString_SIZE] = {
    ...
    L"Durante il combattimento, dovrete raccogliere gli oggetti "
    L"manualmente.",
    L"Durante il combattimento, dovrete rilasciare gli oggetti "
    L"manualmente.",
...
static const wchar_t *s_it_pMovementMenuStrings[pMovementMenuStrings_SIZE] = {
    ...
    L"Rotta spostamento esercito",         // done with movement menu, start plotting
                                           // movement
    L"Annulla",                            // cancel this menu
...
static const wchar_t *s_it_pUpdateMercStrings[pUpdateMercStrings_SIZE] = {
    ...
    L"Scaduto contratto mercenari:",           // this pop up came up due to a merc
                                               // contract ending
    L"Portato a termine incarico mercenari:",  // this pop up....due to more than
                                               // one merc finishing assignments
    L"Mercenari di nuovo al lavoro:",          // this pop up ....due to more than one
                                               // merc waking up and returing to work
    L"Mercenari a riposo:",                    // this pop up ....due to more than one merc being
                                               // tired and going to sleep
    L"Contratti in scadenza:",                 // this pop up came up due to a merc contract
                                               // ending
};
...
static const wchar_t s_it_pMapScreenBottomText[] =
...
static const wchar_t s_it_pFinanceTitle[] =
    L"Contabile aggiuntivo";  // the name we made up for the financial program in
                              // the game
...
static const wchar_t *s_it_pTransactionText[pTransactionText_SIZE] = {
    ...
    L"Analisi del profilo I.M.P.",  // IMP is the acronym for International
                                    // Mercenary Profiling
    ...
    L"Equipaggiamento esercito in %ls",  // initial cost to equip a town's
                                         // militia
    L"Oggetti acquistati da%ls.",        // is used for the Shop keeper interface.  The
                                         // dealers name will be appended to the end of
                                         // the string.
...
static const wchar_t *s_it_pSkyriderText[pSkyriderText_SIZE] = {
    ...
    L"A Skyrider bisogna ancora dare $%d",  // skyrider is still owed an amount
                                            // of money
    L"Skyrider non ha passeggeri. Se avete intenzione di trasportare mercenari "
    L"in questo settore, assegnateli prima al Veicolo/Elicottero.",
};
...
static const wchar_t s_it_str_left_equipment[] =
    L"L'equipaggio di %ls è ora disponibile a %ls (%c%d).";
...
static const wchar_t
    *s_it_pMapScreenPrevNextCharButtonHelpText[pMapScreenPrevNextCharButtonHelpText_SIZE] = {
...
static const wchar_t *s_it_pTrashItemText[pTrashItemText_SIZE] = {
    L"Non lo vedrete mai più. Siete sicuri?",  // do you want to continue and
                                               // lose the item forever
    L"Questo oggetto sembra DAVVERO importante. Siete DAVVERO SICURISSIMI di "
    L"volerlo gettare via?",  // does the user REALLY want to trash this item
};
...
static const wchar_t *s_it_pMapErrorString[pMapErrorString_SIZE] = {
    ...
    L"I mercenari devono essere assegnati a una squadra o a un veicolo per "
    L"potersi muovere.",
    L"Non avete ancora membri nella squadra.",          // you have no members, can't do
                                                        // anything
    L"I mercenari non possono attenersi agli ordini.",  // merc can't comply with
                                                        // your order
                                                        // 6-10
    L"%ls ha bisogno di una scorta per muoversi. Inseritelo in una squadra che "
    L"ne è provvista.",  // merc can't move unescorted .. for a male
    L"%ls ha bisogno di una scorta per muoversi. Inseritela in una squadra che "
    L"ne è provvista.",  // for a female
    ...
    L"Siete stati vittima di un'imboscata da parte dai Bloodcat nel settore "
    L"%ls!",
    L"Siete appena entrati in quella che sembra una tana di un Bloodcat nel "
    L"settore I16!",
    L"",
    L"La zona SAM in %ls è stata assediata.",
    // 16-20
    L"La miniera di %ls è stata assediata. La vostra entrata giornaliera è "
    L"stata ridotta di %ls per giorno.",
    L"Il nemico ha assediato il settore %ls senza incontrare resistenza.",
    L"Almeno uno dei vostri mercenari non ha potuto essere affidato a questo "
    L"incarico.",
    ...
    L"I militari che non si trovano entro i confini della città non possono "
    L"essere spostati inquesto settore.",
    // 31-35
    ...
    L"Osservate che solo due mercenari potrebbero addestrare i militari in "
    L"questo settore.",
    L"Il robot non può muoversi senza il suo controller. Metteteli nella "
    L"stessa squadra.",
};

// help text used during strategic route plotting
static const wchar_t *s_it_pMapPlotStrings[pMapPlotStrings_SIZE] = {
    L"Cliccate di nuovo su una destinazione per confermare la vostra meta "
    L"finale, oppure cliccate su un altro settore per fissare più tappe.",
...
static const wchar_t *s_it_pBullseyeStrings[pBullseyeStrings_SIZE] = {
    ...
    L"I mercenari non possono essere trasportati, lo spazio aereo non è "
    L"sicuro!",
    L"Annullato. Il settore d'arrivo è immutato",
    L"Lo spazio aereo sopra %ls non è più sicuro! Il settore d'arrivo è stato "
    L"spostato a %ls.",
};
...
static const wchar_t
    *s_it_pMiscMapScreenMouseRegionHelpText[pMiscMapScreenMouseRegionHelpText_SIZE] = {
        L"Entra nell'inventario (|I|n|v|i|o)",
        L"Getta via l'oggetto",
        L"Esci dall'inventario (|I|n|v|i|o)",
};

static const wchar_t s_it_str_he_leaves_where_drop_equipment[] =
    L"Volete che %ls lasci il suo equipaggiamento dove si trova ora (%ls) o in "
    L"seguito a %ls (%ls) dopo aver preso il volo da Arulco?";
static const wchar_t s_it_str_she_leaves_where_drop_equipment[] =
    L"Volete che %ls lasci il suo equipaggiamento dove si trova ora (%ls) o in "
    L"seguito a %ls (%ls) dopo aver preso il volo da Arulco?";
static const wchar_t s_it_str_he_leaves_drops_equipment[] =
    L"%ls sta per partire e spedirà il suo equipaggiamento a %ls.";
static const wchar_t s_it_str_she_leaves_drops_equipment[] =
    L"%ls sta per partire e spedirà il suo equipaggiamento a %ls.";
...
static const wchar_t *s_it_pImpPopUpStrings[pImpPopUpStrings_SIZE] = {
    ...
    L"L'analisi preliminare del vostro stato finanziario mostra che non potete "
    L"offrire un'analisi di profilo.",
    L"Opzione non valida questa volta.",
    L"Per completare un profilo accurato, dovete aver spazio per almeno uno "
    L"dei membri della squadra.",
...
static const wchar_t *s_it_pImpButtonText[pImpButtonText_SIZE] = {
    ...
    L"NO, VOGLIO RICOMINCIARE.",  // no, I want to start over the profiling
                                  // process
    L"SÌ",
    L"NO",
    ...
    L"Immatricolazione",  // the IMP site registry..when name and gender is
                          // selected
...
static const wchar_t *s_it_pExtraIMPStrings[pExtraIMPStrings_SIZE] = {
    ...
    L"Con gli attributi ora assegnati, puoi procedere alla selezione del "
    L"ritratto.",
    L"Per completare il processo, seleziona il campione della voce che più ti "
    L"piace.",
};
...
static const wchar_t *s_it_pFilesSenderList[pFilesSenderList_SIZE] = {
    L"Rapporto",       // the recon report sent to the player. Recon is an
                       // abbreviation for reconissance
    L"Intercetta #1",  // first intercept file .. Intercept is the title of the
...
static const wchar_t *s_it_pHistoryStrings[pHistoryStrings_SIZE] = {
    ...
    L"Acquistato contratto d'assicurazione per %ls.",    // insurance contract
                                                         // purchased
    L"Annullato contratto d'assicurazione per %ls.",     // insurance contract
                                                         // canceled
    L"Versamento per richiesta assicurazione per %ls.",  // insurance claim
                                                         // payout for merc
    L"Esteso contratto di %ls di 1 giorno.",             // Extented "mercs name"'s for a
                                                         // day
    // 11-15
    L"Esteso contratto di %ls di 1 settimana.",  // Extented "mercs name"'s for a
                                                 // week
    L"Esteso contratto di %ls di 2 settimane.",  // Extented "mercs name"'s 2
                                                 // weeks
...
static const wchar_t s_it_pErrorStrings[] =
    L"Connessione intermittente all'host. Tempi d'attesa più lunghi per il "
    L"trasferimento.";

static const wchar_t s_it_pPersonnelString[] = L"Mercenari:";  // mercs we have

static const wchar_t s_it_pWebTitle[] = L"sir-FER 4.0";  // our name for the version of the browser,
...
static const wchar_t *s_it_pPersonnelDepartedStateStrings[pPersonnelDepartedStateStrings_SIZE] = {
    ...
    L"Ucciso in azione", L"Licenziato", L"Sposato", L"Contratto Scaduto", L"Liberato",
};
...
static const wchar_t
    *s_it_pPersonnelCurrentTeamStatsStrings[pPersonnelCurrentTeamStatsStrings_SIZE] = {
        L"Più basso",
        L"Normale",
        L"Più alto",
};
...
static StrPointer s_it_LargeTacticalStr[LargeTacticalStr_SIZE] = {
    ...
    L"Il nemico, non avendo alcuna pietà delle anime della squadra, divorerà "
    L"ognuno di voi!",
...
static const wchar_t *s_it_MercAccountText[MercAccountText_SIZE] = {
    ...
    L"Costo",
    L"Totale:",
    L"Conferma il pagamento di %ls?",  // the %ls is a string that contains the
...
static const wchar_t *s_it_sFuneralString[sFuneralString_SIZE] = {
    L"Impresa di pompe funebri di McGillicutty: Il dolore delle famiglie che "
    L"hanno fornito il loro aiuto dal 1983.",
    L"Precedentemente mercenario dell'A.I.M. Murray \"Pops\" McGillicutty è un "
    L"impresario di pompe funebri qualificato e con molta esperienza.",
    L"Essendo coinvolto profondamente nella morte e nel lutto per tutta la sua "
    L"vita, Pops sa quanto sia difficile affrontarli.",
    L"L'impresa di pompe funebri di McGillicutty offre una vasta gamma di "
    L"servizi funebri, da una spalla su cui piangere a ricostruzioni "
    L"post-mortem per corpi mutilati o sfigurati.",
    L"Lasciate che l'impresa di pompe funebri di McGillicutty vi aiuti e i "
    L"vostri amati riposeranno in pace.",
    ...
    // send flowers ).
    L"Purtroppo, il resto di questo sito non è stato completato a causa di una "
    L"morte in famiglia. Aspettando la lettura del testamento e la riscossione "
    L"dell'eredità, il sito verrà ultimato non appena possibile.",
    L"Vi porgiamo, comunque, le nostre condolianze in questo momento di "
    L"dolore. Contatteci ancora.",
};
...
static const wchar_t *s_it_sFloristText[sFloristText_SIZE] = {
    ...
    L"Consegna il giorno successivo in quasi tutto il mondo, garantito. "
    L"Applicate alcune restrizioni.",
    L"I prezzi più bassi in tutto il mondo, garantito!",
    L"Mostrateci un prezzo concorrente più basso per qualsiasi progetto, e "
    L"riceverete una dozzina di rose, gratuitamente.",
    L"Flora, fauna & fiori in volo dal 1981.",
    L"I nostri paracadutisti decorati ex-bomber lanceranno il vostro bouquet "
    L"entro un raggio di dieci miglia dalla locazione richiesta. Sempre e "
    L"ovunque!",
    L"Soddisfiamo la vostra fantasia floreale.",
    L"Lasciate che Bruce, il nostro esperto in composizioni floreali, "
    L"selezioni con cura i fiori più freschi e della migliore qualità dalle "
    L"nostre serre più esclusive.",
...
static const wchar_t *s_it_sFloristGalleryText[sFloristGalleryText_SIZE] = {
    ...
    L"Ricorda: c'è un supplemento di 10$ per tutti i bouquet appassiti o "
    L"schiacciati.",
...
static const wchar_t *s_it_BobbyROrderFormText[BobbyROrderFormText_SIZE] = {
    ...
    L"S&C (Vedete luogo consegna)",  // S&H is an acronym for Shipping and
                                     // Handling
    L"Totale finale",                // The grand total of all item totals + the shipping and
                                     // handling
    L"Luogo consegna",
    ...
    L"Non potete permettervi di pagare questo.",  // 20	// A popup message that
                                                  // to warn of not enough money
    L"<NESSUNO>",  // Gets displayed when there is no valid city selected
    L"Siete sicuri di volere spedire quest'ordine a %ls?",  // A popup that asks
                                                            // if the city
                                                            // selected is the
                                                            // correct one
    L"peso del pacco**",                                    // Displays the weight of the package
    L"** Peso min.",  // Disclaimer states that there is a minimum weight for the
                      // package
    L"Spedizioni",
};
...
static const wchar_t *s_it_BobbyRText[BobbyRText_SIZE] = {
    ...
    L"Cliccate sull'oggetto. Sinistro per aggiungere pezzi, destro per "
    L"toglierne. Una volta selezionata la quantità, procedete col nuovo "
    L"ordine.",
    ...
    L"* funzionale al %",  // if the item is damaged, displays the percent
                           // function of the item

    // Popup that tells the player that they can only order 10 items at a time

    L"Darn! Quest'ordine qui accetterà solo 10 oggetti. Se avete intenzione di "
    L"ordinare più merce (ed è quello che speriamo), fate un ordine a parte e "
    L"accettate le nostre scuse.",
    ...
    L"Ci dispiace, ma siamo momentaneamente sprovvisti di oggetti di questo "
    L"genere.",
};
 ...
 * ammo. */
static const wchar_t s_it_str_bobbyr_guns_num_guns_that_use_ammo[] =
    L"La vostra squadra ha %d arma(i) che usa(no) questo tipo di munizioni";
...
static const wchar_t *s_it_BobbyRaysFrontText[BobbyRaysFrontText_SIZE] = {
    ...
    L"Questo è il negozio con la fornitura militare e le armi più recenti e "
    L"potenti!",
    L"Possiamo trovare la soluzione perfetta per tutte le vostre esigenze "
    L"riguardo agli esplosivi.",
...
static const wchar_t *s_it_AimSortText[AimSortText_SIZE] = {
    ...
    L"Visualizza le facce dei mercenari disponibili", L"Rivedi il file di ogni singolo mercenario",
...
static const wchar_t *s_it_CharacterInfo[CharacterInfo_SIZE] = {
    ...
    L"Ulteriori informazioni",     // Title for the additional info for the merc's
                                   // bio
    L"Membri attivi",              // Title of the page
    L"Dispositivo opzionale:",     // Displays the optional gear cost
    L"Deposito MEDICO richiesto",  // If the merc required a medical deposit,
                                   // this is displayed
};
...
static const wchar_t *s_it_VideoConfercingText[VideoConfercingText_SIZE] = {
    ...
    L"con deposito"  // Displays if you are hiring the merc with the medical
                     // deposit
};
...
static const wchar_t *s_it_AimPopUpText[AimPopUpText_SIZE] = {
    ...
    L"NON IN GRADO DI TRASFERIRE",                // Player doesn't have enough money, message
                                                  // 1
...
static const wchar_t s_it_AimLinkText[] =
...
static const wchar_t *s_it_SKI_Text[SKI_SIZE] = {
    ...
    L"COSTO TOTALE",          // The total cost of the the items in the Dealer inventory
                              // area
    L"VALORE TOTALE",         // The total value of items player wishes to sell
    L"STIMATO",               // Button text for dealer to evaluate items the player wants to
                              // sell
    L"TRANSAZIONE",           // Button text which completes the deal. Makes the
                              // transaction.
    L"FINE",                  // Text for the button which will leave the shopkeeper interface.
    L"COSTO DI RIPARAZIONE",  // The amount the dealer will charge to repair the
                              // merc's goods
    L"1 ORA",                 // SINGULAR! The text underneath the inventory slot when an item
                              // is given to the dealer to be repaired
    L"%d ORE",                // PLURAL!   The text underneath the inventory slot when an item
                              // is given to the dealer to be repaired
    L"RIPARATO",              // Text appearing over an item that has just been repaired by a
                              // NPC repairman dealer
    L"Non c'è abbastanza spazio nel vostro margine di ordine.",  // Message box
                                                                 // that tells the
                                                                 // user there is
                                                                 // no more room
                                                                 // to put there
                                                                 // stuff
    L"%d MINUTI",  // The text underneath the inventory slot when an item is
...
static const wchar_t *s_it_SkiMessageBoxText[SkiMessageBoxText_SIZE] = {
    L"Volete sottrarre %ls dal vostro conto principale per coprire la "
    L"differenza?",
...
static const wchar_t *s_it_zOptionsText[zOptionsText_SIZE] = {
    ...
    L"Avete bisogno dell'opzione 'Parlato' o di quella 'Sottotitoli' per poter "
    L"giocare.",
};
...
static const wchar_t *s_it_zSaveLoadText[zSaveLoadText_SIZE] = {
    ...
    L"La versione del gioco nel file della partita salvata è diverso dalla "
    L"versione attuale. È abbastanza sicuro proseguire. Continuate?",
    L"I file della partita salvata potrebbero essere annullati. Volete "
    L"cancellarli tutti?",

    L"Tentativo di caricare una versione salvata più vecchia. Aggiornate e "
    L"caricate automaticamente quella salvata?",
    L"Tentativo di caricare una vecchia versione salvata. Aggiornate e "
    L"caricate automaticamente quella salvata?",
    ...
    L"Lo spazio su disco si sta esaurendo. Sono disponibili solo %d MB, mentre "
    L"per giocare a Jagged dovrebbero esserci almeno %d MB liberi .",

    L"Salvataggio in corso...",  // When saving a game, a message box with this
...
static const wchar_t *s_it_zMarksMapScreenText[zMarksMapScreenText_SIZE] = {
    L"Livello mappa",
    L"Non avete soldati. Avete bisogno di addestrare gli abitanti della città "
    L"per poter disporre di un esercito cittadino.",
    ...
    // stuff for contracts
    L"Visto che non potete pagare il contratto, non avete neanche i soldi per "
    L"coprire il premio dell'assicurazione sulla vita di questo nercenario.",
    L"%ls premio dell'assicurazione costerà %ls per %d giorno(i) extra. Volete "
    L"pagare?",
...
static const wchar_t s_it_pLandMarkInSectorString[] =
    L"La squadra %d ha notato qualcuno nel settore %ls";

// confirm the player wants to pay X dollars to build a militia force in town
static const wchar_t *s_it_pMilitiaConfirmStrings[pMilitiaConfirmStrings_SIZE] = {
    L"Addestrare una squadra dell'esercito cittadino costerà $",  // telling
                                                                  // player
                                                                  // how
                                                                  // much
                                                                  // it
                                                                  // will
                                                                  // cost
    L"Approvate la spesa?",        // asking player if they wish to pay the
                                   // amount requested
    L"Non potete permettervelo.",  // telling the player they can't
                                   // afford to train this town
    L"Continuate ad aeddestrare i soldati in %ls (%ls %d)?",  // continue
                                                              // training
                                                              // this
                                                              // town?
    L"Costo $",  // the cost in dollars to train militia
    L"(S/N)",    // abbreviated yes/no
    L"Addestrare l'esrecito cittadino nei settori di %d costerà $ %d. "
    L"%ls",  // cost to train sveral sectors at once
    L"Non potete permettervi il $%d per addestrare l'esercito "
    L"cittadino qui.",
    L"%ls ha bisogno di una percentuale di %d affinché possiate "
    L"continuare ad addestrare i soldati.",
...
static const wchar_t s_it_gzCopyrightText[] =
...
static const wchar_t *s_it_zOptionsScreenHelpText[zOptionsToggleText_SIZE] = {
    ...
    L"Se i sottotitoli sono attivati, utilizzate questa opzione per leggere "
    L"tranquillamente i dialoghi NPC.",

    // Toggle smoke animation
    L"Disattivate questa opzione, se il fumo dinamico diminuisce la frequenza "
    L"d'aggiornamento.",
    ...
    // Never move my mouse
    L"Disattivate questa opzione per muovere automaticamente il mouse sulle "
    L"finestre a comparsa di conferma al loro apparire.",

    // Old selection method
    L"Attivate questa opzione per selezionare i personaggi e muoverli come nel "
    L"vecchio JA (dato che la funzione è stata invertita).",

    // Show movement path
    L"Attivate questa opzione per visualizzare i sentieri di movimento in "
    L"tempo reale (oppure disattivatela utilizzando il tasto MAIUSC).",

    // show misses
    L"Attivate per far sì che la partita vi mostri dove finiscono i proiettili "
    L"quando \"sbagliate\".",

    // Real Time Confirmation
    L"Se attivata, sarà richiesto un altro clic su \"salva\" per il movimento "
    L"in tempo reale.",

    // Sleep/Wake notification
    L"Se attivata, verrete avvisati quando i mercenari in \"servizio\" vanno a "
    L"riposare e quando rientrano in servizio.",

    // Use the metric system
    L"Se attivata, utilizza il sistema metrico di misurazione; altrimenti "
    L"ricorre al sistema britannico.",

    // Merc Lighted movement
    L"Se attivata, il mercenario mostrerà il terreno su cui cammina. "
    L"Disattivatela per un aggiornamento più veloce.",

    // Smart cursor
    L"Se attivata, muovendo il cursore vicino ai vostri mercenari li "
    L"evidenzierà automaticamente.",

    // snap cursor to the door
    L"Se attivata, muovendo il cursore vicino a una porta farà posizionare "
    L"automaticamente il cursore sopra di questa.",
...
static const wchar_t s_it_pSkillAtZeroWarning[] =
...
static const wchar_t *s_it_pMessageStrings[pMessageStrings_SIZE] = {
    ...
    L"RIASSUMI",
    L"MENTI",
    L"Nessuna descrizione",  // Save slots that don't have a
                             // description.
    L"Partita salvata.",
    L"Giorno",
    L"Mercenari",
    L"Slot vuoto",  // An empty save game slot
    L"ppm",         // Abbreviation for Rounds per minute -- the potential #
    ...
    L"%ls si trova ora nel settore %ls%ls",  // Name/Squad has
                                             // arrived in sector A9.
                                             // Order must not change
                                             // without notifying
                                             // SirTech
    L"Versione",
    L"Slot di salvataggio rapido vuoto",
    L"Questo slot è riservato ai salvataggi rapidi fatti dalle "
    L"schermate tattiche e dalla mappa utilizzando ALT+S.",
    L"Aperto",
    L"Chiuso",
    L"Lo spazio su disco si sta esaurendo. Avete liberi solo %ls "
    L"MB e Jagged Alliance 2 ne richiede %ls.",
    L"%ls ha preso %ls.",                 //'Merc name' has caught 'item' -- let
                                          // SirTech know if name comes after item.
    L"%ls ha assunto della droga.",       //'Merc name' has taken the drug
    L"%ls non ha alcuna abilità medica",  //'Merc name' has no
                                          // medical skill.
    ...
    L"%ls è passato a %ls.",                  //"Item" passed to "merc".  Please try
                                              // to keep the item %ls before the merc
                                              //%ls, otherwise, must notify SirTech.
    L"Nessun spazio per passare %ls a %ls.",  // pass "item" to
                                              // "merc".  Same
                                              // instructions as
                                              // above.
    ...
    L"di",  // used in the shop keeper inteface to mark the ownership
    ...
    L"Cursore 3D attivo.",
    L"Squadra %d attiva.",
    L"Non potete permettervi di pagare a %ls un salario "
    L"giornaliero di %ls",  // first %ls is the mercs name, the
                            // seconds is a string containing the
                            // salary
    L"Salta",
    L"%ls non può andarsene da solo.",
    L"Un salvataggio è stato chiamato SaveGame99.sav. Se "
    L"necessario, rinominatelo da SaveGame01 a SaveGame10 e così "
    L"potrete accedervi nella schermata di caricamento.",
    L"%ls ha bevuto del %ls",
    L"Un pacco è arivato a Drassen.",
    L"%ls dovrebbe arrivare al punto designato di partenza "
    L"(settore %ls) nel giorno %d, approssimativamente alle ore "
    L"%ls.",  // first %ls is mercs name, next is the sector location
              // and name where they will be arriving in, lastely is
              // the day an the time of arrival
...
static const wchar_t *s_it_pMilitiaButtonsHelpText[pMilitiaButtonsHelpText_SIZE] = {
    L"Raccogli (Clicca di destro)/lascia (Clicca di sinistro) le truppe "
    L"verdi",  // button help text informing player they can pick up or drop
...
static const wchar_t s_it_pMapScreenJustStartedHelpText[] =
    L"Andate all'A.I.M. e arruolate alcuni mercenari (*Hint* è nel Laptop)";

static const wchar_t s_it_pAntiHackerString[] =
...
static const wchar_t *s_it_gzNonPersistantPBIText[gzNonPersistantPBIText_SIZE] = {
    L"È in corso una battaglia. Potete solo ritirarvi dalla schermata "
    L"delle tattiche.",
    L"|Entra nel settore per continuare l'attuale battaglia in corso.",
    L"|Automaticamente decide l'esito della battaglia in corso.",
    L"Non potete decidere l'esito della battaglia in corso "
    L"automaticamente, se siete voi ad attaccare.",
    L"Non potete decidere l'esito della battaglia in corso "
    L"automaticamente, se subite un'imboscata.",
    L"Non potete decidere l'esito della battaglia in corso "
    L"automaticamente, se state combattendo contro le creature nelle "
    L"miniere.",
    L"Non potete decidere l'esito della battaglia in corso "
    L"automaticamente, se ci sono civili nemici.",
    L"Non potete decidere l'esito della battaglia in corso "
    L"automaticamente, se ci sono dei Bloodcat.",
    L"BATTAGLIA IN CORSO",
    L"Non potete ritirarvi ora.",
};

static const wchar_t *s_it_gzMiscString[gzMiscString_SIZE] = {
    L"I vostri soldati continuano a combattere senza l'aiuto dei vostri "
    L"mercenari...",
...
static const wchar_t *s_it_zGioDifConfirmText[zGioDifConfirmText_SIZE] = {
    // L"You have chosen NOVICE mode. This setting is appropriate for those new
    // to Jagged Alliance, those new to strategy games in general, or those
    // wishing shorter battles in the game. Your choice will affect things
    // throughout the entire course of the game, so choose wisely. Are you sure
    // you want to play in Novice mode?",
    L"Avete selezionato la modalità PRINCIPIANTE. Questo scenario è adatto a "
    L"chi gioca per la prima volta a Jagged Alliance, a chi prova a giocare "
    L"per la prima volta in generale o a chi desidera combattere battaglie più "
    L"brevi nel gioco. La vostra decisione influirà sull'intero corso della "
    L"partita; scegliete, quindi, con attenzione. Siete sicuri di voler "
    L"giocare nella modalità PRINCIPIANTE?",

    // L"You have chosen EXPERIENCED mode. This setting is suitable for those
    // already familiar with Jagged Alliance or similar games. Your choice will
    // affect things throughout the entire course of the game, so choose wisely.
    // Are you sure you want to play in Experienced mode?",
    L"Avete selezionato la modalità ESPERTO. Questo scenario è adatto a chi ha "
    L"già una certa dimestichezza con Jagged Alliance o con giochi simili. La "
    L"vostra decisione influirà sull'intero corso della partita; scegliete, "
    L"quindi, con attenzione. Siete sicuri di voler giocare nella modalità "
    L"ESPERTO?",

    // L"You have chosen EXPERT mode. We warned you. Don't blame us if you get
    // shipped back in a body bag. Your choice will affect things throughout the
    // entire course of the game, so choose wisely. Are you sure you want to play
    // in Expert mode?",
    L"Avete selezionato la modalità PROFESSIONISTA. Siete avvertiti. Non "
    L"malediteci, se vi ritroverete a brandelli. La vostra decisione influirà "
    L"sull'intero corso della partita; scegliete, quindi, con attenzione. "
    L"Siete sicuri di voler giocare nella modalità PROFESSIONISTA?",

};

static const wchar_t *s_it_gzLateLocalizedString[gzLateLocalizedString_SIZE] = {
    // 1-5
    L"Il robot non può lasciare questo settore, se nessuno sta usando il "
    L"controller.",
    ...
    L"%ls non ha bisogno immediatamente di pronto soccorso o di fasciature, "
    L"quanto piuttosto di cure mediche più serie e/o riposo.",
    ...
    L"Non è al momento sicuro comprimere il tempo visto che avete dei "
    L"mercenari nel settore %ls.",
    L"Non è al momento sicuro comprimere il tempo quando i mercenari sono "
    L"nelle miniere infestate dalle creature.",
    ...
    // 39
    L"Nessuno dei vostri rimanenti mercenari è in grado di combattere. "
    L"L'esercito combatterà contro le creature da solo.",
    ...
    // 55
    L"Non potete comprimere il tempo mentre visualizzate l'inventario del "
    L"settore.",
...
static const wchar_t s_it_str_ceramic_plates_smashed[] =
    L"%ls's ceramic plates have been smashed!";  // TODO translate

static const wchar_t s_it_str_arrival_rerouted[] =
...
static const wchar_t *s_it_str_stat_list[str_stat_list_SIZE] = {
    s_it_str_stat_health,    s_it_str_stat_agility,      s_it_str_stat_dexterity,
    s_it_str_stat_strength,  s_it_str_stat_leadership,   s_it_str_stat_wisdom,
    s_it_str_stat_exp_level, s_it_str_stat_marksmanship, s_it_str_stat_mechanical,
    s_it_str_stat_explosive, s_it_str_stat_medical};

static const wchar_t *s_it_str_aim_sort_list[str_aim_sort_list_SIZE] = {
    s_it_str_aim_sort_price,     s_it_str_aim_sort_experience, s_it_str_aim_sort_marksmanship,
    s_it_str_aim_sort_medical,   s_it_str_aim_sort_explosives, s_it_str_aim_sort_mechanical,
    s_it_str_aim_sort_ascending, s_it_str_aim_sort_descending,
};
