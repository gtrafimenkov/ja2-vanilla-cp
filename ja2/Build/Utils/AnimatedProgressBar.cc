...
void CreateProgressBar(const UINT8 ubProgressBarID, const UINT16 x, const UINT16 y, const UINT16 w,
                       const UINT16 h) {
...
void DefineProgressBarPanel(UINT32 ubID, UINT8 r, UINT8 g, UINT8 b, UINT16 usLeft, UINT16 usTop,
  ...
  pCurr->usLtColor = Get16BPPColor(FROMRGB((UINT8)std::min((uint16_t)255, (UINT16)(r * 1.33)),
                                           (UINT8)std::min((uint16_t)255, (UINT16)(g * 1.33)),
                                           (UINT8)std::min((uint16_t)255, (UINT16)(b * 1.33))));
  pCurr->usDkColor =
      Get16BPPColor(FROMRGB((UINT8)(r * 0.75), (UINT8)(g * 0.75), (UINT8)(b * 0.75)));
}
...
void SetProgressBarTitle(UINT32 ubID, const wchar_t *pString, Font const font, UINT8 ubForeColor,
                         UINT8 ubShadowColor) {
  PROGRESSBAR *pCurr;
  Assert(ubID < MAX_PROGRESSBARS);
  pCurr = pBar[ubID];
...
// color.
void SetProgressBarMsgAttributes(UINT32 ubID, Font const font, UINT8 ubForeColor,
                                 UINT8 ubShadowColor) {
  PROGRESSBAR *pCurr;
  Assert(ubID < MAX_PROGRESSBARS);
  pCurr = pBar[ubID];
...
void SetRelativeStartAndEndPercentage(UINT8 const id, UINT32 const uiRelStartPerc,
                                      UINT32 const uiRelEndPerc, wchar_t const *const str) {
