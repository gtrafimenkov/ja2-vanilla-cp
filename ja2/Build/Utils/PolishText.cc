...
static StrPointer s_pl_AmmoCaliber[AmmoCaliber_SIZE] = {
...
// WP is Warsaw Pact
// cal is an abbreviation for calibre
static StrPointer s_pl_BobbyRayAmmoCaliber[BobbyRayAmmoCaliber_SIZE] = {
    L"0",         L".38 cal", L"9mm",    L".45 cal",  L".357 cal",
...
static StrPointer s_pl_Message[Message_SIZE] = {
    ...
    L"Element, który masz na kursorze myszy może być dołączony do pewnych "
    L"przedmiotów, poprzez umieszczenie go w jednym z czterech slotów.",
    L"Element, który masz na kursorze myszy może być dołączony do pewnych "
    L"przedmiotów, poprzez umieszczenie go w jednym z czterech slotów. (Jednak "
    L"w tym przypadku, przedmioty do siebie nie pasują.)",
    ...
    L"NIE SĄDZĘ!",
    L"Aby podróżować helikopterem Skyridera, musisz najpierw zmienić przydział "
    L"najemników na POJAZD/HELIKOPTER.",
...
static const wchar_t *s_pl_g_towns_locative[g_towns_locative_SIZE] = {
    L"",      L"Omercie", L"Drassen",
    L"Alma",  // TODO fill in correct forms
    L"Grumm", L"Tixa",    L"Cambria", L"San Mona", L"Estoni",
...
static const wchar_t *s_pl_pMilitiaString[pMilitiaString_SIZE] = {
    ...
    L"Nie możesz przemieszczać oddziałów samoobrony gdy nieprzyjaciel jest w "
    L"sektorze!",
};
...
static const wchar_t *s_pl_pDoorTrapStrings[pDoorTrapStrings_SIZE] = {
    L"nie posiada żadnych pułapek", L"ma założony ładunek wybuchowy", L"jest pod napięciem",
...
static const wchar_t *s_pl_pNoiseVolStr[pNoiseVolStr_SIZE] = {L"CICHY", L"WYRAŹNY", L"GŁOŚNY",
                                                              L"BARDZO GŁOŚNY"};
...
static const wchar_t *s_pl_pDirectionStr[pDirectionStr_SIZE] = {
    L"PŁN-WSCH", L"WSCH", L"PŁD-WSCH", L"PŁD", L"PŁD-ZACH", L"ZACH", L"PŁN-ZACH", L"PŁN"};
...
static const wchar_t *s_pl_gpStrategicString[gpStrategicString_SIZE] = {
    ...
    L"Wycofuje wszystkie oddziały do sąsiedniego sekto|ra.",  // multiple groups
                                                              // with same
                                                              // previous sector
    ...
    //%c%d is the sector -- ex:  A9
    L"Nieprzyjaciel zatakował twoich najemników w sektorze %ls.  Żaden z "
    L"twoich najemników nie może walczyć!",
    //%c%d is the sector -- ex:  A9
    L"Stworzenia zatakowały twoich najemników w sektorze %ls.  Żaden z twoich "
    L"najemników nie może walczyć!",
...
static StrPointer s_pl_gWeaponStatsDesc[gWeaponStatsDesc_SIZE] = {
    L"Waga (%ls):",  // change kg to another weight unit if your standard is not
                     // kilograms, and TELL SIR-TECH!
    L"Stan:",
...
static const wchar_t *s_pl_gMoneyStatsDesc[gMoneyStatsDesc_SIZE] = {
    ...
    L"Kwota",
    L"Wydzielić:",  // the amount he wants to separate from the overall balance
...
static StrPointer s_pl_TacticalStr[TacticalStr_SIZE] = {
    ...
    L"Zamek %ls.", L"Brak zamka.", L"Zamek nie ma pułapek.",
    // The %ls is a merc name
    L"%ls nie posiada odpowiedniego klucza.", L"Zamek nie ma pułapek.", L"Zamknięte.", L"DRZWI",
    L"ZABEZP.", L"ZAMKNIĘTE", L"OTWARTE", L"ROZWALONE", L"Tu jest przełącznik. Włączyć go?",
    L"Rozbroić pułapkę?", L"Więcej...",

    // In the next 2 strings, %ls is an item name

    L"%ls - położono na ziemi.", L"%ls - przekazano do - %ls.",

    // In the next 2 strings, %ls is a name

    L"%ls otrzymał(a) całą zapłatę.", L"%ls - należność wobec niej/niego wynosi jeszcze %d.",
    L"Wybierz częstotliwość sygnału detonującego:",  // in this case, frequency
                                                     // refers to a radio signal
    L"Ile tur do eksplozji:",                     // how much time, in turns, until the bomb blows
    L"Ustaw częstotliwość zdalnego detonatora:",  // in this case, frequency
                                                  // refers to a radio signal
    ...
    L"Na pewno chcesz zaatakować - %ls?", L"Pojazdy nie mogą zmieniać pozycji.",
    L"Robot nie może zmieniać pozycji.",

    // In the next 3 strings, %ls is a name

    L"%ls nie może zmienić pozycji w tym miejscu.", L"%ls nie może tu otrzymać pierwszej pomocy.",
    L"%ls nie potrzebuje pierwszej pomocy.", L"Nie można ruszyć w to miejsce.",
    L"Oddział jest już kompletny. Nie ma miejsca dla nowych rekrutów.",  // there's
                                                                         // no
                                                                         // room
                                                                         // for a
                                                                         // recruit
    ...
    L"Rzucić tam? To niemożliwe.",              // Merc can't throw to the destination he
                                                // selected
    ...
    L"%ls ma włączone potwierdzenia głosowe.", L"%ls ma wyłączone potwierdzenia głosowe.",
    ...
    L"%ls - pułapka została uruchomiona.", L"%ls przybył(a) na miejsce.",
    L"%ls stracił(a) wszystkie Punkty Akcji.", L"%ls jest nieosiągalny(na).",
    L"%ls ma już założone opatrunki.", L"%ls nie ma bandaży.", L"Wróg w sektorze!",
    ...
    L"Nie można opuścić sektora z tej strony.", L"%ls jest zbyt daleko.", L"Usuń korony drzew",
    ...
    L"Wpłata/Podjęcie pieniędzy",  // Help text over the $ button on the Single
                                   // Merc Panel
    ...
    // money (e.g. $200)
    L"Zgadzasz się zapłacić %ls?", L"Zgadzasz się na darmowe leczenie?",
    ...
    L"Teraz nie można automatycznie udzielić pierwszej pomocy", L"Przejście zablokowane dla - %ls",
...
static const wchar_t *s_pl_pExitingSectorHelpText[pExitingSectorHelpText_SIZE] = {
    // Helptext for the "Go to Sector" checkbox button, that explains
    // what will happen when the box is checked.
    L"Jeśli zaznaczysz tę opcję, to sąsiedni sektor zostanie "
    L"natychmiast załadowany.",
    L"Jeśli zaznaczysz tę opcję, to na czas podróży pojawi się "
    L"automatycznie ekran mapy.",

    // If you attempt to leave a sector when you have multiple squads in
    // a hostile sector.
    L"Ten sektor jest okupowany przez wroga i nie możesz tu zostawić "
    L"najemników.\nMusisz uporać się z tą sytuacją zanim załadujesz "
    L"inny sektor.",
    ...
    L"Gdy wyprowadzisz swoich pozostałych najemników z tego "
    L"sektora,\nsąsiedni sektor zostanie automatycznie załadowany.",
    L"Gdy wyprowadzisz swoich pozostałych najemników z tego "
    L"sektora,\nto na czas podróży pojawi się automatycznie ekran "
    L"mapy.",

    // If an EPC is the selected merc, it won't allow the merc to leave
    // alone as the merc is being escorted.  The "single" button is
    // disabled.
    L"%ls jest pod eskortą twoich najemników i nie może bez nich "
    L"opuścić tego sektora.",
    ...
    // There are several strings depending on the gender of the merc and
    // how many EPCs are in the squad. DO NOT USE THE NEWLINE HERE AS IT
    // IS USED FOR BOTH HELPTEXT AND SCREEN MESSAGES!
    L"%ls nie może sam opuścić tego sektora, gdyż %ls jest pod jego "
    L"eskortą.",  // male singular
    L"%ls nie może sama opuścić tego sektora, gdyż %ls jest pod jej "
    L"eskortą.",  // female singular
    L"%ls nie może sam opuścić tego sektora, gdyż eskortuje inne "
    L"osoby.",  // male plural
    L"%ls nie może sama opuścić tego sektora, gdyż eskortuje inne "
    L"osoby.",  // female plural
    ...
    L"automatycznie znajdzie się w osobnym oddziale.",

    // Standard helptext for all movement.  Explains what will happen
    // (moving the squad)
    L"Jeśli zaznaczysz tę opcję, aktualnie\nwybrany oddział opuści ten "
    L"sektor.",

    // This strings is used BEFORE the "exiting sector" interface is
    // created.  If you have an EPC selected and you attempt to
    // tactically traverse the EPC while the escorting mercs aren't near
    // enough (or dead, dying, or unconscious), this message will appear
    // and the "exiting sector" interface will not appear.  This is just
    // like the situation where This string is special, as it is not used
    // as helptext.  Do not use the special newline character (\n) for
    // this string.
    L"%ls jest pod eskortą twoich najemników i nie może bez nich "
    L"opuścić tego sektora. Aby opuścić sektor twoi najemnicy muszą "
    L"być w pobliżu.",
};

static const wchar_t *s_pl_pRepairStrings[pRepairStrings_SIZE] = {
    L"Wyposażenie",      // tell merc to repair items in inventory
    L"Baza rakiet Z-P",  // tell merc to repair SAM site - SAM is an acronym for
...
static const wchar_t *s_pl_pHelicopterEtaStrings[pHelicopterEtaStrings_SIZE] = {
    ...
    L"Helikopter ma mało paliwa i musi wylądować na terenie wroga.",  // warning
                                                                      // that the
                                                                      // sector
                                                                      // the
                                                                      // helicopter
...
static const wchar_t s_pl_sMapLevelString[] = L"Poziom:";  // what level below the ground is the
                                                           // player viewing in mapscreen

static const wchar_t s_pl_gsLoyalString[] =
    L"%d%% Lojalności";  // the loyalty rating of a town ie : Loyal 53%
...
static const wchar_t *s_pl_pMapInventoryErrorString[pMapInventoryErrorString_SIZE] = {
    ...
    L"%ls nie może stąd zabrać tego przedmiotu, gdyż nie jest w tym "
    L"sektorze.",
    L"Podczas walki nie można korzystać z tego panelu.",
    L"Podczas walki nie można korzystać z tego panelu.",
    L"%ls nie może tu zostawić tego przedmiotu, gdyż nie jest w tym "
    L"sektorze.",
};
...
static const wchar_t *s_pl_pUpdateMercStrings[pUpdateMercStrings_SIZE] = {
    ...
    L"Wygasł kontrakt najemników:",   // this pop up came up due to a merc
                                      // contract ending
    L"Najemnicy wypełnili zadanie:",  // this pop up....due to more than one merc
                                      // finishing assignments
    L"Najemnicy wrócili do pracy:",   // this pop up ....due to more than one merc
                                      // waking up and returing to work
    L"Odpoczywający najemnicy:",      // this pop up ....due to more than one merc
                                      // being tired and going to sleep
    L"Wkrótce wygasną kontrakty:",    // this pop up came up due to a merc contract
                                      // ending
};
...
static const wchar_t *s_pl_pMapScreenBottomFastHelp[pMapScreenBottomFastHelp_SIZE] = {
    ...
    L"strona (|P|g|U|p)",  // previous message in scrollable list
    L"Następny komunikat (|S|t|r|z|a|ł|k|a |w |d|ó|ł)\nNastępna strona "
    L"(|P|g|D|n)",                                   // next message in the scrollable list
    L"Włącz/Wyłącz kompresję czasu (|S|p|a|c|j|a)",  // start/stop time
                                                     // compression
};

static const wchar_t s_pl_pMapScreenBottomText[] =
...
static const wchar_t s_pl_pFinanceTitle[] = L"Księgowy Plus";  // the name we made up for the
...
static const wchar_t *s_pl_pTransactionText[pTransactionText_SIZE] = {
    ...
    L"Analiza profilu w IMP",      // IMP is the acronym for International Mercenary
                                   // Profiling
    ...
    L"Zakupy u - %ls.",             // is used for the Shop keeper interface.  The dealers
                                    // name will be appended to the end of the string.
...
static const wchar_t *s_pl_pSkyriderText[pSkyriderText_SIZE] = {
    ...
    L"Skyriderowi trzeba jeszcze zapłacić %d$",  // skyrider is still owed an
                                                 // amount of money
    L"Skyrider nie ma pasażerów. Jeśli chcesz przetransportować najemników, "
    L"zmień ich przydział na POJAZD/HELIKOPTER.",
};
...
static const wchar_t
    *s_pl_pMapScreenPrevNextCharButtonHelpText[pMapScreenPrevNextCharButtonHelpText_SIZE] = {
        L"Poprzedni najemnik (|S|t|r|z|a|ł|k|a |w |l|e|w|o)",   // previous merc
                                                                // in the list
        L"Następny najemnik (|S|t|r|z|a|ł|k|a |w |p|r|a|w|o)",  // next merc in
                                                                // the list
};

static const wchar_t s_pl_pEtaString[] =
    L"PCP:";  // eta is an acronym for Estimated Time of Arrival

static const wchar_t *s_pl_pTrashItemText[pTrashItemText_SIZE] = {
    L"Więcej tego nie zobaczysz. Czy na pewno chcesz to zrobić?",  // do you
                                                                   // want to
                                                                   // continue
                                                                   // and
                                                                   // lose
                                                                   ...
                                                                   // item
                                                                   // forever
    L"To wygląda na coś NAPRAWDĘ ważnego. Czy NA PEWNO chcesz to "
    L"zniszczyć?",  // does the user REALLY want to trash this item
};
...
static const wchar_t *s_pl_pMapErrorString[pMapErrorString_SIZE] = {
    ...
    L"Najemnik nie może wypełnić tego rozkazu.",  // merc can't comply with your
                                                  // order
                                                  // 6-10
    L"%ls musi mieć eskortę, aby się przemieszczać. Umieść go w oddziale z "
    L"eskortą.",  // merc can't move unescorted .. for a male
    L"%ls musi mieć eskortę, aby się przemieszczać. Umieść ją w oddziale z "
    L"eskortą.",  // for a female
    ...
    // 16-20
    L"%ls - kopalnia została przejęta. Twój dzienny przychód został "
    L"zredukowany do %ls.",
    ...
    L"Żołnierze samoobrony, którzy znajdują się poza granicami miasta, nie "
    L"mogą być przeniesieni do innego sektora.",
    // 31-35
    ...
    L"Nie możesz kompresować czasu dopóki nie zatrudnisz sobie kilku nowych "
    L"najemników!",
    ...
    L"Jeden lub więcej członków tego oddziału nie może się w tej chwili "
    L"przemieszczać.",
    // 46-50
    L"Jeden lub więcej INNYCH członków tego oddziału nie może się w tej chwili "
    L"przemieszczać.",
    L"Pojazd jest uszkodzony!",
    L"Pamiętaj, że w jednym sektorze tylko dwóch najemników może trenować "
    L"żołnierzy samoobrony.",
    L"Robot nie może się poruszać bez operatora. Umieść ich razem w jednym "
    L"oddziale.",
};

// help text used during strategic route plotting
static const wchar_t *s_pl_pMapPlotStrings[pMapPlotStrings_SIZE] = {
    L"Kliknij ponownie sektor docelowy, aby zatwierdzić trasę podróży, lub "
    L"kliknij inny sektor, aby ją wydłużyć.",
...
static const wchar_t *s_pl_pBullseyeStrings[pBullseyeStrings_SIZE] = {
    ...
    L"Najemnicy nie mogą tu przylatywać. Przestrzeń powietrzna nie jest "
    L"zabezpieczona!",
    L"Anulowano. Sektor zrzutu nie został zmieniony.",
    L"Przestrzeń powietrzna nad %ls nie jest już bezpieczna! Sektor zrzutu "
    L"został przesunięty do %ls.",
};

// help text for mouse regions

static const wchar_t
    *s_pl_pMiscMapScreenMouseRegionHelpText[pMiscMapScreenMouseRegionHelpText_SIZE] = {
        L"Otwórz wyposażenie (|E|n|t|e|r)",
        L"Zniszcz przedmiot",
        L"Zamknij wyposażenie (|E|n|t|e|r)",
};

static const wchar_t s_pl_str_he_leaves_where_drop_equipment[] =
    L"Czy %ls ma zostawić swój sprzęt w sektorze, w którym się obecnie "
    L"znajduje (%ls), czy w %ls (%ls), skąd odlatuje?";
static const wchar_t s_pl_str_she_leaves_where_drop_equipment[] =
    L"Czy %ls ma zostawić swój sprzęt w sektorze, w którym się obecnie "
    L"znajduje (%ls), czy w %ls (%ls), skąd odlatuje?";
static const wchar_t s_pl_str_he_leaves_drops_equipment[] =
    L"%ls wkrótce odchodzi i zostawi swój sprzęt w %ls.";
static const wchar_t s_pl_str_she_leaves_drops_equipment[] =
    L"%ls wkrótce odchodzi i zostawi swój sprzęt w %ls.";
...
static const wchar_t *s_pl_pImpPopUpStrings[pImpPopUpStrings_SIZE] = {
    ...
    L"Wstępna kontrola stanu twoich finansów wykazała, że nie stać cię na "
    L"analizę profilu.",
    L"Opcja tym razem nieaktywna.",
    L"Aby wykonać profil, musisz mieć miejsce dla przynajmniej jednego członka "
    L"załogi.",
...
static const wchar_t *s_pl_pImpButtonText[pImpButtonText_SIZE] = {
    ...
    L"NIE, CHCĘ ZACZĄĆ OD NOWA.",  // no, I want to start over the profiling
                                   // process
    L"TAK",
    L"NIE",
...
static const wchar_t *s_pl_pExtraIMPStrings[pExtraIMPStrings_SIZE] = {
    ...
    L"Aby zakończyć proces, wybierz próbkę głosu, która ci najbardziej "
    L"odpowiada."};
...
static const wchar_t *s_pl_pFilesSenderList[pFilesSenderList_SIZE] = {
    L"Raport Rozp.",  // the recon report sent to the player. Recon is an
...
static const wchar_t *s_pl_pHistoryStrings[pHistoryStrings_SIZE] = {
    ...
    L"Przedłużono kontrakt z: %ls o 1 dzień.",    // Extented "mercs name"'s for a
                                                  // day
    // 11-15
    L"Przedłużono kontrakt z: %ls o 1 tydzień.",   // Extented "mercs name"'s for
                                                   // a week
    L"Przedłużono kontrakt z: %ls o 2 tygodnie.",  // Extented "mercs name"'s 2
                                                   // weeks
...
static const wchar_t s_pl_pErrorStrings[] =
...
static const wchar_t s_pl_pWebTitle[] = L"sir-FER 4.0";  // our name for the version of the browser,
...
static const wchar_t *s_pl_pPersonnelDepartedStateStrings[pPersonnelDepartedStateStrings_SIZE] = {
    ...
    L"Śmierć w akcji", L"Zwolnienie", L"Małżeństwo", L"Koniec kontraktu", L"Rezygnacja",
};
...
static const wchar_t
    *s_pl_pPersonnelCurrentTeamStatsStrings[pPersonnelCurrentTeamStatsStrings_SIZE] = {
        L"Najniższy",
        L"Średni",
        L"Najwyższy",
};
...
static const wchar_t *s_pl_InsInfoText[InsInfoText_SIZE] = {L"Wstecz", L"Dalej"};
...
static const wchar_t *s_pl_MercAccountText[MercAccountText_SIZE] = {
    ...
    L"Opłata",
    L"Razem:",
    L"Czy na pewno chcesz zatwierdzić płatność: %ls?",  // the %ls is a string
                                                        // that contains the
                                                        // dollar amount ( ex.
                                                        // "$150" )
};
...
static const wchar_t *s_pl_sFuneralString[sFuneralString_SIZE] = {
    L"Zakład pogrzebowy McGillicutty, pomaga rodzinom pogrążonym w smutku od "
    L"1983.",
    L"Kierownik, były najemnik A.I.M. Murray \'Pops\' McGillicutty jest "
    L"doświadczonym pracownikiem zakładu pogrzebowego.",
    L"Przez całe życie obcował ze śmiercią, 'Pops' wie jak trudne są te "
    L"chwile.",
    L"Zakład pogrzebowy McGillicutty oferuje szeroki zakres usług, od "
    L"duchowego wsparcia po rekonstrukcję silnie zniekształconych zwłok.",
    L"Pozwól by McGillicutty ci pomógł a twój ukochany będzie spoczywał w "
    L"pokoju.",
    ...
    // send flowers ).
    L"Niestety, z powodu śmierci w rodzinie, nie działają jeszcze wszystkie "
    L"elementy tej strony.",
...
static const wchar_t *s_pl_sFloristText[sFloristText_SIZE] = {
    ...
    L"Gwarantujemy dostawę w dowolny punkt na Ziemi, następnego dnia po "
    L"złożeniu zamówienia!",
    L"Oferujemy najniższe ceny na świecie!",
    L"Pokaż nam ofertę z niższą ceną, a dostaniesz w nagrodę tuzin róż, za "
    L"darmo!",
    L"Latająca flora, fauna i kwiaty od 1981.",
    L"Nasz ozdobiony bombowiec zrzuci twój bukiet w promieniu co najwyżej "
    L"dziesięciu mil od żądanego miejsca. Kiedy tylko zechcesz!",
    L"Pozwól nam zaspokoić twoje kwieciste fantazje.",
    L"Bruce, nasz światowej renomy projektant bukietów, zerwie dla ciebie "
    L"najświeższe i najwspanialsze kwiaty z naszej szklarni.",
...
static const wchar_t *s_pl_sOrderFormText[sOrderFormText_SIZE] = {
    ...
    L"Ze względu na rozmiar karteczek, tekst nie może zawierać więcej niż 75 "
    L"znaków.",
...
static const wchar_t *s_pl_sFloristCards[sFloristCards_SIZE] = {L"Kliknij swój wybór", L"Wstecz"};
...
static const wchar_t *s_pl_BobbyROrderFormText[BobbyROrderFormText_SIZE] = {
    ...
    L"Nie stać cię na to.",       // 20	// A popup message that to warn of not enough
                                  // money
    L"<BRAK>",                    // Gets displayed when there is no valid city selected
    L"Miejsce docelowe przesyłki: %ls. Potwierdzasz?",  // A popup that asks if
                                                        // the city selected is
                                                        // the correct one
    L"Waga przesyłki*",                                 // Displays the weight of the package
    L"* Min. Waga",  // Disclaimer states that there is a minimum weight for the
                     // package
    L"Dostawy",
};
...
static const wchar_t *s_pl_BobbyRText[BobbyRText_SIZE] = {
    ...
    L"Kliknij wybrane towary. Lewym klawiszem zwiększasz ilość towaru, a "
    L"prawym zmniejszasz. Gdy już skompletujesz swoje zakupy przejdź do "
    L"formularza zamówienia.",  // instructions on how to order
    ...
    L"* Stan: %",         // if the item is damaged, displays the percent function of
                          // the item

    // Popup that tells the player that they can only order 10 items at a time

    L"Przepraszamy za to utrudnienie, ale na jednym zamówieniu może się "
    L"znajdować tylko 10 pozycji! Jeśli potrzebujesz więcej, złóż kolejne "
    L"zamówienie.",

    // A popup that tells the user that they are trying to order more items then
    // the store has in stock

    L"Przykro nam. Chwilowo nie mamy tego więcej na magazynie. Proszę "
    L"spróbować później.",
 ...
 * ammo. */
static const wchar_t s_pl_str_bobbyr_guns_num_guns_that_use_ammo[] =
    L"Twój zespół posiada %d szt. broni do której pasuje amunicja tego typu";
...
static const wchar_t *s_pl_AimSortText[AimSortText_SIZE] = {
    ...
    L"Portrety najemników", L"Akta najemnika", L"Pokaż galerię byłych członków A.I.M."};
...
static const wchar_t *s_pl_CharacterInfo[CharacterInfo_SIZE] = {
    ...
    L"Wymagany jest zastaw na życie",  // If the merc required a medical deposit,
                                       // this is displayed
};
...
static const wchar_t *s_pl_VideoConfercingText[VideoConfercingText_SIZE] = {
    ...
    L"z zastawem"  // Displays if you are hiring the merc with the medical
                   // deposit
};
...
static const wchar_t *s_pl_AimPopUpText[AimPopUpText_SIZE] = {
    ...
    L"PRZEPROWADZENIE TRANSFERU NIE MOŻLIWE",  // Player doesn't have enough
                                               // money, message 1
...
static const wchar_t *s_pl_AimHistoryText[AimHistoryText_SIZE] = {
    ...
    L"Poprzednia str.", L"Strona główna", L"Byli członkowie", L"Następna str."};
...
static const wchar_t *s_pl_SKI_Text[SKI_SIZE] = {
    ...
    L"KOSZT OGÓŁEM",      // The total cost of the the items in the Dealer inventory
                          // area
    L"WARTOŚĆ OGÓŁEM",    // The total value of items player wishes to sell
    L"WYCENA",            // Button text for dealer to evaluate items the player wants to
                          // sell
    L"TRANSAKCJA",        // Button text which completes the deal. Makes the
                          // transaction.
    L"OK",                // Text for the button which will leave the shopkeeper interface.
    L"KOSZT NAPRAWY",     // The amount the dealer will charge to repair the merc's
                          // goods
    L"1 GODZINA",         // SINGULAR! The text underneath the inventory slot when an
                          // item is given to the dealer to be repaired
    L"%d GODZIN(Y)",      // PLURAL!   The text underneath the inventory slot when an
                          // item is given to the dealer to be repaired
    L"NAPRAWIONO",        // Text appearing over an item that has just been repaired by
                          // a NPC repairman dealer
    L"Brak miejsca by zaoferować więcej rzeczy.",  // Message box that tells the
                                                   // user there is no more room
                                                   // to put there stuff
    L"%d MINUT(Y)",  // The text underneath the inventory slot when an item is
...
static const wchar_t *s_pl_zSaveLoadText[zSaveLoadText_SIZE] = {
    ...
    L"Wersja gry w zapisanym pliku różni się od bieżącej. Prawdopodobnie można "
    L"bezpiecznie kontynuować. Kontynuować?",
    L"Zapisane pliki gier mogą być uszkodzone. Czy chcesz je usunąć?",

    L"Próba odczytu starszej wersji zapisu gry.  Zaktualizować ten zapis i "
    L"odczytać grę?",
    L"Próba odczytu starszej wersji zapisu gry.  Zaktualizować ten zapis i "
    L"odczytać grę?",
    ...
    L"Brak miejsca na dysku twardym.  Na dysku wolne jest %d MB, a wymagane "
    L"jest przynajmniej %d MB.",

    L"Zapisuję...",  // When saving a game, a message box with this string
...
static const wchar_t *s_pl_zMarksMapScreenText[zMarksMapScreenText_SIZE] = {
    L"Poziom mapy",
    L"Nie masz jeszcze żołnierzy samoobrony.  Musisz najpierw wytrenować "
    L"mieszkańców miast.",
    L"Dzienny przychód",
    ...
    // stuff for contracts
    L"Mimo, że możesz opłacić kontrakt, to jednak nie masz gotówki by opłacić "
    L"składkę ubezpieczeniową za najemnika.",
    L"%ls - składka ubezpieczeniowa najemnika będzie kosztować %ls za %d "
    L"dzień(dni). Czy chcesz ją opłacić?",
    L"Inwentarz sektora",
...
static const wchar_t *s_pl_pMilitiaConfirmStrings[pMilitiaConfirmStrings_SIZE] = {
    L"Szkolenie oddziału samoobrony będzie kosztowało $",   // telling
                                                            // player
                                                            // how
                                                            // much it
                                                            // will
                                                            // cost
    L"Zatwierdzasz wydatek?",                               // asking player if they wish to pay
                                                            // the amount requested
    L"Nie stać cię na to.",                                 // telling the player they can't afford
                                                            // to train this town
    L"Kontynuować szkolenie samoobrony w - %ls (%ls %d)?",  // continue
                                                            // training
                                                            // this
                                                            // town?
    L"Koszt $",                                             // the cost in dollars to train militia
    L"( T/N )",                                             // abbreviated yes/no
    L"Szkolenie samoobrony w %d sektorach będzie kosztowało $ %d. "
    L"%ls",  // cost to train sveral sectors at once
    L"Nie masz %d$, aby wyszkolić samoobronę w tym mieście.",
    L"%ls musi mieć %d%% lojalności, aby można było kontynuować "
    L"szkolenie samoobrony.",
...
static const wchar_t s_pl_gzCopyrightText[] =
...
static const wchar_t *s_pl_zOptionsScreenHelpText[zOptionsToggleText_SIZE] = {
    ...
    L"Jeśli napisy są włączone, opcja ta pozwoli ci spokojnie je przeczytać "
    L"podczas dialogu.",
    ...
    // Never move my mouse
    L"Wyłącz tę opcję, aby kursor myszki automatycznie ustawiał się nad "
    L"pojawiającymi się okienkami dialogowymi.",

    // Old selection method
    L"Włącz tę opcję, aby wybór postaci działał tak jak w poprzedniej wersji "
    L"gry.",
    ...
    L"Gdy opcja ta jest włączona, każdy ruch najemnika w trybie Real-Time "
    L"będzie wymagał dodatkowego, potwierdzającego kliknięcia.",

    // Sleep/Wake notification
    L"Gdy opcja ta jest włączona, wyświetlana będzie informacja, że najemnik "
    L"położył się spać lub wstał i wrócił do pracy.",

    // Use the metric system
    L"Gdy opcja ta jest włączona, gra używa systemu metrycznego.",

    // Merc Lighted movement
    L"Gdy opcja ta jest włączona, teren wokół najemnika będzie oświetlony "
    L"podczas ruchu. Wyłącz tę opcję, jeśli obniża płynność gry.",

    // Smart cursor
    L"Gdy opcja ta jest włączona, kursor automatycznie ustawia się na "
    L"najemnikach gdy znajdzie się w ich pobliżu.",

    // snap cursor to the door
    L"Gdy opcja ta jest włączona, kursor automatycznie ustawi się na drzwiach "
    L"gdy znajdzie się w ich pobliżu.",
    ...
    // toggle wireframe
    L"Gdy opcja ta jest włączona, wyświetlane są zarysy niewidocznych ścian. ( "
    L"|W )",

    L"Gdy opcja ta jest włączona, kursor ruchu wyświetlany jest w 3D. "
    L"(|H|o|m|e)",

};

static const wchar_t *s_pl_gzGIOScreenText[gzGIOScreenText_SIZE] = {L"POCZĄTKOWE USTAWIENIA GRY",
                                                                    L"Styl gry",
                                                                    L"Realistyczny",
                                                                    L"S-F",
                                                                    L"Opcje broni",
...
static const wchar_t s_pl_pSkillAtZeroWarning[] =
    L"Na pewno? Wartość zero oznacza brak jakichkolwiek umiejętności w tej "
    L"dziedzinie.";
...
static const wchar_t *s_pl_pMessageStrings[pMessageStrings_SIZE] = {
    ...
    L"strz/min",       // Abbreviation for Rounds per minute -- the potential # of
                       // bullets fired in a minute.
    ...
    L"%ls przybył(a) do sektora %ls%ls",  // Name/Squad has arrived in sector
                                          // A9.  Order must not change
                                          // without notifying SirTech
    L"Wersja",
    L"Wolna pozycja na szybki zapis",
    L"Ta pozycja zarezerwowana jest na szybkie zapisy wykonywane podczas "
    L"gry kombinacją klawiszy ALT+S.",
    L"Otw.",
    L"Zamkn.",
    L"Brak miejsca na dysku twardym.  Na dysku wolne jest %ls MB, a "
    L"wymagane jest przynajmniej %ls MB.",
    L"%ls złapał(a) %ls",                   //'Merc name' has caught 'item' -- let SirTech
    ...
    // When firing heavier weapons in close quarters, you may not have
    ...
    // Simple text indications that appear in the game, when the merc can do
    // one of these things.
    L"Upuść",
    L"Rzuć",
    L"Podaj",

    L"%ls przekazano do - %ls.",                          //"Item" passed to "merc".  Please try to
                                                          // keep the item %ls before the merc %ls,
                                                          // otherwise, must notify SirTech.
    L"Brak wolnego miejsca, by przekazać %ls do - %ls.",  // pass "item" to
                                                          // "merc".  Same
                                                          // instructions as
                                                          // above.

    // A list of attachments appear after the items.  Ex:  Kevlar vest (
    // Ceramic Plate 'Attached )'
    L" dołączono)",
    ...
    // Wireframes are shown through buildings to reveal doors and windows
    // that can't otherwise be seen in an isometric engine.  You can toggle
    // this mode freely in the game.
    L"Dodatkowe siatki włączone.",
    L"Dodatkowe siatki wyłączone.",

    // These are used in the cheat modes for changing levels in the game.
    // Going from a basement level to an upper level, etc.
    ...
    L" - ",  // used in the shop keeper inteface to mark the ownership of the
             // item eg Red's gun
    ...
    L"Oddział %d aktywny.",
    L"%ls - Nie stać cię by wypłacić jej/jemu dzienną pensję w wysokości "
    L"%ls.",  // first %ls is the mercs name, the seconds is a string
              // containing the salary
    L"Pomiń",
    L"%ls nie może odejść sam(a).",
    L"Utworzono zapis gry o nazwie SaveGame99.sav. W razie potrzeby zmień "
    L"jego nazwę na SaveGame01..10. Wtedy będzie można go odczytać ze "
    L"standardowego okna odczytu gry.",
    L"%ls wypił(a) trochę - %ls",
    L"Przesyłka dotarła do Drassen.",
    L"%ls przybędzie do wyznaczonego punktu zrzutu (sektor %ls) w dniu %d, "
    L"około godziny %ls.",  // first %ls is mercs name, next is the sector
...
static const wchar_t *s_pl_pMilitiaButtonsHelpText[pMilitiaButtonsHelpText_SIZE] = {
    L"Podnieś(Prawy klawisz myszy)/upuść(Lewy klawisz myszy) Zielonych "
    L"żołnierzy",  // button help text informing player they can pick up or
                   // drop militia with this button
    L"Podnieś(Prawy klawisz myszy)/upuść(Lewy klawisz myszy) "
    L"Doświadczonych żołnierzy",
...
static const wchar_t s_pl_pMapScreenJustStartedHelpText[] =
    L"Zajrzyj do A.I.M. i zatrudnij kilku najemników (*Wskazówka* musisz "
    L"otworzyć laptopa)";

static const wchar_t s_pl_pAntiHackerString[] =
...
static const wchar_t *s_pl_gzNonPersistantPBIText[gzNonPersistantPBIText_SIZE] = {
    ...
    L"Nie można automatycznie rozstrzygnąć walki, gdy walczysz ze "
    L"stworzeniami w kopalni.",
    L"Nie można automatycznie rozstrzygnąć walki, gdy w sektorze są wrodzy "
    L"cywile.",
    L"Nie można automatycznie rozstrzygnąć walki, gdy w sektorze są dzikie "
    L"koty.",
    L"TRWA WALKA",
...
static const wchar_t *s_pl_sRepairsDoneString[sRepairsDoneString_SIZE] = {
    ...
    L"%ls skończył(a) naprawiać broń i ochraniacze wszystkich członków "
    L"oddziału",
...
static const wchar_t *s_pl_zGioDifConfirmText[zGioDifConfirmText_SIZE] = {
    L"Wybrano opcję Nowicjusz. Opcja ta jest przeznaczona dla "
    L"niedoświadczonych graczy, lub dla tych, którzy nie mają ochoty na długie "
    L"i ciężkie walki. Pamiętaj, że opcja ta ma wpływ na przebieg całej gry. "
    L"Czy na pewno chcesz grać w trybie Nowicjusz?",
    L"Wybrano opcję Doświadczony. Opcja ta jest przenaczona dla graczy "
    L"posiadających już pewne doświadczenie w grach tego typu. Pamiętaj, że "
    L"opcja ta ma wpływ na przebieg całej gry. Czy na pewno chcesz grać w "
    L"trybie Doświadczony?",
    L"Wybrano opcję Ekspert. Jakby co, to ostrzegaliśmy cię. Nie obwiniaj nas, "
    L"jeśli wrócisz w plastikowym worku. Pamiętaj, że opcja ta ma wpływ na "
    L"przebieg całej gry. Czy na pewno chcesz grać w trybie Ekspert?",
};
...
static const wchar_t *s_pl_gzLateLocalizedString[gzLateLocalizedString_SIZE] = {
    ...
    L"%ls nie potrzebuje pierwszej pomocy lecz opieki lekarza lub dłuższego "
    L"odpoczynku.",
    ...
    L"Niebezpiecznie jest kompresować teraz czas, gdyż masz najemników w "
    L"sektorze %ls.",
    L"Niebezpiecznie jest kompresować teraz czas, gdyż masz najemników w "
    L"kopalni zaatakowanej przez robale.",
    ...
    // 39
    L"Żaden z twoich najemników nie jest w stanie walczyć.  Żołnierze "
    L"samoobrony sami będą walczyć z robalami.",
    ...
    L"W jednym sektorze, szkolenie samoobrony może prowadzić tylko %d "
    L"instruktor(ów).",
    L"Na pewno?",
...
static const wchar_t s_pl_str_ceramic_plates_smashed[] =
    L"%ls's ceramic plates have been smashed!";  // TODO translate

static const wchar_t s_pl_str_arrival_rerouted[] =
...
static const wchar_t s_pl_str_stat_explosive[] =
    L"Zn. mat. wyb.";  // Znajomość materiałów wybuchowych
static const wchar_t s_pl_str_stat_medical[] = L"Wiedza medyczna";

static const wchar_t *s_pl_str_stat_list[str_stat_list_SIZE] = {
    s_pl_str_stat_health,    s_pl_str_stat_agility,      s_pl_str_stat_dexterity,
    s_pl_str_stat_strength,  s_pl_str_stat_leadership,   s_pl_str_stat_wisdom,
    s_pl_str_stat_exp_level, s_pl_str_stat_marksmanship, s_pl_str_stat_mechanical,
    s_pl_str_stat_explosive, s_pl_str_stat_medical};

static const wchar_t *s_pl_str_aim_sort_list[str_aim_sort_list_SIZE] = {
    s_pl_str_aim_sort_price,     s_pl_str_aim_sort_experience, s_pl_str_aim_sort_marksmanship,
    s_pl_str_aim_sort_medical,   s_pl_str_aim_sort_explosives, s_pl_str_aim_sort_mechanical,
    s_pl_str_aim_sort_ascending, s_pl_str_aim_sort_descending,
};
