...
static void InitEditorItemStatsButtons(void) {
  ...
  iEditorButton[ITEMSTATS_HIDDEN_BTN] = CreateCheckBoxButton(
      485, y + 5, EDITORDIR "/smcheckbox.sti", MSYS_PRIORITY_NORMAL, ItemStatsToggleHideCallback);
  iEditorButton[ITEMSTATS_DELETE_BTN] =
      CreateTextButton(L"Delete", FONT10ARIAL, FONT_RED, FONT_BLACK, 600, y + 81, 36, 16,
...
static void MakeButtonDir(UINT idx, UINT dir, INT16 x, INT16 y) {
  ...
  GUIButtonRef const btn =
      CreateIconButton(giEditMercDirectionIcons[0], (dir + 1) % 8, x, TASKBAR_Y + y, 30, 30,
                       MSYS_PRIORITY_NORMAL + 1, MercsDirectionSetCallback);
  iEditorButton[idx] = btn;
  wchar_t str[30];
...
static void InitEditorMercsToolbar(void) {
  ...
  MakeCheck(MERCS_CREATURETOGGLE, 4, 42, MercsToggleCreatures, EDITORDIR "/smcheckbox.sti",
            L"Toggle viewing of creatures");
  MakeCheck(MERCS_REBELTOGGLE, 4, 62, MercsToggleRebels, EDITORDIR "/smcheckbox.sti",
            L"Toggle viewing of rebels");
  MakeCheck(MERCS_CIVILIANTOGGLE, 4, 82, MercsToggleCivilians, EDITORDIR "/smcheckbox.sti",
  ...
  iEditorButton[MERCS_DETAILEDCHECKBOX] = CreateCheckBoxButton(
      103, y + 5, EDITORDIR "/checkbox.sti", MSYS_PRIORITY_NORMAL, MercsDetailedPlacementCallback);

  MakeButton(MERCS_GENERAL, 100, 22, MercsGeneralModeCallback, EDITORDIR "/mercgeneral.sti",
             L"General information mode");
  MakeButton(MERCS_APPEARANCE, 134, 22, MercsAppearanceModeCallback,
             EDITORDIR "/mercappearance.sti", L"Physical appearance mode");
  MakeButton(MERCS_ATTRIBUTES, 100, 48, MercsAttributesModeCallback,
             EDITORDIR "/mercattributes.sti", L"Attributes mode");
  MakeButton(MERCS_INVENTORY, 134, 48, MercsInventoryModeCallback, EDITORDIR "/mercinventory.sti",
             L"Inventory mode");
  MakeButton(MERCS_PROFILE, 100, 74, MercsProfileModeCallback, EDITORDIR "/mercprofile.sti",
             L"Profile ID mode");
  MakeButton(MERCS_SCHEDULE, 134, 74, MercsScheduleModeCallback, EDITORDIR "/mercschedule.sti",
             L"Schedule mode");
  // Workaround for identical buttons.
  iEditorButton[MERCS_SCHEDULE]->uiFlags |= BUTTON_NO_DUPLICATE;
  MakeButton(MERCS_GLOWSCHEDULE, 134, 74, MercsScheduleModeCallback,
             EDITORDIR "/mercglowschedule.sti", L"Schedule mode");
  HideEditorButton(MERCS_GLOWSCHEDULE);

  iEditorButton[MERCS_DELETE] =
      MakeTextButton(L"DELETE", FONT_DKBLUE, 600, 2, 40, 20, MercsDeleteCallback);
  iEditorButton[MERCS_DELETE]->SetFastHelpText(L"Delete currently selected merc (DEL).");
  iEditorButton[MERCS_NEXT] =
      MakeTextButton(L"NEXT", FONT_DKBLUE, 600, 22, 40, 20, MercsNextCallback);
  iEditorButton[MERCS_NEXT]->SetFastHelpText(L"Find next merc (SPACE).");

  // Priority Existance
  MakeCheck(MERCS_PRIORITYEXISTANCE_CHECKBOX, 170, 5, MercsPriorityExistanceCallback,
            EDITORDIR "/checkbox.sti", L"Toggle priority existance");

  // If merc has keys
  MakeCheck(MERCS_HASKEYS_CHECKBOX, 170, 30, MercsHasKeysCallback, EDITORDIR "/checkbox.sti",
            L"Toggle whether or not placement has/naccess to all doors.");

  // Orders
  iEditorButton[MERCS_ORDERS_STATIONARY] =
      MakeTextButton(L"STATIONARY", FONT_GRAY2, 200, 8, 70, 12, MercsSetOrdersCallback);
  iEditorButton[MERCS_ORDERS_ONGUARD] =
      MakeTextButton(L"ON GUARD", FONT_GRAY2, 200, 20, 70, 12, MercsSetOrdersCallback);
  iEditorButton[MERCS_ORDERS_ONCALL] =
      MakeTextButton(L"ON CALL", FONT_GRAY2, 200, 32, 70, 12, MercsSetOrdersCallback);
  iEditorButton[MERCS_ORDERS_SEEKENEMY] =
      MakeTextButton(L"SEEK ENEMY", FONT_GRAY2, 200, 44, 70, 12, MercsSetOrdersCallback);
  iEditorButton[MERCS_ORDERS_CLOSEPATROL] =
      MakeTextButton(L"CLOSE PATROL", FONT_GRAY2, 270, 8, 70, 12, MercsSetOrdersCallback);
  iEditorButton[MERCS_ORDERS_FARPATROL] =
      MakeTextButton(L"FAR PATROL", FONT_GRAY2, 270, 20, 70, 12, MercsSetOrdersCallback);
  ...
  iEditorButton[MERCS_ATTITUDE_DEFENSIVE] =
      MakeTextButton(L"DEFENSIVE", FONT_GRAY4, 200, 64, 70, 12, MercsSetAttitudeCallback);
  iEditorButton[MERCS_ATTITUDE_BRAVESOLO] =
      MakeTextButton(L"BRAVE SOLO", FONT_GRAY4, 200, 76, 70, 12, MercsSetAttitudeCallback);
  iEditorButton[MERCS_ATTITUDE_BRAVEAID] =
      MakeTextButton(L"BRAVE AID", FONT_GRAY4, 200, 88, 70, 12, MercsSetAttitudeCallback);
  iEditorButton[MERCS_ATTITUDE_AGGRESSIVE] =
      MakeTextButton(L"AGGRESSIVE", FONT_GRAY4, 270, 64, 70, 12, MercsSetAttitudeCallback);
  ...
  iEditorButton[MERCS_DIRECTION_FIND] =
      CreateTextButton(L"Find", FONT12POINT1, FONT_MCOLOR_BLACK, FONT_BLACK, 390, y + 35, 30, 30,
                       MSYS_PRIORITY_NORMAL + 1, MercsFindSelectedMercCallback);
  ...
  iEditorButton[MERCS_CIVILIAN_GROUP] =
      MakeTextButton(gszCivGroupNames[0], FONT_YELLOW, 574, 50, 60, 25, MercsCivilianGroupCallback);
  ...
  MakeButton(MERCS_BODYTYPE_DOWN, 460, 4, MercsSetBodyTypeCallback, EDITORDIR "/leftarrow.sti",
             L"Previous body type");
  MakeButton(MERCS_BODYTYPE_UP, 560, 4, MercsSetBodyTypeCallback, EDITORDIR "/rightarrow.sti",
             L"Next body type");

  MakeCheck(MERCS_SCHEDULE_VARIANCE1, 309, 15, MercsScheduleToggleVariance1Callback,
            EDITORDIR "/smcheckbox.sti", L"Toggle time variance (+ or - 15 minutes)");
  MakeCheck(MERCS_SCHEDULE_VARIANCE2, 309, 36, MercsScheduleToggleVariance2Callback,
            EDITORDIR "/smcheckbox.sti", L"Toggle time variance (+ or - 15 minutes)");
  MakeCheck(MERCS_SCHEDULE_VARIANCE3, 309, 57, MercsScheduleToggleVariance3Callback,
            EDITORDIR "/smcheckbox.sti", L"Toggle time variance (+ or - 15 minutes)");
  MakeCheck(MERCS_SCHEDULE_VARIANCE4, 309, 78, MercsScheduleToggleVariance4Callback,
            EDITORDIR "/smcheckbox.sti", L"Toggle time variance (+ or - 15 minutes)");

  MakeButtonSchedule(MERCS_SCHEDULE_ACTION1, 186, 13, 77, 16, MercsScheduleAction1Callback,
                     L"No action");
  MakeButtonSchedule(MERCS_SCHEDULE_ACTION2, 186, 34, 77, 16, MercsScheduleAction2Callback,
                     L"No action");
  MakeButtonSchedule(MERCS_SCHEDULE_ACTION3, 186, 55, 77, 16, MercsScheduleAction3Callback,
                     L"No action");
  MakeButtonSchedule(MERCS_SCHEDULE_ACTION4, 186, 76, 77, 16, MercsScheduleAction4Callback,
                     L"No action");
  ...
  MakeButtonSchedule(MERCS_SCHEDULE_CLEAR, 516, 2, 77, 16, MercsScheduleClearCallback,
                     L"Clear Schedule");
...
static void InitEditorBuildingsToolbar(void) {
  ...
  iEditorButton[BUILDING_TOGGLE_WALL_VIEW] =
      MakeTextButton(L"WALLS", FONT_YELLOW, 110, 55, 50, 15, BuildingToggleWallViewCallback);
  iEditorButton[BUILDING_TOGGLE_INFO_VIEW] =
      MakeTextButton(L"ROOM INFO", FONT_YELLOW, 110, 70, 50, 15, BuildingToggleInfoViewCallback);

  // Selection method buttons
  MakeButton(BUILDING_PLACE_WALLS, 180, 10, BuildingWallCallback, EDITORDIR "/wall.sti",
             L"Place walls using selection method");
  MakeButton(BUILDING_PLACE_DOORS, 210, 10, BuildingDoorCallback, EDITORDIR "/door.sti",
             L"Place doors using selection method");
  MakeButton(BUILDING_PLACE_ROOFS, 240, 10, BuildingRoofCallback, EDITORDIR "/roof.sti",
             L"Place roofs using selection method");
  MakeButton(BUILDING_PLACE_WINDOWS, 180, 40, BuildingWindowCallback, EDITORDIR "/window.sti",
             L"Place windows using selection method");
  MakeButton(BUILDING_PLACE_BROKEN_WALLS, 210, 40, BuildingCrackWallCallback,
             EDITORDIR "/crackwall.sti", L"Place damaged walls using selection method.");
  MakeButton(BUILDING_PLACE_FURNITURE, 240, 40, BuildingFurnitureCallback, EDITORDIR "/decor.sti",
             L"Place furniture using selection method");
  MakeButton(BUILDING_PLACE_DECALS, 180, 70, BuildingDecalCallback, EDITORDIR "/decal.sti",
             L"Place wall decals using selection method");
  MakeButton(BUILDING_PLACE_FLOORS, 210, 70, BuildingFloorCallback, EDITORDIR "/floor.sti",
             L"Place floors using selection method");
  MakeButton(BUILDING_PLACE_TOILETS, 240, 70, BuildingToiletCallback, EDITORDIR "/toilet.sti",
             L"Place generic furniture using selection method");

  // Smart method buttons
  MakeButton(BUILDING_SMART_WALLS, 290, 10, BuildingSmartWallCallback, EDITORDIR "/wall.sti",
             L"Place walls using smart method");
  MakeButton(BUILDING_SMART_DOORS, 320, 10, BuildingSmartDoorCallback, EDITORDIR "/door.sti",
             L"Place doors using smart method");
  MakeButton(BUILDING_SMART_WINDOWS, 290, 40, BuildingSmartWindowCallback, EDITORDIR "/window.sti",
             L"Place windows using smart method");
  MakeButton(BUILDING_SMART_BROKEN_WALLS, 320, 40, BuildingSmartCrackWallCallback,
             EDITORDIR "/crackwall.sti", L"Place damaged walls using smart method");
  MakeButton(BUILDING_DOORKEY, 290, 70, BuildingDoorKeyCallback, EDITORDIR "/key.sti",
             L"Lock or trap existing doors");

  MakeButton(BUILDING_NEW_ROOM, 370, 10, BuildingNewRoomCallback, EDITORDIR "/newroom.sti",
             L"Add a new room");
  MakeButton(BUILDING_CAVE_DRAWING, 370, 10, BuildingCaveDrawingCallback, EDITORDIR "/caves.sti",
             L"Edit cave walls.");
  MakeButton(BUILDING_SAW_ROOM, 370, 40, BuildingSawRoomCallback, EDITORDIR "/sawroom.sti",
             L"Remove an area from existing building.");
  MakeButton(BUILDING_KILL_BUILDING, 370, 70, BuildingKillBuildingCallback,
             EDITORDIR "/delroom.sti", L"Remove a building");
  MakeButton(BUILDING_NEW_ROOF, 400, 70, BuildingNewRoofCallback, EDITORDIR "/newroof.sti",
             L"Add/replace building's roof with new flat roof.");
  MakeButton(BUILDING_COPY_BUILDING, 430, 70, BuildingCopyBuildingCallback,
             EDITORDIR "/copyroom.sti", L"Copy a building");
  MakeButton(BUILDING_MOVE_BUILDING, 460, 70, BuildingMoveBuildingCallback,
             EDITORDIR "/moveroom.sti", L"Move a building");
  MakeButton(BUILDING_DRAW_ROOMNUM, 410, 10, BuildingDrawRoomNumCallback,
             EDITORDIR "/addtileroom.sti", L"Draw room number");
  MakeButton(BUILDING_ERASE_ROOMNUM, 440, 10, BuildingEraseRoomNumCallback,
             EDITORDIR "/killtileroom.sti", L"Erase room numbers");

  MakeButton(BUILDING_TOGGLE_ERASEMODE, 500, 40, BtnEraseCallback, EDITORDIR "/eraser.sti",
             L"Toggle erase mode");

  MakeButton(BUILDING_UNDO, 530, 40, BtnUndoCallback, EDITORDIR "/undo.sti", L"Undo last change");
  MakeButton(BUILDING_CYCLE_BRUSHSIZE, 500, 70, BtnBrushCallback, EDITORDIR "/paint.sti",
             L"Cycle brush size");
}
...
static void InitEditorItemsToolbar(void) {
  ...
  iEditorButton[ITEMS_AMMO] =
      CreateTextButton(L"Ammo", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 159, y + 80, 40, 20,
                       MSYS_PRIORITY_NORMAL, ItemsAmmoCallback);
  iEditorButton[ITEMS_ARMOUR] =
      CreateTextButton(L"Armour", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 199, y + 80, 52, 20,
                       MSYS_PRIORITY_NORMAL, ItemsArmourCallback);
  iEditorButton[ITEMS_EXPLOSIVES] =
      CreateTextButton(L"Explosives", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 251, y + 80, 69,
                       20, MSYS_PRIORITY_NORMAL, ItemsExplosivesCallback);
  iEditorButton[ITEMS_EQUIPMENT1] =
      CreateTextButton(L"E1", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 320, y + 80, 21, 20,
                       MSYS_PRIORITY_NORMAL, ItemsEquipment1Callback);
  iEditorButton[ITEMS_EQUIPMENT2] =
      CreateTextButton(L"E2", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 341, y + 80, 21, 20,
                       MSYS_PRIORITY_NORMAL, ItemsEquipment2Callback);
  iEditorButton[ITEMS_EQUIPMENT3] =
      CreateTextButton(L"E3", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 362, y + 80, 21, 20,
                       MSYS_PRIORITY_NORMAL, ItemsEquipment3Callback);
  iEditorButton[ITEMS_TRIGGERS] =
      CreateTextButton(L"Triggers", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 383, y + 80, 59, 20,
                       MSYS_PRIORITY_NORMAL, ItemsTriggersCallback);
  iEditorButton[ITEMS_KEYS] =
      CreateTextButton(L"Keys", BLOCKFONT, FONT_MCOLOR_DKWHITE, FONT_BLACK, 442, y + 80, 38, 20,
                       MSYS_PRIORITY_NORMAL, ItemsKeysCallback);
...
static void InitEditorMapInfoToolbar(void) {
  ...
  MakeButton(MAPINFO_TOGGLE_FAKE_LIGHTS, 120, 2, BtnFakeLightCallback, EDITORDIR "/fakelight.sti",
             L"Toggle fake ambient lights.");

  iEditorButton[MAPINFO_RADIO_PANEL] =
      CreateLabel(NULL, 0, 0, 0, 207, y + 2, 70, 50, MSYS_PRIORITY_NORMAL);
  ...
  MakeButton(MAPINFO_DRAW_EXITGRIDS, 305, 12, MapInfoDrawExitGridCallback,
             EDITORDIR "/exitgridbut.sti", L"Add exit grids (r-clk to query existing).");
  MakeButton(MAPINFO_CYCLE_BRUSHSIZE, 420, 70, BtnBrushCallback, EDITORDIR "/paint.sti",
             L"Cycle brush size");
  MakeButton(MAPINFO_UNDO, 510, 70, BtnUndoCallback, EDITORDIR "/undo.sti", L"Undo last change");
  MakeButton(MAPINFO_TOGGLE_ERASEMODE, 540, 70, BtnEraseCallback, EDITORDIR "/eraser.sti",
             L"Toggle erase mode");
  MakeButton(MAPINFO_NORTH_POINT, 540, 5, MapInfoEntryPointsCallback, EDITORDIR "/north.sti",
             L"Specify north point for validation purposes.");
  MakeButton(MAPINFO_WEST_POINT, 525, 26, MapInfoEntryPointsCallback, EDITORDIR "/west.sti",
             L"Specify west point for validation purposes.");
  MakeButton(MAPINFO_EAST_POINT, 555, 26, MapInfoEntryPointsCallback, EDITORDIR "/east.sti",
             L"Specify east point for validation purposes.");
  MakeButton(MAPINFO_SOUTH_POINT, 540, 47, MapInfoEntryPointsCallback, EDITORDIR "/south.sti",
             L"Specify south point for validation purposes.");
  MakeButton(MAPINFO_CENTER_POINT, 590, 15, MapInfoEntryPointsCallback, EDITORDIR "/center.sti",
             L"Specify center point for validation purposes.");
  MakeButton(MAPINFO_ISOLATED_POINT, 590, 36, MapInfoEntryPointsCallback, EDITORDIR "/isolated.sti",
...
static void InitEditorOptionsToolbar(void) {
  ...
  MakeButton(OPTIONS_NEW_BASEMENT, 101, 41, BtnNewBasementCallback, EDITORDIR "/new.sti",
             L"New basement");
  MakeButton(OPTIONS_NEW_CAVES, 131, 41, BtnNewCavesCallback, EDITORDIR "/new.sti",
             L"New cave level");
  ...
  MakeButton(OPTIONS_CHANGE_TILESET, 221, 41, BtnChangeTilesetCallback, EDITORDIR "/tileset.sti",
             L"Select tileset");
  MakeButton(OPTIONS_LEAVE_EDITOR, 251, 41, BtnCancelCallback, EDITORDIR "/cancel.sti",
             L"Leave Editor mode");
...
static void InitEditorTerrainToolbar(void) {
  ...
  MakeButton(TERRAIN_PLACE_CLIFFS, 160, 40, BtnBanksCallback, EDITORDIR "/banks.sti",
             L"Place banks and cliffs");
  MakeButton(TERRAIN_PLACE_ROADS, 190, 40, BtnRoadsCallback, EDITORDIR "/road.sti", L"Draw roads");
  MakeButton(TERRAIN_PLACE_DEBRIS, 220, 40, BtnDebrisCallback, EDITORDIR "/debris.sti",
             L"Draw debris");
  MakeButton(TERRAIN_PLACE_TREES, 250, 40, BtnObjectCallback, EDITORDIR "/tree.sti",
             L"Place trees & bushes");
  MakeButton(TERRAIN_PLACE_ROCKS, 280, 40, BtnObject1Callback, EDITORDIR "/num1.sti",
             L"Place rocks");
  MakeButton(TERRAIN_PLACE_MISC, 310, 40, BtnObject2Callback, EDITORDIR "/num2.sti",
  ...
  MakeButton(TERRAIN_TOGGLE_ERASEMODE, 160, 70, BtnEraseCallback, EDITORDIR "/eraser.sti",
             L"Toggle erase mode");
  MakeButton(TERRAIN_CYCLE_BRUSHSIZE, 190, 70, BtnBrushCallback, EDITORDIR "/paint.sti",
             L"Cycle brush size");
  MakeButton(TERRAIN_RAISE_DENSITY, 280, 70, BtnIncBrushDensityCallback, EDITORDIR "/uparrow.sti",
             L"Raise brush density");
  MakeButton(TERRAIN_LOWER_DENSITY, 350, 70, BtnDecBrushDensityCallback, EDITORDIR "/downarrow.sti",
