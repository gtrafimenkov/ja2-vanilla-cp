...
void BuildingToggleRoofViewCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    // Name could be a bit misleading.  It'll hide or show the roofs based on
    // the fBuildingShowRoofs value.
    UpdateRoofsView();
  }
}
...
void BuildingToggleWallViewCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    // Name could be a bit misleading.  It'll hide or show the walls based on
    // the fBuildingShowWalls value.
    UpdateWallsView();
  }
}
...
static void ItemsScrollRight() {
  int16_t const max_col = std::max(((eInfo.sNumItems + 1) / 2) - 6, 0);
...
void MercsInventorySlotCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    int32_t const uiSlot = btn->GetUserData();
...
void MercsSetEnemyColorCodeCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    uint8_t const ubColorCode = btn->GetUserData();
...
void MercsSetOrdersCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) SetMercOrders(btn->GetUserData());
}

// multiple attitude buttons
void MercsSetAttitudeCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) SetMercAttitude(btn->GetUserData());
}

// multiple direction buttons
void MercsDirectionSetCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) SetMercDirection(btn->GetUserData());
}
...
void MercsSetRelativeEquipmentCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) SetMercRelativeEquipment(btn->GetUserData());
}

void MercsSetRelativeAttributesCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) SetMercRelativeAttributes(btn->GetUserData());
}
