...
// array which keeps track of which item is in which slot.  This is dependant on
// the selected merc, so these temp values must be updated when different mercs
// are selected, and reset when a merc detailed placement is created.
OBJECTTYPE *gpMercSlotItem[9] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
// Because we only support these nine slots, they aren't continuous values, so
// this array helps processing functions by referring to this array to get the
// appropriate slot.
...
// values indicating which merc inventory slot is hilited and which slot is
// selected.
...
static uint8_t gubPrevMercMode = MERC_NOMODE;
...
// Determines if the user is allowed to edit merc colors.  User must
// specifically click on the checkbox by the colors to confirm that colors will
// be specified.  If not, the colors will be randomly generated upon creation in
// the game.
BOOLEAN gfCanEditMercColors = FALSE;
// A rendering flag that is set whenever a full update of the merc editing
// information needs to be done.
BOOLEAN gfRenderMercInfo = FALSE;
// When the user specifies a profile index for the merc, all editing is
// disabled.  This is because the profile contains all of the information.  When
// this happens, all of the editing buttons are disabled, but you are allowed to
// view stats, inventory, etc., as well as specify orders, attitude, and
// direction.
BOOLEAN gfCanEditMercs = TRUE;
// When in inventory mode, this flag is set when the user wishes to get an item,
// which requires hooking into the item editing features.  This is processed
// during the editor update, which in turn, sets the new mode.
BOOLEAN gfMercGetItem = FALSE;
// As soon as an item is selected, the items index is stored here, so the item
// can be copied into the slot for editing and rendering purposes.  This is a
// temp store value only when leaving the editor items mode.
...
static SoldierBodyType const bEnemyArray[] = {BODY_RANDOM, REGMALE, BIGMALE, STOCKYMALE,
                                              REGFEMALE,   TANK_NW, TANK_NE};
static SoldierBodyType const bCreatureArray[] = {BLOODCAT,    LARVAE_MONSTER, INFANT_MONSTER,
                                                 YAF_MONSTER, YAM_MONSTER,    ADULTFEMALEMONSTER,
                                                 AM_MONSTER,  QUEENMONSTER};
static SoldierBodyType const bRebelArray[] = {BODY_RANDOM, FATCIV,     MANCIV,   REGMALE,
                                              BIGMALE,     STOCKYMALE, REGFEMALE};
static SoldierBodyType const bCivArray[] = {
    BODY_RANDOM, FATCIV,        MANCIV,  MINICIV,    DRESSCIV,      HATKIDCIV,
    KIDCIV,      REGMALE,       BIGMALE, STOCKYMALE, REGFEMALE,     HUMVEE,
    ELDORADO,    ICECREAMTRUCK, JEEP,    CRIPPLECIV, ROBOTNOWEAPON, COW};
...
void ProcessMercEditing() {
  ...
  uint8_t ubPaletteRep = GetPaletteRepIndexFromID(soldier_pal);
  const int32_t start = iEditColorStart[ubType];
  const uint8_t range = gubpNumReplacementsPerRange[ubType];
  if (iEditWhichStat & 1) {
...
void AddMercToWorld(int32_t iMapIndex) {
  ...
  if (IsLocationSittable(iMapIndex, gfRoofPlacement)) {
    ...
    // Generate detailed placement information given the temp placement
    // information.
...
static SOLDIERTYPE *IsMercHere(int32_t iMapIndex);

void HandleRightClickOnMerc(int32_t iMapIndex) {
  const SOLDIERTYPE *const this_merc = IsMercHere(iMapIndex);
  if (this_merc != NULL) {
    if (g_selected_merc != this_merc) {  // We want to edit a new merc (or different merc)
      // We need to avoid the editing of player mercs.
      if (!FindSoldierInitNodeBySoldier(*this_merc))
        return;  // this is a player merc (which isn't in the list), or an error
                 // in logic.
      IndicateSelectedMerc(this_merc->ubID);
    }
  } else if (g_selected_merc != NULL &&
             IsLocationSittable(iMapIndex, gfRoofPlacement))  // We want to move the selected
                                                              // merc to this new location.
  {
...
void AddMercWaypoint(uint32_t iMapIndex) {
  ...
  if (g_selected_merc == NULL || g_selected_merc->ubID <= gTacticalStatus.Team[OUR_TEAM].bLastID ||
      g_selected_merc->ubID >= MAXMERCS)
    return;
...
//----------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------
//	The following are the button callback functions for the merc editing
// pages
//----------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------
...
static void ChangeBodyType(int8_t bOffset);
...
static void ShowEditMercColorSet(uint8_t ubPaletteRep, int16_t sSet);

//	Displays the palette of the given merc (used by the edit merc color
// page)
...
void DisplayWayPoints() {
  ...
  if (pSoldier == NULL || pSoldier->ubID <= gTacticalStatus.Team[OUR_TEAM].bLastID ||
      pSoldier->ubID >= MAXMERCS)
    return;
...
void SetMercRelativeAttributes(int8_t bLevel) {
  ...
  // We also have to modify the existing soldier incase the user wishes to enter
  // game.
...
void IndicateSelectedMerc(int16_t sID) {
  ...
  // updates the edited merc.  If this call isn't here, it is possible to update
  // the newly selected merc with the wrong information.
  ...
  switch (sID) {
    case SELECT_NEXT_MERC:
      prev = gpSelected;
      if (g_selected_merc == NULL || !gpSelected) {  // if no merc selected, then select the first
                                                     // one in list.
        ...
        } else {  // we are at the end of the list, so select the first merc in the
                  // list.
          gpSelected = gSoldierInitHead;
        }
      }
  ...
  if (bTeam != -1) {  // We are searching for the next occurence of a particular team.
    prev = gpSelected;
    if (g_selected_merc == NULL || !gpSelected) {  // if no merc selected, then select the first one
                                                   // in list.
      gpSelected = gSoldierInitHead;
    } else {                   // validate this merc in the list.
      if (gpSelected->next) {  // select the next merc in the list
    ...
    while (gpSelected != prev) {
      ...
      if (gpSelected->pSoldier && gpSelected->pSoldier->bVisible == 1 &&
          gpSelected->pSoldier->bTeam == bTeam) {  // we have found a visible soldier on the desired
                                                   // team, so select him.
        break;
      }
      gpSelected = gpSelected->next;
    ...
    if (gpSelected == prev) {  // we have cycled through the list already, so choose the same
                               // guy (if he is on the desired team)...
      if (!gpSelected->pSoldier || gpSelected->pSoldier->bVisible != 1 ||
  ...
  if (sID < 0) {  // We want to center the screen on the next merc, and update
                  // the interface.
  ...
  // Whatever the case, we want to update the gui to press the appropriate
  // buttons depending on the merc's attributes. Click the appropriate team
  // button
...
static void CalcStringForValue(wchar_t *str, size_t length, int32_t iValue, uint32_t uiMax);
...
// In the merc editing, all detailed placement values for generated attributes
// are set to -1. When making a generated attribute static, we then set the value
// to its applicable value. This function is similar to the itoa function except
// that -1 is converted to a null string.
static void CalcStringForValue(wchar_t *str, size_t length, int32_t iValue, uint32_t uiMax) {
...
static void ExtractAndUpdateMercAttributes() {
  ...
  // It just so happens that GetNumericStrict...() will return -1 for any blank
  // fields. -1 values in the detailed placement work nicely, because that
  // signifies that specific field isn't static.  Any other value becomes static,
  // and static values override any generated values.
  gpSelected->pDetailedPlacement->bExpLevel =
      (int8_t)std::min(GetNumericStrictValueFromField(0), 100);
  gpSelected->pDetailedPlacement->bLife = (int8_t)std::min(GetNumericStrictValueFromField(1), 100);
  gpSelected->pDetailedPlacement->bLifeMax =
      (int8_t)std::min(GetNumericStrictValueFromField(2), 100);
  gpSelected->pDetailedPlacement->bMarksmanship =
      (int8_t)std::min(GetNumericStrictValueFromField(3), 100);
  gpSelected->pDetailedPlacement->bStrength =
      (int8_t)std::min(GetNumericStrictValueFromField(4), 100);
  gpSelected->pDetailedPlacement->bAgility =
      (int8_t)std::min(GetNumericStrictValueFromField(5), 100);
  gpSelected->pDetailedPlacement->bDexterity =
      (int8_t)std::min(GetNumericStrictValueFromField(6), 100);
  gpSelected->pDetailedPlacement->bWisdom =
      (int8_t)std::min(GetNumericStrictValueFromField(7), 100);
  gpSelected->pDetailedPlacement->bLeadership =
      (int8_t)std::min(GetNumericStrictValueFromField(8), 100);
  gpSelected->pDetailedPlacement->bExplosive =
      (int8_t)std::min(GetNumericStrictValueFromField(9), 100);
  gpSelected->pDetailedPlacement->bMedical =
      (int8_t)std::min(GetNumericStrictValueFromField(10), 100);
  gpSelected->pDetailedPlacement->bMechanical =
      (int8_t)std::min(GetNumericStrictValueFromField(11), 100);
  gpSelected->pDetailedPlacement->bMorale =
      (int8_t)std::min(GetNumericStrictValueFromField(11), 100);

  // make sure that experience level ranges between 1 and 9
  if (gpSelected->pDetailedPlacement->bExpLevel != -1)
    gpSelected->pDetailedPlacement->bExpLevel =
        std::max(std::min(gpSelected->pDetailedPlacement->bExpLevel, (int8_t)9), (int8_t)1);
...
static void ExtractAndUpdateMercProfile() {
  ...
  sNum = (int16_t)std::min(GetNumericStrictValueFromField(0), NUM_PROFILES);
  if (sNum == -1) {
...
void ExtractAndUpdateMercSchedule() {
  ...
  if (!gpSelected->pSoldier->ubScheduleID) {  // The soldier doesn't actually have a schedule
                                              // yet, so create one if necessary (not blank)
...
static void ChangeBodyType(int8_t const offset) {
  ...
  switch (bp.bTeam) {
    case ENEMY_TEAM:
      body_types = bEnemyArray;
      n = lengthof(bEnemyArray);
      break;
    case CREATURE_TEAM:
      body_types = bCreatureArray;
      n = lengthof(bCreatureArray);
      break;
    case MILITIA_TEAM:
      body_types = bRebelArray;
      n = lengthof(bRebelArray);
      break;
    case CIV_TEAM:
      body_types = bCivArray;
      n = lengthof(bCivArray);
      break;
    default:
      abort();  // HACK000E
  }
  int32_t next = 0;  // XXX HACK000E
...
// There are 4 exclusive entry points in a map.  Only one of each type can exist
// on a map, and these points are used to validate the map by attempting to
// connect the four points together.  If one of the points is isolated, then the
// map will be rejected.  It isn't necessary to specify all four points.  You
// wouldn't want to specify a north point if there isn't going to be any
// traversing to adjacent maps from that side.
...
void SetMercEditingMode(uint8_t ubNewMode) {
  ...
  // Depending on the mode we were just in, we may want to extract and update
  // the merc first.  Then we change modes...
  ...
  // attributes, which was just handled above, then turn everything off, and
  // exit.
  ...
  if (g_selected_merc != NULL && ubNewMode == MERC_TEAMMODE) {
    // attempt to weed out conditions where we select a team that matches the
    // currently selected merc.  We don't want to deselect him in this case.
    if ((gpSelected->pSoldier->bTeam == ENEMY_TEAM && iDrawMode == DRAW_MODE_ENEMY) ||
        ...
        (gpSelected->pSoldier->bTeam == CIV_TEAM &&
         iDrawMode == DRAW_MODE_CIVILIAN)) {  // Same team, so don't deselect merc.
                                              // Instead, keep the previous editing mode
      ...
      // Different teams, so deselect the current merc and the detailed checkbox
      // if applicable.
      IndicateSelectedMerc(SELECT_NO_MERC);
      ShowEditorButtons(FIRST_MERCS_BUTTON, LAST_MERCS_TEAMMODE_BUTTON);
  ...
  if (gubCurrMercMode > MERC_BASICMODE) {  // Add the detailed buttons if applicable.
    ...
    UnclickEditorButton(MERCS_DETAILEDCHECKBOX);
  // Now we are setting up the button states for the new mode, as well as show
  // the applicable buttons for the detailed placement modes.
  ...
  switch (gubCurrMercMode) {
    ...
    case MERC_BASICMODE:
      ...
      if (iDrawMode == DRAW_MODE_CREATURE) {  // Set up alternate general mode.  This one
                                              // doesn't allow you to specify relative attributes
    ...
    case MERC_GENERALMODE:
      ...
      if (iDrawMode == DRAW_MODE_CREATURE) {  // Set up alternate general mode.  This one
                                              // doesn't allow you to specify relative equipment
        // but requires you to specify a body type.
        HideEditorButtons(FIRST_MERCS_REL_EQUIPMENT_BUTTON, LAST_MERCS_REL_EQUIPMENT_BUTTON);
        ShowEditorButtons(FIRST_MERCS_BODYTYPE_BUTTON, LAST_MERCS_BODYTYPE_BUTTON);
...
void UpdateMercsInfo() {
  ...
  switch (gubCurrMercMode) {
    ...
    case MERC_PROFILEMODE: {  // scope trick
      wchar_t tempStr[500];
      swprintf(tempStr, lengthof(tempStr),
               L"By specifying a profile index, all of the information will be "
               L"extracted from the profile "
               L"and override any values that you have edited.  It will also "
               L"disable the editing features "
               L"though, you will still be able to view stats, etc.  Pressing "
               L"ENTER will automatically "
               L"extract the number you have typed.  A blank field will clear "
               L"the profile.  The current "
               L"number of profiles range from 0 to %d.",
               NUM_PROFILES);
      ...
      if (gpSelected->pDetailedPlacement->ubProfile == NO_PROFILE) {
        ...
        mprintf(240, 435, L"Current Profile:  %ls",
    ...
    case MERC_SCHEDULEMODE:
      ...
      switch (gpSelected->pSoldier->bOrders) {
        case STATIONARY:
          MPrint(430, 363, L"STATIONARY");
          break;
        case ONCALL:
          MPrint(430, 363, L"ON CALL");
          break;
        case ONGUARD:
          MPrint(430, 363, L"ON GUARD");
          break;
        case SEEKENEMY:
          MPrint(430, 363, L"SEEK ENEMY");
          break;
        case CLOSEPATROL:
          MPrint(430, 363, L"CLOSE PATROL");
          break;
        case FARPATROL:
          MPrint(430, 363, L"FAR PATROL");
          break;
        case POINTPATROL:
          MPrint(430, 363, L"POINT PATROL");
          break;
        case RNDPTPATROL:
          MPrint(430, 363, L"RND PT PATROL");
          break;
      }
      SetFontForeground(FONT_YELLOW);
        ...
        switch (gubScheduleInstructions) {
          case SCHEDULE_INSTRUCTIONS_DOOR1:
            swprintf(str, lengthof(str),
                     L"Click on the gridno adjacent to the door that you wish to %ls.", keyword);
            break;
          case SCHEDULE_INSTRUCTIONS_DOOR2:
            swprintf(str, lengthof(str),
                     L"Click on the gridno where you wish to move after you %ls "
                     L"the door.",
                     keyword);
            break;
          ...
          case SCHEDULE_INSTRUCTIONS_SLEEP:
            wcsncpy(str,
                    L"Click on the gridno where you wish to sleep at.  Person will "
                    L"automatically return to original position after waking up.",
                    lengthof(str));
          default:
            return;
        }
        wcscat(str, L"  Hit ESC to abort entering this line in the schedule.");
        DisplayWrappedString(436, 392, 149, 2, FONT10ARIAL, FONT_YELLOW, str, FONT_BLACK,
                             LEFT_JUSTIFIED);
      }
      break;
  }
...
// When a detailed placement merc is in the inventory panel, there is a overall
// region blanketing this panel.  As the user moves the mouse around and clicks,
// etc., this function is called by the region callback functions to handle these
// cases.  The event types are defined in Editor Taskbar Utils.h.  Here are the
// internal functions...
...
static BOOLEAN PointInRect(SGPRect *pRect, int32_t x, int32_t y) {
  return (x >= pRect->iLeft && x <= pRect->iRight && y >= pRect->iTop && y <= pRect->iBottom);
}

static void DrawRect(SGPRect *pRect, int16_t color) {
  SGPVSurface::Lock l(FRAME_BUFFER);
  SetClippingRegionAndImageWidth(l.Pitch(), 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
  RectangleDraw(TRUE, pRect->iLeft + MERCPANEL_X, pRect->iTop + MERCPANEL_Y,
                pRect->iRight + MERCPANEL_X, pRect->iBottom + MERCPANEL_Y, color,
                l.Buffer<uint16_t>());
}
...
static void RenderSelectedMercsInventory() {
  ...
  for (i = 0; i < 9; i++) {
    if (gpMercSlotItem[i]) {  // Render the current image.
      ...
      // Render the text
      switch (i) {
        case 2:  // legs (to the right of the box, but move it down to make room
                 // for right hand text)
          xp = mercRects[i].iRight + 2;
          yp = mercRects[i].iTop + 8;
          break;
        case 3:  // left hand (underneath box and to the left -- obscurred by
                 // checkbox)
...
// 1)  Allows a new item to be created via usItem and assigned to the currently
// selected merc. 2)  Converts the image from interface size to the smaller
// panel used by the editor.  The slots
//		 in the editor are approximately 80% of that size.  This involves
// scaling calculations.  These 		 images are saved in individual slots are are
// blitted to the screen during rendering, not here.
// NOTE:  Step one can be skipped (when selecting an existing merc).  By setting
// the
static void AddNewItemToSelectedMercsInventory(BOOLEAN fCreate) {
  ...
  if (fCreate) {
    ...
    == gusMercsNewItemIndex ) { //User selected same item, so ignore.
  ...
  // allow the slot to point to the selected merc's inventory for
  // editing/rendering purposes.
  gpMercSlotItem[gbCurrSelect] =
      &gpSelected->pDetailedPlacement->Inv[gbMercSlotTypes[gbCurrSelect]];

  if (!fCreate) {  // it is possible to have a null item which we don't want to
                   // blit!  Also, we need to set the
  ...
  // GOAL:
  // From here on, we are going to first render the new item into a temp buffer,
  // then crop the image in the buffer and scale it down to fit into it's
  // associated slot in the panel (which on average will require scaling the item
  // by 80%).  We have to do a bunch of calculations to override the offsets,
  // etc. Each slot has it's own smaller version buffer, and this is what gets
  // drawn when the rendering happens.
  ...
  int32_t const iSrcWidth = pObject.usWidth;
  int32_t const iSrcHeight = pObject.usHeight;
  SGPBox const src_rect = {SrcRect.iLeft + pObject.sOffsetX, SrcRect.iTop + pObject.sOffsetY,
                           iSrcWidth, iSrcHeight};

  // if the source image width is less than 30 (small slot), then modify the
  // DstRect.
  if (iSrcWidth < 30)
  ...
  // compare the sizes of the cropped image to the destination buffer size, and
  // calculate the scalar value.  It is possible to have scalars > 1.0, in which
  // case, we change it to 1.0 and use the other value.
  ...
  // use the new width and height values to calculate the new dest rect (center
  // the item)
  SGPBox const dst_rect = {(DstRect.iRight - DstRect.iLeft - iDstWidth) / 2,
                           (DstRect.iBottom - DstRect.iTop - iDstHeight) / 2, iDstWidth,
                           iDstHeight};
...
// This function is called by the move and click callback functions for the
// region blanketing the 9 slots in the inventory panel.  It passes the event
// type as well as the relative x and y positions which are processed here.  This
// basically checks for new changes in hilighting and selections, which will set
// the rendering flag, and getitem flag if the user wishes to choose an item.
void HandleMercInventoryPanel(int16_t sX, int16_t sY, int8_t bEvent) {
  int8_t x;
  if (!gfCanEditMercs && bEvent == GUI_RCLICK_EVENT) {  // if we are dealing with a profile merc, we
                                                        // can't allow editing
  ...
  switch (bEvent) {
    case GUI_MOVE_EVENT:
      ...
      if (gbCurrHilite != -1) {  // only turn off if it isn't already off.  This
                                 // avoids unecessary rendering.
        gbCurrHilite = -1;
        gfRenderMercInfo = TRUE;
      }
      break;
...
static void UpdateMercItemSlots() {
  ...
  if (!gpSelected->pDetailedPlacement) {
    ...
    for (x = 0; x < 9; x++) {
      // Set the curr select and the addnewitem function will handle the rest,
      // including rebuilding the nine slot buffers, etc.
      gbCurrSelect = x;
...
void SetEnemyColorCode(uint8_t const colour_code) {
  SOLDIERINITNODE const &sel = *gpSelected;
...
void RenderMercStrings() {
  CFOR_EACH_SOLDIERINITNODE(curr) {
    ...
    if (has_patrol_order != has_patrol_points) {
      ...
      wchar_t const *const msg = has_patrol_points ? L"Waypoints with no patrol orders"
...
void UpdateScheduleAction(uint8_t ubNewAction) {
  ...
  iEditorButton[MERCS_SCHEDULE_ACTION1 + gubCurrentScheduleActionIndex]->SpecifyText(
...
void FindScheduleGridNo(uint8_t ubScheduleData) {
  int32_t iMapIndex;  // XXX HACK000E
...
void ClearCurrentSchedule() {
  ...
  for (i = 0; i < 4; i++) {
    ...
    gCurrSchedule.usTime[i] = 0xffff;
    SetExclusive24HourTimeValue((uint8_t)(i + 1),
                                gCurrSchedule.usTime[i]);  // blanks the field
    gCurrSchedule.usData1[i] = 0xffff;
...
static void UpdateScheduleInfo() {
  ...
  if (gpSelected->pSoldier->ubScheduleID) {
    ...
    for (i = 0; i < 4; i++) {  // Update the text and buttons
      ...
      iEditorButton[MERCS_SCHEDULE_ACTION1 + i]->SpecifyText(
    ...
    // Copy the schedule over to the current global schedule used for editing
    // purposes.
...
void PasteMercPlacement(int32_t iMapIndex) {
  ...
  if (IsLocationSittable(iMapIndex, gfRoofPlacement)) {
    ...
    if (pSoldier != NULL) {
      ...
      pNode = AddBasicPlacementToSoldierInitList(gTempBasicPlacement);
      pNode->pSoldier = pSoldier;
      if (gSaveBufferBasicPlacement.fDetailedPlacement) {  // Add the static detailed placement
                                                           // information in the same newly created
      ...
      ScreenMsg(FONT_MCOLOR_LTRED, MSG_INTERFACE,
                L"Placement not pasted as the maximum number of placements for "
                L"this team is already used.");
    }
  }
}
