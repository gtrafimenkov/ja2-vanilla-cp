...
void SetEditorBuildingTaskbarMode(UINT16 usNewMode) {
  ...
  // Clicking on certain buttons will automatically activate/deactive certain
  // views.
  switch (usNewMode) {
    ...
    case BUILDING_CAVE_DRAWING:
      ...
      fNewWalls = TRUE;
      fNewRoomInfo = FALSE;
      if (usNewMode == BUILDING_PLACE_FLOORS) gusSelectionType = gusSavedSelectionType;
      break;
    default:
      return;
  }
...
void SetEditorSmoothingMode(UINT8 ubNewMode) {
  ...
  switch (ubNewMode) {
    ...
    case SMOOTHING_CAVES:
      ...
      if (iCurrentTaskbar == TASK_BUILDINGS) ShowEditorButton(BUILDING_CAVE_DRAWING);
