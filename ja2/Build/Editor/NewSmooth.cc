...
static bool CaveAtGridNo(int32_t const map_idx) {
...
static uint16_t GetCaveTileIndexFromPerimeterValue(uint8_t ubTotal) {
  uint16_t usType = FIRSTWALL;
  uint16_t usIndex;  // HACK000E
...
static uint8_t CalcNewCavePerimeterValue(int32_t iMapIndex) {
  uint8_t ubTotal = 0;
...
// These walls have shadows associated with them, and are draw when the wall is
// drawn.
#define EXTERIOR_L_SHADOW_INDEX 30
#define INTERIOR_BOTTOMEND_SHADOW_INDEX 31

// Wall Look Up Table containing variants and indices with each row being a
// different walltype.
static const int8_t gbWallTileLUT[NUM_WALL_TYPES][7] = {
...
// NOTE:  passing null for wall/roof type will force the function to search for
// the nearest
//  existing respective type.
static void BuildSlantRoof(int32_t iLeft, int32_t iTop, int32_t iRight, int32_t iBottom,
                           uint16_t usWallType, uint16_t usRoofType, BOOLEAN fVertical) {
  int32_t i;
  int32_t iMapIndex;
  if (fVertical) {
    iMapIndex = iBottom * WORLD_COLS + iLeft;
    // This happens to be the only mapindex that needs to be backed up.  The
    // rest have already been done because of the building code before this.
    AddToUndoList(iMapIndex + 8);
    // Add the closest viewable pieces.  There are two aframe walls pieces, and
    // extended aframe roof pieces.
...
// From a given gridNo and perspective (wallpiece), it will calculate the new
// piece, and where to place it as well as handle the special cases. NOTE:
// Placing top and left pieces are placed relative to the gridno, and the gridNo
// will
// shift accordingly to place the piece.  Pretend you are the floor, and you
// want to place a piece to the left.  You pass your position, and
// INTERIOR_LEFT, with interior meaning from the inside of a building.  If you
// were outside the building, you would call EXTERIOR_LEFT.  The left tile will
// be placed on gridNo - 1!  Up tiles will be placed on gridNo - 160.
// NOTE:  Passing NULL for usWallType will force it to calculate the closest
// existing wall type, and
//  use that for building this new wall.  It is necessary for restructuring a
//  building, but not for adding on to an existing building, where the type is
//  already known.
void BuildWallPiece(uint32_t iMapIndex, uint8_t ubWallPiece, uint16_t usWallType) {
  int16_t sIndex;
  uint16_t ubWallClass;  // XXX HACK000E
  LEVELNODE *pStruct;
  if (!usWallType) {
  ...
  switch (ubWallPiece) {
    case EXTERIOR_TOP:
      ...
      if (!gfBasement && GetVerticalWall(iMapIndex - 1) &&
          !GetVerticalWall(iMapIndex + WORLD_COLS -
                           1)) {  // Special case where a shadow has to be created
                                  // as it now is a bottom corner and
        // must contribute to the bottom shadow.
        AddToUndoList(iMapIndex - 1);
        uint16_t usTileIndex =
            GetTileIndexFromTypeSubIndex(usWallType, INTERIOR_BOTTOMEND_SHADOW_INDEX);
        AddExclusiveShadow(iMapIndex - 1, usTileIndex);
      }
      if ((pStruct = GetVerticalWall(iMapIndex)))  // right corner
      {  // Special case where placing the new wall will generate a corner to the
        // right, so replace
        // the vertical piece with a bottomend.
        sIndex = PickAWallPiece(EXTERIOR_BOTTOMEND);
        AddToUndoList(iMapIndex);
        uint16_t usTileIndex = GetTileIndexFromTypeSubIndex(usWallType, sIndex);
        ReplaceStructIndex(iMapIndex, pStruct->usIndex, usTileIndex);
      }
      ubWallClass = EXTERIOR_L;
      if (!gfBasement) {
        // All exterior_l walls have shadows.
        uint16_t usTileIndex = GetTileIndexFromTypeSubIndex(usWallType, EXTERIOR_L_SHADOW_INDEX);
    ...
    case EXTERIOR_LEFT:
      ...
      if (GetHorizontalWall(iMapIndex)) {  // Special case where placing the new wall will
                                           // generate a corner.  This piece
...
static void EraseRoof(uint32_t iMapIndex);

void RebuildRoofUsingFloorInfo(int32_t const map_idx, uint16_t roof_type) {
  ...
  uint16_t const roof_idx = top && left       ? TOPLEFT_ROOF_INDEX
  ...
  uint16_t const tile_idx = GetTileIndexFromTypeSubIndex(roof_type, roof_idx);
...
void RebuildRoof(uint32_t const map_idx, uint16_t roof_type) {
  if (roof_type == 0) roof_type = SearchForRoofType(map_idx);
  if (roof_type == 0xFFFF) return;  // No roof type around, so don't draw one.
  ...
  // NOTE:  There is no support for more than 2 side on a roof, so if there is,
  // draw TOPLEFT
...
static void BulldozeNature(uint32_t iMapIndex) {
  AddToUndoList(iMapIndex);
  ...
  RemoveAllStructsOfTypeRange(iMapIndex, FIRSTOSTRUCT,
                              LASTOSTRUCT);  // outside objects.
...
static void EraseFloor(uint32_t iMapIndex) {
  AddToUndoList(iMapIndex);
...
// Specialized function that will delete only the TOP_RIGHT oriented wall in the
// gridno to the left and the TOP_LEFT oriented wall in the gridno up one as well
// as the other building information at this gridno.
static void EraseFloorOwnedBuildingPieces(uint32_t iMapIndex) {
  LEVELNODE *pStruct = NULL;

  if (!gfBasement && !FloorAtGridNo(iMapIndex)) {  // We don't have ownership issues if there
                                                   // isn't a floor here.
    return;
  }
  EraseBuilding(iMapIndex);
  // FIRST, SEARCH AND DESTROY FOR A LEFT NEIGHBORING TILE WITH A TOP_RIGHT
  // ORIENTED WALL
  pStruct = gpWorldLevelData[iMapIndex - 1].pStructHead;
  while (pStruct != NULL) {
    if (pStruct->usIndex != NO_TILE) {
      const uint32_t uiTileType = GetTileType(pStruct->usIndex);
  ...
  // FINALLY, SEARCH AND DESTROY FOR A TOP NEIGHBORING TILE WITH A TOP_LEFT
  // ORIENTED WALL
...
void RemoveBuildingSectionFromWorld(SGPRect *pSelectRegion) {
  ...
  for (y = top; y <= bottom; y++)
    for (x = left; x <= right; x++) {
      iMapIndex = y * WORLD_COLS + x;
      // NOTE:  Top and bottom walls MUST be placed first -- it minimizes the
  ...
  if (gfBasement) {
    ...
    for (y = top; y <= bottom; y++)
      for (x = left; x <= right; x++) {
        iMapIndex = y * WORLD_COLS + x;
        uint16_t usTileIndex = GetTileIndexFromTypeSubIndex(usFloorType, 1);
...
void AddBuildingSectionToWorld(SGPRect *pSelectRegion) {
  int32_t top, left, right, bottom, x, y;
  uint32_t iMapIndex;
  uint16_t usFloorType, usWallType;
  ...
  uint16_t usRoofType = (uint16_t)-1;  // XXX HACK000E

  // Special case scenario:
  // If the user selects a floor without walls, then it is implied that the user
  // wishes to change the floor for say a kitchen which might have a different
  // floor type.
  ...
  // 1ST PASS:  Determine if there are any floor tiles in this region.  If there
  // are, then
  //  that signifies that we are concantenating this building to an existing
  //  one.  Otherwise, we are just drawing an individual building.  If we find a
  //  floor, extract the type so we know how to draw it later.
  fNewBuilding = TRUE;
  for (y = top; y <= bottom; y++)
    for (x = left; x <= right; x++) {
      iMapIndex = y * WORLD_COLS + x;
      if (FloorAtGridNo(iMapIndex)) {
        ...
        fNewBuilding = FALSE;
        // Extract the floor type.  We already checked if there was a floor
        // here, so it is assumed.
        pFloor = gpWorldLevelData[iMapIndex].pLandHead;
        while (pFloor) {
          const uint32_t uiTileType = GetTileType(pFloor->usIndex);
        ...
        if (usWallType != 0xffff && usRoofType != 0xffff &&
            usFloorType != 0xffff) {  // we have extracted all of the information
  ...
  // 2ND PASS:  Remove all walls in the region that border no floor tile, or
  // simply walls
  //  that are considered exterior walls.  That way, it won't wreck the inside
  //  of a building if you select too much interior.  Also, gridnos that delete
  //  walls will also delete the floor and roof tiles there.  That signifies
  //  that the floorless parts will be resmoothed, and rebuilt in the third
  //  pass.
  for (y = top; y <= bottom; y++)
    for (x = left; x <= right; x++) {
      iMapIndex = y * WORLD_COLS + x;
      ...
      } else  // we will be building outside of this structure, so bulldoze the
              // nature -- trees, rocks, etc.
      {
        BulldozeNature(iMapIndex);
      }
    }
  // 3RD PASS:  Process the region, and all walls of floorless tiles are rebuilt
  ...
  // 4TH PASS:  Process the region, and all floorless tiles get new roofs and
  // floors.
  for (y = top; y <= bottom; y++)
    for (x = left; x <= right; x++) {
      iMapIndex = y * WORLD_COLS + x;
...
void AnalyseCaveMapForStructureInfo() {
  FOR_EACH_WORLD_TILE(i) {
    for (LEVELNODE *k = i->pStructHead; k; k = k->pNext) {
      ...
      uint16_t const sub_idx = GetSubIndexFromTileIndex(k->usIndex);
