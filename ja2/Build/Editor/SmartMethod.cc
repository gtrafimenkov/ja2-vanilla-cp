...
static uint16_t CalcSmartWindowIndex(uint16_t usWallOrientation) {
  return (uint16_t)(33 + usWallOrientation * 3 + gubWindowUIValue);
}

static uint16_t CalcSmartDoorIndex(uint16_t usWallOrientation) {
  // convert the orientation values as the graphics are in reverse order
  // orientation values:   INSIDE_TOP_LEFT=1,  INSIDE_TOP_RIGHT=2,
  // OUTSIDE_TOP_LEFT=3, OUTSIDE_TOP_RIGHT=4 door graphics order:
  // INSIDE_TOP_LEFT=15, INSIDE_TOP_RIGHT=10, OUTSIDE_TOP_LEFT=5,
  // OUTSIDE_TOP_RIGHT=0
...
static uint16_t CalcSmartDoorType() { return (uint16_t)(FIRSTDOOR + gubDoorUIValue / 2); }

static uint16_t CalcSmartBrokenWallIndex(uint16_t usWallOrientation) {
  ...
  if (gubBrokenWallUIValue < 2)  // broken walls
  {
    ...
    // orientation values:   INSIDE_TOP_LEFT=1, INSIDE_TOP_RIGHT=2,
    // OUTSIDE_TOP_LEFT=3, OUTSIDE_TOP_RIGHT=4
    //																			4
    // 6 8 10 door graphics order:  INSIDE_TOP_LEFT=4, INSIDE_TOP_RIGHT=6,
    // OUTSIDE_TOP_LEFT=0, OUTSIDE_TOP_RIGHT=2
  ...
  // orientation values:   INSIDE_TOP_LEFT=1, INSIDE_TOP_RIGHT=2,
  // OUTSIDE_TOP_LEFT=3, OUTSIDE_TOP_RIGHT=4 door graphics order:
  // INSIDE_TOP_LEFT=1, INSIDE_TOP_RIGHT=2, OUTSIDE_TOP_LEFT=5,
  // OUTSIDE_TOP_RIGHT=6
...
BOOLEAN CalcDoorInfoUsingSmartMethod(uint32_t iMapIndex, uint16_t *pusDoorType,
  ...
  if (pWall) {
    uint16_t usWallOrientation = GetWallOrientation(pWall->usIndex);
...
// This is a very difficult function to document properly.  The reason being is
// that it is sooo subliminal by nature.  I have thought up of priorities and
// choose the best piece to draw based on the surrounding conditions.  Here are
// the priorities which are referenced below via comments: A)  If there is
// currently a bottom piece and a right piece, immediately exit. B)  We are
// currently over a bottom piece.  Now, we don't automatically want to draw a
// right piece here 		for multiple reasons.  First, the UI will be too quick and
// place bottom and right pieces for every 		place the user clicks, which isn't
// what we want.  Therefore, we look to see if there is a right
//    piece in the y-1 gridno.  It would then make sense to place a right piece
//    down here.  Regardless,
//		if we encounter a bottom piece here, we will exit.
// C)  This is the counterpart to B, but we are looking at a current right
// piece, and are determining if 		we should place a bottom piece based on another
// bottom piece existing in the x-1 gridno. D)  Now, we analyse the neighboring
// tiles and determine the orientations that would add weight to the
//    current tile either towards drawing a horizontal piece or a vertical
//    piece.
// E)  Now that we have the information, we give the highest priority to any
// weights that match the current 		wall piece type selected by the user.  Based on
// that, we will only consider the best match of the 		type and use it.  If there
// are no matches on type, we continue. F)  We failed to find weights matching
// the current wall type, but before we give up using the user's wall 		type, there
// are two more cases.  When there is a bottom wall in the y+1 position or a
// right wall in 		the x+1 position.  If there are matching walls, there, then we
// draw two pieces to connect the current 		gridno with the respective position.
  ...
  if (usNumV[gubWallUIValue - FIRSTWALL] | usNumH[gubWallUIValue - FIRSTWALL]) {
    if (usNumV[gubWallUIValue - FIRSTWALL] >= usNumH[gubWallUIValue - FIRSTWALL]) {
      if (FloorAtGridNo(iMapIndex + 1)) {  // inside
        BuildWallPiece(iMapIndex, EXTERIOR_RIGHT, gubWallUIValue);
        // Change to extended piece if it is a new top right corner to cover the
        // end part.
...
void PasteSmartDoor(uint32_t iMapIndex) {
  ...
  if ((pWall = GetVerticalWall(iMapIndex))) {
    ...
    uint16_t usTileIndex = GetTileIndexFromTypeSubIndex(usDoorType, usIndex);
...
void PasteSmartWindow(uint32_t iMapIndex) {
  ...
  pWall = GetVerticalWall(iMapIndex);
  if (pWall) {
    uint16_t const usWallType = GetWallType(pWall, iMapIndex);
    uint16_t usWallOrientation = GetWallOrientation(pWall->usIndex);
    usIndex = CalcSmartWindowIndex(usWallOrientation);
    // Calculate the new graphic for the window type selected.

    AddToUndoList(iMapIndex);
    uint16_t usNewWallIndex = GetTileIndexFromTypeSubIndex(usWallType, usIndex);
