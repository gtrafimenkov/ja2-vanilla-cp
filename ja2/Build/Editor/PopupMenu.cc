// FUNCTIONALITY:
// This popup menu functions IDENTICALLY to Windows95's methodology.  You can
// use the arrow keys, or the mouse in two methods.  The first method is where
// you release the mouse button before moving the cursor inside the popup menu,
// which makes it persistant (it won't go away until you click again), or
// non-persistant, where you release the button inside the menu after your
// initial click (non-persistant). IMPORTANT NOTE: Linda Currie, the main user of
// the editor dislikes the non-persistant functionality, however, it is still
// supported.  Just remove the commented line of code (search for UNCOMMENT),
// and it's fixed -- it is currently disabled.
...
on a popup menu activation button.
        1)  The user releases the mouse before moving the cursor onto
                        the popup region.  The popup menu will end as soon as
the user presses the mouse button.  If the cursor is out of the popup region,
then it returns 0. 2)  The user holds the mouse while moving the cursor onto the
                        popup region.  The popup menu will end as soon as the
user releases the mouse button.  If the cursor is out of the popup region, then
it returns 0.
...
void InitPopupMenu(GUIButtonRef const button, PopupMenuID const ubPopupMenuID,
  ...
  while ((usMenuHeight >= usY && (ubDirection == DIR_UPLEFT || ubDirection == DIR_UPRIGHT)) ||
         (SCREEN_HEIGHT - usMenuHeight >= usY &&
          (ubDirection == DIR_DOWNLEFT ||
           ubDirection == DIR_DOWNRIGHT))) {  // menu has too many entries.  Increase the
                                              // number of columns until the height is
  ...
  // now we have the number of columns as well as the max number of entries per
  // column, and the total menu height.

  // We now calculate the total width of the menu as well as the max width of
  // each column.
  ubCounter = 0;
  usMenuWidth = 0;
  ...
  MSYS_DefineRegion(&popupRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, MSYS_PRIORITY_HIGHEST,
...
static UINT8 GetPopupIndexFromMousePosition(void) {
  ...
  {
    // subtract the top y coord of the popup region from the mouse's yPos as
    // well as an extra pixel at the top of the region which is ignored in menu
    // selection, divide this number by the height of a menu entry, then add one.
    // This will return the menu index from 1 (at the top) to n (at the bottom).
...
static void PopupMenuHandle(void) {
  ...
  if (gPopup.ubActiveType == POPUP_ACTIVETYPE_NOT_YET_DETERMINED) {
        ...
        && gusMouseYPos < gPopup.usBottom - 1)  // two pixel gap on bottom ignored
    {
      ...
      // KRIS:  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      // Disabled this because Linda doesn't like it... (I like it though, and
      // it works)
      //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      return;
    } else if (!gfLeftButtonState) {  // left button has been released before
                                      // entering region -- persistant
