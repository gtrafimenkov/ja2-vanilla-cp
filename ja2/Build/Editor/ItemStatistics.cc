...
static AttachmentInfo g_weapon_attachment[] = {
    AttachmentInfo(SILENCER, L"SILENCER"),     AttachmentInfo(SNIPERSCOPE, L"SNIPERSCOPE"),
    AttachmentInfo(LASERSCOPE, L"LASERSCOPE"), AttachmentInfo(BIPOD, L"BIPOD"),
    AttachmentInfo(DUCKBILL, L"DUCKBILL"),     AttachmentInfo(UNDER_GLAUNCHER, L"G-LAUNCHER")};
...
void ShowItemStatsPanel() {
  ...
  if (iCurrentTaskbar == TASK_MERCS || !gpItem) HideEditorButton(ITEMSTATS_HIDDEN_BTN);
...
static void ExtractAndUpdateKeysGUI();
...
static void RemoveActionItemsGUI();
static void RemoveAmmoGUI();
static void RemoveArmourGUI();
static void RemoveEquipGUI();
static void RemoveExplosivesGUI();
static void RemoveGameTypeFlags();
static void RemoveGunGUI();
static void RemoveKeysGUI();
static void RemoveMoneyGUI();
static void RemoveOwnershipGUI();
static void RemoveTriggersGUI();

static void RemoveItemGUI() {
  if (!gpItem) return;
...
static void SetupActionItemsGUI();
static void SetupAmmoGUI();
static void SetupArmourGUI();
static void SetupEquipGUI();
static void SetupExplosivesGUI();
static void SetupGameTypeFlags();
static void SetupGunGUI();
static void SetupKeysGUI();
static void SetupMoneyGUI();
...
void SpecifyItemToEdit(OBJECTTYPE *pItem, int32_t iMapIndex) {
  ...
  if (iCurrentTaskbar == TASK_ITEMS) ShowEditorButton(ITEMSTATS_HIDDEN_BTN);

  // Determine the type of item so that we can determine the dynamic editing
  // mode.
  ...
  if (Item[gpItem->usItem].usItemClass == IC_MONEY) {
    ...
    switch (Item[gpItem->usItem].usItemClass) {
      ...
      case IC_NONE:
        gbEditingMode = EDITING_NOTHING;
        if (!(gpItem->fFlags & OBJECT_UNDROPPABLE)) gbEditingMode = EDITING_DROPPABLE;
        break;
      default:
...
static void SetupGameTypeFlags() {
  if (gpEditingItemPool) {
    giBothCheckboxButton = MakeRadioButton(
        573, BothModesCheckboxCallback, L"Item appears in both Sci-Fi and Realistic modes. (|B)");
    giRealisticCheckboxButton = MakeRadioButton(595, RealisticOnlyCheckboxCallback,
                                                L"Item appears in |Realistic mode only.");
    giSciFiCheckboxButton =
        MakeRadioButton(616, SciFiOnlyCheckboxCallback, L"Item appears in |Sci-Fi mode only.");

    GUIButtonRef btn;
    WORLDITEM &wi = GetWorldItem(gpEditingItemPool->iItemIndex);
    switch (wi.usFlags & (WORLD_ITEM_REALISTIC_ONLY | WORLD_ITEM_SCIFI_ONLY)) {
      case WORLD_ITEM_REALISTIC_ONLY:
        btn = giRealisticCheckboxButton;
        break;
      case WORLD_ITEM_SCIFI_ONLY:
        btn = giSciFiCheckboxButton;
        break;
      default:
        btn = giBothCheckboxButton;
        break;
    }
...
static bool MakeAttachmentButton(AttachmentInfo &a, int16_t const x, int16_t const y,
  ...
  GUIButtonRef const btn = CreateTextButton(a.label, SMALLCOMPFONT, FONT_YELLOW, FONT_BLACK, x, y,
                                            w, 12, MSYS_PRIORITY_NORMAL, click);
  btn->SetUserPtr(&a);
  a.button = btn;
...
static void SetupGunGUI() {
  ...
  if (gpEditingItemPool) {
    swprintf(str, lengthof(str), L"%d",
             100 - GetWorldItem(gpEditingItemPool->iItemIndex).ubNonExistChance);
...
static void ExtractAndUpdateGunGUI() {
  ...
  if (gpEditingItemPool) {
    ...
    giDefaultExistChance =
        (giDefaultExistChance == -1) ? 100 : std::max(1, std::min(giDefaultExistChance, 100));
...
static void SetupOwnershipGUI() {
  ...
  AddTextInputField(485, 380, 25, 15, MSYS_PRIORITY_NORMAL, str, 3, INPUTTYPE_NUMERICSTRICT);
  giOwnershipGroupButton = CreateTextButton(gszCivGroupNames[gpItem->ubOwnerCivGroup],
                                            SMALLCOMPFONT, FONT_YELLOW, FONT_BLACK, 485, 415, 80,
...
static void SetupActionItemsGUI() {
  ...
  guiActionItemButton = CreateTextButton(pStr, FONT10ARIAL, FONT_YELLOW, FONT_BLACK, 510, 410, 100,
                                         20, MSYS_PRIORITY_NORMAL, ActionItemCallback);
}
  ...
  if (gpEditingItemPool) {
    ...
    if (gpItem->bFrequency >= PANIC_FREQUENCY_3) {
      giAlarmTriggerButton =
          CreateCheckBoxButton(485, 405, EDITORDIR "/smcheckbox.sti", MSYS_PRIORITY_NORMAL,
                               AlarmTriggerCheckboxCallback);
      giAlarmTriggerButton->SetFastHelpText(
          L"If the panic trigger is an alarm trigger,\nenemies won't attempt "
          L"to use it if they\nare already aware of your presence.");
      if (gpItem->fFlags & OBJECT_ALARM_TRIGGER) giAlarmTriggerButton->uiFlags |= BUTTON_CLICKED_ON;
    }
  }
}
