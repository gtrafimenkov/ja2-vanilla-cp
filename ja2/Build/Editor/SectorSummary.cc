...
static const wchar_t *const gszVersionType[5] = {L"Pre-Alpha", L"Alpha", L"Demo", L"Beta",
                                                 L"Release"};
...
// If you don't wish to create a global summary, you can deny it.  This safely
// locks the system from generating one.
...
// When set, the summary window stays up until told otherwise.  When clear, the
// summary will disappear when the assigned key (F5) is released.  The latter
// mode is initiated when F5 is held down for longer than .4 seconds, and is
// useful for quickly looking at the information in the current map being edited.
static BOOLEAN gfPersistantSummary;
// When set, a grid is overlayed on top of the sector.  This grid defines each
// of the 256 sectors.  It is on by default.
static BOOLEAN gfRenderGrid;
// When set, parts of the map are darkened, showing that those sectors don't
// exist in the currently selected layer.  When clear, the entire map is shown in
// full color.
static BOOLEAN gfRenderProgress;
// When set, only the map section is rerendered.
static BOOLEAN gfRenderMap;
// When set, then we are overriding the ability to use normal methods for
// selecting sectors for saving and loading.  Immediately upon entering the text
// input mode; for the temp file; then we are assuming that the user will type in
// a name that doesn't follow standard naming conventions for the purposes of the
// campaign editor.  This is useful for naming a temp file for saving or
// loading.
static BOOLEAN gfTempFile;
// When set, only the alternate version of the maps will be displayed.  This is
// used for alternate maps in particular sectors, such as the SkyRider quest
// which could be located at one of four maps randomly.  If that particular map
// is chosen, then the alternate map will be used.
...
// Override status.  Hide is when there is nothing to override, readonly, when
// checked is to override a readonly status file, so that you can write to it,
// and overwrite, when checked, allows you to save, replacing the existing file.
...
void CreateSummaryWindow() {
  ...
  iSummaryButton[SUMMARY_BACKGROUND] =
      CreateLabel(NULL, 0, 0, 0, 0, 0, SCREEN_WIDTH, 360, MSYS_PRIORITY_HIGH - 1);

  iSummaryButton[SUMMARY_OKAY] =
      CreateTextButton(L"Okay", FONT12POINT1, FONT_BLACK, FONT_BLACK, 585, 325, 50, 30,
                       MSYS_PRIORITY_HIGH, SummaryOkayCallback);

  iSummaryButton[SUMMARY_GRIDCHECKBOX] =
      MakeCheckBox(MAP_LEFT, MAP_BOTTOM + 5, SummaryToggleGridCallback, true);
  gfRenderGrid = TRUE;

  iSummaryButton[SUMMARY_PROGRESSCHECKBOX] =
      MakeCheckBox(MAP_LEFT + 50, MAP_BOTTOM + 5, SummaryToggleProgressCallback, true);
  gfRenderProgress = TRUE;

  iSummaryButton[SUMMARY_ALL] =
      CreateTextButton(L"A", SMALLCOMPFONT, FONT_BLACK, FONT_BLACK, MAP_LEFT + 110, MAP_BOTTOM + 5,
                       16, 16, MSYS_PRIORITY_HIGH, SummaryToggleLevelCallback);
  ...
  iSummaryButton[SUMMARY_G] =
      CreateTextButton(L"G", SMALLCOMPFONT, FONT_BLACK, FONT_BLACK, MAP_LEFT + 128, MAP_BOTTOM + 5,
                       16, 16, MSYS_PRIORITY_HIGH, SummaryToggleLevelCallback);
  ...
  iSummaryButton[SUMMARY_B1] =
      CreateTextButton(L"B1", SMALLCOMPFONT, FONT_BLACK, FONT_BLACK, MAP_LEFT + 146, MAP_BOTTOM + 5,
                       16, 16, MSYS_PRIORITY_HIGH, SummaryToggleLevelCallback);
  ...
  iSummaryButton[SUMMARY_B2] =
      CreateTextButton(L"B2", SMALLCOMPFONT, FONT_BLACK, FONT_BLACK, MAP_LEFT + 164, MAP_BOTTOM + 5,
                       16, 16, MSYS_PRIORITY_HIGH, SummaryToggleLevelCallback);
  ...
  iSummaryButton[SUMMARY_B3] =
      CreateTextButton(L"B3", SMALLCOMPFONT, FONT_BLACK, FONT_BLACK, MAP_LEFT + 182, MAP_BOTTOM + 5,
                       16, 16, MSYS_PRIORITY_HIGH, SummaryToggleLevelCallback);
  ...
  iSummaryButton[SUMMARY_ALTERNATE] =
      MakeCheckBox(MAP_LEFT, MAP_BOTTOM + 25, SummaryToggleAlternateCallback, gfAlternateMaps);

  iSummaryButton[SUMMARY_LOAD] =
      CreateTextButton(L"LOAD", FONT12POINT1, FONT_BLACK, FONT_BLACK, MAP_LEFT, MAP_BOTTOM + 45, 50,
                       26, MSYS_PRIORITY_HIGH, SummaryLoadMapCallback);
  iSummaryButton[SUMMARY_SAVE] =
      CreateTextButton(L"SAVE", FONT12POINT1, FONT_BLACK, FONT_BLACK, MAP_LEFT + 55,
                       MAP_BOTTOM + 45, 50, 26, MSYS_PRIORITY_HIGH, SummarySaveMapCallback);
  iSummaryButton[SUMMARY_OVERRIDE] =
      MakeCheckBox(MAP_LEFT + 110, MAP_BOTTOM + 59, SummaryOverrideCallback);

  iSummaryButton[SUMMARY_UPDATE] =
      CreateTextButton(L"Update", FONT12POINT1, FONT_BLACK, FONT_BLACK, 255, 15, 40, 16,
                       MSYS_PRIORITY_HIGH, SummaryUpdateCallback);
  ...
  // iSummaryButton[SUMMARY_SCIFI]->SetFastHelpText(L"Display items that appear
  // in SciFi mode."); iSummaryButton[SUMMARY_REAL ]->SetFastHelpText(L"Display
  // items that appear in Realistic mode.");
  ...
  for (i = 1; i < NUM_SUMMARY_BUTTONS; i++) iSummaryButton[i]->Hide();
...
static void RenderSectorInformation(void) {
    ...
    if (s->fTooManyExitGridDests) {
      ...
      if (ubNumInvalid) {
        ...
        switch (s->ubNumExitGridDests) {
          ...
          case 4:
            mprintf(10, 265,
                    L"ExitGrids:  3 -- 1) Qty: %d, 2) Qty: %d, 3) Qty: %d, 4) "
                    L"Qty: %d",
                    s->usExitGridSize[0], s->usExitGridSize[1], s->usExitGridSize[2],
                    s->usExitGridSize[3]);
            break;
  ...
  usLine = 275;
  mprintf(10, usLine,
          L"Enemy Relative Attributes:  %d bad, %d poor, %d norm, %d good, %d "
          L"great (%+d Overall)",
  ...
  usLine += 10;
  mprintf(10, usLine,
          L"Enemy Relative Equipment:  %d bad, %d poor, %d norm, %d good, %d "
          L"great (%+d Overall)",
...
void RenderSummaryWindow() {
  ...
  if (gfRenderSummary) {
    ...
    if (!gfItemDetailsMode) {
      mprintf(10, 5, L"CAMPAIGN EDITOR -- %ls Version 1.%02d", gszVersionType[GLOBAL_SUMMARY_STATE],
              GLOBAL_SUMMARY_VERSION);
    }

    // This section builds the proper header to be displayed for an existing
    // global summary.
    if (!gfWorldLoaded) {
      SetFontForeground(FONT_RED);
    ...
    if (gfGlobalSummaryExists) {
      ...
      if (gusNumEntriesWithOutdatedOrNoSummaryInfo && !gfOutdatedDenied) {
        ...
        MPrint(10, 30,
               L"The more maps that need to be updated, the longer it takes.  "
               L"It'll take ");
        ...
        MPrint(10, 65,
               L"Do you wish to regenerate info for ALL these maps at this "
               L"time (y/n)?");
        ...
        if (gfTempFile) {
          SetFontForeground(FONT_YELLOW);
          MPrint(10, 30,
                 L"Entering a temp file name that doesn't follow campaign "
                 L"editor conventions...");
          goto SPECIALCASE_LABEL;  // OUCH!!!
        } else if (!gfWorldLoaded) {
          SetFontForeground(FONT_YELLOW);
          MPrint(10, 30,
                 L"You need to either load an existing map or create a new map "
                 L"before being");
      ...
      MPrint(10, 20,
             L"You currently have no summary data.  By creating one, you will "
             L"be able to keep track");
      MPrint(10, 30,
             L"of information pertaining to all of the sectors you edit and "
             L"save.  The creation process");
      MPrint(10, 40,
             L"will analyse all maps in your /MAPS directory, and generate a "
             L"new one.  This could");
      MPrint(10, 50,
             L"take a few minutes depending on how many valid maps you have.  "
             L"Valid maps are");
      MPrint(10, 60,
             L"maps following the proper naming convention from a1.dat - "
             L"p16.dat.  Underground maps");
      MPrint(10, 70,
             L"are signified by appending _b1 to _b3 before the .dat (ex:  "
             L"a9_b1.dat).");
      SetFontForeground(FONT_LTRED);
      ...
      for (y = 0; y < 16; y++) {
        ...
        for (x = 0; x < 16; x++) {
          if (giCurrentViewLevel == ALL_LEVELS_MASK) {
            ...
            if (ubNumUndergroundLevels) {  // display the number of underground
                                           // levels.  If there
          ...
          FRAME_BUFFER->ShadowRect(ClipRect.iLeft, ClipRect.iTop, ClipRect.iRight,
                                   ClipRect.iBottom);
...
static void CreateGlobalSummary(void) {
  ...
  fputs(
      ...
      "This directory or its contents shouldn't be included with final "
      "release.\n",
      f);
  fclose(f);
...
static BOOLEAN LoadSummary(const INT32 x, const INT32 y, const UINT8 level,
  ...
  if (!f_sum) {
    ...
    if (sum->ubSummaryVersion < MINIMUMVERSION || dMajorMapVersion < getMajorMapVersion()) {
    ...
    *anchor = sum;
    ...
    if (sum->ubSummaryVersion < GLOBAL_SUMMARY_VERSION) ++gusNumEntriesWithOutdatedOrNoSummaryInfo;
  }
...
static void LoadGlobalSummary(void) {
  ...
  for (INT32 y = 0; y < 16; ++y) {
    for (INT32 x = 0; x < 16; ++x) {
      ...
      if (LoadSummary(x, y, 4, "_a"))
        sector_levels |= ALTERNATE_GROUND_MASK;  // alternate ground level
  ...
  if (gfMustForceUpdateAllMaps) {
    DebugMsg(TOPIC_JA2EDITOR, DBG_LEVEL_1,
             String("A MAJOR MAP UPDATE EVENT HAS BEEN DETECTED FOR %d MAPS!!!!.",
                    gusNumberOfMapsToBeForceUpdated));
  }
...
void WriteSectorSummaryUpdate(const char *const filename, const UINT8 ubLevel,
  ...
  snprintf(summary_filename, lengthof(summary_filename), DEVINFO_DIR "/%.*s.sum",
...
static void ExtractTempFilename(void) {
  ...
  if (!wcslen(str)) wcsncpy(gszDisplayName, L"test.dat", lengthof(gszDisplayName));
}
...
void ApologizeOverrideAndForceUpdateEverything() {
  ...
  gfMajorUpdate = TRUE;
  iSummaryButton[SUMMARY_BACKGROUND] =
      CreateLabel(NULL, 0, 0, 0, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, MSYS_PRIORITY_HIGH - 1);
  // Draw it
  iSummaryButton[SUMMARY_BACKGROUND]->Draw();
  InvalidateScreen();
  ...
  SetFont(FONT10ARIAL);
  SetFontForeground(FONT_YELLOW);
  swprintf(str, lengthof(str), L"There are %d maps requiring a major version update.",
           gusNumberOfMapsToBeForceUpdated);
...
static void SetupItemDetailsMode(BOOLEAN fAllowRecursion) {
  ...
  // Now fileseek directly to the file position where the number of world items
  // are stored
  ...
  // Now compare this number with the number the summary thinks we should have.
  // If they are different, the the summary doesn't match the map.  What we will
  // do is force regenerate the map so that they do match
  ...
  // Passed the gauntlet, so now allocate memory for it, and load all the world
  ...
  // We need to do two passes.  The first pass simply processes all the enemies
  // and counts all the droppable items keeping track of two different values.
  // The first value is the number of droppable items that come off of enemy
  // detailed placements, the other counter keeps track of the number of
  // droppable items that come off of normal enemy placements.  After doing this,
  // the memory is allocated for the tables that will store all the item summary
  // information, then the second pass will repeat the process, except it will
  // record the actual items.
  ...
  // PASS #2
  // During this pass, simply copy all the data instead of counting it, now that
