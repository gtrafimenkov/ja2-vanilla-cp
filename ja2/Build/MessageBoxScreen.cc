...
static MessageBoxStyle const g_msg_box_style[] = {
    {DIALOG_MERC_POPUP_BACKGROUND, DIALOG_MERC_POPUP_BORDER, INTERFACEDIR "/popupbuttons.sti", 0, 1,
     FONT_MCOLOR_WHITE, DEFAULT_SHADOW, CURSOR_NORMAL},  // MSG_BOX_BASIC_STYLE
    {WHITE_MERC_POPUP_BACKGROUND, RED_MERC_POPUP_BORDER, INTERFACEDIR "/msgboxredbuttons.sti", 0, 1,
     2, NO_SHADOW, CURSOR_LAPTOP_SCREEN},  // MSG_BOX_RED_ON_WHITE
    {GREY_MERC_POPUP_BACKGROUND, BLUE_MERC_POPUP_BORDER, INTERFACEDIR "/msgboxgreybuttons.sti", 0,
     1, 2, FONT_MCOLOR_WHITE, CURSOR_LAPTOP_SCREEN},  // MSG_BOX_BLUE_ON_GREY
    {DIALOG_MERC_POPUP_BACKGROUND, DIALOG_MERC_POPUP_BORDER, INTERFACEDIR "/popupbuttons.sti", 2, 3,
     FONT_MCOLOR_WHITE, DEFAULT_SHADOW, CURSOR_NORMAL},  // MSG_BOX_BASIC_SMALL_BUTTONS
    {IMP_POPUP_BACKGROUND, DIALOG_MERC_POPUP_BORDER, INTERFACEDIR "/msgboxgreybuttons.sti", 0, 1, 2,
     FONT_MCOLOR_WHITE, CURSOR_LAPTOP_SCREEN},  // MSG_BOX_IMP_STYLE
    {LAPTOP_POPUP_BACKGROUND, LAPTOP_POP_BORDER, INTERFACEDIR "/popupbuttons.sti", 0, 1,
     FONT_MCOLOR_WHITE, DEFAULT_SHADOW, CURSOR_LAPTOP_SCREEN}  // MSG_BOX_LAPTOP_DEFAULT
};

static MessageBoxStyle const g_msg_box_style_default = {BASIC_MERC_POPUP_BACKGROUND,
                                                        BASIC_MERC_POPUP_BORDER,
                                                        INTERFACEDIR "/msgboxbuttons.sti",
                                                        0,
                                                        1,
                                                        FONT_MCOLOR_WHITE,
                                                        DEFAULT_SHADOW,
...
void DoMessageBox(MessageBoxStyleID const ubStyle, wchar_t const *const zString,
                  ScreenID const uiExitScreen, MessageBoxFlags const usFlags,
                  MSGBOX_CALLBACK const ReturnCallback, SGPBox const *const centering_rect) {
  GetMousePos(&pOldMousePosition);

  // this variable can be unset if ur in a non gamescreen and DONT want the msg
  // box to use the save buffer
  ...
  MessageBoxStyle const &style =
      ubStyle < lengthof(g_msg_box_style) ? g_msg_box_style[ubStyle] : g_msg_box_style_default;
  ...
  // findout if cursor locked, if so, store old params and store, restore when
  // done
  ...
  switch (usFlags) {
    ...
    case MSG_BOX_FLAG_OK:
      ...
      gMsgBox.uiOKButton = MakeButton(pMessageStrings[MSG_OK], font_colour, shadow_colour, x, y,
                                      OKMsgBoxCallback, cursor);
      break;

    case MSG_BOX_FLAG_YESNO:
      x += (usTextBoxWidth - (MSGBOX_BUTTON_WIDTH + dx)) / 2;
      gMsgBox.uiYESButton = MakeButton(pMessageStrings[MSG_YES], font_colour, shadow_colour, x, y,
                                       YESMsgBoxCallback, cursor);
      gMsgBox.uiNOButton = MakeButton(pMessageStrings[MSG_NO], font_colour, shadow_colour, x + dx,
                                      y, NOMsgBoxCallback, cursor);
      break;

    case MSG_BOX_FLAG_CONTINUESTOP:
      x += (usTextBoxWidth - (MSGBOX_BUTTON_WIDTH + dx)) / 2;
      gMsgBox.uiYESButton = MakeButton(pUpdatePanelButtons[0], font_colour, shadow_colour, x, y,
                                       YESMsgBoxCallback, cursor);
      gMsgBox.uiNOButton = MakeButton(pUpdatePanelButtons[1], font_colour, shadow_colour, x + dx, y,
                                      NOMsgBoxCallback, cursor);
      break;

    case MSG_BOX_FLAG_OKCONTRACT:
      x += (usTextBoxWidth - (MSGBOX_BUTTON_WIDTH + dx)) / 2;
      gMsgBox.uiYESButton = MakeButton(pMessageStrings[MSG_OK], font_colour, shadow_colour, x, y,
                                       YESMsgBoxCallback, cursor);
      gMsgBox.uiNOButton = MakeButton(pMessageStrings[MSG_REHIRE], font_colour, shadow_colour,
                                      x + dx, y, ContractMsgBoxCallback, cursor);
      break;

    case MSG_BOX_FLAG_GENERICCONTRACT:
      x += (usTextBoxWidth - (MSGBOX_BUTTON_WIDTH + dx * 2)) / 2;
      gMsgBox.uiYESButton = MakeButton(gzUserDefinedButton1, font_colour, shadow_colour, x, y,
                                       YESMsgBoxCallback, cursor);
      gMsgBox.uiNOButton = MakeButton(gzUserDefinedButton2, font_colour, shadow_colour, x + dx, y,
                                      NOMsgBoxCallback, cursor);
      gMsgBox.uiOKButton = MakeButton(pMessageStrings[MSG_REHIRE], font_colour, shadow_colour,
                                      x + dx * 2, y, ContractMsgBoxCallback, cursor);
      break;
    ...
    case MSG_BOX_FLAG_YESNOLIE:
      ...
      gMsgBox.uiNOButton = MakeButton(pMessageStrings[MSG_NO], font_colour, shadow_colour, x + dx,
                                      y, NOMsgBoxCallback, cursor);
      gMsgBox.uiOKButton = MakeButton(pMessageStrings[MSG_LIE], font_colour, shadow_colour,
                                      x + dx * 2, y, LieMsgBoxCallback, cursor);
      break;
    ...
    case MSG_BOX_FLAG_OKSKIP:
      ...
      gMsgBox.uiYESButton = MakeButton(pMessageStrings[MSG_OK], font_colour, shadow_colour, x, y,
                                       YESMsgBoxCallback, cursor);
      gMsgBox.uiNOButton = MakeButton(pMessageStrings[MSG_SKIP], font_colour, shadow_colour, x + dx,
                                      y, NOMsgBoxCallback, cursor);
      break;
  }
...
static ScreenID ExitMsgBox(MessageBoxReturnValue const ubExitCode) {
  ...
  // if you are in a non gamescreen and DONT want the msg box to use the save
  // buffer, unset gfDontOverRideSaveBuffer in your callback
  if ((gMsgBox.uiExitScreen != GAME_SCREEN || fRestoreBackgroundForMessageBox) &&
    ...
    InvalidateRegion(gMsgBox.sX, gMsgBox.sY, gMsgBox.sX + gMsgBox.usWidth,
      ...
      case MSG_BOX_FLAG_FOUR_NUMBERED_BUTTONS:
        switch (InputEvent.usParam) {
          case '1':
            ...
            break;
          case '2':
            gMsgBox.bHandled = MSG_BOX_RETURN_2;
            break;
          case '3':
            gMsgBox.bHandled = MSG_BOX_RETURN_3;
            break;
          case '4':
            gMsgBox.bHandled = MSG_BOX_RETURN_4;
            break;
        }
        break;
      default:
...
void DoScreenIndependantMessageBox(wchar_t const *const zString, MessageBoxFlags const usFlags,
...
// a basic box that don't care what screen we came from
void DoLowerScreenIndependantMessageBox(wchar_t const *const zString, MessageBoxFlags const usFlags,
                                        MSGBOX_CALLBACK const ReturnCallback) {
  SGPBox const centering_rect = {0, INV_INTERFACE_START_Y / 2, SCREEN_WIDTH,
                                 INV_INTERFACE_START_Y / 2};
  DoScreenIndependantMessageBoxWithRect(zString, usFlags, ReturnCallback, &centering_rect);
}

static void DoScreenIndependantMessageBoxWithRect(wchar_t const *const msg,
  ...
  switch (ScreenID const screen = guiCurrentScreen) {
    ...
    case GAME_SCREEN:
      DoMessageBox(MSG_BOX_BASIC_STYLE, msg, screen, flags, callback, centering_rect);
      break;
    case LAPTOP_SCREEN:
      DoLapTopSystemMessageBoxWithRect(MSG_BOX_LAPTOP_DEFAULT, msg, screen, flags, callback,
                                       centering_rect);
      break;
    case MAP_SCREEN:
      DoMapMessageBoxWithRect(MSG_BOX_BASIC_STYLE, msg, screen, flags, callback, centering_rect);
      break;
    case OPTIONS_SCREEN:
      DoOptionsMessageBoxWithRect(msg, screen, flags, callback, centering_rect);
      break;
    case SAVE_LOAD_SCREEN:
      DoSaveLoadMessageBoxWithRect(msg, screen, flags, callback, centering_rect);
      break;
    default:
      break;
  }
