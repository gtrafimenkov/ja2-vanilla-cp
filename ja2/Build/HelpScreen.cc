...
extern void PrintNumberOnTeam();
extern void PrintBalance();
...
// #define	HLP_SCRN__NUMBER_BYTES_IN_TEXT_BUFFER
//( 2 * HLP_SCRN__WIDTH_OF_TEXT_BUFFER * HLP_SCRN__HEIGHT_OF_1_LINE_IN_BUFFER *
...
static const int32_t gHelpScreenBtnTextRecordNum[HELP_SCREEN_NUMBER_OF_HELP_SCREENS]
...
//  must use this cause you have ur cursor over a button when entering the help
//  screen, the button will burn though.
// It does this cause that region loses it focus so it draws the button again.
static uint8_t gubRenderHelpScreenTwiceInaRow = 0;
...
BOOLEAN ShouldTheHelpScreenComeUp(HelpScreenID const ubScreenID,
                                  BOOLEAN const fForceHelpScreenToComeUp) {
  ...
  // should the screen come up, based on the users choice for it automatically
  // coming up
...
static void EnterHelpScreen();
static void ExitHelpScreen();
static void GetHelpScreenUserInput();
static void HandleHelpScreen();
...
static void BtnHelpScreenDontShowHelpAgainCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnHelpScreenExitCallback(GUI_BUTTON *btn, int32_t reason);
static void ChangeHelpScreenSubPage();
static void CreateHelpScreenButtons();
static void CreateHelpScreenTextBuffer();
static void CreateScrollAreaButtons();
static void SetSizeAndPropertiesOfHelpScreen();
...
static void EnterHelpScreen() {
  ...
  MSYS_DefineRegion(&gHelpScreenFullScreenMask, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                    MSYS_PRIORITY_HIGHEST, gHelpScreen.usCursor, MSYS_NO_CALLBACK,
                    MSYS_NO_CALLBACK);
  ...
  guiHelpScreenExitBtn = QuickCreateButton(giExitBtnImage, usPosX, usPosY, MSYS_PRIORITY_HIGHEST,
  ...
  if (!gHelpScreen.fForceHelpScreenToComeUp) {
    gHelpScreenDontShowHelpAgainToggle =
        CreateCheckBoxButton(usPosX, (uint16_t)(usPosY - 3), INTERFACEDIR "/optionscheckboxes.sti",
...
static void HelpScreenMouseMoveScrollBox(int32_t usMousePosY);
static void RefreshAllHelpScreenButtons();
static void SpecialHandlerCode();

static void HandleHelpScreen() {
  // if any of the possible screens need to have a some code done every loop..
  // its done in here
  SpecialHandlerCode();
...
static void DisplayCurrentScreenTitleAndFooter();
static void DrawHelpScreenBackGround();
static void RenderTextBufferToScreen();
...
static void RenderHelpScreen() {
  ...
  if (!gfHaveRenderedFirstFrameToSaveBuffer) {
    ...
    BlitBufferToBuffer(FRAME_BUFFER, guiSAVEBUFFER, gHelpScreen.usScreenLocX,
                       gHelpScreen.usScreenLocY, gHelpScreen.usScreenWidth,
...
static void DeleteScrollArrowButtons();
static void DestroyHelpScreenTextBuffer();
static void HelpScreenSpecialExitCode();
...
static void DrawHelpScreenBackGround() {
  ...
  if (gHelpScreen.bNumberOfButtons != 0) {
    BltVideoObject(FRAME_BUFFER, guiHelpScreenBackGround, HLP_SCRN_BUTTON_BORDER, usPosX,
                   gHelpScreen.usScreenLocY);
    usPosX += HELP_SCREEN_BUTTON_BORDER_WIDTH;
  }

  BltVideoObject(FRAME_BUFFER, guiHelpScreenBackGround, HLP_SCRN_DEFAULT_TYPE, usPosX,
                 gHelpScreen.usScreenLocY);
...
static void SetSizeAndPropertiesOfHelpScreen() {
  ...
  //
  // these are the default settings, so if the screen uses different then
  // defualt, set them in the switch
  //
  {
...
static void BtnHelpScreenBtnsCallback(GUI_BUTTON *btn, int32_t reason);
static void GetHelpScreenText(uint32_t uiRecordToGet, wchar_t *pText);
  ...
  if (gHelpScreen.bNumberOfButtons != 0) {
    ...
    for (i = 0; i < gHelpScreen.bNumberOfButtons; i++) {
      ...
      guiHelpScreenBtns[i] = CreateIconAndTextButton(
          ...
          DEFAULT_SHADOW, HELP_SCREEN_BTN_FONT_OFF_COLOR, DEFAULT_SHADOW, usPosX, usPosY,
          MSYS_PRIORITY_HIGHEST, BtnHelpScreenBtnsCallback);
...
static void ChangeToHelpScreenSubPage(int8_t bNewPage);
static void ChangeTopLineInTextBufferByAmount(int32_t iAmouontToMove);
static void PrepareToExitHelpScreen();
...
// Handles anything spcial that must be done when exiting the specific screen we
// are about to reenter ( eg. dirtying of the screen )
static void HelpScreenSpecialExitCode() {
...
static void PrepareToExitHelpScreen() { gfHelpScreenExit = TRUE; }

// Handles anything special that must be done when exiting the specific screen
// we are about to reenter ( eg. dirtying of the screen )
static void SpecialHandlerCode() {
  // switch on the current screen
  switch (gHelpScreen.bCurrentHelpScreen) {
    case HELP_SCREEN_LAPTOP:
...
static uint16_t RenderLaptopHelpScreen();
static uint16_t RenderMapScreenHelpScreen();
static uint16_t RenderMapScreenNoOneHiredYetHelpScreen();
static uint16_t RenderMapScreenNotYetInArulcoHelpScreen();
static uint16_t RenderMapScreenSectorInventoryHelpScreen();
static uint16_t RenderTacticalHelpScreen();

static uint16_t RenderSpecificHelpScreen() {
...
static void GetHelpScreenTextPositions(uint16_t *pusPosX, uint16_t *pusPosY, uint16_t *pusWidth) {
  // if there are buttons
...
static void DisplayCurrentScreenTitleAndFooter() {
  ...
  switch (gHelpScreen.bCurrentHelpScreen) {
    case HELP_SCREEN_LAPTOP:
      iStartLoc = HLP_TXT_LAPTOP_TITLE;
      break;
    case HELP_SCREEN_MAPSCREEN:
      iStartLoc = HLP_TXT_WELCOM_TO_ARULCO_TITLE;
      break;
    case HELP_SCREEN_TACTICAL:
      iStartLoc = HLP_TXT_TACTICAL_TITLE;
      break;
    case HELP_SCREEN_MAPSCREEN_NO_ONE_HIRED:
      iStartLoc = HLP_TXT_MPSCRN_NO_1_HIRED_YET_TITLE;
      break;
    case HELP_SCREEN_MAPSCREEN_NOT_IN_ARULCO:
      iStartLoc = HLP_TXT_MPSCRN_NOT_IN_ARULCO_TITLE;
      break;
    case HELP_SCREEN_MAPSCREEN_SECTOR_INVENTORY:
      iStartLoc = HLP_TXT_SECTOR_INVTRY_TITLE;
      break;
    case HELP_SCREEN_OPTIONS:
      break;
    case HELP_SCREEN_LOAD_GAME:
  ...
  if (iStartLoc != -1) {
    ...
    // Display the Title
    IanDisplayWrappedString(usPosX, gHelpScreen.usScreenLocY + HELP_SCREEN_TITLE_OFFSET_Y, usWidth,
                            HELP_SCREEN_GAP_BTN_LINES, HELP_SCREEN_TITLE_BODY_FONT,
                            HELP_SCREEN_TITLE_BODY_COLOR, zText, HELP_SCREEN_TEXT_BACKGROUND, 0);
  }
...
static void BtnHelpScreenBtnsCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    // Get the btn id
    int8_t const bRetValue = btn->GetUserData();
    ...
    /*
       ...
       gHelpScreen.bNumberOfButtons ) ? gHelpScreen.bNumberOfButtons-1 :
       bRetValue;

                    gHelpScreen.ubHelpScreenDirty =
       HLP_SCRN_DRTY_LVL_REFRESH_TEXT;
...
static void ChangeToHelpScreenSubPage(int8_t bNewPage) {
  ...
  // for some reason if the we are passing in a # that is greater then the max,
  // set it to the max
...
static void GetHelpScreenText(const uint32_t uiRecordToGet, wchar_t *const pText) {
  LoadEncryptedDataFromFile(BINARYDATADIR "/help.edt", pText,
                            HELPSCREEN_RECORD_SIZE * uiRecordToGet, HELPSCREEN_RECORD_SIZE);
}

// returns the number of vertical pixels printed
static uint16_t GetAndDisplayHelpScreenText(uint32_t uiRecord, uint16_t usPosX, uint16_t usPosY,
                                            uint16_t usWidth) {
  ...
  usNumVertPixels = IanDisplayWrappedString(usPosX, usPosY, usWidth, HELP_SCREEN_GAP_BTN_LINES,
                                            ...
                                            zText, HELP_SCREEN_TEXT_BACKGROUND, 0);
  ...
  switch (gHelpScreen.bCurrentHelpScreenActiveSubPage) {
    case HLP_SCRN_LPTP_OVERVIEW:
      ...
      /*
         ...
         GetAndDisplayHelpScreenText( HLP_TXT_LAPTOP_OVERVIEW_P1, usPosX, usPosY,
         usWidth );
         ...
         HELP_SCREEN_TEXT_BODY_FONT );
                              ...
                              usTotalNumberOfVerticalPixels +=
         GetAndDisplayHelpScreenText( HLP_TXT_LAPTOP_OVERVIEW_P2, usPosX, usPosY,
         usWidth );
      */
      break;
...
HelpScreenID HelpScreenDetermineWhichMapScreenHelpToShow() {
  return fShowMapInventoryPool ? HELP_SCREEN_MAPSCREEN_SECTOR_INVENTORY
...
static void CreateHelpScreenTextBuffer() {
  guiHelpScreenTextBufferSurface =
      AddVideoSurface(HLP_SCRN__WIDTH_OF_TEXT_BUFFER, HLP_SCRN__HEIGHT_OF_TEXT_BUFFER, PIXEL_DEPTH);
  guiHelpScreenTextBufferSurface->SetTransparency(0);
}

static void DestroyHelpScreenTextBuffer() { DeleteVideoSurface(guiHelpScreenTextBufferSurface); }

static void ClearHelpScreenTextBuffer();

static void RenderCurrentHelpScreenTextToBuffer() {
...
static void DisplayHelpScreenTextBufferScrollBox();

static void RenderTextBufferToScreen() {
  SGPBox const SrcRect = {0,
                          gHelpScreen.iLineAtTopOfTextBuffer * HLP_SCRN__HEIGHT_OF_1_LINE_IN_BUFFER,
                          HLP_SCRN__WIDTH_OF_TEXT_BUFFER, HLP_SCRN__HEIGHT_OF_TEXT_AREA - 2 * 8};
  BltVideoSurface(FRAME_BUFFER, guiHelpScreenTextBufferSurface, gHelpScreen.usLeftMarginPosX,
...
static void ChangeHelpScreenSubPage() {
  ...
  bool const enable =
      gHelpScreen.usTotalNumberOfLinesInBuffer > HLP_SCRN__MAX_NUMBER_DISPLAYED_LINES_IN_BUFFER;
...
// - is up, + is down
static void ChangeTopLineInTextBufferByAmount(int32_t const delta) {
  HELP_SCREEN_STRUCT &hlp = gHelpScreen;
  int32_t &top = hlp.iLineAtTopOfTextBuffer;
  int32_t const max_top =
      hlp.usTotalNumberOfLinesInBuffer - HLP_SCRN__MAX_NUMBER_DISPLAYED_LINES_IN_BUFFER;
  int32_t new_top = top + delta;
...
static void CalculateHeightAndPositionForHelpScreenScrollBox(int32_t *piHeightOfScrollBox,
...
static void BtnHelpScreenScrollArrowsCallback(GUI_BUTTON *btn, int32_t reason);
static void SelectHelpScrollAreaCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectHelpScrollAreaMovementCallBack(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateScrollAreaButtons() {
  ...
  guiHelpScreenScrollArrowImage[0] =
      LoadButtonImage(INTERFACEDIR "/helpscreen.sti", 14, 10, 11, 12, 13);
  ...
  giHelpScreenScrollArrows[0] =
      QuickCreateButton(guiHelpScreenScrollArrowImage[0], usPosX, usPosY, MSYS_PRIORITY_HIGHEST,
  ...
  // Create the scroll arrows
  giHelpScreenScrollArrows[1] =
      QuickCreateButton(guiHelpScreenScrollArrowImage[1], usPosX, usPosY, MSYS_PRIORITY_HIGHEST,
                        BtnHelpScreenScrollArrowsCallback);
...
static void HelpScreenMouseMoveScrollBox(int32_t const mouse_y) {
  int32_t bar_h;
  int32_t bar_y;
  CalculateHeightAndPositionForHelpScreenScrollBox(&bar_h, &bar_y);

  int32_t const max_h = HLP_SCRN__HEIGHT_OF_SCROLL_AREA - bar_h;
  if (max_h == 0) return;

  HELP_SCREEN_STRUCT &hlp = gHelpScreen;
  if (hlp.iLastMouseClickY == -1) {
    hlp.iLastMouseClickY =
        bar_y <= mouse_y && mouse_y < bar_y + bar_h ? mouse_y - bar_y : bar_h / 2;
  }

  int32_t const min_y = HLP_SCRN__SCROLL_POSY + hlp.iLastMouseClickY;
  int32_t const txt_h =
      hlp.usTotalNumberOfLinesInBuffer - HLP_SCRN__MAX_NUMBER_DISPLAYED_LINES_IN_BUFFER;
  int32_t const pos = txt_h * (mouse_y - min_y) / max_h;
  int32_t const delta = pos - hlp.iLineAtTopOfTextBuffer;
...
static void BtnHelpScreenScrollArrowsCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN || reason & MSYS_CALLBACK_REASON_LBUTTON_REPEAT) {
    int32_t const iButtonID = btn->GetUserData();
