...
// ARM: NOTE that these map "events" are never actually saved in a player's game
// in any way
enum MapEvent {
  MAP_EVENT_NONE,
...
// for the flashing of the contract departure time...for when mercs are leaving
// in an hour or less
...
static void ContractBoxGlow() {
  /* Why not?
          fResetContractGlow = FALSE;
  */
}

static void ContractListRegionBoxGlow(uint16_t usCount) {
  ...
  // glow contract box
  uint16_t usColor = GlowColor(iColorNum);
  SGPVSurface::Lock l(FRAME_BUFFER);
  SetClippingRegionAndImageWidth(l.Pitch(), 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
  RectangleDraw(TRUE, TIME_REMAINING_X, usY, TIME_REMAINING_X + TIME_REMAINING_WIDTH,
                usY + GetFontHeight(MAP_SCREEN_FONT) + 2, usColor, l.Buffer<uint16_t>());
...
static void GlowItem() {
  ...
  SGPVSurface::Lock l(FRAME_BUFFER);
  SetClippingRegionAndImageWidth(l.Pitch(), 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
  RectangleDraw(TRUE, 3, 80, 64, 104, usColor, l.Buffer<uint16_t>());
  InvalidateRegion(3, 80, 65, 105);
}

static void GlowTrashCan() {
  static int32_t iColorNum = 10;
  ...
  uint16_t usColor = GlowColor(iColorNum);
  {
    ...
    RectangleDraw(TRUE, TRASH_CAN_X, TRASH_CAN_Y, TRASH_CAN_X + TRASH_CAN_WIDTH,
                  TRASH_CAN_Y + TRASH_CAN_HEIGHT, usColor, l.Buffer<uint16_t>());
...
void DrawFace() {
  ...
  // draws the face of the currently selected merc, being displayed int he upper
  // left hand corner
...
static void RenderHandPosItem() {
  ...
  INVRenderItem(guiSAVEBUFFER, s, s->inv[HANDPOS], SOLDIER_HAND_X, SOLDIER_HAND_Y, 58, 23,
...
static void RenderIconsForUpperLeftCornerPiece(const SOLDIERTYPE *const s) {
  ...
  if (s->usLifeInsurance > 0) {
    ...
    BltVideoObject(guiSAVEBUFFER, guiULICONS, 2, CHAR_ICON_X,
  ...
  if (s->usMedicalDeposit > 0) {
    ...
    BltVideoObject(guiSAVEBUFFER, guiULICONS, 1, CHAR_ICON_X,
                   CHAR_ICON_CONTRACT_Y + 2 * CHAR_ICON_SPACING);
  }
}

static void DrawStringRight(const wchar_t *str, uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                            Font);

static void PrintStat(uint32_t const change_time, uint16_t const stat_gone_up_bit,
                      int8_t const stat_val, int16_t const x, int16_t const y) {
  uint8_t const colour =
      change_time == 0 || GetJA2Clock() >= CHANGE_STAT_RECENTLY_DURATION + change_time
...
static void DrawCharStats(SOLDIERTYPE const &s) {
  ...
  uint16_t const up = s.usValueGoneUp;
...
static void DrawString(const wchar_t *pString, uint16_t uiX, uint16_t uiY, Font);
static void DrawStringCentered(const wchar_t *str, uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                               Font);

static void DrawCharHealth(SOLDIERTYPE const &s) {
  if (s.bAssignment != ASSIGNMENT_POW) {
    int8_t const life = s.bLife;
    int8_t const life_max = s.bLifeMax;
    wchar_t buf[9];
    ...
    int16_t x;
    int16_t y;
    FindFontCenterCoordinates(CHAR_HP_X, CHAR_HP_Y, CHAR_HP_WID, CHAR_HP_HEI, buf, CHAR_FONT, &x,
                              &y);

    // How is character's life?
    uint32_t const health_percent = life_max > 0 ? 100 * life / life_max : 0;
    uint8_t const cur_colour = health_percent == 0 ? FONT_METALGRAY :  // Dead
                                   health_percent < 25 ? FONT_RED
                                                       :  // Very bad
                                   health_percent < 50 ? FONT_YELLOW
                                                       :  // Not good
    ...
    uint8_t const max_colour = !recent_change                      ? CHAR_TEXT_FONT_COLOR
    ...
    DrawString(buf, x, CHAR_HP_Y, CHAR_FONT);
  } else {  // POW - health unknown
    SetFontForeground(CHAR_TEXT_FONT_COLOR);
    DrawStringCentered(pPOWStrings[1], CHAR_HP_X, CHAR_HP_Y, CHAR_HP_WID, CHAR_HP_HEI, CHAR_FONT);
  }
}

static void ConvertMinTimeToETADayHourMinString(uint32_t uiTimeInMin, wchar_t *sString,
                                                size_t Length);
...
static void DrawCharacterInfo(SOLDIERTYPE const &s) {
  ...
  // Draw particular info about a character that are neither attributes nor
  // skills
  SetFontAttributes(CHAR_FONT, CHAR_TEXT_FONT_COLOR);
  ...
  DrawStringCentered(assignment, CHAR_ASSIGN_X, CHAR_ASSIGN1_Y, CHAR_ASSIGN_WID, CHAR_ASSIGN_HEI,
                     CHAR_FONT);
  ...
  switch (s.bAssignment) {
    ...
    case REPAIR:
      assignment2 = s.fFixingRobot ? pRepairStrings[3] :  // Robot
#if 0                                                     /* XXX was commented out */
				s.fFixingSAMSite              ? pRepairStrings[1] : // SAM site
#endif
                        s.bVehicleUnderRepairID != -1
                        ? pShortVehicleStrings[GetVehicle(s.bVehicleUnderRepairID).ubVehicleType]
                        :                   // Vehicle
    ...
    default: {
      ...
      if (g && PlayerGroupInMotion(g)) {  // Show ETA
        uint32_t const arrival_time = GetWorldTotalMin() + CalculateTravelTimeOfGroup(g);
  ...
  DrawStringCentered(assignment2, CHAR_ASSIGN_X, CHAR_ASSIGN2_Y, CHAR_ASSIGN_WID, CHAR_ASSIGN_HEI,
                     CHAR_FONT);
  ...
  if (s.bLife > 0) {
    if (s.ubWhatKindOfMercAmI == MERC_TYPE__AIM_MERC || s.ubProfile == SLAY) {
      ...
      int32_t time_remaining = s.iEndofContractTime - GetWorldTotalMin();

      if (s.bAssignment == IN_TRANSIT &&
          time_remaining > (int32_t)(s.iTotalContractLength *
                                     NUM_MIN_IN_DAY)) { /* If the merc is in transit and if the
                                                         * time left on the contract is
                                                         * greater than the contract time */
        time_remaining = s.iTotalContractLength * NUM_MIN_IN_DAY;
      }

      if (time_remaining >= 24 * 60) {  // More than a day, display in green
        ...
        swprintf(buf, lengthof(buf), L"%.1f%ls/%d%ls", time_left, days, s.iTotalContractLength,
                 days);
      } else {  // Less than a day, display hours left in red
        if (time_remaining > 5) time_remaining += 59;
        time_remaining /= 60;
        wchar_t const *const hours = gpStrategicString[STR_PB_HOURS_ABBREVIATION];
        wchar_t const *const days = gpStrategicString[STR_PB_DAYS_ABBREVIATION];
        swprintf(buf, lengthof(buf), L"%d%ls/%d%ls", time_remaining, hours, s.iTotalContractLength,
                 days);
      }
      contract = buf;
    } else if (s.ubWhatKindOfMercAmI == MERC_TYPE__MERC) {
      int32_t const been_hired_for = GetWorldTotalMin() / NUM_MIN_IN_DAY - s.iStartContractTime;
      wchar_t const *const days = gpStrategicString[STR_PB_DAYS_ABBREVIATION];
      swprintf(buf, lengthof(buf), L"%d%ls/%d%ls", p.iMercMercContractLength, days, been_hired_for,
               days);
      contract = buf;
    }
  }
  SetFontForeground(CHAR_TEXT_FONT_COLOR);
  DrawStringCentered(contract, CHAR_TIME_REMAINING_X, CHAR_TIME_REMAINING_Y,
                     CHAR_TIME_REMAINING_WID, CHAR_TIME_REMAINING_HEI, CHAR_FONT);

  // Salary
  int32_t daily_cost;
  if (s.ubWhatKindOfMercAmI == MERC_TYPE__AIM_MERC) {  // Daily rate
    switch (s.bTypeOfLastContract) {
      case CONTRACT_EXTEND_2_WEEK:
        daily_cost = p.uiBiWeeklySalary / 14;
        break;
      case CONTRACT_EXTEND_1_WEEK:
        daily_cost = p.uiWeeklySalary / 7;
        break;
      default:
  ...
  if (p.sMedicalDepositAmount > 0) {
    ...
    DrawStringRight(buf, CHAR_MEDICAL_X, CHAR_MEDICAL_Y, CHAR_MEDICAL_WID, CHAR_MEDICAL_HEI,
                    CHAR_FONT);
  }

  wchar_t const *const morale = s.bAssignment == ASSIGNMENT_POW ? pPOWStrings[1]
                                                                :  // POW - morale unknown
                                    s.bLife == 0 ? L""
                                                 : GetMoraleString(s);
  DrawStringCentered(morale, CHAR_MORALE_X, CHAR_MORALE_Y, CHAR_MORALE_WID, CHAR_MORALE_HEI,
                     CHAR_FONT);
}
...
int32_t GetPathTravelTimeDuringPlotting(PathSt *pPath) {
  ...
  while (pPath->pNext) {
    if (!fSkipFirstNode) {
      ...
      pCurrent.x = (uint8_t)(pPath->uiSectorId % MAP_WORLD_X);
      pCurrent.y = (uint8_t)(pPath->uiSectorId / MAP_WORLD_X);

      // grab the next location
      pNext.x = (uint8_t)(pPath->pNext->uiSectorId % MAP_WORLD_X);
      pNext.y = (uint8_t)(pPath->pNext->uiSectorId / MAP_WORLD_X);
...
static int32_t GetGroundTravelTimeOfSoldier(const SOLDIERTYPE *s);

static void DisplayGroundEta() {
  ...
  const uint32_t iTotalTime = GetGroundTravelTimeOfSoldier(s);
  ...
  if (iTotalTime < 60 * 24) {
    ...
    uint32_t Minutes = iTotalTime % 60;
    uint32_t Hours = iTotalTime / 60;
    ...
    // show days and hours
    uint32_t Hours = iTotalTime / 60 % 24;
    uint32_t Days = iTotalTime / (60 * 24);
...
struct HighLightState {
  int32_t colour_idx;
  bool delta;
  int32_t old_highlight;
};

static void HighLightSelection(HighLightState &state, int32_t const line, uint16_t const x,
                               uint16_t const w, BOOLEAN (&predicate)(int16_t)) {
  ...
  uint16_t const colour = GlowColor(state.colour_idx);
  int32_t const h = Y_SIZE + Y_OFFSET;
  for (int16_t i = 0; i != MAX_CHARACTER_COUNT; ++i) {
    if (!predicate(i)) continue;

    uint16_t y = Y_START - 1 + i * h;
...
static void HighLightAssignLine() {
  static HighLightState state = {STARTING_COLOR_NUM, false, MAX_CHARACTER_COUNT + 1};
  HighLightSelection(state, giAssignHighLine, ASSIGN_X, ASSIGN_WIDTH,
                     IsCharacterSelectedForAssignment);
}

static void HighLightDestLine() {
  static HighLightState state = {STARTING_COLOR_NUM, false, MAX_CHARACTER_COUNT + 1};
  HighLightSelection(state, giDestHighLine, DEST_ETA_X, DEST_ETA_WIDTH,
                     CharacterIsGettingPathPlotted);
}

static void HighLightSleepLine() {
  static HighLightState state = {STARTING_COLOR_NUM, false, MAX_CHARACTER_COUNT + 1};
...
static void EnableDisableTeamListRegionsAndHelpText();
...
static void DisplayCharacterList() {
  ...
  for (int16_t i = 0; i != MAX_CHARACTER_COUNT; ++i) {
    ...
    uint8_t foreground =
        i == (int16_t)giHighLine           ? FONT_WHITE
        : s.bLife == 0                     ? FONT_METALGRAY
        : CharacterIsGettingPathPlotted(i) ? FONT_LTBLUE
                                           :
                                           /* Not in current sector? */
            s.sSectorX != sSelMapX || s.sSectorY != sSelMapY || s.bSectorZ != iCurrentMapSectorZ
            ? 5
            :
            /* Mobile? */
            s.bAssignment < ON_DUTY || s.bAssignment == VEHICLE ? FONT_YELLOW
                                                                : FONT_MAP_DKYELLOW;
    SetFontForeground(foreground);

    uint16_t y = Y_START + i * (Y_SIZE + Y_OFFSET) + 1;
    if (i >= FIRST_VEHICLE) y += 6;
    ...
    GetMapscreenMercDepartureString(s, str, lengthof(str),
                                    foreground != FONT_WHITE ? &foreground : 0);
    SetFontForeground(foreground);
...
static void AddTeamPanelSortButtonsForMapScreen();
static void BlitBackgroundToSaveBuffer();
static void CheckForAndRenderNewMailOverlay();
static void CheckForInventoryModeCancellation();
static void CheckIfPlottingForCharacterWhileAirCraft();
static void CheckToSeeIfMouseHasLeftMapRegionDuringPathPlotting();
static void ContractButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void CreateDestroyMapCharacterScrollButtons();
static void CreateDestroyTrashCanRegion();
static void CreateMouseRegionsForTeamList();
static void DetermineIfContractMenuCanBeShown();
static void FaceRegionBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void HandleAnimatedCursorsForMapScreen();
static void HandleAssignmentsDoneAndAwaitingFurtherOrders();
static void HandleChangeOfHighLightedLine();
static void HandleChangeOfInfoChar();
static void HandleCharBarRender();
static void HandleCommonGlowTimer();
static void HandleContractTimeFlashForMercThatIsAboutLeave();
static void HandleCursorOverRifleAmmo();
static void HandleHighLightingOfLinesInTeamPanel();
static void HandleMapInventoryCursor();
static uint32_t HandleMapUI();
static void HandlePostAutoresolveMessages();
static void HandlePreBattleInterfaceWithInventoryPanelUp();
static void HandleSpontanousTalking();
static void InitPreviousPaths();
static void InterruptTimeForMenus();
static void ItemRegionBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void ItemRegionMvtCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void MapScreenMarkRegionBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void MapscreenMarkButtonsDirty();
static void MonitorMapUIMessage();
static void PlotPermanentPaths();
static void PlotTemporaryPaths();
static void RenderMapCursorsIndexesAnims();
static void RenderTeamRegionBackground();
static BOOLEAN RequestGiveSkyriderNewDestination();
static void ResetAllSelectedCharacterModes();
static void UpDateStatusOfContractBox();
static void UpdateBadAssignments();
static void UpdateCursorIfInLastSector();
static void UpdatePausedStatesDueToTimeCompression();
static void UpdateStatusOfMapSortButtons();
static void UpdateTheStateOfTheNextPrevMapScreenCharacterButtons();

ScreenID MapScreenHandle() try {
  ...
  if (!fInMapMode) {
    ...
    // if the current time compression mode is something legal in mapscreen,
    // keep it
    ...
    // NOTE: Must come after border buttons are created, since it may toggle
    // them!
    if (!AnyMercsHired()) {
    ...
    {
      // Only select start sector, if there is no current selection, otherwise
      // leave it as is.
    ...
    // tactical scroll of messages not allowed to beep until new message is
    // added in tactical
    ...
    giMapContractButton =
        QuickCreateButtonImg(INTERFACEDIR "/contractbutton.sti", 0, 1, CONTRACT_X + 5,
                             CONTRACT_Y - 1, MSYS_PRIORITY_HIGHEST - 5, ContractButtonCallback);
    giMapContractButton->SpecifyGeneralTextAttributes(pContractButtonString, MAP_SCREEN_FONT,
                                                      CHAR_TEXT_FONT_COLOR, FONT_BLACK);
  ...
  */
  ...
  // are we attempting to plot a foot/vehicle path during aircraft mode..if so,
  // stop it
  ...
  if (!fDisableDueToBattleRoster) {
    /*
                    // ATE: OK mark is rendering the item every frame - which
       isn't good
  ...
  if (iHelicopterVehicleId != -1 && fShowAircraftFlag && iCurrentMapSectorZ == 0 &&
      !fShowMapInventoryPool) {
    // this is done on EVERY frame, I guess it beats setting entire map dirty
    // all the time while he's moving...
  ...
  if (fShowDescriptionFlag) {
    // unmark done button
    if (gpItemDescObject->usItem == MONEY || Item[gpItemDescObject->usItem].usItemClass & IC_GUN) {
      MapscreenMarkButtonsDirty();
    }
  ...
  if (fShowMapScreenMovementList) {
    // redisplay Movement box to blit it over any border buttons, since if long
    // enough it can overlap them
    ForceUpDateOfBox(ghMoveBox);
    DisplayOnePopupBox(ghMoveBox, FRAME_BUFFER);
  }

  if (fShowContractMenu) {
    // redisplay Contract box to blit it over any map sort buttons, since they
    // overlap
  ...
  if (gfFirstMapscreenFrame) {
    ...
    // handle exiting from mapscreen due to both exit button clicks and keyboard
    // equivalents
...
static void DrawString(const wchar_t *pString, uint16_t uiX, uint16_t uiY, Font const font) {
...
static void RenderMapHighlight(int16_t sMapX, int16_t sMapY, uint16_t usLineColor,
                               BOOLEAN fStationary);
static void RestoreMapSectorCursor(int16_t sMapX, int16_t sMapY);
...
static void RenderMapCursorsIndexesAnims() {
  ...
  // handle highlighting of selected sector ( YELLOW ) - don't show it while
  // plotting movement
  if (fDrawCursors && bSelectedDestChar == -1 && !fPlotForHelicopter) {
    ...
    // always render this one, it's too much of a pain detecting overlaps with
    // the white cursor otherwise
...
static BOOLEAN AnyMovableCharsInOrBetweenThisSector(int16_t sSectorX, int16_t sSectorY,
                                                    int8_t bSectorZ);
static BOOLEAN CanMoveBullseyeAndClickedOnIt(int16_t sMapX, int16_t sMapY);
static void CancelChangeArrivalSectorMode();
static void CancelOrShortenPlottedPath();
static void CreateBullsEyeOrChopperSelectionPopup();
...
static void StartChangeSectorArrivalMode();
static void StartConfirmMapMoveMode(int16_t sMapY);
  ...
  switch (new_event) {
    ...
    case MAP_EVENT_PLOT_PATH:
      ...
      /*
         ...
         to map grid values for zoomed in if(fZoomFlag)
                                                       {
      ...
      */
      ...
      if (fPlotForHelicopter) {
        ...
        SOLDIERTYPE *const s = gCharactersList[bSelectedDestChar].merc;
        const int16_t dst_sector = GetLastSectorIdInCharactersPath(s);
    ...
    case MAP_EVENT_CLICK_SECTOR:
      ...
      if (GetMouseMapXY(&sMapX, &sMapY)) {
        /*
                                        if (fZoomFlag)
                                        {
                                                // convert to zoom out coords from
           screen coords sMapX = ( int16_t )( iZoomX / MAP_GRID_X + sMapX ) / 2;
                              sMapY = ( int16_t )( iZoomY / MAP_GRID_Y + sMapY ) /
           2;
                                                //sMapX = ( int16_t ) ( ( ( iZoomX )
           / ( MAP_GRID_X * 2) ) + sMapX / 2 );
                                                //sMapX = ( int16_t ) ( ( ( iZoomY )
           / ( MAP_GRID_Y * 2) ) + sMapY / 2 );
                                        }
        */
        ...
        if (fMapInventoryItem) {
          ...
          if (!fShowMapInventoryPool) {
            // start it up ( NOTE: for the item OWNER'S sector, regardless of
            // which sector player clicks )
        ...
        // don't permit other click handling while item is in cursor (entering PBI
        // would permit item teleports, etc.)
        ...
        if (gfInChangeArrivalSectorMode) {
          if (SectorInfo[(SECTOR(sMapX, sMapY))].ubTraversability[THROUGH_STRATEGIC_MOVE] !=
            ...
            if (!StrategicMap[CALCULATE_STRATEGIC_INDEX(sMapX, sMapY)].fEnemyAirControlled) {
              ...
              // change arrival sector for all mercs currently in transit who are
              // showing up at the landing zone
        ...
        } else  // not already changing arrival sector
        {
          if (CanMoveBullseyeAndClickedOnIt(sMapX, sMapY)) {
            ...
            // NOTE: The helicopter icon is NOT necessarily directly over the
            // helicopter's current sector!!!
        ...
        if (fShowAircraftFlag) {
          ...
          if (bSelectedDestChar == -1 && !fPlotForHelicopter) {
            // if we're on the surface level, and the click is over the helicopter
            // icon NOTE: The helicopter icon is NOT necessarily directly over the
            // helicopter's current sector!!!
        ...
        {
          // sector must be already selected to initiate movement plotting!  This
          // is to allow selecting sectors with mercs in them without necessarily
          // initiating movement right away.
          if (fWasAlreadySelected) {
            ...
            if (AnyMovableCharsInOrBetweenThisSector(sMapX, sMapY, (int8_t)iCurrentMapSectorZ)) {
              ...
              if (iCurrentMapSectorZ == 0) {
                ...
                // NOTE: must allow move box to come up, since there may be
                // movable characters between sectors which are unaffected by
                // combat / hostiles / air raid in the sector proper itself!! This
                // also allows all strategic movement error handling to be
                // centralized in CanCharacterMoveInStrategic()
...
static bool HandleKeyESC() {
  ...
  if (gfPreBattleInterfaceActive && !gfPersistantPBI) {  // Non persistant PBI.  Allow ESC to close
                                                         // it and return to mapscreen.
  ...
  // in militia popup?
  else if (sSelectedMilitiaTown != 0 && sGreensOnCursor == 0 && sRegularsOnCursor == 0 &&
           sElitesOnCursor == 0) {
...
static void Teleport() {
  ...
  // check to see if this person is moving, if not...then assign them to mvt
  // group
  if (s.ubGroupID == 0) {
  ...
  int16_t const sDeltaX = sMapX - s.sSectorX;
  int16_t const sDeltaY = sMapY - s.sSectorY;
  int16_t sPrevX;
  int16_t sPrevY;
  ...
  // set where they are, were/are going, then make them arrive there and check
  // for battle
...
static void ChangeCharacterListSortMethod(int32_t iValue);
static void RequestContractMenu();
static void RequestToggleMercInventoryPanel();
static void SelectAllCharactersInSquad(int8_t bSquadNumber);
...
static void HandleModNone(uint32_t const key, MapEvent &new_event) {
  switch (key) {
    ...
    case 'a':
      if (gfPreBattleInterfaceActive) {  // Activate autoresolve in prebattle
                                         // interface.
        ActivatePreBattleAutoresolveAction();
      } else if (!fShowMapInventoryPool)  // only handle border button keyboard
                                          // equivalents if the button is visible!
      {
        ToggleAirspaceMode();
      }
      break;
    ...
    case 'h':
      // ARM: Feb01/98 - Cancel out of mapscreen movement plotting if Help
...
static void HandleModShift(uint32_t const key) {
  switch (key) {
    case '0':
    case '1':
    case '2':
...
static void HandleModCtrl(uint32_t const key) {
  switch (key) {
    case 'a':
      if (CHEATER_CHEAT_LEVEL()) {
        gfAutoAmbush ^= 1;
        wchar_t const *const msg =
            gfAutoAmbush ? L"Enemy ambush test mode enabled." : L"Enemy ambush test mode disabled.";
    ...
    case 't':
      if (CHEATER_CHEAT_LEVEL()) Teleport();
      break;
    ...
    case 'z':
      if (CHEATER_CHEAT_LEVEL()) {
        gfAutoAIAware ^= 1;
        wchar_t const *const msg =
            gfAutoAIAware ? L"Strategic AI awareness maxed." : L"Strategic AI awareness normal.";
        ScreenMsg(FONT_WHITE, MSG_TESTVERSION, msg);
      }
      break;
  }
...
static void HandleModAlt(uint32_t const key) {
  switch (key) {
    case 'a':
      if (giHighLine != -1) {
    ...
    case 'l':
      // Although we're not actually going anywhere, we must still be in a state
...
static void DestroyMouseRegionsForTeamList();
...
void EndMapScreen(BOOLEAN fDuringFade) {
  ...
  // update paused states, we are exiting...need to reset for any pathing or
  // menus displayed
  UnLockPauseState();
  UpdatePausedStatesDueToTimeCompression();
...
static BOOLEAN GetMapXY(int16_t sX, int16_t sY, int16_t *psMapWorldX, int16_t *psMapWorldY);
...
static void RenderMapHighlight(int16_t sMapX, int16_t sMapY, uint16_t usLineColor,
  ...
  */
  ...
  /*
          else if (!IsTheCursorAllowedToHighLightThisSector(sMapX , sMapY) &&
    fZoomFlag && fStationary)
          {
                  return;
          }
          else if (!IsTheCursorAllowedToHighLightThisSector(iZoomX / (MAP_GRID_X
    * 2) + sMapX / 2, iZoomY / (MAP_GRID_Y * 2) + sMapY / 2) && fZoomFlag &&
    !fStationary)
    {
                  return;
          }
  ...
  */
  ...
  /*
          if (fZoomFlag)
          {
                  ...
                  RectangleDraw( TRUE, sScreenX-MAP_GRID_X, sScreenY-MAP_GRID_Y
     - 1, sScreenX +  MAP_GRID_ZOOM_X - MAP_GRID_X, sScreenY +  MAP_GRID_ZOOM_Y
     - MAP_GRID_Y - 1, usLineColor, pDestBuf ); InvalidateRegion(
     sScreenX-MAP_GRID_X - 3, sScreenY-MAP_GRID_Y - 4, sScreenX +
     DMAP_GRID_ZOOM_X - MAP_GRID_X, sScreenY + DMAP_GRID_ZOOM_Y - MAP_GRID_Y - 1
     );
          }
          else
  */
  {
    ...
    InvalidateRegion(sScreenX, sScreenY - 2, sScreenX + DMAP_GRID_X + 1,
                     sScreenY + DMAP_GRID_Y - 1);
  }

  RestoreClipRegionToFullScreenForRectangle(uiDestPitchBYTES);
}

static BOOLEAN CheckIfClickOnLastSectorInPath(int16_t sX, int16_t sY);
static void DestinationPlottingCompleted();
  ...
  if (gMapViewRegion.uiFlags & MSYS_MOUSE_IN_AREA) {
    ...
    } else  // L-button is NOT down at the moment
    {
      if (fLBBeenPressedInMapView) {
        ...
        // left click cancels MAP UI messages, unless we're in confirm map move
        // mode
        ...
        // game just started or we're in the prebattle interface or if we are
        // about to hit pre-battle
        if (DidGameJustStart() || gfPreBattleInterfaceActive || fDisableMapInterfaceDueToBattle) {
          return;
        }
        ...
        if (bSelectedDestChar != -1 || fPlotForHelicopter) {
          ...
          /*
                                                  // translate screen values to
             map grid values for zoomed in if(fZoomFlag)
                                                  {
                                                          sMapX=(uint16_t)iZoomX/MAP_GRID_X+sMapX;
                                                          sMapX=sMapX/2;
                                                          sMapY=(uint16_t)iZoomY/MAP_GRID_Y+sMapY;
        ...
        } else  // not plotting movement
        {
          ...
          if (!fEndPlotting && !fJustFinishedPlotting) {
            // make this sector selected / trigger movement box / start
            // helicopter plotting / changing arrival sector
...
static void HandleMilitiaRedistributionClick();
  ...
  if (gMapViewRegion.uiFlags & MSYS_MOUSE_IN_AREA) {
    ...
    } else  // R-button is NOT down at the moment
    {
      if (fRBBeenPressedInMapView) {
        ...
        // right click cancels MAP UI messages, unless we're in confirm map move
        // mode
        if (g_ui_message_overlay != NULL && !gfInConfirmMapMoveMode) {
          CancelMapUIMessage();
          return;
        ...
        if (bSelectedDestChar != -1 || fPlotForHelicopter) {
          ...
          // sector must be selected to bring up militia or town info boxes for
          // it
          if ((sMapX == sSelMapX) && (sSelMapY == sMapY)) {
            ...
            {
              // if on the surface, or a real underground sector (we've visited
              // it)
              if ((iCurrentMapSectorZ == 0) ||
...
static void MAPInvClickCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void MAPInvClickCamoCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void MAPInvMoveCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void MAPInvMoveCamoCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
void CreateDestroyMapInvButton() {
  ...
  if (fShowInventoryFlag && !fOldShowInventoryFlag) {
    ...
    InitInvSlotInterface(gMapScreenInvPocketXY, &gSCamoXY, MAPInvMoveCallback, MAPInvClickCallback,
                         MAPInvMoveCamoCallback, MAPInvClickCamoCallback);
    ...
    // force immediate reblit of item in HANDPOS now that it's not blitted while
    // in inventory mode
    fCharacterInfoPanelDirty = TRUE;
  }
}

static void BltCharInvPanel() {
  wchar_t sString[32];
  ...
  FindFontRightCoordinates(MAP_ARMOR_X, MAP_ARMOR_Y, MAP_ARMOR_W, MAP_ARMOR_H, sString, BLOCKFONT2,
                           &usX, &usY);
  ...
  FindFontRightCoordinates(MAP_WEIGHT_X, MAP_WEIGHT_Y, MAP_WEIGHT_W, MAP_WEIGHT_H, sString,
                           BLOCKFONT2, &usX, &usY);
  MPrint(usX, usY, sString);
  ...
  FindFontRightCoordinates(MAP_CAMO_X, MAP_CAMO_Y, MAP_CAMO_W, MAP_CAMO_H, sString, BLOCKFONT2,
                           &usX, &usY);
  MPrint(usX, usY, sString);

  if (InKeyRingPopup() || InItemStackPopup()) {
    // shade the background
    guiSAVEBUFFER->ShadowRect(PLAYER_INFO_X, PLAYER_INFO_Y, PLAYER_INFO_X + 261,
...
static void MAPInvMoveCallback(MOUSE_REGION *pRegion, int32_t iReason) {
  ...
  //  if( ( iReason == MSYS_CALLBACK_REASON_MOVE ) || ( iReason ==
...
void MAPInternalInitItemDescriptionBox(OBJECTTYPE *pObject, uint8_t ubStatusIndex,
  ...
  InternalInitItemDescriptionBox(pObject, MAP_ITEMDESC_START_X, MAP_ITEMDESC_START_Y, ubStatusIndex,
                                 pSoldier);
...
static void MAPBeginItemPointer(SOLDIERTYPE *pSoldier, uint8_t ubHandPos);
    ...
    } else  // item in cursor
    {
      ...
      if (uiHandPos == HANDPOS || uiHandPos == SECONDHANDPOS || uiHandPos == HELMETPOS ||
        ...
        if (ValidAttachment(usNewItemIndex, usOldItemIndex)) {
          ...
          DoScreenIndependantMessageBox( Message[ STR_MERGE_ITEMS ],
          MSG_BOX_FLAG_YESNO, MergeMessageBoxCallBack ); return;
          */
        }
...
static void RenderAttributeStringsForUpperLeftHandCorner(SGPVSurface *const uiBufferToRenderTo) {
  ...
  DrawString(pUpperLeftMapScreenStrings[0],
             220 - StringPixLength(pUpperLeftMapScreenStrings[0], CHAR_FONT) / 2, 6, CHAR_FONT);
...
static void DisplayThePotentialPathForCurrentDestinationCharacterForMapScreenInterface(
    int16_t sMapX, int16_t sMapY) {
...
static void ChangeMapScreenMaskCursor(uint16_t usCursor);
...
static void ClearPreviousPaths();
static void RestorePreviousPaths();

void AbortMovementPlottingMode() {
  // invalid if we're not plotting movement
  Assert(bSelectedDestChar != -1 || fPlotForHelicopter);

  // make everybody go back to where they were going before this plotting
  // session started
  ...
  // clear the character's temporary path (this is the route being constantly
  // updated on the map)
  pTempCharacterPath = ClearStrategicPathList(pTempCharacterPath, 0);

  // clear the helicopter's temporary path (this is the route being constantly
  // updated on the map)
...
static void CheckToSeeIfMouseHasLeftMapRegionDuringPathPlotting() {
  ...
  if ((gMapViewRegion.uiFlags & MSYS_MOUSE_IN_AREA) == 0) {
    if (fInArea) {
      ...
      if (fTempPathAlreadyDrawn &&
          (bSelectedDestChar != -1 || fPlotForHelicopter || fDrawTempHeliPath)) {
...
static void RenderCharacterInfoBackground();
...
static void MakeRegion(MOUSE_REGION *const r, const uint32_t idx, const uint16_t x,
                       const uint16_t y, const uint16_t w, MOUSE_CALLBACK const move,
                       MOUSE_CALLBACK const click, const wchar_t *const help) {
  MSYS_DefineRegion(r, x, y, x + w, y + Y_SIZE + 1, MSYS_PRIORITY_NORMAL + 1, MSYS_NO_CURSOR, move,
                    click);
  MSYS_SetRegionUserData(r, 0, idx);
  r->SetFastHelpText(help);
}

static void TeamListAssignmentRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListAssignmentRegionMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListContractRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListContractRegionMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListDestinationRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListDestinationRegionMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListInfoRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListInfoRegionMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListSleepRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TeamListSleepRegionMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);

static void CreateMouseRegionsForTeamList() {
  // will create mouse regions for assignments, path plotting, character info
  // selection

  // the info region...is the background for the list itself
  for (uint32_t i = 0; i < MAX_CHARACTER_COUNT; ++i) {
    const uint16_t y = Y_START + i * (Y_SIZE + 2) + (i >= FIRST_VEHICLE ? 6 : 0);

    const uint16_t w = NAME_WIDTH;
    CharacterRegions &r = g_character_regions[i];
    MakeRegion(&r.name, i, NAME_X, y, w, TeamListInfoRegionMvtCallBack,
               TeamListInfoRegionBtnCallBack,
               pMapScreenMouseRegionHelpText[0]);  // name region
    MakeRegion(&r.assignment, i, ASSIGN_X, y, ASSIGN_WIDTH, TeamListAssignmentRegionMvtCallBack,
               TeamListAssignmentRegionBtnCallBack,
               pMapScreenMouseRegionHelpText[1]);  // assignment region
    MakeRegion(&r.sleep, i, SLEEP_X, y, SLEEP_WIDTH, TeamListSleepRegionMvtCallBack,
               TeamListSleepRegionBtnCallBack,
               pMapScreenMouseRegionHelpText[5]);  // sleep region
    // same function as name regions, so uses the same callbacks
    MakeRegion(&r.location, i, LOC_X, y, LOC_WIDTH, TeamListInfoRegionMvtCallBack,
               TeamListInfoRegionBtnCallBack,
               pMapScreenMouseRegionHelpText[0]);  // location region
    MakeRegion(&r.destination, i, DEST_ETA_X, y, DEST_ETA_WIDTH,
               TeamListDestinationRegionMvtCallBack, TeamListDestinationRegionBtnCallBack,
               pMapScreenMouseRegionHelpText[2]);  // destination region
    MakeRegion(&r.contract, i, TIME_REMAINING_X, y, TIME_REMAINING_WIDTH,
               TeamListContractRegionMvtCallBack, TeamListContractRegionBtnCallBack,
               pMapScreenMouseRegionHelpText[3]);  // contract region
  }
}
...
static BOOLEAN HandleCtrlOrShiftInTeamPanel(int8_t bCharNumber);
...
static void TeamListDestinationRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (s != NULL) {
      ...
      // select all characters/vehicles that MUST accompany the clicked merc
      // (same squad/vehicle)
      SelectUnselectedMercsWhoMustMoveWithThisGuy();

      // Find out if this guy and everyone travelling with him is allowed to
      // move strategically NOTE: errors are reported within...
      if (CanChangeDestinationForChar(*s)) {
        ...
        // remember the current paths for all selected characters so we can
        // restore them if need be
...
static void TeamListSleepRegionBtnCallBack(MOUSE_REGION *pRegion, int32_t iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    // set to new info character...make sure is valid.. not in transit and alive
    // and concious
...
static void ContractRegionBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
  ...
  if (GetMouseMapXY(&sMapX, &sMapY)) {
    if (fPlotForHelicopter) {
      ...
      /*
                              ...
                              {
                                      sMapX =  ( int16_t )( ( ( iZoomX ) / (
         WORLD_MAP_X ) ) + sMapX ); sMapX /= 2;

                                      sMapY =  ( int16_t )( ( ( iZoomY ) / (
         WORLD_MAP_X ) ) + sMapY ); sMapY /= 2;
                              }
      */
...
static void DisplayIconsForMercsAsleep();
...
static void HandleShadingOfLinesForContractMenu();
...
static void CheckIfPlottingForCharacterWhileAirCraft() {
  // if we are in aircraft mode and plotting for character, reset plotting
  // character
  if (fShowAircraftFlag) {
...
static void HandleShadingOfLinesForContractMenu() {
  ...
  if (s->ubWhatKindOfMercAmI == MERC_TYPE__AIM_MERC && s->bLife >= OKLIFE) {
    ...
    int32_t const balance = LaptopSaveInfo.iCurrentBalance;
    ShadeStringInBox(box, CONTRACT_MENU_DAY, p.sSalary > balance);
    ShadeStringInBox(box, CONTRACT_MENU_WEEK, (int32_t)p.uiWeeklySalary > balance);
    ShadeStringInBox(box, CONTRACT_MENU_TWO_WEEKS, (int32_t)p.uiBiWeeklySalary > balance);
  } else {
  ...
  // if THIS soldier is involved in a fight (dismissing in a hostile sector IS
  // ok...)
  ShadeStringInBox(box, CONTRACT_MENU_TERMINATE,
...
void ReBuildCharactersList() {
  ...
  // add in characters
  for (int16_t sCount = 0; sCount < MAX_CHARACTER_COUNT; ++sCount) {
    // clear this slot
    gCharactersList[sCount].merc = NULL;
  }

  FOR_EACH_IN_TEAM(s, OUR_TEAM) { AddCharacter(s); }
...
static void EnableDisableTeamListRegionsAndHelpText() {
  // check if valid character here, if so, then do nothing..other wise set help
  // text timer to a gazillion
  int8_t bCharNum;
  ...
  for (bCharNum = 0; bCharNum < MAX_CHARACTER_COUNT; bCharNum++) {
    ...
    if (s == NULL) {
      ...
      // if the character can't move at this time, then the region handler must
      // be able to tell the player why not
...
static void HandleSpontanousTalking() {
  // simply polls if the talking guy is done, if so...send an end command to
  // continue dialogue
  if (!DialogueActive()) {
    SOLDIERTYPE *const s = GetSelectedInfoChar();
...
static void HandleNewDestConfirmation(int16_t sMapX, int16_t sMapY);
static void RebuildWayPointsForAllSelectedCharsGroups();
...
static BOOLEAN CheckIfClickOnLastSectorInPath(int16_t sX, int16_t sY) {
  ...
  if (fLastSectorInPath) {
    ...
    // if we really did plot a path (this will skip message if left click on
    // current sector with no path)
...
static void UpdateCursorIfInLastSector() {
  ...
  // check to see if we are plotting a path, if so, see if we are highlighting
  // the last sector int he path, if so, change the cursor
  if (bSelectedDestChar != -1 || fPlotForHelicopter) {
    GetMouseMapXY(&sMapX, &sMapY);
    ...
    if (!fShowAircraftFlag) {
      if (bSelectedDestChar != -1) {
        ...
        if (sMapX + sMapY * MAP_WORLD_X ==
            GetLastSectorIdInCharactersPath(gCharactersList[bSelectedDestChar].merc)) {
...
static void HandleChangeOfHighLightedLine() {
  ...
  if (giHighLine != iOldHighLine) {
    ...
    if (giHighLine == -1) {
      ...
      // don't do during plotting, allowing selected character to remain
      // highlighted and their destination column to glow!
      if (bSelectedDestChar == -1 && !fPlotForHelicopter) {
        giDestHighLine = -1;
      }
    }
...
static void HandleCharBarRender() {
  ...
  if (s->bLife != 0 && s->bAssignment != ASSIGNMENT_DEAD && s->bAssignment != ASSIGNMENT_POW) {
...
static void TrashCanBtnCallback(MOUSE_REGION *, int32_t const reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (OBJECTTYPE *const o = gpItemPointer) {
      ...
      DoMapMessageBox(MSG_BOX_BASIC_STYLE, msg, MAP_SCREEN, MSG_BOX_FLAG_YESNO,
                      TrashItemMessageBoxCallBack);
    }
  }
}

static void TrashCanMoveCallback(MOUSE_REGION *pRegion, int32_t iReason) {
  if (iReason & MSYS_CALLBACK_REASON_GAIN_MOUSE) {
    if (fMapInventoryItem) {
...
static void DoneInventoryMapBtnCallback(GUI_BUTTON *btn, int32_t reason);
...
static void CreateDestroyTrashCanRegion() {
  ...
  if (fShowInventoryFlag && !fCreated) {
    ...
    giMapInvDoneButton =
        QuickCreateButtonImg(INTERFACEDIR "/done_button2.sti", 0, 1, INV_BTN_X, INV_BTN_Y,
...
static void MonitorMapUIMessage() {
  ...
  if (g_ui_message_overlay != NULL) {
    ...
    {
      // and we've now exceed its period of maximum persistance (without user
      // input)
...
static void UpdateBadAssignments() {
  CFOR_EACH_IN_CHAR_LIST(c) { CheckIfSoldierUnassigned(c->merc); }
}
...
static BOOLEAN CharacterIsInLoadedSectorAndWantsToMoveInventoryButIsNotAllowed(
...
static void UpdateTheStateOfTheNextPrevMapScreenCharacterButtons() {
  ...
  /* ARM: Commented out at KM's request, it won't reenabled them when coming
     back from PBI, on Feb. 22, 99 else if (!fShowInventoryFlag) // make sure
     that we are in fact showing the mapscreen inventory
          {
                  return;
          }
  */
    ...
    // standard checks
    bool const enable = !fShowDescriptionFlag && GetNumberOfPeopleInCharacterList() >= 2 &&
...
static void PrevInventoryMapBtnCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    GoToPrevCharacterInList();
  }
}

static void NextInventoryMapBtnCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    GoToNextCharacterInList();
  }
}
...
static void CreateDestroyMapCharacterScrollButtons() {
  ...
  if (fInMapMode && !fCreated) {
    const int16_t prio = MSYS_PRIORITY_HIGHEST - 5;

    giCharInfoButton[0] =
        QuickCreateButtonImg(INTERFACEDIR "/map_screen_bottom_arrows.sti", 11, 4, -1, 6, -1, 67, 69,
                             prio, PrevInventoryMapBtnCallback);
    giCharInfoButton[1] =
        QuickCreateButtonImg(INTERFACEDIR "/map_screen_bottom_arrows.sti", 12, 5, -1, 7, -1, 67, 87,
...
void TellPlayerWhyHeCantCompressTime() {
  ...
  if (PauseStateLocked()) {
    ...
    // no mercs hired, ever
    DoMapMessageBox(MSG_BOX_BASIC_STYLE, pMapScreenJustStartedHelpText, MAP_SCREEN, MSG_BOX_FLAG_OK,
    ...
    DoMapMessageBox(MSG_BOX_BASIC_STYLE, gzLateLocalizedString[STR_LATE_02], MAP_SCREEN,
                    MSG_BOX_FLAG_OK, MapScreenDefaultOkBoxCallback);
    ...
    DoMapMessageBox(MSG_BOX_BASIC_STYLE, gzLateLocalizedString[STR_LATE_55], MAP_SCREEN,
                    MSG_BOX_FLAG_OK, MapScreenDefaultOkBoxCallback);
  }
  // ARM: THIS TEST SHOULD BE THE LAST ONE, BECAUSE IT ACTUALLY RESULTS IN
  // SOMETHING HAPPENING NOW. KM:  Except if we are in a creature lair and
  // haven't loaded the sector yet (no battle yet)
    ...
    if (OnlyHostileCivsInSector()) {
      ...
      // The NEW non-persistant PBI is used instead of a dialog box explaining
      // why we can't compress time.
      InitPreBattleInterface(0, false);
    }
...
static void MapSortBtnCallback(GUI_BUTTON *btn, int32_t reason) {
...
static void AddTeamPanelSortButtonsForMapScreen() {
  ...
  for (int32_t i = 0; i < MAX_SORT_METHODS; ++i) {
    giMapSortButton[i] =
        QuickCreateButtonImg(filename, iImageIndex[i], iImageIndex[i] + 6, gMapSortButtons[i].iX,
                             gMapSortButtons[i].iY, MSYS_PRIORITY_HIGHEST - 5, MapSortBtnCallback);
    giMapSortButton[i]->SetUserData(i);
    giMapSortButton[i]->SetFastHelpText(wMapScreenSortButtonHelpText[i]);
  }
}

static int16_t CalcLocationValueForChar(const SOLDIERTYPE *);
static int32_t GetContractExpiryTime(const SOLDIERTYPE *s);
static void SwapCharactersInList(int32_t iCharA, int32_t iCharB);
...
static void SortListOfMercsInTeamPanel(BOOLEAN fRetainSelectedMercs) {
  ...
  if (fRetainSelectedMercs) {
    ...
    // keep currently selected merc, but reset the selected list (which isn't
    // saved/restored, that's why)
    ResetSelectedListForMapScreen();
  }
...
static void HandleAssignmentsDoneAndAwaitingFurtherOrders() {
  ...
  // grab the current time
  const int32_t iCurrentTime = GetJA2Clock();
...
// Kris:  Added this function to blink the email icon on top of the laptop
// button whenever we are in
//       mapscreen and we have new email to read.
static void CheckForAndRenderNewMailOverlay() {
  if (fNewMailFlag) {
    if (GetJA2Clock() % 1000 < 667) {
      if (guiMapBottomExitButtons[MAP_EXIT_TO_LAPTOP]
              ->Clicked()) {  // button is down, so offset the icon
        ...
        if (!guiMapBottomExitButtons[MAP_EXIT_TO_LAPTOP]->Enabled()) {
          ...
          SGPVSurface::Lock l(FRAME_BUFFER);
          Blt16BPPBufferHatchRect(l.Buffer<uint16_t>(), l.Pitch(), &area);
        }
        InvalidateRegion(463, 417, 481, 430);
      }
    } else {  // The blink is now off, so mark the button dirty so that it'll
              // render next frame.
...
static bool CanToggleSelectedCharInventory() {
  ...
  if (!fShowInventoryFlag && (gpItemPointer || fMapInventoryItem) &&
...
bool MapCharacterHasAccessibleInventory(SOLDIERTYPE const &s) {
  return s.bAssignment != IN_TRANSIT && s.bAssignment != ASSIGNMENT_POW && !IsMechanical(s) &&
         s.ubWhatKindOfMercAmI != MERC_TYPE__EPC && s.bLife >= OKLIFE;
}
...
static void CheckForInventoryModeCancellation() {
  if (fShowInventoryFlag || fShowDescriptionFlag) {
    ...
    if (!CanToggleSelectedCharInventory()) {
      // get out of inventory mode if it's on!  (could have just bled below
      // OKLIFE)
      if (fShowInventoryFlag) {
        fShowInventoryFlag = FALSE;
        fTeamPanelDirty = TRUE;
...
void ChangeSelectedMapSector(int16_t sMapX, int16_t sMapY, int8_t bMapZ) {
  // ignore while map inventory pool is showing, or else items can be
  // replicated, since sector inventory always applies only to the currently
  // selected sector!!!
...
BOOLEAN CanExtendContractForSoldier(const SOLDIERTYPE *const s) {
  ...
  if (s->uiStatusFlags & SOLDIER_VEHICLE || s->bAssignment == IN_TRANSIT ||
      s->bAssignment == ASSIGNMENT_POW) {
    // can't extend contracts at this time
    return (FALSE);
  }

  // mercs below OKLIFE, M.E.R.C. mercs, EPCs, and the Robot use the Contract
  // menu so they can be DISMISSED/ABANDONED!
...
BOOLEAN CanChangeSleepStatusForSoldier(const SOLDIERTYPE *const pSoldier) {
  ...
  if (IsMechanical(*pSoldier) || (pSoldier->bAssignment == IN_TRANSIT) ||
  ...
  // this merc MAY be able to sleep/wake up - we'll allow player to click and
  // find out
  return (TRUE);
}
...
static void ExplainWhySkyriderCantFly();
...
static void CancelOrShortenPlottedPath() {
  ...
  */
  ...
  if (fShowAircraftFlag) {
    ...
    // try to delete portion of path AFTER the current sector for the helicopter
    uiReturnValue =
        ClearPathAfterThisSectorForCharacter(gCharactersList[bSelectedDestChar].merc, sMapX, sMapY);
  }
  ...
  // this triggers the path node animation to reset itself back to the first
  // node
  fDeletedNode = TRUE;
...
static int32_t GetContractExpiryTime(const SOLDIERTYPE *const pSoldier) {
...
void ChangeSelectedInfoChar(int8_t bCharNumber, BOOLEAN fResetSelectedList) {
  ...
  if (fResetSelectedList) {
    // reset selections of all listed characters.  Do this even if this guy is
    // already selected. NOTE: this keeps the currently selected info char
    // selected
    ResetSelectedListForMapScreen();
  }
...
void CopyPathToAllSelectedCharacters(PathSt *pPath) {
  ...
  CFOR_EACH_IN_CHAR_LIST(c) {
    ...
    if (cur_path != pPath) {
      ...
      // don't use CopyPathToCharactersSquadIfInOne(), it will whack the
      // original pPath by replacing that merc's path!
    }
  }
}
...
void CancelPathsOfAllSelectedCharacters() {
  ...
  // cancel destination for the clicked and ALL other valid & selected
  // characters with a route set
  CFOR_EACH_SELECTED_IN_CHAR_LIST(c) {
    SOLDIERTYPE *const pSoldier = c->merc;
    ...
    if (GetLengthOfMercPath(pSoldier) > 0) {
      ...
      // cancel the entire path (also clears vehicles for any passengers
      // selected, and handles reversing directions)
      if (pSoldier->uiStatusFlags & SOLDIER_VEHICLE) {
...
static void ConvertMinTimeToETADayHourMinString(const uint32_t uiTimeInMin, wchar_t *const sString,
                                                const size_t Length) {
  ...
  // there ain't enough room to show both the day and ETA: and without ETA it's
  // confused as the current time
  //	swprintf(sString, L"%ls %ls %d, %02d:%02d", pEtaString, pDayStrings,
  // uiDay, uiHour, uiMin); 	swprintf(sString, L"%ls %d, %02d:%02d", pDayStrings,
  // uiDay, uiHour, uiMin);
  swprintf(sString, Length, L"%ls %02d:%02d", pEtaString, uiHour, uiMin);
}

static int32_t GetGroundTravelTimeOfSoldier(const SOLDIERTYPE *const s) {
  int32_t iTravelTime = 0;
  ...
  // add travel time for any prior path segments (stored in the selected
  // character's mercpath, but waypoints aren't built)
  iTravelTime += GetPathTravelTimeDuringPlotting(GetSoldierMercPathPtr(s));

  return (iTravelTime);
}

static int16_t CalcLocationValueForChar(const SOLDIERTYPE *const s) {
  // don't reveal location of POWs!
  if (s->bAssignment == ASSIGNMENT_POW) return 0;

  return SECTOR(s->sSectorX, s->sSectorY) +
...
static BOOLEAN AnyMovableCharsInOrBetweenThisSector(int16_t sSectorX, int16_t sSectorY,
  ...
  CFOR_EACH_IN_TEAM(pSoldier, OUR_TEAM) {
    ...
    // don't count vehicles - in order for them to move, somebody must be in the
    // sector with them
    if (pSoldier->uiStatusFlags & SOLDIER_VEHICLE) {
      continue;
    }
    ...
    if ((pSoldier->sSectorX == sSectorX) && (pSoldier->sSectorY == sSectorY) &&
      ...
      // NOTE that we consider mercs between sectors, mercs < OKLIFE, and
      // sleeping mercs to be "movable". This lets CanCharacterMoveInStrategic()
      // itself report the appropriate error message when character is clicked
      return (TRUE);
    }
  }
...
static uint8_t PlayerMercsInHelicopterSector();
...
static BOOLEAN RequestGiveSkyriderNewDestination() {
  ...
  if (CanHelicopterFly()) {
    ...
    if (!gfSkyriderEmptyHelpGiven && (GetNumberOfPassengersInHelicopter() == 0) &&
      ...
      DoMapMessageBox(MSG_BOX_BASIC_STYLE, pSkyriderText[2], MAP_SCREEN, MSG_BOX_FLAG_OK,
                      MapScreenDefaultOkBoxCallback);
...
static void RandomAwakeSelectedMercConfirmsStrategicMove();
...
static void HandleNewDestConfirmation(int16_t sMapX, int16_t sMapY) {
  ...
  if (fPlotForHelicopter) {
    ...
    // NOTE: We don't this this for the helicopter any more, since it clashes
    // with Skyrider's own confirmation msg
...
static void RandomAwakeSelectedMercConfirmsStrategicMove() {
  ...
  if (ubNumMercs > 0) {
    ...
    // select that merc so that when he speaks we're showing his portrait and
    // not someone else
...
static void HandleMilitiaRedistributionClick() {
  ...
  // if on the surface
  if (iCurrentMapSectorZ == 0) {
    int8_t const bTownId = GetTownIdForSector(SECTOR(sSelMapX, sSelMapY));
    ...
    if ((bTownId != BLANK_SECTOR) && !fTownStillHidden) {
      ...
      int8_t const sam_id = GetSAMIdFromSector(sSelMapX, sSelMapY, 0);
      if (sam_id != -1 && fSamSiteFound[sam_id]) {  // Cannot move militia around sam sites.
...
static BOOLEAN CanMoveBullseyeAndClickedOnIt(int16_t sMapX, int16_t sMapY) {
  ...
  if (fShowAircraftFlag && bSelectedDestChar == -1 && !fPlotForHelicopter) {
...
static void BullsEyeOrChopperSelectionPopupCallback(MessageBoxReturnValue const ubExitValue) {
  ...
  if (ubExitValue == MSG_BOX_RETURN_YES) {
    // chose chopper
    // have to set a flag 'cause first call to
    // RequestGiveSkyriderNewDestination() triggers another msg box & won't work
...
static void WakeUpAnySleepingSelectedMercsOnFootOrDriving() {
  CFOR_EACH_SELECTED_IN_CHAR_LIST(i) {
    ...
    if (s.bAssignment >= ON_DUTY &&
        (s.bAssignment != VEHICLE || !SoldierMustDriveVehicle(s, false)))
      continue;
...
static void HandlePostAutoresolveMessages() {
  // KM: Autoresolve sets up this global sector value whenever the enemy gains
  // control of a sector.  As soon as we leave autoresolve and enter mapscreen,
  // then this gets called and handles ownership change for the sector. It also
  // brings up a dialog stating to the player what happened, however, the
  // internals of those functions breaks autoresolve and the game crashes after
  // autoresolve is finished.  The value doesn't need to be saved.
  //
  // An additional case is when creatures kill all opposition in the sector. For
  // each surviving monster, civilians are "virtually" murdered and loyalty hits
  // will be processed.
  if (gsCiviliansEatenByMonsters >= 1) {
    AdjustLoyaltyForCivsEatenByMonsters((uint8_t)SECTORX(gsEnemyGainedControlOfSectorID),
                                        (uint8_t)SECTORY(gsEnemyGainedControlOfSectorID),
    ...
    SetThisSectorAsEnemyControlled(SECTORX(gsEnemyGainedControlOfSectorID),
                                   SECTORY(gsEnemyGainedControlOfSectorID), 0);
...
wchar_t const *GetMapscreenMercAssignmentString(SOLDIERTYPE const &s) {
  return s.bAssignment == VEHICLE ? pShortVehicleStrings[GetVehicle(s.iVehicleId).ubVehicleType]
                                  : pAssignmentStrings[s.bAssignment];
}

void GetMapscreenMercLocationString(SOLDIERTYPE const &s, wchar_t *const buf, size_t const n) {
  if (s.bAssignment == IN_TRANSIT) {  // Show blank
    wcsncpy(buf, L"--", n);
  } else if (s.bAssignment == ASSIGNMENT_POW) {  // POW - location unknown
    wcsncpy(buf, pPOWStrings[1], n);
  } else {  // Put parentheses around it when he's between sectors
    swprintf(buf, n, s.fBetweenSectors ? L"(%ls%ls%ls)" : L"%ls%ls%ls", pMapVertIndex[s.sSectorY],
             pMapHortIndex[s.sSectorX], pMapDepthIndex[s.bSectorZ]);
  }
}
...
void GetMapscreenMercDestinationString(SOLDIERTYPE const &s, wchar_t *const buf, size_t n) {
   ...
   * matter) */
  if (s.bAssignment != ASSIGNMENT_DEAD && s.bAssignment != ASSIGNMENT_POW && s.bLife != 0) {
    int32_t x;
    int32_t y;
    if (s.bAssignment == IN_TRANSIT) {  // Show the sector he'll be arriving in
      x = SECTORX(g_merc_arrive_sector);
      y = SECTORY(g_merc_arrive_sector);
    } else if (GetLengthOfMercPath(&s) > 0) {  // He's going somewhere
      int16_t const sector = GetLastSectorIdInCharactersPath(&s);
...
void GetMapscreenMercDepartureString(SOLDIERTYPE const &s, wchar_t *const buf, size_t const n,
                                     uint8_t *const text_colour) {
  if ((s.ubWhatKindOfMercAmI != MERC_TYPE__AIM_MERC && s.ubProfile != SLAY) || s.bLife == 0) {
    wcsncpy(buf, gpStrategicString[STR_PB_NOTAPPLICABLE_ABBREVIATION], n);
    return;
  }

  int32_t mins_remaining = s.iEndofContractTime - GetWorldTotalMin();
  if (s.bAssignment == IN_TRANSIT) {
    int32_t const contract_length = s.iTotalContractLength * NUM_MIN_IN_DAY;
    if (mins_remaining > contract_length) {
      mins_remaining = contract_length;
    }
  }

  if (mins_remaining >= MAP_TIME_UNDER_THIS_DISPLAY_AS_HOURS) {  // 3 or more days remain
    int32_t const days_remaining = mins_remaining / (24 * 60);
  ...
  // less than 3 days
  int32_t const hours_remaining = mins_remaining > 5 ? (mins_remaining + 59) / 60 : 0;
  ...
  if (text_colour) {
    *text_colour =
        mins_remaining <= MINS_TO_FLASH_CONTRACT_TIME && fFlashContractFlag ? FONT_WHITE : FONT_RED;
  }
}

static void InitPreviousPaths() { g_prev_path = NULL; }
...
static bool RestorePath(PathSt *&path, uint8_t const group_id) {
  if (g_prev_path) {
  ...
  } else  // character(s) plotting
  {
    CFOR_EACH_SELECTED_IN_CHAR_LIST(c) {
      ...
      if (pSoldier->uiStatusFlags & SOLDIER_VEHICLE) {
        ...
        // invalid pSoldier - that guy can't possibly be moving, he's on a
        // non-vehicle assignment!
        Assert(0);
        continue;
      }
...
static void ClearPreviousPaths() { g_prev_path = ClearStrategicPathList(g_prev_path, 0); }
...
static void SelectAllCharactersInSquad(int8_t bSquadNumber) {
  ...
  for (bCounter = 0; bCounter < MAX_CHARACTER_COUNT; bCounter++) {
    ...
    // if this guy is on that squad or in a vehicle which is assigned to that
    // squad NOTE: There's no way to select everyone aboard Skyrider with this
    // function...
    if (pSoldier->bAssignment == bSquadNumber ||
        IsSoldierInThisVehicleSquad(pSoldier, bSquadNumber)) {
      if (fFirstOne) {
        // make the first guy in the list who is in this squad the selected info
        // char
...
BOOLEAN CanDrawSectorCursor() {
  return
      ...
      !fShowTownInfo && ghTownMineBox == NO_POPUP_BOX && !fShowUpdateBox &&
      GetNumberOfMercsInUpdateList() == 0 && sSelectedMilitiaTown == 0 && !gfMilitiaPopupCreated &&
      !gfStartedFromMapScreen && !fShowMapScreenMovementList && ghMoveBox == NO_POPUP_BOX &&
      !fMapInventoryItem;
}
...
static void RestoreMapSectorCursor(int16_t sMapX, int16_t sMapY) {
  ...
  /*
          if(fZoomFlag)
                  RestoreExternBackgroundRect( ((int16_t)( sScreenX - MAP_GRID_X
     )), ((int16_t)( sScreenY - MAP_GRID_Y )), DMAP_GRID_ZOOM_X,
     DMAP_GRID_ZOOM_Y); else
  */
...
static void RequestContractMenu() {
  ...
  // in case we have multiple guys selected, turn off everyone but the guy we're
  // negotiating with
...
static void MapscreenMarkButtonsDirty() {
  ...
  if (!fShowMapInventoryPool) {
    ...
    if (fShowAttributeMenu) {
      // don't redraw the town button, it would wipe out a chunk of the
      // attribute menu
