...
static uint16_t gusMapPathingData[256];
...
static int16_t const diStratDelta[] = {
    -MAP_WIDTH,     // N
    1 - MAP_WIDTH,  // NE
    1,              // E
    1 + MAP_WIDTH,  // SE
...
int32_t FindStratPath(int16_t const sStart, int16_t const sDestination, GROUP const &g,
                      BOOLEAN const fTacticalTraversal) {
  ...
  // ******** Fudge by Bret (for now), curAPcost is never initialized in this
  // function, but should be! so this is just to keep things happy!
  ...
  const GROUP *const heli_group =
      iHelicopterVehicleId != -1 ? GetGroup(GetHelicopter().ubMovementGroup) : 0;
  ...
  do {
    ...
    for (iCnt = 0; iCnt < 8; iCnt += 2) {
      ...
      if (gfPlotToAvoidPlayerInfuencedSectors && newLoc != sDestination) {
        sSectorX = (int16_t)(newLoc % MAP_WORLD_X);
        sSectorY = (int16_t)(newLoc / MAP_WORLD_X);
        ...
        if (!OkayForEnemyToMoveThroughSector((uint8_t)SECTOR(sSectorX, sSectorY))) {
          continue;
        }
      }

      // are we plotting path or checking for existance of one?
      nextCost = GetSectorMvtTimeForGroup(SECTOR(curLoc % MAP_WORLD_X, curLoc / MAP_WORLD_X),
                                          iCnt / 2, &g);
      if (nextCost == TRAVERSE_TIME_IMPOSSIBLE) continue;

      if (&g == heli_group) {
        // is a heli, its pathing is determined not by time (it's always the
        // same) but by total cost Skyrider will avoid uncontrolled airspace as
        // much as possible...
      ...
      // if we're building this path due to a tactical traversal exit, we have
      // to force the path to the next sector be in the same direction as the
      // traversal, even if it's not the shortest route, otherwise pathing can
      // crash!  This can happen in places where the long way around to next
      // sector is actually shorter: e.g. D5 to D6.  ARM
      if (fTacticalTraversal) {
        // if it's the first sector only (no cost yet)
        if (curCost == 0 && (newLoc == sDestination)) {
          if (GetTraversability((int16_t)(SECTOR(curLoc % 18, curLoc / 18)),
                                (int16_t)(SECTOR(newLoc % 18, newLoc / 18))) != GROUNDBARRIER) {
            nextCost = 0;
          }
        }
      } else {
  ...
  if (pathFound) {
    ...
    // return path length : serves as a "successful" flag and a path length
    // counter
    return (iCnt);
  }
...
PathSt *BuildAStrategicPath(int16_t const start_sector, int16_t const end_sector, GROUP const &g,
                            BOOLEAN const fTacticalTraversal) {
  if (end_sector < MAP_WORLD_X - 1) return NULL;

  const int32_t path_len = FindStratPath(start_sector, end_sector, g, fTacticalTraversal);
  ...
  int32_t cur_sector = start_sector;
  int32_t delta = 0;
  PathSt *path = head;
...
PathSt *AppendStrategicPath(PathSt *pNewSection, PathSt *pHeadOfPathList) {
  // will append a new section onto the end of the head of list, then return the
...
PathSt *ClearStrategicPathList(PathSt *const pHeadOfPath, const int16_t sMvtGroup) {
...
PathSt *ClearStrategicPathListAfterThisSector(PathSt *pHeadOfPath, int16_t sX, int16_t sY,
  ...
  // we want to KEEP the target sector, not delete it, so advance to the next
  // sector
  pNode = pNode->pNext;
...
int16_t GetLastSectorIdInCharactersPath(const SOLDIERTYPE *pCharacter) {
  // will return the last sector of the current path, or the current sector if
  // there's no path
  int16_t sLastSector = (pCharacter->sSectorX) + (pCharacter->sSectorY) * (MAP_WORLD_X);
  PathSt *pNode = NULL;
...
void RebuildWayPointsForGroupPath(PathSt *const pHeadOfPath, GROUP &g) {
  ...
  // KRIS!  Added this because it was possible to plot a new course to the same
  // destination, and the
  //       group would add new arrival events without removing the existing
  //       one(s).
  ...
  if (g.fPlayer) {
    ...
    // allows assignments to flash right away if their subject moves
    // away/returns (robot/vehicle being repaired), or
    // patient/doctor/student/trainer being automatically put on a squad via the
    // movement menu.
  ...
  if ((pNode == NULL) || (pNode->pNext == NULL)) {
    ...
    // NOTE: AI groups never reverse direction between sectors, Kris cheats &
    // teleports them back to their current sector!
  ...
  if (g.fBetweenSectors) {
    // figure out which direction we're already going in  (Otherwise iOldDelta
    // starts at 0)
    iOldDelta = CALCULATE_STRATEGIC_INDEX(g.ubNextX, g.ubNextY) -
                CALCULATE_STRATEGIC_INDEX(g.ubSectorX, g.ubSectorY);
  }

  // build a brand new list of waypoints, one for initial direction, and another
  // for every "direction change" thereafter
  while (pNode->pNext) {
    iDelta = pNode->pNext->uiSectorId - pNode->uiSectorId;
    Assert(iDelta != 0);  // same sector should never repeat in the path list

    // Waypoints are only added at "pivot points" - whenever there is a change
    // in orthogonal direction. If we're NOT currently between sectors,
    // iOldDelta will start off at 0, which means that the first node can't be
    // added as a waypoint.  This is what we want - for stationary mercs, the
    // first node in a path is the CURRENT sector.
  ...
  // there must have been at least one next node, or we would have bailed out on
  // "no path" earlier
  Assert(!fFirstNode);
  ...
  // at this point, the final sector in the path must be identical to this
  // group's last waypoint
  wp = GetFinalWaypoint(&g);
  AssertMsg(wp, "Path exists, but no waypoints were added!  AM-0");
  AssertMsg(pNode->uiSectorId == (uint32_t)CALCULATE_STRATEGIC_INDEX(wp->x, wp->y),
            "Last waypoint differs from final path sector!  AM-0");

  // see if we've already reached the first sector in the path (we never
  // actually left the sector and reversed back to it)
...
BOOLEAN MoveGroupFromSectorToSector(GROUP &g, int16_t const sStartX, int16_t const sStartY,
                                    int16_t const sDestX, int16_t const sDestY) {
  PathSt *pNode = BuildAStrategicPath((int16_t)CALCULATE_STRATEGIC_INDEX(sStartX, sStartY),
                                      (int16_t)CALCULATE_STRATEGIC_INDEX(sDestX, sDestY), g, FALSE);
...
static BOOLEAN MoveGroupFromSectorToSectorButAvoidLastSector(GROUP &g, int16_t const sStartX,
                                                             int16_t const sStartY,
                                                             int16_t const sDestX,
                                                             int16_t const sDestY) {
  PathSt *pNode = BuildAStrategicPath((int16_t)CALCULATE_STRATEGIC_INDEX(sStartX, sStartY),
                                      (int16_t)CALCULATE_STRATEGIC_INDEX(sDestX, sDestY), g, FALSE);
...
BOOLEAN MoveGroupFromSectorToSectorButAvoidPlayerInfluencedSectors(GROUP &g, int16_t const sStartX,
                                                                   int16_t const sStartY,
                                                                   int16_t const sDestX,
                                                                   int16_t const sDestY) {
...
MoveGroupFromSectorToSectorButAvoidPlayerInfluencedSectorsAndStopOneSectorBeforeEnd(
    GROUP &g, int16_t const sStartX, int16_t const sStartY, int16_t const sDestX,
    int16_t const sDestY) {
  // init sectors with soldiers in them
  InitSectorsWithSoldiersList();
...
// clears the contents of the soldier's mercpPath, as well as his vehicle path
// if he is a / or is in a vehicle
static void ClearPathForSoldier(SOLDIERTYPE *pSoldier) {
...
static void AddSectorToFrontOfMercPath(PathSt **ppMercPath, uint8_t ubSectorX, uint8_t ubSectorY);
