...
// IMPORTANT:
// This function adds a NEWLY allocated schedule to the list.  The pointer
// passed is totally separate.  So make sure that you delete the pointer if you
// don't need it anymore.  The editor uses a single static node to copy data
// from, hence this method.
...
// cleans out the schedule list without touching events, for saving & loading
// games
...
void DeleteSchedule(uint8_t ubScheduleID) {
  ...
  if (!gpScheduleList) {
    // ScreenMsg( 0, MSG_BETAVERSION, L"Attempting to delete schedule that
    // doesn't exist -- KM : 2" );
    return;
  }
...
static void PrepareScheduleForAutoProcessing(SCHEDULENODE *pSchedule, uint32_t uiStartTime,
                                             uint32_t uiEndTime);
...
void ProcessTacticalSchedule(uint8_t ubScheduleID) {
  ...
  if (fAutoProcess) {
    ...
    // Grab the last time the eventlist was queued.  This will tell us how much
    // time has passed since that moment, and how long we need to auto process
    // this schedule.
...
// all of the schedule IDs from scratch and adjusts the effected structures
// accordingly.
void OptimizeSchedules() {
  ...
  while (pSchedule) {
    ...
    if (ubOldScheduleID != gubScheduleID) {  // The schedule ID has changed, so
                                             // change all links accordingly.
      ...
      CFOR_EACH_SOLDIERINITNODE(pNode) {
        if (pNode->pDetailedPlacement &&
          ...
          // Temporarily add 100 to the ID number to ensure that it doesn't get
          // used again later. We will remove it immediately after this loop is
          // complete.
...
void PrepareSchedulesForEditorEntry() {
  ...
  // Delete all schedule events.  The editor will automatically warp all
  // civilians to their starting locations.
  DeleteAllStrategicEventsOfType(EVENT_PROCESS_TACTICAL_SCHEDULE);
...
// Called when leaving the editor to enter the game.  This posts all of the
// events that apply.
...
void LoadSchedules(HWFILE const f) {
  ...
  for (uint8_t n = n_schedules; n != 0; --n) {
    uint8_t data[36];
...
void ReverseSchedules() {
  ...
  // First, remove any gaps which would mess up the reverse ID assignment by
  // optimizing the schedules.
  OptimizeSchedules();
  pReverseHead = NULL;
  while (gpScheduleList) {
    // reverse the ID
...
void SaveSchedules(HWFILE const f) {
  ...
  uint8_t n_to_save = std::min(n_schedules, 32);
  FileWrite(f, &n_to_save, sizeof(uint8_t));
...
BOOLEAN BumpAnyExistingMerc(int16_t sGridNo) {
  ...
  const int16_t sNewGridNo =
      FindGridNoFromSweetSpotWithStructDataFromSoldier(pSoldier, STANDING, 5, 1, pSoldier);
  // const int16_t sNewGridNo =
  // FindGridNoFromSweetSpotExcludingSweetSpot(pSoldier, sGridNo, 10);
...
static void PerformActionOnDoorAdjacentToGridNo(uint8_t ubScheduleAction, uint16_t usGridNo);

static void AutoProcessSchedule(SCHEDULENODE *pSchedule, int32_t index) {
  int8_t bDirection;

  if (gTacticalStatus.uiFlags & LOADING_SAVED_GAME) {
    // CJC, November 28th:  when reloading a saved game we want events posted
    // but no events autoprocessed since that could change civilian positions.
    // So rather than doing a bunch of checks outside of this function, I
    // thought it easier to screen them out here.
    return;
  }
  ...
  if (GameState::getInstance()->isEditorMode()) {
    if (pSoldier->ubProfile != NO_PROFILE) {
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("Autoprocessing schedule action %ls for %ls (%d) at time "
                      "%02ld:%02ld (set for %02d:%02d), data1 = %d",
                      gszScheduleActions[pSchedule->ubAction[index]], pSoldier->name,
                      pSoldier->ubID, GetWorldHour(), guiMin, pSchedule->usTime[index] / 60,
                      pSchedule->usTime[index] % 60, pSchedule->usData1[index]));
    } else {
      DebugMsg(TOPIC_JA2, DBG_LEVEL_3,
               String("Autoprocessing schedule action %ls for civ (%d) at time "
                      "%02ld:%02ld (set for %02d:%02d), data1 = %d",
                      gszScheduleActions[pSchedule->ubAction[index]], pSoldier->ubID,
...
static int8_t GetEmptyScheduleEntry(SCHEDULENODE *pSchedule);
static BOOLEAN ScheduleHasMorningNonSleepEntries(SCHEDULENODE *pSchedule);
static void SecureSleepSpot(SOLDIERTYPE *pSoldier, uint16_t usSleepSpot);
...
static void PostSchedule(SOLDIERTYPE *pSoldier) {
  ...
  // if this schedule doesn't have a time associated with it, then generate a
  // time, but only if it is a sleep schedule.
  for (i = 0; i < MAX_SCHEDULE_ACTIONS; i++) {
      ...
      if (pSchedule->usTime[i] == 0xffff) {
        ...
        if (ScheduleHasMorningNonSleepEntries(pSchedule)) {
          ...
          if (bEmpty != -1) {
            ...
            // NB the wakeup call must be ordered first! so we have to create
            // the wake action and then swap the two.
            ...
            // no morning entries but no space for a wakeup either, will sleep
            // till next non-sleep event
          }
        }
        break;  // The break is here because nobody should have more than one
                // sleep schedule with no time specified.
      }
    }
  }
  ...
  /*
  //First thing we need is to get the time that the map was last loaded.  If
  more than 24 hours,
  //then process only 24 hours.  If less, then process all the schedules that
  would have happened within
  //that period of time.
  uiEndTime = GetWorldTotalMin();
  ...
  */
  ...
  // Need a way to determine if the player has actually modified doors since
  // this civilian was last loaded
...
static void PrepareScheduleForAutoProcessing(SCHEDULENODE *pSchedule, uint32_t uiStartTime,
  ...
  if (uiStartTime > uiEndTime) {  // The start time is later in the day than the
                                  // end time, which means we'll be wrapping
    ...
    for (i = 0; i < MAX_SCHEDULE_ACTIONS; i++) {
      ...
      if (pSchedule->usTime[i] <= uiEndTime) {
        ...
        // CJC: Note that end time is always passed in here as the current time
        // so GetWorldDayInMinutes will be for the correct day
...
// Leave at night, come back in the morning.  The time variances are a couple
// hours, so the town doesn't turn into a ghost town in 5 minutes.
static void PostDefaultSchedule(SOLDIERTYPE *pSoldier) {
  ...
  // Create a new node at the head of the list.  The head will become the new
  // schedule we are about to add.
  curr = gpScheduleList;
...
// Assumes that a schedule has just been processed.  This takes the current
// time, and compares it to the schedule, and looks for the next schedule action
// that would get processed and posts it.
...
BOOLEAN ExtractScheduleDoorLockAndUnlockInfo(SOLDIERTYPE *pSoldier, uint32_t *puiOpeningTime,
  ...
  *puiClosingTime = 0;
  ...
  pSchedule = GetSchedule(pSoldier->ubScheduleID);
  if (!pSchedule) {
    // If person had default schedule then would have been assigned and this
    // would have succeeded. Hence this is an error.
    return (FALSE);
  }
...
static uint16_t FindSleepSpot(SCHEDULENODE *pSchedule) {
  int8_t bLoop;
...
static void ReplaceSleepSpot(SCHEDULENODE *pSchedule, uint16_t usNewSpot) {
  int8_t bLoop;
...
static void SecureSleepSpot(SOLDIERTYPE *const pSoldier, uint16_t const usSleepSpot) {
  // start after this soldier's ID so we don't duplicate work done in previous
  // passes
  for (uint32_t i = pSoldier->ubID + 1; i <= gTacticalStatus.Team[CIV_TEAM].bLastID; ++i) {
    ...
    uint16_t const usSleepSpot2 = FindSleepSpot(pSchedule);
    if (usSleepSpot2 != usSleepSpot) continue;

    // conflict!
    // uint8_t ubDirection;
    // const uint16_t usNewSleepSpot = FindGridNoFromSweetSpotWithStructData(&s2,
    // s2.usAnimState, usSleepSpot2, 3, &ubDirection, FALSE);
    uint16_t const usNewSleepSpot = FindGridNoFromSweetSpotExcludingSweetSpot(&s2, usSleepSpot2, 3);
