...
// remembers which player group is the Continue/Stop prompt about?  No need to
// save as long as you can't save while prompt ON
static GROUP *gpGroupPrompting = NULL;

static uint32_t uniqueIDMask[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
static uint8_t AddGroupToList(GROUP *pGroup);

// Player grouping functions
//.........................
// Creates a new player group, returning the unique ID of that group.  This is
// the first step before adding waypoints and members to the player group.
GROUP *CreateNewPlayerGroupDepartingFromSector(uint8_t const ubSectorX, uint8_t const ubSectorY) {
  ...
  pNew->ubOriginalSector = (uint8_t)SECTOR(ubSectorX, ubSectorY);
  pNew->fPlayer = TRUE;
...
GROUP *CreateNewVehicleGroupDepartingFromSector(uint8_t const ubSectorX, uint8_t const ubSectorY) {
  AssertMsg(ubSectorX >= 1 && ubSectorX <= 16,
...
void AddPlayerToGroup(GROUP &g, SOLDIERTYPE &s) {
  ...
  if (!i) {
    ...
    for (; i->next; i = i->next) {
      AssertMsg(i->pSoldier->ubProfile != s.ubProfile,
                String("Attempting to add an already existing merc to group "
                       "(ubProfile=%d).",
                       s.ubProfile));
    }
    i->next = p;
...
void RemovePlayerFromGroup(SOLDIERTYPE &s) {
  ...
  //     Because the release build has no assertions, it was still possible for
  //     the group to be null, causing a crash.  Instead of crashing, it'll
  //     simply return false.
  if (!pGroup) return;
  // end

  AssertMsg(pGroup, String("Attempting to RemovePlayerFromGroup( %d, %d ) from "
                           "non-existant group",
                           s.ubGroupID, s.ubProfile));

  RemovePlayerFromPGroup(*pGroup, s);
}

static void SetLocationOfAllPlayerSoldiersInGroup(GROUP const &, int16_t x, int16_t y, int8_t z);
...
BOOLEAN GroupReversingDirectionsBetweenSectors(GROUP *pGroup, uint8_t ubSectorX, uint8_t ubSectorY,
  ...
  // if we're not between sectors, or we are but we're continuing in the same
  // direction as before
  ...
  // The new direction is reversed, so we have to go back to the sector we just
  // left.
  ...
  if (pGroup->fPlayer) {
    // ARM: because we've changed the group's ubSectoryX and ubSectorY, we must
    // now also go and change the sSectorX and sSectorY of all the soldiers in
    // this group so that they stay in synch.  Otherwise pathing and movement
    // problems will result since the group is in one place while the merc is in
    // another...
    SetLocationOfAllPlayerSoldiersInGroup(*pGroup, pGroup->ubSectorX, pGroup->ubSectorY, 0);
  }

  // IMPORTANT: The traverse time doesn't change just because we reverse
  // directions!  It takes the same time no matter which direction you're going
  // in!  This becomes critical in case the player reverse directions again
  // before moving!

  // The time it takes to arrive there will be exactly the amount of time we
  ...
  if (pGroup->uiArrivalTime > GetWorldTotalMin()) {
    ...
    // IMPORTANT: this can't be called during RebuildWayPointsForGroupPath(),
    // since it will clear the mercpath prematurely by assuming the mercs are
    // now at their final destination when only the first waypoint is in
    // place!!! To handle this situation, RebuildWayPointsForGroupPath() will
    // issue it's own call after it's ready for it.
...
BOOLEAN GroupBetweenSectorsAndSectorXYIsInDifferentDirection(GROUP *pGroup, uint8_t ubSectorX,
  ...
  // Determine the direction the group would need to travel in to reach the
  // given sector
  ...
  // Compare the dx/dy's.  If they're exactly the same, group is travelling in
  // the same direction as before, so we're not changing directions. Note that
  // 90-degree orthogonal changes are considered changing direction, as well as
  // the full 180-degree reversal. That's because the party must return to the
  // previous sector in each of those cases, too.
  if (currDX == newDX && currDY == newDY) return (FALSE);

  // yes, we're between sectors, and we'd be changing direction to go to the
  // given sector
  return (TRUE);
}
...
BOOLEAN AddWaypointToPGroup(GROUP *const g, uint8_t const x,
                            uint8_t const y)  // Same, but overloaded
{
  AssertMsg(1 <= x && x <= 16,
            String("AddWaypointToPGroup with out of range sectorX value of %d", x));
  AssertMsg(1 <= y && y <= 16,
            String("AddWaypointToPGroup with out of range sectorY value of %d", y));
  ...
  uint8_t n_aligned_axes = 0;
  bool reversing_direction = FALSE;
  WAYPOINT *wp = g->pWaypoints;
  if (!wp) {
    if (GroupReversingDirectionsBetweenSectors(g, x, y, TRUE)) {
      if (g->fPlayer) { /* Because we reversed, we must add the new current
                         * sector back at the head of everyone's mercpath */
      ...
      // ARM:  Kris - new rulez.  Must still fall through and add a waypoint
      // anyway!!!
  ...
  if (!reversing_direction) {
    if (n_aligned_axes == 0) {
      AssertMsg(FALSE,
                String("Invalid DIAGONAL waypoint being added for groupID %d. AM-0", g->ubGroupID));
      return FALSE;
    }
  ...
  if (wp) {  // Add the waypoint to the end of the list
    wp->next = new_wp;
  } else {  // We are adding the first waypoint.
    g->pWaypoints = new_wp;
    ...
    if (!reversing_direction) { /* We need to calculate the next sector the
                                 * group is moving to and post an
                                 * event for it. */
      InitiateGroupMovementToNextSector(g);
    }
  }

  if (g->fPlayer) {  // Also, nuke any previous "tactical traversal" information.
    CFOR_EACH_PLAYER_IN_GROUP(curr, g) { curr->pSoldier->ubStrategicInsertionCode = 0; }
  }
...
BOOLEAN AddWaypointStrategicIDToPGroup(GROUP *pGroup, uint32_t uiSectorID) {
  uint8_t ubSectorX, ubSectorY;
  ubSectorX = (uint8_t)GET_X_FROM_STRATEGIC_INDEX(uiSectorID);
  ubSectorY = (uint8_t)GET_Y_FROM_STRATEGIC_INDEX(uiSectorID);
  return AddWaypointToPGroup(pGroup, ubSectorX, ubSectorY);
}
...
GROUP *CreateNewEnemyGroupDepartingFromSector(uint32_t uiSector, uint8_t ubNumAdmins,
  ...
  pNew->pWaypoints = NULL;
  pNew->ubSectorX = (uint8_t)SECTORX(uiSector);
  pNew->ubSectorY = (uint8_t)SECTORY(uiSector);
...
static uint8_t AddGroupToList(GROUP *const g) {
  // First, find a unique ID
  for (uint8_t id = 0; ++id;) {
    const uint32_t index = id / 32;
...
static void RemoveGroupFromList(GROUP *const g) {
  for (GROUP **i = &gpGroupList; *i != NULL; i = &(*i)->next) {
    ...
    *i = g->next;
    ...
    const uint32_t index = g->ubGroupID / 32;
    const uint32_t bit = g->ubGroupID % 32;
    const uint32_t mask = 1 << bit;
...
class CharacterDialogueEventBeginPrebattleInterface : public CharacterDialogueEvent {
 public:
  CharacterDialogueEventBeginPrebattleInterface(SOLDIERTYPE &soldier,
                                                GROUP *const initiating_battle_group)
      : CharacterDialogueEvent(soldier), initiating_battle_group_(initiating_battle_group) {}
...
// If this is called, we are setting the game up to bring up the prebattle
// interface.  Before doing so, one of the involved mercs will pipe up.  When he
// is finished, we automatically go into the mapscreen, regardless of the mode we
// are in.
static void PrepareForPreBattleInterface(GROUP *pPlayerDialogGroup, GROUP *pInitiatingBattleGroup) {
  ...
  if (fDisableMapInterfaceDueToBattle) {
    AssertMsg(0,
              "fDisableMapInterfaceDueToBattle is set before attempting to "
              "bring up PBI.  Please send PRIOR save if possible and "
              "details on anything that just happened before this battle.");
    return;
  }
  ...
  AssertMsg(pPlayerDialogGroup->pPlayerList,
            String("Player group %d doesn't have *any* players in it!  "
                   "(Finding dialog group)",
...
static void HandleOtherGroupsArrivingSimultaneously(uint8_t x, uint8_t y, uint8_t z);
static void NotifyPlayerOfBloodcatBattle(uint8_t ubSectorX, uint8_t ubSectorY);
...
static BOOLEAN CheckConditionsForBattle(GROUP *pGroup) {
  ...
  if (gfWorldLoaded) {  // look for people arriving in the currently loaded
                        // sector.  This handles reinforcements.
    const GROUP *const curr = FindPlayerMovementGroupInSector(gWorldSectorX, gWorldSectorY);
    if (!gbWorldSectorZ &&
        PlayerMercsInSector((uint8_t)gWorldSectorX, (uint8_t)gWorldSectorY,
                            gbWorldSectorZ) &&
      ...
      if (!g.fBetweenSectors) {
        if (g.ubSectorX == pGroup->ubSectorX && g.ubSectorY == pGroup->ubSectorY && !g.ubSectorZ) {
          if (!GroupHasInTransitDeadOrPOWMercs(g) &&
            ...
            // Now, a player group is in this sector.  Determine if the group
            // contains any mercs that can fight. Vehicles, EPCs and the robot
            // doesn't count.  Mercs below OKLIFE do.
  ...
  if (pGroup->fPlayer) {
    ...
    if (pGroup->uiFlags & GROUPFLAG_HIGH_POTENTIAL_FOR_AMBUSH &&
        fBattlePending) {  // This group has just arrived in a new sector from an
                           // adjacent sector that he retreated from
      // If this battle is an encounter type battle, then there is a 90% chance
      // that the battle will become an ambush scenario.
      gfHighPotentialForAmbush = TRUE;
    }

    // If there are bloodcats in this sector, then it internally checks and
    // handles it
  ...
  if (!fAliveMerc && !fMilitiaPresent) {  // empty vehicle, everyone dead, don't
                                          // care.  Enemies don't care.
    return FALSE;
  }

  if (fBattlePending) {  // A battle is pending, but the player's could be all
                         // unconcious or dead.
    // Go through every group until we find at least one concious merc.  The
    // looping will determine if there are any live mercs and/or concious ones.
    // If there are no concious mercs, but alive ones, then we will go straight
    // to autoresolve, where the enemy will likely annihilate them or capture
    // them. If there are no alive mercs, then there is nothing anybody can do.
    // The enemy will completely ignore this, and continue on.

    if (gubNumGroupsArrivedSimultaneously) {  // Because this is a battle case,
                                              // clear all the group flags
      FOR_EACH_GROUP(curr) {
    ...
    if (!fCombatAbleMerc) {  // Prepare for instant autoresolve.
      ...
      if (fMilitiaPresent) {
        ...
        GetSectorIDString(pGroup->ubSectorX, pGroup->ubSectorY, pGroup->ubSectorZ, pSectorStr,
                          lengthof(pSectorStr), TRUE);
        swprintf(str, lengthof(str), gpStrategicString[STR_DIALOG_ENEMIES_ATTACK_UNCONCIOUSMERCS],
                 pSectorStr);
...
// This will get called after a battle is auto-resolved or automatically after
// arriving at the next sector during a move and the area is clear.
void CalculateNextMoveIntention(GROUP *pGroup) {
  ...
  // Should be surely an enemy group that has just made a new decision to go
  // elsewhere!
  ...
  if (pGroup->ubSectorX == wp->x &&
      pGroup->ubSectorY == wp->y) {  // We have reached the next waypoint, so now
                                     // determine what the next waypoint is.
    switch (pGroup->ubMoveType) {
      ...
      case CIRCULAR:
        wp = wp->next;
        if (!wp) {  // reached the end of the patrol route.  Set to the first
                    // waypoint in list, indefinately.
          // NOTE:  If the last waypoint isn't exclusively aligned to the x or y
          // axis of the first 			 waypoint, there will be an assertion failure inside
          // the waypoint movement code.
...
static void AwardExperienceForTravelling(GROUP &g) {
  uint32_t const traverse_time = g.uiTraverseTime;
  CFOR_EACH_PLAYER_IN_GROUP(i, &g) {
    ...
    if (s.bLifeMax < 100) {
      ...
      uint32_t const points = traverse_time / (450 / (100 - s.bLifeMax));
      if (points > 0) StatChange(s, HEALTHAMT, (uint8_t)points, FROM_SUCCESS);
    }

    if (s.bStrength < 100) {
      uint32_t const carried_percent = CalculateCarriedWeight(&s);
      if (carried_percent > 50) {
        uint32_t const points = traverse_time / (450 / (100 - s.bStrength));
...
static void AddCorpsesToBloodcatLair(int16_t sSectorX, int16_t sSectorY) {
...
static void ReportVehicleOutOfGas(VEHICLETYPE const &, uint8_t x, uint8_t y);
static void SpendVehicleFuel(SOLDIERTYPE &, int16_t fuel_spent);
static int16_t VehicleFuelRemaining(SOLDIERTYPE const &);
...
void GroupArrivedAtSector(GROUP &g, BOOLEAN const check_for_battle, BOOLEAN const never_left) {
  ...
  if (g.fPlayer) {
    ...
    if (PLAYERGROUP const *curr = g.pPlayerList) {
      int8_t const assignment = curr->pSoldier->bAssignment;
      if (assignment < ON_DUTY) ResetDeadSquadMemberList(assignment);
    }

    if (g.fVehicle) {
      VEHICLETYPE const &v = GetVehicleFromMvtGroup(g);
      if (!IsHelicopter(v) && !g.pPlayerList) { /* Nobody here, better just get out now. With
                                                 * vehicles, arriving empty is probably ok, since
                                                 * passengers might have been killed but vehicle
                                                 * lived. */
        return;
      }
    } else {
      if (!g.pPlayerList) {  // Nobody here, better just get out now
  ...
  uint8_t const x = g.ubNextX;
  uint8_t const y = g.ubNextY;
  uint8_t const z = g.ubSectorZ;

  // Check for exception cases which
  bool const exception_queue =
      gTacticalStatus.bBoxingState != NOT_BOXING && !g.fPlayer && x == 5 && y == 4 && z == 0;
  ...
  if (exception_queue ||
      (check_for_battle && gTacticalStatus.fEnemyInSector &&
       FindPlayerMovementGroupInSector(gWorldSectorX, gWorldSectorY) &&
       (x != gWorldSectorX || y != gWorldSectorY || gbWorldSectorZ > 0)) ||
      AreInMeanwhile() ||
      ...
      (check_for_battle && HostileBloodcatsPresent())) { /* Queue battle! Delay arrival by a random
                                                          * value ranging from 3-5 minutes, so it
                                                          * doesn't get the player too suspicious
                                                          * after it happens to him a few times,
                                                          * which, by the way, is a rare occurrence.
                                                          */
    if (AreInMeanwhile()) { /* Tack on only 1 minute if we are in a meanwhile
                             * scene. This effectively
                             * prevents any battle from occurring while inside a
                             * meanwhile scene. */
      ++g.uiArrivalTime;
    } else {
    ...
    if (g.fPlayer && g.uiArrivalTime - ABOUT_TO_ARRIVE_DELAY > GetWorldTotalMin()) {
      AddStrategicEvent(EVENT_GROUP_ABOUT_TO_ARRIVE, g.uiArrivalTime - ABOUT_TO_ARRIVE_DELAY,
                        g.ubGroupID);
    }
    return;
  }
  ...
  if (g.fPlayer) {
    ...
    if (!g.fVehicle) {  // Gotta be walking to get tougher
  ...
  if (g.fPlayer) {
    // If this is the last sector along player group's movement path (no more
    // waypoints)
    if (GroupAtFinalDestination(&g)) {  // Clear their strategic movement (mercpaths and waypoints)
      ClearMercPathsAndWaypointsForAllInGroup(g);
    }
    ...
    if (z == 0) {
      ...
      switch (GetTownIdForSector(SECTOR(x, y))) {
        ...
        default:
          int8_t const sam_id = GetSAMIdFromSector(x, y, 0);
          if (sam_id != -1) SetSAMSiteAsFound(sam_id);
          break;
      }
    }

    uint8_t insertion_direction;
    uint8_t strategic_insertion_code;
    if (x < g.ubPrevX) {
    ...
    if (!g.fVehicle) {
      ...
      if (g.pPlayerList) {
        ...
        who = s.bAssignment >= ON_DUTY ? s.name :     // A loner
      ...
      if (!IsHelicopter(v)) {
        ...
        // If this sector is currently loaded, add vehicle to the tactical
        // engine
        ...
        CFOR_EACH_PLAYER_IN_GROUP(i, &g) {
          ...
          // If this sector is currently loaded, add passenger to the tactical
          // engine
          if (here) UpdateMercInSector(s, x, y, z);
        }
      } else {
        if (HandleHeliEnteringSector(v.sSectorX,
                                     v.sSectorY)) {  // Helicopter destroyed
    ...
    if (who) { /* Don't print any messages when arriving underground (there's no
                * delay involved) or if we never left (cancel) */
      if (GroupAtFinalDestination(&g) && z == 0 && !never_left) {
        ScreenMsg(FONT_MCOLOR_DKRED, MSG_INTERFACE, pMessageStrings[MSG_ARRIVE], who,
                  pMapVertIndex[y], pMapHortIndex[x]);
      }
    }
    ...
    if (!group_destroyed) {
      ...
      if (!g.fVehicle || !IsGroupTheHelicopterGroup(g)) {
        ...
        if (SECTOR(x, y) == SEC_I16 && !GetSectorFlagStatus(x, y, z, SF_ALREADY_VISITED)) {
  ...
  if (!group_destroyed) {
    ...
    if (check_for_battle && !CheckConditionsForBattle(&g) && !gfWaitingForInput) {
      ...
      g.uiFlags = 0;
      if (gubNumAwareBattles != 0) { /* When the AI is looking for the players, and a battle is
                                      * initiated, then decrement the value, otherwise the queen
                                      * will continue searching to infinity. */
        --gubNumAwareBattles;
      }
    }
  }
...
static void HandleNonCombatGroupArrival(GROUP &g, bool const main_group, bool const never_left) {
  if (StrategicAILookForAdjacentGroups(&g)) { /* The routine actually just deleted the enemy group
                                               * (player's don't get deleted), so we are done! */
    return;
  }

  if (g.fPlayer) {  // The group will always exist after the AI was processed.

    bool const is_heli_group = g.fVehicle && IsGroupTheHelicopterGroup(g);
    if (!is_heli_group) {  // Take control of sector
    ...
    if (GroupAtFinalDestination(&g)) {
      ...
      if (PlayerMercsInSector(sSelMapX, sSelMapY, iCurrentMapSectorZ) ==
          0) {  // Make this sector strategically selected
      ...
      if (!is_heli_group)  // Else Skyrider speaks for heli movement
      {
        ...
        // If traversing tactically, or we never left (just canceling), don't do
        // this
...
static void HandleOtherGroupsArrivingSimultaneously(uint8_t const x, uint8_t const y,
                                                    uint8_t const z) {
...
restart:
  for (STRATEGICEVENT *i = gpEventList; i && i->uiTimeStamp <= now; i = i->next) {
    ...
    GROUP &g = *GetGroup((uint8_t)i->uiParam);
...
static void PrepareGroupsForSimultaneousArrival() {
  ...
  uint32_t latest_arrival_time = 0;
  FOR_EACH_PLAYER_GROUP(i) {
    GROUP &g = *i;
  ...
  if (first_group.fPlayer &&
      first_group.uiArrivalTime - ABOUT_TO_ARRIVE_DELAY > GetWorldTotalMin()) {
    AddStrategicEvent(EVENT_GROUP_ABOUT_TO_ARRIVE,
                      first_group.uiArrivalTime - ABOUT_TO_ARRIVE_DELAY, first_group.ubGroupID);
  }
...
static BOOLEAN PossibleToCoordinateSimultaneousGroupArrivals(GROUP *const first_group) {
  ...
  wchar_t const *const pStr = n_nearby_groups == 1 ? gpStrategicString[STR_DETECTED_SINGULAR]
                                                   : gpStrategicString[STR_DETECTED_PLURAL];
  wchar_t const *const enemy_type = gubEnemyEncounterCode == ENTERING_BLOODCAT_LAIR_CODE
  ...
  wchar_t str[255];
  size_t const n = swprintf(str, lengthof(str), pStr, enemy_type, 'A' + first_group->ubSectorY - 1,
                            first_group->ubSectorX);
...
static void DelayEnemyGroupsIfPathsCross(GROUP &player_group) {
  FOR_EACH_ENEMY_GROUP(i) {
    GROUP &g = *i;
    // Check to see if this group will arrive in next sector before the player
    // group.
...
static void InitiateGroupMovementToNextSector(GROUP *pGroup) {
  ...
  // Analyse the group and determine which direction it will move from the
  // current sector.
  dx = wp->x - pGroup->ubSectorX;
  dy = wp->y - pGroup->ubSectorY;
  if (dx && dy) {  // Can't move diagonally!
    AssertMsg(0, String("Attempting to move to waypoint in a diagonal "
                        "direction from sector %d,%d to sector %d,%d",
                        pGroup->ubSectorX, pGroup->ubSectorY, wp->x, wp->y));
  }
  AssertMsg(dx != 0 || dy != 0,
            String("Attempting to move to waypoint %d, %d that you are already at!", wp->x, wp->y));
  ...
  ubSector = (uint8_t)SECTOR(pGroup->ubSectorX, pGroup->ubSectorY);
  if (!pGroup->ubSectorZ) {
    BOOLEAN fCalcRegularTime = TRUE;
    if (!pGroup->fPlayer) {  // Determine if the enemy group is "sleeping".  If
                             // so, then simply delay their arrival time by the
                             // amount of time
  ...
  AssertMsg(pGroup->uiTraverseTime != TRAVERSE_TIME_IMPOSSIBLE,
            String("Group %d (%s) attempting illegal move from %c%d to %c%d (%s).",
  ...
  if (gfTacticalTraversal && gpTacticalTraversalGroup == pGroup) {
    if (gfUndergroundTacticalTraversal) {  // underground movement between
                                           // sectors takes 1 minute.
      pGroup->uiTraverseTime = 1;
  ...
  // NOTE: if the group is already between sectors, DON'T MESS WITH ITS ARRIVAL
  // TIME!  THAT'S NOT OUR JOB HERE!!!

  // special override for AI patrol initialization only
  if (gfRandomizingPatrolGroup) {  // We're initializing the patrol group, so
                                   // randomize the enemy groups to have extremely
                                   // quick and varying
  ...
  // For the case of player groups, we need to update the information of the
  // soldiers.
  if (pGroup->fPlayer) {
    if (pGroup->uiArrivalTime - ABOUT_TO_ARRIVE_DELAY > GetWorldTotalMin()) {
    ...
    // If an enemy group will be crossing paths with the player group, delay the
    // enemy group's arrival time so that the player will always encounter that
    // group.
    if (!pGroup->ubSectorZ) {
...
static void SetGroupPrevSectors(GROUP &g, uint8_t const x, uint8_t const y) {
  g.ubPrevX = x;
  g.ubPrevY = y;
}
...
void RemoveGroup(GROUP &g) {
  ...
  // Determine what type of group we have (because it requires different
  // methods)
  if (g.fPlayer) {
    while (g.pPlayerList) {
...
void SetGroupSectorValue(int16_t const x, int16_t const y, int16_t const z, GROUP &g) {
  RemoveGroupWaypoints(g);
...
void SetEnemyGroupSector(GROUP &g, uint8_t const sector_id) {
...
static void SetGroupNextSectorValue(int16_t const x, int16_t const y, GROUP &g) {
  RemoveGroupWaypoints(g);
  // Set sector x and y to passed values
  g.ubNextX = x;
  g.ubNextY = y;
...
int32_t CalculateTravelTimeOfGroup(GROUP const *const pGroup) {
  int32_t iDelta;
  ...
  iDelta = (int32_t)(SECTOR(pGroup->ubSectorX, pGroup->ubSectorY) -
  ...
  if (pGroup->fBetweenSectors) {
    // to get travel time to the first sector, use the arrival time, this way it
    // accounts for delays due to simul. arrival
    if (pGroup->uiArrivalTime >= GetWorldTotalMin()) {
...
int32_t FindTravelTimeBetweenWaypoints(WAYPOINT const *const pSource, WAYPOINT const *const pDest,
                                       GROUP const *const pGroup) {
  ...
  for (ubCurrentSector = ubStart; ubCurrentSector != ubEnd; ubCurrentSector += (int8_t)iDelta) {
    ...
    AssertMsg(
        iThisCostInTime != TRAVERSE_TIME_IMPOSSIBLE,
        String("Group %d (%s) attempting illegal move from sector %d, "
               "dir %d (%s).",
               pGroup->ubGroupID, (pGroup->fPlayer) ? "Player" : "AI", ubCurrentSector, ubDirection,
...
int32_t GetSectorMvtTimeForGroup(uint8_t const ubSector, uint8_t const direction,
                                 GROUP const *const g) {
  /* Determine the group's method(s) of transportation.  If more than one, we
   * will always use the highest time. */
  uint8_t const transport_mask = g->ubTransportationMask;
  uint8_t const traverse_type = SectorInfo[ubSector].ubTraversability[direction];
  int32_t best_traverse_time = 1000000;
  ...
  if (transport_mask & FOOT) {
    ...
    switch (traverse_type) {
      ...
      case EW_RIVER:
        ...
        break;
      default:
        return TRAVERSE_TIME_IMPOSSIBLE;
    }
    int32_t const traverse_time = FOOT_TRAVEL_TIME * 100 / traverse_mod;
    if (best_traverse_time > traverse_time) best_traverse_time = traverse_time;

    if (g->fPlayer) {
      int32_t highest_encumbrance = 100;
      CFOR_EACH_PLAYER_IN_GROUP(curr, g) {
         ...
         * rate. */
        int32_t const encumbrance = CalculateCarriedWeight(s);
  ...
  if (transport_mask & CAR) {
    ...
    int32_t const traverse_time = CAR_TRAVEL_TIME * 100 / traverse_mod;
    if (best_traverse_time > traverse_time) best_traverse_time = traverse_time;
  }
  ...
  if (transport_mask & TRUCK) {
    ...
    int32_t const traverse_time = TRUCK_TRAVEL_TIME * 100 / traverse_mod;
    if (best_traverse_time > traverse_time) best_traverse_time = traverse_time;
  }
  ...
  if (transport_mask & TRACKED) {
    ...
    int32_t const traverse_time = TRACKED_TRAVEL_TIME * 100 / traverse_mod;
    if (best_traverse_time > traverse_time) best_traverse_time = traverse_time;
  }

  if (transport_mask & AIR) {
    int32_t const traverse_time = AIR_TRAVEL_TIME;
    if (best_traverse_time > traverse_time) best_traverse_time = traverse_time;
  }
...
uint8_t PlayerMercsInSector(uint8_t const x, uint8_t const y, uint8_t const z) {
  uint8_t n_mercs = 0;
...
uint8_t PlayerGroupsInSector(uint8_t const x, uint8_t const y, uint8_t const z) {
  uint8_t n_groups = 0;
  CFOR_EACH_PLAYER_GROUP(g) {
    if (g->fBetweenSectors) continue;
    if (g->ubSectorX != x || g->ubSectorY != y || g->ubSectorZ != z) continue;
...
bool PlayerIDGroupInMotion(uint8_t const id) {
  GROUP *const g = GetGroup(id);
  return g && PlayerGroupInMotion(g);
}

// is the player group in motion?
BOOLEAN PlayerGroupInMotion(GROUP const *const pGroup) { return (pGroup->fBetweenSectors); }
...
void HandleArrivalOfReinforcements(GROUP const *const g) {
  if (g->fPlayer) { /* We don't have to worry about filling up the player slots,
                     * because it is impossible to have more player's in the
                     * game than the number of slots available for the player.
                     */
...
BOOLEAN PlayersBetweenTheseSectors(int16_t const sec_src, int16_t const sec_dst,
                                   int32_t *const n_enter, int32_t *const n_exit,
                                   BOOLEAN *const about_to_arrive_enter) {
  *n_enter = 0;
  *n_exit = 0;
  *about_to_arrive_enter = FALSE;

  GROUP const *const bg = gpBattleGroup;
  int16_t const sec_battle = bg ? SECTOR(bg->ubSectorX, bg->ubSectorY) : -1;
  ...
  CFOR_EACH_PLAYER_GROUP(i) {
    ...
    int16_t const sec_prev = SECTOR(g.ubPrevX, g.ubPrevY);
    int16_t const sec_cur = SECTOR(g.ubSectorX, g.ubSectorY);
    int16_t const sec_next = SECTOR(g.ubNextX, g.ubNextY);
    ...
    if (may_retreat_from_battle || (sec_cur == sec_src && sec_next == sec_dst)) {
      ...
      if (g.fVehicle && !is_heli_group) {  // subtract 1, we don't wanna count
                                           // the vehicle itself for purposes of
                                           // showing a number on the map
        n_mercs--;
      }
...
void MoveAllGroupsInCurrentSectorToSector(uint8_t const x, uint8_t const y, uint8_t const z) {
...
void SaveStrategicMovementGroupsToSaveGameFile(HWFILE const f) {
  ...
  FileWrite(f, &uiNumberOfGroups, sizeof(uint32_t));
...
void LoadStrategicMovementGroupsFromSavedGameFile(HWFILE const f) {
  ...
  uint32_t uiNumberOfGroups;
  FileRead(f, &uiNumberOfGroups, sizeof(uint32_t));

  // loop through all the nodes and add them to the LL
  GROUP **anchor = &gpGroupList;
  for (uint32_t i = uiNumberOfGroups; i != 0; --i) {
  ...
  //@@@ TEMP!
  // Rebuild the uniqueIDMask as a very old bug broke the uniqueID assignments
...
static void SavePlayerGroupList(HWFILE const f, GROUP const *const g) {
  ...
  FileWrite(f, &uiNumberOfNodesInList, sizeof(uint32_t));

  // Loop through and save only the players profile id
  CFOR_EACH_PLAYER_IN_GROUP(p, g) {
    // Save the ubProfile ID for this node
    const uint32_t uiProfileID = p->pSoldier->ubProfile;
    FileWrite(f, &uiProfileID, sizeof(uint32_t));
  }
}
...
static void LoadPlayerGroupList(HWFILE const f, GROUP *const g) {
  ...
  uint32_t node_count;
  FileRead(f, &node_count, sizeof(uint32_t));
  ...
  for (uint32_t i = node_count; i != 0; --i) {
    ...
    uint32_t profile_id;
    FileRead(f, &profile_id, sizeof(uint32_t));
...
static void SaveWayPointList(HWFILE const f, GROUP const *const g) {
  ...
  FileWrite(f, &uiNumberOfWayPoints, sizeof(uint32_t));
...
static void LoadWayPointList(HWFILE const f, GROUP *const g) {
  ...
  uint32_t uiNumberOfWayPoints;
  FileRead(f, &uiNumberOfWayPoints, sizeof(uint32_t));

  WAYPOINT **anchor = &g->pWaypoints;
  for (uint32_t i = uiNumberOfWayPoints; i != 0; --i) {
...
void CalculateGroupRetreatSector(GROUP *pGroup) {
  ...
  if (pGroup->fPlayer) {  // update the previous sector for the mercs
    CFOR_EACH_PLAYER_IN_GROUP(pPlayer, pGroup) {
      pPlayer->pSoldier->ubPrevSectorID = (uint8_t)SECTOR(pGroup->ubPrevX, pGroup->ubPrevY);
    }
  }
}
...
void RetreatGroupToPreviousSector(GROUP &g) {
  ...
  if (g.ubPrevX != 16 || g.ubPrevY != 16) {  // Group has a previous sector
    g.ubNextX = g.ubPrevX;
    g.ubNextY = g.ubPrevY;

    // Determine the correct direction
    int32_t const dx = g.ubNextX - g.ubSectorX;
    int32_t const dy = g.ubNextY - g.ubSectorY;
    if (dx == 0 && dy == -1)
      direction = NORTH_STRATEGIC_MOVE;
    else if (dx == 1 && dy == 0)
      direction = EAST_STRATEGIC_MOVE;
    else if (dx == 0 && dy == 1)
      direction = SOUTH_STRATEGIC_MOVE;
    else if (dx == -1 && dy == 0)
      direction = WEST_STRATEGIC_MOVE;
    else {
      AssertMsg(0, String("Player group attempting illegal retreat from %c%d to %c%d.",
                          g.ubSectorY + 'A' - 1, g.ubSectorX, g.ubNextY + 'A' - 1, g.ubNextX));
    }
  ...
  // Calc time to get to next waypoint
  uint8_t const sector = SECTOR(g.ubSectorX, g.ubSectorY);
  g.uiTraverseTime = GetSectorMvtTimeForGroup(sector, direction, &g);
  AssertMsg(g.uiTraverseTime != TRAVERSE_TIME_IMPOSSIBLE,
            String("Group %d (%s) attempting illegal move from %c%d to %c%d (%s).", g.ubGroupID,
                   g.fPlayer ? "Player" : "AI", g.ubSectorY + 'A', g.ubSectorX, g.ubNextY + 'A',
                   g.ubNextX, gszTerrain[SectorInfo[sector].ubTraversability[direction]]));

  // Because we are in the strategic layer, don't make the arrival instantaneous
  // (towns)
  ...
  if (g.fVehicle) {  // Vehicle, set fact it is between sectors too
    VEHICLETYPE &v = GetVehicleFromMvtGroup(g);
    v.fBetweenSectors = TRUE;
  }
...
GROUP *FindEnemyMovementGroupInSector(const uint8_t ubSectorX, const uint8_t ubSectorY) {
  FOR_EACH_ENEMY_GROUP(g) {
    if (g->ubSectorX == ubSectorX && g->ubSectorY == ubSectorY && g->ubSectorZ == 0) {
      return g;
    }
  }
  return NULL;
...
GROUP *FindPlayerMovementGroupInSector(const uint8_t x, const uint8_t y) {
  FOR_EACH_PLAYER_GROUP(i) {
    GROUP &g = *i;
    // NOTE: These checks must always match the INVOLVED group checks in PBI!!!
    if (g.ubGroupSize != 0 && !g.fBetweenSectors && g.ubSectorX == x && g.ubSectorY == y &&
        g.ubSectorZ == 0 && !GroupHasInTransitDeadOrPOWMercs(g) &&
        (!IsGroupTheHelicopterGroup(g) || !fHelicopterIsAirBorne)) {
      return &g;
    }
  }
...
BOOLEAN GroupAtFinalDestination(const GROUP *const pGroup) {
  ...
  if (!wp) {  // no waypoints, so the group is at it's destination.  This happens
              // when
    // an enemy group is created in the destination sector (which is legal for
    // staging groups which always stop adjacent to their real sector
    // destination)
    return TRUE;
  }
...
// The sector supplied resets ALL enemy groups in the sector specified.  See
// comments in ResetMovementForEnemyGroup() for more details on what the
// resetting does.
...
// currently between sectors.  If they were 50% of the way from sector A10 to
// A11, then after this function is called, then that group would be 0% of the
// way from sector A10 to A11.  In no way does this function effect the strategic
// path for the group.
static void ResetMovementForEnemyGroup(GROUP *pGroup) {
  ...
  if (!pGroup->fBetweenSectors || !pGroup->ubNextX ||
      !pGroup->ubNextY) {  // Reset the group's assignment by moving it to the
                           // group's original sector as it's pending group.
  ...
  // Calculate the new arrival time (all data pertaining to movement should be
  // valid)
...
// given it's current position in the route and the pGroup->ubMoveType must be
// ONE_WAY.  If the group is currently IN the sector, or just left the sector, it
// will return FALSE.
BOOLEAN GroupWillMoveThroughSector(GROUP *pGroup, uint8_t ubSectorX, uint8_t ubSectorY) {
  ...
  AssertMsg(pGroup->ubMoveType == ONE_WAY,
            String("GroupWillMoveThroughSector() -- Attempting to test group "
                   "with an invalid move type.  ubGroupID: %d, ubMoveType: %d, "
                   "sector: %c%d -- KM:0",
                   pGroup->ubGroupID, pGroup->ubMoveType, pGroup->ubSectorY + 'A' - 1,
                   pGroup->ubSectorX));

  // Preserve the original sector values, as we will be temporarily modifying
  // the group's ubSectorX/Y values as we traverse the waypoints.
  ...
  while (wp) {
    while (pGroup->ubSectorX != wp->x || pGroup->ubSectorY != wp->y) {
      ...
      dy = wp->y - pGroup->ubSectorY;
      if (dx && dy) {  // Can't move diagonally!
        AssertMsg(0, String("GroupWillMoveThroughSector() -- Attempting to "
                            "process waypoint in a diagonal direction from sector "
                            "%c%d to sector %c%d for group at sector %c%d -- KM:0",
      ...
      {
        AssertMsg(0, String("GroupWillMoveThroughSector() -- Attempting to process "
                            "same waypoint at %c%d for group at %c%d -- KM:0",
      ...
      // Check to see if it the sector we are checking to see if this group will
      // be moving through.
...
static int16_t VehicleFuelRemaining(SOLDIERTYPE const &vs) {
  Assert(vs.uiStatusFlags & SOLDIER_VEHICLE);
  return vs.sBreathRed;
}

static void SpendVehicleFuel(SOLDIERTYPE &vs, int16_t const fuel_spent) {
  Assert(vs.uiStatusFlags & SOLDIER_VEHICLE);
...
void AddFuelToVehicle(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pVehicle) {
  ...
  // Soldier has gas can, so now add gas to vehicle while removing gas from the
  // gas can. A gas can with 100 status translate to 50% of a fillup.
...
static void ReportVehicleOutOfGas(VEHICLETYPE const &v, uint8_t const x, uint8_t const y) {
  // Report that the vehicle that just arrived is out of gas
  wchar_t str[255];
  swprintf(str, lengthof(str), gzLateLocalizedString[STR_LATE_05], pVehicleStrings[v.ubVehicleType],
           y + 'A' - 1, x);
  DoScreenIndependantMessageBox(str, MSG_BOX_FLAG_OK, 0);
}

static void SetLocationOfAllPlayerSoldiersInGroup(GROUP const &g, int16_t const x, int16_t const y,
                                                  int8_t const z) {
  CFOR_EACH_PLAYER_IN_GROUP(i, &g) {
    if (!i->pSoldier) continue;
    SOLDIERTYPE &s = *i->pSoldier;
...
void RandomizePatrolGroupLocation(GROUP *pGroup) {  // Make sure this is an enemy patrol group
  WAYPOINT *wp;
  ...
  if (ubChosen >= ubMaxWaypointID) {  // They chose a waypoint going in the
                                      // reverse direction, so translate it
  ...
  // Traverse through the waypoint list again, to extract the location they are
  // at.
  wp = pGroup->pWaypoints;
  while (wp && ubChosen) {
    ubChosen--;
  ...
  ubSectorID = (uint8_t)SECTOR(wp->x, wp->y);
...
// Whenever a player group arrives in a sector, and if bloodcats exist in the
// sector, roll the dice to see if this will become an ambush random encounter.
  ...
  if (ubSectorID == SEC_N5 ||
    ...
    if (pSector->bBloodCats == -1) {
      pSector->bBloodCats = pSector->bBloodCatPlacements;
               ...
               pSector->bBloodCats < pSector->bBloodCatPlacements) {  // Slowly have them
                                                                      // recuperate if we haven't
      ...
      pSector->bBloodCats += (int8_t)std::min(iHoursElapsed / 18, iBloodCatDiff);
    }
    ...
    if (gfAutoAmbush || PreChance(ubChance)) {
      // randomly choose from 5-8, 7-10, 9-12 bloodcats based on easy, normal,
      ...
      bProgressMaxCats = (int8_t)std::max(
      ...
      pSector->bBloodCats = (int8_t)std::min(bDifficultyMaxCats, bProgressMaxCats);
      ...
      if (gGameOptions.ubDifficultyLevel !=
        ...
        pSector->bBloodCats = (int8_t)std::min(pSector->bBloodCats, bNumMercMaxCats);
        pSector->bBloodCats = (int8_t)std::max(pSector->bBloodCats, (int8_t)3);
      }

      // ensure that there aren't more bloodcats than placements
      pSector->bBloodCats = (int8_t)std::min(pSector->bBloodCats, pSector->bBloodCatPlacements);
    }
  } else if (ubSectorID != SEC_I16) {
    if (!gfAutoAmbush && PreChance(95)) {  // already ambushed here.  But 5%
                                           // chance of getting ambushed again!
...
void PlaceGroupInSector(GROUP &g, int16_t const prev_x, int16_t const prev_y, int16_t const next_x,
                        int16_t const next_y, int8_t const z, bool const check_for_battle) {
  ClearMercPathsAndWaypointsForAllInGroup(g);
...
void SetGroupArrivalTime(GROUP &g, uint32_t arrival_time) {
  ...
  if (IsGroupTheHelicopterGroup(g)) {
    ...
    if (arrival_time > now + g.uiTraverseTime) {
      AssertMsg(FALSE, String("SetGroupArrivalTime: Setting invalid arrival time %d "
                              "for group %d, WorldTime = %d, TraverseTime = %d",
                              arrival_time, g.ubGroupID, now, g.uiTraverseTime));
  ...
  if (!fPlayerPrompted) {
    ...
    if (!GroupAtFinalDestination(&g)) {
      if (AnyMercInGroupCantContinueMoving(g)) {
        ...
        // NOTE: Of course, it would be better if they continued onwards once
        // everyone was ready to go again, in which case we'd want to preserve
        // the plotted path, but since the player can mess with the squads, etc.
        // in the mean-time, that just seemed to risky to try to support.  They
        // could get into a fight and be too injured to move, etc.  Basically,
        // we'd have run a complete CanCharacterMoveInStrategic(0 check on all
        // of them. It's a wish list task for AM...

        // stop time so player can react if group was already on the move and
        // suddenly halts
        StopTimeCompression();
      } else {
...
static bool WildernessSectorWithAllProfiledNPCsNotSpokenWith(int16_t x, int16_t y, int8_t z);
...
static bool HandlePlayerGroupEnteringSectorToCheckForNPCsOfNote(GROUP &g) {
  ...
  int16_t const x = g.ubSectorX;
  int16_t const y = g.ubSectorY;
  int8_t const z = g.ubSectorZ;

  // Don't do this for underground sectors.
  if (z != 0) return false;

  // Skip towns/pseudo-towns (anything that shows up on the map as being
  // special).
  ...
  wchar_t msg[128];
  swprintf(msg, lengthof(msg), pLandMarkInSectorString, g.pPlayerList->pSoldier->bAssignment + 1,
           sector_name);

  MessageBoxFlags const flags =
      GroupAtFinalDestination(&g) ? MSG_BOX_FLAG_OK : MSG_BOX_FLAG_CONTINUESTOP;
  DoScreenIndependantMessageBox(msg, flags,
                                HandlePlayerGroupEnteringSectorToCheckForNPCsOfNoteCallback);
  // wait, we're prompting the player
  return true;
}

static bool WildernessSectorWithAllProfiledNPCsNotSpokenWith(int16_t const x, int16_t const y,
                                                             int8_t const z) {
  bool found_somebody = false;
  for (uint8_t pid = FIRST_RPC; pid != NUM_PROFILES; ++pid) {
    ...
    if (p.ubLastDateSpokenTo != 0 ||
        p.ubMiscFlags & (PROFILE_MISC_FLAG_RECRUITED |
                         PROFILE_MISC_FLAG_EPCACTIVE)) { /* Already spoke to this guy, don't
                                                          * prompt about this sector again,
                                                          * regardless of status of other
                                                          * NPCs here. Although Hamous
                                                          * wanders around, he never shares
                                                          * the same wilderness sector as
...
static void HandlePlayerGroupEnteringSectorToCheckForNPCsOfNoteCallback(
    MessageBoxReturnValue const exit_value) {
