...
// gain for hiring an NPC from a particular town (represents max. at 100% town
// attachment)
...
// on a scale of 1-100, this is a measure of how much each town hates the
...
UINT8 gubTownRebelSentiment[NUM_TOWNS] = {
    ...
    30,  // DRASSEN,	- Rebel friendly, makes it pretty easy to get first
         // mine's income going at the start
    12,  // ALMA			- Military town, high loyalty to Queen, need
         // quests to get 100%
    15,  // GRUMM,		- Close to Meduna, strong influence
    20,  // TIXA,		- Not a real town
    15,  // CAMBRIA, - Artificially much lower 'cause it's big and central and
         // too easy to get loyalty up there
    ...
    35,  // CHITZENA, - Artificially high 'cause there's not enough fights near
         // it to get the loyalty up otherwise
};
...
void StartTownLoyaltyIfFirstTime(INT8 bTownId) {
  ...
  if (!gTownLoyalty[bTownId].fStarted && gfTownUsesLoyalty[bTownId]) {
    // set starting town loyalty now, equally to that town's current rebel
    // sentiment - not all towns begin equally loyal
    gTownLoyalty[bTownId].ubRating = gubTownRebelSentiment[bTownId];

    // if player hasn't made contact with Miguel yet, or the rebels hate the
    // player
    if (!CheckFact(FACT_MIGUEL_READ_LETTER, 0) || CheckFact(FACT_REBELS_HATE_PLAYER, 0)) {
      // if town is Omerta
      if (bTownId == OMERTA) {
        // start loyalty there at 0, since rebels distrust the player until
        // Miguel receives the letter
        gTownLoyalty[bTownId].ubRating = 0;
      } else {
        // starting loyalty is halved - locals not sure what to make of the
        // player's presence
...
void IncrementTownLoyalty(INT8 bTownId, UINT32 uiLoyaltyIncrease) {
  ...
  // modify loyalty change by town's individual attitude toward rebelling (20 is
  // typical)
  uiLoyaltyIncrease *= (5 * gubTownRebelSentiment[bTownId]);
  uiLoyaltyIncrease /= 100;

  // this whole thing is a hack to avoid rolling over the -32 to 32k range on
...
void HandleMurderOfCivilian(const SOLDIERTYPE *const pSoldier) {
  ...
  if (pSoldier->ubProfile != NO_PROFILE) {
    // ignore murder of NPCs if they're not loyal to the rebel cause - they're
    // really just enemies in civilian clothing
  ...
  if (bKillerTeam == OUR_TEAM) {
    ...
    HandleMoraleEvent(killer, MORALE_KILLED_CIVILIAN, killer->sSectorX, killer->sSectorY,
                      killer->bSectorZ);
  }
  ...
  // check if this town does use loyalty (to skip a lot of unnecessary
  // computation)
  if (!gfTownUsesLoyalty[bTownId]) {
    return;
  }
  ...
  if (bKillerTeam != OUR_TEAM) {
    // If the murder is not fully witnessed, there's a chance of player being
    // blamed for it even if it's not his fault
    switch (bSeenState) {
      case 0:
        // nobody saw anything.  When body is found, chance player is blamed
        // depends on the town's loyalty at this time
        uiChanceFalseAccusal = MAX_LOYALTY_VALUE - gTownLoyalty[bTownId].ubRating;
        break;
      case 1:
        // civilians saw the killer, but not the victim. 10 % chance of blaming
        // player whether or not he did it
    ...
    if (Random(100) < uiChanceFalseAccusal) {
      ...
      // not really player's fault, reduce penalty, because some will believe it
      // to be a lie
  ...
  // this is a hack: to avoid having to adjust the values, divide by 1.75 to
  // compensate for the distance 0
  iLoyaltyChange *= 100;
...
void HandleTownLoyaltyForNPCRecruitment(SOLDIERTYPE *pSoldier) {
  ...
  if (bTownId == gMercProfiles[pSoldier->ubProfile].bTown) {
    ...
    // DESIGN QUESTION: How easy is it to abuse this bonus by repeatedly hiring
    // the guy over & over (at worst daily? even more often if terminated &
    // rehired?)  (ARM)
  }
}
...
void RemoveRandomItemsInSector(INT16 const sSectorX, INT16 const sSectorY, INT16 const sSectorZ,
  ...
  if (gWorldSectorX != sSectorX || gWorldSectorY != sSectorY || gbWorldSectorZ != sSectorZ) {
    ...
    for (WORLDITEM *wi = pItemList; wi != end; ++wi) {
      // if the item exists, and is visible and reachable, see if it should be
      // stolen
      ...
      ScreenMsg(MSG_FONT_RED, MSG_DEBUG, L"%ls stolen in %ls!", ItemNames[wi->o.usItem],
                wSectorName);
    }
  ...
  for (bTownId = FIRST_TOWN; bTownId < NUM_TOWNS; bTownId++) {
    if (bTownId == OMERTA) {
      ...
      if (gTownLoyalty[bTownId].ubRating > HOSTILE_OMERTA_LOYALTY_RATING) {
        // loyalty in Omerta tanks big time, and will stay low permanently since
        // this becomes its maximum
        SetTownLoyalty(bTownId, HOSTILE_OMERTA_LOYALTY_RATING);
        // note that rebel sentiment isn't affected - they remain loyal to
...
void AdjustLoyaltyForCivsEatenByMonsters(INT16 sSectorX, INT16 sSectorY, UINT8 ubHowMany) {
  ...
  swprintf(str, lengthof(str), gpStrategicString[STR_DIALOG_CREATURES_KILL_CIVILIANS], ubHowMany,
           pSectorString);
...
void HandleGlobalLoyaltyEvent(UINT8 ubEventType, INT16 sSectorX, INT16 sSectorY, INT8 bSectorZ) {
  ...
  // these are ->hundredths<- of loyalty points, so choose appropriate values
  // accordingly! the closer a town is to the event, the more pronounced the
  // effect upon that town is
  switch (ubEventType) {
    ...
    case GLOBAL_LOYALTY_NATIVE_KILLED:
      // note that there is special code (much more severe) for murdering
      // civilians in the currently loaded sector. this event is intended more for
      // processing militia casualties, and the like
      iLoyaltyChange = -50;
      break;
...
static void AffectAllTownsLoyaltyByDistanceFrom(INT32 iLoyaltyChange, INT16 sSectorX,
  ...
  // calc distance to the event sector from EACH SECTOR of each town, keeping
  // only the shortest one for every town
  FOR_EACH_TOWN_SECTOR(i) {
    bTownId = i->town;

    // skip path test if distance is already known to be zero to speed this up a
    // bit
    if (iShortestDistance[bTownId] > 0) {
      // calculate across how many sectors the fastest travel path from event to
      // this town sector
      iThisDistance =
          FindStratPath(sEventSector, SECTOR_INFO_TO_STRATEGIC_INDEX(i->sector), g, FALSE);
  ...
  for (bTownId = FIRST_TOWN; bTownId < NUM_TOWNS; bTownId++) {
    ...
    // This number here controls how many sectors away is the "norm" for the
    // unadjusted loyalty change value
    if (iShortestDistance[bTownId] < LOYALTY_EVENT_DISTANCE_THRESHOLD) {
      ...
      // don't allow it to go below -100, that would change the sign of the
      // loyalty effect!
...
void CheckIfEntireTownHasBeenLiberated(INT8 bTownId, INT16 sSectorX, INT16 sSectorY) {
  ...
  if (!gTownLoyalty[bTownId].fLiberatedAlready && IsTownUnderCompleteControlByPlayer(bTownId)) {
    ...
    // Orta/Tixa were previously controlled
...
void HandleLoyaltyChangeForNPCAction(UINT8 ubNPCProfileId) {
  switch (ubNPCProfileId) {
    ...
    case YANNI:
      ...
      // NOTE: This affects Chitzena,too, a second time, so first value is
      // discounted for it
...
static UINT32 PlayerStrength() {
  ...
  CFOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s->bInSector ||
        (s->fBetweenSectors && s->ubPrevSectorID % 16 + 1 == gWorldSectorX &&
         s->ubPrevSectorID / 16 + 1 == gWorldSectorY && s->bSectorZ == gbWorldSectorZ)) {
...
void MaximizeLoyaltyForDeidrannaKilled() {
  ...
  for (bTownId = FIRST_TOWN; bTownId < NUM_TOWNS; bTownId++) {
    // it's possible one of the towns still has creature problems, but it's too
    // much of a pain to worry about it now
