...
void BeginContractRenewalSequence() {
  for (CONTRACT_NEWAL_LIST_NODE const *i = ContractRenewalList, *const end =
                                                                    i + ubNumContractRenewals;
       i != end; ++i) {
...
void HandleContractRenewalSequence() {
  if (gfContractRenewalSquenceOn) {
    ...
    SOLDIERTYPE *const pSoldier = FindSoldierByProfileID(
        ContractRenewalList[ubCurrentContractRenewal].ubProfileID);  // Steve Willis, 80
        ...
        class DialogueEventContract : public CharacterDialogueEvent {
         public:
          DialogueEventContract(SOLDIERTYPE &soldier, bool const wants_to_renew)
              : CharacterDialogueEvent(soldier), wants_to_renew_(wants_to_renew) {}
...
BOOLEAN MercContractHandling(SOLDIERTYPE *const s, UINT8 const ubDesiredAction) {
  ...
  if (s->usLifeInsurance && s->bAssignment != ASSIGNMENT_POW) {
    ...
    } else {  // Can afford, ask if they want it
      HandleNotifyPlayerCanAffordInsurance(s, contract_length, iCostOfInsurance);
    }
  } else {  // No need to query for life insurance
    HandleImportantMercQuote(s, QUOTE_ACCEPT_CONTRACT_RENEWAL);
...
static UINT16 FindRefusalReason(SOLDIERTYPE const *const s) {
  ...
  for (UINT8 i = 0; i < 2; ++i) {
    ...
    if (p.bHatedCount[i] != 0) {  // tolerance is > 0, only gripe if in same sector
    ...
    return i == 0 ? QUOTE_HATE_MERC_1_ON_TEAM_WONT_RENEW : QUOTE_HATE_MERC_2_ON_TEAM_WONT_RENEW;
  }
  ...
  if (bMercID >= 0 && IsMercOnTeamAndInOmertaAlreadyAndAlive(bMercID)) {
    if (p.bLearnToHateCount == 0) {
      ...
      if (pHated && pHated->sSectorX == s->sSectorX && pHated->sSectorY == s->sSectorY &&
...
BOOLEAN WillMercRenew(SOLDIERTYPE *const s, BOOLEAN const say_quote) {
  ...
  if (s->fSignedAnotherContract) {
    // NOTE: Having a buddy around will NOT stop a merc from leaving on another
    // contract (IC's call)
    if (say_quote) {
  ...
  switch (GetFirstBuddyOnTeam(p)) {
    case 0:
      ...
      break;
    case 1:
      buddy_quote = QUOTE_RENEWING_CAUSE_BUDDY_2_ON_TEAM;
      break;
    case 2:
      buddy_quote = QUOTE_RENEWING_CAUSE_LEARNED_TO_LIKE_BUDDY_ON_TEAM;
      break;
    default:
      buddy_quote = QUOTE_NONE;
      break;
  }
...
poor performance by player if( pSoldier->ubWhatKindOfMercAmI !=
MERC_TYPE__AIM_MERC ) return( FALSE );
...
// this is called once a day (daily update) for every merc working for the
// player
...
static void NotifyPlayerOfMercDepartureAndPromptEquipmentPlacement(SOLDIERTYPE &,
                                                                   bool add_rehire_button);
  ...
  class CharacterDialogueEventContractEnding : public CharacterDialogueEvent {
   public:
    CharacterDialogueEventContractEnding(SOLDIERTYPE &s, bool const add_rehire_button)
        : CharacterDialogueEvent(s), add_rehire_button_(add_rehire_button) {}
    ...
    bool Execute() {
      ...
      SOLDIERTYPE &s = soldier_;
      // If the soldier may have some special action when he/she leaves the
      // party, handle it
...
void StrategicRemoveMerc(SOLDIERTYPE &s) {
  ...
  } else if (ubHistoryCode == HISTORY_MERC_FIRED ||
             s.bAssignment == ASSIGNMENT_POW) {  // The merc was fired
    AddCharacterToFiredList(&s);
  } else {  // The merc is leaving for some other reason
  ...
  if (s.bAssignment >= ON_DUTY &&
      s.ubGroupID) {                 // He/she is in a mvt group, remove and destroy the group
    if (s.bAssignment != VEHICLE) {  // Can only remove groups if they aren't
                                     // persistant (not in a squad or vehicle)
  ...
  // if the merc was a POW, remember it becuase the merc cant show up in AIM or
  // MERC anymore
  if (s.bAssignment == ASSIGNMENT_POW) {
    p.bMercStatus = MERC_FIRED_AS_A_POW;
  } else  // else the merc CAN get his medical deposit back
  {
    // Determine how much of a Medical deposit is going to be refunded to the
    // player
...
static void CalculateMedicalDepositRefund(SOLDIERTYPE const &s) {
  ...
  } else if (s.bLife > 0) {  // The merc is injured, refund a partial amount
  ...
  } else {  // The merc is dead, refund nothing
 ...
 * left */
static void NotifyPlayerOfMercDepartureAndPromptEquipmentPlacement(SOLDIERTYPE &s,
                                                                   bool add_rehire_button) {
  pLeaveSoldier = &s;
  ...
  if (s.ubProfile < FIRST_RPC || FIRST_NPC <= s.ubProfile) {  // The character is not an RPC
    ...
    GetShortSectorString(SECTORX(elsewhere), SECTORY(elsewhere), gzUserDefinedButton2,
                         lengthof(gzUserDefinedButton2));

    wchar_t const *const town = g_towns_locative[GetTownIdForSector(elsewhere)];
    wchar_t const *const text =
        sex == MALE ? str_he_leaves_where_drop_equipment : str_she_leaves_where_drop_equipment;
  ...
  no_choice:
    wchar_t const *const text =
        sex == MALE ? str_he_leaves_drops_equipment : str_she_leaves_drops_equipment;
  ...
  if (fInMapMode) {
    ...
    DoMessageBox(MSG_BOX_BASIC_STYLE, msg, guiCurrentScreen, flags, MercDepartEquipmentBoxCallBack,
                 0);
  }
}
...
static void MercDepartEquipmentBoxCallBack(MessageBoxReturnValue const exit_value) {
  ...
  switch (exit_value) {
    ...
    default: {
      bool const in_drassen =
          !StrategicMap[BOBBYR_SHIPPING_DEST_SECTOR_X + BOBBYR_SHIPPING_DEST_SECTOR_Y * MAP_WORLD_X]
               .fEnemyControlled;
...
void FindOutIfAnyMercAboutToLeaveIsGonnaRenew(void) {
  ...
  } else if (soldier_who_will_quit)  // Check if we should display line for the
                                     // guy who does not want to stay
  {
...
static void HandleNotifyPlayerCanAffordInsurance(SOLDIERTYPE *pSoldier, UINT8 ubLength,
  ...
  // Set the length to the global variable ( so we know how long the contract is
  // in the callback )
