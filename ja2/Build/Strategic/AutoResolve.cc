...
#define FOR_EACH_AR_MERC(iter)                                                    \
  for (SOLDIERCELL *iter = gpMercs, *const iter##__end = &gpMercs[gpAR->ubMercs]; \
       iter != iter##__end; ++iter)

#define FOR_EACH_AR_CIV(iter)                                                  \
  for (SOLDIERCELL *iter = gpCivs, *const iter##__end = &gpCivs[gpAR->ubCivs]; \
       iter != iter##__end; ++iter)

#define FOR_EACH_AR_ENEMY(iter)                                                         \
  for (SOLDIERCELL *iter = gpEnemies, *const iter##__end = &gpEnemies[gpAR->ubEnemies]; \
       iter != iter##__end; ++iter)
...
static void EliminateAllFriendlies() {
  if (gpAR) {
    ...
    gpAR->ubAliveMercs = 0;
    FOR_EACH_AR_CIV(i) { i->pSoldier->bLife = 0; }
    gpAR->ubAliveCivs = 0;
  }
}
...
void EliminateAllEnemies(uint8_t ubSectorX, uint8_t ubSectorY) {
  ...
  // RemoveAutoResolveInterface(0 won't happen, so we must process the enemies
  // killed right here & give out loyalty bonuses as if the battle had been
  // fought & won
  if (!gpAR) {
  ...
  if (gpAR) {
    FOR_EACH_AR_ENEMY(i) { i->pSoldier->bLife = 0; }
...
static void AssociateEnemiesWithStrategicGroups() {
  ...
  // Now go through our enemies in the autoresolve array, and assign the
  // ubGroupID to the soldier Stationary groups have a group ID of 0
  FOR_EACH_AR_ENEMY(i) {
  ...
  CFOR_EACH_ENEMY_GROUP(pGroup) {
    if (pGroup->ubSectorX == gpAR->ubSectorX && pGroup->ubSectorY == gpAR->ubSectorY) {
...
static void CalculateSoldierCells(BOOLEAN fReset) {
    ...
    for (x = 0; x < gpAR->ubMercCols; x++)
      for (y = 0; i && y < gpAR->ubMercRows; y++, i--) {
        ...
        if (AM_AN_EPC(gpMercs[index].pSoldier)) {
          if (AM_A_ROBOT(gpMercs[index].pSoldier)) {  // treat robot as a merc for the
                                                      // purpose of combat.
        ...
        c.pRegion = new MouseRegion(c.xp, c.yp, 50, 44, MSYS_PRIORITY_HIGH, 0,
  ...
  if (gpAR->ubCivs) {
    iStartY = iTop + (gpAR->rect.h - ((gpAR->ubMercRows + gpAR->ubCivRows) * 47 + 7)) / 2 +
...
static void RenderSoldierCell(SOLDIERCELL *const c) {
  ...
  if (c->pSoldier->bLife == 0) {
    ...
    if (c->pSoldier->bLife < OKLIFE &&
        !(c->uiFlags & (CELL_HITBYATTACKER | CELL_HITLASTFRAME |
                        CELL_CREATURE))) {  // Merc is unconcious (and not taking
                                            // damage), so darken his portrait.
...
static void BuildInterfaceBuffer() {
  ...
  // Blit the 4 corners
  BltVideoObject(gpAR->iInterfaceBuffer, gpAR->iPanelImages, TL_BORDER, DestRect.iLeft,
                 DestRect.iTop);
  BltVideoObject(gpAR->iInterfaceBuffer, gpAR->iPanelImages, TR_BORDER, DestRect.iRight - 10,
                 DestRect.iTop);
  BltVideoObject(gpAR->iInterfaceBuffer, gpAR->iPanelImages, BL_BORDER, DestRect.iLeft,
                 DestRect.iBottom - 9);
  BltVideoObject(gpAR->iInterfaceBuffer, gpAR->iPanelImages, BR_BORDER, DestRect.iRight - 10,
                 DestRect.iBottom - 9);
...
uint32_t VirtualSoldierDressWound(SOLDIERTYPE *pSoldier, SOLDIERTYPE *pVictim, OBJECTTYPE *pKit,
  ...
  if (!(gTacticalStatus.uiFlags & TURNBASED) ||
      !(gTacticalStatus.uiFlags & INCOMBAT)) {  // Set to a value which looks good based on out
                                                // tactical turns duration
  ...
  if (bPtsLeft && pVictim->bBleeding) {  // if we have enough points to bandage
                                         // all remaining bleeding this turn
...
static void AutoBandageMercs() {
  ...
  FOR_EACH_AR_MERC(i) {
    SOLDIERTYPE &s = *i->pSoldier;
    while (s.bBleeding != 0 && s.bLife != 0) {  // This merc needs medical attention
      if (!kit) {
        kit = FindMedicalKit();
        if (!kit) {
          complete = false;
  ...
  wchar_t const *const msg =
      complete ? gzLateLocalizedString[STR_LATE_13] : gzLateLocalizedString[STR_LATE_10];
...
static void RenderAutoResolve() {
  ...
  FOR_EACH_AR_MERC(i) { RenderSoldierCell(i); }
  FOR_EACH_AR_CIV(i) { RenderSoldierCell(i); }
  FOR_EACH_AR_ENEMY(i) { RenderSoldierCell(i); }
  ...
  if (gpAR->fPendingSurrender) {
    DisplayWrappedString(gpAR->sCenterStartX + 16, 230 + gpAR->bVerticalOffset, 108, 2, FONT10ARIAL,
  ...
  if (gpAR->ubBattleStatus != BATTLE_IN_PROGRESS) {
      ...
      switch (gpAR->ubBattleStatus) {
        ...
        case BATTLE_CAPTURED:
          FOR_EACH_IN_TEAM(i, OUR_TEAM) {
            SOLDIERTYPE &s = *i;
            if (s.bLife != 0 && !IsMechanical(s)) {  // Merc is alive and not a vehicle or robot
    ...
    switch (gpAR->ubBattleStatus) {
      ...
      case BATTLE_CAPTURED:
        if (gpAR->ubBattleStatus == BATTLE_SURRENDERED) {
          ...
          DisplayWrappedString(gpAR->sCenterStartX + 16, 310, 108, 2, FONT10ARIAL, FONT_YELLOW,
                               gpStrategicString[STR_ENEMY_CAPTURED], FONT_BLACK, LEFT_JUSTIFIED);
    ...
    SetFont(FONT10ARIAL);
    swprintf(str, lengthof(str), L"%ls:  %d%ls %02d%ls", gpStrategicString[STR_AR_TIME_ELAPSED],
             gpAR->uiTotalElapsedBattleTimeInMilliseconds / 60000, gsTimeStrings[1],
             gpAR->uiTotalElapsedBattleTimeInMilliseconds % 60000 / 1000, gsTimeStrings[2]);
    xp = gpAR->sCenterStartX + 70 - StringPixLength(str, FONT10ARIAL) / 2;
...
static void CreateAutoResolveInterface() {
  ...
  MSYS_DefineRegion(&ar->AutoResolveRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                    MSYS_PRIORITY_HIGH - 1, 0, MSYS_NO_CALLBACK, MSYS_NO_CALLBACK);
  ar->fRenderAutoResolve = TRUE;
  ar->fExitAutoResolve = FALSE;

  // Load the general panel image pieces, to be combined to make the dynamically
  // sized window.
  ...
  if (gubEnemyEncounterCode != CREATURE_ATTACK_CODE) {
    ...
    cell = MakeEnemyTroops(cell, ar->ubElites, ar, SOLDIER_CLASS_ELITE, ELITE_FACE,
                           gpStrategicString[STR_AR_ELITE_NAME]);
    cell = MakeEnemyTroops(cell, ar->ubTroops, ar, SOLDIER_CLASS_ARMY, TROOP_FACE,
                           gpStrategicString[STR_AR_TROOP_NAME]);
    cell = MakeEnemyTroops(cell, ar->ubAdmins, ar, SOLDIER_CLASS_ADMINISTRATOR, ADMIN_FACE,
  ...
  if (ar->ubSectorX == gWorldSectorX && ar->ubSectorY == gWorldSectorY && gbWorldSectorZ == 0) {
  ...
  MakeButton(RETREAT_BUTTON, dx + 51, dy + 34, RetreatButtonCallback, FALSE,
             gpStrategicString[STR_AR_RETREAT_BUTTON]);
  if (!ar->ubMercs) DisableButton(ar->iButton[RETREAT_BUTTON]);

  MakeButton(BANDAGE_BUTTON, dx + 11, dy + 5, BandageButtonCallback, TRUE, 0);
  MakeButton(DONEWIN_BUTTON, dx + 51, dy + 5, DoneButtonCallback, TRUE,
             gpStrategicString[STR_AR_DONE_BUTTON]);
  MakeButton(DONELOSE_BUTTON, dx + 25, dy + 5, DoneButtonCallback, TRUE,
             gpStrategicString[STR_AR_DONE_BUTTON]);
...
static void RemoveAutoResolveInterface(bool const delete_for_good) {
  ...
  if (delete_for_good) {  // Delete the soldier instances -- done when we are
                          // completely finished.
    ...
    if (IsAnybodyWounded()) {  // ARM: only one event is needed regardless of how
                               // many are bleeding
    ...
    // End capture squence
    if (ar.ubBattleStatus == BATTLE_SURRENDERED || ar.ubBattleStatus == BATTLE_CAPTURED) {
      EndCaptureSequence();
    }
  }
  ...
  for (int32_t i = 0; i != MAX_ALLOWABLE_MILITIA_PER_SECTOR; ++i) {
    ...
    switch (s.ubSoldierClass) {
      ...
      case SOLDIER_CLASS_REG_MILITIA:
        current_rank = REGULAR_MILITIA;
        break;
      case SOLDIER_CLASS_ELITE_MILITIA:
        current_rank = ELITE_MILITIA;
        break;
      default:
        break;
    }
  ...
  if (delete_for_good) {
    ...
    if (ar.ubBattleStatus == BATTLE_VICTORY) { /* Get rid of any extra enemies that could be here.
                                                * It is possible for the number of total enemies to
                                                * exceed 32, but autoresolve can only process
                                                * 32. We basically cheat by eliminating the rest of
                                                * them. */
  ...
  if (delete_for_good) {  // Warp the game time accordingly

    WarpGameTime(ar.uiTotalElapsedBattleTimeInMilliseconds / 1000,
  ...
  // KM : Aug 09, 1999 Patch fix -- Would break future dialog while time
  // compressing
...
static void RetreatButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (gpAR->pRobotCell) {  // if robot is retreating, set the retreat time to
                             // be the same as the robot's controller.
      ...
      FOR_EACH_AR_MERC(i) {
        ...
        // Found the controller, make the robot's retreat time match the
        // contollers.
...
static void MercCellMouseClickCallback(MOUSE_REGION *reg, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (gpAR->pRobotCell) {  // if controller is retreating, make the robot
                             // retreat too.
      const SOLDIERTYPE *const robot_controller = gpAR->pRobotCell->pSoldier->robot_remote_holder;
      if (robot_controller == NULL) {
        gpAR->pRobotCell->uiFlags &= ~CELL_RETREATING;
        gpAR->pRobotCell->uiFlags |= CELL_DIRTY;
        gpAR->pRobotCell->usNextAttack = 0xffff;
      } else if (robot_controller == pCell->pSoldier) {  // Found the controller, make the robot's
                                                         // retreat time match the contollers.
...
static void CalculateAutoResolveInfo() {
  ...
  CFOR_EACH_GROUP(i) {
    ...
    CFOR_EACH_PLAYER_IN_GROUP(pPlayer, &g) {
      ...
      // NOTE: Must check each merc individually, e.g. Robot without controller
      // is an uninvolved merc on an involved group!
      if (PlayerMercInvolvedInThisCombat(s)) {
        gpMercs[gpAR->ubMercs].pSoldier = &s;

        //!!! CLEAR OPPCOUNT HERE.  All of these soldiers are guaranteed to not
        //! be in tactical anymore.
        // ClearOppCount(&s);
...
static void CalculateRowsAndColumns() {
  ...
  // Now, that we have the number of mercs, militia, and enemies, it is possible
  // that there may be some conflicts.  Our goal is to make the window as small
  // as possible.  Bumping up the number of columns to 5 or rows to 10 will force
  // one or both axes to go full screen.  If we have high numbers of both, then
  // we will have to.
  ...
  // If we have both mercs and militia, we must make sure that the height to
  // width ratio is never higher than a factor of two.
  ...
  // Here is a extremely bitchy case.  The formulae throughout this module work
  // for most cases. However, when combining mercs and civs, the column must be
  // the same.  However, we retract that in cases where there are less mercs than
  // available to fill up *most* of the designated space.
...
static void RenderSoldierCellHealth(SOLDIERCELL *pCell) {
  ...
  if (pCell->pSoldier->bLife) {
    ...
    if (cnt > 3 &&
        pCell->pSoldier->bLife != pCell->pSoldier->bLifeMax) {  // Merc has taken damage, even
                                                                // though his life if good.
      usColor = FONT_YELLOW;
    }
  ...
  if (pCell->uiFlags & CELL_RETREATED && gpAR->ubBattleStatus != BATTLE_VICTORY) {
    ...
    if (pCell->pSoldier->bLife >= OKLIFE) {  // Retreating is shared with the status string.
...
static void DetermineTeamLeader(BOOLEAN fFriendlyTeam) {
  ...
  // For each team (civs and players count as same team), find the merc with the
  // best leadership ability.
  if (fFriendlyTeam) {
...
static void CalculateAttackValues() {
  ...
  for (int32_t i = 0; i < gpAR->ubMercs; i++) {
    ...
    // Give player controlled mercs a significant bonus to compensate for lack
    ...
    // bExpLevel adds a bonus of 7% per level after 2, level 1 soldiers get a 7%
    // decrease usBonus += 7 * (pSoldier->bExpLevel-2);
    ...
    if (pCell->uiFlags & CELL_EPC) {  // strengthen the defense (seeing the mercs
                                      // will keep them safe).
      pCell->usAttack = 0;
  ...
  // Now, because we are starting a new battle, we want to get the ball rolling
  // a bit earlier.  So, we will take the usBestAttack value and subtract 60% of
  // it from everybody's next attack.
  usBestAttack = usBestAttack * 60 / 100;
  FOR_EACH_AR_MERC(i) { i->usNextAttack -= usBestAttack; }
  FOR_EACH_AR_CIV(i) { i->usNextAttack -= usBestAttack; }
  FOR_EACH_AR_ENEMY(i) { i->usNextAttack -= usBestAttack; }
}
...
static void AttackTarget(SOLDIERCELL *pAttacker, SOLDIERCELL *pTarget) {
  ...
  if (pTarget->uiFlags & CELL_RETREATING &&
      !(pAttacker->uiFlags &
        CELL_FEMALECREATURE)) {  // Attacking a retreating merc is harder. Modify
                                 // the attack value to 70% of it's value.
  ...
  if (pAttacker->uiFlags & CELL_FEMALECREATURE) {
    ...
    fMelee = TRUE;
    fClaw = TRUE;
  } else if (!FireAShot(pAttacker)) {  // Maybe look for a weapon, such as a
                                       // knife or grenade?
    fMelee = TRUE;
    fKnife = AttackerHasKnife(pAttacker);
    if (TargetHasLoadedGun(pTarget->pSoldier)) {    // Penalty to attack with melee weapons
                                                    // against target with loaded gun.
  ...
  if (usAttack < usDefence) {
    if (pTarget->pSoldier->bLife >= OKLIFE ||
        !PreRandom(5)) {  // Attacker misses -- use up a round of ammo.  If
                          // target is unconcious, then 80% chance of hitting.
  ...
  if (!fMelee) {
    ...
    if (pAttacker->uiFlags & CELL_MERC) {  // Attacker is a player, so increment
                                           // the number of shots that hit.
      gMercProfiles[pAttacker->pSoldier->ubProfile].usShotsHit++;
      // MARKSMANSHIP GAIN: Attacker's shot hits
      StatChange(*pAttacker->pSoldier, MARKAMT, 6,
                 FROM_SUCCESS);  // in addition to 3 for taking a shot
    }
    if (pTarget->uiFlags & CELL_MERC) {  // Target is a player, so increment the
    ...
    if (pTarget->pSoldier->bLife >= CONSCIOUSNESS || pTarget->uiFlags & CELL_CREATURE) {
      if (gpAR->fSound) DoMercBattleSound(pTarget->pSoldier, BATTLE_SOUND_HIT1);
    }
    if (!(pTarget->uiFlags & CELL_CREATURE) && iNewLife < OKLIFE &&
        pTarget->pSoldier->bLife >= OKLIFE) {  // the hit caused the merc to
                                               // fall.  Play the falling sound
...
static void TargetHitCallback(SOLDIERCELL *pTarget, int32_t index) {
  ...
  if (pTarget->uiFlags & CELL_MERC &&
      pTarget->usHitDamage[index]) {  // Target is a player, so increment the
                                      // times he has been wounded.
    gMercProfiles[pTarget->pSoldier->ubProfile].usTimesWounded++;
    // EXPERIENCE GAIN: Took some damage
  ...
  if (iNewLife <= 0) {  // soldier has been killed
    if (pTarget->pAttacker[index]->uiFlags &
        CELL_PLAYER) {  // Player killed the enemy soldier -- update his stats as
                        // well as any assisters.
      SOLDIERCELL *pKiller;
...
static BOOLEAN IsBattleOver() {
  ...
  if (gpAR->pRobotCell) {  // Do special robot checks
    ...
    if (pRobot->robot_remote_holder == NULL) {  // Robot can't fight anymore.
      gpAR->usPlayerAttack -= gpAR->pRobotCell->usAttack;
      gpAR->pRobotCell->usAttack = 0;
      if (iNumInvolvedMercs == 1 && !gpAR->ubAliveCivs) {  // Robot is the only one left in battle,
                                                           // so instantly kill him.
...
static BOOLEAN AttemptPlayerCapture() {
  ...
  if (gpAR->fCaptureNotPermittedDueToEPCs) {  // EPCs make things much more
                                              // difficult when considering
                                              // capture.  Simply don't allow it.
    return FALSE;
  }
  ...
  // if the number of alive enemies doesn't double the number of alive mercs,
  // don't offer surrender.
  ...
  // So far, the conditions are right.  Now, we will determine if the the
  // remaining players are wounded and/or unconcious.  If any are concious, we
  // will prompt for a surrender, otherwise, it is automatic.
  fConcious = FALSE;
  FOR_EACH_AR_MERC(i) {
...
static void SetupDoneInterface() {
  ...
  FOR_EACH_AR_MERC(i) {  // So they can't retreat!
    i->pRegion->Disable();
  }
}
  ...
  while (iTimeSlice > 0) {
    ...
    while (--iTotal) {
      ...
      if ((iTimeSlice != 0x7fffffff && GetJA2Clock() > gpAR->uiCurrTime + 17) ||
        ...
        // leave now, which will allow for updating of the graphics (and mouse
        // cursor), and all of the necessary locals are saved via static
        // variables.  It'll check the fContinue flag, and goto the
        // CONTINUE_BATTLE label the next time this function is called.
        fContinue = TRUE;
        return;
      }
    CONTINUE_BATTLE:
      if (IsBattleOver() ||
          (gubEnemyEncounterCode != CREATURE_ATTACK_CODE && AttemptPlayerCapture()))
        return;
      ...
      // Apply damage and play miss/hit sounds if delay between firing and hit
      // has expired.
      ...
      if (pAttacker->pSoldier->bLife < OKLIFE || pAttacker->uiFlags & CELL_RETREATED) {
        if (!(pAttacker->uiFlags & CELL_CREATURE) || !pAttacker->pSoldier->bLife)
          continue;  // can't attack if you are unconcious or not around (Or a
                     // live creature)
      }
