...
void BobbyRayPurchaseEventCallback(const UINT8 ubOrderID) {
  ...
  const BOOLEAN fSectorLoaded =
      CloseCrate(BOBBYR_SHIPPING_DEST_SECTOR_X, BOBBYR_SHIPPING_DEST_SECTOR_Y,
                 BOBBYR_SHIPPING_DEST_SECTOR_Z, BOBBYR_SHIPPING_DEST_GRIDNO);
  ...
  if (CheckFact(FACT_PABLO_WONT_STEAL, 0)) {
    ...
    // Since Pacos has some money, reduce record of # of shipments since last
    // bribed...
    ubShipmentsSinceNoBribes /= 2;
    uiChanceOfTheft = 0;
  } else {
    ++ubShipmentsSinceNoBribes;
    // this chance might seem high but it's only applied at most to every second
    // item
  ...
  for (UINT8 i = 0; i < shipment->ubNumberPurchases; ++i) {
    ...
    for (UINT8 j = 0; j < ubItemsPurchased; ++j) {
      ...
      if (fSectorLoaded) {
        // add ubItemsPurchased to the chance of theft so the chance increases
        // when there are more items of a kind being ordered
  ...
  if (!fSectorLoaded) {
    ...
    // The item are to be added to the Top part of Drassen, grid loc's  10112,
    // 9950
    AddItemsToUnLoadedSector(BOBBYR_SHIPPING_DEST_SECTOR_X, BOBBYR_SHIPPING_DEST_SECTOR_Y,
                             BOBBYR_SHIPPING_DEST_SECTOR_Z, usStandardMapPos, uiCount, pObject, 0,
                             0, 0, INVISIBLE);
    if (uiStolenCount > 0) {
      AddItemsToUnLoadedSector(BOBBYR_SHIPPING_DEST_SECTOR_X, BOBBYR_SHIPPING_DEST_SECTOR_Y,
                               BOBBYR_SHIPPING_DEST_SECTOR_Z, PABLOS_STOLEN_DEST_GRIDNO,
                               uiStolenCount, pStolenObject, 0, 0, 0, INVISIBLE);
    }
    MemFree(pObject);
...
static void HandleDelayedItemsArrival(UINT32 uiReason) {
  ...
  if ((gWorldSectorX == BOBBYR_SHIPPING_DEST_SECTOR_X) &&
    ...
    SaveWorldItemsToTempItemFile(BOBBYR_SHIPPING_DEST_SECTOR_X, BOBBYR_SHIPPING_DEST_SECTOR_Y,
                                 BOBBYR_SHIPPING_DEST_SECTOR_Z, uiNumWorldItems, pTemp);
    MemFree(pTemp);
  }
}
...
void CheckForKingpinsMoneyMissing(BOOLEAN fFirstCheck) {
  ...
  if (fKingpinWillDiscover) {
    ...
    // set event 2 days from now that if the player has not given Kingpin his
    // money back, he sends email to the player
    AddFutureDayStrategicEvent(EVENT_SET_BY_NPC_SYSTEM, Random(120), FACT_KINGPIN_KNOWS_MONEY_GONE,
                               2);
  }
}
      ...
      case FACT_KINGPIN_KNOWS_MONEY_GONE:
        ...
        if (!CheckFact(FACT_KINGPIN_KNOWS_MONEY_GONE, 0)) {
          ...
          if (gubQuest[QUEST_KINGPIN_MONEY] == QUESTNOTSTARTED) {
            // KP knows money is gone, hasn't told player, if this event is called
            // then the 2 days are up... send email
...
void HandleEarlyMorningEvents(void) {
  ...
  if (gMercProfiles[HAMOUS].bLife > 0 && FindSoldierByProfileIDOnPlayerTeam(HAMOUS) == NULL &&
  ...
  // set Carmen to be placed on the map in case he moved and is waiting off
  // screen
...
void CheckForMissingHospitalSupplies(void) {
  ...
  CFOR_EACH_WORLD_ITEM(wi) {
    ...
    while (pItemPool) {
      ...
      if (o.bStatus[0] > 60) {
        if (o.usItem == FIRSTAIDKIT || o.usItem == MEDICKIT || o.usItem == REGEN_BOOSTER ||
            o.usItem == ADRENALINE_BOOSTER) {
          ubMedicalObjects++;
        }
      }
...
static void DropOffItemsInMeduna(UINT8 ubOrderNum) {
  ...
  const BOOLEAN fSectorLoaded =
      CloseCrate(MEDUNA_ITEM_DROP_OFF_SECTOR_X, MEDUNA_ITEM_DROP_OFF_SECTOR_Y,
  ...
  if (!fSectorLoaded) {
    ...
    // The item are to be added to the Top part of Drassen, grid loc's  10112,
    // 9950
    AddItemsToUnLoadedSector(MEDUNA_ITEM_DROP_OFF_SECTOR_X, MEDUNA_ITEM_DROP_OFF_SECTOR_Y,
                             MEDUNA_ITEM_DROP_OFF_SECTOR_Z, MEDUNA_ITEM_DROP_OFF_GRIDNO, uiCount,
                             pObject, 0, 0, 0, INVISIBLE);
    MemFree(pObject);
    pObject = NULL;
  }
