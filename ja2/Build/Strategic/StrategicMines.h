...
struct MINE_STATUS_TYPE {
  ...
  UINT32
  uiMaxRemovalRate;  // fastest rate we can move ore from this mine in period

  UINT32 uiRemainingOreSupply;  // the total value left to this mine (-1 means
                                // unlimited)
  UINT32 uiOreRunningOutPoint;  // when supply drop below this, workers tell
  ...
  UINT32 uiTimePlayerProductionStarted;  // time in minutes when
