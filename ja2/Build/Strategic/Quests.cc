...
static BOOLEAN CheckNPCIsEnemy(uint8_t ubProfileID) {
  ...
  if (pNPC->bSide == OUR_TEAM || pNPC->bNeutral) {
    if (pNPC->ubCivilianGroup != NON_CIV_GROUP) {
      // although the soldier is NOW the same side, this civ group could be set
      // to "will become hostile"
...
BOOLEAN CheckFact(Fact const usFact, uint8_t const ubProfileID) {
  ...
  switch (usFact) {
    ...
    case FACT_NEW_BOBBYRAY_SHIPMENT_WAITING:
      if (gubFact[FACT_PABLO_PUNISHED_BY_PLAYER] == TRUE && !gubFact[FACT_PABLO_RETURNED_GOODS] &&
  ...
  case FACT_SKYRIDER_CLOSE_TO_CHOPPER:
    ...
    case FACT_JOEY_NEAR_MARTHA:
      gubFact[usFact] = CheckNPCWithin(JOEY, MARTHA, 5) &&
    ...
    case FACT_SHANK_IN_SQUAD_BUT_NOT_SPEAKING:
      gubFact[usFact] = FindSoldierByProfileIDOnPlayerTeam(SHANK) != NULL &&
    ...
    case FACT_LOYALTY_LOW:
      bTown = gMercProfiles[ubProfileID].bTown;
      if ((bTown != BLANK_SECTOR) && gTownLoyalty[bTown].fStarted && gfTownUsesLoyalty[bTown]) {
        // if Skyrider, ignore low loyalty until he has monologues, and wait at
        // least a day since the latest monologue to avoid a hot/cold attitude
    ...
    case FACT_84_AND_85_TRUE:
      gubFact[usFact] =
          CheckFact(FACT_84, ubProfileID) && CheckFact(FACT_HANS_AT_SPOT, ubProfileID);
      break;
    ...
    case FACT_245:  // Can dimitri be recruited? should be true if already true, OR
                    // if Miguel has been recruited already
...
void InternalStartQuest(uint8_t ubQuest, int16_t sSectorX, int16_t sSectorY,
  ...
  if (gubQuest[ubQuest] == QUESTNOTSTARTED) {
    ...
    if (fUpdateHistory) {
      AddHistoryToPlayersLog(HISTORY_QUEST_STARTED, ubQuest, GetWorldTotalMin(), sSectorX,
                             sSectorY);
    }
  } else {
    gubQuest[ubQuest] = QUESTINPROGRESS;
...
void InternalEndQuest(uint8_t ubQuest, int16_t sSectorX, int16_t sSectorY, BOOLEAN fUpdateHistory) {
  if (gubQuest[ubQuest] == QUESTINPROGRESS) {
    ...
    if (fUpdateHistory) {
      AddHistoryToPlayersLog(HISTORY_QUEST_FINISHED, ubQuest, GetWorldTotalMin(), sSectorX,
                             sSectorY);
    }
  } else {
    gubQuest[ubQuest] = QUESTDONE;
...
void CheckForQuests(uint32_t uiDay) {
  ...
  // -------------------------------------------------------------------------------
  // The game always starts with DELIVER LETTER quest, so turn it on if it
  ...
  // This quest gets turned OFF through conversation with Miguel - when user
  // hands Miguel the letter
}
