       ...
       // slots repaired in this pass
        ...
        {/* pockets */ 12,
         {BIGPOCK1POS, BIGPOCK2POS, BIGPOCK3POS, BIGPOCK4POS, SMALLPOCK1POS, SMALLPOCK2POS,
          SMALLPOCK3POS, SMALLPOCK4POS, SMALLPOCK5POS, SMALLPOCK6POS, SMALLPOCK7POS,
          SMALLPOCK8POS}}};
...
static int16_t gsAssignmentBoxesX;
static int16_t gsAssignmentBoxesY;
...
static int8_t gbTrainingMode = -1;
...
// number we divide the total pts accumlated per day by for each assignment
// period
...
// how many points of healing each hospital patients gains per hour in the
// hospital
#define HOSPITAL_HEALING_RATE \
  5  // a top merc doctor can heal about 4 pts/hour maximum, but that's spread
     // among patients!
...
// activity levels for natural healing ( the higher the number, the slower the
// natural recover rate
...
// NOTE: A repairman must generate a least this many points / hour to be ABLE to
repair a SAM site at all! #define SAM_SITE_REPAIR_DIVISOR		10
...
BOOLEAN IsTheSAMSiteInSectorRepairable( int16_t sSectorX, int16_t sSectorY, int16_t
sSectorZ ); BOOLEAN SoldierInSameSectorAsSAM( SOLDIERTYPE *pSoldier ); BOOLEAN
CanSoldierRepairSAM( SOLDIERTYPE *pSoldier, int8_t bRepairPoints ); BOOLEAN
...
void ChangeSoldiersAssignment(SOLDIERTYPE *pSoldier, int8_t bAssignment) {
  // This is the most basic assignment-setting function.  It must be called
  // before setting any subsidiary values like fFixingRobot.  It will clear all
  // subsidiary values so we don't leave the merc in a messed up state!

  pSoldier->bAssignment = bAssignment;
  /// don't kill iVehicleId, though, 'cause militia training tries to put guys
  /// back in their vehicles when it's done(!)
...
static bool AreAssignmentConditionsMet(SOLDIERTYPE const &s, AssignmentConditions const c) {
  return (c & AC_IMPASSABLE || SectorIsPassable(SECTOR(s.sSectorX, s.sSectorY))) &&
         ...
         (c & AC_MECHANICAL || !IsMechanical(s)) && (c & AC_MOVING || !s.fBetweenSectors) &&
         (c & AC_UNDERGROUND || s.bSectorZ == 0) && !IsCharacterInTransit(s) &&
         s.bAssignment != ASSIGNMENT_POW;
}

static BOOLEAN BasicCanCharacterDoctor(const SOLDIERTYPE *const s) {
  return s->bMedical > 0 && AreAssignmentConditionsMet(*s, AC_UNDERGROUND);
}
...
static BOOLEAN DoesCharacterHaveAnyItemsToRepair(SOLDIERTYPE const *, int8_t bHighestPass);
...
static BOOLEAN HasCharacterFinishedRepairing(SOLDIERTYPE *pSoldier) {
  ...
  // NOTE: This must detect situations where the vehicle/robot has left the
  // sector, in which case we want the guy to say "assignment done", so we
  // return that he can no longer repair

  // check if we are repairing a vehicle
  if (pSoldier->bVehicleUnderRepairID != -1) {
    fCanStillRepair =
        CanCharacterRepairVehicle(*pSoldier, GetVehicle(pSoldier->bVehicleUnderRepairID));
  }
...
static BOOLEAN CanCharacterRepairAnotherSoldiersStuff(const SOLDIERTYPE *pSoldier,
                                                      const SOLDIERTYPE *pOtherSoldier);
static int8_t FindRepairableItemOnOtherSoldier(const SOLDIERTYPE *pSoldier, uint8_t ubPassType);
static bool IsItemRepairable(uint16_t item_id, int8_t status);

static BOOLEAN DoesCharacterHaveAnyItemsToRepair(SOLDIERTYPE const *const pSoldier,
                                                 int8_t const bHighestPass) {
  ...
  CFOR_EACH_SOLDIER_INV_SLOT(i, *pSoldier) {
    ...
    for (int8_t bLoop = 0; bLoop < MAX_ATTACHMENTS; ++bLoop) {
  ...
  if (bHighestPass != -1) {
    ...
    CFOR_EACH_IN_TEAM(pOtherSoldier, OUR_TEAM) {
      if (CanCharacterRepairAnotherSoldiersStuff(pSoldier, pOtherSoldier)) {
        // okay, seems like a candidate!  Check if he has anything that needs
        // unjamming or repairs

        for (int8_t bPocket = HANDPOS; bPocket <= SMALLPOCK8POS; ++bPocket) {
        ...
        // repair everyone's hands and armor slots first, then headgear, and
        // pockets last
        for (ubPassType = REPAIR_HANDS_AND_ARMOR; ubPassType <= (uint8_t)bHighestPass;
             ubPassType++) {
          int8_t const bPocket = FindRepairableItemOnOtherSoldier(pOtherSoldier, ubPassType);
          if (bPocket != NO_SLOT) {
            return (TRUE);
          }
        }
...
static BOOLEAN BasicCanCharacterRepair(const SOLDIERTYPE *const s) {
  return s->bMechanical > 0 && AreAssignmentConditionsMet(*s, AC_UNDERGROUND);
}

static BOOLEAN CanCharacterRepairButDoesntHaveARepairkit(const SOLDIERTYPE *const s) {
  return BasicCanCharacterRepair(s) &&
...
static BOOLEAN CanCharacterRepair(SOLDIERTYPE const *const pSoldier) {
  ...
  // NOTE: This will not detect situations where character lacks the SKILL to
  // repair the stuff that needs repairing... So, in that situation, his
  // assignment will NOT flash, but a message to that effect will be reported
  // every hour.
...
static BOOLEAN CanCharacterPatient(const SOLDIERTYPE *const s) {
  return s->bLife > 0 && s->bLife != s->bLifeMax &&
         AreAssignmentConditionsMet(*s, AC_UNCONSCIOUS | AC_EPC | AC_UNDERGROUND);
}

static BOOLEAN CanSectorContainMilita(const int16_t x, const int16_t y, const int16_t z) {
  return (z == 0 && StrategicMap[CALCULATE_STRATEGIC_INDEX(x, y)].bNameId !=
                        BLANK_SECTOR) ||  // is there a town?
...
static BOOLEAN BasicCanCharacterTrainMilitia(const SOLDIERTYPE *const s) {
  ...
  return s->bLeadership > 0 && CanSectorContainMilita(s->sSectorX, s->sSectorY, s->bSectorZ) &&
         NumEnemiesInAnySector(s->sSectorX, s->sSectorY, s->bSectorZ) == 0 &&
         AreAssignmentConditionsMet(*s, AC_NONE);
}

static int8_t CountMilitiaTrainersInSoldiersSector(const SOLDIERTYPE *s);

BOOLEAN CanCharacterTrainMilitia(const SOLDIERTYPE *const s) {
  return BasicCanCharacterTrainMilitia(s) &&
...
BOOLEAN DoesSectorMercIsInHaveSufficientLoyaltyToTrainMilitia(const SOLDIERTYPE *const s) {
  // underground training is not allowed (code doesn't support and it's a
  // reasonable enough limitation)
...
// only 2 trainers are allowed per sector, so this function counts the # in a
// guy's sector
static int8_t CountMilitiaTrainersInSoldiersSector(const SOLDIERTYPE *const pSoldier) {
  int8_t bCount = 0;
  CFOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s != pSoldier && s->bLife >= OKLIFE && s->sSectorX == pSoldier->sSectorX &&
        s->sSectorY == pSoldier->sSectorY && s->bSectorZ == pSoldier->bSectorZ &&
...
static int8_t GetTrainingStatValue(const SOLDIERTYPE *const s, const int8_t stat) {
  switch (stat) {
    case STRENGTH:
      return s->bStrength;
    case DEXTERITY:
    ...
    case HEALTH:
      return s->bLifeMax;
    case MARKSMANSHIP:
      return s->bMarksmanship;
    case MEDICAL:
      return s->bMedical;
    case MECHANICAL:
      return s->bMechanical;
    case LEADERSHIP:
      return s->bLeadership;
...
static BOOLEAN CanCharacterTrainStat(const SOLDIERTYPE *const s, int8_t bStat,
                                     const BOOLEAN fTrainSelf, const BOOLEAN fTrainTeammate) {
  ...
  const int8_t stat_val = GetTrainingStatValue(s, bStat);
  return stat_val != 0 && (!fTrainTeammate || stat_val >= MIN_RATING_TO_TEACH) &&
...
static BOOLEAN CanCharacterTrainTeammates(SOLDIERTYPE const *const pSoldier) {
  ...
  // ARM: we allow this even if there are no students assigned yet.  Flashing is
  // warning enough.
  return (TRUE);
}
...
static BOOLEAN CanCharacterBeTrainedByOther(SOLDIERTYPE const *const pSoldier) {
  ...
  // ARM: we now allow this even if there are no trainers assigned yet. Flashing
  // is warning enough.
  return (TRUE);
}
...
static bool CanCharacterSleep(SOLDIERTYPE const &s, bool const explain_why_not) {
  if (!AreAssignmentConditionsMet(s, AC_IMPASSABLE | AC_COMBAT | AC_EPC |
                                         AC_IN_HELI_IN_HOSTILE_SECTOR | AC_MOVING | AC_UNDERGROUND))
    return false;

  wchar_t const *why;
  if (s.fBetweenSectors)  // Traveling?
  {
    if (s.bAssignment != VEHICLE) {  // Can't sleep while walking
      why = zMarksMapScreenText[5];
      goto cannot_sleep;
    } else  // in a vehicle
    {
      // If this guy has to drive (because nobody else can)
      if (SoldierMustDriveVehicle(s, false)) {  // Can't sleep while driving a vehicle
...
static BOOLEAN CanCharacterBeAwakened(SOLDIERTYPE *pSoldier, BOOLEAN fExplainWhyNot) {
  ...
  // merc collapsed due to being dead tired, you can't wake him up until he
...
static bool CanCharacterVehicle(SOLDIERTYPE const &s) {  // Can character enter/leave vehicle?
  return fInMapMode &&  // Strictly for visual reasons - we don't want them just
                        // vanishing if in tactical
         AnyAccessibleVehiclesInSoldiersSector(s) &&
         (  // If we're in battle in the current sector, disallow
             !gTacticalStatus.fEnemyInSector || s.sSectorX != gWorldSectorX ||
             s.sSectorY != gWorldSectorY || s.bSectorZ != gbWorldSectorZ) &&
...
static JoinSquadResult CanCharacterSquad(SOLDIERTYPE const &s, int8_t const squad_no) {
...
static void HandleDoctorsInSector(int16_t x, int16_t y, int8_t z);
static void HandleNaturalHealing();
static void HandleRepairmenInSector(int16_t sX, int16_t sY, int8_t bZ);
static void HandleRestFatigueAndSleepStatus();
static void HandleTrainingInSector(int16_t sMapX, int16_t sMapY, int8_t bZ);
...
static BOOLEAN CanSoldierBeHealedByDoctor(const SOLDIERTYPE *pSoldier, const SOLDIERTYPE *pDoctor,
                                          BOOLEAN fThisHour, BOOLEAN fSkipKitCheck,
                                          BOOLEAN fSkipSkillCheck);

static uint8_t GetNumberThatCanBeDoctored(SOLDIERTYPE *pDoctor, BOOLEAN fThisHour,
                                          BOOLEAN fSkipKitCheck, BOOLEAN fSkipSkillCheck) {
  // go through list of characters, find all who are patients/doctors healable
  // by this doctor
...
static uint16_t TotalMedicalKitPoints(SOLDIERTYPE *pSoldier);
static BOOLEAN MakeSureMedKitIsInHand(SOLDIERTYPE *pSoldier);

uint16_t CalculateHealingPointsForDoctor(SOLDIERTYPE *pDoctor, uint16_t *pusMaxPts,
  ...
  // calculate normal doctoring rate - what it would be if his stats were
  // "normal" (ignoring drugs, fatigue, equipment condition) and equipment was
  // not a hindrance
...
static uint16_t ToolKitPoints(SOLDIERTYPE *pSoldier);
...
uint8_t CalculateRepairPointsForRepairman(SOLDIERTYPE *pSoldier, uint16_t *pusMaxPts,
  ...
  // calculate normal repair rate - what it would be if his stats were "normal"
  // (ignoring drugs, fatigue, equipment condition) and equipment was not a
  // hindrance
  *pusMaxPts = (pSoldier->bMechanical * pSoldier->bDexterity * (100 + (5 * pSoldier->bExpLevel))) /
  ...
  // figure out what shape his "equipment" is in ("coming" in JA3: Viagra -
  // improves the "shape" your "equipment" is in)
...
// how many points worth of doctoring does the character have in his medical
// kits?
static uint16_t TotalMedicalKitPoints(SOLDIERTYPE *pSoldier) {
  uint8_t ubPocket;
  uint16_t usKitpts = 0;
...
static void HandleDoctorsInSector(int16_t const x, int16_t const y, int8_t const z) {
...
static uint16_t HealPatient(SOLDIERTYPE *pPatient, SOLDIERTYPE *pDoctor,
...
static void HealCharacters(SOLDIERTYPE *const pDoctor) {
  ...
  if (ubTotalNumberOfPatients > 0) {
    ...
    // find how many healing points can be evenly distributed to each wounded,
    // healable merc
  ...
  // if there's nobody else here who can EVER be helped by this doctor
  // (regardless of whether they got healing this hour)
  if (GetNumberThatCanBeDoctored(pDoctor, HEALABLE_EVER, FALSE, FALSE) == 0) {
    // then this doctor has done all that he can do, but let's find out why and
    // tell player the reason
    ...
    // try again, but skip the skill check!
      ...
      // he could doctor somebody, but can't because he doesn't have enough
      // skill!
...
static uint8_t GetMinHealingSkillNeeded(SOLDIERTYPE const *const patient) {
...
static BOOLEAN CanSoldierBeHealedByDoctor(SOLDIERTYPE const *const patient,
                                          SOLDIERTYPE const *const doctor, BOOLEAN const fThisHour,
                                          BOOLEAN const fSkipKitCheck,
                                          BOOLEAN const fSkipSkillCheck) {
...
// heal patient, given doctor and total healing pts available to doctor at this
// time
static uint16_t HealPatient(SOLDIERTYPE *pPatient, SOLDIERTYPE *pDoctor,
                            uint16_t usHundredthsHealed) {
  ...
  usTotalHundredthsUsed = usHundredthsHealed;  // we'll subtract any unused amount later if we
                                               // become fully healed...
  ...
  if (pPatient->bLife < pPatient->bLifeMax) {
    ...
    // go through doctor's pockets and heal, starting at with his in-hand item
    // the healing pts are based on what type of medkit is in his hand, so we
    // HAVE to start there first!
    for (bPocket = HANDPOS; bPocket <= SMALLPOCK8POS; bPocket++) {
      OBJECTTYPE &o = pDoctor->inv[bPocket];
      if (IsMedicalKitItem(&o)) {
        // ok, we have med kit in this pocket, use it  (use only half if it's
        // worth double)
        bPointsUsed = UseKitPoints(o, bPointsToUse, *pDoctor);
        bPointsHealed = bPointsUsed;
  ...
  if (pPatient->bLife == pPatient->bLifeMax) {
    ...
    /* ARM Removed.  This is duplicating the check in
       UpdatePatientsWhoAreDoneHealing(), guy would show up twice!
...
static void HealHospitalPatient(SOLDIERTYPE *pPatient, uint16_t usHealingPtsLeft);
...
static void CheckForAndHandleHospitalPatients() {
  ...
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s->bAssignment == ASSIGNMENT_HOSPITAL && s->sSectorX == HOSPITAL_SECTOR_X &&
        s->sSectorY == HOSPITAL_SECTOR_Y && s->bSectorZ == 0) {
      // heal this character
...
static void HealHospitalPatient(SOLDIERTYPE *pPatient, uint16_t usHealingPtsLeft) {
  ...
  /*  Stopping hospital patients' bleeding must be handled immediately, not
     during a regular hourly check
...
static void HandleRepairmenInSector(int16_t const x, int16_t const y, int8_t const z) {
  FOR_EACH_IN_TEAM(i, OUR_TEAM) {
    SOLDIERTYPE &s = *i;
    if (s.sSectorX != x) continue;
    if (s.sSectorY != y) continue;
...
/* No point in allowing SAM site repair any more.  Jan/13/99.  ARM
int8_t HandleRepairOfSAMSite( SOLDIERTYPE *pSoldier, int8_t bPointsAvailable,
...
pSoldier->bSectorZ))
        ...
        else if( ( pSoldier -> sSectorX == gWorldSectorX ) && ( pSoldier ->
bSectorZ == gbWorldSectorZ )&&( pSoldier -> sSectorY == gWorldSectorY ) )
        {
        ...
        sStrategicSector = CALCULATE_STRATEGIC_INDEX( pSoldier->sSectorX,
pSoldier->sSectorY );

        // do we have more than enough?
        if( 100 - StrategicMap[ sStrategicSector ].bSAMCondition >=
bPointsAvailable / SAM_SITE_REPAIR_DIVISOR )
        {
                // no, use up all we have
                StrategicMap[ sStrategicSector ].bSAMCondition +=
bPointsAvailable / SAM_SITE_REPAIR_DIVISOR; bPtsUsed = bPointsAvailable - (
bPointsAvailable % SAM_SITE_REPAIR_DIVISOR );
        ...
        {
                // yep
                bPtsUsed = SAM_SITE_REPAIR_DIVISOR * ( 100 - StrategicMap[
sStrategicSector ].bSAMCondition ); StrategicMap[ sStrategicSector
].bSAMCondition = 100;

//ARM: NOTE THAT IF THIS CODE IS EVER RE-ACTIVATED, THE SAM GRAPHICS SHOULD
CHANGE NOT WHEN THE SAM SITE RETURNS TO
// FULL STRENGTH (condition 100), but as soon as it reaches
MIN_CONDITION_TO_FIX_SAM!!!

                // Bring Hit points back up to full, adjust graphic to full
graphic..... UpdateSAMDoneRepair( pSoldier -> sSectorX, pSoldier -> sSectorY,
pSoldier -> bSectorZ );
        }
...
static int8_t FindRepairableItemOnOtherSoldier(const SOLDIERTYPE *const pSoldier,
                                               const uint8_t ubPassType) {
  int8_t bLoop, bLoop2;
...
static void DoActualRepair(SOLDIERTYPE *pSoldier, uint16_t usItem, int8_t *pbStatus,
                           uint8_t *pubRepairPtsLeft) {
  int16_t sRepairCostAdj;
  uint16_t usDamagePts, usPtsFixed;

  // get item's repair ease, for each + point is 10% easier, each - point is 10%
  // harder to repair
  ...
  // repairs on electronic items take twice as long if the guy doesn't have the
  // skill
  ...
  } else  // not enough, partial fix only, if any at all
  {
    ...
    // NOTE: a really crappy repairman with only 1 pt/hr CAN'T repair
    // electronics or difficult items!
    if (usPtsFixed > 0) {
      *pbStatus += usPtsFixed;
...
static bool DoRepair(SOLDIERTYPE *const repairer, SOLDIERTYPE const *const owner,
                     uint16_t const item, int8_t &status, uint8_t *const repair_pts_left) {
  // if it's repairable and NEEDS repairing
  if (!IsItemRepairable(item, status)) return false;
  DoActualRepair(repairer, item, &status, repair_pts_left);
  if (status == 100) {  // report it as fixed
    if (repairer == owner) {
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, g_langRes->Message[STR_REPAIRED],
                repairer->name, ItemNames[item]);
    } else {  // NOTE: may need to be changed for localized versions
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, gzLateLocalizedString[STR_LATE_35],
                repairer->name, owner->name, ItemNames[item]);
    }
  }
  return true;
}

static bool RepairObject(SOLDIERTYPE *const repairer, SOLDIERTYPE const *const owner,
                         OBJECTTYPE *const pObj, uint8_t *const repair_pts_left) {
  bool something_was_repaired = false;

  uint8_t const n_items = pObj->ubNumberOfObjects;
  for (uint8_t i = 0; i != n_items; ++i) {
    uint16_t const item = pObj->usItem;
    int8_t &status = pObj->bStatus[i];
  ...
  for (uint8_t i = 0; i != MAX_ATTACHMENTS; ++i) {
    uint16_t const item = pObj->usAttachItem[i];
    if (item == NOTHING) continue;
    int8_t &status = pObj->bAttachStatus[i];
    if (!DoRepair(repairer, owner, item, status, repair_pts_left)) continue;
    something_was_repaired = true;
    if (*repair_pts_left == 0) break;  // we're out of points!
  }
...
static uint8_t HandleRepairOfRobotBySoldier(uint8_t ubRepairPts, BOOLEAN *pfNothingLeftToRepair);
static void RepairItemsOnOthers(SOLDIERTYPE *pSoldier, uint8_t *pubRepairPtsLeft);
static BOOLEAN UnjamGunsOnSoldier(SOLDIERTYPE *pOwnerSoldier, SOLDIERTYPE *pRepairSoldier,
                                  uint8_t *pubRepairPtsLeft);
...
static void HandleRepairBySoldier(SOLDIERTYPE &s) {
  ...
  uint16_t usMax;
  uint8_t initial_repair_pts = CalculateRepairPointsForRepairman(&s, &usMax, TRUE);
  if (initial_repair_pts == 0) {  // No points
    AssignmentDone(&s, TRUE, TRUE);
    return;
  }
  uint8_t repair_pts_left = initial_repair_pts;

  // check if we are repairing a vehicle
  if (s.bVehicleUnderRepairID != -1) {
    VEHICLETYPE const &v = GetVehicle(s.bVehicleUnderRepairID);
    if (CanCharacterRepairVehicle(s, v)) {  // Attempt to fix vehicle
  ...
  // check if we are repairing a robot
    ...
    for (int8_t i = 0; i != 2; ++i) {
      ...
      for (int8_t pocket = start; pocket <= end; ++pocket) {
  ...
  uint8_t const repair_pts_used = initial_repair_pts - repair_pts_left;
  if (repair_pts_used > 0) {
    ...
    if (Random(100) <
        repair_pts_used * 5)  // CJC: added a x5 as this wasn't going down anywhere fast enough
    {
      // kit item damaged/depleted, burn up points of toolkit..which is in right
      // hand
  ...
  } else  // still has stuff to repair
  {
    ...
    if (repair_pts_used == 0) {
      ...
      if (FindObj(&s, TOOLKIT) == NO_SLOT) {
        // he could (maybe) repair something, but can't because he doesn't have
        // a tool kit!
...
static bool IsItemRepairable(uint16_t const item_id, int8_t const status) {
  return status < 100 && Item[item_id].fFlags & ITEM_REPAIRABLE;
}

static uint8_t CalcSoldierNeedForSleep(SOLDIERTYPE const &);

// rest the character
static void RestCharacter(SOLDIERTYPE *pSoldier) {
  // handle the sleep of this character, update bBreathMax based on sleep they
  // have
  ...
  if (pSoldier->bBreathMax < BREATHMAX_PRETTY_TIRED) {
    // real tired, rest rate is 50% higher (this is to prevent absurdly long
    // sleep times for totally exhausted mercs)
...
static void FatigueCharacter(SOLDIERTYPE &s) {
  ...
  int8_t const divisor = 24 - CalcSoldierNeedForSleep(s);
  int8_t max_breath_loss = 50 / divisor;
  ...
  if (s.fBetweenSectors && s.bAssignment != VEHICLE) {  // Soldier is on foot and traveling. Factor
                                                        // encumbrance into fatigue rate.
    int32_t const percent_encumbrance = CalculateCarriedWeight(&s);
    if (percent_encumbrance > 100) {
      int32_t const breath_loss = max_breath_loss * percent_encumbrance / 100;
      max_breath_loss = std::min(breath_loss, 127);
    }
  }

  int8_t breath_max = s.bBreathMax;

  if (breath_max < BREATHMAX_PRETTY_TIRED) {  // Real tired, fatigue rate is 50% higher
...
static void TrainSoldierWithPts(SOLDIERTYPE *pSoldier, int16_t sTrainPts);
static BOOLEAN TrainTownInSector(SOLDIERTYPE *pTrainer, int16_t sMapX, int16_t sMapY,
                                 int16_t sTrainingPts);

// ONCE PER HOUR, will handle ALL kinds of training (self, teaching, and town)
// in this sector
static void HandleTrainingInSector(const int16_t sMapX, const int16_t sMapY, const int8_t bZ) {
  uint8_t ubStat;
  BOOLEAN fAtGunRange = FALSE;
  int16_t sTotalTrainingPts = 0;
  int16_t sTrainingPtsDueToInstructor = 0;
  ...
  // Training in underground sectors is disallowed by the interface code, so
  // there should never be any
  if (bZ != 0) {
    return;
  }

  // if sector not under our control, has enemies in it, or is currently in
  // combat mode
  if (!SectorOursAndPeaceful(sMapX, sMapY, bZ)) {
    // then training is canceled for this hour.
    // This is partly logical, but largely to prevent newly trained militia from
    // appearing in mid-battle
    return;
  }
  ...
  // init trainer list
  const SOLDIERTYPE *pStatTrainerList[NUM_TRAINABLE_STATS];  // can't have more "best" trainers
                                                             // than trainable stats
  memset(pStatTrainerList, 0, sizeof(pStatTrainerList));

  // build list of teammate trainers in this sector.

  // Only the trainer with the HIGHEST training ability in each stat is
  // effective.  This is mainly to avoid having to sort them from highest to
  // lowest if some form of trainer degradation formula was to be used for
  // multiple trainers.
    ...
    CFOR_EACH_IN_TEAM(pTrainer, OUR_TEAM) {
      if (pTrainer->sSectorX == sMapX && pTrainer->sSectorY == sMapY && pTrainer->bSectorZ == bZ) {
        ...
        if (pTrainer->bAssignment == TRAIN_TEAMMATE && pTrainer->bTrainStat == ubStat &&
            EnoughTimeOnAssignment(*pTrainer) && !pTrainer->fMercAsleep) {
        ...
        if (EnoughTimeOnAssignment(*pStudent) && !pStudent->fMercAsleep) {
          ...
          if (pStudent->bAssignment == TRAIN_BY_OTHER) {
            ...
            if (pTrainer != NULL) {
              /* Assignment distance limits removed.  Sep/11/98.  ARM
                                                                      // if this
                 sector either ISN'T currently loaded, or it is but the trainer
                 is close enough to the student if ( ( sMapX != gWorldSectorX )
                 || ( sMapY != gWorldSectorY ) || ( pStudent -> bSectorZ !=
                 gbWorldSectorZ ) || PythSpacesAway(pStudent->sGridNo,
                 pTrainer->sGridNo) < MAX_DISTANCE_FOR_TRAINING &&
                                                                                      EnoughTimeOnAssignment(*pTrainer))
              */
              // NB this EnoughTimeOnAssignment() call is redundent since it is
              // called up above
              // if (EnoughTimeOnAssignment(*pTrainer))
              {
                // valid trainer is available, this gives the student a large
                // training bonus!
  ...
  if (CanSectorContainMilita(sMapX, sMapY, bZ)) {
    ...
    for (uint32_t uiCnt = 0; uiCnt < ubTownTrainers; uiCnt++) {
      // top trainer has full effect (divide by 1), then divide by 2, 4, 8, etc.
      // sTownTrainingPts = TownTrainer[ uiCnt ].sTrainingPts / (uint16_t) pow(2,
      // uiCnt);
      ...
      if (sTownTrainingPts > 0) {
        ...
        if (fTrainingCompleted) {
          // there's no carryover into next session for extra training (cause
          // player might cancel), so break out of loop
          break;
        }
      }
    }
...
int16_t GetBonusTrainingPtsDueToInstructor(const SOLDIERTYPE *pInstructor,
  ...
  // return the bonus training pts of this instructor with this student,...if
  // student null, simply assignment student skill of 0 and student wisdom of
  // 100
  ...
  *pusMaxPts = 0;
  ...
  if (pStudent == NULL) {
    ...
    // for trainee's stat skill, must use the natural value, not the effective
    // one, to avoid drunks training beyond cap
  ...
  // check to see if student better than/equal to instructor's effective skill,
  // if so, return 0 don't use natural skill - if the guy's too doped up to tell
  // what he know, student learns nothing until sobriety returns!
  ...
  *pusMaxPts = (bTrainerNatSkill - bTraineeSkill) *
  ...
  if (*pusMaxPts == 0) {
    // we know trainer is better than trainee, make sure they are at least 10
    // pts better
...
int16_t GetSoldierTrainingPts(const SOLDIERTYPE *s, int8_t bTrainStat, BOOLEAN fAtGunRange,
  ...
  *pusMaxPts = 0;
  ...
  const int8_t bSkill = GetTrainingStatValue(s, bTrainStat);
...
int16_t GetSoldierStudentPts(const SOLDIERTYPE *s, int8_t bTrainStat, BOOLEAN fAtGunRange,
  ...
  *pusMaxPts = std::max(s->bWisdom * (TRAINING_RATING_CAP - bSkill) / SELF_TRAINING_DIVISOR, 1);
  ...
  // for each trainable stat
  sBestTrainingPts = -1;
  uint16_t usBestMaxTrainerPts = 0;  // XXX HACK000E

  // search team for active instructors in this sector
  CFOR_EACH_IN_TEAM(pTrainer, OUR_TEAM) {
    if (pTrainer->sSectorX == s->sSectorX && pTrainer->sSectorY == s->sSectorY &&
        pTrainer->bSectorZ == s->bSectorZ) {
      // if he's training teammates in this stat
      // NB skip the EnoughTime requirement to display what the value should be
      // even if haven't been training long yet...
      if (pTrainer->bAssignment == TRAIN_TEAMMATE && pTrainer->bTrainStat == bTrainStat &&
          !pTrainer->fMercAsleep) {
        sTrainingPtsDueToInstructor = GetBonusTrainingPtsDueToInstructor(
            pTrainer, s, bTrainStat, fAtGunRange, &usMaxTrainerPts);
...
static void TrainSoldierWithPts(SOLDIERTYPE *const s, const int16_t train_pts) {
  ...
  // give this merc a few chances to increase a stat (TRUE means it's training,
  // reverse evolution doesn't apply)
...
static BOOLEAN TrainTownInSector(SOLDIERTYPE *pTrainer, int16_t sMapX, int16_t sMapY,
  ...
  // trainer gains leadership - training argument is FROM_SUCCESS, because the
  // trainer is not the one training!
  ...
  // NOTE: Leave this at 100, change TOWN_TRAINING_RATE if necessary.  This
  // value gets reported to player as a %age!
  if (pSectorInfo->ubMilitiaTrainingPercentDone >= 100) {
    // zero out training completion - there's no carryover to the next training
    // session
...
int16_t GetTownTrainPtsForCharacter(const SOLDIERTYPE *pTrainer, uint16_t *pusMaxPts) {
  ...
  *pusMaxPts =
  ...
  uint16_t sTotalTrainingPts = (EffectiveWisdom(pTrainer) + EffectiveLeadership(pTrainer) +
  ...
  // RPCs get a small training bonus for being more familiar with the locals and
  // their customs/needs than outsiders
  ...
  *pusMaxPts += ((bTrainingBonus * *pusMaxPts) / 100);
  ...
  /* ARM: Decided this didn't make much sense - the guys I'm training damn well
     BETTER be loyal - and screw the rest!
          // get town index
          ubTownId = StrategicMap[ pTrainer -> sSectorX + pTrainer -> sSectorY *
     MAP_WORLD_X ].bNameId; Assert(ubTownId != BLANK_SECTOR);

          // adjust for town loyalty
          sTotalTrainingPts = (sTotalTrainingPts * gTownLoyalty[ ubTownId
     ].ubRating) / 100;
  */
...
void MakeSoldiersTacticalAnimationReflectAssignment(SOLDIERTYPE *const s) {
  ...
  switch (s->bAssignment) {
    case DOCTOR:
      SoldierInSectorDoctor(s, s->usStrategicInsertionData);
      break;
    case PATIENT:
      SoldierInSectorPatient(s, s->usStrategicInsertionData);
      break;
    case REPAIR:
      SoldierInSectorRepair(s, s->usStrategicInsertionData);
      break;
...
void AssignmentDone(SOLDIERTYPE *pSoldier, BOOLEAN fSayQuote, BOOLEAN fMeToo) {
  if ((pSoldier->bInSector) && (gfWorldLoaded)) {
    if (pSoldier->bAssignment == DOCTOR) {
      const uint16_t state = (guiCurrentScreen == GAME_SCREEN ? END_DOCTOR : STANDING);
      ChangeSoldierState(pSoldier, state, 1, TRUE);
    } else if (pSoldier->bAssignment == REPAIR) {
      const uint16_t state = (guiCurrentScreen == GAME_SCREEN ? END_REPAIRMAN : STANDING);
      ChangeSoldierState(pSoldier, state, 1, TRUE);
...
static void HandleHealingByNaturalCauses(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->bAssignment == ASSIGNMENT_POW) {
    // use high activity level to simulate stress, torture, poor conditions for
    // healing
    bActivityLevelDivisor = HIGH_ACTIVITY_LEVEL;
  }
  if (pSoldier->fMercAsleep || pSoldier->bAssignment == PATIENT ||
  ...
  } else  // this includes being in a vehicle - that's neither very strenous, nor
          // very restful
  {
  ...
  // gain that many hundredths of life points back, divided by the activity
  // level modifier
...
static BOOLEAN HandleRemoveMenu(const int8_t remove_char) {
...
static void AssignmentMenuMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void AssignmentMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateDestroyMouseRegionsForAssignmentMenu() {
  ...
  if (fShowAssignmentMenu && !fCreated) {
    ...
    PopUpBox const *const box =
        s.ubWhatKindOfMercAmI == MERC_TYPE__EPC ? ghEpcBox : ghAssignmentBox;
    SGPBox const &area = GetBoxArea(box);
    uint16_t const x = area.x;
    uint16_t y = area.y + GetTopMarginSize(ghAssignmentBox);
    uint16_t const w = area.w;
    uint16_t const dy = GetLineSpace(box) + GetFontHeight(GetBoxFont(box));

    // Add mouse region for each line of text and set user data
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(ghAssignmentBox); ++i) {
      MOUSE_REGION *const r = &gAssignmentMenuRegion[i];
      MSYS_DefineRegion(r, x, y, x + w, y + dy, MSYS_PRIORITY_HIGHEST - 4, MSYS_NO_CURSOR,
                        AssignmentMenuMvtCallBack, AssignmentMenuBtnCallback);
...
static void VehicleMenuMvtCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void VehicleMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void VehicleMenuCancelBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateDestroyMouseRegionForVehicleMenu() {
  ...
  if (!fCreated && fShowVehicleMenu) {
    SGPBox const &area = GetBoxArea(box);
    uint16_t const x = area.x;
    uint16_t y = area.y + GetTopMarginSize(ghAssignmentBox);  // XXX wrong box?
    uint16_t const w = area.w;
    uint16_t const h = GetLineSpace(box) + GetFontHeight(GetBoxFont(box));
    ...
    FOR_EACH_VEHICLE(i) {
      ...
      MSYS_DefineRegion(r, x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 4, MSYS_NO_CURSOR,
                        VehicleMenuMvtCallback, VehicleMenuBtnCallback);
      r->SetUserPtr(&v);
      y += h;
      ++r;
    }
    ...
    // cancel line
    MSYS_DefineRegion(r, x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 4, MSYS_NO_CURSOR,
                      VehicleMenuMvtCallback, VehicleMenuCancelBtnCallback);
    ...
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(box); ++i) {
...
static void HandleShadingOfLinesForVehicleMenu() {
  ...
  CFOR_EACH_VEHICLE(i) {
    ...
    PopUpShade const shade = IsEnoughSpaceInVehicle(v) ? POPUP_SHADE_NONE : POPUP_SHADE_SECONDARY;
...
static void VehicleMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (IsEnoughSpaceInVehicle(v)) {
      ...
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, gzLateLocalizedString[STR_LATE_18],
                zVehicleName[v.ubVehicleType]);
    }
...
static void VehicleMenuMvtCallback(MOUSE_REGION *pRegion, int32_t iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_GAIN_MOUSE) {
    const int32_t line = (int32_t)(pRegion - gVehicleMenuRegion);
...
static BOOLEAN IsRobotInThisSector(int16_t sSectorX, int16_t sSectorY, int8_t bSectorZ);
...
static void DisplayRepairMenu(SOLDIERTYPE const &s) {
  ...
  if (s.bSectorZ == 0) {  // Run through list of vehicles in sector and add them to pop up box
    CFOR_EACH_VEHICLE(i) {
      VEHICLETYPE const &v = *i;
  ...
  if (IsRobotInThisSector(s.sSectorX, s.sSectorY, s.bSectorZ)) {  // Robot
...
static void RepairMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void RepairMenuMvtCallback(MOUSE_REGION *pRegion, int32_t iReason);

static void MakeRepairRegion(const int32_t idx, const uint16_t x, const uint16_t y,
                             const uint16_t w, const uint16_t h, const uint32_t data) {
  ...
  if (fShowRepairMenu && !fCreated) {
    ...
    if (s.bSectorZ == 0) {
      ...
      CFOR_EACH_VEHICLE(i) {
        ...
        // other vehicles *in the sector* are listed, but later shaded dark if
    ...
    /* No point in allowing SAM site repair any more.  Jan/13/99.  ARM
                    // SAM site
                    if (IsThisSectorASAMSector(s.sSectorX, s.sSectorY,
       s.bSectorZ) && IsTheSAMSiteInSectorRepairable(s.sSectorX, s.sSectorY,
       s.bSectorZ))
                    {
                            MakeRepairRegion(idx++, x, y, w, h,
       REPAIR_MENU_SAM_SITE); y += h;
                    }
    */
    ...
    // remove these regions
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(ghRepairBox); ++i) {
  ...
  if (pSoldier && pSoldier->bActive && (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP)) {
    ...
    /* No point in allowing SAM site repair any more.  Jan/13/99.  ARM
                    else if( iRepairWhat == REPAIR_MENU_SAM_SITE )
                    {
                            ...
                            if (pSoldier->bAssignment != REPAIR ||
       !pSoldier->fFixingSAMSite)
                            {
                                    SetTimeOfAssignmentChangeForMerc( pSoldier
       );
                            ...
                            // the second argument is irrelevant here, function
       looks at pSoldier itself to know what's being repaired
                            SetAssignmentForList( ( int8_t ) REPAIR, 0 );
                            fShowAssignmentMenu = FALSE;
    ...
    */
      ...
      // itself to know what's being repaired
      SetAssignmentForList((int8_t)REPAIR, 0);
      fShowAssignmentMenu = FALSE;
...
static void MakeSureToolKitIsInHand(SOLDIERTYPE *pSoldier) {
  ...
  if (pSoldier->inv[HANDPOS].usItem != TOOLKIT) {
    // run through rest of inventory looking for toolkits, swap the first one
    // into hand if found
...
static BOOLEAN MakeSureMedKitIsInHand(SOLDIERTYPE *pSoldier) {
  ...
  // run through rest of inventory looking for MEDICAL BAGS, swap the first one
  // into hand if found
  for (bPocket = SECONDHANDPOS; bPocket <= SMALLPOCK8POS; bPocket++) {
...
void HandleShadingOfLinesForAssignmentMenus() {
  ...
  if (s && s->bActive) {
    if (s->ubWhatKindOfMercAmI == MERC_TYPE__EPC) {
      ...
      {  // doctor
        PopUpShade const shade = !BasicCanCharacterDoctor(s) ? POPUP_SHADE
      ...
      {  // repair
        PopUpShade const shade = !BasicCanCharacterRepair(s) ? POPUP_SHADE
...
static void AssignmentScreenMaskBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
void CreateDestroyScreenMaskForAssignmentAndContractMenus() {
  ...
  if (!fCreated && (fShowAssignmentMenu || fShowContractMenu || fShowTownInfo)) {
    MSYS_DefineRegion(&gAssignmentScreenMaskRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
...
static void ContractMenuMvtCallback(MOUSE_REGION *pRegion, int32_t iReason);
static void ContractMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
void CreateDestroyMouseRegionsForContractMenu() {
  ...
  if (fShowContractMenu && !fCreated) {
    ...
    SGPBox const &area = GetBoxArea(box);
    uint16_t const x = area.x;
    uint16_t y = area.y + GetTopMarginSize(box);
    uint16_t const w = area.w;
    uint16_t const dy = GetLineSpace(box) + GetFontHeight(GetBoxFont(box));
    ...
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(box); ++i) {
      ...
      MSYS_DefineRegion(r, x, y, x + w, y + dy, MSYS_PRIORITY_HIGHEST - 4, MSYS_NO_CURSOR,
                        ContractMenuMvtCallback, ContractMenuBtnCallback);
      MSYS_SetRegionUserData(r, 0, i);
      y += dy;
    }
...
static void TrainingMenuMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void TrainingMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateDestroyMouseRegionsForTrainingMenu() {
  ...
  if (!fCreated && fShowTrainingMenu) {
    ...
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(box); ++i) {
      ...
      MSYS_DefineRegion(r, x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 3, MSYS_NO_CURSOR,
                        TrainingMenuMvtCallBack, TrainingMenuBtnCallback);
      MSYS_SetRegionUserData(r, 0, i);
      y += h;
    }
...
static void AttributeMenuMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void AttributesMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateDestroyMouseRegionsForAttributeMenu() {
  ...
  if (!fCreated && fShowAttributeMenu) {
    ...
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(box); ++i) {
      ...
      MSYS_DefineRegion(r, x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 2, MSYS_NO_CURSOR,
                        AttributeMenuMvtCallBack, AttributesMenuBtnCallback);
      MSYS_SetRegionUserData(r, 0, i);
      y += h;
    }
...
static void RemoveMercMenuMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void RemoveMercMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateDestroyMouseRegionsForRemoveMenu() {
  ...
  if ((fShowAssignmentMenu || fShowContractMenu) && !fCreated) {
    ...
    if (fShowContractMenu) {
      ...
      pos = &AssignmentPosition, SetBoxXY(ghAssignmentBox, pos->iX, pos->iY);
    }
    ...
    int32_t y = area.y + GetTopMarginSize(ghAttributeBox);
    int32_t const w = area.w;
    int32_t const h =
        GetLineSpace(ghRemoveMercAssignBox) + GetFontHeight(GetBoxFont(ghRemoveMercAssignBox));

    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(ghRemoveMercAssignBox); ++i) {
      // add mouse region for each line of text..and set user data
      MOUSE_REGION *const r = &gRemoveMercAssignRegion[i];
      MSYS_DefineRegion(r, x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 2, MSYS_NO_CURSOR,
                        RemoveMercMenuMvtCallBack, RemoveMercMenuBtnCallback);
      MSYS_SetRegionUserData(r, 0, i);
      y += h;
    }
...
static void SquadMenuMvtCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SquadMenuBtnCallback(MOUSE_REGION *pRegion, int32_t iReason);
...
static void CreateDestroyMouseRegionsForSquadMenu() {
  ...
  if (fShowSquadMenu && !fCreated) {
    ...
    int32_t const x = area.x;
    int32_t y = area.y + GetTopMarginSize(ghSquadBox);
    int32_t const w = area.w;
    int32_t const h = GetLineSpace(ghSquadBox) + GetFontHeight(GetBoxFont(ghSquadBox));
    ...
    for (i = 0; i < GetNumberOfLinesOfTextInBox(ghSquadBox) - 1; ++i) {
      ...
      MSYS_DefineRegion(r, x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 2, MSYS_NO_CURSOR,
                        SquadMenuMvtCallBack, SquadMenuBtnCallback);
      MSYS_SetRegionUserData(r, 0, i);
      y += h;
    }

    // now create cancel region
    MSYS_DefineRegion(&gSquadMenuRegion[i], x, y, x + w, y + h, MSYS_PRIORITY_HIGHEST - 2,
    ...
    for (uint32_t i = 0; i < GetNumberOfLinesOfTextInBox(ghSquadBox); ++i) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    switch (iValue) {
      ...
      case (REMOVE_MERC):
        ...
        // Def: 10/13/99:  When a merc is either dead or a POW, the Remove Merc
        // popup comes up instead of the
        // Assign menu popup.  When the the user removes the merc, we need to make
        // sure the assignment menu
        // doesnt come up ( because the both assign menu and remove menu flags are
        // needed for the remove pop up to appear dont ask why?!! )
        fShownContractMenu = FALSE;
        fShownAssignmentMenu = FALSE;
        break;
    }
  ...
  switch (s->ubWhatKindOfMercAmI) {
    ...
    case MERC_TYPE__AIM_MERC:
      if (WillMercRenew(s, FALSE))  // Only do this if they want to renew
      {
        ...
        uint16_t const quote = GetWorldTotalMin() - s->uiTimeOfLastContractUpdate < 60 * 48
    ...
    switch (iValue) {
      ...
      case (CONTRACT_MENU_TERMINATE):
        ...
        if (gfInContractMenuFromRenewSequence) {
          ...
          DoMapMessageBox(MSG_BOX_BASIC_STYLE, gzLateLocalizedString[STR_LATE_48], MAP_SCREEN,
...
static void SquadMenuBtnCallback(MOUSE_REGION *const pRegion, int32_t const reason) {
  // btn callback handler for assignment region
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    int32_t const value = MSYS_GetRegionUserData(pRegion, 0);

    if (value == SQUAD_MENU_CANCEL) {  // Stop displaying, leave
      UnHighLightBox(ghAssignmentBox);
      fShowSquadMenu = FALSE;
      fTeamPanelDirty = TRUE;
      fMapScreenBottomDirty = TRUE;
    ...
    switch (iValue) {
      ...
      case (TRAIN_MENU_TOWN):
        if (BasicCanCharacterTrainMilitia(pSoldier)) {
          int8_t const bTownId = GetTownIdForSector(SECTOR(pSoldier->sSectorX, pSoldier->sSectorY));

          // if it's a town sector (the following 2 errors can't happen at
          // non-town SAM sites)
          if (bTownId != BLANK_SECTOR) {
          ...
          if (CountMilitiaTrainersInSoldiersSector(pSoldier) >= MAX_MILITIA_TRAINERS_PER_SECTOR) {
            swprintf(sString, lengthof(sString), gzLateLocalizedString[STR_LATE_47],
...
static PopUpBox *MakeBox(const SGPPoint pos, const uint32_t flags) {
  return CreatePopUpBox(pos, flags | POPUP_BOX_FLAG_RESIZE, FRAME_BUFFER, guiPOPUPBORDERS,
                        guiPOPUPTEX, 6, 6, 4, 4, 2);
}

static uint32_t GetLastSquadListedInSquadMenu();
...
static void CreateSquadBox() {
  ...
  // add strings for box
  uint32_t const uiMaxSquad = GetLastSquadListedInSquadMenu();
  for (uint32_t i = 0; i <= uiMaxSquad; ++i) {
    // get info about current squad and put in  string
    wchar_t buf[64];
    swprintf(buf, lengthof(buf), L"%ls ( %d/%d )", pSquadMenuStrings[i], NumberOfPeopleInSquad(i),
  ...
  SGPBox const &area = GetBoxArea(box);
  int16_t const max_y = SCREEN_HEIGHT - area.h;
  if (giBoxY > max_y) {
...
static void CreateEPCBox() {
  ...
  for (int32_t i = 0; i < MAX_EPC_MENU_STRING_COUNT; ++i) {
...
static void HandleShadingOfLinesForSquadMenu() {
  ...
  SOLDIERTYPE const &s = *GetSelectedAssignSoldier(FALSE);
  uint32_t const max_squad = GetLastSquadListedInSquadMenu();
  for (uint32_t i = 0; i <= max_squad; ++i) {
    JoinSquadResult const bResult = CanCharacterSquad(s, (int8_t)i);
...
void CreateContractBox(const SOLDIERTYPE *const s) {
  ...
  if (s) {
    MERCPROFILESTRUCT const *const p =
        s->ubWhatKindOfMercAmI == MERC_TYPE__AIM_MERC ? &GetProfile(s->ubProfile) : 0;
    for (uint32_t i = 0; i < MAX_CONTRACT_MENU_STRING_COUNT; ++i) {
      int32_t salary;
      switch (i) {
        case CONTRACT_MENU_DAY:
          salary = p ? p->sSalary : 0;
          break;
        case CONTRACT_MENU_WEEK:
          salary = p ? p->uiWeeklySalary : 0;
          break;
        case CONTRACT_MENU_TWO_WEEKS:
          salary = p ? p->uiBiWeeklySalary : 0;
          break;
...
static void CreateAttributeBox() {
  ...
  for (uint32_t i = 0; i < MAX_ATTRIBUTE_STRING_COUNT; ++i) {
...
static void CreateTrainingBox() {
  ...
  for (uint32_t i = 0; i < MAX_TRAIN_STRING_COUNT; ++i) {
...
static void CreateAssignmentsBox() {
  ...
  // No soldier is legal here!  Gets called during every mapscreen
  // initialization even when nobody is assign char
  SOLDIERTYPE *const s = GetSelectedAssignSoldier(TRUE);

  for (uint32_t i = 0; i < MAX_ASSIGN_STRING_COUNT; ++i) {
...
void CreateMercRemoveAssignBox() {
  ...
  for (uint32_t i = 0; i < MAX_REMOVE_MERC_COUNT; ++i) {
...
void DetermineBoxPositions() {
  ...
  int16_t x = gsAssignmentBoxesX;
  int16_t y = gsAssignmentBoxesY;
...
static void RepositionMouseRegions() {
  int16_t const sDeltaX = gsAssignmentBoxesX - gAssignmentMenuRegion[0].RegionTopLeftX;
  int16_t const sDeltaY = gsAssignmentBoxesY - gAssignmentMenuRegion[0].RegionTopLeftY +
...
static void AdjustBoxPos(SGPBox const &assignment_area, PopUpBox *const other_box,
                         int32_t const offset_line) {
  SGPBox const &other_area = GetBoxArea(other_box);

  int16_t const max_x = SCREEN_WIDTH - assignment_area.w - other_area.w;
  if (gsAssignmentBoxesX > max_x) {
    gsAssignmentBoxesX = max_x;
    SetRenderFlags(RENDER_FLAG_FULL);
  }

  int16_t const dy = (GetFontHeight(MAP_SCREEN_FONT) + 2) * offset_line;
  int16_t const ah = assignment_area.h;
  int16_t const oh = other_area.h + dy;
  int16_t const max_y = gsVIEWPORT_WINDOW_END_Y - (ah > oh ? ah : oh);
  if (gsAssignmentBoxesY > max_y) {
    gsAssignmentBoxesY = max_y;
    SetRenderFlags(RENDER_FLAG_FULL);
  }

  int16_t const x = gsAssignmentBoxesX + assignment_area.w;
  int16_t const y = gsAssignmentBoxesY + dy;
...
static void CheckAndUpdateTacticalAssignmentPopUpPositions() {
  ...
  if (fShowRepairMenu) {
    ...
    int16_t const max_x = SCREEN_WIDTH - assignment_area.w - train_area.w - attr_area.w;
    if (gsAssignmentBoxesX > max_x) {
      gsAssignmentBoxesX = max_x;
      SetRenderFlags(RENDER_FLAG_FULL);
    }

    int16_t const dy = (GetFontHeight(MAP_SCREEN_FONT) + 2) * ASSIGN_MENU_TRAIN;
    int16_t const max_y = gsVIEWPORT_WINDOW_END_Y - attr_area.h - dy;
    if (gsAssignmentBoxesY > max_y) {
      gsAssignmentBoxesY = max_y;
      SetRenderFlags(RENDER_FLAG_FULL);
    }
    ...
    int16_t const max_x = SCREEN_WIDTH - assignment_area.w;
    if (gsAssignmentBoxesX > max_x) {
      gsAssignmentBoxesX = max_x;
      SetRenderFlags(RENDER_FLAG_FULL);
    }

    int16_t const max_y = gsVIEWPORT_WINDOW_END_Y - assignment_area.h;
    if (gsAssignmentBoxesY > max_y) {
      gsAssignmentBoxesY = max_y;
      SetRenderFlags(RENDER_FLAG_FULL);
    }
...
static void PositionCursorForTacticalAssignmentBox() {
  ...
  int32_t const h = GetLineSpace(box) + GetFontHeight(GetBoxFont(box));
  SGPBox const &area = GetBoxArea(box);
...
static void HandleRestFatigueAndSleepStatus() {
  { /* Run through all player characters and handle their rest, fatigue, and
    ...
    uint16_t sleep_quote = QUOTE_NEED_SLEEP;
    FOR_EACH_IN_TEAM(i, OUR_TEAM) {
      SOLDIERTYPE &s = *i;
      if (IsMechanical(s)) continue;
      ...
      if (CharacterIsTakingItEasy(s)) {  // Let them rest some
        RestCharacter(&s);
      } else {  // Wear them down
        FatigueCharacter(s);
      }
      ...
      if (s.bBreathMax <= BREATHMAX_ABSOLUTE_MINIMUM) {
        ...
        /* He goes to sleep, provided it's at all possible (it still won't
         * happen in a hostile sector, etc.) */
        if (!SetMercAsleep(s, false)) continue;

        if (s.bAssignment < ON_DUTY ||
            s.bAssignment == VEHICLE) {  // On a squad/vehicle, complain, then drop
        ...
        s.fMercCollapsedFlag = TRUE;
                 ...
                 !s.fForcedToStayAwake) {  // Pretty tired, and not forced to
                                           // stay awake
        if (s.bAssignment >= ON_DUTY && s.bAssignment != VEHICLE) {  // Not on squad/in vehicle
          ...
          // If the first one
          if (!reason_added) {  // Tell player about it
  ...
  { /* Now handle waking. Needs seperate list queue, that's why it has its own
    ...
    FOR_EACH_IN_TEAM(i, OUR_TEAM) {
      ...
      if (s.bAssignment == ASSIGNMENT_POW) continue;
      if (s.bAssignment == IN_TRANSIT) continue;
      // Guys between sectors CAN wake up while between sectors (sleeping in
      // vehicle)

      if (s.bBreathMax >= BREATHMAX_CANCEL_COLLAPSE) {  // Reset the collapsed flag well before
                                                        // reaching the wakeup state
...
static BOOLEAN IsRobotInThisSector(int16_t const sSectorX, int16_t const sSectorY,
                                   int8_t const bSectorZ) {
  SOLDIERTYPE const *const s = GetRobotSoldier();
  return s && s->sSectorX == sSectorX && s->sSectorY == sSectorY && s->bSectorZ == bSectorZ &&
         !s->fBetweenSectors;
}
...
static BOOLEAN CanCharacterRepairRobot(SOLDIERTYPE const *const pSoldier) {
  ...
  /* Assignment distance limits removed.  Sep/11/98.  ARM
          ...
          if( ( pSoldier -> sSectorX == gWorldSectorX ) && ( pSoldier ->
     sSectorY == gWorldSectorY ) && ( pSoldier -> bSectorZ == gbWorldSectorZ ) )
          {
...
static uint8_t RepairRobot(SOLDIERTYPE *pRobot, uint8_t ubRepairPts,
                           BOOLEAN *pfNothingLeftToRepair);

static uint8_t HandleRepairOfRobotBySoldier(uint8_t const ubRepairPts,
                                            BOOLEAN *const pfNothingLeftToRepair) {
  SOLDIERTYPE *pRobot = NULL;
...
static void PreSetAssignment(SOLDIERTYPE &s, int8_t const assignment) {
  PreChangeAssignment(s);
  fTeamPanelDirty = TRUE;
  fMapScreenBottomDirty = TRUE;
  gfRenderPBInterface = TRUE;
...
static void PostSetAssignment(SOLDIERTYPE &s, int8_t const assignment) {
...
static void SetSoldierAssignmentTrainTown(SOLDIERTYPE &s) {
  ...
  if (!pMilitiaTrainerSoldier && !SectorInfo[SECTOR(s.sSectorX, s.sSectorY)].fMilitiaTrainingPaid) {
    // show a message to confirm player wants to charge cost
...
static void SetSoldierAssignmentTrainSelf(SOLDIERTYPE &s, int8_t const stat) {
...
static void SetSoldierAssignmentTrainTeammate(SOLDIERTYPE &s, int8_t const stat) {
...
static void SetSoldierAssignmentTrainByOther(SOLDIERTYPE &s, int8_t const stat) {
  if (!CanCharacterTrainStat(&s, stat, TRUE, FALSE)) return;
...
void SetSoldierAssignmentRepair(SOLDIERTYPE &s, BOOLEAN const sam, BOOLEAN const robot,
                                int8_t const vehicle_id) {
  if (!CanCharacterRepair(&s)) return;
  PreSetAssignment(s, REPAIR);
  if (s.bAssignment != REPAIR || s.fFixingSAMSite != sam || s.fFixingRobot != robot ||
...
/* No point in allowing SAM site repair any more.  Jan/13/99.  ARM
BOOLEAN CanSoldierRepairSAM( SOLDIERTYPE *pSoldier, int8_t bRepairPoints)
{
        ...
        //can it be fixed?
        if (!IsTheSAMSiteInSectorRepairable(pSoldier->sSectorX,
pSoldier->sSectorY, pSoldier->bSectorZ))
        {
                return( FALSE );
        }

        // is he good enough?  (Because of the division of repair pts in SAM
repair formula, a guy with any less that this
...
sSectorZ )
{
        ...
        for( iCounter = 0; iCounter < NUMBER_OF_SAMS; iCounter++ )
        {
                ...
                {
                        bSAMCondition = StrategicMap[ CALCULATE_STRATEGIC_INDEX(
sSectorX, sSectorY ) ].bSAMCondition;

                        if( ( bSAMCondition < 100 ) && ( bSAMCondition >=
MIN_CONDITION_TO_FIX_SAM ) )
                        {
                                return( TRUE );
                        }
...
BOOLEAN SoldierInSameSectorAsSAM( SOLDIERTYPE *pSoldier )
{
        ...
        for( iCounter = 0; iCounter < NUMBER_OF_SAMS; iCounter++ )
        {
                if( pSamList[ iCounter] == SECTOR( pSoldier -> sSectorX,
pSoldier -> sSectorY ) )
                {
                        return( TRUE );
                }
...
pSoldier -> sSectorY ) )
                {
// Assignment distance limits removed.  Sep/11/98.  ARM
//			if( ( PythSpacesAway( pSamGridNoAList[ iCounter ],
pSoldier -> sGridNo ) < MAX_DISTANCE_FOR_REPAIR )||( PythSpacesAway(
pSamGridNoBList[ iCounter ], pSoldier -> sGridNo ) < MAX_DISTANCE_FOR_REPAIR ) )
                        {
                                return( TRUE );
                        }
                }
...
static void HandleShadingOfLinesForTrainingMenu() {
  ...
  PopUpShade const shade = !BasicCanCharacterTrainMilitia(&s) ? POPUP_SHADE
...
static void HandleShadingOfLinesForAttributeMenus() {
  ...
  SOLDIERTYPE const &s = *GetSelectedAssignSoldier(FALSE);
  for (int8_t stat = 0; stat < ATTRIB_MENU_CANCEL; ++stat) {
...
static BOOLEAN ValidTrainingPartnerInSameSectorOnAssignmentFound(SOLDIERTYPE *pTargetSoldier,
...
static void ReportTrainersTraineesWithoutPartners() {
  ...
  // check each trainee
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s->bAssignment == TRAIN_BY_OTHER && s->bLife > 0 &&
...
void PutMercInAsleepState(SOLDIERTYPE &s) {
  ...
  if (gfWorldLoaded && s.bInSector) {
    uint16_t const state = guiCurrentScreen == GAME_SCREEN ? GOTO_SLEEP : SLEEPING;
...
BOOLEAN PutMercInAwakeState(SOLDIERTYPE *pSoldier) {
  if (pSoldier->fMercAsleep) {
    if ((gfWorldLoaded) && (pSoldier->bInSector)) {
      const uint16_t state = (guiCurrentScreen == GAME_SCREEN ? WKAEUP_FROM_SLEEP : STANDING);
      ChangeSoldierState(pSoldier, state, 1, TRUE);
    }
...
void SetTimeOfAssignmentChangeForMerc(SOLDIERTYPE *pSoldier) {
  ...
  // assigning new PATIENTs gives a DOCTOR something to do, etc., so set flag to
  // recheck them all. CAN'T DO IT RIGHT AWAY IN HERE 'CAUSE WE TYPICALLY GET
  // CALLED *BEFORE* bAssignment GETS SET TO NEW VALUE!!
  gfReEvaluateEveryonesNothingToDo = TRUE;
}

BOOLEAN AnyMercInGroupCantContinueMoving(GROUP const &g) {
  BOOLEAN group_must_stop = FALSE;
  uint16_t quote = QUOTE_NEED_SLEEP;
...
bool PlayerSoldierTooTiredToTravel(SOLDIERTYPE &s) {
  ...
  if (s.fMercAsleep) {  // Asleep, and can't be awakened?
    if (!CanCharacterBeAwakened(&s, FALSE)) return true;
  } else {  // If awake, but so tired they can't move/drive anymore
...
static bool AssignMercToAMovementGroup(SOLDIERTYPE &s) {
  // If merc doesn't have a group or is in a vehicle or on a squad assign to
  // group
...
static void NotifyPlayerOfAssignmentAttemptFailure(int8_t bAssignment) {
  // notify player
...
BOOLEAN HandleSelectedMercsBeingPutAsleep(BOOLEAN const wake_up, BOOLEAN const display_warning) {
  ...
  CFOR_EACH_SELECTED_IN_CHAR_LIST(c) {
    // if the current character in the list is valid...then grab soldier pointer
    // for the character
  ...
  // if there was anyone processed, check for success and inform player of
  // failure
  if (!success && display_warning) {
    // inform player not everyone could be woken up/put to sleep
    wchar_t const *const warning = wake_up ? pMapErrorString[27] : pMapErrorString[26];
...
void BandageBleedingDyingPatientsBeingTreated() {
        ...
        } else  // assigned to DOCTOR/PATIENT
        {
          ...
          if (pDoctor != NULL) {
            ...
            if (iKitSlot != NO_SLOT) {
              ...
              if (usKitPts) {
                uiKitPtsUsed =
                    VirtualSoldierDressWound(pDoctor, pSoldier, &kit, usKitPts, usKitPts);
                UseKitPoints(kit, (uint16_t)uiKitPtsUsed, *pDoctor);
  ...
  // this event may be posted many times because of multiple assignment changes.
  // Handle it only once per minute!
...
void ReEvaluateEveryonesNothingToDo() {
    ...
    switch (pSoldier->bAssignment) {
      ...
      case TRAIN_TEAMMATE:
        fNothingToDo = !CanCharacterTrainStat(pSoldier, pSoldier->bTrainStat, FALSE, TRUE) ||
                       !ValidTrainingPartnerInSameSectorOnAssignmentFound(pSoldier, TRAIN_BY_OTHER,
                                                                          pSoldier->bTrainStat);
        break;

      case TRAIN_BY_OTHER:
        fNothingToDo = !CanCharacterTrainStat(pSoldier, pSoldier->bTrainStat, TRUE, FALSE) ||
                       !ValidTrainingPartnerInSameSectorOnAssignmentFound(pSoldier, TRAIN_TEAMMATE,
                                                                          pSoldier->bTrainStat);
        break;
  ...
  // redraw the map, in case we're showing teams, and someone just came on duty
  // or off duty, their icon needs updating
  fMapPanelDirty = TRUE;
}

void SetAssignmentForList(int8_t const bAssignment, int8_t const bParam) {
  // if not in mapscreen, there is no functionality available to change multiple
  // assignments simultaneously!
  if (!fInMapMode) return;

  SOLDIERTYPE *const sel =
      bSelectedAssignChar != -1 ? gCharactersList[bSelectedAssignChar].merc : NULL;
    ...
    switch (bAssignment) {
      ...
      case REPAIR:
        if (CanCharacterRepair(&s)) {
          // make sure he can repair the SPECIFIC thing being repaired too (must
          // be in its sector, for example)
          BOOLEAN const fCanFixSpecificTarget =
#if 0
						sel->fFixingSAMSite              ? CanSoldierRepairSAM(&s, SAM_SITE_REPAIR_DIVISOR)                     :
#endif
              sel->bVehicleUnderRepairID != -1
                  ? CanCharacterRepairVehicle(s, GetVehicle(sel->bVehicleUnderRepairID))
              : s.fFixingRobot ? CanCharacterRepairRobot(&s)
                               :  // XXX s in condition seems wrong, should probably be sel
                  TRUE;
          if (fCanFixSpecificTarget) {
            SetSoldierAssignmentRepair(s, sel->fFixingSAMSite, sel->fFixingRobot,
                                       sel->bVehicleUnderRepairID);
            fItWorked = TRUE;
          }
        }
        break;
      ...
      case SQUAD_19:
      case SQUAD_20:
        switch (CanCharacterSquad(s, (int8_t)bAssignment)) {
          case CHARACTER_CAN_JOIN_SQUAD: {
            ...
            if (t && t->fBetweenSectors && s.bAssignment >= ON_DUTY && s.ubGroupID != 0) {
  ...
  // check if we should start/stop flashing any mercs' assignment strings after
  // these changes
  gfReEvaluateEveryonesNothingToDo = TRUE;
}
...
static BOOLEAN ValidTrainingPartnerInSameSectorOnAssignmentFound(SOLDIERTYPE *pTargetSoldier,
                                                                 int8_t bTargetAssignment,
                                                                 int8_t bTargetStat) {
  int16_t sTrainingPts = 0;
  BOOLEAN fAtGunRange = FALSE;
  uint16_t usMaxPts;

  // this function only makes sense for training teammates or by others, not for
  // self training which doesn't require partners
  Assert((bTargetAssignment == TRAIN_TEAMMATE) || (bTargetAssignment == TRAIN_BY_OTHER));

  CFOR_EACH_IN_TEAM(pSoldier, OUR_TEAM) {
    // if the guy is not the target, has the assignment we want, is training the
    // same stat, and is in our sector, alive and is training the stat we want
    if (pSoldier != pTargetSoldier && pSoldier->bAssignment == bTargetAssignment &&
        // CJC: this seems incorrect in light of the check for bTargetStat and
        // in any case would cause a problem if the trainer was assigned and we
        // weren't!
        ...
        pSoldier->bSectorZ == pTargetSoldier->bSectorZ && pSoldier->bTrainStat == bTargetStat &&
        pSoldier->bLife > 0) {
      // so far so good, now let's see if the trainer can really teach the
      // student anything new
...
void UnEscortEPC(SOLDIERTYPE *const s) {
  if (fInMapMode) {
    ...
    switch (s->ubProfile) {
      case JOHN:
        other = FindSoldierByProfileIDOnPlayerTeam(MARY);
        break;
      case MARY:
        other = FindSoldierByProfileIDOnPlayerTeam(JOHN);
        break;
      default:
        other = NULL;
        break;
...
static uint8_t CalcSoldierNeedForSleep(SOLDIERTYPE const &s) {
  // Base comes from profile
  uint8_t need_for_sleep = GetProfile(s.ubProfile).ubNeedForSleep;

  /* < 1/4 health -> 4 more
   * < 2/4 health -> 2 more
   * < 3/4 health -> 1 more */
  uint8_t const part_health = 4 * s.bLife / s.bLifeMax;
...
static BOOLEAN CanCharacterRepairAnotherSoldiersStuff(const SOLDIERTYPE *const pSoldier,
                                                      const SOLDIERTYPE *const pOtherSoldier) {
  ...
  if (pOtherSoldier->bAssignment == IN_TRANSIT || pOtherSoldier->bAssignment == ASSIGNMENT_POW ||
      pOtherSoldier->bAssignment == ASSIGNMENT_DEAD || IsMechanical(*pSoldier) ||
...
static void RepairItemsOnOthers(SOLDIERTYPE *pSoldier, uint8_t *pubRepairPtsLeft) {
  ...
  // repair everyone's hands and armor slots first, then headgear, and pockets
  // last
  for (ubPassType = REPAIR_HANDS_AND_ARMOR; ubPassType <= FINAL_REPAIR_PASS; ubPassType++) {
    ...
    FOR_EACH_IN_TEAM(pOtherSoldier, OUR_TEAM) {
      // check character is valid, alive, same sector, not between, has
      // inventory, etc.
      if (CanCharacterRepairAnotherSoldiersStuff(pSoldier, pOtherSoldier)) {
    ...
    while (*pubRepairPtsLeft > 0) {
      ...
      if (pBestOtherSoldier != NULL) {
        // yes, repair all items (for this pass type!) on this soldier that need
        // repair
        do {
...
static BOOLEAN UnjamGunsOnSoldier(SOLDIERTYPE *pOwnerSoldier, SOLDIERTYPE *pRepairSoldier,
  ...
  // try to unjam everything before beginning any actual repairs.. successful
  // unjamming costs 2 points per weapon
  for (bPocket = HANDPOS; bPocket <= SMALLPOCK8POS; bPocket++) {
    // the object a weapon? and jammed?
      ...
      if (*pubRepairPtsLeft >= REPAIR_COST_PER_JAM) {
        *pubRepairPtsLeft -= REPAIR_COST_PER_JAM;
        ...
        if (pRepairSoldier == pOwnerSoldier) {
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, gzLateLocalizedString[STR_LATE_53],
                    pRepairSoldier->name, ItemNames[pOwnerSoldier->inv[bPocket].usItem]);
        } else {
          // NOTE: may need to be changed for localized versions
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, gzLateLocalizedString[STR_LATE_54],
