...
#define QUEST_DBS_SELECTED_ITEM_BUTN_X \
  QUEST_DBS_SELECTED_NPC_BUTN_X  // QUEST_DBS_FIRST_SECTION_WIDTH +
                                 // QUEST_DBS_SECOND_SECTION_WIDTH + 105
...
static const wchar_t *const QuestStates[] = {L"N.S.", L"In Prog.", L"Done"};
...
struct SCROLL_BOX {
  ...
  uint8_t ubCurScrollBoxAction;  // Holds the status of the current action (
                                 // create; destroy... )
};
...
static uint16_t gusQuestDebugBlue;

static SCROLL_BOX gNpcListBox;  // The Npc Scroll box
static SCROLL_BOX gItemListBox;

static SCROLL_BOX *gpActiveListBox;  // Only 1 scroll box is active at a time,
                                     // this is set to it.

int16_t gsQdsEnteringGridNo = 0;

static uint8_t gubTextEntryAction = QD_DROP_DOWN_NO_ACTION;
static BOOLEAN gfTextEntryActive = FALSE;

static BOOLEAN gfUseLocalNPCs = FALSE;

static uint8_t gubNPCInventoryPopupAction = QD_DROP_DOWN_NO_ACTION;

static uint8_t gubCurrentNpcInSector[QUEST_DBS_SIZE_NPC_ARRAY];
static uint8_t gubNumNPCinSector;

static uint8_t gubCurQuestSelected;
static uint16_t gusCurFactSelected;
static uint16_t gusFactAtTopOfList;

static BOOLEAN gfNpcLogButton = FALSE;

static int32_t giHaveSelectedItem =
    -1;  // If it is not the first time in, dont reset the Selected ITem
static int32_t giHaveSelectedNPC =
    -1;  // If it is not the first time in, dont reset the selected NPC

static int32_t giSelectedMercCurrentQuote = -1;
...
static void DisplaySelectedItem();
static void DisplaySelectedNPC();
...
static void CreateDestroyDisplayTextEntryBox(uint8_t ubAction, const wchar_t *pString,
                                             TEXT_ENTRY_CALLBACK EntryCallBack);
static void EnterQuestDebugSystem();
static void ExitQuestDebugSystem();
static void GetUserInput();
static void HandleQuestDebugSystem();
static void RenderQuestDebugSystem();
static uint8_t WhichPanelShouldTalkingMercUse();
...
static GUIButtonRef MakeButton(const wchar_t *text, int16_t x, int16_t y, int16_t w, int16_t h,
                               int16_t priority, GUI_CALLBACK click) {
  return CreateTextButton(text, QUEST_DBS_FONT_STATIC_TEXT, QUEST_DBS_COLOR_STATIC_TEXT, FONT_BLACK,
                          x, y, w, h, priority, click);
}
...
static GUIButtonRef MakeCheckBox(int16_t const x, int16_t const y, GUI_CALLBACK const click,
  ...
  GUIButtonRef const b =
      CreateCheckBoxButton(x, y, INTERFACEDIR "/checkbox.sti", MSYS_PRIORITY_HIGH + 2, click);
  if (checked) b->uiFlags |= BUTTON_CLICKED_ON;
  return b;
}

static void AddNPCsInSectorToArray();
static void BtnQDPgDownButtonButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQDPgUpButtonButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugAddItemToLocationButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugAddNpcToTeamToggleCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugAllOrSectorNPCToggleCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugChangeDayButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugCurItemButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugCurNPCButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugAddNpcToLocationButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugExitButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugGiveItemToNPCButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugNPCLogButtonButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugNPCRefreshButtonButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugRPCSaySectorDescToggleCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugRestoreNPCInvButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugStartMercTalkingButtonButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void BtnQuestDebugViewNPCInvButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void EnableQDSButtons();
static void ScrollFactListRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void ScrollQuestListRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);

static void EnterQuestDebugSystem() {
  uint8_t i;
  uint16_t usPosX, usPosY;
  wchar_t zName[128];
  //	uint16_t	usListBoxFontHeight = GetFontHeight( QUEST_DBS_FONT_LISTBOX_TEXT
  //) + 2;
  ...
  MSYS_DefineRegion(&gQuestDebugSysScreenRegions, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
                    MSYS_PRIORITY_HIGH, CURSOR_LAPTOP_SCREEN, MSYS_NO_CALLBACK, MSYS_NO_CALLBACK);

  guiQuestDebugExitButton =
      MakeButton(QuestDebugText[QUEST_DBS_EXIT_QUEST_DEBUG], 535, 450, 100, 25,
                 MSYS_PRIORITY_HIGH + 10, BtnQuestDebugExitButtonCallback);

  // Check box to toggle between all and local npc's
  guiQuestDebugAllOrSectorNPCToggle =
      MakeCheckBox(QUEST_DBS_NPC_CHCKBOX_TGL_X, QUEST_DBS_NPC_CHCKBOX_TGL_Y,
                   BtnQuestDebugAllOrSectorNPCToggleCallback);

  // Currently Selected NPC button
  guiQuestDebugCurNPCButton =
      MakeButton(QuestDebugText[QUEST_DBS_SELECTED_NPC], QUEST_DBS_SELECTED_NPC_BUTN_X,
                 QUEST_DBS_SELECTED_NPC_BUTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugCurNPCButtonCallback);

  // Currently Selected item button
  guiQuestDebugCurItemButton =
      MakeButton(QuestDebugText[QUEST_DBS_SELECTED_ITEM], QUEST_DBS_SELECTED_ITEM_BUTN_X,
                 QUEST_DBS_SELECTED_ITEM_BUTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugCurItemButtonCallback);

  // Add NPC to location
  guiQuestDebugAddNpcToLocationButton =
      MakeButton(QuestDebugText[QUEST_DBS_ADD_CURRENT_NPC], QUEST_DBS_ADD_NPC_BTN_X,
                 QUEST_DBS_ADD_NPC_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugAddNpcToLocationButtonCallback);

  // Add item to location
  guiQuestDebugAddItemToLocationButton =
      MakeButton(QuestDebugText[QUEST_DBS_ADD_CURRENT_ITEM], QUEST_DBS_ADD_ITEM_BTN_X,
                 QUEST_DBS_ADD_ITEM_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugAddItemToLocationButtonCallback);

  // Give item to Npc
  guiQuestDebugGiveItemToNPCButton =
      MakeButton(QuestDebugText[QUEST_DBS_GIVE_ITEM_TO_NPC], QUEST_DBS_GIVE_ITEM_TO_NPC_BTN_X,
                 QUEST_DBS_GIVE_ITEM_TO_NPC_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugGiveItemToNPCButtonCallback);

  // Change Day
  guiQuestDebugChangeDayButton =
      MakeButton(QuestDebugText[QUEST_DBS_CHANGE_DAY], QUEST_DBS_CHANGE_DAY_BTN_X,
                 QUEST_DBS_CHANGE_DAY_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugChangeDayButtonCallback);

  // View NPC Inventory
  guiQuestDebugViewNPCInvButton =
      MakeButton(QuestDebugText[QUEST_DBS_VIEW_NPC_INVENTORY], QUEST_DBS_VIEW_NPC_INV_BTN_X,
                 QUEST_DBS_VIEW_NPC_INV_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugViewNPCInvButtonCallback);

  // Restore NPC Inventory
  guiQuestDebugRestoreNPCInvButton =
      MakeButton(QuestDebugText[QUEST_DBS_RESTORE_NPC_INVENTORY], QUEST_DBS_RESTORE_NPC_INV_BTN_X,
                 QUEST_DBS_RESTORE_NPC_INV_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugRestoreNPCInvButtonCallback);

  // NPC log button
  swprintf(zName, lengthof(zName), L"%ls - (%ls)", QuestDebugText[QUEST_DBS_NPC_LOG_BUTTON],
           gfNpcLogButton ? L"On" : L"Off");
  guiQuestDebugNPCLogButtonButton = MakeButton(
      zName, QUEST_DBS_NPC_LOG_BTN_X, QUEST_DBS_NPC_LOG_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH,
      QDS_BUTTON_HEIGHT, MSYS_PRIORITY_HIGH + 2, BtnQuestDebugNPCLogButtonButtonCallback);

  guiQuestDebugNPCRefreshButtonButton =
      MakeButton(QuestDebugText[QUEST_DBS_REFRESH_NPC], QUEST_DBS_NPC_REFRESH_BTN_X,
                 QUEST_DBS_NPC_REFRESH_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugNPCRefreshButtonButtonCallback);

  // Start the selected merc talking
  guiQuestDebugStartMercTalkingButtonButton =
      MakeButton(QuestDebugText[QUEST_DBS_START_MERC_TALKING], QUEST_DBS_START_MERC_TALKING_BTN_X,
                 QUEST_DBS_START_MERC_TALKING_BTN_Y, QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT,
                 MSYS_PRIORITY_HIGH + 2, BtnQuestDebugStartMercTalkingButtonButtonCallback);

  guiQDPgUpButtonButton = MakeButton(
      QuestDebugText[QUEST_DBS_PG_FACTS_UP], QUEST_DBS_SECOND_COL_NUMBER_X + 5,
      QUEST_DBS_SECOND_COL_NUMBER_Y + QUEST_DBS_LIST_TEXT_OFFSET, QUEST_DBS_LIST_BOX_WIDTH,
      QDS_BUTTON_HEIGHT, MSYS_PRIORITY_HIGH + 2, BtnQDPgUpButtonButtonCallback);

  guiQDPgDownButtonButton =
      MakeButton(QuestDebugText[QUEST_DBS_PG_FACTS_DOWN], QUEST_DBS_SECOND_COL_NUMBER_X + 5,
                 QUEST_DBS_SECOND_COL_NUMBER_Y + 15 * QUEST_DBS_NUM_DISPLAYED_FACTS +
                     QUEST_DBS_LIST_TEXT_OFFSET,
                 QUEST_DBS_LIST_BOX_WIDTH, QDS_BUTTON_HEIGHT, MSYS_PRIORITY_HIGH + 2,
                 BtnQDPgDownButtonButtonCallback);

  // checkbox for weather to add the merc to the players team
  guiQuestDebugAddNpcToTeamToggle =
      MakeCheckBox(QUEST_DBS_ADD_NPC_TO_TEAM_BTN_X, QUEST_DBS_ADD_NPC_TO_TEAM_BTN_Y,
                   BtnQuestDebugAddNpcToTeamToggleCallback, gfAddNpcToTeam);

  // checkbox for weather have rpc say the sector description
  guiQuestDebugRPCSaySectorDescToggle =
      MakeCheckBox(QUEST_DBS_RPC_TO_SAY_SECTOR_DESC_BTN_X, QUEST_DBS_RPC_TO_SAY_SECTOR_DESC_BTN_Y,
                   BtnQuestDebugRPCSaySectorDescToggleCallback, gfRpcToSaySectorDesc);
  ...
  for (i = 0; i < QUEST_DBS_NUM_DISPLAYED_QUESTS; i++) {
    MSYS_DefineRegion(&gQuestListRegion[i], usPosX, usPosY, usPosX + QUEST_DBS_FIRST_SECTION_WIDTH,
                      usPosY + usFontHeight, MSYS_PRIORITY_HIGH + 2, CURSOR_WWW, MSYS_NO_CALLBACK,
                      ScrollQuestListRegionCallBack);  // CURSOR_LAPTOP_SCREEN
  ...
  for (i = 0; i < QUEST_DBS_NUM_DISPLAYED_FACTS; i++) {
    MSYS_DefineRegion(&gFactListRegion[i], usPosX, usPosY, usPosX + QUEST_DBS_SECOND_SECTION_WIDTH,
                      usPosY + usFontHeight, MSYS_PRIORITY_HIGH + 2, CURSOR_WWW, MSYS_NO_CALLBACK,
                      ScrollFactListRegionCallBack);  // CURSOR_LAPTOP_SCREEN
  ...
  if (giHaveSelectedNPC != -1) {
    const ProfileID pid =
        (gfUseLocalNPCs ? gubCurrentNpcInSector[giHaveSelectedNPC] : giHaveSelectedNPC);
  ...
  if (giHaveSelectedItem != -1) {
    ...
    swprintf(zItemDesc, lengthof(zItemDesc), L"%d - %ls", giHaveSelectedItem,
             ShortItemNames[giHaveSelectedItem]);
...
static BOOLEAN CreateDestroyDisplaySelectNpcDropDownBox();
static void EndMercTalking();
static void QuestDebug_EnterTactical();
...
static void AddKeyToGridNo(int32_t iKeyID);
static void CreateDestroyDisplayNPCInventoryPopup(uint8_t ubAction);
static void HandleQDSTalkingMerc();
...
static void DisplayCurrentGridNo();
static void DisplayFactInformation();
static void DisplayFactList();
static void DisplayNPCInfo();
static void DisplayQDSCurrentlyQuoteNum();
static void DisplayQuestInformation();
static void DisplayQuestList();
static void DisplaySectionLine();

static void RenderQuestDebugSystem() {
  ButtonDestBuffer->Fill(gusQuestDebugBlue);

  // display the title
  DisplayWrappedString(0, 5, SCREEN_WIDTH, 2, QUEST_DBS_FONT_TITLE, QUEST_DBS_COLOR_TITLE,
                       QuestDebugText[QUEST_DBS_TITLE], FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
...
static void DisplayCurrentGridNo() {
  if (gsQdsEnteringGridNo != 0) {
    ...
    swprintf(zTemp, lengthof(zTemp), L"%ls:  %d", QuestDebugText[QUEST_DBS_CURRENT_GRIDNO],
             gsQdsEnteringGridNo);
    DrawTextToScreen(zTemp, QUEST_DBS_NPC_CURRENT_GRIDNO_X, QUEST_DBS_NPC_CURRENT_GRIDNO_Y,
                     ...
                     QUEST_DBS_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  }
}

static FACETYPE *GetQDSFace(const uint8_t panel_merc_should_use) {
  return panel_merc_should_use == QDS_REGULAR_PANEL ? gTalkingMercSoldier->face : gTalkPanel.face;
}

static void DoQDSMessageBox(wchar_t const *zString, ScreenID uiExitScreen, MessageBoxFlags,
                            MSGBOX_CALLBACK ReturnCallback);
static void IncrementActiveDropDownBox(int16_t sIncrementValue);
    ...
    if (!HandleTextInput(&Event) && Event.usEvent == KEY_DOWN) {
      switch (Event.usParam) {
        ...
        case 'd': {
          ...
          swprintf(zTemp, lengthof(zTemp), L"Items dropped will be in %ls condition",
...
static void QuestDebug_EnterTactical() { EnterTacticalScreen(); }
...
static void DisplayQuestInformation() {
  ...
  DisplayWrappedString(0, QUEST_DBS_SECTION_TITLE_Y, QUEST_DBS_FIRST_SECTION_WIDTH, 2,
                       ...
                       QuestDebugText[QUEST_DBS_QUESTS], FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
...
static void DisplayQuestList() {
  ...
  usPosY = QUEST_DBS_FIRST_COL_NUMBER_Y +
           QUEST_DBS_LIST_TEXT_OFFSET;  //&& (usCount <
  ...
  for (usLoop1 = 0, usCount = 0; (usLoop1 < MAX_QUESTS); usLoop1++) {
    ...
    DisplayWrappedString(QUEST_DBS_FIRST_COL_TITLE_X, usPosY, QUEST_DBS_TITLE_COL_WIDTH, 2,
                         ...
                         QuestDescText[usLoop1], FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);

    // Display Quest status text
    DisplayWrappedString(QUEST_DBS_FIRST_COL_STATUS_X, usPosY, QUEST_DBS_STATUS_COL_WIDTH, 2,
                         QUEST_DBS_FONT_STATIC_TEXT, QUEST_DBS_COLOR_STATIC_TEXT,
                         QuestStates[gubQuest[usLoop1]], FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
...
static void DisplayFactList() {
  ...
  for (usLoop1 = gusFactAtTopOfList, usCount = 0;
    ...
    if (FactDescText[usLoop1][0] == '\0') {
      ...
      ReduceStringLength(sTemp, lengthof(sTemp), QUEST_DBS_SECOND_TITLE_COL_WIDTH,
                         QUEST_DBS_FONT_DYNAMIC_TEXT);
      //			DisplayWrappedString(QUEST_DBS_SECOND_COL_TITLE_X,
      // usPosY, QUEST_DBS_SECOND_TITLE_COL_WIDTH, 2,
      // QUEST_DBS_FONT_DYNAMIC_TEXT, QUEST_DBS_COLOR_STATIC_TEXT,
      // FactDescText[usLoop1], FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
      DrawTextToScreen(sTemp, QUEST_DBS_SECOND_COL_TITLE_X, usPosY,
                       QUEST_DBS_SECOND_TITLE_COL_WIDTH, QUEST_DBS_FONT_DYNAMIC_TEXT,
                       QUEST_DBS_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
    }
...
static void DisplaySelectedListBox();
static void QuestDebugTextEntryDisableScreenRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void ScrollAreaMovementCallBack(MOUSE_REGION *pRegion, int32_t reason);
static void ScrollAreaRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void ScrollArrowsRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
static void SelectNpcListMovementCallBack(MOUSE_REGION *pRegion, int32_t reason);
static void SelectNpcListRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason);
  ...
  switch (gpActiveListBox->ubCurScrollBoxAction) {
    ...
    case QD_DROP_DOWN_CREATE: {
      ...
      for (i = 0; i < gpActiveListBox->usNumDisplayedItems; i++) {
        MSYS_DefineRegion(&gSelectedNpcListRegion[i], usPosX, usPosY,
                          usPosX + gpActiveListBox->usScrollWidth, usPosY + usFontHeight,
                          MSYS_PRIORITY_HIGH + 20, CURSOR_WWW, SelectNpcListMovementCallBack,
      ...
      if (!gfBackgroundMaskEnabled) {
        MSYS_DefineRegion(&gQuestTextEntryDebugDisableScreenRegion, 0, 0, SCREEN_WIDTH,
                          SCREEN_HEIGHT, MSYS_PRIORITY_HIGH + 15, CURSOR_LAPTOP_SCREEN,
                          MSYS_NO_CALLBACK, QuestDebugTextEntryDisableScreenRegionCallBack);
...
static void DrawQdsScrollRectangle();
...
static void DisplaySelectedListBox() {
  ...
  // Bottom arrow
  BltVideoObject(FRAME_BUFFER, guiQdScrollArrowImage, 1, usPosX,
...
static void GetDebugLocationString(uint16_t usProfileID, wchar_t *pzText, size_t Length);
...
static void DisplaySelectedNPC() {
  ...
  for (i = gpActiveListBox->usItemDisplayedOnTopOfList;
    ...
    DrawTextToScreen(p.zNickname, usPosX, usPosY, 0, QUEST_DBS_FONT_DYNAMIC_TEXT,
                     QUEST_DBS_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
    ...
    // the location value
    DrawTextToScreen(sTempString, usLocationX - 2, usPosY, 0, QUEST_DBS_FONT_DYNAMIC_TEXT,
                     QUEST_DBS_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
  ...
  if (gpActiveListBox->sCurSelectedItem >= 0) {
    ...
    ProfileID const pid = (gfUseLocalNPCs ? gubCurrentNpcInSector[gpActiveListBox->sCurSelectedItem]
                                          : gpActiveListBox->sCurSelectedItem);
    MERCPROFILESTRUCT const &p = GetProfile(pid);

    // the highlighted name
    DrawTextToScreen(p.zNickname, gpActiveListBox->usScrollPosX, usPosY, 0,
                     QUEST_DBS_FONT_LISTBOX_TEXT, 2, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
    GetDebugLocationString(pid, sTempString, lengthof(sTempString));
    ...
    // the location value
    DrawTextToScreen(sTempString, usLocationX, usPosY, 0, QUEST_DBS_FONT_LISTBOX_TEXT, 2,
...
static void DisplaySelectedItem() {
  ...
  for (i = gpActiveListBox->usItemDisplayedOnTopOfList;
    ...
    DrawTextToScreen(ItemName, usPosX, usPosY, 0, QUEST_DBS_FONT_DYNAMIC_TEXT,
                     QUEST_DBS_COLOR_DYNAMIC_TEXT, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
    usPosY += usFontHeight;
  }
  ...
  if (gpActiveListBox->sCurSelectedItem >= 0) {
    ...
    DrawTextToScreen(ItemName, gpActiveListBox->usScrollPosX, usPosY, 0,
                     QUEST_DBS_FONT_LISTBOX_TEXT, 2, FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
    SetFontShadow(DEFAULT_SHADOW);

    swprintf(zButtonName, lengthof(zButtonName), L"%d - %ls", gpActiveListBox->sCurSelectedItem,
             ItemName);
...
static void CalcPositionOfNewScrollBoxLocation();
...
static void CalcPositionOfNewScrollBoxLocation() {
  ...
  float dValue;
  int16_t sHeight = 0;
  //	int16_t	sHeightOfScrollBox = (int16_t)(gpActiveListBox->usScrollBarHeight
  /// (float)(gpActiveListBox->usMaxArrayIndex - gpActiveListBox->usStartIndex )
  //+ .5); int16_t	sHeightOfScrollBox =
  // (int16_t)(gpActiveListBox->usScrollBarHeight /
  // (float)(gpActiveListBox->usMaxArrayIndex ) + .5);
  ...
  if (sMouseYPos > sStartPosOfScrollArea ||
    ...
    //		sIncrementValue = (int16_t)( ( dValue ) * (
    // gpActiveListBox->usMaxArrayIndex - gpActiveListBox->usStartIndex ) + .5 );
    ...
    /*
       ...
       sIncrementValue ) <= 0 ) gpActiveListBox->usItemDisplayedOnTopOfList =
       gpActiveListBox->usStartIndex; else
                                    gpActiveListBox->usItemDisplayedOnTopOfList
       = sIncrementValue;
       ...
       gpActiveListBox->usMaxNumDisplayedItems ) )
                                    gpActiveListBox->usItemDisplayedOnTopOfList
       = gpActiveListBox->usMaxArrayIndex -
       gpActiveListBox->usMaxNumDisplayedItems; else
                                    gpActiveListBox->usItemDisplayedOnTopOfList
       = sIncrementValue;
                    }
  ...
  /*
          if( sMouseYPos < gpActiveListBox->usScrollBoxY )
          {
                  ...
                  if( gpActiveListBox->usNumDisplayedItems <
     gpActiveListBox->usMaxNumDisplayedItems )
                  {
     ...
     sIncrementValue;
          ...
          else if( sMouseYPos > ( gpActiveListBox->usScrollBoxY +
     sHeightOfScrollBox ) )//usScrollBoxEndY
          {
                  if( ( gpActiveListBox->sCurSelectedItem + 10 ) <
     gpActiveListBox->usMaxArrayIndex-1 ) sIncrementValue = 10; else
                          sIncrementValue = 1;
...
static void AddNPCToGridNo(int32_t iGridNo);

static void BtnQuestDebugAddNpcToLocationButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    wchar_t zTemp[512];
    swprintf(zTemp, lengthof(zTemp), L"%ls where %ls will be added.",
...
static void AddItemToGridNo(int32_t iGridNo);

static void BtnQuestDebugAddItemToLocationButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    wchar_t zTemp[512];
    swprintf(zTemp, lengthof(zTemp), L"%ls where the %ls will be added.",
             QuestDebugText[QUEST_DBS_ENTER_GRID_NUM],
             ShortItemNames[gItemListBox.sCurSelectedItem]);
...
static void BtnQuestDebugGiveItemToNPCButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    const ProfileID pid = (gfUseLocalNPCs ? gubCurrentNpcInSector[gNpcListBox.sCurSelectedItem]
...
static void ChangeDayNumber(int32_t iDayToChangeTo);

static void BtnQuestDebugChangeDayButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    wchar_t zTemp[512];
    swprintf(zTemp, lengthof(zTemp), L"%ls   Current Day is %d",
...
static void RefreshAllNPCInventory();
...
static void BtnQuestDebugNPCRefreshButtonButtonCallback(GUI_BUTTON *const btn,
                                                        int32_t const reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    bool success = false;
    uint8_t ubMercID = 0;
    if (gfUseLocalNPCs) {
    ...
    wchar_t const *const msg =
        success ? QuestDebugText[QUEST_DBS_REFRESH_OK] : QuestDebugText[QUEST_DBS_REFRESH_FAILED];
    wchar_t buf[128];
...
static void StartMercTalkingFromQuoteNum(int32_t iQuoteToStartTalkingFrom);

static void BtnQuestDebugStartMercTalkingButtonButtonCallback(GUI_BUTTON *btn, int32_t reason) {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    // Ask for the initial quote num to start talking from
    //		DoQDSMessageBox(zTemp, QUEST_DEBUG_SCREEN, MSG_BOX_FLAG_OK,
    // NULL);
...
static void BtnQuestDebugTextEntryOkBtnButtonCallback(GUI_BUTTON *btn, int32_t reason);
static void DestroyQuestDebugTextInputBoxes();
static void InitQuestDebugTextInputBoxes();
...
static void CreateDestroyDisplayTextEntryBox(uint8_t ubAction, const wchar_t *pString,
  ...
  switch (ubAction) {
    ...
    case QD_DROP_DOWN_CREATE: {
      ...
      if (!gfBackgroundMaskEnabled) {
        MSYS_DefineRegion(&gQuestTextEntryDebugDisableScreenRegion, 0, 0, SCREEN_WIDTH,
                          SCREEN_HEIGHT, MSYS_PRIORITY_HIGH + 40, CURSOR_LAPTOP_SCREEN,
                          MSYS_NO_CALLBACK, QuestDebugTextEntryDisableScreenRegionCallBack);
        gfBackgroundMaskEnabled = TRUE;
      }
      ...
      guiQuestDebugTextEntryOkBtn =
          MakeButton(L"OK", QUEST_DBS_TEB_X + QUEST_DBS_TEB_WIDTH / 2 - 12,
                     QUEST_DBS_TEB_Y + QUEST_DBS_TEB_HEIGHT - 30, 30, 25, MSYS_PRIORITY_HIGH + 50,
...
static void ChangeQuestState(int32_t iNumber);
...
static void ScrollQuestListRegionCallBack(MOUSE_REGION *pRegion, int32_t iReason) {
  if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    swprintf(String, lengthof(String), L"%ls %ls %d \"%ls\" ( %ls )",
...
static void ChangeFactState(int32_t iNumber);
...
static void DestroyQuestDebugTextInputBoxes() { KillTextInputMode(); }
...
static void BtnQuestDebugNPCInventOkBtnButtonCallback(GUI_BUTTON *btn, int32_t reason);
  ...
  switch (ubAction) {
    ...
    case QD_DROP_DOWN_CREATE: {
      ...
      guiQuestDebugNPCInventOkBtn =
          MakeButton(L"OK", QUEST_DBS_NPC_INV_POPUP_X + QUEST_DBS_NPC_INV_POPUP_WIDTH / 2 - 12,
                     QUEST_DBS_NPC_INV_POPUP_Y + QUEST_DBS_NPC_INV_POPUP_HEIGHT - 30, 30, 25,
                     MSYS_PRIORITY_HIGH + 50, BtnQuestDebugNPCInventOkBtnButtonCallback);
    ...
    case QD_DROP_DOWN_DISPLAY: {
      ...
      if (s) {
        ...
        DrawTextToScreen(QuestDebugText[QUEST_DBS_NPC_INVENTORY], QUEST_DBS_NPC_INV_POPUP_X,
                         ...
                         QUEST_DBS_FONT_TITLE, QUEST_DBS_COLOR_TITLE, FONT_MCOLOR_BLACK,
                         CENTER_JUSTIFIED);

        // Dispaly the current npc name
        DrawTextToScreen(GetProfile(pid).zNickname, QUEST_DBS_NPC_INV_POPUP_X,
                         QUEST_DBS_NPC_INV_POPUP_Y + 20, QUEST_DBS_NPC_INV_POPUP_WIDTH,
                         QUEST_DBS_FONT_TITLE, QUEST_DBS_COLOR_SUBTITLE, FONT_MCOLOR_BLACK,
                         CENTER_JUSTIFIED);
        ...
        for (i = 0; i < NUM_INV_SLOTS; i++) {
          ...
          DrawTextToScreen(ShortItemNames[s->inv[i].usItem], QUEST_DBS_NPC_INV_POPUP_X + 140,
                           usPosY, 0, QUEST_DBS_FONT_DYNAMIC_TEXT, QUEST_DBS_COLOR_DYNAMIC_TEXT,
                           FONT_MCOLOR_BLACK, LEFT_JUSTIFIED);
          usPosY += usFontHeight;
        }
      }
...
static int16_t IsMercInTheSector(uint16_t usMercID);
...
void NpcRecordLoggingInit(ProfileID const npc_id, ProfileID const merc_id, uint8_t const quote_id,
  ...
  try {
    ...
    AutoSGPFile f(firstTime ? FileMan::openForWriting(QUEST_DEBUG_FILE)
                            : FileMan::openForAppend(QUEST_DEBUG_FILE));
    firstTime = false;
    ...
    snprintf(buf, lengthof(buf),
             ...
             "\tTesting Record #: %d\n",
             npc_id, GetProfile(npc_id).zNickname, merc_id, GetProfile(merc_id).zNickname,
             quote_id);
...
static void EnableQDSButtons() {
  ...
  EnableButton(guiQuestDebugGiveItemToNPCButton,
               gItemListBox.sCurSelectedItem != -1 && gNpcListBox.sCurSelectedItem != 0);

  if (gfUseLocalNPCs) {
    bool const enable =
        IsMercInTheSector(gubCurrentNpcInSector[gNpcListBox.sCurSelectedItem]) != -1;
...
static void DoQDSMessageBox(wchar_t const *const zString, ScreenID const uiExitScreen,
...
static int32_t GetMaxNumberOfQuotesToPlay();
static void SetQDSMercProfile();
...
static void StartMercTalkingFromQuoteNum(int32_t iQuoteToStartTalkingFrom) {
  ...
  // create a mask to block out the screen
  if (!gfBackgroundMaskEnabled) {
    MSYS_DefineRegion(&gQuestTextEntryDebugDisableScreenRegion, 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT,
...
static void EndMercTalking() {
  ...
  if (!gfQuestDebugExit) guiQDPgUpButtonButton->uiFlags &= ~BUTTON_FORCE_UNDIRTY;
...
static void HandleQDSTalkingMerc() {
  ...
  if (giSelectedMercCurrentQuote != -1) {
    ...
    // Call this function to enable or disable the flags in the faces struct (
    // without modifing the pause state )
    ...
    if (!GetQDSFace(ubPanelMercShouldUse)->fTalking) {
      ...
      if (giSelectedMercCurrentQuote < GetMaxNumberOfQuotesToPlay()) {
        ...
        else if (gfRpcToSaySectorDesc && MIGUEL <= gTalkingMercSoldier->ubProfile &&
                 gTalkingMercSoldier->ubProfile <= DIMITRI) {
          // ATE: Trigger the sector desc here
          CharacterDialogueUsingAlternateFile(*gTalkingMercSoldier, giSelectedMercCurrentQuote,
                                              DIALOGUE_NPC_UI);
        } else {
          CharacterDialogue(gTalkingMercSoldier->ubProfile, giSelectedMercCurrentQuote,
                            gTalkPanel.face, DIALOGUE_NPC_UI, FALSE);
        }
...
static void DisplayQDSCurrentlyQuoteNum() {
  ...
  DisplayWrappedString(QDS_CURRENT_QUOTE_NUM_BOX_X + 5, usPosY,
                       ...
                       FONT_MCOLOR_WHITE, zTemp, FONT_MCOLOR_BLACK, CENTER_JUSTIFIED);
...
static void DisableFactMouseRegions() { FOR_EACH(MOUSE_REGION, i, gFactListRegion) i->Disable(); }

static void EnableFactMouseRegions() { FOR_EACH(MOUSE_REGION, i, gFactListRegion) i->Enable(); }

static int32_t GetMaxNumberOfQuotesToPlay() {
  const ProfileID pid = gNpcListBox.sCurSelectedItem;

  int32_t iNumberOfQuotes;
  if (gfRpcToSaySectorDesc && MIGUEL <= pid && pid <= DIMITRI) {
    // If it is the RPCs and they are to say the sector descs
    iNumberOfQuotes = 34;
  } else if (GetProfile(pid).ubMiscFlags & PROFILE_MISC_FLAG_RECRUITED && MIGUEL <= pid &&
             pid <= MADDOG) {
...
static void GetDebugLocationString(const uint16_t usProfileID, wchar_t *const pzText,
                                   const size_t Length) {
