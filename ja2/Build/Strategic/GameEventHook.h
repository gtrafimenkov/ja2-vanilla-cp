...
// This value is added to the param value for NPC-system-created events which
// are based on an action rather than a fact:
#define NPC_SYSTEM_EVENT_ACTION_PARAM_BONUS 10000

// One Time Events
// These events are scheduled to happen, and when their time is up, they are
...
// an end time.  If this is used, then the execution of the event will happen
// once at the beginning of the event and once at the expiration time of the
// event.  A flag, pEvent->fEndEvent is set for the second call.  It is up to you
...
// the same time every day.  Time of day lighting is a perfect example.  Other
