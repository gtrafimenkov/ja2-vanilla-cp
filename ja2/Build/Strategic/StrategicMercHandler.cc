...
void StrategicHandlePlayerTeamMercDeath(SOLDIERTYPE &s) {
  ...
  if (!AM_AN_EPC(&s) && !AM_A_ROBOT(&s)) { /* Keep track of how many mercs have died under
                                            * player's command (for death rate, can't wait until
                                            * removed from team) */
  ...
  if (s.usLifeInsurance) {
    if (guiCurrentScreen != AUTORESOLVE_SCREEN) {  // Check whether this was obviously a suspicious
                                                   // death
      if (now >= s.uiStartTimeOfInsuranceContract &&
          now - s.uiStartTimeOfInsuranceContract < 60) {  // Killed within an hour of being insured
        p.ubSuspiciousDeath = VERY_SUSPICIOUS_DEATH;
      } else if (s.attacker->bTeam == OUR_TEAM ||
                 !gTacticalStatus.fEnemyInSector) { /* Killed by someone on our team or
                                                     * while there weren't any opponents
                                                     * around, cause insurance company to
                                                     * suspect fraud and investigate this
                                                     * claim */
  ...
  if (!AM_AN_EPC(&s) && !AM_A_ROBOT(&s)) {  // Change morale of others based on this
    HandleMoraleEvent(&s, MORALE_TEAMMATE_DIED, s.sSectorX, s.sSectorY, s.bSectorZ);
  }

  if (s.ubWhatKindOfMercAmI == MERC_TYPE__MERC) {  // It's a MERC merc, record the time of his death
...
void MercDailyUpdate() {
  ...
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    if (s->bAssignment == ASSIGNMENT_POW) {
      ...
      // player has hired him, so he'll eligible to get killed off on another
      // job
      ...
      if (FIRST_RPC <= s->ubProfile && s->ubProfile < FIRST_NPC) {
        ...
        if (iMoneyOwedToMerc != 0) {
          ...
          if (LaptopSaveInfo.iCurrentBalance >= iMoneyOwedToMerc) {
            ...
            AddTransactionToPlayersBook(PAYMENT_TO_NPC, s->ubProfile, GetWorldTotalMin(),
                                        -iMoneyOwedToMerc);
            ...
            ScreenMsg(FONT_MCOLOR_WHITE, MSG_INTERFACE,
                      pMessageStrings[MSG_CANT_AFFORD_TO_PAY_NPC_DAILY_SALARY_MSG], p.zNickname,
                      zMoney);
    ...
    if (s->bAssignment != ASSIGNMENT_DEAD && s->bAssignment != IN_TRANSIT) {
      // increment the "man days" played counter for each such merc in the
      // player's employment
  ...
  for (int32_t cnt = 0; cnt < NUM_PROFILES; ++cnt) {
    ...
    if (p.uiDayBecomesAvailable > 0) {
          ...
          p.bMercStatus != MERC_FIRED_AS_A_POW)  // if the merc CAN become ready
      {
        ...
        if (p.ubMiscFlags3 & PROFILE_MISC_FLAG3_PLAYER_LEFT_MSG_FOR_MERC_AT_AIM) {
          // remove the Flag, so if the merc goes on another assignment, the
          // player can leave an email.
          p.ubMiscFlags3 &= ~PROFILE_MISC_FLAG3_PLAYER_LEFT_MSG_FOR_MERC_AT_AIM;

          // TO DO: send E-mail to player telling him the merc has returned from
          // an assignment
          AddEmail(AIM_REPLY_BARRY + cnt * AIM_REPLY_LENGTH_BARRY, AIM_REPLY_LENGTH_BARRY, 6 + cnt,
                   GetWorldTotalMin());
        }
      }
    } else  // was already available today
    {
      if (cnt < AIM_AND_MERC_MERCS) {
        ...
        if (cnt < MAX_NUMBER_MERCS) {  // A.I.M. merc
          ...
          // player has now had a chance to hire him, so he'll eligible to get
          // killed off on another job
          p.ubMiscFlags3 |= PROFILE_MISC_FLAG3_PLAYER_HAD_CHANCE_TO_HIRE;
        } else {  // M.E.R.C. merc - very rarely get other work
          uiChance = 1 * p.bExpLevel;

          // player doesn't have a chance to hire any M.E.R.C's until after
          // Speck's E-mail is sent
    ...
    // Decrement morale hangover (merc appears hirable, he just gives lame
    // refusals during this time, though)
...
// ATE: This function deals with MERC MERC and NPC's leaving because of not
// getting paid... NOT AIM renewals....
...
BOOLEAN SoldierHasWorseEquipmentThanUsedTo(SOLDIERTYPE *pSoldier) {
  ...
  // Modify these values based on morale - lower opinion of equipment if morale
  // low, increase if high this of course assumes default morale is 50
  if (bBestGun != -1) {
...
void MercComplainAboutEquipment(uint8_t ubProfile) {
  ...
  if (pSoldier != NULL) {
    if (!pSoldier->fMercAsleep && pSoldier->bLife >= OKLIFE && pSoldier->bAssignment < ON_DUTY) {
...
void UpdateBuddyAndHatedCounters() {
  FOR_EACH_IN_TEAM(s, OUR_TEAM) {
    ...
    CFOR_EACH_IN_TEAM(other, OUR_TEAM) {
      // Is this guy in the same sector and on active duty (or in the same
      // moving group)
      if (other != s && other->bAssignment < ON_DUTY) {
        ...
        } else {  // Check to see if the location is the same
          ...
          switch (i) {
              ...
              if (p.bHated[i] == ubOtherProfileID) {
                ...
                if (hated_count > 0) {
                  ...
                  if (hated_count == 0 && s->bInSector &&
                      gTacticalStatus.fEnemyInSector) {  // Just reduced count to 0 but we
                                                         // have enemy in sector
                    hated_count = 1;
            ...
            case 2:
              if (p.bLearnToHate == ubOtherProfileID) {
                if (p.bLearnToHateCount > 0) {
                  p.bLearnToHateCount--;
                  if (p.bLearnToHateCount == 0 && s->bInSector &&
                      gTacticalStatus.fEnemyInSector) {  // Just reduced count to 0 but we
                                                         // have enemy in sector...
                    p.bLearnToHateCount = 1;
                             ...
                             (p.bLearnToHateCount == p.bLearnToHateTime / 2 ||
                              (p.bLearnToHateCount < p.bLearnToHateTime / 2 &&
                               p.bLearnToHateCount % TIME_BETWEEN_HATED_COMPLAINTS ==
                                   0))) {  // Complain!
                    TacticalCharacterDialogue(s, QUOTE_LEARNED_TO_HATE_MERC);
                    StopTimeCompression();
                  } else if (p.bLearnToHateCount == 0) {  // Set as bHated[2];
                    p.bHated[2] = p.bLearnToHate;
                    p.bMercOpinion[ubOtherProfileID] = HATED_OPINION;

                    if (s->ubWhatKindOfMercAmI == MERC_TYPE__MERC ||
                        (s->ubWhatKindOfMercAmI == MERC_TYPE__NPC &&
                         (s->ubProfile == DEVIN || s->ubProfile == SLAY || s->ubProfile == IGGY ||
                          s->ubProfile == CONRAD))) {  // Leave now! (handle equipment too)
                      TacticalCharacterDialogue(s, QUOTE_MERC_QUIT_LEARN_TO_HATE);
                      MakeCharacterDialogueEventContractEnding(*s, false);
                      s->ubLeaveHistoryCode = HISTORY_MERC_QUIT;

                    } else if (s->ubWhatKindOfMercAmI == MERC_TYPE__NPC) {  // Whine again
                      TacticalCharacterDialogue(s, QUOTE_LEARNED_TO_HATE_MERC);
                    }
                  }
                  if (p.bLearnToHateCount < p.bLearnToHateTime / 2) {  // Gradual opinion drop
                    p.bMercOpinion[ubOtherProfileID] +=
                        (HATED_OPINION - p.bMercOpinion[ubOtherProfileID]) /
                        (p.bLearnToHateCount + 1);
                  }
                } else {
                  if (!fUpdatedTimeTillNextHatedComplaint) {
            ...
            case 3:
              if (p.bLearnToLikeCount > 0 && p.bLearnToLike == ubOtherProfileID) {
                p.bLearnToLikeCount--;
                if (p.bLearnToLikeCount == 0) {  // Add to liked!
                  p.bBuddy[2] = p.bLearnToLike;
                  p.bMercOpinion[ubOtherProfileID] = BUDDY_OPINION;
                } else if (p.bLearnToLikeCount <
                           p.bLearnToLikeTime / 2) {  // Increase opinion of them!
                  p.bMercOpinion[ubOtherProfileID] +=
                      (BUDDY_OPINION - p.bMercOpinion[ubOtherProfileID]) /
                      (p.bLearnToLikeCount + 1);
                  break;
                }
              }
              break;
  ...
  FOR_EACH_IN_TEAM(i, OUR_TEAM) {
    ...
    if (s.bCamo > 0 && !HAS_SKILL_TRAIT(&s, CAMOUFLAGED)) {
      if (s.bCamo <= 2) {
        ...
        ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, g_langRes->Message[STR_CAMO_WORN_OFF],
                  s.name);
