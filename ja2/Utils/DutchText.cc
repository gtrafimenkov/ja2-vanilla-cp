...
static StrPointer s_dut_Message[Message_SIZE] = {
    ...
    L"Het item dat je aanwijst, kan vastgemaakt worden aan een bepaald item "
    L"door het in een van de vier uitbreidingssloten te plaatsen.",
    L"Het item dat je aanwijst, kan vastgemaakt worden aan een bepaald item "
    L"door het in een van de vier uitbreidingssloten te plaatsen. (Echter, het "
    L"item is niet compatibel.)",
    ...
    L"Ik denk het NIET!",
    L"Chopper van Skyrider gebruiken? Eerst huurlingen TOEWIJZEN aan "
    L"VOERTUIG/HELIKOPTER.",
...
static const wchar_t *s_dut_pMilitiaString[pMilitiaString_SIZE] = {
    ...
    L"Milities kunnen niet herplaatst worden als er nog vijanden in de buurt "
    L"zijn!",
};
...
static const wchar_t *s_dut_gpStrategicString[gpStrategicString_SIZE] = {
    L"%ls zijn ontdekt in sector %c%d en een ander team arriveert "
    L"binnenkort.",  // STR_DETECTED_SINGULAR
    L"%ls zijn ontdekt in sector %c%d en andere teams arriveren "
    L"binnenkort.",                                    // STR_DETECTED_PLURAL
    ...
    L"Sector",      // The Sector A9 part of the header
    ...
    // Various single words for direct translation.  The Civilians represent
    // the civilian militia occupying the sector being attacked.  Limited to
    // 9-10 chars
    ...
    L"Ziet er hier niet goed uit. Het is onbegaanbaar. Probeer een andere "
    L"locatie.",
    ...
    // line.  Do not use spaces before or after the \n.
    ...
    L"T|rek alle groepen terug en\nga naar hun vorige sectors.",  // multiple
                                                                  // groups
                                                                  // with same
                                                                  // previous
                                                                  // sector
    ...
    // 1st %d refers to the number of civilians eaten by monsters,  %c%d is
    // the sector -- ex:  A9 Note:  the minimum number of civilians eaten
    // will be two.
    L"Beesten vallen aan en doden %d burgers in sector %ls.",
    //%ls is the sector location -- ex:  A9: Omerta
    L"Vijand valt je huurlingen aan in sector %ls. Geen enkele huurling "
    L"kan vechten!",
    //%ls is the sector location -- ex:  A9: Omerta
    L"Beesten vallen je huurlingen aan in sector %ls. Geen enkele huurling "
    L"kan vechten!",
...
static const wchar_t *s_dut_gMoneyStatsDesc[gMoneyStatsDesc_SIZE] = {
    ...
    L"Bedrag",
    L"Splitsen:",  // the amount he wants to separate from the overall balance to
...
static StrPointer s_dut_TacticalStr[TacticalStr_SIZE] = {
    ...
    L"Het slot heeft %ls.", L"Er is geen slot.", L"Geen boobytrap op het slot.",
    // The %ls is a merc name
    L"%ls heeft niet de juiste sleutel.", L"Slot heeft geen boobytrap.", L"Op slot.", L"DEUR",
    L"VAL", L"OP SLOT", L"OPEN", L"KAPOT", L"Hier zit een schakelaar. Activeren?",
    L"Boobytrap ontmantelen?", L"Meer...",

    // In the next 2 strings, %ls is an item name

    L"%ls is op de grond geplaatst.", L"%ls is gegeven aan %ls.",

    // In the next 2 strings, %ls is a name

    L"%ls is helemaal betaald.", L"%ls heeft tegoed nog %d.",
    L"Kies detonatie frequentie:",          // in this case, frequency refers to a radio
                                            // signal
    L"Aantal beurten tot ontploffing:",     // how much time, in turns, until the
                                            // bomb blows
    L"Stel frequentie in van ontsteking:",  // in this case, frequency refers to
                                            // a radio signal
    ...
    L"Zeker weten dat je %ls wil aanvallen?", L"Ah, voertuigen kunnen plaats niet veranderen.",
    L"De robot kan niet van plaats veranderen.",

    // In the next 3 strings, %ls is a name

    L"%ls kan niet naar die plaats gaan.", L"%ls kan hier geen EHBO krijgen.",
    L"%ls heeft geen EHBO nodig.", L"Kan daar niet heen.",
    L"Je team is vol. Geen ruimte voor rekruut.",  // there's no room for a
                                                   // recruit on the player's team
    ...
    L"Hier gooien? Kan niet.",               // Merc can't throw to the destination he
                                             // selected
    ...
    L"Wissel Team ( |S|h|i|f|t |S|p|a|c|e )",      // L"Change Squad ( |S|h|i|f|t
                                                   // |S|p|a|c|e )",
    L"Rijden",                                     // L"Drive",
    ...
    L"Vanaf deze kant kun je de sector niet verlaten.",  // L"You can't leave the
                                                         // sector from this
                                                         // side.",
    ...
    L"De Koningin vertellen wat ze wil weten?",      // L"Tell the Queen what she
                                                     // wants to know?",
    L"Vingerafdruk-ID nodig",                        // L"Fingerprint ID aquired",
    L"Vingerafdruk-ID ongeldig. Wapen funct. niet",  // L"Invalid fingerprint ID.
                                                     // Weapon non-functional",
    L"Doelwit nodig",                                // L"Target aquired",
    L"Pad geblokkeerd",                              // L"Path Blocked",
    L"Geld Storten/Opnemen",       // Help text over the $ button on the Single Merc
                                   // Panel ("Deposit/Withdraw Money")
    ...
    L"Voertuig kan alleen tussen sectors reizen",  // L"Vehicle can only travel
                                                   // between sectors",
    ...
    L"Iemand anders probeert deze deur te gebruiken.",  // L"Somebody else is
                                                        // trying to use this
                                                        // door.",
...
static const wchar_t *s_dut_pExitingSectorHelpText[pExitingSectorHelpText_SIZE] = {
    ...
    L"Als aangekruist, dan wordt de aanliggende sector meteen "
    L"geladen.",
    L"Als aangekruist, dan worden de huurlingen automatisch op "
    ...
    L"Deze sector is door de vijand bezet en huurlingen kun je "
    L"niet achterlaten.\nJe moet deze situatie oplossen voor het "
    L"laden van andere sectors.",

    // Because you only have one squad in the sector, and the "move
    // all" option is checked, the "go to sector" option is locked to
    // on. The helptext explains why it is locked.
    L"Als de overgebleven huurlingen uit deze sector "
    L"trekken,\nwordt de aanliggende sector onmiddellijk geladen.",
    L"Als de overgebleven huurlingen uit deze sector "
    L"trekken,\nwordt je automatisch in het landkaartscherm "
    L"geplaatst,\nrekening houdend met de reistijd van je "
    L"huurlingen.",

    // If an EPC is the selected merc, it won't allow the merc to
    // leave alone as the merc is being escorted.  The "single"
    // button is disabled.
    L"%ls moet geëscorteerd worden door jouw huurlingen\nen kan de "
    L"sector niet alleen verlaten.",

    // If only one conscious merc is left and is selected, and there
    // are EPCs in the squad, the merc will be prohibited from
    // leaving alone. There are several strings depending on the
    // gender of the merc and how many EPCs are in the squad. DO NOT
    // USE THE NEWLINE HERE AS IT IS USED FOR BOTH HELPTEXT AND
    // SCREEN MESSAGES!
    L"%ls kan de sector niet alleen verlaten omdat hij %ls "
    L"escorteert.",  // male singular
    L"%ls kan de sector niet alleen verlaten omdat zij %ls "
    L"escorteert.",  // female singular
    L"%ls kan de sector niet alleen verlaten omdat hij meerdere "
    L"karakters escorteert.",  // male plural
    L"%ls kan de sector niet alleen verlaten omdat zij meerdere "
    L"karakters escorteert.",  // female plural

    // If one or more of your mercs in the selected squad aren't in
    // range of the traversal area, then the  "move all" option is
    // disabled, and this helptext explains why.
    L"Al je huurlingen moeten in de buurt zijn om het team te "
    L"laten reizen.",
    ...
    L"automatisch bij een uniek team gevoegd worden.",

    // Standard helptext for all movement.  Explains what will
    // happen (moving the squad)
    L"Als aangekruist, dan zal je geselecteerde\nteam verder "
    L"reizen, de sector verlatend.",

    // This strings is used BEFORE the "exiting sector" interface is
    // created.  If you have an EPC selected and you attempt to
    // tactically traverse the EPC while the escorting mercs aren't
    // near enough (or dead, dying, or unconscious), this message
    // will appear and the "exiting sector" interface will not
    // appear.  This is just like the situation where This string is
    // special, as it is not used as helptext.  Do not use the
    // special newline character (\n) for this string.
    L"%ls wordt geëscorteerd door jouw huurlingen en kan de sector "
    L"niet alleen verlaten. Je huurlingen moeten eerst in de buurt "
    L"zijn.",
};

static const wchar_t *s_dut_pRepairStrings[pRepairStrings_SIZE] = {
    L"Items",         // tell merc to repair items in inventory
    L"SAM-Stelling",  // tell merc to repair SAM site - SAM is an acronym for
...
static const wchar_t *s_dut_pHelicopterEtaStrings[pHelicopterEtaStrings_SIZE] = {
    ...
    L"Helikopter heeft weinig brandstof en moet landen in vijandelijk "
    L"gebied!",  // warning that the sector the helicopter is going to use
                 // for refueling is under enemy control ->
    L"Passagiers: ",
    L"Selecteer Skyrider of Aanvoer Drop-plaats?",  // L"Select Skyrider or
                                                    // the Arrivals
                                                    // Drop-off?",
    L"Skyrider",
...
static const wchar_t s_dut_sMapLevelString[] =
    L"Subniv.:";  // what level below the ground is the player viewing in
                  // mapscreen ("Sublevel:")

static const wchar_t s_dut_gsLoyalString[] =
    L"%d%% Loyaal";  // the loyalty rating of a town ie : Loyal 53%
...
static const wchar_t *s_dut_pUpdateMercStrings[pUpdateMercStrings_SIZE] = {
    ...
    L"Contract Huurling verlopen:",  // this pop up came up due to a merc
                                     // contract ending
    L"Huurling Taak Volbracht:",     // this pop up....due to more than one merc
                                     // finishing assignments
    L"Huurling weer aan het Werk:",  // this pop up ....due to more than one merc
                                     // waking up and returing to work
    L"Huurling zegt Zzzzzzz:",       // this pop up ....due to more than one merc
                                     // being tired and going to sleep
    L"Contract Loopt Bijna Af:",     // this pop up came up due to a merc contract
                                     // ending
};
...
static const wchar_t *s_dut_pMapScreenBottomFastHelp[pMapScreenBottomFastHelp_SIZE] = {
    ...
    L"Vorig Bericht (|U|p)\nVorige Pagina (|P|g|U|p)",          // previous message
                                                                // in scrollable list
    L"Volgend Bericht (|D|o|w|n)\nVolgende pagina (|P|g|D|n)",  // next
                                                                // message in
                                                                // the
                                                                // scrollable
                                                                // list
...
static const wchar_t s_dut_pMapScreenBottomText[] =
...
static const wchar_t s_dut_pFinanceTitle[] = L"Account Plus";  // the name we made up for the
...
static const wchar_t *s_dut_pTransactionText[pTransactionText_SIZE] = {
    ...
    L"IMP Profiel Analyse",         // IMP is the acronym for International Mercenary
                                    // Profiling
    ...
    L"Items gekocht van %ls.",   // is used for the Shop keeper interface.  The
...
static const wchar_t *s_dut_pSkyriderText[pSkyriderText_SIZE] = {
    ...
    L"Skyrider heeft geen passagiers. Als je huurlingen in deze sector wil "
    L"vervoeren, wijs ze dan eerst toe aan Voertuig/Helikopter.",
};
...
static const wchar_t s_dut_str_left_equipment[] =
    L"%ls's uitrusting is nu beschikbaar in %ls (%c%d).";
...
static const wchar_t
    *s_dut_pMapScreenPrevNextCharButtonHelpText[pMapScreenPrevNextCharButtonHelpText_SIZE] = {
        L"Vorige Huurling (|L|e|f|t)",      // previous merc in the list
        L"Volgende Huurling (|R|i|g|h|t)",  // next merc in the list
};

static const wchar_t s_dut_pEtaString[] =
    L"aank:";  // eta is an acronym for Estimated Time of Arrival

static const wchar_t *s_dut_pTrashItemText[pTrashItemText_SIZE] = {
    L"Je bent het voor altijd kwijt. Zeker weten?",  // do you want to continue
                                                     // and lose the item forever
    L"Dit item ziet er HEEL belangrijk uit. Weet je HEEL, HEEL zeker dat je "
    L"het wil weggooien?",  // does the user REALLY want to trash this item
...
static const wchar_t *s_dut_pMapErrorString[pMapErrorString_SIZE] = {
    ...
    L"Om te verplaatsen moeten huurlingen eerst toegewezen worden aan een team "
    L"of voertuig.",
    ...
    L"Huurling kan order niet opvolgen.",  // merc can't comply with your order
                                           // 6-10
    L"%ls heeft een escorte nodig. Plaats hem in een team.",   // merc can't move
                                                               // unescorted ..
                                                               // for a male
    L"%ls heeft een escorte nodig. Plaats haar in een team.",  // for a female
    L"Huurling is nog niet in Arulco aangekomen!",
    L"Het lijkt erop dat er eerst nog contractbesprekingen gehouden moeten "
    L"worden.",
    L"",
    // 11-15
    L"Reisorders? Er is daar een gevecht gaande!",
    L"Je bent in een hinderlaag gelokt van Bloodcats in sector %ls!",
    L"Je bent in sector I16 iets binnengelopen dat lijkt op het hol van een "
    L"bloodcat!",
    L"",
    L"De SAM-stelling in %ls is overgenomen.",
    // 16-20
    L"De mijn in %ls is overgenomen. Je dagelijkse inkomen is gereduceerd tot "
    L"%ls per dag.",
    ...
    L"Milities niet binnen de stadsgrenzen kunnen niet verplaatst worden naar "
    L"een andere sector.",
    // 31-35
    ...
    L"Totdat je nieuwe huurlingen in dienst neemt, kan de tijd niet versneld "
    L"worden!",
    ...
    // 46-50
    L"Eén of meer leden van de ANDERE huurlingen kunnen zich op dit moment "
    L"niet verplaatsen.",
    L"Voertuig is te beschadigd!",
    L"Let op dat maar twee huurlingen milities in een sector mogen trainen.",
    L"De robot kan zich zonder bediening niet verplaatsen. Plaats ze in "
    L"hetzelfde team.",
};

// help text used during strategic route plotting
static const wchar_t *s_dut_pMapPlotStrings[pMapPlotStrings_SIZE] = {
    L"Klik nogmaals op de bestemming om de route te bevestigen, of klik op een "
    L"andere sector om meer routepunten te plaatsen.",
    L"Route bevestigd.",
...
static const wchar_t *s_dut_pBullseyeStrings[pBullseyeStrings_SIZE] = {
    ...
    L"Huurlingen kunnen hier niet ingevlogen worden, het luchtruim is "
    L"onveilig!",
    L"Afgebroken. Aankomst-sector onveranderd",
    L"Luchtruim boven %ls is niet langer veilig! Aankomst-sector is verplaatst "
    L"naar %ls.",
};
...
static const wchar_t
    *s_dut_pMiscMapScreenMouseRegionHelpText[pMiscMapScreenMouseRegionHelpText_SIZE] = {
        L"Naar Inventaris (|E|n|t|e|r)",
        L"Gooi Item Weg",
        L"Verlaat Inventaris (|E|n|t|e|r)",
};

static const wchar_t s_dut_str_he_leaves_where_drop_equipment[] =
    L"Laat %ls zijn uitrusting achterlaten waar hij nu is (%ls) of later in "
    L"%ls (%ls) bij het nemen van de vlucht uit Arulco?";
static const wchar_t s_dut_str_she_leaves_where_drop_equipment[] =
    L"Laat %ls haar uitrusting achterlaten waar ze nu is (%ls) of later in %ls "
    L"(%ls) bij het nemen van de vlucht uit Arulco?";
static const wchar_t s_dut_str_he_leaves_drops_equipment[] =
    L"%ls gaat binnenkort weg en laat zijn uitrusting achter in %ls.";
static const wchar_t s_dut_str_she_leaves_drops_equipment[] =
...
static const wchar_t *s_dut_pImpPopUpStrings[pImpPopUpStrings_SIZE] = {
    ...
    L"Om een nauwkeurig profiel te maken, moet je ruimte hebben voor tenminste "
    L"één teamlid.",
...
static const wchar_t *s_dut_pImpButtonText[pImpButtonText_SIZE] = {
    ...
    L"Ja, ik kies het geselecteerde antwoord.",  // ("Yes, I choose the
                                                 // highlighted answer.")
    L"Ja", L"Nee",
    ...
    L"NEE, IK WIL OPNIEUW BEGINNEN.",  // no, I want to start over the profiling
                                       // process ("NO, I WANT TO START OVER.")
    L"JA, ZEKER.",                     // ("YES, I DO.")
    L"NEE",
...
static const wchar_t *s_dut_pExtraIMPStrings[pExtraIMPStrings_SIZE] = {
    ...
    L"Nu de eigenschappen gekozen zijn, kun je verder gaan met de "
    L"portretselectie.",
...
static const wchar_t *s_dut_pFilesSenderList[pFilesSenderList_SIZE] = {
    L"Int. Verslag",  // the recon report sent to the player. Recon is an
...
static const wchar_t *s_dut_pHistoryStrings[pHistoryStrings_SIZE] = {
    ...
    L"Opdracht van Enrico Chivaldori geaccepteerd.",  // ("Accepted Assignment
                                                      // From Enrico Chivaldori")
    // 6-10
    ...
    // 11-15
    L"%ls's contract verlengd met 1 week.",   // Extented "mercs name"'s for a
                                              // week
    ...
    L"Voedsel zou morgen in Omerta moeten zijn",  // ("Food should be in Omerta
                                                  // by tomorrow")
    // 21-25
    L"%ls weggegaan, wordt Daryl Hick's vrouw",  // ("%ls left team to become
                                                 // Daryl Hick's wife")
    L"%ls's contract afgelopen.",                // ("%ls's contract expired.")
    L"%ls aangenomen.",                          // ("%ls was recruited.")
    L"Enrico klaagde over de voortgang",         // ("Enrico complained about lack of
                                                 // progress")
    ...
    L"Info verkregen over gevangenis Tixa.",  // ("Found out about a prison
                                              // called Tixa.")
    // 31-35
    L"Van geheime wapenfabriek gehoord, Orta genaamd.",   // ("Heard about a
                                                          // secret weapons plant
                                                          // called Orta.")
    L"Onderzoeker in Orta geeft wat raketwerpers.",       // ("Scientist in Orta
                                                          // donated a slew of rocket
                                                          // rifles.")
    L"Koningin Deidranna kickt op lijken.",               // ("Queen Deidranna has a use for
                                                          // dead bodies.")
    L"Frank vertelde over knokwedstrijden in San Mona.",  // ("Frank talked about
                                                          // fighting matches in
                                                          // San Mona.")
    L"Een patiënt dacht dat ie iets in de mijnen zag.",   // ("A patient thinks he
                                                          // saw something in the
                                                          // mines.")
    // 36-40
    L"Pers. ontmoet; Devin - verkoopt explosieven.",  // ("Met someone named
                                                      // Devin - he sells
                                                      // explosives.")
    L"Beroemde ex-AIM huurling Mike ontmoet!",        // ("Ran into the famous ex-AIM
                                                      // merc Mike!")
    L"Tony ontmoet - handelt in wapens.",             // ("Met Tony - he deals in arms.")
    L"Raketwerper gekregen van Serg. Krott.",         // ("Got a rocket rifle from
                                                      // Sergeant Krott.")
    L"Kyle akte gegeven van Angel's leerwinkel.",     // ("Gave Kyle the deed to
                                                      // Angel's leather shop.")
    // 41-45
    L"Madlab bood aan robot te bouwen.",          // ("Madlab offered to build a robot.")
    L"Gabby maakt superbrouwsel tegen beesten.",  // ("Gabby can make stealth
                                                  // concoction for bugs.")
    L"Keith is er mee opgehouden.",               // ("Keith is out of business.")
    L"Howard geeft Koningin Deidranna cyanide.",  // ("Howard provided cyanide to
                                                  // Queen Deidranna.")
    L"Keith ontmoet - handelaar in Cambria.",     // ("Met Keith - all purpose
                                                  // dealer in Cambria.")
    // 46-50
    L"Howard ontmoet - medicijnendealer in Balime",    // ("Met Howard - deals
                                                       // pharmaceuticals in
                                                       // Balime")
    L"Perko ontmoet - heeft reparatiebedrijfje.",      // ("Met Perko - runs a small
                                                       // repair business.")
    L"Sam van Balime ontmoet - verkoopt ijzerwaren.",  // ("Met Sam of Balime -
                                                       // runs a hardware shop.")
    L"Franz verkoopt elektronica en andere dingen.",   // ("Franz deals in
                                                       // electronics and other
                                                       // goods.")
    L"Arnold runt reparatiezaak in Grumm.",            // ("Arnold runs a repair shop in
                                                       // Grumm.")
    // 51-55
    L"Fredo repareert elektronica in Grumm.",            // ("Fredo repairs electronics in
                                                         // Grumm.")
    L"Van rijke vent in Balime donatie gekregen.",       // ("Received donation from
                                                         // rich guy in Balime.")
    L"Schroothandelaar Jake ontmoet.",                   // ("Met a junkyard dealer named Jake.")
    L"Vaag iemand gaf ons elektronische sleutelkaart.",  // ("Some bum gave us an
                                                         // electronic keycard.")
    L"Walter omgekocht om kelderdeur open te maken.",    // ("Bribed Walter to
                                                         // unlock the door to the
                                                         // basement.")
    // 56-60
    L"Als Dave gas heeft, geeft hij deze weg.",  // ("If Dave has gas, he'll
                                                 // provide free fillups.")
    L"Geslijmd met Pablo.",                      // ("Greased Pablo's palms.")
    L"Kingpin bewaard geld in San Mona mine.",   // ("Kingpin keeps money in San
                                                 // Mona mine.")
    L"%ls heeft Extreme Fighting gewonnen",      // ("%ls won Extreme Fighting
                                                 // match")
    L"%ls heeft Extreme Fighting verloren",      // ("%ls lost Extreme Fighting
                                                 // match")
    // 61-65
    L"%ls gediskwalificeerd v. Extreme Fighting",  // ("%ls was disqualified in
                                                   // Extreme Fighting")
    L"Veel geld gevonden in een verlaten mijn.",   // ("Found a lot of money
                                                   // stashed in the abandoned
                                                   // mine.")
    L"Huurmoordenaar van Kingpin ontdekt.",        // ("Encountered assassin sent by
                                                   // Kingpin.")
    L"Controle over sector verloren",              // ENEMY_INVASION_CODE ("Lost control of
                                                   // sector")
...
static const wchar_t s_dut_pErrorStrings[] =
...
static const wchar_t s_dut_pWebTitle[] = L"sir-FER 4.0";  // our name for the version of the
...
static const wchar_t *s_dut_pPersonnelDepartedStateStrings[pPersonnelDepartedStateStrings_SIZE] = {
    ...
    L"Omgekomen tijdens gevechten", L"Weggestuurd", L"Getrouwd", L"Contract Afgelopen", L"Gestopt",
};
...
static const wchar_t
    *s_dut_pPersonnelCurrentTeamStatsStrings[pPersonnelCurrentTeamStatsStrings_SIZE] = {
        L"Laagste",
        L"Gemiddeld",
        L"Hoogste",
};
...
static const wchar_t *s_dut_MercAccountText[MercAccountText_SIZE] = {
    ...
    L"Prijs",
    L"Totaal:",
    L"Weet je zeker de betaling van %ls te autoriseren?",  // the %ls is a string
                                                           // that contains the
                                                           // dollar amount ( ex.
                                                           // "$150" )
};
...
static const wchar_t *s_dut_sFuneralString[sFuneralString_SIZE] = {
    ...
    L"Begrafenisondernemer en voormalig A.I.M. huurling Murray \"Pops\" "
    L"McGillicutty is een kundig en ervaren begrafenisondernemer.",
    L"Pops weet hoe moeilijk de dood kan zijn, in heel zijn leven heeft hij te "
    L"maken gehad met de dood en sterfgevallen.",
    L"McGillicutty's Mortuarium biedt een breed scala aan stervensbegeleiding, "
    L"van een schouder om uit te huilen tot recontructie van misvormde "
    L"overblijfselen.",
    L"Laat McGillicutty's Mortuarium u helpen en laat uw dierbaren zacht "
    L"rusten.",
    ...
    // send flowers ).
    L"Helaas is deze pagina nog niet voltooid door een sterfgeval in de "
    L"familie. Afhankelijk van de laatste wil en uitbetaling van de "
    L"beschikbare activa wordt de pagina zo snel mogelijk voltooid.",
    L"Ons medeleven gaat uit naar jou, tijdens deze probeerperiode. Kom nog "
    L"eens langs.",
};
...
static const wchar_t *s_dut_sFloristText[sFloristText_SIZE] = {
    ...
    L"Volgende dag gebracht, wereldwijd, gegarandeerd. Enkele beperkingen zijn "
    L"van toepassing.",
    L"Laagste prijs in de wereld, gegarandeerd!",
    L"Toon ons een lagere geadverteerde prijs voor een regeling en ontvang "
    L"gratis een dozijn rozen.",
    L"Flora, Fauna & Bloemen sinds 1981.",
    L"Onze onderscheiden ex-bommenwerperpiloten droppen je boeket binnen een "
    L"tien kilometer radius van de gevraagde locatie. Altijd!",
    L"Laat ons al je bloemenfantasieën waarmaken.",
    L"Laat Bruce, onze wereldberoemde bloemist, de verste bloemen met de "
    L"hoogste kwaliteit uit onze eigen kassen uitzoeken.",
...
static const wchar_t *s_dut_sOrderFormText[sOrderFormText_SIZE] = {
    ...
    L"Wegens de grootte kaarten, mogen je berichten niet langer zijn dan 75 "
    L"karakters.",
...
static const wchar_t *s_dut_sFloristGalleryText[sFloristGalleryText_SIZE] = {
    ...
    L"Let op: er geldt een extra tarief van $10 voor geplette en verlepte "
    L"boeketten.",
...
static const wchar_t *s_dut_BobbyROrderFormText[BobbyROrderFormText_SIZE] = {
    ...
    L"Eindtotaal",              // The grand total of all item totals + the shipping and
                                // handling
    L"Bezorglocatie",
    ...
    L"Je kunt dit niet betalen.",   // 20	// A popup message that to warn of not
                                    // enough money
    L"<GEEN>",                      // Gets displayed when there is no valid city selected
    L"Weet je zeker dat je de bestelling wil sturen naar %ls?",  // A popup that
                                                                 // asks if the
                                                                 // city selected
                                                                 // is the
                                                                 // correct one
    L"Gewicht Pakket**",  // Displays the weight of the package
    L"** Min. Gew.",      // Disclaimer states that there is a minimum weight for the
                          // package
    L"Zendingen",
};
...
static const wchar_t *s_dut_BobbyRText[BobbyRText_SIZE] = {
    ...
    L"Klik op de item(s). Voor meer dan één, blijf dan klikken. Rechtsklikken "
    L"voor minder. Als je alles geselecteerd hebt, dat je wil bestellen, ga "
    L"dan naar het bestelformulier.",
    ...
    L"* % Functioneel",  // if the item is damaged, displays the percent function
                         // of the item

    // Popup that tells the player that they can only order 10 items at a time

    L"Verdraaid! Dit on-line bestelformulier accepteert maar 10 items per "
    L"keer. Als je meer wil bestellen (en dat hopen we), plaats dan "
    L"afzonderlijke orders en accepteer onze excuses.",

    // A popup that tells the user that they are trying to order more items then
    // the store has in stock

    L"Sorry. We hebben niet meer van die zaken in het magazijn. Probeer het "
    L"later nog eens.",
 ...
 * ammo. */
static const wchar_t s_dut_str_bobbyr_guns_num_guns_that_use_ammo[] =
    L"Je team heeft %d wapen(s) gebruik makende van deze munitie";
...
static const wchar_t *s_dut_AimSortText[AimSortText_SIZE] = {
    ...
    L"Bekijk portretfotoindex van huurlingen", L"Bekijk het huurlingendossier",
...
static const wchar_t *s_dut_CharacterInfo[CharacterInfo_SIZE] = {
    ...
    L"MEDISCHE aanbetaling nodig",  // If the merc required a medical deposit,
                                    // this is displayed
};
...
static const wchar_t *s_dut_VideoConfercingText[VideoConfercingText_SIZE] = {
    ...
    L"+ med. depo",  // Displays if you are hiring the merc with the medical
                     // deposit
};
...
static const wchar_t *s_dut_AimScreenText[AimScreenText_SIZE] = {
    ...
    L"A.I.M. en A.I.M.-logo zijn geregistreerde handelsmerken in de meeste "
    L"landen.",
...
static const wchar_t *s_dut_SKI_Text[SKI_SIZE] = {
    ...
    L"TOTALE KOSTEN",            // The total cost of the the items in the Dealer inventory
                                 // area
    L"TOTALE WAARDE",            // The total value of items player wishes to sell
    L"EVALUEER",                 // Button text for dealer to evaluate items the player wants to
                                 // sell
    L"TRANSACTIE",               // Button text which completes the deal. Makes the
                                 // transaction.
    L"OK",                       // Text for the button which will leave the shopkeeper interface.
    L"REP. KOSTEN",              // The amount the dealer will charge to repair the merc's
                                 // goods
    L"1 UUR",                    // SINGULAR! The text underneath the inventory slot when an item
                                 // is given to the dealer to be repaired
    L"%d UREN",                  // PLURAL!   The text underneath the inventory slot when an item
                                 // is given to the dealer to be repaired
    L"GEREPAREERD",              // Text appearing over an item that has just been repaired
                                 // by a NPC repairman dealer
    L"Er is geen ruimte meer.",  // Message box that tells the user there is no
...
static const wchar_t *s_dut_zSaveLoadText[zSaveLoadText_SIZE] = {
    ...
    L"De spelversie van het bewaarde spel verschilt van de huidige versie. "
    L"Waarschijnlijk is het veilig om door te gaan. Doorgaan?",
    L"De bewaarde spelen zijn waarschijnlijk ongeldig. Deze verwijderen?",

    L"Attempting to load an older version save.  Automatically update and load "
    L"the save?",
    L"Attempting to load an older version save.  Automatically update and load "
    L"the save?",
    ...
    L"Er is te weinig ruimte op de harde schijf. Er is maar %d MB vrij en "
    L"Jagged heeft tenminste %d MB nodig.",

    L"Bewaren...",  // When saving a game, a message box with this string appears
                    // on the screen
...
static const wchar_t *s_dut_zMarksMapScreenText[zMarksMapScreenText_SIZE] = {
    ...
    // stuff for contracts
    L"Je kunt wel het contract betalen, maar je hebt geen geld meer om de "
    L"levensverzekering van de huurling te betalen.",
...
static const wchar_t s_dut_pLandMarkInSectorString[] =
...
static const wchar_t *s_dut_pMilitiaConfirmStrings[pMilitiaConfirmStrings_SIZE] = {
    L"Een stadsmilitie trainen kost $",             // telling player how much it will
                                                    // cost
    L"Uitgave goedkeuren?",                         // asking player if they wish to pay the amount
                                                    // requested
    L"je kunt dit niet betalen.",                   // telling the player they can't afford to
                                                    // train this town
    L"Doorgaan met militie trainen %ls (%ls %d)?",  // continue training this
                                                    // town?
    L"Kosten $",                                    // the cost in dollars to train militia
    L"( J/N )",                                     // abbreviated yes/no
    L"Stadsmilities trainen in %d sectors kost $ %d. %ls",  // cost to train
                                                            // sveral sectors
                                                            // at once
    L"Je kunt de $%d niet betalen om de stadsmilitie hier te trainen.",
    L"%ls heeft een loyaliteit nodig van %d procent om door te gaan met "
    L"milities trainen.",
...
static const wchar_t s_dut_gzCopyrightText[] =
...
static const wchar_t *s_dut_zOptionsScreenHelpText[zOptionsToggleText_SIZE] = {
    ...
    L"Als ondertitels AANstaan, schakel dit ook in om tijd te hebben de "
    L"NPC-dialogen te lezen.",
    ...
    // Never move my mouse
    L"Schakel deze optie UIT als je wil dat de muis automatisch gepositioneerd "
    L"wordt bij bevestigingsdialogen.",

    // Old selection method
    L"Schakel deze optie IN als je karakters wil selecteren zoals in de vorige "
    L"JAGGED ALLIANCE (methode is tegengesteld dus).",

    // Show movement path
    L"Schakel deze optie IN om bewegingspaden te tonen in real-time (schakel "
    L"het uit en gebruik dan de SHIFT-toets om paden te tonen).",

    // show misses
    L"Schakel IN om het spel de plaats van inslag van je kogels te tonen "
    L"wanneer je \"mist\".",

    // Real Time Confirmation
    L"Als INGESCHAKELD, een extra \"veiligheids\"-klik is nodig om in "
    L"real-time te bewegen.",

    // Sleep/Wake notification
    L"INGESCHAKELD zorgt voor berichten of huurlingen op een \"missie\" slapen "
    L"of werken.",

    // Use the metric system
    L"Wanneer INGESCHAKELD wordt het metrieke stelsel gebruikt, anders het "
    L"Imperiale stelsel.",

    // Merc Lighted movement
    L"Wanneer INGESCHAKELD, de huurling verlicht de grond tijdens het lopen. "
    L"Schakel UIT voor sneller spelen.",

    // Smart cursor
    L"Wanneer INGESCHAKELD zullen huurlingen dichtbij de cursor automatisch "
    L"oplichten.",

    // snap cursor to the door
    L"Wanneer INGESCHAKELD zal de cursor dichtbij een deur automatisch boven "
    L"de deur gepositioneerd worden.",
    ...
    // toggle wireframe
    L"Wanneer INGESCHAKELD worden Draadmodellen (|W) van niet-zichtbare muren "
    L"getoond.",
...
static const wchar_t *s_dut_gzGIOScreenText[gzGIOScreenText_SIZE] = {L"SPEL-INSTELLINGEN",
                                                                     L"Speelstijl",
                                                                     L"Realistisch",
                                                                     L"SF",
                                                                     L"Wapenopties",
...
static const wchar_t s_dut_pSkillAtZeroWarning[] =
...
static const wchar_t *s_dut_pMessageStrings[pMessageStrings_SIZE] = {
    ...
    L"rpm",        // Abbreviation for Rounds per minute -- the potential # of bullets
                   // fired in a minute.
    ...
    L"%ls is gearriveerd in sector %ls%ls",  // Name/Squad has arrived in sector
                                             // A9.  Order must not change without
                                             // notifying SirTech
    L"Versie",
    L"Leeg SnelBewaarSlot",
    L"Dit slot is gereserveerd voor SnelBewaren tijdens tactische en "
    L"kaartoverzichten m.b.v. ALT+S.",
    L"Geopend",
    L"Gesloten",
    L"Schijfruimte raakt op. Er is slects %ls MB vrij en Jagged Alliance 2 "
    L"heeft %ls MB nodig.",
    L"%ls heeft %ls gevangen.",         //'Merc name' has caught 'item' -- let SirTech
    ...
    L"%ls gegeven aan %ls.",                  //"Item" passed to "merc".  Please try to keep the
                                              // item %ls before the merc %ls, otherwise, must
                                              // notify SirTech.
    L"Geen plaats om %ls aan %ls te geven.",  // pass "item" to "merc".  Same
    ...
    L"3D Cursor AAN.",
    L"Team %d actief.",
    L"Je kunt %ls's dagelijkse salaris van %ls niet betalen",  // first %ls is
                                                               // the mercs name,
                                                               // the seconds is a
                                                               // string
                                                               // containing the
    ...
    L"Een spel is bewaard onder de naam SaveGame99.sav. Indien nodig, hernoem "
    L"het naar SaveGame10 zodat je het kan aanroepen in het Laden-scherm.",
    L"%ls dronk wat %ls",
    L"Een pakket is in Drassen gearriveerd.",
    L"%ls zou moeten arriveren op het aangewezen punt (sector %ls) op dag %d, "
    L"om ongeveer %ls.",
...
static const wchar_t *s_dut_pMilitiaButtonsHelpText[pMilitiaButtonsHelpText_SIZE] = {
    L"Raap op(Klik Rechts)/drop(Klik links) Groene Troepen",  // button help
                                                              // text
                                                              // informing
                                                              // player they
                                                              // can pick up
...
static const wchar_t s_dut_pMapScreenJustStartedHelpText[] =
    L"Ga naar AIM en huur wat huurlingen in ( *Hint* dat kan bij Laptop )";

static const wchar_t s_dut_pAntiHackerString[] =
...
static const wchar_t *s_dut_gzNonPersistantPBIText[gzNonPersistantPBIText_SIZE] = {
    L"Er is een gevecht gaande. Je kan alleen terugtrekken m.b.v. het "
    L"tactische scherm.",
    L"B|etreedt sector om door te gaan met het huidige gevecht.",
    L"Los huidige gevecht |automatisch op.",
    L"Gevecht kan niet automatisch opgelost worden als je de aanvaller "
    L"bent.",
    L"Gevecht kan niet automatisch opgelost worden als je in een "
    L"hinderlaag ligt.",
    L"Gevecht kan niet automatisch opgelost worden als je vecht met "
    L"beesten in de mijnen.",
    L"Gevecht kan niet automatisch opgelost worden als er vijandige "
    L"burgers zijn.",
    L"Gevecht kan niet automatisch opgelost worden als er nog bloodcats "
    L"zijn.",
    L"GEVECHT GAANDE",
...
static const wchar_t *s_dut_pNewNoiseStr[pNewNoiseStr_SIZE] = {
    L"%ls hoort een %ls geluid uit %ls.",         L"%ls hoort een %ls geluid van BEWEGING uit %ls.",
    L"%ls hoort een %ls KRAKEND geluid uit %ls.", L"%ls hoort een %ls SPETTEREND geluid uit %ls.",
    L"%ls hoort een %ls INSLAG uit %ls.",         L"%ls hoort een %ls EXPLOSIE naar %ls.",
    L"%ls hoort een %ls SCHREEUW naar %ls.",      L"%ls hoort een %ls INSLAG naar %ls.",
    L"%ls hoort een %ls INSLAG naar %ls.",        L"%ls hoort een %ls VERSPLINTEREN uit %ls.",
...
static const wchar_t *s_dut_gzCreditNameTitle[gzCreditNameTitle_SIZE] = {
    ...
    L"Strategische Systemen & Programmeur",  // Kris Marnes "Strategic Systems &
                                             // Editor Programmer"
...
static const wchar_t *s_dut_gzCreditNameFunny[gzCreditNameFunny_SIZE] = {
    ...
    L"(de enige verantwoordelijke persoon)",  // Andrew \"Big Cheese Doddle\"
                                              // Emmons
...
static const wchar_t *s_dut_zGioDifConfirmText[zGioDifConfirmText_SIZE] = {
    L"Je hebt de NOVICE-modus geselecteerd. Deze instelling is geschikt voor "
    L"diegenen die Jagged Alliance voor de eerste keer spelen, voor diegenen "
    L"die nog niet zo bekend zijn met strategy games, of voor diegenen die "
    L"kortere gevechten in de game willen hebben.",  // Je keuze beïnvloedt
                                                     // dingen in het hele verloop
                                                     // van de game, dus weet wat
                                                     // je doet. Weet je zeker dat
                                                     // je in de Novice-modus wilt
                                                     // spelen?",
    L"Je hebt de EXPERIENCED-modus geselecteerd. Deze instelling is geschikt "
    L"voor diegenen die al bekend zijn met Jagged Alliance of dergelijke "
    L"games. Je keuze beïnvloedt dingen in het hele verloop van de game, dus "
    L"weet wat je doet. Weet je zeker dat je in de Experienced-modus wilt "
    L"spelen ?",
    L"Je hebt de EXPERT-modus geselecteerd. We hebben je gewaarschuwd. Geef "
    L"ons niet de schuld als je in een kist terugkomt. Je keuze beïnvloedt "
    L"dingen in het hele verloop van de game, dus weet wat je doet. Weet je "
    L"zeker dat je in de Expert-modus wilt spelen?",
};

static const wchar_t *s_dut_gzLateLocalizedString[gzLateLocalizedString_SIZE] = {
    // 1-5
    L"De robot kan de sector niet verlaten als niemand de besturing "
    L"gebruikt.",
    ...
    // the following two strings are combined with the pNewNoise[] strings
    // above to report noises heard above or below the merc
    L"boven",
    L"onder",
    ...
    L"%ls heeft geen eerste hulp nodig, maar échte medische hulp of iets "
    L"dergelijks.",
    ...
    L"Het is nu onveilig om de tijd te versnellen omdat je huurlingen hebt "
    L"in sector %ls.",
    L"Het is nu onveilig om de tijd te versnellen als er huurlingen zijn "
    L"in de mijnen met beesten.",
    ...
    // 39
    L"Geen enkele huurling van je is in staat om te vechten. De militie "
    L"zal zelf tegen de beesten vechten.",
...
static const wchar_t s_dut_str_ceramic_plates_smashed[] =
    L"%ls's ceramic plates have been smashed!";  // TODO translate

static const wchar_t s_dut_str_arrival_rerouted[] =
...
static const wchar_t *s_dut_str_stat_list[str_stat_list_SIZE] = {
    s_dut_str_stat_health,    s_dut_str_stat_agility,      s_dut_str_stat_dexterity,
    s_dut_str_stat_strength,  s_dut_str_stat_leadership,   s_dut_str_stat_wisdom,
    s_dut_str_stat_exp_level, s_dut_str_stat_marksmanship, s_dut_str_stat_mechanical,
    s_dut_str_stat_explosive, s_dut_str_stat_medical};

static const wchar_t *s_dut_str_aim_sort_list[str_aim_sort_list_SIZE] = {
    s_dut_str_aim_sort_price,     s_dut_str_aim_sort_experience, s_dut_str_aim_sort_marksmanship,
    s_dut_str_aim_sort_medical,   s_dut_str_aim_sort_explosives, s_dut_str_aim_sort_mechanical,
    s_dut_str_aim_sort_ascending, s_dut_str_aim_sort_descending,
};
