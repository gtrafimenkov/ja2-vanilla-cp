...
template <SoundID, SoundID, bool>
struct SoundRangeHelper;
template <SoundID first, SoundID last>
struct SoundRangeHelper<first, last, true> {
  operator SoundID() { return static_cast<SoundID>(first + Random(last - first + 1)); }
};

template <SoundID first, SoundID last>
static inline SoundID SoundRange() {
